...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/STE6602B.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*ホーマック伝票データ（共通）
     SELECT   JHTTEGF   ASSIGN    TO    DA-01-VI-JHTTEGL1
...
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
     ...
     03  WK-DEN-F111              PIC  9(08)     VALUE  ZERO.
...
***  出力取引先ＣＤ
 01  WK-TOKCD                     PIC  9(08)   VALUE  883.
...
*
 ...
 01  GYO1                         CHARACTER  TYPE  IS  YB.
     ...
     03  FILLER                   PIC  X(01)     VALUE   SPACE.
     03  HATYUK                   PIC  N(06).
     03  NOUNM2                   PIC  X(15)     VALUE   SPACE.
*
 01  GYO2.
...
*--- << 伝票データ >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      JHTTEGF.
...
*ファイルＯＰＥＮ
     OPEN     INPUT     JHTTEGF  HTOKMS  HTENMS.
     OPEN     I-O       DSPFILE.
     OPEN     OUTPUT    PRTFILE.
*    初期値セット
     MOVE     ZERO                TO   DENNO1.
...
***  発注日
     MOVE     DEN-F111            TO   WK-DEN-F111.
     MOVE     WK-DEN-F111(3:2)    TO   HACDY1 HACDY2.
     MOVE     WK-DEN-F111(5:2)    TO   HACDM1 HACDM2.
     MOVE     WK-DEN-F111(7:2)    TO   HACDD1 HACDD2.
***  伝票番号
     MOVE     DEN-F02             TO   DEN1   DEN2.
***  行番号
     MOVE     DEN-F03             TO   LINE1  LINE2.
***  納品日
     MOVE     DEN-F112            TO   WK-DEN-F112.
     MOVE     WK-DEN-F112(3:2)    TO   NOUDY1 NOUDY2.
     MOVE     WK-DEN-F112(5:2)    TO   NOUDM1 NOUDM2.
     MOVE     WK-DEN-F112(7:2)    TO   NOUDD1 NOUDD2.
***  納品先
     MOVE     WK-TOKCD            TO   TEN-F52.
     MOVE     DEN-F07             TO   TEN-F011.
     PERFORM  HTENMS-READ-SEC.
     IF       HTENMS-INV-FLG = "INV"
              MOVE  ALL "*"       TO   NOUNM1 NOUNM2
     ELSE
              MOVE  TEN-F04       TO   NOUNM1 NOUNM2
     END-IF.
***  取引先ＣＤ
*****MOVE     DEN-F01             TO   TOKCD1 TOKCD2.
     MOVE     880                 TO   TOKCD1 TOKCD2.
***  取引先名
     MOVE     "K.K ｻｶﾀﾉﾀﾈ"        TO   TOKNM1 TOKNM2.
***  商品名１
     MOVE     DEN-F1421           TO   SYONM1-1 SYONM2-1.
***  商品名２
     MOVE     DEN-F1422           TO   SYONM1-2 SYONM2-2.
***  商品ＣＤ
***  MOVE     DEN-F25             TO   WK-SYOCD.
***  MOVE     WK-HEN-SYOCD        TO   JAN1   JAN2.
     MOVE     DEN-F25             TO   JAN1   JAN2.
***  発注数量
     MOVE     DEN-F15             TO   WK-SURYO.
     MOVE     WK-SURYO1           TO   HATYU1-1 HATYU2-1.
     MOVE     WK-SURYO2           TO   HATYU1-2 HATYU2-2.
***  単価（原価単価）
     MOVE     DEN-F172            TO   WK-GENKA.
     MOVE     WK-GENKA1           TO   GENKA1.
     MOVE     WK-GENKA2           TO   GENKA2.
***  金額
     MOVE     DEN-F181            TO   KIN.
***  仕入単位
***  発注区分
*    EVALUATE DEN-A14
*        WHEN  0
...
*    END-IF.
***  部門
     MOVE     DEN-F12             TO   BUMON.
***  単価（売価単価）
     MOVE     DEN-F173            TO   BAIKA.
***  _番
...
*テスト印字項目セット
     MOVE   NC"Ｎ"               TO   HATYUK.
     MOVE   NC"ＮＮ"             TO   TEK-TEKNM1 TEK-TANNM1.
     ...
     MOVE   ALL "*"              TO   TOKNM1 TOKNM2 SYONM1-1
                                      ...
                                      NOUNM1 NOUNM2 TEISEI-LINE
                                      TEK-TEKNM2 TEK-TANNM2.
*明細０行目出力　　　
     WRITE    PRT-REC   FROM      GYO0      AFTER     2.
*明細１行目出力　　　
     WRITE    PRT-REC   FROM      GYO1      AFTER     2.
...
*
     ...
     MOVE     "FTE38031"          TO        DSP-FMT.
*項目設定
***  伝票枚数
     MOVE     WK-MAI              TO        MD01.
***  プログラムＩＤ
     MOVE     "STE6602B"          TO        PGID.
***  日付
...
***  伝票タイプ
     MOVE     "手書納品シール"    TO        DTYPE.
***  取引先名
     MOVE     "ホーマック北海道"  TO        TORINM.
*画面出力
     WRITE    FTE38031.
*
 DSP-WRITE-EXIT.
...
*
     CLOSE    JHTTEGF   PRTFILE   DSPFILE  HTOKMS  HTENMS.
*
 END-EXIT.
     EXIT.
****************************************************************
