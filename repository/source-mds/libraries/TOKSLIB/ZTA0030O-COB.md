...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/ZTA0030O.COB`
...
**** 端末識別
     03  WK-JYOKEN-F04     PIC 9(02) VALUE     ZERO.
**** 品単ＳＰＡＣＥ詰めＷＫ
     ...
     READ     HJYOKEN
         INVALID
              ...
              MOVE  9             TO   END-FLG
                                  GO   TO   INIT-EXIT
         NOT  INVALID
              MOVE  JYOKEN-F04    TO   WK-JYOKEN-F04
     END-READ.
*
...
******************************************************************
 DSP-INIT-SEC           SECTION.
     MOVE     SPACE               TO   ZTA0030.
     MOVE     SPACE               TO   DSP-AREA.
     MOVE    "ZTA0030"            TO   DSP-FMT.
...
*        本社以外の端末の時、他倉庫データはエラー
         IF   ( WK-JYOKEN-F04  NOT  =    01 )    AND
              ( TANA-F04       NOT  =    WK-JYOKEN-F04 )
              IF   ERR-NO         =    ZERO
                   MOVE  6        TO   ERR-NO
              END-IF
              MOVE   "R"          TO   EDIT-OPTION  OF  HI002
     ...
     IF       HI001               =    1
              ...
              READ      ZSOKMS
                 ...
                 NOT  INVALID
                    MOVE   SOK-F02 TO   HO002
              END-READ
              IF   WK-JYOKEN-F04  NOT =     01
                   IF   SOK-F01   NOT =     WK-JYOKEN-F04
                        MOVE   12   TO   ERR-NO
                        MOVE  "R"   TO   EDIT-OPTION  OF  HI003
                        MOVE  "C"   TO   EDIT-CURSOR  OF  HI003
                   END-IF
...
* 登録時存在レコードのチェック（ＺＴＡＮＡＤＴ２により）
     IF       HI001                    =    1
              IF       TB(IX01)        =   ZERO
****************  MOVE  HI003          TO   TAN-F04
****************  MOVE  BI004   (IX01) TO   TAN-F05
****************  MOVE  BI005   (IX01) TO   TAN-F06
****************  MOVE  BI006   (IX01) TO   TAN-F07
****************  MOVE  BI007   (IX01) TO   TAN-F08
****************  MOVE  BI001   (IX01) TO   TAN-F15(1:1)
****************  MOVE  BI002   (IX01) TO   TAN-F15(2:3)
****************  MOVE  BI003   (IX01) TO   TAN-F15(5:2)
****************  READ      ZTANADT2
****************      INVALID
****************            CONTINUE
****************      NOT   INVALID
 ...
 BODY-ATTR-CLEAR        SECTION.
     ...
     PERFORM       VARYING   IX01      FROM      1    BY   1
         ...
         MOVE  " "                TO   EDIT-CURSOR OF BI001 (IX01)
                                       ...
                                       EDIT-CURSOR OF BI014 (IX01)
     END-PERFORM.
*N** MOVE    "BODY"               TO   DSP-GRP.
*N** PERFORM            DSP-WR-SEC.
 BODY-ATTR-EXIT.
     EXIT.
******************************************************************
*             テイル処理
