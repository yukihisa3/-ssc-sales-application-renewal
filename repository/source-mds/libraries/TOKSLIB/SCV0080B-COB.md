...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SCV0080B.COB`
...
*基本スケジュールマスタ
     SELECT  JHMKIHF   ASSIGN    TO        DA-01-VI-JHMKIHL1
                       ...
                       FILE      STATUS    KIH-ST.
*スポット受信スケジュールマスタ
     SELECT  JHMSJSF   ASSIGN    TO        DA-01-VI-JHMSJSL1
                       ORGANIZATION        INDEXED
                       ACCESS    MODE      SEQUENTIAL
                       RECORD    KEY       SJS-F02
                                           SJS-F03
...
*システム日付６桁→８桁変換（サブ日付チェック／変換）
 INIT010.
     ...
     MOVE      LINK-OUT-YMD TO     WK-DATE  WK-START-DATE
                                   WK-END-DATE.
*ＰＧスタートメッセージ
     DISPLAY "***SCV0080B START " WK-YY "/" WK-MM "/" WK-DD
...
*スポット受信スケジュールマスタスタート
     ...
     START     JHMSJSF   KEY  IS   >=   SJS-F02 SJS-F03
               INVALID
               DISPLAY "************************"  UPON CONS
...
*ＰＧスタートメッセージ
     ACCEPT    WK-TIME  FROM     TIME.
     DISPLAY "***SCV0080B END   " WK-YY "/" WK-MM "/" WK-DD
         " " WK-TIME-1(1:2) ":" WK-TIME-1(3:2) "***" UPON CONS.
*
 END-EXIT.
     EXIT.
...
****************************************************************
 JHMSJSF-READ-SEC      SECTION.
     MOVE     "JHMSJSF-READ-SEC"   TO   S-NAME.
*スポット受信スケジュールマスタ読込み
 SJS010.
     READ      JHMSJSF    AT  END
               MOVE     "END"      TO   END-FLG2
               GO                  TO   JHMSJSF-READ-EXIT
     END-READ.
*作成区分チェック
     IF        SJS-F01  =  "2"
               GO                  TO   SJS010
