...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSI0281L.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 支払合計ファイル >>--*
     SELECT   SITGKFF   ASSIGN         DA-01-S-SITGKFF
                        ORGANIZATION   SEQUENTIAL
                        STATUS         SSI-ST.
*----<< 請求合計Ｆ >>--*
     SELECT   SETGKFH   ASSIGN         DA-01-VI-SETGKFH1
...
****************************************************************
 FILE                   SECTION.
*----<< 支払合計ファイル >>--*
 FD  SITGKFF            LABEL RECORD   IS   STANDARD
...
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
 01  PG-ID              PIC  X(08)     VALUE   "SSI0281L".
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HEAD01.
     03  FILLER         CHARACTER TYPE BAIKAKU-1-5.
         ...
         05  FILLER     PIC  X(08)     VALUE  "SSI0281L".
...
*----<< 支払合計ファイル >>--*
 SSI-ERR                SECTION.
     USE AFTER     EXCEPTION PROCEDURE      SITGKFF.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### " PG-ID " SITGKFF  ERROR " SSI-ST " "
...
*店舗コード
     MOVE     SSI-F06                  TO   ME-05  TEN-F011.
*店舗名称
     ...
     END-IF.
*伝票番号
     MOVE     SSI-F08(3:9)             TO   ME-08.
*支払金額
     MOVE     SSI-F15                  TO   WK-KINGAKU.
     IF       SSI-F18  NOT =  ZERO
              MOVE   SSI-F18           TO   WK-KINGAKU
     END-IF.
     IF       SSI-F11  =  6
...
*レコード区分内容，量販伝票_内容，データ内容
     EVALUATE CHK-FLG
         WHEN "1"
              MOVE    WK-KINGAKU            TO   ME-09
              MOVE    ZERO                  TO   ME-091
              MOVE    WK-KINGAKU            TO   ME-092
              MOVE NC"請求データなし"       TO   ME-10
              ADD     1                     TO   CNT-SEI
              MOVE    0                     TO   OUT-FLG
         WHEN "2"
              IF      SEI-F06  =  WK-KINGAKU
                      IF   SSI-F11  =  1 OR 2 OR 3
