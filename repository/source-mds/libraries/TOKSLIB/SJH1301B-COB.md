...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SJH1301B.COB`
...
*振分先マスタ
     SELECT   FURIMS1   ASSIGN    TO        DA-01-VI-FURIMS1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       FUR-F01
...
*    振分先マスタ
******************************************************************
 FD  FURIMS1            LABEL RECORD   IS   STANDARD.
     COPY     HFURIMS   OF        XFDLIB
              JOINING   FUR       PREFIX.
******************************************************************
*    当日スケジュールマスタ
******************************************************************
...
*    ｶｳﾝﾄ
 ...
 01  FUR-INV-FLG             PIC  X(03)     VALUE  SPACE.
...
*システム日付の編集
 ...
 01  WK-ST.
     ...
     03  FUR-STATUS        PIC  X(02).
     03  VLD-STATUS        PIC  X(02).
*
 01  MSG-AREA.
     03  MSG-START.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  ST-PG          PIC   X(08)  VALUE "SJH1301B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH1301B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
 FILEERR-SEC9           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   FURIMS1.
     MOVE      "FURIMS1 "   TO   AB-FILE.
     MOVE      FUR-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
...
*
     ...
     MOVE     SPACE     TO        WK-DO-REC.
     INITIALIZE                   WK-DO-REC.
...
*各量販店領域転送
*    ルート
     MOVE     WK-DH11       TO        FUR-F01.
     PERFORM  FUR-RD-SEC.
     IF       FUR-INV-FLG  =  "INV"
              MOVE   01     TO        HEN-A11
     ELSE
              MOVE FUR-F02  TO        HEN-A11
     END-IF.
*    取引先コード
     MOVE     WK-DH11       TO        HEN-A01.
*    ルート振り分け用
     MOVE     WK-DH11       TO        HEN-A27.
*    伝票_
     MOVE     WK-DH03       TO        HEN-A02.
*    行_
     MOVE     WK-DD03       TO        HEN-A03.
*    ヘッド部門
     MOVE     WK-DH15       TO        HEN-A04.
*    社名
     MOVE  "ﾄｰｸﾎｰﾑｾﾝﾀｰ"     TO        HEN-A05.
*    取引先名称
     MOVE     WK-DH12       TO        HEN-A061.
     MOVE     WK-DH13       TO        HEN-A062.
*    発注日
     MOVE     WK-DH09       TO        HEN-A07.
*    納品日
     MOVE     WK-DH10       TO        HEN-A08.
*    便区分
     MOVE     WK-DH18       TO        HEN-A09.
*    伝票区分
     MOVE     WK-DH08       TO        HEN-A13.
*    商品コード
     MOVE     WK-DD11       TO        HEN-A14.
*    商品名
     MOVE     WK-DD12       TO        HEN-A151.
     MOVE     WK-DD13       TO        HEN-A152.
*    ＪＡＮコード
     MOVE     WK-DD04       TO        HEN-A16.
*    発注入数
     MOVE     WK-DD05       TO        HEN-A17.
*    発注数
     MOVE     WK-DD08       TO        HEN-A18.
*    原価単価（下２桁小数点扱いの為，１００で除算）
     COMPUTE  HEN-A19  =  WK-DD09   /   100.
*    原価金額
     COMPUTE  HEN-A20  =  HEN-A19  *   WK-DD08.
*    売価単価
     MOVE     WK-DD10       TO        HEN-A21.
*    売価金額
     COMPUTE  HEN-A22  =  WK-DD10   *   WK-DD08.
*    店舗コード
     MOVE     WK-DH06       TO        HEN-A23.
*    店舗名称
     MOVE     WK-DH14       TO        HEN-A25.
*    単位コード
     MOVE     WK-DD07       TO        HEN-A26.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
****************************************************************
 FUR-RD-SEC   SECTION.
*
     MOVE    "FUR-RD-SEC"    TO   S-NAME.
     READ     FURIMS1    INVALID
              MOVE   "INV"   TO   FUR-INV-FLG
              NOT  INVALID
              MOVE   SPACE   TO   FUR-INV-FLG
     END-READ.
*
 FUR-RD-EXIT.
     EXIT.
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名（商品名）
         MOVE    HEN-A151(1:15)  TO  HEN-F1421
         MOVE    HEN-A151(16:10) TO  HEN-F1422(1:10)
*        商品名（規格）
         MOVE    HEN-A152(1:5)   TO  HEN-F1422(11:5)
       NOT INVALID
*        商品名（商品名）
         MOVE    HEN-A151(1:15)  TO  HEN-F1421
         MOVE    HEN-A151(16:10) TO  HEN-F1422(1:10)
...
*売価金額
     MOVE        HEN-A22   TO        HEN-F182.
*店舗名（備考）
*****MOVE        HEN-A25   TO        HEN-F22.
*自社得意先コード
     IF  FLG-TOK   =    ZERO
*    得意先マスタ検索
...
*    ルート条件マスタ検索
     ...
     MOVE     HEN-A11      TO        RUT-F03  HEN-F42.
     READ     JHMRUTL1
         INVALID
           MOVE  1         TO        INV-RUT
...
*
     CLOSE     CVCSG001  JHSHENL1  FURIMS1
               SHOTBL1   MEIMS1
               JHMRUTL1  TOKMS2
               JHMKENL1  JHMTJSL1.
*    ＶＬＤＦ出力処理
