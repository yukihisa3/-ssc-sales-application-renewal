...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY4002L.COB`
...
*    顧客名　　　　　　　：　サカタのタネ（株）殿　　　　　　　*
*    業務名　　　　　　　：　販売管理システム　　　　　　　　　*
*    モジュール名　　　　：　量販店チェックリスト（セリ）　　　*
*    作成日／作成者　　　：　2000/02/28 TAKAHASHI              *
*    　　　　　　　　　　：　画面日付，時刻表示　　　　　　　　*
*                        ：　日付変換サブルーチン使用　　　　　*
*    再利用ＰＧ　　　　　：                                    *
*    更新日／更新者　　　：　                                  *
...
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         21-GS-DSPF
                        ...
                        STATUS         DSP-ST.
*----<< 量販データ >>--*
     SELECT   HSERJNL   ASSIGN         DA-01-VI-HSERJNL1
...
*----<< 表示ファイル >>--*
 FD  DSPFILE            LABEL     RECORD   IS   STANDARD.
     COPY     FSY40021  OF        XMDLIB.
*----<< 量販データ >>--*
 FD  HSERJNL            LABEL     RECORD   IS   STANDARD.
...
*
*----<< ﾌｱｲﾙ ｽﾃｰﾀｽ >>--*
 01  HSERJNL-ST        PIC  X(02).
 01  HTOKMS-ST         PIC  X(02).
 01  HSHOTBL-ST        PIC  X(02).
*
*----<< ﾗﾍﾞﾙﾏｲｽｳｹｲｻﾝﾜｰｸ  >>--*
 01  RABERU-WORK.
     03  WK-RABERU           PIC  9(04)  VALUE  ZERO.
...
*----<< ｲﾘｽｳﾜｰｸ >>--*
 01  WORK-IRISU.
     03  WK-IRISU       PIC  9(04)  VALUE  ZERO.
*
*----<< ｲﾘｽｳﾜｰｸ >>--*
 01  WORK-JANCD.
     03  WK-SER-F121    PIC  X(13)  VALUE  SPACE.
*
*----<< BREAK KEY >>--*
 01  BREAK-KEY.
     03  NEW.
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 ...
 01  HEAD03.
     03  FILLER         CHARACTER TYPE MODE-2.
         ...
         05  FILLER     PIC  X(06)     VALUE  SPACE.
         05  FILLER     PIC  N(04)     VALUE  NC"原単一割".
         05  FILLER     PIC  X(05)     VALUE  SPACE.
...
*********05  FILLER     PIC  X(01)     VALUE  SPACE.
 ...
 01  HEAD031.
     03  FILLER         CHARACTER TYPE MODE-2.
         05  FILLER     PIC  X(01)     VALUE  SPACE.
         05  FILLER     PIC  N(07)     VALUE NC"ユポラベル名称".
 01  HEAD04.
     03  FILLER         CHARACTER TYPE MODE-2.
         05  FILLER     PIC  X(01)     VALUE  SPACE.
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSY4002L DSPFILE ERROR " DSP-CNTL " "
              WK-Y "." WK-M "." WK-D " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HSERJNL   HTOKMS    HSHOTBL   DSPFILE.
     STOP     RUN.
*----<< 量販データ >>--*
 HSERJNL-ERR            SECTION.
     ...
     ACCEPT   WK-DATE        FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY4002L HSERJNL ERROR " HSERJNL-ST " "
              WK-Y "." WK-M "." WK-D " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HSERJNL   HTOKMS    HSHOTBL   DSPFILE.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   WK-DATE        FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY4002L HTOKMS ERROR " HTOKMS-ST " "
              WK-Y "." WK-M "." WK-D " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HSERJNL   HTOKMS    HSHOTBL   DSPFILE.
...
*----<< 商品変換テーブル >>--*
 HSHOTBL-ERR            SECTION.
     ...
     ACCEPT   WK-DATE        FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY4002L HSHOTBL ERROR " HSHOTBL-ST " "
              WK-Y "." WK-M "." WK-D " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HSERJNL   HTOKMS    HSHOTBL   DSPFILE.
...
*
     MOVE     ST-MEMO-X      TO   SER-F01.
     MOVE     ZERO           TO   SER-F02.
     PERFORM  900-SER-START-READ.
...
*--------------------------------------------------------------*
 241-LIST-PRINT         SECTION.
     ...
     AND      NEW  NOT  =    OLD
              ...
              ADD       1    TO   IX
*             ラベル枚数
              MOVE      "*ﾗﾍﾞﾙｽｳ*"     TO   ME-27X(IX)
              DIVIDE   KEI-SUU        BY        WK-IRISU
                                      GIVING    WK-SHO
                                      REMAINDER WK-AMARI
*********DISPLAY "WK-SHO1 = " WK-SHO UPON CONS
*********DISPLAY "SYOCD   = " WK-SER-F121 UPON CONS
              IF       WK-SER-F121(5:3)  =  "505"
                       COMPUTE  WK-RABERU  =  WK-SHO  *  2
                       IF       WK-AMARI NOT = ZERO
                          COMPUTE  WK-RABERU  =  WK-RABERU +  2
                       END-IF
                       MOVE  WK-RABERU    TO    ME-28(IX)
*********DISPLAY "WK-SHO2 = " WK-SHO UPON CONS
              ELSE
                       MOVE  WK-SHO   TO        ME-28(IX)
*********DISPLAY "WK-SHO3 = " WK-SHO UPON CONS
              END-IF
              INITIALIZE          GOKEI-AREA
              MOVE      1  TO   SEN-FLG
              PERFORM   2412-MEIS04-PRINT
...
*
     ...
     AND      NEW  NOT       =    OLD
              ...
              ADD       1              TO   IX
              MOVE      "ｾﾘ NO."       TO   ME-27X(IX)
              MOVE      SER-F01        TO   ME-281(IX)
              MOVE      SER-F02        TO   ME-282(IX)
     END-IF.
*
     MOVE     NEW            TO   OLD.
     MOVE     SER-F66        TO   WK-IRISU.
     MOVE     SER-F121       TO   WK-SER-F121.
     IF       NEW  NOT  =    HIGH-VALUE
...
*転送（ＭＥＩＳ０１）
     ...
     MOVE     SER-F11        TO   ME-03.
     MOVE     SER-F11        TO   TOK-F01.
     PERFORM  900-TOK-READ.
     MOVE     TOK-F03        TO   ME-04.
     MOVE     SER-F04        TO   ME-06.
     MOVE     SER-F05        TO   ME-07.
     MOVE     SER-F63        TO   ME-08.
     MOVE     SER-F071       TO   ME-09.
     MOVE     SER-F072       TO   ME-10.
     MOVE     SER-F08        TO   ME-11.
     MOVE     SER-F09        TO   ME-12.
     MOVE     SER-F10        TO   ME-13.
     MOVE     SER-F641       TO   YUPNM1.
     MOVE     SER-F642       TO   YUPNM2.
     MOVE     SER-F643       TO   YUPNM3.
*\\  93.06.30 START  \\
*****MOVE     SER-F572       TO   ME-14.
     MOVE     SER-F60        TO   ME-14.
*\\  93.06.30 END    \\
*転送（ＭＥＩＳ０２）
     MOVE     SPACE          TO   MEIS02.
     MOVE     SER-F121       TO   ME-15.
     MOVE     SER-F1211      TO   ME-16.
     MOVE     SER-F1212      TO   ME-17.
**   MOVE     SER-F11        TO   SHO-F01.
**   MOVE     SER-F121       TO   SHO-F02.
**   PERFORM  900-SHO-READ.
**   MOVE     SHO-F031       TO   ME-18.
**   MOVE     SHO-F032       TO   WK-032.
     MOVE     SER-F621       TO   ME-18.
     MOVE     SER-F622       TO   WK-032.
     MOVE     WK-0321        TO   ME-19.
     MOVE     WK-0322        TO   ME-20.
     MOVE     WK-0323        TO   ME-21.
     MOVE     SER-F131       TO   ME-141.
     MOVE     SER-F132       TO   ME-23.
     MOVE     SER-F141       TO   ME-24.
     MOVE     SER-F142       TO   ME-25.
     MOVE     SER-F15        TO   ME-26.
     MOVE     SER-F17        TO   RUTOCD.
     MOVE     SER-F65        TO   ME-22.
     MOVE     SER-F66        TO   ME-142.
...
*印刷
       ...
       WRITE    PRT-REC        FROM MEIS021   AFTER     1
...
*****IF       SER-F162 (I)   NOT  =    ZERO
     IF       SER-F162 (I)   NOT  =    ZERO
     AND      SER-F163 (I)   NOT  =    ZERO
              ADD       1    TO   IX
              MOVE      "("            TO   ME-265L(IX)
              MOVE      SER-F161 (I)   TO   ME-265 (IX)
              MOVE      ")"            TO   ME-265R(IX)
              MOVE      "("            TO   ME-27L(IX)
              MOVE      SER-F162 (I)   TO   ME-27 (IX)
              MOVE      ")"            TO   ME-27R(IX)
              MOVE      SER-F163 (I)   TO   ME-28 (IX)
              ADD       SER-F163 (I)   TO   KEI-SUU
     END-IF.
     IF       IX   =    17
              PERFORM   2412-MEIS04-PRINT
     END-IF.
...
*
     ...
     WRITE    PRT-REC   FROM      HEAD031   AFTER     1.
...
*--------------------------------------------------------------*
 900-SER-START-READ     SECTION.
     START    HSERJNL   KEY  >=   SER-F01  SER-F02
              INVALID   KEY
...
*
     IF       NEW       NOT  =    HIGH-VALUE
              MOVE      SER-F01        TO   NEW-MEMO-1
              MOVE      SER-F02        TO   NEW-MEMO-2
**************MOVE      SER-F66        TO   WK-IRISU
**************MOVE      SER-F121       TO   WK-SER-F121
     END-IF.
 900-SER-READ-EXIT.
