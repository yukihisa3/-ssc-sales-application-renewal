...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SJH0100I.COB`
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG12.
         05  FILLER              PIC   N(20)
             VALUE NC"月次区分を入力してください".
     03  ERR-MSG13.
         05  FILLER              PIC   N(20)
             VALUE NC"月次区分に誤りがあります".
     03  ERR-MSG14.
         05  FILLER              PIC   N(20)
         VALUE NC"０、１５、３０、４５分で入力して下さい".
 01  ERR-MSG-AREA-R      REDEFINES     ERR-MSG-AREA.
     03  ERR-MSG-R   OCCURS  14  PIC   N(20).
*
*ファイルエラーメッセージ
 01  FILE-ERR.
...
*
***  月次区分未入力チェック　　　　
     IF       DSP-GJKBN (X)  =    SPACE
              IF  ERR-FLG =  ZERO
                  MOVE   12      TO   ERR-FLG
              END-IF
              MOVE  "R"      TO   EDIT-OPTION  OF  DSP-GJKBN(X)
              MOVE  "C"      TO   EDIT-CURSOR  OF  DSP-GJKBN(X)
     ELSE
***  月次区分入力値チェック　　　　
        IF       DSP-GJKBN (X)  NOT  =  "1"  AND "9"
                 IF  ERR-FLG =  ZERO
                     MOVE   13      TO   ERR-FLG
                 END-IF
                 MOVE  "R"      TO   EDIT-OPTION  OF  DSP-GJKBN(X)
...
*リバース，カーソルパーク解除
     PERFORM  VARYING X FROM 1 BY 1 UNTIL X > 15
              ...
              MOVE "M"   TO EDIT-OPTION OF DSP-GJKBN (X)
              MOVE SPACE TO EDIT-CURSOR OF DSP-GJKBN (X)
...
*
     PERFORM  VARYING X FROM 1 BY 1 UNTIL X > 15
         ...
         READ      JHMNITF
              INVALID  KEY
                   DISPLAY " *** INVALID ERR *** " UPON CONS
                   STOP RUN
              NOT  INVALID
                   MOVE      DSP-JKKBN(X)   TO      NIT-F02
                   MOVE      DSP-GJKBN(X)   TO      NIT-F10
