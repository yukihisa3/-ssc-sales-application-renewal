...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SZE0100L.COB`
...
****<< 商品在庫マスタファイル >>****************************
     SELECT   ZENZAIF    ASSIGN    TO        DA-01-VI-ZENZAIL1
                        ORGANIZATION        IS   INDEXED
                        ACCESS    MODE      IS   SEQUENTIAL
                        RECORD    KEY       IS   ZAI-F00
                                                 ZAI-F01
                                                 ZAI-F02
                                                 ZAI-F03
...
****  フラグ                  ****
 ...
 01  WK-BUMON                PIC  X(04)  VALUE  SPACE.
...
****  見出し行１             ****
 01  MIDASI-1.
     ...
     02  FILLER              PIC  X(08)  VALUE  "SZE0100L".
...
****  見出し行２             ****
 01  MIDASI-2       CHARACTER     TYPE   IS   NIHONGO.
     ...
     02  FILLER              PIC  N(03)  VALUE
         NC"棚　番".
     02  FILLER              PIC  X(02)  VALUE  SPACE.
     02  FILLER              PIC  N(03)  VALUE  NC"カナ名".
...
*S***02  FILLER              PIC  N(03)  VALUE
*S***    NC"カナ名".
*S***02  FILLER              PIC  X(09)  VALUE  SPACE.
     02  FILLER              PIC  N(04)  VALUE
         NC"商品名称".
...
****  見出し行４             ****
 01  MIDASI-4       CHARACTER     TYPE   IS   NIHONGO.
     ...
     02  FILLER              PIC  N(06)  VALUE
         NC"【部門ＣＤ：".
     02  H-BUMON-CD          PIC  X(04).
     02  FILLER              PIC  X(02)  VALUE  SPACE.
     02  H-BUMON-NM          PIC  N(15).
     02  FILLER              PIC  X(01)  VALUE  SPACE.
...
****  明細行１               ****
 01  MEISAI-1       CHARACTER     TYPE   IS   YB.
     ...
     02  PRT-TANA            PIC  X(08)B(02).
     02  PRT-KANA            PIC  X(15)B(01).
     02  PRT-GEN             PIC  --,---,--9.99.
     02  PRT-TOURI           PIC  --,---,--9.99.
*****02  PRT-TOHEP           PIC  --,---,--9.99.
     02  PRT-TONYK           PIC  --,---,--9.99.
     02  PRT-MINYK           PIC  --,---,--9.99.
     02  PRT-SUMI            PIC  --,---,--9.99.
     02  PRT-JINYK           PIC  --,---,--9.99.
****  明細行２               ****
...
*****02  PRT-KANA            PIC  X(15)B.
     ...
     02  PRT-KANO            PIC  --,---,--9.99.
     02  PRT-JIURI           PIC  --,---,--9.99.
*****02  PRT-JIHEP           PIC  --,---,--9.99.
     02  PRT-TOSYK           PIC  --,---,--9.99.
     02  PRT-MISYK           PIC  --,---,--9.99.
     02  PRT-ZEN             PIC  --,---,--9.99.
     02  PRT-JISYK           PIC  --,---,--9.99.
****  明細行３               ****
...
**** メッセージ情報           ****
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SZE0100L".
...
*      0.0       全体処理                                  *
************************************************************
 SZE0100L-START         SECTION.
     PERFORM       INIT-SEC.
     PERFORM       MAIN-SEC        UNTIL  END-FLG = "END".
     PERFORM       END-SEC.
     IF   PG-END   NOT =  "END"
          MOVE     SPACE   TO   END-FLG
          GO  TO   SZE0100L-START
     END-IF.
     STOP     RUN.
 SZE0100L-END.
     EXIT.
...
************************************************************
 INIT-SEC               SECTION.
     OPEN     I-O       DSPF.
     OPEN     INPUT     ZENZAIF   ZSOKMS   HMEIMS
              HJYOKEN.
     OPEN     OUTPUT    PRINTF.
*
*システム日付・時刻の取得
...
*
     MOVE    "FZE01001" TO     DSP-FORMAT.
     MOVE     SPACE     TO     FZE01001.
     MOVE     SPACE     TO     MSG1.
     MOVE     PMSG01    TO     MSG2.
     MOVE     62        TO     L-CNT.
     MOVE     0         TO     P-CNT.
...
*データ検索
     ...
     START    ZENZAIF    KEY  IS  >=  ZAI-F00
                                      ZAI-F01
                                      ZAI-F02
                                      ZAI-F03
              INVALID      MOVE  "END"    TO  END-FLG
              NOT INVALID  PERFORM  ZENZAIF-READ-SEC
     END-START.
*
 INIT-END.
     EXIT.
...
*部門コード
     MOVE     SPACE           TO   SBUNM.
     IF       SBUCD       NOT =   SPACE
              MOVE   SBUCD    TO   JYO-F02
              PERFORM   JYO-READ-SEC
              IF     INV-FLG   =   ZERO
                     MOVE   JYO-F03     TO SBUNM
              ELSE
                     MOVE   MSG-ERR6    TO MSG1
                     MOVE   "R"         TO EDIT-OPTION OF SBUCD
                     MOVE   "C"         TO EDIT-CURSOR OF SBUCD
              END-IF
     END-IF.
*
     MOVE     SPACE           TO   EBUNM.
     ...
     END-IF.
     IF       EBUCD       NOT =   "9999"
              MOVE   EBUCD         TO   JYO-F02
              PERFORM   JYO-READ-SEC
              IF     INV-FLG   =   ZERO
                     MOVE   JYO-F03     TO EBUNM
              ELSE
                     MOVE   MSG-ERR6    TO MSG1
                     MOVE   "R"         TO EDIT-OPTION OF EBUCD
                     MOVE   "C"         TO EDIT-CURSOR OF EBUCD
              END-IF
     END-IF.
*
     IF       MSG1      =   SPACE      AND
...
*             ゼロのみ
     ...
     END-IF.
*    部門ＣＤがブレイクした場合
     IF       ZAI-F00  NOT =  WK-BUMON
              PERFORM       MIDASI-SEC
              MOVE    ZAI-F00   TO   WK-BUMON
     END-IF.
*    行カウンターが６２以上の場合
     IF       L-CNT         >=      62
              PERFORM       MIDASI-SEC
...
*
     ...
     END-IF.
*    全社
     MOVE  NC"　　全社在庫　　"  TO      H-TOKHAN.
*    部門ＣＤセット
     MOVE     ZAI-F00    TO      H-BUMON-CD.
*    部門名称取得
     MOVE     ZAI-F00    TO      JYO-F02.
     PERFORM  JYO-READ-SEC.
     IF       HJYOKEN-INV-FLG = "INV"
              MOVE  ALL NC"＊"   TO    H-BUMON-NM
     ELSE
              MOVE  JYO-F03      TO    H-BUMON-NM
     END-IF.
*
...
*          在庫マスタ・読み込み
************************************************************
 ZENZAIF-READ-SEC       SECTION.
     READ     ZENZAIF
              AT END    MOVE  "END"  TO  END-FLG
                        GO   TO   ZENZAIF-READ-END
     END-READ.
*部門ＢＲＥＡＫ？
     IF       ZAI-F00   >    EBUCD
              MOVE  "END"    TO   END-FLG
...
************************************************************
 OPT-CLR-SEC        SECTION.
     MOVE  SPACE         TO  EDIT-CURSOR  OF  SBUCD
                             EDIT-CURSOR  OF  EBUCD
                             EDIT-CURSOR  OF  SSOKCD
     ...
     MOVE "M"            TO  EDIT-OPTION  OF  SBUCD
                             EDIT-OPTION  OF  EBUCD
                             EDIT-OPTION  OF  SSOKCD
...
************************************************************
 END-SEC                SECTION.
     CLOSE    DSPF  PRINTF  ZENZAIF  ZSOKMS  HMEIMS
              HJYOKEN.
 END-END.
     EXIT.
*****************<<  PROGRAM  END  >>***********************
