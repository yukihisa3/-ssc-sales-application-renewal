...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKEIHEN.COB`
...
*----<< 条件ファイル >>-*
     SELECT   HJYOKEN   ASSIGN    TO        DA-01-VI-JYOKEN1
                        ...
                        FILE      STATUS    JYO-ST1.
*----<< 伝票データ >>-*
     SELECT   HDENJNL   ASSIGN    TO        DA-01-VI-DENJNL5
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      DYNAMIC
...
*--------------------------------------------------------------*
 01  PGM-ID                  PIC  X(08)     VALUE  "SKEIHEN ".
*----<< ﾃﾞｨｽﾌﾟﾚｲ ｺﾝﾄﾛｰﾙ ｴﾘｱ >>-*
 01  DSP-CNTL.
     03  DSP-FMT             PIC  X(08).
     03  DSP-GRP             PIC  X(08).
...
****                              LINK-OUT-YMD8.
**** IF       LINK-OUT-RET   =    ZERO
****          MOVE      LINK-OUT-YMD8  TO   WK-NOUHIN-DT
**** ELSE
****          MOVE      ZERO           TO   WK-NOUHIN-DT
**** END-IF.
*前月のＡＣＯＳ締日以前のデータは処理できない。
*****DISPLAY "WK-ACOS-DATE = " WK-ACOS-DATE UPON CONS.
*****DISPLAY "WK-NOUHIN-DT = " WK-NOUHIN-DT UPON CONS.
*****IF   WK-ACOS-DATE   >=   WK-NOUHIN-DT
...
*    システム日付８桁変換
     ...
     CALL    "SKYDTCKB" USING     LINK-IN-KBN
                                  ...
                                  LINK-OUT-RET
                                  LINK-OUT-YMD8.
**** DISPLAY "WK-URI-DATE   = " WK-URI-DATE   UPON CONS.
**** DISPLAY "WK-ACOS-DATE  = " WK-ACOS-DATE  UPON CONS.
**** DISPLAY "R00021        = " LINK-OUT-YMD8 UPON CONS.
**** DISPLAY "WK-ACOS-DATE1 = " WK-ACOS-DATE1 UPON CONS.
...
*
     ...
     MOVE     0              TO   DEN-F03.
     MOVE     0              TO   END-FLG.
     START    HDENJNL   KEY  NOT  <  DEN-F01     DEN-F23
                                     DEN-F04     DEN-F051
                                     DEN-F03
         INVALID   KEY
              MOVE      1         TO   END-FLG
...
*--------------------------------------------------------------*
 900-DEN-DELETE         SECTION.
     ...
     MOVE     0              TO   DEN-F03.
     MOVE     0              TO   END-FLG.
     START    HDENJNL  KEY  NOT  <  DEN-F01  DEN-F23  DEN-F04
                                    DEN-F051 DEN-F03
        INVALID  KEY
              MOVE      1        TO   END-FLG
        NOT   INVALID  KEY
...
**** 出荷日 ***
     ...
     END-EVALUATE.
*登録担当者・更新担当者・承認担当者初期値セット
     MOVE  DEN-F06      TO        DEN-F59  DEN-F60  DEN-F61.
*登録日・更新日・承認日初期値セット
     MOVE  WK-SYS-DATE  TO        DEN-F62  DEN-F63  DEN-F64.
*代表権限・権限初期値セット
