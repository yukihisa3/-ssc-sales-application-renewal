...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY1602L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*島忠伝票データ（共通）　　
     SELECT   SCSHIRED  ASSIGN    TO        SCSHIRED
...
***  ﾌﾗｸﾞ ｴﾘｱ
 01  FLG-AREA.
     ...
     03  HD1-CHK-FLG              PIC  X(01)     VALUE  SPACE.
***  ｶｳﾝﾄ
 01  CNT-AREA.
...
***  ﾜｰｸ ｴﾘｱ
 ...
 01  SEC-NAME.
     ...
     03  S-NAME                   PIC  X(30).
***  担当者格納
 01  WK-DEN-A29                   PIC  N(03)     VALUE  SPACE.
***  ﾃﾞﾝﾋﾟﾖｳ ｷ-
 01  WRK-DENNO.
     03  DENNO1                   PIC  9(09).
***  ﾒｯｾｰｼﾞｴﾘｱ
...
*
 01  HD11.
     ...
     03  HD11-021                 PIC  N(04)
                                  CHARACTER  TYPE  IS  YB.
     03  HD11-022                 PIC  Z9.
     03  HD11-023                 PIC  X.
     03  HD11-024                 PIC  Z9.
     03  HD11-025                 PIC  N(08)
                                  CHARACTER  TYPE  IS  YB.
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      SCSHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     SCSHIRED AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    島忠以外は読み飛ばし（取引先）
     IF       DEN-F01  NOT =  72028
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
...
*明細１行目
***  商品名
     MOVE     DEN-A23             TO   DT1-011.
     MOVE     DEN-A24             TO   DT1-012.
***  訂正伝票時処理
...
*明細２行目
***  規格
     MOVE     DEN-A25             TO        DT2-01.
***  ＪＡＮコード
     MOVE     DEN-A19             TO        DT2-02.
***  単位
     MOVE     DEN-A26             TO        DT2-03.
***  数量
...
*ヘッダクリア
     MOVE     SPACE               TO   HD11  HD12  HD2  HD3.
*
*ヘッダ１行目　
***  Ｃ欄
     MOVE     "10"                TO    HD11-01  HD12-01.
***  Ｄ欄
     MOVE     SPACE               TO    HD1-CHK-FLG.
     IF       DEN-A16   NOT =  ZERO
              MOVE     NC"＊＊＊　"    TO   HD11-021
              MOVE     DEN-A16(1:2)    TO   HD11-022
              MOVE     "."             TO   HD11-023
              MOVE     DEN-A16(3:2)    TO   HD11-024
              MOVE     NC"　チラシ　＊＊＊"
                                       TO   HD11-025
              MOVE     "A"             TO   HD1-CHK-FLG
     END-IF.
***  Ｄ欄
     IF       DEN-A28   =  "1"
              MOVE     NC"＊＊＊　お客様注文品　＊＊＊"
                                       TO   HD12-02
              MOVE     "B"             TO   HD1-CHK-FLG
     END-IF.
*
*ヘッダ２行目　
***  社名
     MOVE     NC"島忠"            TO    HD2-01.
***  仕入先コード１
     MOVE     DEN-A05             TO    HD2-02.
***  担当営業所区分
     MOVE     "("                 TO    HD2-03.
     MOVE     DEN-A17             TO    HD2-04.
     MOVE     ")"                 TO    HD2-05.
*
*ヘッダ　３　行目　
***  店名
...
***  事業部＋店ＣＤ
     MOVE     DEN-A02             TO    HD3-02.
     MOVE     DEN-A03             TO    HD3-03.
***  部門
     MOVE     DEN-A04             TO    HD3-04.
***  伝票_
     MOVE     DEN-F02             TO    HD3-05.
***  仕入先ＣＤ
     MOVE     DEN-A06             TO    HD3-06.
***  取引先名（下段）
     MOVE     NC"（株）サカタのタネ　" TO    HD3-07.
***  発注日
     MOVE     DEN-A13(3:2)        TO    HD3-081.
     MOVE     DEN-A13(5:2)        TO    HD3-082.
     MOVE     DEN-A13(7:2)        TO    HD3-083.
***  納品日
     MOVE     DEN-A14(3:2)        TO    HD3-091.
     MOVE     DEN-A14(5:2)        TO    HD3-092.
     MOVE     DEN-A14(7:2)        TO    HD3-093.
*
*訂正フラグチェック(TL-WRITE-SECで使用）
     IF       DEN-F53 = 1
              MOVE    NC"○"      TO    HD11-03  HD12-03
     END-IF.
*
*ヘッダ部出力
     EVALUATE   HD1-CHK-FLG
         WHEN   SPACE
                WRITE PRT-REC FROM HD11 AFTER 2
         WHEN   "A"
                WRITE PRT-REC FROM HD11 AFTER 2
         WHEN   "B"
                WRITE PRT-REC FROM HD12 AFTER 2
     END-EVALUATE.
...
*項目クリア
     MOVE     SPACE               TO   HD11 HD12 HD2 HD3
                                       ...
                                       TL1.
*テスト印字項目セット
     MOVE   ALL "9"              TO   HD12-01 HD3-03  HD3-05
                                      HD3-06  HD3-081 HD3-082
                                      HD3-083 HD3-091 HD3-092
                                      HD3-093 DT2-041 DT2-042
                                      DT2-051 DT2-052 DT2-061
                                      DT2-062 DT2-07  DT2-08
                                      DT2-09  TL1-011 TL1-012
                                      TL1-021 TL1-022 TL1-03
                                      TL1-04.
     MOVE   ALL "*"              TO   HD2-02  HD2-04  HD2-03
                                      HD2-05  HD3-02  DT2-02.
     MOVE   ALL NC"＊"           TO   HD12-02 HD12-03 HD2-01
                                      HD3-01  HD3-07  DT1-011
                                      DT1-012 DT2-01  DT2-03
                                      TL2-01.
*ヘッダ部
*****WRITE    PRT-REC          FROM   HD12       AFTER    3.
     WRITE    PRT-REC          FROM   HD12       AFTER    2.
     WRITE    PRT-REC          FROM   HD2        AFTER    3.
     WRITE    PRT-REC          FROM   HD3        AFTER    2.
     MOVE     SPACE            TO     PRT-REC.
...
*
     CLOSE    SCSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
