...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKY1004B.COB`
...
*    サブシステム　　　　：　共通システム                      *
*    業務名　　　　　　　：　手書伝票発行　　　　　　　        *
*    モジュール名　　　　：　共通手書伝票発行（タイプ用_型）  *
*    作成日／作成者　　　：　99/09/30  /HAGIWARA               *
*    更新日／更新者　　　：　                                  *
*    更新内容　　　　　　：                                    *
...
***  ﾍﾝｼｭｳｴﾘｱ
 01  HENSHU-AREA.
     03  HENKAN-DATE              PIC  9(08)     VALUE   ZERO.
...
*    帳票初期化
     MOVE     SPACE              TO   FKY10042.
*    ワーク初期化
     MOVE     ZERO               TO   DENNO1.
     MOVE     ZERO               TO   CNT-DENPYO.
     MOVE     SPACE              TO   END-FLG.
     MOVE     SPACE              TO   FKY10041.
*
 INIT-EXIT.
     EXIT.
****************************************************************
...
*
     ADD      1         TO        I.
*****商品名
     MOVE     DEN-F1421           TO   DM01(I).
     MOVE     DEN-F1422           TO   DM02(I).
*****商品コード／ＪＡＮコード
     MOVE     DEN-F25             TO   DM03(I).
*****色／入数
*****サイズ／ケース
*****単位
*****数量
     MOVE     DEN-F15             TO   HENKAN-SURYO.
     MOVE     HENKAN-SURYO(1:9)   TO   DM07(I).
     MOVE     HENKAN-SURYO(10:1)  TO   DM08(I).
     ADD      HENKAN-SURYO        TO   G-SURYO.
*****原価単価（パックタイプ変換）
     MOVE     DEN-F172            TO   HENKAN-GENKA.
...
****************************************************************
 HEAD-WT-SEC            SECTION.
*****社名
     MOVE     WK-TOKKN            TO   HD01.
*****店舗名称
     MOVE     DEN-F30             TO   HD02.
     MOVE     SPACE               TO   SEN.
*2013/05/22 NAV ST 下記、条件にドイトを追加
*2017/12/18 NAV ST 下記、条件にイオン九州を追加
*****グリーン_対応
*****IF       DEN-F01   =  7
*****IF       DEN-F01   =  7  OR 10545
     IF       DEN-F01   =  7  OR 10545 OR 33189
*2013/05/22 NAV ED 下記、条件にドイトを追加
*2017/12/18 NAV ED 下記、条件にイオン九州を追加
              GO                  TO   HEAD010
     END-IF.
*****法人コード
     IF       DEN-F01  NOT =  1225
...
*    バロー対応
     IF       DEN-F01  =  13020
              MOVE  DEN-F07(1:2)  TO   HOJIN
              MOVE     "-"        TO   SEN
     END-IF.
 HEAD010.
*****店舗コード
     MOVE     DEN-F07             TO   HD03.
*****分類コード
     MOVE     DEN-F12             TO   HD04.
*****伝票区分
*****MOVE     DEN-F051            TO   HD05.
     MOVE     DEN-F132            TO   HD05.
*****伝票番号
     MOVE     DEN-F02             TO   HD06.
*****取引先コード
     MOVE     DEN-F01             TO   HD08.
*#2021/03/05 NAV ST 取引先ＣＤ変換
     EVALUATE DEN-F01
         WHEN   87371    MOVE  8737      TO   HD08
         WHEN   87372    MOVE  8737      TO   HD08
         WHEN   87373    MOVE  8737      TO   HD08
         WHEN   87374    MOVE  8737      TO   HD08
         WHEN   87375    MOVE  8737      TO   HD08
         WHEN   87376    MOVE  8737      TO   HD08
         WHEN   883      MOVE  880       TO   HD08
         WHEN   882      MOVE  880       TO   HD08
         WHEN   14272    MOVE  1427      TO   HD08
         WHEN   14273    MOVE  1427      TO   HD08
         WHEN   100442   MOVE  100441    TO   HD08
         WHEN   100428   MOVE  100427    TO   HD08
         WHEN   139381   MOVE  13938     TO   HD08
         WHEN   171371   MOVE  17137     TO   HD08
         WHEN   1731     MOVE  173       TO   HD08
         WHEN   1732     MOVE  173       TO   HD08
         WHEN   7601     MOVE  760       TO   HD08
         WHEN   7602     MOVE  760       TO   HD08
         WHEN   12251    MOVE  1225      TO   HD08
         WHEN   12252    MOVE  1225      TO   HD08
         WHEN   OTHER    MOVE  DEN-F01   TO   HD08
     END-EVALUATE.
*#2021/03/05 NAV ED 取引先ＣＤ変換
*****発注日（パックタイプ変換）
     MOVE     DEN-F112            TO   HENKAN-DATE.
     MOVE     HENKAN-DATE(3:6)    TO   HD11.
*****納品日
*****便
*
 HEAD-WT-EXIT.
...
*    END-IF.
*    ファイルのＣＬＯＳＥ
     CLOSE    SHTDENF   DSPFILE   HTOKMS   PRTF.
*
 END-EXIT.
     EXIT.
******************************************************************
 END PROGRAM  SKY1004B.
******************************************************************
