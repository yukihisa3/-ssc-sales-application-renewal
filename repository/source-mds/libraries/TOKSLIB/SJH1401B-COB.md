...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SJH1401B.COB`
...
*    ｶｳﾝﾄ
 ...
 01  CAN-FLG                 PIC  9(01)     VALUE  ZERO.
*
*データ情報退避ワーク
 01  WK-DENB.
     03  WK-DENB1.
         05  WK-DENB01       PIC  X(01).
     03  WK-DENB2.
         05  WK-DENB02       PIC  X(02).
         05  WK-DENB03       PIC  X(01).
         05  WK-DENB04       PIC  9(09).
     03  WK-DENB3.
         05  WK-DENB05       PIC  X(03).
     03  WK-DENB4.
         05  WK-DENB06       PIC  9(05).
         05  WK-DENB07       PIC  9(03).
         05  WK-DENB08       PIC  9(01).
         05  WK-DENB09       PIC  9(02).
         05  WK-DENB10       PIC  9(06).
         05  WK-DENB11       PIC  9(06).
         05  WK-DENB12       PIC  9(06).
     03  WK-DENB5.
         05  WK-DENB13       PIC  X(05).
     03  WK-DENB6.
         05  WK-DENB14       PIC  X(11).
         05  WK-DENB15       PIC  9(03).
         05  WK-DENB16       PIC  X(09).
         05  WK-DENB17       PIC  9(06).
         05  WK-DENB18       PIC  X(04).
         05  WK-DENB19       PIC  X(03).
     03  WK-DENB191          PIC  X(02).
     03  WK-DENB7.
         05  WK-DENB20       PIC  X(40).
*
*    ヘッドレコード退避ワーク
 01  WK-DENC.
     03  WK-DENC1.
         05  WK-DENC01       PIC  X(01).
         05  WK-DENC02       PIC  X(02).
         05  WK-DENC03       PIC  X(20).
     03  WK-DENC2.
         05  WK-DENC04       PIC  X(30).
         05  WK-DENC05       PIC  X(30).
     03  WK-DENC3.
         05  WK-DENC06       PIC S9(09).
         05  WK-DENC07       PIC S9(09).
     03  WK-DENC4.
         05  WK-DENC08       PIC  X(26).
         05  WK-DENC09       PIC  X(01).
*
*    明細レコード退避ワーク
 01  WK-DEND.
     03  WK-DEND1.
         05  WK-DEND01       PIC  X(01).
         05  WK-DEND02       PIC  X(02).
     03  WK-DEND2.
         05  WK-DEND03       PIC  9(02).
         05  WK-DEND04       PIC  X(13).
         05  WK-DEND05       PIC  9(01).
     03  WK-DEND3.
         05  WK-DEND06       PIC  X(09).
     03  WK-DEND4.
         05  WK-DEND07       PIC S9(05)V9.
         05  WK-DEND08       PIC S9(07)V99.
         05  WK-DEND09       PIC S9(07).
         05  WK-DEND10       PIC S9(10).
         05  WK-DEND11       PIC S9(10).
     03  WK-DEND5.
         05  WK-DEND12       PIC  X(09).
     03  WK-DEND6.
         05  WK-DEND13.
             07  WK-DEND131  PIC  X(20).
             07  WK-DEND132  PIC  X(20).
     03  WK-DEND7.
         05  WK-DEND14       PIC  X(09).
*
 01  WK-AREA.
*システム日付の編集
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJH1401B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH1401B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
     ...
     MOVE     SPACE     TO        WK-DENB.
     INITIALIZE                   WK-DENB.
     MOVE     SPACE     TO        WK-DENC.
     INITIALIZE                   WK-DENC.
     MOVE     SPACE     TO        WK-DEND.
     INITIALIZE                   WK-DEND.
*
******************
*システム日付編集*
...
*データ情報処理
        IF    DEN-01A(IDX)   =   "B"
              IF   DEN-01C(IDX)   =   "2"
                   MOVE      ZERO          TO   CAN-FLG
                   MOVE      SPACE         TO   WK-DENB
                   INITIALIZE                   WK-DENB
                   MOVE      DEN-01(IDX)   TO   WK-DENB
              ELSE
                   MOVE      1             TO   CAN-FLG
              END-IF
        END-IF
...
*各量販店領域転送
*    データ種別
     MOVE     WK-DENB02     TO        HEN-A01.
*    伝票種別
     MOVE     WK-DENB03     TO        HEN-A02.
*    伝票番号
     MOVE     WK-DENB04     TO        HEN-A03.
*    店舗コード
     MOVE     WK-DENB06     TO        HEN-A04.
*    部門コード
     MOVE     WK-DENB07     TO        HEN-A05.
*    商品区分
     MOVE     WK-DENB08     TO        HEN-A06.
*    伝票区分
     MOVE     WK-DENB09     TO        HEN-A07.
*    発注日
     MOVE     WK-DENB10     TO        HEN-A08.
*    納品日
     MOVE     WK-DENB11     TO        HEN-A09.
*    取引先コード
     MOVE     WK-DENB12     TO        HEN-A10.
*    特売区分名
     MOVE     WK-DENB14     TO        HEN-A11.
*    オーダー番号
     MOVE     WK-DENB15     TO        HEN-A12.
*    備考（Ｆ）
     MOVE     WK-DENB16     TO        HEN-A13.
*    有効期限日
     MOVE     WK-DENB17     TO        HEN-A14.
*    センター表示
     MOVE     WK-DENB18     TO        HEN-A15.
*    ＤＭＣ表示
     MOVE     WK-DENB19     TO        HEN-A16.
*    店舗名
     MOVE     WK-DENC04     TO        HEN-A17.
*    取引先名
     MOVE     WK-DENC05     TO        HEN-A18.
*    レコード区分
     MOVE     WK-DEND01     TO        HEN-A19.
*    伝票行番号
     MOVE     WK-DEND03     TO        HEN-A20.
*    商品コード
     MOVE     WK-DEND04     TO        HEN-A21.
*    重点管理区分
     MOVE     WK-DEND05     TO        HEN-A22.
*    数量
     MOVE     WK-DEND07     TO        HEN-A23.
*    原単価
     MOVE     WK-DEND08     TO        HEN-A24.
*    売単価
     MOVE     WK-DEND09     TO        HEN-A25.
*    原価金額
     MOVE     WK-DEND10     TO        HEN-A26.
*    売価金額
     MOVE     WK-DEND11     TO        HEN-A27.
*    品名（上段）
     MOVE     WK-DEND131    TO        HEN-A281.
*    品名（下段）
     MOVE     WK-DEND132    TO        HEN-A282.
*    原価金額計
     MOVE     WK-DENC06     TO        HEN-A29.
*    売価金額計
     MOVE     WK-DENC07     TO        HEN-A30.
*    ルート
     IF       WK-DENB191  NOT  NUMERIC
              MOVE     99            TO        HEN-A31
     ELSE
              MOVE     WK-DENB191    TO        HEN-A31
     END-IF.
*
     PERFORM  TENSO-SEC.
*
...
*担当者コード
     MOVE        99        TO        HEN-F06.
*店コード
     MOVE        HEN-A04   TO        HEN-F07.
*  商品変換テーブル検索
     MOVE        SPACE     TO        TBL-REC.
     INITIALIZE                      TBL-REC.
     MOVE        HEN-A10   TO        TBL-F01.
...
*****COMPUTE     HEN-F111  = WK-NEN * 10000 + HEN-A08.
     MOVE       "3"        TO        LINK-IN-KBN.
...
*****COMPUTE     HEN-F112  = WK-NEN * 10000 + HEN-A09.
*納品日＝０の為，有効期限を転送
*****COMPUTE     HEN-F112  = WK-NEN * 10000 + HEN-A14.
     MOVE       "3"        TO        LINK-IN-KBN.
     MOVE        HEN-A14   TO        LINK-IN-YMD6.
     CALL       "SKYDTCKB"   USING   LINK-IN-KBN
                                     LINK-IN-YMD6
                                     LINK-IN-YMD8
                                     LINK-OUT-RET
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名
         MOVE    HEN-A281(1:15)  TO   HEN-F1421
         MOVE    HEN-A281(16:5)  TO   HEN-F1422(1:5)
         MOVE    HEN-A282(1:10)  TO   HEN-F1422(6:10)
       NOT INVALID
         MOVE    HEN-A281(1:15)  TO   HEN-F1421
         MOVE    HEN-A281(16:5)  TO   HEN-F1422(1:5)
         MOVE    HEN-A282(1:10)  TO   HEN-F1422(6:10)
     END-READ.
*数量
     MOVE        HEN-A23   TO        HEN-F15.
*単
     MOVE       "1"        TO        HEN-F16.
*原価単価
     MOVE        HEN-A24   TO        HEN-F172.
*売価単価
     MOVE        HEN-A25   TO        HEN-F173.
*原価金額
     MOVE        HEN-A26   TO        HEN-F181.
*売価金額
     MOVE        HEN-A27   TO        HEN-F182.
*自社得意先コード
     IF  FLG-TOK   =    ZERO
*    得意先マスタ検索
         MOVE    SPACE         TO    TOK-REC
         INITIALIZE                  TOK-REC
         MOVE    HEN-A10       TO    TOK-F01
         READ    TOKMS2
             INVALID
               MOVE  SPACE     TO    TOK-REC
               INITIALIZE            TOK-REC
...
*    ルート条件マスタ検索
     ...
     MOVE     HEN-A10      TO        RUT-F01.
     MOVE     SPACE        TO        RUT-F02.
     MOVE     HEN-A31      TO        RUT-F03  HEN-F42.
     READ     JHMRUTL1
         INVALID
           MOVE  1         TO        INV-RUT
...
*訂正前数量
     MOVE        HEN-A23   TO        HEN-F50.
*修正原価単価
     MOVE        HEN-A24   TO        HEN-F512.
*修正売価単価
     MOVE        HEN-A25   TO        HEN-F513.
*修正原価金額
     MOVE        HEN-A26   TO        HEN-F521.
*修正売価金額
     MOVE        HEN-A27   TO        HEN-F522.
*
 TENSO-EXIT.
     EXIT.
****************************************************************
