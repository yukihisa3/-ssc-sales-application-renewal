...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY4401L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*バロー伝票データ（共通）
     SELECT   BRSHIRED  ASSIGN    TO        BRSHIRED
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      BRSHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     BRSHIRED  AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    バロー以外は読み飛ばし（取引先）
     IF       DEN-F01  =  3020  OR  13020
              CONTINUE
     ELSE
              GO                 TO   DEN-CNT-SEC
     END-IF.
...
*明細１行目
***  商品名（１）
     MOVE     DEN-A20(1:15)       TO   DT1-01.
***  ＪＡＮコード
     MOVE     DEN-A24             TO   DT1-02.
***  訂正伝票時処理
...
*明細２行目
***  商品名（２）
     MOVE     DEN-A20(16:15)      TO   DT2-01
***  商品コード
     MOVE     DEN-A19             TO   DT2-02.
***  入数
***  ケース(発注数)
***  数量
     MOVE     DEN-F50             TO        DT2-03.
...
***  発注日
     MOVE     DEN-A14             TO    HD3-08.
***  納品日
     MOVE     DEN-A15             TO    HD3-09.
...
***  テール３行目
     WRITE    PRT-REC        FROM      TLG  AFTER  2.
*
*
     MOVE     SPACE     TO        PRT-REC.
...
*
     CLOSE    BRSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
