...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSKT200.COB`

## ソースコード

```cobol
****************************************************************
*新基幹システムより移行  1999/10/16 T.TAKAHASHI                *
*    顧客名　　　　　　　：　サカタのタネ（株）殿　　　　　　　*
*    業務名　　　　　　　：　販売管理システム　　　　　　　　　*
*    モジュール名　　　　：　売上エラーデータ確認リスト　　　　*
*    作成日／更新日　　　：　93/03/17                          *
*    作成者／更新者　　　：　ＮＡＶ　　　　　　　　　　　　　　*
*    処理概要　　　　　　：　ワークＦより，売上エラー確認　　　*
*                        ：　リストを作成する。　　　　　　　　*
...
*--------------------------------------------------------------*
 01  WK-PGID            PIC  X(08)  VALUE  "SSKT200 ".
 01  FLAGS.
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 ...
 01  HEAD02.
     ...
     03  FILLER         CHARACTER TYPE MODE-2.
         05  FILLER     PIC  X(02)     VALUE  SPACE.
         05  FILLER     PIC  N(05)     VALUE  NC"指定伝票_".
 01  HEAD03.
     03  FILLER         CHARACTER TYPE MODE-2.
         05  FILLER     PIC  X(01)     VALUE  SPACE.
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### OSKT200 DSPFILE ERROR " DSP-CNTL " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HKYOTU   HTOKMS    HTENMS    DSPFILE.
     STOP     RUN.
*----<< 伝票データ >>--*
 HKYOTU-ERR            SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### OSKT200 HKYOTU ERROR " HKYOTU-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HKYOTU   HTOKMS    HTENMS    DSPFILE.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### OSKT200 HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HKYOTU   HTOKMS    HTENMS    DSPFILE.
...
*----<< 店舗マスタ >>--*
 HTENMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### OSKT200 HTENMS ERROR " HTENMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     CLOSE    HKYOTU   HTOKMS    HTENMS    DSPFILE.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     MOVE     SPACE          TO   FSK200.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FSK200"       TO   DSP-FMT.
     ...
     EVALUATE DSP-FNC
         ...
         WHEN ENT
              IF   (    R00002    IS   NUMERIC
                   AND  R00003    IS   NUMERIC
                   AND  R00002    >    R00003    )
              OR   (    R00004    IS   NUMERIC
                   AND  R00005    IS   NUMERIC
                   AND  R00004    >    R00005    )
...
*
     ...
     IF       R00002    IS   NUMERIC
              MOVE      R00002    TO   DEN-F01
     END-IF.
     IF       R00004    IS   NUMERIC
              MOVE      R00004    TO   DEN-F02
     END-IF.
