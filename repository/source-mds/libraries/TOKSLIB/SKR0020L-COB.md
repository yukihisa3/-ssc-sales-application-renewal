...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKR0020L.COB`
...
*    業務名　　　　　　　：　在庫管理システム　　　　　　　　　*
*    モジュール名　　　　：　送り状印刷　　　　　　            *
*    作成日／作成者　　　：　2002/02/20 /NAV                   *
*                            郵便番号８桁化                    *
*    更新日／更新者　　　：　2006/0201   NAV T.MATSUNO         *
*    再利用ＰＧ　　　　　：  ZDA0220B.SKTSLIB                  *
*    処理概要　　　　　　：　送り状Ｆを読み，画面より入力された*
*                            処理区分を判定し，送り状を印刷する*
*    更新概要　　　　　　：　出荷日変更　(2006/02/01)          *
****************************************************************
...
*
 ...
 FILE-CONTROL.
*-----<<  送り状ファイル  >>-----*
     SELECT   TOKURIF   ASSIGN    TO        DA-01-S-TOKURIF
...
*-----<<  送り状ファイル  >>-----*
 FD  TOKURIF.
     COPY     TOKURIF   OF   XFDLIB  JOINING   OKU  PREFIX.
*-----<<  画面ファイル  >>-----*
 FD  DSPF.
     COPY   FKR00103    OF        XMDLIB.
*-----<<  プリントファイル  >>-----*
 FD  PRTF.
...
****  プリントファイル  ****
 COPY   FKR00104         OF        XMDLIB.
 COPY   FKR00105         OF        XMDLIB.
 COPY   FKR00106         OF        XMDLIB.
 COPY   FKR00107         OF        XMDLIB.
****  画面制御項目  ****
 01  DSP-CONTROL.
...
*日付／時刻
 ...
 01  WK-SYSYY.
     03  WK-SYSYY1                PIC  X(02)  VALUE  "20".
     03  WK-SYSYY2                PIC  X(02)  VALUE  ZERO.
*画面表示日付編集
 01  HEN-DATE.
...
****  （株）サカタのタネ殿エリア   ****
 01  WK-AREA.
     03 WK-PRT1              PIC  X(08)   VALUE  "701-0165".
     03 WK-PRT2              PIC  N(12)   VALUE
             NC"岡山県岡山市大内田８１９".
     03 WK-PRT3              PIC  N(13)   VALUE
             NC"　カトーレック（株）岡山内".
     03 WK-PRT31             PIC  N(13)   VALUE
             NC"　サカタのタネ配送センター".
     03 WK-PRT4              PIC  X(12)   VALUE  "086-292-5777".
****  郵便番号編集
 01  YUBIN.
...
**** メッセージ情報           ****
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SKR0020L".
...
************************************************************
 END-SEC                SECTION.
* ファイルＣＬＯＳＥ
     CLOSE    TOKURIF   DSPF  PRTF.
     DISPLAY "(SKR0020L)ﾆｭｳﾘｮｸ ｹﾝｽｳ(ｵﾔ) = " IN-CNT1  UPON  CONS.
     DISPLAY "(SKR0020L)ﾆｭｳﾘｮｸ ｹﾝｽｳ(ｺ)  = " IN-CNT2  UPON  CONS.
     DISPLAY "(SKR0020L)ﾌﾟﾘﾝﾄ  ｹﾝｽｳ(ｵﾔ) = " PRT-CNT1 UPON  CONS.
     DISPLAY "(SKR0020L)ﾌﾟﾘﾝﾄ  ｹﾝｽｳ(ｺ)  = " PRT-CNT2 UPON  CONS.
 END-END.
     EXIT.
*----------------------------------------------------------*
*      2.1       初期画面処理                              *
*----------------------------------------------------------*
 INIT-DSP-SEC        SECTION.
     MOVE     SPACE          TO   FKR00103.
     MOVE    "FKR00103"      TO   DSP-FORMAT.
...
*伝票種類チェック
     IF  (  R00002 =  1  OR  2  OR  3  OR  4)
         EVALUATE    R00002
             ...
             WHEN    2
                     MOVE    NC"カトーレック" TO  W00002
             WHEN    3
                     MOVE    NC"西濃運輸"     TO  W00002
             WHEN    4
...
*----------------------------------------------------------*
 PRT2-SEC         SECTION.
     MOVE     SPACE            TO   FKR00105.
*システム日付セット
     MOVE     HEN-DATE-YYYY    TO   SYSYY.
     MOVE     HEN-DATE-MM      TO   SYSMM.
     MOVE     HEN-DATE-DD      TO   SYSDD.
* ファイルからＰＲＴへ
***  郵便番号
     MOVE  OKU-F151            TO   YUBIN1.
     MOVE  OKU-F152            TO   YUBIN2.
...
* 発送元印刷チェック
     IF   OKU-F14  =  1
          ...
          MOVE  WK-PRT2             TO   W22002
          MOVE  WK-PRT3             TO   W22003
          MOVE  WK-PRT31            TO   W22004
          MOVE  WK-PRT4             TO   W22005
     END-IF.
***  個数
     MOVE  OKU-F11             TO   W001.
* 送り状印刷
     MOVE     SPACE            TO   PRT-PRO.
     MOVE     "FKR00105"       TO   PRT-FMT.
     MOVE     SPACE            TO   PRT-CON1.
     MOVE     "GRP201"         TO   PRT-GRP.
     PERFORM   VARYING     IX     FROM   1   BY   1  UNTIL
                                      IX   >   OKU-F12
               WRITE    PRT-AREA       FROM    FKR00105
               ADD      1                TO    PRT-CNT1
**             CLOSE    PRTF
**             OPEN    OUTPUT   PRTF
     END-PERFORM.
...
*----------------------------------------------------------*
 PRT3-SEC         SECTION.
     MOVE     SPACE            TO   FKR00106.
*システム日付セット
*****MOVE     HEN-DATE-YYYY    TO   SYSYY3.
*****MOVE     HEN-DATE-MM      TO   SYSMM3.
*****MOVE     HEN-DATE-DD      TO   SYSDD3.
     MOVE     OKU-F04(1:2)     TO   WK-SYSYY2.
     MOVE     WK-SYSYY         TO   SYSYY3.
*****MOVE     OKU-F04(1:2)     TO   SYSYY3.
     MOVE     OKU-F04(3:2)     TO   SYSMM3.
     MOVE     OKU-F04(5:2)     TO   SYSDD3.
*****MOVE     WK-Y             TO   SYSYY3.
*****MOVE     WK-M             TO   SYSMM3.
*****MOVE     WK-D             TO   SYSDD3.
* ファイルからＰＲＴへ
...
* 発送元印刷チェック
     IF   OKU-F14  =  1
          ...
          MOVE  WK-PRT2             TO   W33002
          MOVE  WK-PRT3             TO   W33003
          MOVE  WK-PRT31            TO   W33004
...
* 送り状印刷
     MOVE     SPACE            TO   PRT-PRO.
     MOVE     "FKR00106"       TO   PRT-FMT.
     MOVE     SPACE            TO   PRT-CON1.
     MOVE     "GRP301"         TO   PRT-GRP.
     PERFORM   VARYING     IX     FROM   1   BY   1  UNTIL
                                      IX   >   OKU-F12
               WRITE    PRT-AREA       FROM    FKR00106
               ADD      1                TO    PRT-CNT1
**             CLOSE    PRTF
**             OPEN    OUTPUT   PRTF
     END-PERFORM.
...
* 伝票種類判定
     EVALUATE    OKU-F01
         ...
         WHEN    2
                 MOVE    NC"カトーレック" TO  W40009
         WHEN    3
                 MOVE    NC"西濃運輸"     TO  W40009
         WHEN    OTHER
                 MOVE    SPACE            TO  W40009
     END-EVALUATE.
* 送り状印刷
     MOVE     SPACE            TO   PRT-PRO.
     MOVE     "FKR00107"       TO   PRT-FMT.
     MOVE     SPACE            TO   PRT-CON1.
     MOVE     "GRP401"         TO   PRT-GRP.
     PERFORM   VARYING     IX     FROM   1   BY   1  UNTIL
                                      IX   >   OKU-F13
               WRITE    PRT-AREA       FROM    FKR00107
               ADD      1                TO    PRT-CNT2
**             CLOSE    PRTF
**             OPEN    OUTPUT   PRTF
     END-PERFORM.
