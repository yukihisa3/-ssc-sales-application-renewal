...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/STE1602B.COB`
...
***  ﾜｰｸ  ｴﾘｱ
 01  WRK-AREA.
     03  WK-TORICD                PIC  9(08)     VALUE   72028.
     ...
     03  WK-TANT01                PIC  X(04)     VALUE   SPACE.
     03  WK-TANT02                PIC  X(04)     VALUE   SPACE.
*--- ﾃﾞﾝﾋﾟﾖｳ ｷ-
     03  WRK-DENNO.
...
*    ﾐﾀﾞｼ
 01  HEAD-99.
     ...
     03  HD99                     PIC  X(02).
*    ﾐﾀﾞｼ
 01  HEAD-00                      CHARACTER  TYPE  YA.
     03  FILLER                   PIC  X(08)     VALUE   SPACE.
     03  FILLER                   PIC  N(08) VALUE NC"島忠".
*
 01  HEAD-01.
...
*
*    ﾒｲｻｲ
 01  BODY-01                      CHARACTER  TYPE  IS  YB.
     03  FILLER                   PIC  X(07)     VALUE   SPACE.
     03  MS01                     PIC  N(16).
*
 01  BODY-02                      CHARACTER  TYPE  IS  YB.
     03  FILLER                   PIC  X(07)     VALUE   SPACE.
     03  MS02                     PIC  N(16).
     03  FILLER                   PIC  X(01)     VALUE   SPACE.
     03  MS03                     PIC  X(13).
...
****************************************************************
 INIT-SEC               SECTION.
*    ファイルのＯＰＥＮ
     OPEN     INPUT     HKYOTU1  HTENMS  HMEIMS.
     OPEN     I-O       DSPFILE.
*    ワーク初期化
     MOVE     ZERO               TO   DENNO1.
     MOVE     ZERO               TO   CNT-DENPYO.
     MOVE     SPACE              TO   END-FLG.
     MOVE     SPACE              TO   FTE16021.
*伝票枚数カウント
...
*    伝票総枚数セット
     ...
     MOVE    "FTE16021" TO        DSP-FMT.
*    画面表示
     WRITE    FTE16021.
*****MOVE    "MD01R"    TO        DSP-GRP.
*****WRITE    FTE16021.
 DSP-WRITE-EXIT.
     EXIT.
****************************************************************
...
*    明細項目セット
*    商品名称マスタより商品名称取得
     MOVE     KYO-F1411          TO   MEI-F011.
     MOVE     KYO-F1412          TO   MEI-F012.
     PERFORM  HMEIMS-READ-SEC.
     IF       HMEIMS-INV-FLG = "INV"
              MOVE  ALL NC"＊"   TO   MS01 MS02
     ELSE
              MOVE  MEI-F021     TO   MS01
              MOVE  MEI-F022     TO   MS02
     END-IF.
*    取引先商品コード
     MOVE     KYO-F25            TO   MS03.
*    数量
     MOVE     KYO-F15            TO   MS04.
     ADD      KYO-F15            TO   G-SURYO.
*    原価単価
...
******************MOVE KYO-F1422  TO   WK-BIKO02
                  MOVE KYO-F1422(1:4) TO WK-TANT01
                  MOVE KYO-F1422(5:4) TO WK-TANT02
              END-IF
              GO                  TO   READ-000
     END-IF.
*    全プリント時判定
...
*    店舗名称取得
     ...
     MOVE     KYO-F07            TO   TEN-F011.
     PERFORM  HTENMS-READ-SEC.
     IF       HTENMS-INV-FLG = "INV"
              MOVE  ALL NC"＊"   TO   HD01
     ELSE
              MOVE  TEN-F03      TO   HD01
     END-IF.
*    分類コード
     MOVE     KYO-F12            TO    HD02.
*    店舗コード
     MOVE     KYO-F07            TO    HD03.
*    伝票区分
     MOVE     KYO-F131           TO    HD04.
*    伝票Ｎｏ
     MOVE     KYO-F02            TO    HD05.
*    取引先コード
     MOVE     KYO-F01            TO    HD06.
     MOVE     NC"_サカタのタネ" TO    HD07.
*    納品日
     MOVE     KYO-F112           TO    WK-F112.
     MOVE     WK-F112(3:6)       TO    HD08.
*
     WRITE    PRT-REC     FROM   HEAD-99    AFTER    2.
     WRITE    PRT-REC     FROM   HEAD-00    AFTER    3.
...
*    数量
     MOVE   G-SURYO              TO   TL01.
*    原価金額合計
     MOVE   G-GENKA              TO   TL02.
*    売価金額合計
     MOVE   G-BAIKA              TO   TL03.
*    仕入申請書■
*    備考
     MOVE   WK-BIKO01            TO   BIKO01.
*****MOVE   WK-BIKO02            TO   BIKO02.
*    発注担当者
     MOVE   WK-TANT01            TO   TANT01.
     MOVE   WK-TANT02            TO   TANT02.
*    テイル印字制御
...
*    テスト印字項目セット
     MOVE   ALL "9"              TO   HD03    HD05    HD06
                                      ...
                                      MS07    MS08    MS09
                                      TL01    TL02    TL03.
     MOVE   ALL "*"              TO   HD02    HD04    MS03
                                      BIKO01  BIKO02
                                      TANT01  TANT02.
     MOVE   ALL NC"＊"           TO   HD01    MS01    MS02.
*    ヘッド印字
