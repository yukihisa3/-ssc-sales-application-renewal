...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSI0130B.COB`
...
*----<< 支払明細ファイル >>--*
     SELECT   HSEIGKF   ASSIGN         DA-01-VI-SEIGKF21
                        ...
                        STATUS         SEI-ST.
*----<< 照合ファイル >>--*
     SELECT   SEKSYOGO  ASSIGN         DA-01-S-SEKSYOGO
                        ORGANIZATION   SEQUENTIAL
                        STATUS         SYO-ST.
*
****************************************************************
...
*----<< 取引先マスタ >>--*
 FD  HSEIGKF            LABEL RECORD   IS   STANDARD.
     COPY     SETGKFA   OF        XFDLIB
              JOINING   SEI       PREFIX.
*----<< 照合ファイル >>--*
 FD  SEKSYOGO           LABEL RECORD   IS   STANDARD
                        BLOCK CONTAINS  4   RECORDS.
     COPY     SITGKFD   OF        XFDLIB
              JOINING   SYO       PREFIX.
*--------------------------------------------------------------*
...
*----<< ﾌﾟﾛｸﾞﾗﾑID  >>--*
 01  PG-ID.
     03  ID-PG          PIC  X(08)     VALUE  "SSI0130B".
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*----<< 照合ファイル >>--*
 SYO-ERR                SECTION.
     USE AFTER     EXCEPTION PROCEDURE      SEKSYOGO.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### " PG-ID " SEKSYOGO ERROR " SYO-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
...
*レコード転送
     MOVE     SSI-REC             TO   SYO-REC.
*金額変換
     MOVE     SSI-F28(2:9)        TO   WK-SIHARAI.
     IF       SSI-F28(1:1)   =   "-"
              COMPUTE     WK-KINGAKU   =   HEN-SIHARAI  *  (-1)
     ELSE
              MOVE        HEN-SIHARAI      TO   WK-KINGAKU
              IF  SSI-F24  =  "55"
                  COMPUTE WK-KINGAKU = WK-KINGAKU * (-1)
              END-IF
     END-IF.
*金額比較（レコード区分＝””→正常データ）
*        （レコード区分＝”２”→支払金額違い）
     IF       WK-KINGAKU     =    SEI-F06
              MOVE     SPACE      TO   SYO-F19
     ELSE
              MOVE     "2"        TO   SYO-F19
              ADD       1         TO   KIN-ERR
     END-IF.
...
*照合ファイル初期化
     ...
     INITIALIZE                        SYO-REC.
*締切日
     MOVE     SEI-F02             TO   SYO-F03.
*店舗コード
     MOVE     SEI-F03             TO   SYO-F22.
*検収日
     MOVE     SEI-F04             TO   SYO-F26.
*伝票_
     MOVE     SEI-F05             TO   SYO-F21.
*レコード区分（エラー区分）
     MOVE     "3"                 TO   SYO-F19.
*取引先コード
     MOVE     SEI-F01(3:6)        TO   SYO-F25.
*伝票区分
     MOVE     SEI-F07             TO   SYO-F24.
*請求金額
     MOVE     SEI-F06             TO   WK-SEIKYU(2:9).
     MOVE     SEI-F06             TO   SYO-F28(2:9).
     IF       SEI-F06   <  0
              MOVE     "-"        TO   WK-SEIKYU(1:1)
     ELSE
              MOVE     "0"        TO   WK-SEIKYU(1:1)
     END-IF.
     MOVE     WK-SEIKYU           TO   SYO-F28.
*照合ファイル出力
     WRITE    SYO-REC.
     ADD      1                   TO   SIH-ERR.
*
...
*--------------------------------------------------------------*
 900-SSI-READ           SECTION.
     READ     SSIHARAD
         ...
         NOT AT END
              MOVE      SSI-F21        TO   SSI-DENNO
     END-READ.
 900-SSI-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL ALL    請求明細ファイル　 START                     *
*--------------------------------------------------------------*
 900-SEI-ST-READ        SECTION.
*
     MOVE     SSI-F25        TO      WK-TORICD.
     MOVE     HEN-TORICD     TO      SEI-F01.
     MOVE     SSI-F21        TO      SEI-F05.
