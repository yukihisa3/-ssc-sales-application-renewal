...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKE0910B.COB`
...
*----<< 検収情報データ >>--*
 KEN-ERR              SECTION.
     ...
     DISPLAY  "### SKE0910B KENSOKF    ERROR " KEN-ST " "
...
*----<< 条件ファイル >>--*
 JYO-ERR              SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     "4000"         TO   PROGRAM-STATUS.
     DISPLAY  "### SKE0910B HJYOKEN    ERROR " JYO-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
...
*　　出荷情報データスタート　　　　　　　　　　　　　　　　　　*
****************************************************************
 KENSOKF-START-SEC      SECTION.
*スタートキーセット
     MOVE     1              TO   KEN-F06.
     MOVE     ZERO           TO   KEN-F01 KEN-F02 KEN-F03
     START    KENSOKF   KEY  IS  >=  KEN-F06 KEN-F01 KEN-F02
                                     KEN-F03
              INVALID
              MOVE    "END"  TO   END-FLG
     END-START.
...
*    読込み件数表示
     ...
     END-IF.
*    送信日付が削除基準日付以下の場合
     IF   KEN-F05  <   WK-ZAIKO-DATE
          CONTINUE
     ELSE
          GO        TO   KENSOKF-READ-SEC
     END-IF.
*
 KENSOKF-READ-EXIT.
     EXIT.
