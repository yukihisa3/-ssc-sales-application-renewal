...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY2801L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*キッコリー伝票データ（共通）　　
     SELECT   KISHIRED  ASSIGN    TO        KISHIRED
...
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
     ...
     03  WK-TYOKU1                PIC  X(15)     VALUE  SPACE.
     03  WK-TYOKU2                PIC  X(05)     VALUE  SPACE.
     03  WK-PT                    PIC  X(02)     VALUE  SPACE.
 01  SEC-NAME.
...
*
 01  HD1.
     ...
     03  HD1-01                   PIC  X(20)     VALUE   SPACE.
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      KISHIRED.
...
*明細２行目
***  商品コード
     MOVE     DEN-A13             TO        DT2-01.
     MOVE     DEN-A35             TO        DT2-02.
***  ＪＡＮコード
     MOVE     DEN-A15             TO        DT2-03.
***  色／入数
     MOVE     DEN-A16             TO        DT2-04.
***  ケース数
     MOVE     DEN-A17             TO        DT2-05.
***  単位
***  数量
...
***  訂正無時（TAIL-WT-SEC で使用）
     ...
     END-IF.
*出力行_と未対応の時，エラー行出力
     IF       MEI-CNT  NOT = DEN-A03
              MOVE ALL "*"   TO       ERR1-1
              WRITE PRT-REC FROM ERR-GYO AFTER 1
              WRITE PRT-REC FROM ERR-GYO AFTER 1
              ADD  2         TO       L-CNT
     END-IF.
*
...
*ＨＤ１
***  社名
     MOVE     DEN-A06             TO        HD1-01.
***  取引先名称
     MOVE     DEN-A08             TO        HD1-02.
*ＨＤ２
***  店名
     MOVE     DEN-A07             TO        HD2-01.
***  社／店コード
     MOVE     DEN-A25             TO        HD2-02.
***  分類コード
     MOVE     DEN-A05(2:3)        TO        HD2-03.
***  伝票区分
     MOVE     DEN-A04             TO        HD2-04.
***  伝票_
     MOVE     DEN-A02             TO        HD2-05.
***  取引先コード
     MOVE     DEN-A01(3:6)        TO        HD2-06.
***  発注日
     MOVE     DEN-A09(1:2)        TO        HD2-071.
     MOVE     DEN-A09(3:2)        TO        HD2-072.
     MOVE     DEN-A09(5:2)        TO        HD2-073.
***  納品日
     MOVE     DEN-A10(1:2)        TO        HD2-081.
     MOVE     DEN-A10(3:2)        TO        HD2-082.
     MOVE     DEN-A10(5:2)        TO        HD2-083.
***  便
***  直送先名１
     MOVE     DEN-A11             TO        WK-TYOKU1.
***  直送先名２
     MOVE     DEN-A33             TO        WK-TYOKU2.
***  ＰＴ識別
     MOVE     DEN-A32             TO        WK-PT.
*
...
*
*ヘッダ出力
     WRITE    PRT-REC        FROM      HD1       AFTER    6.
...
*テール２行目
*直送名
     MOVE   WK-TYOKU1             TO        TL2-01.
     MOVE   WK-TYOKU2             TO        TL2-02.
*
*テール３行目
*ＰＴ識別＝”ＰＴ”の時で総量発注時の時
     IF       WK-PT = "P2"
              MOVE "ｿｳﾘｮｳﾊｯﾁｭｳ"   TO        TL3-01
*********     WRITE  PRT-REC  FROM  TL3  AFTER  3
              MOVE SPACE          TO        WK-PT
     END-IF.
*
              ...
              ELSE
                PERFORM  VARYING  I FROM  1  BY  1  UNTIL I > 9
                   ...
                   IF  TSGENKEI(I:1)  NOT = ZERO
                       MOVE   1                 TO   ZERO-FLG
                       MOVE   TSGENKEI(I:1)     TO   TL3-02(J:1)
                   ELSE
                       IF      ZERO-FLG   =    1
...
***  訂正後売価金額合計
              MOVE      ZERO      TO        ZERO-FLG
              MOVE      SPACE     TO        TL3-03
              IF        TSBAIKEI  =  ZERO
              MOVE      ZERO      TO        TL3-03(17:1)
              ELSE
                PERFORM  VARYING  I FROM  1  BY  1  UNTIL I > 9
                   COMPUTE  J  =  I  *  2
                   IF  TSBAIKEI(I:1)     NOT = ZERO
                       MOVE   1                 TO   ZERO-FLG
                       MOVE   TSBAIKEI(I:1)     TO   TL3-03(J:1)
                   ELSE
                       IF      ZERO-FLG   =    1
                               MOVE     ZERO   TO    TL3-03(J:1)
                       END-IF
                   END-IF
                END-PERFORM
              END-IF
...
****************************************************************
 TEST-PRT-SEC            SECTION.
     MOVE     "TEST-PRT-SEC"     TO      S-NAME.
*項目クリア
     MOVE     SPACE               TO   HD1 HD2
                                       DT1 DT2
                                       TL1 TL2 TL3.
*    テスト印字項目セット
     MOVE   ALL "9"              TO    HD2-02   HD2-03
                                       HD2-04   HD2-05
                                       HD2-06   HD2-071
                                       HD2-072  HD2-073
                                       HD2-081  HD2-082
...
*
     CLOSE    KISHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
