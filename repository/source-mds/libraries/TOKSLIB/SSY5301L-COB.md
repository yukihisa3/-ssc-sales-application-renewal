...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY5301L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*伝票データ（共通）　　
     SELECT   JASHIRED  ASSIGN    TO        JASHIRED
...
***  ﾍﾝｼｭｳ ｴﾘｱ
 ...
 01  WK-DATE-R   REDEFINES  WK-DATE.
     ...
     03  WK-DD                    PIC  Z9.
 01  WK-SYATEN.
     03  WK-SYACD                 PIC  9(04).
     03  FILLER                   PIC  X(01)     VALUE   "-".
     03  WK-TENCD                 PIC  9(02).
 01  WK-DENNO.
     03  FILLER                   PIC  X(02)     VALUE   "**".
     03  WK-DENNO1                PIC  9(02).
     03  FILLER                   PIC  X(01)     VALUE   "-".
     03  WK-DENNO2                PIC  9(06).
***  画面用日付取得
 01  WK-DATE8.
...
*
 01  HD2                          CHARACTER  TYPE   IS   YA.
     03  FILLER                   PIC  X(82)     VALUE   SPACE.
     03  HD2-01                   PIC  N(01).
*
 01  HD3.
     03  FILLER                   PIC  X(115)    VALUE   SPACE.
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      JASHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     JASHIRED AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    ジョイフルエーケー以外は読み飛ばし（取引先）
     IF       DEN-F01  NOT =  3043
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
...
*伝票　ブレイクチェック
     IF       DEN-F02   NOT =               DENNO1
              IF   L-CNT   <   25
                   WRITE     PRT-REC   FROM   DT5  AFTER     2
                   ADD       2         TO     L-CNT
              END-IF
              PERFORM   TAIL-WT-SEC
              PERFORM   HEAD-WT-SEC
...
*明細２行目
*    商品名（下段）
     MOVE     DEN-A30             TO   DT2-01.
*    商品コード
     MOVE     DEN-A19             TO   DT2-02.
*    色／入数（発注単位）
     IF       DEN-A20   =   9999
              MOVE     "ﾊﾞﾗ"      TO   DT2-03
     ELSE
              MOVE     DEN-A20    TO   WK-IRISU
              MOVE     WK-IRISU   TO   DT2-03
     END-IF.
*    サイズ／ケース（発注数）
     IF       DEN-A21   NOT =   9999
              MOVE     DEN-A21    TO   DT2-04
     END-IF.
***  数量
...
*ヘッダ４行目
***  社名
     MOVE     DEN-A10             TO        HD4-01.
***  取引先名中段
     MOVE     DEN-A12             TO        HD4-02.
*ヘッダ５行目
***  店名
     MOVE     DEN-A11             TO        HD5-01.
***  店コード
     MOVE     DEN-A041            TO        WK-SYACD.
     MOVE     DEN-A042(4:2)       TO        WK-TENCD.
     MOVE     WK-SYATEN           TO        HD5-02.
***  分類コード(大分類)
     MOVE     DEN-A05             TO        HD5-03.
***  伝票区分
     MOVE     DEN-A06             TO        HD5-04.
***  伝票_
     MOVE     DEN-F02(2:2)        TO        WK-DENNO1.
     MOVE     DEN-F02(4:6)        TO        WK-DENNO2.
     MOVE     WK-DENNO            TO        HD5-05.
***  取引先コード
     MOVE     DEN-F01             TO        HD5-06.
***  発注日
     MOVE     DEN-A07             TO        HD5-07.
***  納品日
     MOVE     DEN-A08             TO        HD5-08.
*
...
***  Ｆ欄
     MOVE     " *EOS*"            TO        TL1-01.
***  原価金額合計
     IF     DEN-A14   =    GENKINKEI
            MOVE   DEN-A14        TO        TL1-05
     ELSE
            MOVE   GENKINKEI      TO        TL1-05
     END-IF.
***  売価金額合計
     IF     DEN-A15   =    BAIKINKEI
            MOVE   DEN-A15        TO        TL1-06
     ELSE
...
*項目クリア
     MOVE     SPACE               TO   HD1 HD2 HD3 HD4 HD5
                                       ...
                                       TL1 TL2.
*テスト印字項目セット
     MOVE   ALL "9"    TO    HD5-02   HD5-03   HD5-04   HD5-05
                             HD5-06   HD5-07   HD5-08
                             DT2-02   DT2-03   DT2-04
                             DT2-051  DT2-061  DT2-071
                             DT2-08   DT2-09   DT2-10
                             TL1-05   TL1-06.
     MOVE   ALL "*"    TO    HD4-01   HD4-02   HD5-01
                             DT1-01   DT2-01
                             TL1-01.
     MOVE   NC"＊"     TO    HD2-01.
*ヘッダ部
...
*
     CLOSE    JASHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
