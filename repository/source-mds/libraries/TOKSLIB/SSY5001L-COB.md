...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY5001L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*イオン伝票データ（共通）　　
     SELECT   TBSHIRED  ASSIGN    TO        TBSHIRED
...
***  自社伝票番号編集
 01  WK-JISYA-DEN.
     03  WK-JISYA-01              PIC  X(08)  VALUE "ｼﾞｼｬNO.:".
     03  WK-JISYA-02              PIC  9(09).
***  頁番号編集
 01  WK-PAGE.
     03  FILLER                   PIC  X(06)  VALUE "(V) P.".
     03  WK-PAGE-01               PIC  X(04).
 01  WK-DEN-A22                   PIC  X(17)  VALUE  SPACE.
 01  WK-DEN-A23                   PIC  X(12)  VALUE  SPACE.
****************************************************************
*    プリントエリア                                            *
****************************************************************
*--------------------------------------------------------------*
...
*
 01  DT1.
     ...
     03  DT1-02                   PIC  ZZZZZZZZZ.
     03  FILLER                   PIC  X(06).
     03  DT1-03                   PIC  ZZZZZZZZZ.
*
 01  DT2.
     03  FILLER                   PIC  X(05)     VALUE   SPACE.
     03  DT2-01                   PIC  X(25).
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      TBSHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     TBSHIRED AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    トステムビバ以外は読み飛ばし（取引先）
*****IF       DEN-F01  NOT =  38709
*****IF       DEN-F01  NOT =  99999
...
*    商品名（上段）
     IF       DEN-A412  =  SPACE
              MOVE     DEN-A411   TO   DT2-01
     ELSE
              MOVE     DEN-A411   TO   DT1-01
     END-IF.
***  訂正伝票時処理
...
*明細２行目
*    商品名（下段）
     IF       DEN-A412  NOT =  SPACE
              MOVE     DEN-A412            TO   DT2-01
     END-IF.
*    商品コード
     MOVE     DEN-A30             TO   DT2-02.
*    色／入数（発注単位）
     MOVE     DEN-A32             TO   DT2-03.
*    サイズ／ケース（発注数）
     MOVE     DEN-A33             TO   DT2-04.
***  数量
     MOVE     DEN-F50             TO   DT2-051.
     MOVE     ZERO                TO   DT2-052.
*    訂正後数量
     IF       DEN-F15   NOT =     DEN-F50
              MOVE      DEN-F15   TO   DT2-061
...
***  原単価（端数部）
     IF       WK-GENKA2   =       ZERO
              MOVE SPACE          TO        DT2-072
     ELSE
              MOVE WK-GENKA2      TO        DT2-072
     END-IF.
***  原価金額
     MOVE     DEN-F521            TO        DT2-08.
***  売価単価
     MOVE     DEN-F513            TO        DT2-09.
***  売価金額
     MOVE     DEN-F522            TO        DT2-10.
*
*原価・売価合計計算
...
*ヘッダ１行目
***  Ｄ欄（上段）
     MOVE     DEN-A201            TO        HD1-01.
*ヘッダ２行目
***  Ｄ欄（中段）
     MOVE     DEN-A202            TO        HD2-01.
***  訂正フラグ＝１の時　訂正区分に"◯"を印刷
     IF       DEN-F53  =  1
              MOVE     NC"◯"     TO        HD2-02
     END-IF.
***  Ｅ欄（中段）
     MOVE     DEN-A21             TO        HD2-03.
*
*ヘッダ３行目
***  Ｄ欄（下段）
     MOVE     DEN-A203            TO        HD3-01.
***  取引先名上段
     MOVE     DEN-A14             TO        HD3-02.
***  頁数
     ADD      1                   TO        PAGE-CNT.
     MOVE     PAGE-CNT            TO        WK-PAGE-01.
     MOVE     WK-PAGE             TO        HD3-03.
*ヘッダ４行目
***  社名
     MOVE     DEN-A12             TO        HD4-01.
***  取引先名中段
     MOVE     DEN-A15             TO        HD4-02.
*ヘッダ５行目
***  店名
     MOVE     DEN-A13             TO        HD5-01.
***  店コード
     MOVE     DEN-A05             TO        HD5-02.
***  分類コード(大分類)
     MOVE     DEN-A06             TO        HD5-03.
***  伝票区分
     MOVE     DEN-A07             TO        HD5-04.
***  伝票■
     MOVE     DEN-A031            TO        HD5-05.
     MOVE     DEN-F02             TO        WK-JISYA-02.
***  取引先コード
     MOVE     DEN-F01             TO        HD5-06.
***  電話番号
     MOVE    "045-945-8816"       TO        HD5-07.
***  発注日
     MOVE     DEN-A09             TO        HD5-08.
***  納品日
     MOVE     DEN-A10             TO        HD5-09.
***  Ｆ欄
...
*テール１行目
***  Ｆ欄
     MOVE     WK-DEN-A22          TO        TL1-01.
***  Ｈ欄
     MOVE     WK-DEN-A23          TO        TL1-02.
***  数量合計
     MOVE     SURYOKEI            TO        TL1-03.
***  Ｈ欄
     IF       GENKINKEI  NOT =  TSGENKEI
     OR       BAIKINKEI  NOT =  TSBAIKEI
              MOVE   TSSURYOKEI   TO        TL1-04
     END-IF.
***  原価金額合計
     MOVE   GENKINKEI             TO        TL1-05.
***  売価金額合計
     MOVE   BAIKINKEI             TO        TL1-06.
*テール２行目
***  自社伝票番号印字
***  MOVE   DEN-F02               TO        WK-JISYA-02.
     MOVE   WK-JISYA-DEN          TO        TL2-01.
...
***  訂正行の印字
***  DISPLAY "GENKINKEI      =  "  GENKINKEI  UPON CONS.
***  DISPLAY "BAIKINKEI      =  "  BAIKINKEI  UPON CONS.
***  DISPLAY "TSGENKEI       =  "  TSGENKEI   UPON CONS.
***  DISPLAY "TSBAIKEI       =  "  TSBAIKEI   UPON CONS.
     IF       GENKINKEI  NOT =  TSGENKEI
     OR       BAIKINKEI  NOT =  TSBAIKEI
              WRITE     PRT-REC   FROM      TL3      AFTER  0
     END-IF.
***  テール３行目
...
*項目クリア
     MOVE     SPACE               TO   HD1 HD2 HD3 HD4 HD5
                                       ...
                                       TL1 TL2.
*テスト印字項目セット
     MOVE   ALL "9"    TO    HD5-02   HD5-05   HD5-06
                             HD5-08   HD5-09
                             DT2-03   DT2-04   DT2-051
                             DT2-052  DT2-061  DT2-062
                             DT2-071  DT2-072  DT2-08
                             DT2-09   DT2-10
                             TL1-03   TL1-04   TL1-05   TL1-06.
     MOVE   ALL "*"    TO    HD1-01
                             HD2-01   HD2-03
                             HD3-01   HD3-02   HD3-03
                             HD4-01   HD4-02
                             HD5-01   HD5-03   HD5-03   HD5-04
                             HD5-07
                             DT1-01
                             DT2-01   DT2-02
                             TL1-01   TL1-02   TL2-01.
     MOVE   NC"＊"     TO    HD2-02.
*ヘッダ部
     WRITE    PRT-REC          FROM    HD1        AFTER    1.
     WRITE    PRT-REC          FROM    HD2        AFTER    1.
...
*
     CLOSE    TBSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
