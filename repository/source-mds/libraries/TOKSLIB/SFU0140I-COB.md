...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SFU0140I.COB`
...
*
 ...
 FILE-CONTROL.
*部門取引先マスタ
     SELECT  BUTOKMF   ASSIGN    TO        BUTOKML1
...
*フラグ領域
 01  FLG-AREA.
     ...
     03  BUTOKMF-INV-FLG          PIC  X(03)  VALUE  SPACE.
*ワーク領域
 01  WRK-AREA.
***  プログラムスイッチ（画面遷移制御）
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG5.
         05  FILLER              PIC   N(25)
             VALUE NC"部門取引先マスタに登録済みです。".
     03  ERR-MSG6.
         05  FILLER              PIC   N(25)
             VALUE NC"部門取引先マスタに未登録です。".
     03  ERR-MSG7.
         05  FILLER              PIC   N(25)
...
*
 BUT-ERR                   SECTION.
     ...
     MOVE        BUT-ST    TO        E-ST.
     MOVE        "BUTOKMF " TO       E-FILE.
     DISPLAY     SEC-NAME  UPON      CONS.
     DISPLAY     ERR-FILE  UPON      CONS.
     DISPLAY     ERR-NAME  UPON      CONS.
     DISPLAY     BUT-ERR   UPON      CONS.
...
*プログラムＩＤ
     MOVE    "SFU0140I"           TO   DSP-PGID.
*ＦＯＲＭＩＤ
     MOVE    "FFU01401"           TO   DSP-FORMID.
*リバース，カーソルパーク解除
*取引先コード
     MOVE    "M"     TO      EDIT-OPTION   OF  DSP-TORICD.
     MOVE    SPACE   TO      EDIT-CURSOR   OF  DSP-TORICD.
*取引先名称
     MOVE    "M"     TO      EDIT-OPTION   OF  DSP-TORINM.
     MOVE    SPACE   TO      EDIT-CURSOR   OF  DSP-TORINM.
*部門
...
*エラーの場合は、ＥＸＩＴへ
     ...
     END-IF.
*部門取引先マスタ読込み
     MOVE     DSP-TORICD     TO   BUT-F01.
     PERFORM  BUTOKMF-READ-SEC.
*処理区分により判定
     EVALUATE DSP-SHORI
         WHEN  1
               IF   BUTOKMF-INV-FLG = SPACE
                    MOVE  5     TO ERR-FLG
                    MOVE "R"    TO EDIT-OPTION OF DSP-TORICD
                    MOVE "C"    TO EDIT-CURSOR OF DSP-TORICD
               ELSE
               ...
               END-IF
         WHEN  2
               IF   BUTOKMF-INV-FLG = "INV"
                    MOVE    6   TO ERR-FLG
                    MOVE "R"    TO EDIT-OPTION OF DSP-TORICD
                    MOVE "C"    TO EDIT-CURSOR OF DSP-TORICD
                    MOVE  SPACE TO DSP-TORINM  DSP-BUMON
               ELSE
                    MOVE "4"    TO PSW
                    MOVE "M"    TO EDIT-OPTION OF DSP-TORICD
                    MOVE  SPACE TO EDIT-CURSOR OF DSP-TORICD
...
*全レコード解放（排他制御用）
     ...
     OPEN     I-O   BUTOKMF.
* 属性クリア
     MOVE     SPACE     TO   BUTOKMF-INV-FLG.
     MOVE     " "       TO   EDIT-CURSOR OF DSP-TORICD.
     MOVE     "M"       TO   EDIT-OPTION OF DSP-TORICD.
*
     MOVE     DSP-TORICD     TO  BUT-F01.
     START    BUTOKMF   KEY  IS  <  BUT-F01
                 WITH   REVERSED  ORDER
         INVALID  KEY
              MOVE  "INV"    TO  BUTOKMF-INV-FLG
         NOT  INVALID  KEY
              PERFORM  BUTOKMF-NEXTREAD-SEC
     END-START.
     IF  BUTOKMF-INV-FLG  =   "INV"
         MOVE      7    TO   ERR-FLG
         MOVE     "C"   TO   EDIT-CURSOR  OF  DSP-TORICD
...
*
 DSP-KAKU-EXIT.
     EXIT.
****************************************************************
*    部門取引先マスタ更新　　処理区分＝１（登録）2.4.1         *
****************************************************************
 FILE-WRT-SEC           SECTION.
     MOVE     "FILE-WRT-SEC"      TO   S-NAME.
*レコード初期クリア
     MOVE     SPACE               TO   BUT-REC.
     INITIALIZE                        BUT-REC.
*キー項目転送
***  取引先コード
     MOVE     DSP-TORICD          TO   BUT-F01.
***  取引先名称
     MOVE     DSP-TORINM          TO   BUT-F02.
***  部門
     MOVE     DSP-BUMON           TO   BUT-F03.
*部門取引先マスタ登録
     WRITE    BUT-REC.
*
 FILE-WRT-EXIT.
...
*
              REWRITE  BUT-REC
     ELSE
              DISPLAY "## BUTOKMF REWRITE ERR ##"  UPON CONS
              DISPLAY "## TORICD =" BUT-F01 " ##"  UPON CONS
              STOP  RUN
     END-IF.
*
 FILE-UPD-EXIT.
...
***           レコード削除
              DELETE   BUTOKMF
     ELSE
              DISPLAY "## BUTOKMF DELETE  ERR ##" UPON CONS
              DISPLAY "## TORICD =" BUT-F01 " ##" UPON CONS
              STOP  RUN
     END-IF.
*
...
*
     MOVE    BUT-F01              TO   DSP-TORICD.
     MOVE    BUT-F02              TO   DSP-TORINM.
     MOVE    BUT-F03              TO   DSP-BUMON.
*
 MST-DSP-EXIT.
     EXIT.
****************************************************************
*             部門取引先マスタ読込み                           *
****************************************************************
 BUTOKMF-READ-SEC         SECTION.
     MOVE   "BUTOKMF-READ-SEC"    TO   S-NAME.
*
     READ    BUTOKMF
         INVALID
             MOVE    "INV"        TO   BUTOKMF-INV-FLG
...
*             部門取引先マスタ順読込み                         *
****************************************************************
 BUTOKMF-NEXTREAD-SEC     SECTION.
     MOVE   "BUTOKMF-NEXTREAD-SEC"     TO   S-NAME.
*
     READ    BUTOKMF    NEXT
         AT END
             MOVE    "INV"        TO   BUTOKMF-INV-FLG
...
****************************************************************
 END-SEC               SECTION.
*ファイル ＣＬＯＳＥ
     CLOSE             BUTOKMF  DSPFILE.
**
 END-EXIT.
     EXIT.
*****************<<  SFU0140I   END PROGRAM  >>******************
