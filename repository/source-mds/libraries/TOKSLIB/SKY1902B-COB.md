...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKY1902B.COB`
...
*----<< 画面　Ｆ >>----*
 FD  DSPF.
     COPY    FKY19021  OF   XMDLIB.
*----<< バーコードファイル  >>---*
 FD  ZJANDT.
     COPY     ZJANDT   OF  XFDLIB
     JOINING  JAN      AS  PREFIX.
*----<< プリンタＦ >>-----*
 FD  PRTF.
     COPY     FKY19022 OF  XMDLIB.
*=============================================================
...
*メッセージ情報
 01  MSG-AREA.
     03  MSG-ABEND1.
         05  FILLER          PIC  X(12)  VALUE  "### SKY1902B".
...
*=============================================================
 INIT-SEC            SECTION.
     DISPLAY  "**  SKY1902B   START  **"   UPON  CONS.
*
*システム日付・時刻の取得
...
*初期画面表示
 DSP-010.
     MOVE      LOW-VALUE         TO        FKY19021.
     MOVE      "FKY19021"        TO        DSP-FRM.
     MOVE      "CL"              TO        DSP-TYP.
...
**
     MOVE      ZERO    TO      MAISU.
     PERFORM   UNTIL   MAISU >=  JAN-F09
        MOVE   SPACE           TO       FKY19022
        MOVE   JAN-F061        TO       MEI1A
        MOVE   JAN-F062        TO       MEI1B
        MOVE   JAN-F07         TO       BUMON1
********MOVE   JAN-F08         TO       KINGK1
        MOVE   JAN-F05         TO       BCD1
        PERFORM    PRICE-HENKAN
        MOVE   W-KINGKX        TO       KINGK1
        ADD      1             TO       MAISU
**
        IF    (MAISU >= JAN-F09) AND (END-FLG = SPACE)
             PERFORM   JAN-READ-SEC
             IF    END-FLG  =  "END"
                   MOVE    9999    TO   MAISU
              ELSE
        ...
        END-IF
        IF   END-FLG = SPACE
           MOVE   JAN-F061  TO       MEI2A
           MOVE   JAN-F062  TO       MEI2B
           MOVE   JAN-F07   TO       BUMON2
********   MOVE   JAN-F08   TO       KINGK2
           MOVE   JAN-F05   TO       BCD2
           PERFORM    PRICE-HENKAN
...
**
        ...
        END-IF
        IF   END-FLG = SPACE
           MOVE   JAN-F061  TO       MEI3A
           MOVE   JAN-F062  TO       MEI3B
           MOVE   JAN-F07   TO       BUMON3
*******    MOVE   JAN-F08   TO       KINGK3
           MOVE   JAN-F05   TO       BCD3
           PERFORM    PRICE-HENKAN
...
**
        ...
        END-IF
        IF   END-FLG = SPACE
           MOVE   JAN-F061  TO       MEI4A
           MOVE   JAN-F062  TO       MEI4B
           MOVE   JAN-F07   TO       BUMON4
********   MOVE   JAN-F08   TO       KINGK4
           MOVE   JAN-F05   TO       BCD4
           PERFORM    PRICE-HENKAN
...
**
        ...
        END-IF
        IF   END-FLG = SPACE
           MOVE   JAN-F061  TO       MEI5A
           MOVE   JAN-F062  TO       MEI5B
           MOVE   JAN-F07   TO       BUMON5
********   MOVE   JAN-F08   TO       KINGK5
           MOVE   JAN-F05   TO       BCD5
           PERFORM    PRICE-HENKAN
...
*
     ...
     PERFORM  VARYING  I  FROM  1  BY  1  UNTIL  I > 5
        ADD      1          TO       J
        IF       I     =     3
           IF   (JAN-F08(1:1) = ZERO) AND (JAN-F08(2:1) = ZERO)
...
*ファイル ＣＬＯＳＥ
     ...
     DISPLAY  "**  SKY1902B     END  **"   UPON  CONS.
 END-EXIT.
     EXIT.
