...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SJA0102I.COB`
...
*カウント領域
 01  CNT-AREA.
     03  CNT-PAGE                 PIC  9(02)  VALUE  ZERO.
*ワーク領域
 01  WRK-AREA.
***  プログラムスイッチ（画面遷移制御）
...
*データ退避用
 01  WK-TLB.
     03  WK-JANTBL          OCCURS    50.
         05  WK-JAN-START             PIC  X(13)  VALUE  SPACE.
         05  WK-JAN-END               PIC  X(13)  VALUE  SPACE.
         05  WK-KAN-START             PIC  X(40)  VALUE  SPACE.
         05  WK-KAN-END               PIC  X(40)  VALUE  SPACE.
*
*ＰＦガイド
 01  PF-MSG-AREA.
     03  PF-MSG1.
...
****  VALUE NC"_取消_終了_項目戻し".
     03  PF-MSG3.
         05  FILLER               PIC   N(20)
      VALUE NC"_取消_終了_次ページ".
     03  PF-MSG4.
         05  FILLER               PIC   N(20)
      VALUE NC"_取消_終了_前ページ".
     03  PF-MSG5.
         05  FILLER               PIC   N(20)
      VALUE NC"_取消_終了_前ページ_次ページ".
 01  PF-MSG-AREA-R       REDEFINES     PF-MSG-AREA.
     03  PF-MSG-R   OCCURS   5   PIC   N(20).
*
*メッセージの取得
 01  ERR-MSG-AREA.
     03  ERR-MSG1.
...
*    ＪＡＮ管理マスタを読みながら明細項目をセット
     PERFORM  VARYING IY FROM 1 BY 1 UNTIL IY > 8
                                     OR  RD-FLG = "END"
***     ＪＡＮコード
        MOVE     KA1-F01          TO   DSP-MJANCD(IY)
***     品名
        MOVE     KA1-F03(1:10)    TO   DSP-HINNM1(IY)
        MOVE     KA1-F03(11:10)   TO   DSP-HINNM2(IY)
***     カナ
        MOVE     KA1-F04          TO   DSP-MKANA(IY)
***
        PERFORM  JANKANF1-READ-SEC
     END-PERFORM.
...
*前ページ
         WHEN   "F011"
                        MOVE    CNT-PAGE      TO  IX
                        MOVE    DSP-MJANCD(1) TO  WK-JAN-START(IX)
                        MOVE    DSP-MKANA(1)  TO  WK-KAN-START(IX)
                        MOVE    DSP-MJANCD(8) TO  WK-JAN-END(IX)
                        MOVE    DSP-MKANA(8)  TO  WK-KAN-END(IX)
***   最終ページから前ページへ移動の時の為，RD-FLG(EOF)クリア
                        MOVE    SPACE         TO  RD-FLG
*
***                     前ページ存在チェック
                        PERFORM   MAEP1-SEC
...
*次ページ
         WHEN   "F012"
                        ...
                        MOVE    DSP-MJANCD(8) TO  WK-JAN-END(IX)
                        MOVE    DSP-MKANA(8)  TO  WK-KAN-END(IX)
*******                 MOVE      DSP-MJANCD(8) TO  WK-JAN-END
*******                 MOVE      DSP-MKANA(8)  TO  WK-KAN-END
***                     次ページ存在チェック
                        PERFORM   ATOP1-SEC
***                     次ページ無時
                        IF   ATOP1-FLG = 1
...
*
 DSP-KAKU-EXIT.
     EXIT.
****************************************************************
*      前ページ処理 -前ページ存在チェック-         2.3.1
****************************************************************
 MAEP1-SEC                  SECTION.
     MOVE     "MAEP1-SEC"    TO   S-NAME.
*
*フラグの初期化
     MOVE          ZERO      TO   MAEP1-FLG.
*    CNT-PAGE=1ならば前ページ無し
     IF  CNT-PAGE  NOT = 1
         COMPUTE  IX   =   CNT-PAGE   -  1
***      ＪＡＮコード順
         IF   RD-FILE-FLG   =    1
              MOVE     WK-JAN-START(IX)    TO   KA1-F01
...
***      カナ順
         ELSE
              MOVE     WK-KAN-START(IX)    TO   KA2-F04
              MOVE     WK-JAN-START(IX)    TO   KA2-F01
...
*
 MAEP1-EXIT.
     EXIT.
****************************************************************
*      後ページ処理 -後ページ存在チェック-         2.3.2
****************************************************************
 ATOP1-SEC                 SECTION.
     MOVE     "ATOP1-SEC"    TO   S-NAME.
*
*フラグ初期化
...
***  ＪＡＮコード順
     IF   RD-FILE-FLG   =    1
          IF   DSP-MJANCD(8)  NOT = SPACE
               MOVE     DSP-MJANCD(8)  TO   KA1-F01
               START    JANKANF1  KEY  IS   >    KA1-F01
                 INVALID
                        MOVE  5   TO   ERR-FLG
                        MOVE  1   TO   ATOP1-FLG
...
***  カナ順
     ELSE
          IF   DSP-MKANA(8)  NOT = SPACE  OR
               DSP-MJANCD(8) NOT = SPACE
               MOVE     DSP-MKANA(8)   TO   KA2-F04
               MOVE     DSP-MJANCD(8)  TO   KA2-F01
                 START    JANKANF2  KEY  IS   >    KA2-F04
                                                   KA2-F01
                   INVALID
                          MOVE  5   TO   ERR-FLG
