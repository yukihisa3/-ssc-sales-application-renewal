...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/TSE0040B.COB`
...
*
 ...
 FILE-CONTROL.
***≪入庫ファイル≫*********************************************
     SELECT  NYKFILF    ASSIGN    TO  DA-S-NYKFILF
...
*----≪入庫ファイル≫------------------------------------------*
 FD  NYKFILF.
     COPY    NYKFILF    OF   XFDLIB    JOINING   NYU  PREFIX.
*----≪条件ファイル≫------------------------------------------*
 FD  HJYOKEN.
...
****************************************************************
 ...
 01  PGM-ID             PIC   X(08)    VALUE     "TSE0040B".
*----≪フラグ≫------------------------------------------------*
 01  FLG.
...
***得意先コード／量販_  退避
     IF  END-FLG   NOT  =    "END"
         ...
         MOVE      NYU-F09   TO   WK-TOKCD
         MOVE      NYU-F07   TO   WK-RYONO
     END-IF.
 SYOKI-EXIT.
     EXIT.
****************************************************************
...
****************************************************************
 MAIN-RTN               SECTION.
***得意先コード／量販_ ブレイク判定
     IF     ( NYU-F09   NOT  =    WK-TOKCD ) OR
            ( NYU-F07   NOT  =    WK-RYONO )
***請求合計ファイルＷＲＩＴＥ処理
              PERFORM   SEI-WR-RTN
     END-IF.
***集計処理
...
***明細移動
     IF  A-FLG     =    ZERO
         ...
         INITIALIZE     SEI-REC
         MOVE      NYU-F09   TO   SEI-F01
         MOVE      WK-HIZUKE TO   SEI-F02
         MOVE      NYU-F12   TO   SEI-F03
         MOVE      NYU-F14   TO   SEI-F04
         MOVE      NYU-F07   TO   SEI-F05
         MOVE      ZERO      TO   SEI-F08
         MOVE      NYU-F14   TO   SEI-F10
         MOVE      NYU-F09   TO   SEI-F11
         MOVE      1         TO   A-FLG
     END-IF.
***伝票区分
...
***請求金額・数量　集計
     IF  ( NYU-F01  =  60 )  AND
         ( NYU-F04  =  1  OR  3  OR  5  OR  7  OR  9 )
           COMPUTE  WK-KIN    =  NYU-F22  *  -1   *  NYU-F18
           ADD      WK-KIN    TO     WK-SEIKIN
           COMPUTE  WK-SUU    =  NYU-F18  *  -1
     ...
     IF  ( NYU-F01  =  61 )  AND
         ( NYU-F04  =  0  OR  2  OR  4  OR  6  OR  8 )
           ...
           GO       TO   SEI-SYUUKEI-EXIT
     END-IF.
     ADD   NYU-F18  TO  WK-SURYO.
     COMPUTE  WK-KIN    =      NYU-F22  *  NYU-F18.
     ADD      WK-KIN    TO     WK-SEIKIN.
*
 SEI-SYUUKEI-EXIT.
     EXIT.
