...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY1403L.COB`
...
*帳票出力エリア初期化
     ...
     MOVE     "FSY14031"         TO   PRT-FORM.
     MOVE     "PW"               TO   PRT-PROC.
     MOVE     SPACE              TO   FSY14031.
*伝票枚数カウント
...
*****『区切線』
     MOVE      "FSY14031"    TO      PRT-FORM.
     MOVE      "PW"          TO      PRT-PROC.
     MOVE      ALL "_"       TO      KUGIRI.
     MOVE      "A001"        TO      PRT-CTL.
     MOVE      "TAIL04"      TO      PRT-GRP.
     WRITE      FSY14031.
*****『初期化』
     MOVE     ZERO               TO   GK-AREA.
*****『下段伝票ヘッダ』
...
*伝票タイトルセット
     IF         TAITL-FLG  =  1
                MOVE NC"発注書兼仕入伝票"         TO  TAITL
     ELSE
                MOVE NC"納品書控え兼物品受領書"   TO  TAITL
     END-IF.
*伝票区分
     MOVE       DOT-A03      TO      DENKU.
*取引先ＣＤ
     MOVE       DOT-A06      TO      TORCD.
*取引先名称
     MOVE       DOT-A18      TO      TORNM.
*オーダー
     MOVE       DOT-A05      TO      ORDR.
*納品日
     ...
     MOVE       WK-HEN-DATE  TO      HATYU.
*店舗ＣＤ
     MOVE       DOT-A11      TO      TENCD.
*部門
     MOVE       DOT-A12      TO      BUMON.
*店舗名
     MOVE       DOT-A53      TO      TENNM.
*商品区分
     MOVE       DOT-A13      TO      SYOKU.
*企画番号
     MOVE       DOT-A07      TO      KIKAK.
*伝票番号
     MOVE       DOT-A14      TO      DENNO.
*バーコード
     ...
     MOVE       WK-JANCD     TO      JANCD.
     MOVE       "-"          TO      WK-HEN-JANCD2 WK-HEN-JANCD4.
     MOVE       WK-HEN-JANCD TO      JANNM.
*ＨＥＡＤ部印刷
     MOVE      "FSY14031"    TO      PRT-FORM.
     MOVE      "PW"          TO      PRT-PROC.
     MOVE      "A000"        TO      PRT-CTL.
     MOVE      "HEAD01"      TO      PRT-GRP.
     WRITE      FSY14031.
*
 HEAD-WRT-EXIT.
     EXIT.
...
*商品名称
     MOVE        DOT-A29   TO        SYNM1.
     MOVE        DOT-A30   TO        SYNM2.
*商品ＣＤ
     MOVE        DOT-A31   TO        SYOCD.
*数量
     MOVE        DOT-F50   TO        SURYO.
*原単価
     MOVE        DOT-F172  TO        WK-DEN172.
     MOVE        WK-DEN172A TO       GENKA.
     IF  WK-DEN172B = ZERO
         MOVE    SPACE     TO        GENOP
     ELSE
         MOVE  WK-DEN172B  TO        GENOP
     END-IF.
*原単価
     MOVE        DOT-F173  TO        BAIKA.
 MEIS-010.
*明細印刷
     MOVE      "FSY14031"    TO      PRT-FORM.
     MOVE      "PW"          TO      PRT-PROC.
     MOVE      "A001"        TO      PRT-CTL.
     MOVE      "MEIS01"      TO      PRT-GRP.
     WRITE      FSY14031.
*数量訂正判定
     IF  DOT-F15  NOT =  DOT-F50
     AND WK-MEISAI  NOT =  SPACE
         MOVE   "========"   TO      TEISE1
         MOVE   DOT-F15      TO      TSURYO
*        明細訂正行印刷
         MOVE   "FSY14031"   TO      PRT-FORM
         MOVE   "PW"         TO      PRT-PROC
         MOVE   "A000"       TO      PRT-CTL
         MOVE   "MEIS02"     TO      PRT-GRP
         WRITE   FSY14031
     END-IF.
*合計加算
...
*    システム日付８桁変換
     ...
     CALL      "SKYDTCKB"    USING   LINK-IN-KBN
                                     ...
                                     LINK-OUT-RET
                                     LINK-OUT-YMD8.
     MOVE LINK-OUT-YMD8(1:4) TO      YUYY.
     MOVE LINK-OUT-YMD8(5:2) TO      YUMM.
     MOVE LINK-OUT-YMD8(7:2) TO      YUDD.
*原価金額合計
     MOVE      DOT-A48       TO      GKGEN.
*売価金額合計
     MOVE      DOT-A49       TO      GKBAI.
*合計印刷
     MOVE      "FSY14031"    TO      PRT-FORM.
     MOVE      "PW"          TO      PRT-PROC.
     MOVE      "A001"        TO      PRT-CTL.
     MOVE      "TAIL01"      TO      PRT-GRP.
     WRITE      FSY14031.
*数量訂正判定
     IF  GK-GENKA-HTY  NOT =  GK-GENKA-TEI
         MOVE   "========="  TO      TEISE2  TEISE3
         MOVE   GK-GENKA-TEI TO      TGKGEN
         MOVE   GK-GENKA-TEI TO      TGKGEN
*        明細訂正行印刷
         MOVE   "FSY14031"   TO      PRT-FORM
         MOVE   "PW"         TO      PRT-PROC
         MOVE   "A000"       TO      PRT-CTL
         MOVE   "TAIL02"     TO      PRT-GRP
         WRITE   FSY14031
     END-IF.
*有効期限印字
     MOVE      "FSY14031"    TO      PRT-FORM.
