...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY4402L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*バロー伝票データ（共通）
     SELECT   BJSHIRED  ASSIGN    TO        BJSHIRED
...
***  ｺﾞｳｹｲ
 01  GOUKEI.
     03  SURYOKEI                 PIC  9(05)V9   VALUE  ZERO.
...
***  摘要欄ワーク
 01  WK-TEKIYO.
     03  WK-TEKIYO-01             PIC  X(12)  VALUE  SPACE.
     03  WK-TEKIYO-02             PIC  X(12)  VALUE  SPACE.
****************************************************************
*    プリントエリア                                            *
****************************************************************
*--------------------------------------------------------------*
...
*
 01  HD1.
     ...
     03  HD1-01                   PIC  X(12)  VALUE  SPACE.
*
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      BJSHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     BJSHIRED  AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    バロー以外は読み飛ばし（取引先）
     IF       DEN-F01  NOT =  13020
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
...
************  MOVE      DEN-A12   TO   WK-TEKIYO-01
************  MOVE      DEN-A13   TO   WK-TEKIYO-02
              PERFORM   HEAD-WT-SEC
************  MOVE      DEN-A12   TO   WK-DEN-A12
              MOVE      1         TO   RD-SW
     END-IF.
*
 FL-READ-EXIT.
...
*明細１行目
***  商品名（１）
     MOVE     DEN-A37(1:15)       TO   DT1-01.
***  ＪＡＮコード
     MOVE     DEN-A26             TO   DT1-02.
***  訂正伝票時処理
...
*明細２行目
***  商品名（２）
     MOVE     DEN-A37(16:10)      TO   DT2-01
***  商品コード
     MOVE     DEN-A36             TO   DT2-02.
***  入数
     MOVE     DEN-A27             TO   DT2-022.
***  ケース(発注数)
     MOVE     DEN-A28             TO   DT2-023.
***  数量
     MOVE     DEN-F50             TO        WK-SURYO.
     MOVE     WK-SURYO1           TO        DT2-03.
     MOVE     WK-SURYO2           TO        DT2-032.
     ADD      DEN-F50             TO        SURYOKEI.
*    訂正後数量
     IF       DEN-F15   NOT =     DEN-F50
              MOVE      DEN-F15   TO        DT2-04
     END-IF.
***  原単価
...
*ヘッダ２行目
***  発注者名
     MOVE     "KKﾊﾞﾛｰ       "     TO    HD1-01.
***  取引先名（１）
     MOVE     NC"_サカタのタネ"  TO    HD2-01.
*ヘッダ３行目
***  店舗名称
...
***  取引先名（２）
     MOVE     "045-945-8816"      TO    HD3-07.
***  発注日
     MOVE     DEN-A08             TO    HD3-08.
***  納品日
     MOVE     DEN-A09             TO    HD3-09.
***  便
     MOVE     DEN-A15             TO    HD3-10.
...
****************************************************************
 TAIL-WT-SEC            SECTION.
     ...
     MOVE     SPACE               TO        TL1 TL3 TL4 TLG.
*テール１行目
***  直送先ＣＤ
**** MOVE     WRK-TYOKU-01        TO        TL1-01.
     MOVE     DEN-A11             TO        TL1-01.
***  数量合計
     MOVE     SURYOKEI            TO        WK-SURYO.
     MOVE     WK-SURYO1           TO        TL1-02.
...
***  摘要１セット
***  MOVE     WK-TEKIYO-01        TO        TL3-01.
***  摘要２セット
***  MOVE     WK-TEKIYO-02        TO        TL4-01.
***  特売区分
     MOVE     DEN-A17             TO        TL3-01.
***  企画ＮＯ
     MOVE     DEN-A18             TO        TL4-01.
*テール３行目
...
***  訂正後原価金額合計
     ...
     OR       BAIKINKEI  NOT =  TSBAIKEI
              ...
              MOVE      SPACE     TO        TLG-01
              IF        TSGENKEI  =  ZERO
                MOVE    ZERO      TO        TLG-01(17:1)
              ELSE
                PERFORM  VARYING  I FROM  1  BY  1  UNTIL I > 9
                   COMPUTE  J  =  I  *  2
                   IF  TSGENKEI(I:1)  NOT = ZERO
                       MOVE   1                 TO   ZERO-FLG
                       MOVE   TSGENKEI(I:1)     TO   TLG-01(J:1)
                   ELSE
                       IF      ZERO-FLG   =    1
                               MOVE     ZERO   TO    TLG-01(J:1)
                       END-IF
                   END-IF
                END-PERFORM
              END-IF
***  訂正後売価金額合計
              MOVE      ZERO      TO        ZERO-FLG
              MOVE      SPACE     TO        TLG-02
              IF        TSBAIKEI  =  ZERO
              MOVE      ZERO      TO        TLG-02(17:1)
              ELSE
                PERFORM  VARYING  I FROM  1  BY  1  UNTIL I > 9
                   COMPUTE  J  =  I  *  2
                   IF  TSBAIKEI(I:1)     NOT = ZERO
                       MOVE   1                 TO   ZERO-FLG
                       MOVE   TSBAIKEI(I:1)     TO   TLG-02(J:1)
                   ELSE
                       IF      ZERO-FLG   =    1
                               MOVE     ZERO   TO    TLG-02(J:1)
                       END-IF
                   END-IF
                END-PERFORM
              END-IF
...
***  伝票訂正（二重線）
     IF       GENKINKEI  NOT =  TSGENKEI
     OR       BAIKINKEI  NOT =  TSBAIKEI
              WRITE     PRT-REC   FROM      TL2       AFTER  0
     END-IF.
     WRITE    PRT-REC        FROM      TL3  AFTER  1.
     WRITE    PRT-REC        FROM      TL4  AFTER  1.
***  テール３行目
     WRITE    PRT-REC        FROM      TLG  AFTER  1.
*
*
     MOVE     SPACE     TO        PRT-REC.
...
*テスト印字項目セット
     ...
     MOVE   ALL "9"              TO   HD3-02  HD3-03  HD3-04
                                      HD3-05  HD3-06  HD3-08
                                      HD3-09  DT2-02  DT2-052
                                      DT2-03  DT2-04  DT2-051
                                      DT2-032 DT2-042
                                      DT2-06  DT2-07  DT2-08
                                      TL1-01
                                      TL1-02  TL1-03  TL1-04.
     MOVE   ALL "*"              TO   HD1-01  HD3-01  HD3-07
                                      HD3-10
...
*
     CLOSE    BJSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
