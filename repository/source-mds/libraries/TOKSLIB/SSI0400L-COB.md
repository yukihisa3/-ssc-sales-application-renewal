...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSI0400L.COB`
...
*----<< 支払合計ファイル >>--*
     SELECT   SITGKFG   ASSIGN         DA-01-S-SITGKFG
                        ...
                        STATUS         SSI-ST.
*----<< 請求合計Ｆ >>--*
     SELECT   SETGKFH   ASSIGN         DA-01-VI-SETGKFK1
...
*----<< 支払合計ファイル >>--*
 FD  SITGKFG            LABEL RECORD   IS   STANDARD
     ...
     COPY     SITGKFG   OF        XFDLIB
              JOINING   SSI       PREFIX.
*----<< 請求合計Ｆ >>--*
 FD  SETGKFH            LABEL RECORD   IS   STANDARD.
...
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
 01  PG-ID              PIC  X(08)     VALUE   "SSI0400L".
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HEAD01.
     03  FILLER         CHARACTER TYPE BAIKAKU-1-5.
         ...
         05  FILLER     PIC  X(08)     VALUE  "SSI0400L".
...
*----<< 支払合計ファイル >>--*
 SSI-ERR                SECTION.
     ...
     DISPLAY  "### " PG-ID " SITGKFG  ERROR " SSI-ST " "
...
*----<< 請求合計Ｆ >>--*
 SETGKFH-ERR             SECTION.
     USE AFTER     EXCEPTION PROCEDURE      SETGKFH.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### " PG-ID " SETGKFH  ERROR " SEI-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
...
*伝票区分名称セット
     EVALUATE SSI-F01
         WHEN  11
              MOVE  NC"明細（伝票計）" TO   ME-03
         WHEN  12
              MOVE  NC"前月繰越額"     TO   ME-03
         WHEN  13
              MOVE  NC"本月総仕入額"   TO   ME-03
         WHEN  14
              MOVE  NC"消費税（合計）" TO   ME-03
         WHEN  15
              MOVE  NC"本月支払額"     TO   ME-03
         WHEN  16
              MOVE  NC"現金支払済"     TO   ME-03
         WHEN  17
              MOVE  NC"差引控除額"     TO   ME-03
         WHEN  18
              MOVE  NC"本月振込額"     TO   ME-03
         WHEN  19
              MOVE  NC"翌月繰越額"     TO   ME-03
         WHEN OTHER
              MOVE  ALL NC"＊"         TO   ME-03
     END-EVALUATE.
     MOVE     SSI-F02                  TO   TEN-F52.
*店舗コード
     MOVE     SSI-F03                  TO   ME-05  TEN-F011.
*店舗名称
...
*支払金額
     IF  SSI-F01   =    11
         ...
         ELSE
              COMPUTE   ME-09     =    SSI-F05   *    -1
              COMPUTE   WK-KINGAKU =   SSI-F05   *    -1
         END-IF
     ELSE
         IF   SSI-F15   =    SPACE
              MOVE SSI-F14             TO   ME-09
              MOVE SSI-F14             TO   WK-KINGAKU
         ELSE
              COMPUTE   ME-09     =    SSI-F14   *    -1
...
*--------------------------------------------------------------*
 900-SSI-READ           SECTION.
     ...
     READ     SITGKFG
         ...
         NOT AT END
*             伝票番号
              MOVE      SSI-F09        TO   WK-DENNO-9
              MOVE      WK-DENNO-9     TO   SSI-KEY
              MOVE      "00"           TO   HEN-DENNO-1
              MOVE      SSI-F09        TO   HEN-DENNO-2
              MOVE      HEN-DENNO-0    TO   WK-DENPYO
              MOVE      WK-DENPYO      TO   SSI-KEY
              ADD       1              TO   IN-CNT
     END-READ.
*
 900-SSI-READ-EXIT.
