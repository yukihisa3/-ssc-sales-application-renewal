...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SYP0010I.COB`
...
****  メッセージ情報  ***
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SYP0010I".
...
*取引先Ｍ索引
     ...
     IF     TOR-INV-FLG   =   SPACE
            ...
            MOVE  "D"          TO   EDIT-OPTION OF TORICD
...
*---    品単・部門右詰め処理
     ...
     PERFORM      BUMON-SUB.
*
     IF  (SSYOCD(I) = SPACE) AND  (SHINT1(I) = SPACE) AND
         (SHINT2(I) = SPACE) AND  (SHINT3(I) = SPACE)
          GO                  TO  BODY-CHK-006
     END-IF.
*
*商品名称Ｍ　索引
 BODY-CHK-000.
     MOVE   TORICD            TO  SHO-F01.
     MOVE   SPACE             TO  SHO-F02.
     MOVE   SSYOCD(I)         TO  SHO-F02.
*---    商品変換ＴＢＬ　読込み
...
*画面入力が量販店の時品単が入力されていたらエラー
            IF   ( SHINT1(I)  NOT =  SPACE  ) OR
                 ( SHINT2(I)  NOT =  SPACE  ) OR
                 ( SHINT3(I)  NOT =  SPACE  )
                   IF    ERR-MSG-CD  =  ZERO
                   ...
                   MOVE  "C"  TO     EDIT-CURSOR OF SHINT1(I)
                                     EDIT-CURSOR OF SHINT2(I)
                                     EDIT-CURSOR OF SHINT3(I)
            END-IF
     ELSE
            MOVE   SSYOCD(I)  TO  MEI-F011
            MOVE   SHINT1(I)  TO  WK-HINTAN-1
            MOVE   SHINT2(I)  TO  WK-HINTAN-2
            MOVE   SHINT3(I)  TO  WK-HINTAN-3
...
*---    商品名称Ｍ　読込み
     ...
     IF     MEI-INV-FLG   =  SPACE
            ...
            END-IF
            IF     SYONM1(I) =  SPACE
                   MOVE      MEI-F021    TO    SYONM1(I)
            END-IF
            IF     SYONM2(I) =  SPACE
                   MOVE      MEI-F022(1:7)    TO    SYONM2(I)
                   MOVE      MEI-F022(8:7)    TO    SYONM3(I)
            END-IF
            IF     JANCD(I) =  SPACE
                   MOVE      MEI-F06     TO    JANCD(I)
            END-IF
     ELSE
            IF     SHO-INV-FLG   =  "INV"
                   IF   ERR-MSG-CD  =  ZERO
