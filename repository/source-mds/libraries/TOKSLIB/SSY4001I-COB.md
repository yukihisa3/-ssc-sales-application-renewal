...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY4001I.COB`
...
*----<< 量販データ >>-*
     SELECT   HSERJNL   ASSIGN    TO        DA-01-VI-HSERJNL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      DYNAMIC
                        RECORD    KEY       SER-F01   SER-F02
                        FILE      STATUS    SER-ST1   SER-ST2.
*----<< 画面ファイル >>-*
...
*----<< 伝票データ >>-*
 FD  HSERJNL            BLOCK     CONTAINS   4   RECORDS
                        LABEL     RECORD     IS  STANDARD.
     COPY     HSERJNL   OF   XFDLIB    JOINING   SER  AS   PREFIX.
*----<< 画面ファイル >>-*
 FD  DSPFILE.
     COPY     FSY40011   OF   XMDLIB.
*--------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*--------------------------------------------------------------*
 01  PGM-ID                  PIC  X(08)     VALUE  "SSY4001I".
...
*    03  FILLER              PIC  9(02).
 ...
 01  WK-R00019               PIC  X(05).
 01  FILLER                  REDEFINES   WK-R00019.
     03  WK-HEN-R00019       PIC  9(05).
 01  INDEXES.
     03  I                   PIC  9(02).
     03  J                   PIC  9(02).
     03  K                   PIC  9(02).
 ...
 01  WK-R021-AREA.
     ...
     03  WK-R021             PIC  X(05).
 01  WK-GENKA-KEISAN.
     03  WK-GENKA1           PIC  9(08)  VALUE  ZERO.
...
*
     ...
     IF       SER-OPEN-FLG  =  1  CLOSE    HSERJNL.
...
* 入数入力
     PERFORM     2O0-DSP-R230     UNTIL     GR-NO  NOT  =  23.
* 原価単価（サカタ）
...
* 備考入力
     PERFORM     2K0-DSP-R190     UNTIL     GR-NO  NOT  =  19.
* 伝発入力
     PERFORM     2L0-DSP-R200     UNTIL     GR-NO  NOT  =  20.
* ルートＣＤ入力
     PERFORM     2M0-DSP-R210     UNTIL     GR-NO  NOT  =  21.
* 数量入力
     PERFORM     2N0-DSP-R220     UNTIL     GR-NO  NOT  =  22.
* 確認入力
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     MOVE     SPACE          TO   FSY40011.
     MOVE     HEN-DATE       TO   SDATE.
     MOVE     HEN-TIME       TO   STIME.
     MOVE     WK-DATE        TO   R00008
                                  R00009.
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FSY40011"     TO   DSP-FMT.
...
*  全レコード解放（排他制御用）
     ...
     OPEN     I-O   HSERJNL.
*
     MOVE     R00003         TO   SER-F01.
     MOVE     R00004         TO   SER-F02.
     MOVE     LOW-VALUE      TO   BREAK-KEY.
     START    HSERJNL   KEY  NOT  <  SER-F01     SER-F02
              INVALID   KEY
                   MOVE      HIGH-VALUE     TO   BREAK-KEY
     END-START.
     IF       NEW  NOT  =  HIGH-VALUE
              PERFORM   900-SER-READ
     END-IF.
*
     IF       NEW  NOT  =  HIGH-VALUE
     AND      R00001  =  1
...
**************OR   R00017  =  SPACE
                   MOVE      MEI-F031       TO   R00017
                   MOVE      MEI-F032       TO   R00018
                   MOVE      MEI-F021       TO   YUPNM1
                   MOVE      MEI-F022(1:7)  TO   YUPNM2
                   MOVE      MEI-F022(8:7)  TO   YUPNM3
              END-IF
     END-IF.
*
     MOVE     13             TO   GR-NO.
...
*    LEVEL  3      入数入力                                    *
*--------------------------------------------------------------*
 2O0-DSP-R230           SECTION.
*
     MOVE     G002           TO   GUIDE.
     MOVE     "IRISU "       TO   WK-GRP.
     PERFORM  900-DSP-READ.
*
     EVALUATE DSP-FNC
     WHEN     PF04
              MOVE      0         TO   GR-NO
              GO   TO   2O0-DSP-R230-EXIT
     WHEN     PF06
              MOVE      14        TO   GR-NO
              GO   TO   2O0-DSP-R230-EXIT
     WHEN     ENT
...
*### 2000/03/21 NAV END   ###*
     ...
     WHEN     OTHER
              ...
              GO   TO   2G0-DSP-R150-EXIT
     END-EVALUATE.
*原価単価入力チェック（入力済の場合，原価の一割増金額を計算）
     IF       R00015  NOT  NUMERIC
     OR       R00015  =    ZERO
              MOVE      21        TO   ERR-FLG
              GO                  TO   2G0-DSP-R150-EXIT
     ELSE
              COMPUTE  WK-GENKA1 ROUNDED  =   R00015  *  1.1
              MOVE     WK-GENKA1  TO   R00029
     END-IF.
*
     MOVE     16             TO   GR-NO.
 2G0-DSP-R150-EXIT.
...
*
     ...
     END-EVALUATE.
*
     MOVE     R00016(8:5)    TO   WK-R00019.
     IF       R00019  NOT =  WK-HEN-R00019
     AND      R00016(1:4) =  "2000"
              MOVE      22        TO   ERR-FLG
              GO   TO   2H0-DSP-R160-EXIT
     END-IF.
...
*
     ...
     EVALUATE   R00001
         WHEN  1
              PERFORM   900-SER-WRITE
         WHEN  2
              PERFORM   900-SER-WRITE
         WHEN  3
              PERFORM   900-SER-DELETE
     END-EVALUATE.
*
     IF  KKNN  =  "Y"
         MOVE           ZERO        TO       B-FLG
...
*    LEVEL  4     量販データ登録・修正　　　　　　　　　　　　 *
*--------------------------------------------------------------*
 900-SER-WRITE          SECTION.
     INITIALIZE    SER-REC.
     MOVE     R00003         TO   SER-F01.
     MOVE     R00004         TO   SER-F02.
     PERFORM  900-SER-RAN-READ.
*
     MOVE     R00002         TO   SER-F03.
     MOVE     R00005         TO   SER-F04.
     MOVE     R00006         TO   SER-F05.
     MOVE     R00007         TO   SER-F63.
     MOVE     "3"            TO   LINK-IN-KBN.
     MOVE     R00008         TO   LINK-IN-YMD6.
     CALL     "SKYDTCKB"  USING   LINK-IN-KBN
                                  LINK-IN-YMD6
     ...
     CALL     "SKYDTCKB"  USING   LINK-IN-KBN
                                  ...
                                  LINK-OUT-RET
                                  LINK-OUT-YMD.
     MOVE    LINK-OUT-YMD   TO   SER-F072.
*--
     MOVE     R00010         TO   SER-F08.
     MOVE     R00011         TO   SER-F09.
     MOVE     R00012         TO   SER-F10.
     MOVE     R00013         TO   SER-F11.
     MOVE     R00016         TO   SER-F121.
     MOVE     R00017         TO   SER-F1211.
     MOVE     R00018         TO   SER-F1212.
     MOVE     R00015         TO   SER-F131.
     MOVE     R00019         TO   SER-F132.
     MOVE     R00025         TO   SER-F141.
     MOVE     R00026         TO   SER-F142.
     MOVE     R00027         TO   SER-F15.
     MOVE     RUTOCD         TO   SER-F17.
     MOVE     YUPNM1         TO   SER-F641.
     MOVE     YUPNM2         TO   SER-F642.
     MOVE     YUPNM3         TO   SER-F643.
     MOVE     R00029         TO   SER-F65.
     MOVE     IRISU          TO   SER-F66.
     PERFORM  VARYING  I  FROM  1  BY  1  UNTIL  I  >  2
         PERFORM  VARYING  J  FROM  1  BY  1  UNTIL  J  > 10
              COMPUTE   K  =  (I - 1) * 10 + J
              IF   WK-TENCD (I J)  IS  NUMERIC
                   MOVE      WK-RTJYUN(I J)    TO   SER-F161 (K)
                   MOVE      WK-TENCD (I J)    TO   SER-F162 (K)
                   MOVE      WK-SURYO (I J)    TO   SER-F163 (K)
              ELSE
                   MOVE      0                 TO   SER-F161 (K)
                                                    SER-F162 (K)
                                                    SER-F163 (K)
              END-IF
         END-PERFORM
     ...
     END-IF.
     MOVE     0              TO   SER-F58.
     MOVE     HENKAN-TI      TO   SER-F59.
     MOVE     R00028         TO   SER-F60.
     MOVE     R00024         TO   SER-F61.
     MOVE     R00020         TO   SER-F62 (1:8)
     MOVE     R00021         TO   SER-F62 (9:5)
     MOVE     R00022         TO   SER-F62 (14:2)
     MOVE     R00023         TO   SER-F62 (16:1)
     MOVE     "3"            TO   LINK-IN-KBN.
...
*    LEVEL  4     量販データ読込み　　　　　　　　　　　　　　 *
*--------------------------------------------------------------*
 900-SER-TO-DSP         SECTION.
     MOVE     SER-F02        TO   YOBI-NO.
     MOVE     SER-F03        TO   R00002.
     MOVE     SER-F04        TO   R00005.
     MOVE     SER-F05        TO   R00006.
*-- 1999/10/05 ７桁注文番号から１５桁注文番号へ
*****MOVE     SER-F06        TO   R00007.
     MOVE     SER-F63        TO   R00007.
*--
     MOVE     SER-F071       TO   R00008.
     MOVE     SER-F072       TO   R00009.
     MOVE     SER-F08        TO   R00010.
     MOVE     SER-F09        TO   R00011.
     MOVE     SER-F10        TO   R00012.
     MOVE     SER-F11        TO   R00013
                                  TOK-F01.
     PERFORM  900-TOK-READ.
     IF       INV-FLG  =  0
              MOVE      TOK-F03   TO   R00014
     ELSE
              MOVE      NC"＊＊＊＊＊＊＊＊＊＊"  TO   R00014
     END-IF.
     MOVE     SER-F131       TO   R00015.
     MOVE     SER-F121       TO   R00016.
     MOVE     SER-F1211      TO   R00017.
     MOVE     SER-F1212      TO   R00018.
     MOVE     SER-F132       TO   R00019.
     MOVE     SER-F62 (1:8)  TO   R00020.
     MOVE     SER-F62 (9:5)  TO   R00021.
     MOVE     SER-F62 (14:2) TO   R00022.
     MOVE     SER-F62 (16:1) TO   R00023.
     MOVE     SER-F61        TO   R00024.
     MOVE     SER-F141       TO   R00025.
     MOVE     SER-F142       TO   R00026.
     MOVE     SER-F15        TO   R00027.
     MOVE     SER-F60        TO   R00028.
     MOVE     SER-F17        TO   RUTOCD.
     MOVE     SER-F641       TO   YUPNM1.
     MOVE     SER-F642       TO   YUPNM2.
     MOVE     SER-F643       TO   YUPNM3.
     MOVE     SER-F65        TO   R00029.
     MOVE     SER-F66        TO   IRISU.
     PERFORM  VARYING  I  FROM  1  BY  1  UNTIL  I  >  2
         PERFORM  VARYING  J  FROM  1  BY  1  UNTIL  J  > 10
              COMPUTE   K  =  (I - 1) * 10 + J
              IF   SER-F161 (K)  NOT  =  0
                   MOVE      SER-F161 (K)      TO   WK-RTJYUN(I J)
                                                    RTJYUN(I J)
                   MOVE      SER-F162 (K)      TO   WK-TENCD (I J)
                                                    TENCD(I J)
                   MOVE      SER-F163 (K)      TO   WK-SURYO (I J)
                   IF        SER-F163 (K)  NUMERIC
                   AND       SER-F163 (K)  NOT =   ZERO
                             MOVE SER-F163 (K) TO   SURYO(I J)
                   END-IF
****************************************************SURYO(I J)
              END-IF
         END-PERFORM
...
*--------------------------------------------------------------*
 900-SER-RAN-READ       SECTION.
     MOVE     0              TO   INV-FLG.
     READ     HSERJNL   INVALID  KEY
              MOVE      1         TO   INV-FLG
              GO   TO   900-SER-RAN-READ-EXIT
     END-READ.
 900-SER-RAN-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  ALL   伝票データＦ　　　ＲＥＡＤ　　　　　　　　　 *
*--------------------------------------------------------------*
 900-SER-READ           SECTION.
     READ     HSERJNL   NEXT      AT   END
              MOVE      HIGH-VALUE     TO   NEW
              GO   TO   900-SER-READ-EXIT
     END-READ.
     IF       SER-F01   NOT  =  R00003
     OR       SER-F02   NOT  =  R00004
              MOVE      HIGH-VALUE     TO   NEW
              GO   TO   900-SER-READ-EXIT
     END-IF.
*
     MOVE     SER-F01        TO   NEW-011.
     MOVE     SER-F02        TO   NEW-012.
 900-SER-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  ALL   伝票データＦ　　　ＲＥＡＤ　　　　　　　　　 *
...
*************DISPLAY "WK-SURYO = " WK-SURYO(X Y) UPON CONS
