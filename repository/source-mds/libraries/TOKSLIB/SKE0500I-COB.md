...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKE0500I.COB`
...
****  メッセージ情報  ***
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SKE0500I".
 ...
 BODY-SUB            SECTION.
     ...
     PERFORM  VARYING  IXB   FROM  1  BY  1
         ...
         IF   SHOCD(IXB)   =   SPACE
              ...
              MOVE   "X"   TO   EDIT-STATUS  OF  IRISU (IXB)
              MOVE   "X"   TO   EDIT-STATUS  OF  HAIBAN(IXB)
         END-IF
     END-PERFORM.
     PERFORM         DSP-READ-SUB.
* アテンション判定
...
*
     PERFORM  VARYING   IXA  FROM  1  BY  1
         ...
         END-IF
*        廃盤区分
         IF HAIBAN(IXA)    NOT =     SPACE
           IF HAIBAN(IXA)  NOT =     "1"
              IF   ERR-MSG-CD     =    ZERO
                   MOVE      14   TO   ERR-MSG-CD
                   MOVE  "C" TO   EDIT-CURSOR  OF  HAIBAN(IXA)
              END-IF
              MOVE   "R"     TO   EDIT-OPTION  OF  HAIBAN(IXA)
           END-IF
         END-IF
     END-PERFORM.
*
     ...
     PERFORM  VARYING   IXA       FROM    1  BY  1
         ...
         IF   SHOCD(IXA)     NOT =     SPACE
              ...
              IF   INVALID-FLG    =    1
                   CONTINUE
              ELSE
                   IF ( JANCD (IXA)    NOT =     MEI-F06 ) OR
                      ( IRISU (IXA)    NOT =     MEI-F07 ) OR
                      ( HAIBAN(IXA)    NOT =     MEI-F08 )
                        MOVE JANCD (IXA)    TO   MEI-F06
                        MOVE IRISU (IXA)    TO   MEI-F07
                        MOVE HAIBAN(IXA)    TO   MEI-F08
                        REWRITE   MEI-REC
                   END-IF
              END-IF
         END-IF
...
*
     ...
     MOVE   "M"     TO   EDIT-OPTION  OF  HAIBAN(IXB).
     MOVE   SPACE   TO   EDIT-CURSOR  OF  HAIBAN(IXB).
*
 EDIT-SET-GYO-END.
     EXIT.
*==========================================================*
