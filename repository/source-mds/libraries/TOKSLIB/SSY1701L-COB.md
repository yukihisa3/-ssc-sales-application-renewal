...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY1701L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*富士シティオ伝票データ（共通）　　
     SELECT   FSHIRED  ASSIGN    TO        FSHIRED
...
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
     ...
     03  WK-TL2-01                PIC  X(20)     VALUE  SPACE.
 01  SEC-NAME.
     03  FILLER                   PIC  X(05)     VALUE " *** ".
     03  S-NAME                   PIC  X(30).
 01  SAVE-AREA.
     03  WK-DEN-A31               PIC  9(01)     VALUE   ZERO.
     03  WK-DEN-A32               PIC  X(01)     VALUE   SPACE.
     03  WK-DEN-A33               PIC  X(06)     VALUE   SPACE.
     03  WK-DEN-A34               PIC  9(09)     VALUE   ZERO.
***  ﾃﾞﾝﾋﾟﾖｳ ｷ-
 01  WRK-DENNO.
     03  DENNO1                   PIC  9(09).
***  ﾒｯｾｰｼﾞｴﾘｱ
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      FSHIRED.
...
*
     ...
     END-READ.
*    富士シティオ以外は読み飛ばし（取引先）
     IF       DEN-F01  NOT =  221582
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
     IF       DEN-A021 NOT =  WK-DEN-NO
              ADD    1        TO      WK-MAI
              MOVE   DEN-A021 TO      WK-DEN-NO
     END-IF.
*
 DEN-CNT-EXIT.
     EXIT.
...
*
 ...
 READ-010.
     IF       ( SYU-FLG     = 1      )  AND
              ...
              GO        TO        FL-READ-EXIT.
     IF       DEN-A021   =    MD03
              MOVE      DEN-A021  TO        DEN-NO
              MOVE      1         TO        SYU-FLG.
 READ-020.
*
     IF       RD-SW     =    0
              MOVE      DEN-A021  TO   DENNO1
              PERFORM   HEAD-WT-SEC
              MOVE      1         TO   RD-SW
     END-IF.
*
...
*
*ヘッダ項目退避
     MOVE     DEN-A31             TO   WK-DEN-A31.
     MOVE     DEN-A32             TO   WK-DEN-A32.
     MOVE     DEN-A33             TO   WK-DEN-A33.
...
*明細２行目
***  商品名
     MOVE     DEN-A18             TO        DT2-01.
***  規格
     MOVE     DEN-A38             TO        DT2-02.
***  商品コード（ＪＡＮ）
     MOVE     DEN-A14             TO        DT2-03.
***  入数
     MOVE     DEN-A15             TO        DT2-04.
***  ケース（発注数）
     MOVE     DEN-A36             TO        DT2-05.
***  発注単位
     MOVE     DEN-A16             TO        DT2-06.
***  数量
...
*ヘッダ１行目
***  社名（富士シティオ）
     MOVE     DEN-A09             TO        HD1-01.
***  取引先名称（サカタのタネ）
     MOVE     DEN-A11             TO        HD1-02.
*ヘッダ２行目　
***  店舗名称
     MOVE     DEN-A10             TO        HD2-01.
***  店舗コード
     MOVE     DEN-A042            TO        HD2-02.
***  分類コード
     MOVE     DEN-A05             TO        HD2-03.
***  伝票区分
     MOVE     DEN-A06             TO        HD2-04.
***  伝票番号
     MOVE     DEN-A021            TO        HD2-05.
***  取引先コード
     MOVE     DEN-A011            TO        HD2-06.
***  取引先電話番号
     MOVE     DEN-A13             TO        HD2-07.
***  発注日
     MOVE     DEN-A07             TO        HD2-08.
***  納品日
     MOVE     DEN-A08             TO        HD2-09.
***  便
     MOVE     DEN-A35             TO        HD2-10.
*
...
***  販促区分
     EVALUATE   WK-DEN-A33
         ...
         WHEN   "1"
                MOVE   "ﾄｸﾊﾞｲ"    TO        TL2-01
         WHEN   "2"
                MOVE   "ｽﾎﾟｯﾄ"    TO        TL2-01
         WHEN   "3"
                MOVE   "ｺﾞｳｶﾞｲ"   TO        TL2-01
         WHEN   "4"
                MOVE   "ｵｽｽﾒ"     TO        TL2-01
         WHEN   "5"
                MOVE   "ｷｶｸ"      TO        TL2-01
     ...
     END-EVALUATE.
***  販促管理_
     MOVE   WK-DEN-A34            TO        TL2-02.
***  承り発注区分
     EVALUATE   WK-DEN-A32
         WHEN   "0"
                MOVE   SPACE      TO        TL2-03
         WHEN   "1"
                MOVE   "ｳｹﾀﾏﾜﾘ"   TO        TL2-03
         WHEN   OTHER
                MOVE   SPACE      TO        TL2-03
     END-EVALUATE.
*
...
***  テール３行目
     WRITE    PRT-REC        FROM      TL3  AFTER    2.
***  テール４行目
...
*項目クリア
     MOVE     SPACE               TO   HD1 HD2
                                       ...
                                       TL1 TL2 TL3.
*テスト印字項目セット
     MOVE   ALL "9"              TO   HD2-02  HD2-04  HD2-05
                                      HD2-06  HD2-08  HD2-09
                                      DT2-04  DT2-05  DT2-071
                                      DT2-072 DT2-081 DT2-082
                                      DT2-091 DT2-10  DT2-11
                                      TL1-01  TL1-02  TL2-02
                                      DT2-092 DT2-12.
     MOVE   ALL "*"              TO   HD1-01  HD1-02  HD2-01
                                      HD2-03  HD2-07  HD2-10
                                      DT1-01  DT1-02  DT2-01
                                      DT2-02  DT2-03  DT2-06
                                      TL2-01  TL2-03  TL3-01.
*
*ヘッダ部
     WRITE    PRT-REC          FROM   HD1       AFTER    6.
     WRITE    PRT-REC          FROM   HD2       AFTER    2.
