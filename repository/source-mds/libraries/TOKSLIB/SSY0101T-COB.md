...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY0101T.COB`
...
*----<< 条件ファイル >>-*
     SELECT   HJYOKEN   ASSIGN    TO        DA-01-VI-JYOKEN1
                        ...
                        FILE      STATUS    JYO-ST1.
*----<< 伝票データ >>-*
     SELECT   HDENJNL   ASSIGN    TO        DA-01-VI-SHTDENL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      DYNAMIC
...
*----<< 画面ファイル >>-*
 FD  DSPFILE.
     COPY     FSK010A   OF   XMDLIB.
*--------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*--------------------------------------------------------------*
 01  PGM-ID                  PIC  X(08)     VALUE  "OSKT010A".
*----<< ﾃﾞｨｽﾌﾟﾚｲ ｺﾝﾄﾛｰﾙ ｴﾘｱ >>-*
 01  DSP-CNTL.
     03  DSP-FMT             PIC  X(08).
     03  DSP-GRP             PIC  X(08).
...
*画面退避用
     COPY   FSK010A  OF XMDLIB  JOINING   SAV  AS   PREFIX.
*
***  93/05/08  START  ***
 01  SAVE-AREA.
...
***  94.05.16  END   ***
     ...
     MOVE      SPACE     TO   FSK010A   TABLE-AREA.
     INITIALIZE               TBL-XX.
     MOVE      ZERO      TO   WK-TAIHI.
...
*
     ...
     MOVE     0              TO   DEN-F03.
     MOVE     0              TO   END-FLG.
     START    HDENJNL   KEY  NOT  <  DEN-F01     DEN-F02
                                     DEN-F04     DEN-F051
                                     DEN-F03
         INVALID   KEY
              MOVE      1         TO   END-FLG
...
*##99/09/09 NAV-ASSIST T.T START ##*必ず頁を１に戻す
     MOVE     1       TO      P-CNT.
*##99/09/09 NAV-ASSIST T.T END   ##*
     IF       KKNN  =  "Y"
     AND   (  R00001  NOT  =  1
       OR   ( R00001  =  1
...
***  94.05.16  END   ***
              MOVE      2              TO   GR-NO
************  DISPLAY "(1) = " R10201(1) UPON CONS
************  DISPLAY "(2) = " R10201(2) UPON CONS
************  DISPLAY "(3) = " R10201(3) UPON CONS
************  DISPLAY "(4) = " R10201(4) UPON CONS
************  DISPLAY "(5) = " R10201(5) UPON CONS
************  DISPLAY "(6) = " R10201(6) UPON CONS
...
*--------------------------------------------------------------*
 900-DEN-DELETE         SECTION.
     ...
     MOVE     0              TO   DEN-F03.
     MOVE     0              TO   END-FLG.
     START    HDENJNL  KEY  NOT  <  DEN-F01  DEN-F02  DEN-F04
                                    DEN-F051 DEN-F03
        INVALID  KEY
              MOVE      1        TO   END-FLG
        NOT   INVALID  KEY
