...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKY9601I.COB`
...
*
*---<<  ユポラベルＦ  >>---*
     SELECT   ZJANCD1  ASSIGN    TO        DA-01-VI-ZJANCD1
...
*    COPY     FKY96011  OF        XMDLIB.
*---<<  ユポラベルＦ  >>---*
 FD  ZJANCD1.
     COPY     ZJANCDT1 OF        XFDLIB
              JOINING   JAN       PREFIX.
*---<<  商品コード変換ＴＢＬ >>---*
 FD  SHOTBL1.
...
****  フラグ  ***
 01  PSW-AREA.
     ...
     02  JAN-END-FLG         PIC  X(03)  VALUE SPACE.
     02  JAN-INV-FLG         PIC  X(03)  VALUE SPACE.
...
****  メッセージ情報  ***
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SKY9601I".
...
**
  FILEERR-SEC2           SECTION.
      USE       AFTER    EXCEPTION
                         PROCEDURE   ZJANCD1.
      MOVE     "ZJANCD1"       TO   ERR-FL-ID.
      MOVE      JAN-STATUS      TO   ERR-STCD.
      DISPLAY   MSG-ABEND1    UPON   CONS.
      DISPLAY   MSG-ABEND2    UPON   CONS.
      MOVE      4000            TO   PROGRAM-STATUS.
      STOP      RUN.
...
*----------------------------------------------------------*
 DSP-INIT-SUB         SECTION.
     MOVE     SPACE          TO   FKY96011.
     MOVE    "FKY96011"      TO   DSP-FORMAT.
     MOVE    "CL"            TO   DSP-PROC.
...
* アテンション判定
     ...
     END-EVALUATE.
*
     MOVE   R00002          TO    JAN-F02.
     MOVE   SPACE           TO    JAN-F03.
     MOVE   R10101(1)       TO    JAN-F03.
     MOVE   SERI1(1)        TO    JAN-F011.
     MOVE   SERI2(1)        TO    JAN-F012.
     START  ZJANCD1 KEY IS >= JAN-F02 JAN-F03 JAN-F011 JAN-F012
           INVALID
     ...
     END-START.
*ユポラベルＦ　ＲＥＡＤ
     MOVE      SPACE        TO    JAN-END-FLG.
     PERFORM   JAN-READ-SUB.
     IF      JAN-END-FLG   =  "END"
             MOVE      14   TO    ERR-MSG-CD
             GO             TO    BODY-SYUSEI-EXIT
     ELSE
                 ...
                 IF    SYORI-F   =   2
                    ...
                    ELSE
                        ...
                        ELSE
                             MOVE  R00002       TO  JAN-F02
                             MOVE  SPACE        TO  JAN-F03
                             MOVE  WK-SYOCD-ATO TO  JAN-F03
                             MOVE  WK-SERI1-ATO TO  JAN-F011
                             MOVE  WK-SERI2-ATO TO  JAN-F012
                             MOVE  SPACE        TO  JAN-END-FLG
                             START  ZJANCD1
                               INVALID
                                 MOVE  12  TO   ERR-MSG-CD
...
*    DISPLAY "WK-CHKMEI-G = " WK-CHKMEI-G(I) UPON CONS.
     IF     WK-CHKMEI-G(I)  =  1
            MOVE   R00002    TO    JAN-F02
            MOVE   R10101(I) TO    JAN-F03
            MOVE   SERI1(I)  TO    JAN-F011
            MOVE   SERI2(I)  TO    JAN-F012
            READ   ZJANCD1
               INVALID
                 CONTINUE
               NOT  INVALID
...
*
     PERFORM   VARYING  I  FROM  1 BY  1
                           UNTIL   (     I       >   5   ) OR
                                   ( JAN-END-FLG = "END" )
               MOVE    JAN-F11          TO       R10001(I)
               IF      JAN-F03  =  JAN-F04
                       MOVE  JAN-F03      TO    R10101(I)
                       MOVE  JAN-F05      TO    WK-HINTAN
               ...
               END-IF
               MOVE    JAN-F05          TO       WK-HINTAN
               MOVE    JAN-F06          TO       R10601(I)
               MOVE    JAN-F071         TO       R10501(I)
               MOVE    JAN-F072(1:7)    TO       R10701(I)
               MOVE    JAN-F072(8:7)    TO       R11101(I)
               MOVE    JAN-F08          TO       R10801(I)
               MOVE    JAN-F09          TO       R10901(I)
               MOVE    JAN-F10          TO       R11001(I)
               MOVE    JAN-F011         TO       SERI1(I)
               MOVE    JAN-F012         TO       SERI2(I)
...
*--- 次頁ポインタの格納 ---*
     IF     JAN-END-FLG   =   SPACE
            MOVE    JAN-F03  TO    WK-SYOCD-ATO
            MOVE    JAN-F011 TO    WK-SERI1-ATO
            MOVE    JAN-F012 TO    WK-SERI2-ATO
     END-IF.
 MEISAI-DSP-EXIT.
     EXIT.
*----------------------------------------------------------*
...
*--- 2000/02/28  NAV  END   ---*
                   ...
                   INITIALIZE            JAN-REC
                   MOVE    R00002        TO   JAN-F02
                   MOVE    WK-RSYO(I)    TO   JAN-F03
                   MOVE    WK-SYO(I)     TO   JAN-F04
                   MOVE    WK-HTAN(I)    TO   JAN-F05
                   MOVE    R10601(I)     TO   JAN-F06
                   MOVE    R10501(I)     TO   JAN-F071
****************** MOVE    R10701(I)     TO   JAN-F072
                   MOVE    R10701(I)     TO   WK-NM21
                   MOVE    R11101(I)     TO   WK-NM22
                   MOVE    WK-NM2        TO   JAN-F072
                   MOVE    R10801(I)     TO   JAN-F08
                   IF      R10901(I)   NOT    NUMERIC
                           MOVE ZERO     TO   JAN-F09
                   ELSE
                           MOVE R10901(I) TO  JAN-F09
                   END-IF
                   MOVE    R11001(I)     TO   JAN-F10
                   MOVE    R10001(I)     TO   JAN-F11
                   MOVE    SERI1(I)      TO   JAN-F011
                   MOVE    SERI2(I)      TO   JAN-F012
*
                   WRITE   JAN-REC
...
*      2.2.1.1   ユポラベルＦ　ＲＥＡＤ
*----------------------------------------------------------*
 JAN-READ-SUB           SECTION.
*
     READ    ZJANCD1
          NEXT  AT  END
             MOVE   "END"    TO   JAN-END-FLG
             GO              TO   JAN-READ-EXIT
     END-READ.
*取引先コード
     IF      JAN-F02  =   WK-TORICD
             CONTINUE
     ELSE
             MOVE   "END"    TO   JAN-END-FLG
     END-IF.
