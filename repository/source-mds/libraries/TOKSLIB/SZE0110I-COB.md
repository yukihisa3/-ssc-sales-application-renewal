...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SZE0110I.COB`
...
***************************************************************
 FILE-CONTROL.
*在庫マスタ（商品コード・倉庫）
     SELECT      ZENZAIL2  ASSIGN    TO        DA-01-VI-ZENZAIL2
...
*在庫マスタ（カナ名称・倉庫・商品コード）
     SELECT      ZENZAIL3  ASSIGN    TO        DA-01-VI-ZENZAIL3
                           ORGANIZATION        INDEXED
                           ACCESS    MODE      SEQUENTIAL
                           RECORD    KEY       ZAI3-F30
                                               ZAI3-F02
...
*
 01  WK-AREA.
     ...
     03  WK-E000-BUMO-CD         PIC X(04)  VALUE SPACE.
     ...
     03  E000-KEY.
       ...
       05    E000-KEY-BUMO-CD    PIC X(04).
     03  PF07-KEY                PIC X(43).
     03  PF08-KEY                PIC X(43).
     03  PF11-KEY.
     ...
     03  PF12-KEY.
       ...
       05    PF12-KEY-BUMO-CD    PIC X(04).
     03  WK-ZAI-BUMO-CD          PIC X(04)  VALUE SPACE.
     ...
     03  WK-ZAI-KEY.
       ...
       05    WK-ZAI-KEY-BUMO-CD  PIC X(04).
     ...
     03  WK-HEAD-KEY.
       ...
       05    WK-HEAD-KEY-BUMO-CD PIC X(04).
     03  WK-MEI-F02G.
...
******************************************************************
 DECLARATIVES.
 ZAI2-ERR               SECTION.
     USE           AFTER     EXCEPTION PROCEDURE         ZENZAIL2.
     ...
     STOP          RUN.
 ZAI3-ERR               SECTION.
     USE           AFTER     EXCEPTION PROCEDURE         ZENZAIL3.
...
*商品コード・カナ名称
     ...
     WHEN     SHOCD          NOT  =    SPACE
              ...
              END-IF
              IF    BUMON         NOT  =   SPACE
                    MOVE  BUMON        TO   PF12-KEY-BUMO-CD
              END-IF
...
*
     IF       INV-SW              =    1
     OR       WK-ZAI-KEY(1:33)    <    WK-HEAD-KEY(1:33)
              MOVE  8             TO   ERR-NO
                                  GO   TO   BEFORE-REC-EXIT
     END-IF.
     MOVE     E000-KEY            TO   PF12-KEY.
     ...
     MOVE     WK-ZAI-BUMO-CD      TO   WK-E000-BUMO-CD.
     ...
     IF       SOKCD               =   SPACE
         ...
         MOVE  WK-E000-BUMO-CD    TO   WK-ZAI-KEY-BUMO-CD
     ...
     PERFORM       VARYING   IX01      FROM      IX01 BY   1
                   UNTIL     IX01           >    7
                   OR        INV-SW         =    1
                   OR   E000-KEY(1:33)  NOT  =    WK-ZAI-KEY(1:33)
     ...
     END-PERFORM.
     IF       INV-SW              =    0
     AND      E000-KEY(1:33)      =    WK-ZAI-KEY(1:33)
              CONTINUE
     ELSE
...
*
     IF       INV-SW              =    0
     AND      WK-ZAI-KEY(1:33)    >=   WK-HEAD-KEY(1:33)
...
*****DISPLAY "PF12-KEY = " PF12-KEY UPON CONS.
     PERFORM            ZAIMS-START-SEC.
     IF       INV-SW              =    1
              MOVE  9             TO   ERR-NO
                                  GO   TO   NEXT-REC-EXIT
...
*
     ...
     PERFORM       VARYING   IX01      FROM      IX01 BY   -1
                   UNTIL     IX01           <    1
                   OR        INV-SW         =    1
                   OR   E000-KEY(1:33) >    WK-ZAI-KEY(1:33)
...
*********DISPLAY "ZAI-F04 = " ZAI-F04 UPON CONS
         ...
         PERFORM        ZAIMS-READ-SEC
     END-PERFORM.
*********DISPLAY "TTL-F04 = " TTL-F04 UPON CONS.
 TOTAL-ADD-EXIT.
     EXIT.
******************************************************************
*             前レコード検索処理
...
******************************************************************
 ZAIMS3-START-RVS       SECTION.
     MOVE     WK-ZAI-KEY-BUMO-CD  TO   ZAI3-F00.
 ...
 ZAIMS2-READ-SEC        SECTION.
     READ     ZENZAIL2        NEXT
         ...
         NOT  AT   END
              ...
              IF   BUMON     NOT  =   "    "
                   IF   BUMON     NOT  =    ZAI2-F00
                                  GO   TO   ZAIMS2-READ-SEC
                   END-IF
                   MOVE  ZAI2-F00      TO   WK-ZAI-KEY-BUMO-CD
              END-IF
              MOVE  ZAI2-F00      TO   WK-ZAI-BUMO-CD
 ...
 ZAIMS3-READ-SEC        SECTION.
     READ     ZENZAIL3        NEXT
         ...
         NOT  AT   END
              MOVE  SPACE         TO   WK-ZAI-KEY
              IF   BUMON     NOT  =   "  "
                   IF   BUMON     NOT  =    ZAI3-F00
                                  GO   TO   ZAIMS3-READ-SEC
                   END-IF
                   MOVE  ZAI3-F00      TO   WK-ZAI-KEY-BUMO-CD
              END-IF
              MOVE  ZAI3-F00      TO   WK-ZAI-BUMO-CD
