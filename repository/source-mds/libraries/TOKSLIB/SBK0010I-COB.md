...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SBK0010I.COB`
...
*フラグ領域
 01  FLG-AREA.
     ...
     03  WK-JYU-FILNM             PIC  X(08)  VALUE  SPACE.
     ...
     03  WK-JYU-TOKNM             PIC  X(15)  VALUE  SPACE.
*ワーク領域
 01  WRK-AREA.
***  プログラムスイッチ（画面遷移制御）
...
*
 ...
 01  PARA-TOKNM            PIC X(15).
 01  PARA-FILNM            PIC X(08).
**************************************************************
 PROCEDURE             DIVISION USING PARA-TOKCD PARA-TOKNM
                                      PARA-FILNM.
**************************************************************
 DECLARATIVES.
 EOS-ERR                   SECTION.
...
*受信起動    (2.4)
         WHEN      "4"       PERFORM   BACKUP-START-SEC
*以外
         WHEN      OTHER     CONTINUE
     END-EVALUATE.
*
        ...
        PERFORM VARYING Y FROM 1 BY 1 UNTIL Y > 15
           ...
           IF  DSP-SENTAK = TBL-SEQ(X Y)
               ...
               OR  TBL-MTOKCD(X Y) NUMERIC
                   ...
                   MOVE TBL-MFILE (X Y)    TO WK-JYU-FILNM
                   ...
                   IF      HTOKMS-INV-FLG NOT = "INV"
                           ...
                           MOVE  TOK-F04     TO WK-JYU-TOKNM
                   ELSE
                           MOVE  ALL NC"＊"  TO DSP-JTORNM
                           MOVE  ALL "*"     TO WK-JYU-TOKNM
                   END-IF
               ELSE
                   MOVE  5  TO ERR-FLG
                   MOVE "R" TO EDIT-OPTION OF DSP-SENTAK
...
*項目画面セット
     PERFORM VARYING Y FROM 1 BY 1 UNTIL Y > 15
         ...
         AND TBL-MTOKCD(P-CNT Y)  NUMERIC
             ...
             MOVE    TBL-MFILE (P-CNT Y) TO DSP-MFILE (Y)
...
*             受信処理スタート                                 *
****************************************************************
 BACKUP-START-SEC      SECTION.
     MOVE     "BACKUP-START-SEC"  TO   S-NAME.
*パラメタセット
     MOVE     WK-JYU-TOKCD        TO   PARA-TOKCD.
     MOVE     WK-JYU-TOKNM        TO   PARA-TOKNM.
     MOVE     WK-JYU-FILNM        TO   PARA-FILNM.
*処理終了へ
     MOVE    "END"         TO         END-FLG.
*
 JYUSIN-START-EXIT.
