...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/STE3301B.COB`
...
*                                            DATE. 99.05.24    *
*                                            BY. HAGIWARA      *
*         UPDATE. 11.10.07 / YOSHIDA.M                         *
*                 基幹サーバ統合                               *
*         UPDATE. 13.03.26 / NAV TAKAHASHI                     *
*                 社名変更に伴う作業（社名変更）               *
****************************************************************
 IDENTIFICATION         DIVISION.
 PROGRAM-ID.            STE3301B.
 AUTHOR.                HAGIWARA.
 DATE-WRITTEN.          99/05/24.
*REKYOKS.
*
******************************************************************
*                                                                *
...
*    ﾒｲｻｲ
 01  BODY-01.
     ...
     03  FILLER                   PIC  X(07)     VALUE   SPACE.
     03  MS031                    PIC  X(08).
     03  FILLER                   PIC  X(99)    VALUE   SPACE.
*
 01  BODY-02.
     03  FILLER                   PIC  X(05)     VALUE   SPACE.
     03  MS02                     PIC  X(15).
...
***2011.10.07 EN
          INVALID  KEY
              DISPLAY NC"山新対象データ無し" UPON CONS
              STOP RUN
     END-START.
*////MOVE    8         TO        ERR-FLG.
*////PERFORM           ERR-MSG-SEC.
...
*****アテンション判定*****
     ...
     END-EVALUATE.
*****処理対象未入力時処理*****
     IF   KUBUN  NOT  NUMERIC
          MOVE     2    TO   ERR-FLG
          PERFORM       ERR-MSG-SEC
          GO       TO   MAIN-EXIT
     ELSE
          MOVE  KUBUN   TO   WRK-R040
     END-IF.
*
     PERFORM       ERR-MSG-SEC.
     PERFORM       DSP-WRITE-SEC.
...
*    伝票総枚数セット
     MOVE     WRK-MAI   TO        DENMAI.
     MOVE     SPACE     TO        DSP-CNTL.
     MOVE    "SCREEN"   TO        DSP-GRP.
     MOVE    "FTE33011" TO        DSP-FMT.
*    画面表示
     WRITE    FTE33011.
 DSP-WRITE-EXIT.
     EXIT.
...
*    取引先商品コード(上段)
     MOVE     KYO-F25(1:8)       TO   MS031.
*    商品名（２）
     MOVE     KYO-F1422          TO   MS02.
*    取引先商品コード(下段)
     MOVE     KYO-F25(9:5)       TO   MS032.
*    数量
