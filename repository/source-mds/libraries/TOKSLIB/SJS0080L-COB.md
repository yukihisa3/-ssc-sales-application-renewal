...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SJS0080L.COB`
...
****  見出し行１             ****
 01  MIDASI1        CHARACTER     TYPE   IS   YA.
     ...
     02  FILLER              PIC  X(08)  VALUE  "SJS0080L".
     02  FILLER              PIC  X(30)  VALUE  SPACE.
     02  FILLER              PIC  N(23)  VALUE
         NC"※※　仕　入　先　別　仕　入　実　績　表　※※".
...
****  明細行１               ****
 01  MEISAI1        CHARACTER     TYPE   IS   YB.
     02  FILLER              PIC  X(01).
     02  PRT-SIRCD           PIC  X(08)B.
     02  PRT-SIRNM           PIC  N(18).
****  明細行２               ****
...
*メッセージ情報
 01  MSG-AREA.
     03  MSG-ABEND1.
         05  FILLER          PIC  X(12)  VALUE  "### SJS0080L".
...
*=============================================================
 CONTROL-SEC         SECTION.
     DISPLAY  "**  SJS0080L   START  **"   UPON  CONS.
*
     PERFORM  INIT-SEC.
     PERFORM  MAIN-SEC    UNTIL  MAIN-FLG  =  99.
     PERFORM  END-SEC.
*
     DISPLAY  "**  SJS0080L    END   **"   UPON  CONS.
     STOP  RUN.
 CONTROL-EXIT.
     EXIT.
*=============================================================
...
*初期画面の処理
     ...
     MOVE "FJS00801"        TO    DSP-FMT.
     MOVE  SPACE            TO    DSP-FJS00801.
     PERFORM  OPT-CLR-SEC.
*年月
...
*仕入先
     IF        DSP-EDSIR    =   SPACE
         MOVE  "99999999"    TO  DSP-EDSIR
     END-IF.
     IF        DSP-STSIR    >   DSP-EDSIR
         IF  ERR-FLG  =  ZERO
             MOVE   2        TO  ERR-FLG
         END-IF
         MOVE  "R"           TO  EDIT-OPTION  OF  DSP-STSIR
                                 EDIT-OPTION  OF  DSP-EDSIR
         MOVE  "C"           TO  EDIT-CURSOR  OF  DSP-STSIR
     END-IF.
*商品
     PERFORM   SHO-SET-SEC.
     IF        DSP-EDSHO    =   SPACE
...
*仕入先
     IF   JIS-F02     =   DSP-EDYM
     AND  JIS-F03     >   DSP-EDSIR
           MOVE   9        TO  JIS-FLG
           GO              TO  JIS-READ-EXIT
     END-IF.
     IF   JIS-F03     <   DSP-STSIR
     OR   JIS-F03     >   DSP-EDSIR
           GO              TO  JIS-READ-SEC
     END-IF.
*商品
     IF   JIS-F02     =   DSP-EDYM
...
*仕入先
     IF    SIR-FLG       =  1
           ...
           MOVE  OLD-F03          TO   SHI-F01  PRT-SIRCD
           READ  ZSHIMS
               INVALID  KEY
                   MOVE     OLD-F03    TO   BUT-F01
                   READ     BUTOKMF
                        INVALID  KEY
                            MOVE  NC"＊"    TO  PRT-SIRNM
                        NOT INVALID  KEY
                            MOVE  BUT-F02   TO  PRT-SIRNM
                   END-READ
               NOT INVALID  KEY
