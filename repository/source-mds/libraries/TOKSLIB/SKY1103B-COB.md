...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SKY1103B.COB`
...
****  プリントファイル  ****
 COPY   FKY11032         OF        XMDLIB.
 COPY   FKY11033         OF        XMDLIB.
 COPY   FKY11034         OF        XMDLIB.
 COPY   FKY11035         OF        XMDLIB.
****  画面制御項目  ****
 01  DSP-CONTROL.
...
**** メッセージ情報           ****
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SKY1103B".
...
************************************************************
 END-SEC                SECTION.
* ファイルＣＬＯＳＥ
     CLOSE    SOKURIF   DSPF  PRTF.
     DISPLAY "(SKY1103B)ﾆｭｳﾘｮｸ ｹﾝｽｳ(ｵﾔ) = " IN-CNT1  UPON  CONS.
     DISPLAY "(SKY1103B)ﾆｭｳﾘｮｸ ｹﾝｽｳ(ｺ)  = " IN-CNT2  UPON  CONS.
     DISPLAY "(SKY1103B)ﾌﾟﾘﾝﾄ  ｹﾝｽｳ(ｵﾔ) = " PRT-CNT1 UPON  CONS.
     DISPLAY "(SKY1103B)ﾌﾟﾘﾝﾄ  ｹﾝｽｳ(ｺ)  = " PRT-CNT2 UPON  CONS.
 END-END.
     EXIT.
*----------------------------------------------------------*
*      2.1       初期画面処理                              *
*----------------------------------------------------------*
 INIT-DSP-SEC        SECTION.
     MOVE     SPACE          TO   FKY11031.
     MOVE    "FKY11031"      TO   DSP-FORMAT.
...
* 送り状印刷
     MOVE     SPACE            TO   PRT-PRO.
     MOVE     "FKY11033"       TO   PRT-FMT.
     MOVE     SPACE            TO   PRT-CON1.
     MOVE     "GRP201"         TO   PRT-GRP.
     PERFORM   VARYING     IX     FROM   1   BY   1  UNTIL
                                      IX   >   OKU-F12
               WRITE    PRT-AREA       FROM    FKY11033
               ADD      1                TO    PRT-CNT1
               CLOSE    PRTF
               OPEN    OUTPUT   PRTF
     END-PERFORM.
...
* 送り状印刷
     MOVE     SPACE            TO   PRT-PRO.
     MOVE     "FKY11034"       TO   PRT-FMT.
     MOVE     SPACE            TO   PRT-CON1.
     MOVE     "GRP301"         TO   PRT-GRP.
     PERFORM   VARYING     IX     FROM   1   BY   1  UNTIL
                                      IX   >   OKU-F12
               WRITE    PRT-AREA       FROM    FKY11034
               ADD      1                TO    PRT-CNT1
               CLOSE    PRTF
               OPEN    OUTPUT   PRTF
     END-PERFORM.
...
* 送り状印刷
     MOVE     SPACE            TO   PRT-PRO.
     MOVE     "FKY11035"       TO   PRT-FMT.
     MOVE     SPACE            TO   PRT-CON1.
     MOVE     "GRP401"         TO   PRT-GRP.
     PERFORM   VARYING     IX     FROM   1   BY   1  UNTIL
                                      IX   >   OKU-F13
               WRITE    PRT-AREA       FROM    FKY11035
               ADD      1                TO    PRT-CNT2
               CLOSE    PRTF
               OPEN    OUTPUT   PRTF
     END-PERFORM.
