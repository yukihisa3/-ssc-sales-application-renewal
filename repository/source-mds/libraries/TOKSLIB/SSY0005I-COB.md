...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY0005I.COB`
...
*
 ...
 01  PARA-JDATE2            PIC   9(08).
 01  PARA-JTIME2            PIC   9(04).
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSY0005I DSPFILE ERROR " DSP-CNTL " "
...
*----<< 伝票データ >>--*
 SHTDENF-ERR            SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY0005I SHTDENF ERROR " SHTDENF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY0005I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY0005I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSY00051.
*    20111117 ADD-BGN
     IF  (PARA-JDATE2 NOT = ZERO )
         MOVE     PARA-JDATE2          TO   JDATE
     END-IF.
     IF  (PARA-JTIME2 NOT = ZERO )
         MOVE     PARA-JTIME2          TO   JTIME
     END-IF.
     IF  (PARA-TORICD2 NOT = ZERO )
         MOVE     PARA-TORICD2         TO   TORICD
     END-IF.
*    20111117 ADD-END
*
...
*
     MOVE    "SSY0005I"      TO   PGID.
     MOVE    "FSY00051"      TO   FORM.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FSY00051"     TO   DSP-FMT.
...
*
*## 1999/10/13 T.T START ##*
*****IF     ( JDATE     =    ZERO ) AND
*****       ( JTIME     =    ZERO ) AND
*****       ( TORICD    =    ZERO )
     IF     (( JDATE NOT NUMERIC ) OR ( JDATE  =  ZERO ))
     AND    (( JTIME NOT NUMERIC ) OR ( JTIME  =  ZERO ))
     AND    (( TORICD NOT NUMERIC ) OR ( TORICD  =  ZERO ))
              MOVE      2         TO   ERR-FLG
...
*
 220-GRP01-CHECK-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      ｿｳｺ ﾃｲｾｲｸﾌﾞﾝ ｼｭﾂﾘｮｸｼﾞｭﾝ ﾆｭｳﾘｮｸ              *
*--------------------------------------------------------------*
...
*
     MOVE     " "            TO   EDIT-CURSOR OF SOKO
                                  EDIT-CURSOR OF TKBN
                                  EDIT-CURSOR OF OUTNO.
     MOVE     "M"            TO   EDIT-OPTION OF SOKO
                                  EDIT-OPTION OF TKBN
                                  EDIT-OPTION OF OUTNO.
*
 CLR-BODY-EXIT.
     EXIT.
*--------------------------------------------------------------*
