...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SJH1701B.COB`
...
*    ｶｳﾝﾄ
 ...
 01  DENPYO-CNT              PIC  9(08)     VALUE  ZERO.
...
*    伝票ヘッダ
     03  WK-DEN0.
         05  WK-DEN0-FIL     PIC  X(64).
         05  WK-DEN0-00      PIC  9(02).
         05  WK-DEN0-99      PIC  X(62).
*    伝票ヘッダ
     03  WK-DEN1.
         05  WK-DEN1-01      PIC  X(01).
         05  WK-DEN1-02      PIC  9(02).
         05  WK-DEN1-03.
             07  WK-DEN1-031 PIC  9(06).
             07  WK-DEN1-032 PIC  9(03).
         05  WK-DEN1-04.
             07  WK-DEN1-041 PIC  X(04).
             07  WK-DEN1-042 PIC  X(02).
             07  WK-DEN1-043 PIC  9(03).
         05  WK-DEN1-05      PIC  9(04).
         05  WK-DEN1-06      PIC  9(02).
         05  WK-DEN1-07      PIC  9(06).
         05  WK-DEN1-08      PIC  9(06).
         05  WK-DEN1-09.
             07  WK-DEN1-091 PIC  9(06).
             07  WK-DEN1-092 PIC  9(02).
         05  WK-DEN1-10      PIC  X(15).
         05  WK-DEN1-11      PIC  X(15).
         05  WK-DEN1-12      PIC  X(15).
         05  WK-DEN1-13      PIC  X(11).
         05  WK-DEN1-14      PIC  9(01).
         05  WK-DEN1-15      PIC  X(01).
         05  WK-DEN1-16      PIC  X(01).
         05  WK-DEN1-17      PIC  9(09).
         05  WK-DEN1-18      PIC  X(12).
         05  WK-DEN1-19      PIC  X(01).
*    伝票明細
     03  WK-DEN2.
         05  WK-DEN2-01      PIC  X(01).
         05  WK-DEN2-02      PIC  9(02).
         05  WK-DEN2-03      PIC  9(02).
         05  WK-DEN2-04      PIC  X(13).
         05  WK-DEN2-05      PIC  9(04).
         05  WK-DEN2-06      PIC  9(04).
         05  WK-DEN2-07      PIC  X(02).
         05  WK-DEN2-08      PIC  9(05)V9.
         05  WK-DEN2-09      PIC  9(07)V99.
         05  WK-DEN2-10      PIC  9(07).
         05  WK-DEN2-11      PIC  9(10).
         05  WK-DEN2-12      PIC  9(10).
         05  WK-DEN2-13      PIC  X(10).
         05  WK-DEN2-14      PIC  X(13).
         05  WK-DEN2-15      PIC  X(14).
         05  WK-DEN2-16      PIC  X(10).
         05  WK-DEN2-17      PIC  X(11).
*
 01  WK-AREA.
*システム日付の編集
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJH1701B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH1701B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
     ...
     MOVE     ZERO      TO        DENPYO-CNT.
     MOVE     SPACE     TO        WK-DEN-REC.
     INITIALIZE                   WK-DEN-REC.
*
******************
*システム日付編集*
******************
...
*             出荷場所件数マスタ出力
              ...
              END-IF
              MOVE      DEN-01(IDX)   TO    WK-DEN1
              ADD       1        TO   CNT-MAISU
              MOVE      ZERO     TO   CNT-KENSU-D
        END-IF
*明細行
        IF    DEN-01A(IDX)  =     "D"
              MOVE      DEN-01(IDX)   TO    WK-DEN2
              ADD       1        TO   CNT-KENSU
...
*各量販店領域転送
*取引先コード
     MOVE     WK-DEN1-09    TO      HEN-A01.
*伝票_
     MOVE     WK-DEN1-03    TO      HEN-A02.
*伝票行_
     MOVE     WK-DEN2-03    TO      HEN-A03.
*社店コード
     MOVE     WK-DEN1-04    TO      HEN-A04.
*分類コード
     MOVE     WK-DEN1-05    TO      HEN-A05.
*発注区分
     MOVE     WK-DEN1-06    TO      HEN-A06.
*発注日
     MOVE     WK-DEN1-07    TO      HEN-A07.
*納品日
     MOVE     WK-DEN1-08    TO      HEN-A08.
*社名
     MOVE     WK-DEN1-10    TO      HEN-A09.
*店舗名称
     MOVE     WK-DEN1-11    TO      HEN-A10.
*取引先名称
     MOVE     WK-DEN1-12    TO      HEN-A11.
*伝票判別用コメント
     MOVE     WK-DEN1-13    TO      HEN-A12.
*電話番号
     MOVE     WK-DEN1-18    TO      HEN-A13.
*商品コード（ＪＡＮコード）
     MOVE     WK-DEN2-04    TO      HEN-A14.
*入数
     MOVE     WK-DEN2-05    TO      HEN-A15.
*発注単位
     MOVE     WK-DEN2-07    TO      HEN-A16.
*メーカー名
     MOVE     WK-DEN2-13    TO      HEN-A17.
*商品名
     MOVE     WK-DEN2-15    TO      HEN-A18.
*発注数量
     MOVE     WK-DEN2-08    TO      HEN-A19.
*原価単価
     MOVE     WK-DEN2-09    TO      HEN-A20.
*売価単価
     MOVE     WK-DEN2-10    TO      HEN-A21.
*原価金額
     MOVE     WK-DEN2-11    TO      HEN-A22.
*売価金額
     MOVE     WK-DEN2-12    TO      HEN-A23.
*入数・ケース*******************
*****MOVE     WK-DEN2-16    TO      HEN-A24.
*規格
*****MOVE     WK-DEN2-16    TO      HEN-A25.
*レギュラー単品コード
*****MOVE     WK-DEN2-18    TO      HEN-A26.
*伝票枚数
     MOVE     ZERO          TO      HEN-A27.
*発注種別
     MOVE     WK-DEN0-00    TO      HEN-A28.
*取引先名称
     MOVE     WK-DEN1-12    TO      HEN-A29.
*電話番号
     MOVE     WK-DEN1-18    TO      HEN-A30.
*消費税区分
     MOVE     WK-DEN1-14    TO      HEN-A31.
*承り発注区分
     MOVE     WK-DEN1-15    TO      HEN-A32.
*販促区分
     MOVE     WK-DEN1-16    TO      HEN-A33.
*販促管理_
     MOVE     WK-DEN1-17    TO      HEN-A34.
*便
     MOVE     WK-DEN1-19    TO      HEN-A35.
*発注数
     MOVE     WK-DEN2-06    TO      HEN-A36.
*取引先品番
     MOVE     WK-DEN2-14    TO      HEN-A37.
*規格
     MOVE     WK-DEN2-16    TO      HEN-A38.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
*取引先コード
     MOVE        HEN-A011  TO        HEN-F01.
*伝票ナンバー
     MOVE        HEN-A021  TO        HEN-F02.
     MOVE        HEN-A021  TO        HEN-F23.
*行_
...
*    商品変換テーブル検索
     ...
     INITIALIZE                      TBL-REC.
     MOVE        HEN-A011  TO        TBL-F01.
     MOVE        HEN-A14   TO        TBL-F02.
     READ    SHOTBL1
       INVALID
         MOVE    SPACE     TO        TBL-REC
         INITIALIZE                  TBL-REC
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
         MOVE    HEN-A18   TO        HEN-F1421
         MOVE    SPACE     TO        HEN-F1422
       NOT INVALID
         MOVE    HEN-A18   TO        HEN-F1421
         MOVE    SPACE     TO        HEN-F1422
     END-READ.
*数量
     MOVE        HEN-A19   TO        HEN-F15.
*単
     MOVE       "1"        TO        HEN-F16.
*原価単価
     MOVE        HEN-A20   TO        HEN-F172.
*売価単価
     MOVE        HEN-A21   TO        HEN-F173.
*原価金額
     MOVE        HEN-A22   TO        HEN-F181.
*売価金額
     MOVE        HEN-A23   TO        HEN-F182.
*店舗名（備考）
*****MOVE        HEN-A10   TO        HEN-F22.
*自社得意先コード
     IF  FLG-TOK   =    ZERO
*    得意先マスタ検索
         MOVE    SPACE         TO    TOK-REC
         INITIALIZE                  TOK-REC
         MOVE    HEN-A011      TO    TOK-F01
         READ    TOKMS2
             INVALID
               MOVE  SPACE     TO    TOK-REC
               INITIALIZE            TOK-REC
...
*訂正前数量
     MOVE        HEN-A19   TO        HEN-F50.
*修正原価単価
     MOVE        HEN-A20   TO        HEN-F512.
*修正売価単価
     MOVE        HEN-A21   TO        HEN-F513.
*修正原価金額
     MOVE        HEN-A22   TO        HEN-F521.
*修正売価金額
     MOVE        HEN-A23   TO        HEN-F522.
*
 TENSO-EXIT.
     EXIT.
****************************************************************
