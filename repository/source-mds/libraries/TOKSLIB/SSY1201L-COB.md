...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY1201L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*コーナン商事伝票データ（共通）　　
     SELECT   USHIRED  ASSIGN    TO         USHIRED
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      USHIRED.
...
*    コーナン以外は読み飛ばし（取引先）
     ...
     END-IF.
*    伝票枚数カウント
     IF       DEN-A20  NOT =  WK-DEN-NO
              ADD    1        TO      WK-MAI
              MOVE   DEN-A20  TO      WK-DEN-NO
     END-IF.
*
 DEN-CNT-EXIT.
     EXIT.
...
*
 ...
 READ-010.
     IF       ( SYU-FLG     = 1      )  AND
              ...
              GO        TO        FL-READ-EXIT.
     IF       DEN-A20   =    MD03
              MOVE      DEN-A20   TO        DEN-NO
              MOVE      1         TO        SYU-FLG.
 READ-020.
*
     IF       RD-SW     =    0
              MOVE      DEN-A20   TO   DENNO1
              PERFORM   HEAD-WT-SEC
              MOVE      1         TO   RD-SW
     END-IF.
*
...
***  取引先商品コード
     MOVE     DEN-A37             TO   DT2-02.
***  入数
     MOVE     DEN-A38             TO   DT2-03.
***  ケース
     MOVE     DEN-A39             TO   DT2-04.
***  数量
...
*ヘッダ２行目
***  社名
     MOVE     "ｺｰﾅﾝｼﾖｳｼﾞ"         TO    HD2-01.
***  取引先名（上段）
     MOVE     DEN-A27(1:20)       TO    HD2-02.
*ヘッダ３行目
***  店名
     MOVE     DEN-A26             TO    HD3-01.
***  店コード
     MOVE     DEN-A16             TO    HD3-02.
***  部門コード
     MOVE     DEN-A17             TO    HD3-03.
***  伝票区分
     MOVE     DEN-A28             TO    HD3-04.
***  伝票_
     MOVE     DEN-A20             TO    HD3-05.
***  取引先コード
     MOVE     DEN-A21A            TO    HD3-06.
***  取引先名（下段）
     MOVE     DEN-A27(21:20)      TO    HD3-07.
***  発注日
     MOVE     DEN-A18             TO    HD3-08.
***  納品日
     MOVE     DEN-A19             TO    HD3-09.
*
*Ｆ欄退避
***  Ｆ欄（島番号）
...
*テスト印字項目セット
     MOVE   ALL "9"               TO   HD1-04  HD3-02  HD3-03
                                       ...
                                       DT2-052 DT2-072.
     MOVE   ALL "*"               TO   HD1-01  HD1-02  HD2-02
