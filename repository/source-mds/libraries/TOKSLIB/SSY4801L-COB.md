...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIB/SSY4801L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*ホーマック伝票データ（共通）
     SELECT   HGSHIRED  ASSIGN    TO        HGSHIRED
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      HGSHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     HGSHIRED  AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    ホーマック新以外は読み飛ばし（取引先）
*****IF       DEN-F01  NOT =  999999
     IF       DEN-F01  NOT =  298
**************DISPLAY "DEN-F01 = " DEN-F01 UPON CONS
              GO                 TO   DEN-CNT-SEC
     END-IF.
*納品区分＝１（センター納品）時は伝票出力無し
     IF       DEN-A15  =  1
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
*****DISPLAY "DEN-A02   = " DEN-A02 UPON CONS.
*****DISPLAY "WK-DEN-NO = " WK-DEN-NO UPON CONS.
...
*明細２行目
***  商品名（２）
     MOVE     DEN-A212            TO   DT2-01
***  商品コード
     MOVE     DEN-A201            TO   DT2-02.
***  入数
***  ケース(発注数)
***  数量
...
***  訂正欄
     IF       DEN-F53  NOT  =    ZERO
              MOVE     NC"◯"     TO        HD0-01
     ELSE
              MOVE     SPACE      TO        HD0-01
     END-IF.
*ヘッダ２行目
***  発注者名
     MOVE     "ﾎｰﾏｰｸﾞﾘｰﾝ(ｶﾌﾞ)"    TO    HD1-01.
***  取引先名（１）
     MOVE     DEN-A071            TO    HD2-01.
*ヘッダ３行目
***  店舗名称
     MOVE     DEN-A09             TO    HD3-01.
***  店舗コード
     MOVE     DEN-A081            TO    HD3-02.
***  分類コード(部コード)
     MOVE     DEN-A101            TO    HD3-03.
***  伝票区分
     MOVE     DEN-A11             TO    HD3-04.
***  伝票_
     MOVE     DEN-A03             TO    HD3-05.
***  取引先コード
     MOVE     DEN-A06             TO    HD3-06.
***  取引先名（２）
     MOVE     DEN-A072            TO    HD3-07.
***  発注日
     MOVE     DEN-A04             TO    HD3-08.
***  納品日
     MOVE     DEN-A05             TO    HD3-09.
...
****************************************************************
 TAIL-WT-SEC            SECTION.
     ...
     MOVE     SPACE               TO        TL1  TL2  TL2.
*テール１行目
***  連番
     MOVE     "("                 TO        WRK-HEN01.
     MOVE     SPACE               TO        WRK-HEN011.
     MOVE     WK-DEN-A12          TO        WRK-HEN02.
     MOVE     SPACE               TO        WRK-HEN021.
     MOVE     ")"                 TO        WRK-HEN03.
     MOVE     WRK-HEN             TO        TL1-01.
***  原価金額合計
...
*テスト印字項目セット
     ...
     MOVE   ALL "9"              TO   HD3-02  HD3-03  HD3-04
                                      ...
                                      DT2-041 DT2-042 DT2-051
                                      DT2-052 DT2-06
                                      DT2-07  DT2-08
                                      TL1-02  TL1-03.
     MOVE   ALL "*"              TO   HD1-01  HD2-01  HD3-01
                                      HD3-07  DT1-01  DT1-02
                                      DT2-01  TL1-01.
*ヘッダ部
*****WRITE    PRT-REC          FROM   HD0        AFTER    3.
     WRITE    PRT-REC          FROM   HD0        AFTER    1.
...
*
     CLOSE    HGSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
