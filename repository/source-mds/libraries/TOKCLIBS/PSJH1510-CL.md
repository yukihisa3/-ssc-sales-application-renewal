...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH1510.CL`
...
/.##ﾊﾟﾗﾒﾀ定義##./
    ...
    PARA      ?JKEKA    ,STRING*2,OUT,VALUE-'  '       /.OUT結果./
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PSJH1510'  /.PROGRAM-ID./
...
SJH1501B:
    ...
    OVRF      FILE-KENSYUD,TOFILE-SEKICHUJ.TOKKLIB
    OVRF      FILE-SIHARAD,TOFILE-SEKICHUS.TOKKLIB
    OVRF      FILE-SHOHIND,TOFILE-SEKICHUD.TOKKLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-SJH1510B.TOKELIBO,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '92'
...
SEKCNT:
    ...
    OVRF      FILE-SKKENSYU,TOFILE-SEKICHUJ.TOKKLIB
    OVRF      FILE-SKSIHARA,TOFILE-SEKICHUS.TOKKLIB
    OVRF      FILE-SKSHOHIN,TOFILE-SEKICHUD.TOKKLIB
    CALL      PGM-SEKCNT1.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
