...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBMK220P.CL`
...
/. *   JOB-ID      :    PBMK220P    2013/03/15               *  ./
/. *   JOB-NAME    :    不照合リスト発行（コメリ）レーザー   *  ./
/. ***********************************************************  ./
    PGM
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PBMK220P'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SBMK230P:
    ...
    CALL      PGM-SBMK230P.TOKELIBO,PARA-(?P1,?P2)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '不照合リスト発行（コメリ）'
              GOTO ABEND END
