...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH9131.CL`
...
/. *   SYSTEM-NAME :    支払伝票データ作成（ＥＤＩＣ用）     *  ./
/. *   JOB-ID      :    PNJH9131                             *  ./
/. *   JOB-NAME    :    ビーバートザン用（制御バイト無し）   *  ./
/. ***********************************************************  ./
...
/.##ﾜｰｸﾃｲｷﾞ ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH9131'  /.PROGRAM-ID./
...
NJH9131B:
    ...
    OVRF      FILE-EDSIHAL1,TOFILE-EDSIHAL1.TOKDLIB
    CALL      PGM-NJH9131B.TOKELIBO,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
              CALL PGM-SNJ0730B.TOKELIBO,
