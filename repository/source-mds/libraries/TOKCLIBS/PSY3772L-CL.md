...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY3772L.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY3772L'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP010:
    ...
    ?OPR2  :=  '支払データ取込済みですか？'
    ?OPR3  :=  '　　　　　　　　　　　　　　　　　　　　　　'
    ?OPR4  :=  'この処理で、支払明細書の発行が行われます。　'
    ?OPR5  :=  '完了後、支払明細書を確認して下さい。　　　　'
...
STEP030:
    ...
    SNDMSG '######################################',TO-XCTL
    SNDMSG '## ナフコ　　　支払明細書発行　開始 ##',TO-XCTL
    SNDMSG '######################################',TO-XCTL
/.##ナフコ支払明細書発行##./
    OVRF   FILE-NFSIHAL2,TOFILE-NFSIHAL2.TOKKLIB

    CALL SSY3772L.TOKELIBO
    IF   @PGMEC   ^=  0    THEN
         SNDMSG '######################################',TO-XCTL
         SNDMSG '## ナフコ　　　支払明細書発行　終了 ##',TO-XCTL
         SNDMSG '######################################',TO-XCTL
    END
