...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY08011.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY08011'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY0803L:
    ...
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY0803L.TOKELIBO,PARA-(?NOUDT1,?NOUDT2)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
    GOTO       RTN
...
SSY0804L:
    ...
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY0804L.TOKELIBO,PARA-(?NOUDT1,?NOUDT2)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
RTN:
