...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY3777N.CL`
...
/.### ﾜｰｸｴﾘｱ定義 ###./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY3777N'    /.ﾌﾟﾛｸﾞﾗﾑID./
    ...
    VAR ?SDTYMD   ,STRING*8,VALUE-'        ' /. 開始データ作成日 ./
    VAR ?EDTYMD   ,STRING*8,VALUE-'        ' /. 終了データ作成日 ./
    VAR ?SYTANCD  ,STRING*2,VALUE-'  '       /. 処理担当者コード ./
    VAR ?KBN      ,STRING*1,VALUE-'1'        /. オンライン／手書 ./
...
/.## ﾗｲﾌﾞﾗﾘﾘｽﾄ登録 ##./
    DEFLIBL TOKELIB/TOKDLIB/TOKFLIB/TOKELIBO/TOKKLIB

/.## ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ ##./
    ?PGNM :=  'ナフコＥＤＩ管理番号リスト指示'
...
/.  OVRDSPF FILE-DSPF,TOFILE-DSPF.XUCL,MEDLIB-TOKELIBO ./
    ...
    CALL PGM-SSY3777N.TOKELIBO
      ,PARA-(?BUMON,?TANCD,?SDTYMD,?EDTYMD,?SYTANCD)
...
SSY3751L:
    ...
    SNDMSG '######################################',TO-XCTL
    SNDMSG '## ナフコＥＤＩ管理番号リスト指示　 ##',TO-XCTL
    SNDMSG '######################################',TO-XCTL
/.  OVRPRTF   FILE-XU04LP,TOFILE-KAHMAPRT.XUCL
./  CALL SSY3751N.TOKELIBO,PARA-(?KBN,?SDTYMD,?EDTYMD,?BUMON)

    IF   @PGMEC    ^=   0    THEN
         SNDMSG '########################################',TO-XCTL
         SNDMSG '## ナフコＥＤＩ管理番号リスト指示エラー#',TO-XCTL
         SNDMSG '########################################',TO-XCTL
         GOTO ABEND
    END
    IF   ?KEKKA     = '99'   THEN
         SNDMSG '########################################',TO-XCTL
         SNDMSG '## ナフコＥＤＩ管理番号マスタリストなし#',TO-XCTL
         SNDMSG '########################################',TO-XCTL
    END
