...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSE02730.CL`
...
/. *   SYSTEM-NAME :    イオン北海道オンラインシステム       *  ./
/. *   JOB-ID      :    PSE02730                             *  ./
/. *   JOB-NAME    :    配信ファイル作成（イオン北海道）　　 *  ./
/. ***********************************************************  ./
    PGM
    VAR       ?PGMEC    ,INTEGER
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSE02730'
    VAR       ?STEP     ,STRING*8
...
/.##ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ##./
    ?PGNM :=  'イオン北海道　請求送信　前処理'
    ?SYORINM1 :=  'イオン北海道　請求送信　前処理'
...
SSE0273B:
    ...
    CALL      PGM-SSE0273B.TOKELIBO,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '【送信ＤＴ作成】'
              GOTO ABEND END
...
PSNDRSJJ:
    ...
    SNDRSJJ COM-KGPATH01.KGNODE01,DFILE-'HSSIONH'
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '【ＧＰ側ＪＯＢ　起動】'
              GOTO ABEND END
