...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH7530.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH7530'  /.PROGRAM-ID./
...
PFEXPORT:
    ...
    FEXPORT FILE-ONLNSNSS.ONLBLIB,MODE-@ADD,PARA-ONLSNS,
            UNIT-3
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NCV7530B:
    ...
    CALL      PGM-NCV7530B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
NJH7530B:
    ...
    CALL      PGM-NJH7530B.TOKELIBO,PARA-(?PARA)
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
...
PFIMPORT:
    ...
    FIMPORT FILE-ONLNSNCL.ONLBLIB,PARA-ONLSNS,UNIT-8
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA := 'K519'
