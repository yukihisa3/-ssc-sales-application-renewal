...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH8920.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSJH8920'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    FEXPORT FILE-LIJFILE.ONLBLIB,MODE-@REP,PARA-LICJYUR,
            UNIT-1
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（受領）】'
                    GOTO ABEND
...
STEP040:
    ...
    OVRF FILE-LIJFILE,TOFILE-LIJFILE.ONLBLIB
    OVRF FILE-DCMJYRL1,TOFILE-DCMJYRL1.TOKKLIB
    CALL SJH8903B.TOKELIBO
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受領データ変換】'
                    GOTO ABEND
    END
