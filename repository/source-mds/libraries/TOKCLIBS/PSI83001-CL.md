...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI83001.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSI83001'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    FEXPORT FILE-HOMSIHDS.TOKKLIB,MODE-@REP,PARA-HOMSIHDT,
            UNIT-2
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理】'
                    GOTO ABEND
...
STEP040:
    ...
    OVRF FILE-HOMSIHDT,TOFILE-HOMSIHDS.TOKKLIB
    OVRF FILE-SITHD831,TOFILE-SITHD831.TOKKLIB
    OVRF FILE-SITME831,TOFILE-SITME831.TOKKLIB
    OVRF FILE-SITTL831,TOFILE-SITTL831.TOKKLIB
    OVRF FILE-SIHARASF,TOFILE-SIHARASS.TOKKLIB
    CALL SSI8300B.TOKELIBO,PARA-('2')
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＪＥＤＩＣＯＳデータ変換】'
                    GOTO ABEND
    END
...
RTN:
    ...
    ?KEKA1 :=  'ホーマック支払の連続変換処理が正常終了'
    ?KEKA2 :=  'しました。支払明細等を発行して下さい。'
    ?KEKA3 :=  ''
    ?KEKA4 :=  ''
