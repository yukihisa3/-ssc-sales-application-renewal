...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH4408.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH4408'  /.PROGRAM-ID./
...
SSY4468B:
    ...
    DEFLIBL   TOKELIB/TOKFLIB/TOKELIBO/TOKJLIB/BMSFLIB/TOKKLIB/
              TOKDTLIB/TOKWLIB/TOKSOLIB/TOKDLIB
    ...
    CALL      PGM-SSY4468B.TOKSOLIB,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
...
SJR0070V:
    ...
    CALL      PGM-SJR0070V.TOKSOLIB,PARA-(?HIDUKE,?JIKAN)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
...
JTKENLST:
 ...
 /. IF   ?JIKAN2 ^=  '0000'  THEN
         SBMJOB JOB-JTKENLST,JOBD-CVCS.XUCL,JOBK-@B,
            PGM-KENLST.TOKCLIBO,VSIZE-5,RSIZE-16,LOG-@YES!1024,
            PARA-(?HIDUKE,?JIKAN2,?TOKCD,'I','1')
    END    ./
