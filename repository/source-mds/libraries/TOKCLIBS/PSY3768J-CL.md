...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY3768J.CL`
...
/.### ﾜｰｸｴﾘｱ定義 ###./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY3768J'    /.ﾌﾟﾛｸﾞﾗﾑID./
    ...
    VAR ?JYUST    ,STRING*8,VALUE-'00000000' /. 受信日開始./
    VAR ?JYUED    ,STRING*8,VALUE-'00000000' /. 受信日終了 ./
    VAR ?SIRST    ,STRING*8,VALUE-'00000000' /. 計上日開始 ./
    VAR ?SIRED    ,STRING*8,VALUE-'00000000' /. 計上日終了 ./
    VAR ?LSTKBN   ,STRING*1,VALUE-'0'        /. 帳票区分 ./
    VAR ?KENSU    ,STRING*7,VALUE-'0000000'  /. 件数 ./
...
SSY3782I:
    ...
    CALL PGM-SSY3782I.TOKELIBO,PARA-(?KBN,?JYUST,?JYUED,?SIRST,
                                     ?SIRED,?LSTKBN)
    IF  @PGMEC ^= 0  THEN
        IF @PGMEC = 4010  THEN
...
SSY3768V:
    ...
    CALL PGM-SSY3768V.TOKELIBO,PARA-(?KBN,?JYUST,?JYUED,?SIRST,
                                     ?SIRED,?LSTKBN,?KENSU)
    IF  @PGMEC ^= 0  THEN
        ?KEKA4 := 'ナフコ受領書／ＣＳＶデータ出力'
        GOTO  ABEND
    ELSE
        IF  ?LSTKBN = '2' THEN
             IF  ?KENSU ^= '0000000' THEN
               FIMPORT FILE-NFJYRTXT.TOKKLIB,PARA-NAFUKO,UNIT-5
               IF  @PGMEC ^= 0  THEN
                   ?KEKA4 := 'ＣＳＶデータ＝＞ＰＣ転送'
                   GOTO  ABEND
               END
