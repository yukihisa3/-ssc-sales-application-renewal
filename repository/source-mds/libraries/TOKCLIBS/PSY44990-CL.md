...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY44990.CL`
...
/. *   サカタのタネ　ホームガーデン部　　　　　　　  　      *  ./
/. *   SYSTEM-NAME :    流通ＢＭＳ対応　　　　　　　　　　   *  ./
/. *   JOB-ID      :    PSY44990    2013/01/23               *  ./
/. *   JOB-NAME    :    流通ＢＭＳ出荷送信解除処理　　　　   *  ./
/. *               :    　バロー（オンライン分）　　　　　   *  ./
/. ***********************************************************  ./
    PGM
    VAR       ?PGMEC  ,INTEGER
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY44990'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
STEP040:
    ...
    CALL  SSY4497I.TOKELIBO,PARA-(?BUMON,?TANCD,?SOKCD,?DSOKCD,
...
STEP050:
    ...
    CALL SSY4499B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,?SOKO,
                                 ?NOUDT)
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＢＭＳ発注データ出荷済ＦＬＧ解除】'
            GOTO ABEND
    END
...
STEP060:
    ...
    CALL SSY4498B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,?SOKO,
                                 ?NOUDT)
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＢＭＳ出荷データ削除】'
            GOTO ABEND
...
RTN:
    ...
    ?KEKA1 :=  'ＢＭＳ出荷処理の取消が正常終了しました。'
    ?KEKA2 :=  '確認後、出荷データの再送を行なって下さい。'
    ?KEKA3 :=  ''
    ?KEKA4 :=  ''
