...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI73020.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSI73020'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP010:
    ...
    ?OPR1  :=  '【ダイユーエイト支払データＣＳＶ出力処理】'
    ?OPR2  :=  'インターネットよりダウンロード完了済ですか？'
    ?OPR3  :=  '　　　　　　　　　　　　　　　　　　　　　　'
    ?OPR4  :=  'この処理で、支払データＣＳＶが出力されます。'
...
STEP030:
    ...
    SNDMSG '######################################',TO-XCTL
    SNDMSG '## ダイユーエイト支払ＣＳＶ出力開始 ##',TO-XCTL
    SNDMSG '######################################',TO-XCTL
    CALL SSI7302B.TOKELIBO
    IF   ?PGCHK1  = '1'    THEN
         SNDMSG '########################################',TO-XCTL
         SNDMSG '## ダイユーエイト支払ＣＳＶ出力エラー ##',TO-XCTL
         SNDMSG '########################################',TO-XCTL
    END
...
STEP090:
    ...
    FIMPORT FILE-DYSIHDT.TOKKLIB,PARA-DAIYUSIH,UNIT-1
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ出力処理】'
                    GOTO ABEND
    END
