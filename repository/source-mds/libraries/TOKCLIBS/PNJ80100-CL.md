...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJ80100.CL`
...
BUMONPRT:
    ...
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
/.##九州全量販店を本社にて移管するため、部門での出力制御変更
    取引先ＣＤでの制御に変更する。ナフコのみ九州へ出力
    2016/08/01より実施
...
SJK0030B:
    ...
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
                                           /.##各種ﾌｧｲﾙ置換え##./
    OVRF      FILE-SHTDENLA,TOFILE-SHTDENLA.TOKFLIB
    OVRF      FILE-SYUSGNL1,TOFILE-SYUSGNL1.TOKDLIB
    OVRF      FILE-JIDKEKLA,TOFILE-JIDKEKLA.TOKDLIB
/.  CALL      PGM-SJK0030B.TOKELIBO,PARA-(?TORICD,?HIDUKE,?JIKAN,
                                          ?NOUHINBI)
./  IF        @PGMEC    ^=   0    THEN
              CALL SNJ9100B.TOKELIBO,
                   PARA-(?HIDUKE,?JIKAN,?TORICD,'9','K623')
              GOTO ABEND END
...
SJK0100B:
    ...
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    OVRF      FILE-JIDKEKLA,TOFILE-JIDKEKLA.TOKDLIB
/.  CALL      PGM-SJK0100B.TOKSOLIB,PARA-(?TORICD,?HIDUKE,?JIKAN,
                                    '00000000','99999999','2')
./  IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
/.##2017/05/24 NAV ADD ED##./
...
SST0010B:
    ...
    CALL      PGM-SST0010B.TOKSOLIB,PARA-(?HIDUKE,?JIKAN,?TORICD,
                                          '  ','00000000')
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
/.##2020/07/17 NAV ADD ED##./

/.##2021/08/04 NAV ADD ST##./
/.##ヨドバシ受注データ変換（発注変更）##./
NJH9430B:
    ...
    CALL      PGM-NJH9430B.TOKSOLIB,PARA-(?PRTBUMON,'99')

    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
/.##2021/08/04 NAV ADD ED##./
