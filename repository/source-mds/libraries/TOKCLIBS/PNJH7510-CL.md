...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH7510.CL`
...
/.##ﾊﾟﾗﾒﾀ定義##./
    ...
    VAR       ?TOKCD1   ,STRING*8,VALUE-'00039241' /.受信取引先./
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH7510'  /.PROGRAM-ID./
...
STEP010:
    ...
    ?OPR1  :=  '＜ＤＣＭサンワ　発注データ取込＞'
    ?OPR2  :=  '取引先：植物部門（３９２４、３９２４１）'
    ?OPR3  :=  'ＳＮＳシステムよりデータダウンロードし、'
    ?OPR4  :=  'バックアップ処理が完了しておりますか？'
    ?OPR5  :=  '確認後、実行して下さい。'
    CALL      OHOM0900.XUCL,PARA-
                            (?OPR1,?OPR2,?OPR3,?OPR4,?OPR5)

/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIB/TOKFLIB/TOKELIBO/TOKKLIB/TOKJLIB/TOKDLIB
...
PFEXPORT:
    ...
    FEXPORT FILE-ONLNSNHS.ONLBLIB,MODE-@ADD,PARA-ONLSNS,
            UNIT-1
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NCV7510B:
    ...
    CALL      PGM-NCV7510B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
/.##受信ﾌｧｲﾙノ編集##./
   ...
   /.##変換開始時間取得##./
    ...
    CALL SCVMSG.TOKELIB,PARA-('ｾｲｷﾞｮﾊﾞｲﾄ DELE START')
/.##制御バイト削除##./
SSY7599B:
    ...
    OVRF      FILE-ONLSNSDT,TOFILE-?FILLIB
    OVRF      FILE-ONLSNSWK,TOFILE-ONLSNSWK.ONLBLIB
    CALL      PGM-SSY7599B.TOKELIBO
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := '日本語制御バイト削除'
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?MSGX :=  '##EDIC制御ﾊﾞｲﾄ削除処理異常]]##'
              SNDMSG ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
              ?KEKA := 'K525'
...
NJH7505B:
    ...
    OVRF      FILE-CVCSG001,TOFILE-ONLSNSWK.ONLBLIB
    ...
    OVRF      FILE-SWJOHOL1,TOFILE-SWJOHOL1.TOKDLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-NJH7510B.TOKELIBO,PARA-(?PARA)
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
...
/.##振分け件数ﾘｽﾄ出力##./
    CALL SCVMSG.TOKELIB,PARA-('ｹﾝｽｳ LST 3924  START')
KENLST:
    ...
    SBMJOB JOB-KENLST,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKCLIBO,
           VSIZE-5,RSIZE-16,LOG-@YES!1024,PARA-(?HIDUKE,?JIKAN,
           ?TOKCD,?LINE,?YUSEN)

/.##振分け件数ﾘｽﾄ出力##./
    CALL SCVMSG.TOKELIB,PARA-('ｹﾝｽｳ LST 39241 START')
KENLST2:
    ...
    SBMJOB JOB-KENLST2,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKCLIBO,
           VSIZE-5,RSIZE-16,LOG-@YES!1024,PARA-(?HIDUKE,?JIKAN,
           ?TOKCD1,?LINE,?YUSEN)
