...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY39201.CL`
...
/.##ﾊﾟﾗﾒﾀ情報##./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY39201'
    VAR       ?STEP     ,STRING*8
...
SHOM0900:
    ...
    ?OPR1  :=  '　　＜ナフコ商品分類パターンマスタＣＳＶ出力＞'
    ?OPR2  :=  '　　　　　　　　　　　　　　　　　　　　　　'
    ?OPR3  :=  '　　　　ナフコ商品分類パターンマスタを'
    ?OPR4  :=  '　　　　　ＣＳＶデータで出力します'
    ?OPR5  :=  '　　　　　　　　　　　　　　　　　　　　　　'
...
CNVDF000:
    ...
    ?MSGX :=  '***   出力先 = HGNAS  ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
    ?MSGX :=  '***   ﾌｧｲﾙID = NFSHBPF.CSV ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES

    CNVDF FILE-NFSHBPF.TOKDLIB,
          PATH-'//HGNAS/KYUSYU/CSV/NFSHBPF.CSV',MODE-@EXP,ADD-@NO,
          ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES

    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := 'ナフコ商品分類パターンＭ　ＣＳＶ出力'
              GOTO ABEND END
