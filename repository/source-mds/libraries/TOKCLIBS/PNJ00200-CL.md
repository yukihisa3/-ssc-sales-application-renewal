...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJ00200.CL`

## ソースコード

```jcl
/. ***********************************************************  ./
/. *     サカタのタネ　ＥＤＩシステム（本社システム）        *  ./
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PNJ00200'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SNJ0020L:
    ...
    OVRF FILE-JSMKAIL1,TOFILE-JSMKAIL1.TOKJLIB  /.回線管理Ｍ./
    CALL PGM-SNJ0020L.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '回線管理Ｍリスト'
              GOTO ABEND
    END
