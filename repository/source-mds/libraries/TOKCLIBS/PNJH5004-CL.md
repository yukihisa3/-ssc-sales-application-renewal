...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH5004.CL`
...
/. ***********************************************************  ./
/. *     サカタのタネ　トステムビバ新EDI                     *  ./
/. *   SYSTEM-NAME :    発注データ変換処理（新）             *  ./
/. *   JOB-ID      :    PNJH5004                             *  ./
/. *   JOB-NAME    :    トステムビバオンラインシステム　　　 *  ./
/. ***********************************************************  ./
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH5004'  /.PROGRAM-ID./
...
SJH5004B:
    ...
    CALL      PGM-NJH5004B.TOKELIBO,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
    ...
    ELSE
              ...
              CALL PGM-SNJ0730B.TOKELIBO,
                   PARA-(?HIDUKE,?JIKAN,?TOKCD,?KEKA)
    /.##北海道分の件数リストを出力する##./
    SBMJOB JOB-KENLSTHT,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKCLIBO,
