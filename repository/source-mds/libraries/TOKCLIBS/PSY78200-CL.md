...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY78200.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY78200'    /.ﾌﾟﾛｸﾞﾗﾑID./
    ...
    VAR ?P10      ,STRING*3,VALUE-'000'         /.出力枚数./
...
/.##実行PG名称ｾｯﾄ##./
    ?PGNM := 'ロイヤルＨＣ納品明細書出力'
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIBO/TOKKLIB/TOKFLIB/TOKELIB
/.##ﾌﾟﾛｸﾞﾗﾑ開始ﾒｯｾｰｼﾞ##./
    ?MSGX :=  '***   '  && ?PGMID  &&   ' START  ***'
    SNDMSG    ?MSGX,TO-XCTL
/.## ﾜｰｸｽﾃｰｼｮﾝ名取得##./
...
SSY7820I:
    ...
    OVRF      FILE-RHSYUKL2,TOFILE-RHSYUKL2.TOKKLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY7820I.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P5,?P6,?P7
                                         ,?P8,?P9)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
...
/.     OVRPRTF   FILE-PRTF,TOFILE-XU04LP.XUCL,MEDLIB-TOKELIBO
       SNDMSG MSG-'##ｽﾄｯｸﾌｫｰﾑ出力 ##',TO-XCTL  ./
/.##本社ﾚｰｻﾞｰへ出力##./
    IF ?CHK = '1' THEN
       OVRPRTF FILE-XU04LP,TOFILE-KAHMAPRT.XUCL
       SNDMSG MSG-'##本社ﾚｰｻﾞｰ出力##',TO-XCTL
    END
/.##ｶﾄｰﾚｯｸﾚｰｻﾞｰへ出力##./
    IF ?CHK = '2' THEN
       OVRPRTF FILE-XU04LP,TOFILE-KAHMAPRK.XUCL
       SNDMSG MSG-'##ｶﾄｰﾚｯｸﾚｰｻﾞｰ出力##',TO-XCTL
    END
/.##片岡ﾚｰｻﾞｰへ出力##./
    IF ?CHK = '3' THEN
       OVRPRTF FILE-XU04LP,TOFILE-KAHMAPTK.XUCL
       SNDMSG MSG-'##片岡ﾚｰｻﾞｰ出力##',TO-XCTL
    END
/.##鴻巣ﾚｰｻﾞｰへ出力##20100203追加./
    IF ?CHK = '4' THEN
       OVRPRTF FILE-XU04LP,TOFILE-KAHMAP84.XUCL
       SNDMSG MSG-'##鴻巣ﾚｰｻﾞｰ出力##',TO-XCTL
    END
/.##さくらﾚｰｻﾞｰへ出力##20140205追加./
    IF ?CHK = '5' THEN
       OVRPRTF FILE-XU04LP,TOFILE-KAHMAPTK.XUCL
       SNDMSG MSG-'##さくらﾚｰｻﾞｰ出力##',TO-XCTL
    END
/.##ｽﾄｯｸﾌｫｰﾑへ出力##./
    IF ?CHK = ' ' THEN
       OVRPRTF   FILE-PRTF,TOFILE-XU04LP.XUCL,MEDLIB-TOKELIBO
    ...
    END
    OVRF      FILE-RHSYUKL2,TOFILE-RHSYUKL2.TOKKLIB
    CALL      PGM-SSY7825L.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6,?P7
                                         ,?P8,?P9,?P10)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ロイヤルＨＣ納品明細書出力'
              GOTO ABEND END
...
RTN:
    ...
    ?KEKA3 :=  ''
    IF ?P10 = '000'  THEN
       ?KEKA2 :=  '＜この条件では対象データはありません＞'
       ?KEKA3 :=  '指定した範囲指定条件を確認して下さい。'
    END
