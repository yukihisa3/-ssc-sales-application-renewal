...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBM01001.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01001'
    VAR ?STEP     ,STRING*8
...
SBM0105B:
    ...
    CALL     PGM-SBM0105B.TOKELIBO,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0111L:
    ...
    CALL      PGM-SBM0111L.TOKELIBO,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SBM0120V:
    ...
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJYRC,TOFILE-BMSJYRC.BMSFLIB
    OVRF      FILE-BMSJYRW1,TOFILE-BMSJYRW1.BMSFLIB
...
/.  CNVDF FILE-BMSJYRC.BMSFLIB,
    ...
    ACTCHK-@NO
./  CNVDF FILE-BMSJYRC.BMSFLIB,PATH-'/HGNAS/BMSJYRC.CSV',
