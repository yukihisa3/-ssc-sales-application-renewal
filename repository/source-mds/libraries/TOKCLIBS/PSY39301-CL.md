...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY39301.CL`
...
/.##ﾊﾟﾗﾒﾀ情報##./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY39301'
    VAR       ?STEP     ,STRING*8
...
CNVDF000:
    ...
    ?MSGX :=  '***   出力先 = HGNAS  ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
    ?MSGX :=  '***   ﾌｧｲﾙID = NFTEPTF.CSV ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES

    CNVDF FILE-NFTEPTF.TOKDLIB,
          PATH-'//HGNAS/KYUSYU/CSV/NFTEPTF.CSV',MODE-@EXP,ADD-@NO,
          ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES

    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := 'ナフコ店舗パターンＭ　ＣＳＶ出力'
              GOTO ABEND END
