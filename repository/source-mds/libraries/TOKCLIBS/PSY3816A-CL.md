...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY3816A.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY3816A'
    VAR       ?STEP     ,STRING*8
    /.端末別ファイルＩＤ編集用./
              /.端末番号./
...
SSY3817B:
    ...
    END
    CALL      PGM-SSY3817B.TOKELIBO,PARA-(?KANRINO,?SAKUCD)
                                     /.   ?SYUPTN,?SYURUI) ./
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
...
SSY3818B:
    ...
    CALL      PGM-SSY3818B.TOKELIBO,PARA-(?KANRINO,?SAKUCD)
...
SSY3819B:
    ...
    CALL      PGM-SSY3819B.TOKELIBO,PARA-(?KANRINO,?SAKUCD)
...
./  OVRPRTF FILE-PRTF,TOFILE-PRTF.XUCL,DEV-PRINTHKL,
    ...
    OVRF      FILE-HCSMXXX1,TOFILE-?HACSXXX1
    CALL      PGM-SSY3822L.TOKELIBO,PARA-(?SYUPTN,?BUMON,?TANCD)
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := '在庫確認書/確定書　発行'
