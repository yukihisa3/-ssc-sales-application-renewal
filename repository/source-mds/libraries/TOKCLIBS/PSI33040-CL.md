...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI33040.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI33040'
    VAR       ?STEP     ,STRING*8
...
SSI3304B:
    ...
    OVRF      FILE-SETGKFG1,TOFILE-SETGKFG1.TOKFLIB
    CALL      PGM-SSI3304B.TOKELIBO,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
