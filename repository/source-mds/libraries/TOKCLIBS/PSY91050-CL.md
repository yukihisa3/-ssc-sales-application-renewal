...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY91050.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY91050'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
/.##実行PG名称ｾｯﾄ##./
    ?PGNM := '荷個数明細書発行'
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIB/TOKELIBO/ZAIEXCEL/TOKFLIB/TOKKLIB/TOKDTLIB
/.##ﾌﾟﾛｸﾞﾗﾑ開始ﾒｯｾｰｼﾞ##./
    ?MSGX :=  '***   '  && ?PGMID  &&   ' START  ***'
    SNDMSG    ?MSGX,TO-XCTL
/.## ﾜｰｸｽﾃｰｼｮﾝ名取得##./
...
SSY9105I:
    ...
    CALL      PGM-SSY9105I.TOKELIBO,PARA-(?P7,?P5,?P1,?P2,?P3,?P4
                                         ,?P6,?P8)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
...
SSY9106L:
    ...
    OVRF      FILE-DKCENTL1,TOFILE-DSCENTL1.ZAIEXCEL
...
/.  IF  ?WS ^= 'WKSTNHT5'  THEN
    ...
    ELSE
          ...
          OUTQ-XOUTQ1,MEDLIB-TOKELIBO
    END
    CALL      PGM-SSY9106L.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6,?P8)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '荷個数明細書'
              GOTO ABEND END
