...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH1214.CL`
...
/. *   UPDATE      :    2023.09.14 S22481880 取引先枝番対応  *  ./
/. ***********************************************************  ./
...
NJH1214B:
  ...
  /.CALL      PGM-NJH1204B.TOKSOLIB,PARA-(?PARA2) ./
    CALL      PGM-NJH1214B.TOKSOLIB,PARA-(?PARA2)
  /.2023.09.14↑                                  ./
...
/.  CALL      PGM-NJH1208B.TOKSOLIB,PARA-(?PARA)./
    CALL      PGM-NJH1218B.TOKSOLIB,PARA-(?PARA)
/.2023/09/14 ED./
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
...
/.  CALL      PGM-NJH1209B.TOKSOLIB,PARA-(?PARA)   ./
    CALL      PGM-NJH1219B.TOKSOLIB,PARA-(?PARA)
/.2023/09/14 ED./
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
...
/.##CALL  PGM-SNJ0570L.TOKELIBO,PARA-(?HIDUKE,?JIKAN,'00002363')
##./IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
...
SNJ0572L:
    ...
    CALL  PGM-SNJ0570L.TOKELIBO,PARA-(?HIDUKE,?JIKAN,'00023631')
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
...
/.##CALL  PGM-SNJ0570L.TOKELIBO,PARA-(?HIDUKE,?JIKAN2,'00002363')
##./IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
...
SNJ0574L:
    ...
    CALL  PGM-SNJ0570L.TOKELIBO,PARA-(?HIDUKE,?JIKAN2,'00023631')
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
...
SJR0151B:
    ...
    CALL      PGM-SJR0151B.TOKSOLIB,PARA-(?HIDUKE,?JIKAN,?TOKCD)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
...
SJR0280B:
    ...
    CALL      PGM-SJR0280B.TOKSOLIB,PARA-(?HIDUKE,?JIKAN,'00023631')
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
