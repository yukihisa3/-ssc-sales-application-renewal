...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSE87033.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSE87033'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    SORTF     INFILE-SETGKFA.TOKFLIB,
              OUTFILE-SETGK87.TOKKLIB,
              WORKFILE-SETGK87X.TOKWLIB,
              KEY-'F01!A',
              SELECT-'F01!EQ!#00000880',
              SELECT1-'F01!EQ!#00000882',
              SELECT2-'F01!EQ!#00000883',
              SELECT3-'F01!EQ!#00013938',
              SELECT4-'F01!EQ!#00100403',
              SELECT5-'F01!EQ!#00100441',
              SELECT6-'F01!EQ!#00100404',
              SELECT7-'F01!EQ!#00100442',
              RCDL-@DSP
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＳＥＬＥＣＴ　ＳＯＲＴ（資材）】'
                    GOTO ABEND
    END
...
STEP040:
    ...
    CALL  SSE8704B.TOKELIBO,PARA-(?SYORIKN1)
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【請求送信データ作成（資材）】'
                    GOTO ABEND
    END
...
STEP070:
    ...
    SORTF     INFILE-SETGKFA.TOKFLIB,
              OUTFILE-SETGK88.TOKKLIB,
              WORKFILE-SETGK88X.TOKWLIB,
              KEY-'F01!A',
              SELECT1-'F01!EQ!#00001427',
              SELECT2-'F01!EQ!#00014272',
              SELECT3-'F01!EQ!#00014273',
              SELECT4-'F01!EQ!#00017137',
              SELECT5-'F01!EQ!#00100427',
              SELECT6-'F01!EQ!#00100428',
              RCDL-@DSP
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＳＥＬＥＣＴ　ＳＯＲＴ（植物）】'
                    GOTO ABEND
    END
...
STEP080:
    ...
    OVRF FILE-HACKENDT,TOFILE-DCMKENDT.TOKKLIB
    OVRF FILE-SHTDENL1,TOFILE-SHTDENL1.TOKFLIB
    CALL  SSE8704B.TOKELIBO,PARA-(?SYORIKN2)
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【請求送信データ作成（植物）】'
                    GOTO ABEND
    END
