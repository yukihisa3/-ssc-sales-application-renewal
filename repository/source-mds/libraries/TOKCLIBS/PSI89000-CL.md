...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI89000.CL`
...
/. ***********************************************************  ./
/. *   _サカタのタネ　ホームガーデン部　　　　　　　　      *  ./
/. *   SYSTEM-NAME :    リック　　　　　　ＷｅｂＥＤＩ対応   *  ./
/. *   JOB-ID      :    PSI89000                             *  ./
/. *   JOB-NAME    :    リック　　　　　　支払データ取込処理 *  ./
/. ***********************************************************  ./
    PGM
    VAR       ?PGMEC  ,INTEGER
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSI89000'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    FEXPORT FILE-RCSIHASF.TOKKLIB,MODE-@REP,PARA-LICSIH,
            UNIT-1
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（資材）】'
                    GOTO ABEND
...
STEP040:
    ...
    OVRF FILE-RCSIHASF,TOFILE-RCSIHASF.TOKKLIB
    OVRF FILE-SKTSIHL1,TOFILE-SKTSIHL1.TOKKLIB
    CALL SSI8900B.TOKELIBO
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＪＥＤＩＣＯＳデータ変換】'
                    GOTO ABEND
    END
...
STEP050:
    ...
    FIMPORT FILE-CLR1130.TOKKLIB,PARA-LICSIH,UNIT-2
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データクリア処理】'
                    GOTO ABEND
    END
