...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY39200.CL`
...
/. *  （株）サカタのタネ                                     *  ./
/. *   SYSTEM-NAME :    ナフコ出荷支援　　　　　　　　       *  ./
/. *   JOB-NAME    :    ナフコ商品分類パターンマスタ　　　   *  ./
/. *                    取込チェック・更新                   *  ./
/. *   JOB-ID      :    PSY39200  2016/08/23                 *  ./
/. *   UPDATE      :                                         *  ./
/. *                                                         *  ./
/. ***********************************************************  ./
    PGM  (P1-?JIKKOU)
...
/.--------------------------------------------------------------./
    ...
    VAR  ?MSG     ,STRING*99(6)
    VAR  ?MSGX    ,STRING*99
    VAR  ?PGMID   ,STRING*8,VALUE-'PSY39200'
    VAR  ?STEP    ,STRING*8
/.--------------------------------------------------------------./
...
ASSIGN:
    ...
    ASSIGN FILE-NFSHBCSV.TOKDLIB!@XCL
  /.?PGMEC :=    @PGMEC
    ?PGMEM :=    @PGMEM
    ?PGMES :=    @PGMES
    IF     ?PGMEC ^=   1020 THEN
    ...
    ASSIGN FILE-NFSHBWK.TOKDLIB!@XCL
    ?PGMEC :=    @PGMEC
    ?PGMEM :=    @PGMEM
    ?PGMES :=    @PGMES
    IF     ?PGMEC ^=   1020 THEN
       ...
       END
    END
    CLRFILE FILE-NFSHBWK.TOKDLIB

    CASE  ?JIKKOU    OF
          #'2'# /.チェック・更新     ./

                ASSIGN FILE-NFSHBPF.TOKDLIB!@XCL
                ?PGMEC :=    @PGMEC
                ?PGMEM :=    @PGMEM
                ?PGMES :=    @PGMES
                IF     ?PGMEC ^=   1020 THEN
...
SHOM0900:
    ...
    CASE      ?JIKKOU    OF
       #'1'#
           ?OPR1 := '＜ナフコ商品分類パターンマスタチェック＞'
           ?OPR2 := ''
           ?OPR3 := '　ＥＸＣＥＬデータの取り込みを行います。'
           ?OPR4 := '　　・連携するデータは出力済ですか？'
           ?OPR5 := '　　・所定フォルダにセット済ですか？'
       #'2'#
           ?OPR1 := '＜ナフコ商品分類パターンマスタ（更新）＞'
           ?OPR2 := '　　　　　！！確認して下さい！！'
...
PFEXPORT:
    ...
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
                ...
                ?PGMES  := @PGMES
                IF      ?PGMEC ^= 0    THEN
                       ?KEKA4 := 'ＥＸＣＥＬ（商品分類）取込異常！！'
                        GOTO   ABEND
                END

/.##商品分類パターンＥＸＣＥＬデータチェック##./
SSY3920B:
    ...
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
             ...
             /. OVRF   FILE-NFSHBCSV,TOFILE-NFSHBCSV.TOKDLIB
                OVRF   FILE-NFSHBWK2,TOFILE-NFSHBWK2.TOKDLIB
                OVRF   FILE-JYOKEN1,TOFILE-JYOKEN1.TOKFLIB ./
                CALL   PGM-SSY3920B.TOKELIBO,
                       PARA-(?BUMON,?TANCD,
                             ?CNT1,?CNT2)
                ?PGMEC := @PGMEC
                ?PGMES := @PGMES
...
SSY3922B:
    ...
    CASE  ?JIKKOU  OF
          ...
          #'2'# /.チェック・更新　   ./
              /.OVRF   FILE-NFSHBPF,TOFILE-NFSHBPF.TOKDLIB./
                CALL   PGM-SSY3922B.TOKELIBO,
                       PARA-(?BUMON,?TANCD)
                ?PGMEC := @PGMEC
                ?PGMES := @PGMES
                IF     ?PGMEC ^= 0    THEN
...
RTN:
    ...
    RELEASE FILE-NFSHBCSV.TOKDLIB!@XCL/NFSHBPF.TOKDLIB!@XCL/
                 NFSHBWK.TOKDLIB!@XCL
