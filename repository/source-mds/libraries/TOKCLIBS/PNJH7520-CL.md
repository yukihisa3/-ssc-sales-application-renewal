...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH7520.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH7520'  /.PROGRAM-ID./
...
PFEXPORT:
    ...
    FEXPORT FILE-ONLNSNJS.ONLBLIB,MODE-@ADD,PARA-ONLSNS,
            UNIT-2
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NCV7510B:
    ...
    CALL      PGM-NCV7520B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
NJH7520B:
    ...
    OVRF      FILE-SWJYURL1,TOFILE-SWJYURL1.TOKDLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-NJH7520B.TOKELIBO,PARA-(?PARA)
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
