...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSSY1001.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSSY1001'
    VAR       ?STEP     ,STRING*8
...
PSSY1001:
    ...
    CALL      PGM-SSY1001I.TOKELIBO,PARA-(?P1,?P2,?P3,
                                         ?P4,?P6,?P7)
    IF        @PGMEC    ^=   0    THEN
        IF    @PGMEC     =   4010 THEN
...
SSY1002B:
    ...
    CALL      PGM-SSY1002B.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
                                  /.##相手商品ｺｰﾄﾞ順##./
    IF        ?P6        =  '2'   THEN
...
/.  OVRPRTF   FILE-PRTF,TOFILE-XU04LP.XUCL,MEDLIB-TOKELIB  ./
    CALL      PGM-SSY1003L.TOKELIBO,PARA-(?P6)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
SSY1006B:
    ...
    OVRF      FILE-SHWHACF,TOFILE-SHWHACF.TOKFLIB
    OVRF      FILE-SHWHACBF,TOFILE-SHWHACBF.TOKWLIB
    CALL      PGM-SSY1006B.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
