...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBM0200A.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PBM02000'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SBM0205A:
    ...
    CALL      PGM-SBM0205A.TOKELIBO,PARA-(?TORICD,?SIMEBI)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細ワーク作成'
              GOTO ABEND
    END
...
SBM0210A:
    ...
    ELSE
        ...
        END
    END
/.  OVRF      FILE-BMSSIHW3,TOFILE-BMSSIHW3.BMSFLIB
./  OVRF      FILE-BMSSIHW1,TOFILE-BMSSIHW1.BMSFLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
/.  CALL      PGM-SBM0210A.NAVELIBO
./  CALL      PGM-SBM0210C.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細書発行'
              GOTO ABEND
    END
