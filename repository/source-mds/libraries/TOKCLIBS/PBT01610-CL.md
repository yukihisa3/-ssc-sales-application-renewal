...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBT01610.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?CLID    ,STRING*08,VALUE-'PBT01610'        /.CLID         ./
   ...
   /.##パラメタ（個別）##./
    ...
    VAR ?SYURUIK ,STRING*01,VALUE-' '         /.抽出種　入K(カＯ)./
    VAR ?KENSUUK ,STRING*08,VALUE-'00000000'  /.　件数　入K(カＯ)./
    VAR ?SYURUIL ,STRING*01,VALUE-' '         /.抽出種　入L(カ手)./
    VAR ?KENSUUL ,STRING*08,VALUE-'00000000'  /.　件数　入L(カ手)./

    VAR ?CYOUHYOU,STRING*01,VALUE-'N'         /.帳票区分　　　　 ./
   ...
   /.##倉庫別ファイルＩＤ編集用##./
    VAR ?FILE     ,STRING*6,VALUE-'LNKNYK'
    VAR ?FILNM    ,STRING*8,VALUE-'        '
    VAR ?LIBNM    ,STRING*8,VALUE-'TOKKLIB '
    VAR ?FILID    ,NAME
    VAR ?LIBID    ,NAME
    VAR ?LNKNYKF  ,NAME!MOD
    VAR ?LNKNYKX  ,STRING*17
...
/.##実行PG名称ｾｯﾄ##./
    ?CLNM := 'ＬＩＮＫＳ入荷連携データ抽出　　　　　'
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIB/TOKELIBO/TOKFLIB/TOKKLIB/LINKSLIB/BMSFLIB
/.##ﾌﾟﾛｸﾞﾗﾑ開始ﾒｯｾｰｼﾞ##./
    ?MSGX :=  '***   '  && ?CLID  &&   ' START  ***'
    SNDMSG    ?MSGX,TO-XCTL
/.## ﾜｰｸｽﾃｰｼｮﾝ名取得##./
...
FILNMCHG:
    ...
    ?FILID    :=    %NAME(?FILNM)
    ?LIBID    :=    %NAME(?LIBNM)
    ?LNKNYKF  :=    %NCAT(?FILID,?LIBID)
    ?LNKNYKX  :=    %STRING(?LNKNYKF)

    ?MSGX     :=  '## 連携ﾌｧｲﾙ名 = ' && ?LNKNYKX && ' ##'
    SNDMSG    MSG-?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
...
SBT0170B:
    ...
    /. 抽出区分(入)＝Y　のみ実行./
    IF        ?PB04   ^=   'Y'          THEN   GOTO   SBT0180B  END

    ?STEP :=   'SBT0170B'
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL

/.  OVRF      FILE-NYKFILL3,TOFILE-NYKFILL3.TOKFLIB
./  OVRF      FILE-NYKFILL3,TOFILE-NYKFILL3.D365DLIB
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    OVRF      FILE-SHOTBL2,TOFILE-SHOTBL2.TOKFLIB
    OVRF      FILE-LNKNYKF,TOFILE-?LNKNYKF
    CALL      PGM-SBT0170B.TOKELIBO
             ,PARA-(?PB01,?PB02,?PB03,?PB04,?PB05,?PB06,?PB07,
                    ?SYURUIG,?KENSUUG)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
SBT0180B:
    ...
    /. ######################## ./
    /. 抽出区分(横)＝Y　のみ実行./
    IF        ?PB05   ^=   'Y'          THEN   GOTO   SBT0330L  END

    ?STEP :=   'SBT0180B'
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL

  /.OVRF      FILE-NYSFILL4,TOFILE-NYSFILL4.TOKFLIB
    OVRF      FILE-NYSFILL6,TOFILE-NYSFILL6.TOKFLIB
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB./
    OVRF      FILE-LNKNYKF,TOFILE-?LNKNYKF
/.##CALL      PGM-SBT0180B.TOKELIBO
             ,PARA-(?PB01,?PB02,?PB03,?PB04,?PB05,?PB06,?PB07,
                    ?SYURUIH,?KENSUUH)
##./?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '入荷連携データ抽出（横持：入出庫）'
              ?ERR   :=  'E'
              GOTO ABEND
    END
...
SBT0190B:
  ...
  /.OVRF      FILE-SYGFILL3,TOFILE-SYGFILL3.TOKFLIB
    ...
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB./
    OVRF      FILE-LNKNYKF,TOFILE-?LNKNYKF
/.##CALL      PGM-SBT0190B.TOKELIBO
             ,PARA-(?PB01,?PB02,?PB03,?PB04,?PB05,?PB06,?PB07,
                    ?SYURUII,?KENSUUI)
##./?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '入荷連携データ抽出（横持：作業）'
              ?ERR   :=  'E'
              GOTO ABEND
    END
...
SBT0200B:
 /. --------------------------------------------------- ./
 /. 不要処理となったため、SBT0200Bは迂回する 2012/10/10 ./
 /. --------------------------------------------------- ./
    GOTO   SBT0330L
  ...
  /.OVRF      FILE-FURRUIL2,TOFILE-FURRUIL2.TOKKLIB
    ...
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB./
    OVRF      FILE-LNKNYKF,TOFILE-?LNKNYKF
    CALL      PGM-SBT0200B.TOKELIBO
             ,PARA-(?PB01,?PB02,?PB03,?PB04,?PB05,?PB06,?PB07,
                    ?SYURUIJ,?KENSUUJ)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '入荷連携データ抽出（横持：振替）'
              GOTO ABEND
    END
...
FIMPORT1:
    ...
    FIMPORT   FILE-?LNKNYKF,TYPE-@FILE,
              PARA-LINKSTK,UNIT-2,OPR-@NO
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '入荷連携データ転送'
