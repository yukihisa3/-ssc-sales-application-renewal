...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI3701D.CL`
...
/. *********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSI3701D'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    CNVDF FILE-NAFUKOSE.TOKWLIB,PATH-'/HGNAS/NAFUKOSE.CSV',
...
STEP050:
    ...
    OVRF FILE-NAFUKOS1,TOFILE-NAFUKOS1.TOKWLIB
    OVRF FILE-SETGK903,TOFILE-SETGK903.TOKKLIB
    CALL SSI3706B.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '【データ編集】'
              GOTO ABEND END
