...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI33000.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI33000'
    VAR       ?STEP     ,STRING*8
...
SSI3301B:
    ...
    OVRF      FILE-MEIDYAMA,TOFILE-MEIDYAMA.TOKWLIB
    OVRF      FILE-HSHIGKF,TOFILE-SITGKFE.TOKFLIB
    CALL      PGM-SSI3301B.TOKELIBO,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
