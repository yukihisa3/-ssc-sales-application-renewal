...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY38500.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY38500'
    VAR       ?STEP     ,STRING*8
    ...
    VAR       ?KEKKA    ,STRING*1,VALUE-' '        /.取消し結果  ./
...
SSY3850I:
    ...
    OVRDSPF   FILE-DSPF,TOFILE-DSPF.XUCL,MEDLIB-TOKELIBO
    CALL      PGM-SSY3850I.TOKELIBO,PARA-(?BUMON,?TANCD,
                                          ?KANRINO)
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC     =   4010 THEN
...
SSY3851B:
    ...
    CALL      PGM-SSY3851B.TOKELIBO,PARA-(?BUMON,?TANCD,
                                          ?KANRINO,?KEKKA)
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := '本発EXCELデータ取消し処理'
              GOTO ABEND END

RTN:
    CASE    ?KEKKA   OF
      #'0'#
            ?KEKA1 := '本発ＥＸＣＥＬデータ取消し処理が'
            ?KEKA2 := '正常終了しました'
            ?KEKA3 := ''
            ?KEKA4 := ''
      #'1'#
