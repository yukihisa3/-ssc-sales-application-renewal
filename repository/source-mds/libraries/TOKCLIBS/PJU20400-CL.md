...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PJU20400.CL`
...
/.##ワーク定義##./
    ...
    VAR  ?PGMID    ,STRING*8,VALUE-'PJU20400'   /.PROGRAM-ID./
...
PASSIGN:
    ...
    ASSIGN FILE-BACK2048.ONLBLIB!@XCL
    IF   @PGMEC    ^=   0    THEN
         IF ?PGMEC = 4095 THEN
            IF ?REBAK < 5  THEN
               ?REBAK := ?REBAK + 1
               ?MSGX := '## 他JOBで資源使用中です BACK2048 ##'
               SNDMSG   ?MSGX,TO-XCTL
               /.### ﾊﾞｯｸｱｯﾌﾟ ﾏﾁｱﾜｾ ###./
               CALL TWAIT30.XUCL
               GOTO   PASSIGN
...
DATABACK:
    ...
    CALL SCVMSG.TOKCLIBO,PARA-('ｼﾞｭｼﾝF BAK2048 START')
    ?STEP := 'DATABACK'
    ?MSGX := '***   '  && ?STEP   &&   '        ***'
    SNDMSG   ?MSGX,TO-XCTL
...
RTN:
    ...
    CALL SCVMSG.TOKCLIBO,PARA-('ｼﾞｭｼﾝF BAK2048 END  ')
    ?SNDMSGR := ' ﾌｧｲﾙ名 = ' && ?JHFNM
...
ABEND:
    ...
    RELEASE LIB-ONLBLIB!@XCL
    CALL SCVMSG.TOKCLIBO,PARA-('ｼﾞｭｼﾝF BAK2048 ABEND')
    ?PGMECX := %STRING(?PGMEC)
    ?MSG(1) := '### ' && ?PGMID && ' ABEND ' && ' ###'
    ?MSG(2) := '### PGMEC = ' &&
               %SBSTR(?PGMECX,8,4) && '   ###'
