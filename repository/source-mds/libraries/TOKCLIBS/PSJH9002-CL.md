...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH9002.CL`
...
/. ***********************************************************  ./
/. *   _サカタのタネ　ホームガーデン部　　　　　　　　      *  ./
/. *   SYSTEM-NAME :    オージョイフルＷｅｂＥＤＩ対応　　   * ./
/. *   JOB-ID      :    PSJH9002                             *  ./
/. *   JOB-NAME    :    オージョイフル受領データ取込処理　　 *  ./
/. ***********************************************************  ./
    PGM
    VAR       ?PGMEC  ,INTEGER
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSJH9002'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    FEXPORT FILE-OJJRYOSF.ONLBLIB,MODE-@REP,PARA-OJJYURYO
           ,UNIT-1
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（受領）】'
                    GOTO ABEND
...
STEP040:
    ...
    OVRF FILE-OJJRYOSF,TOFILE-OJJRYOSF.ONLBLIB
    OVRF FILE-DCMJYRL1,TOFILE-DCMJYRL1.TOKKLIB
    CALL SJH9002B.TOKELIBO
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受領データ変換】'
                    GOTO ABEND
    END
...
RTN:
    ...
    ?KEKA1 :=  'オージョイフル受領データの取込みが完了しました。'
    ?KEKA2 :=  '受領書又はＣＳＶ発行ＯＫです。'
    ?KEKA3 :=  ''
    ?KEKA4 :=  ''
...
ABEND:
    ...
    OVRDSPF FILE-DSPF,TOFILE-DSPF.TOKELIB,MEDLIB-TOKELIB
    ?KEKA1 :=  'オージョイフル受領データの取込みが異常終了しまし'
