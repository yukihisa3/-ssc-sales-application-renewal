...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/REPLM015.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'REPLM010'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP030:
    ...
    PATH-'/SAKATA/DATA/PRKANRI/MSTPRFIL.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO
    IF   @PGMEC    ^=   0    THEN
...
STEP040:
    ...
    SYNCREPL GRP-'KIHON-MASTER-GROUP2',WAIT-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【マスタデータ送信】'
              GOTO ABEND END
