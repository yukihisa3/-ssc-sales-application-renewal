...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY27300.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY27300'
    VAR       ?STEP     ,STRING*8
...
SSY2730B:
    ...
    CALL      PGM-SSY2730B.TOKELIBO,PARA-(?P1,?P2,?P3,
...
SSY2740L:
    ...
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    CALL      PGM-SSY2740L.TOKELIBO,PARA-(?P1)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
