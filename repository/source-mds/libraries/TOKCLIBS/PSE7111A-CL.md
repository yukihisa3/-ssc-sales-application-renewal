...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSE7111A.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSE7111A'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
/.##実行PG名称ｾｯﾄ##./
    ...
    ?SYORINM1 :=  'Ｊ本田請求処理'
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIB/TOKFLIB/TOKKLIB/TOKDLIB
/.##ﾌﾟﾛｸﾞﾗﾑ開始ﾒｯｾｰｼﾞ##./
    ?MSGX :=  '***   '  && ?PGMID  &&   ' START  ***'
...
/.# SAVFILE FILE-JDSYUKF.TOKKLIB/JDSEIKF.TOKKLIB/
    ...
    SAVFILE FILE-JDSYUKF.TOKKLIB/JHSEIKF.TOKDLIB/
            JHJSEKF.TOKDLIB,TODEV-@NONE,MODE-@USED,TOPATH-'/HGNAS/JHONDA
',REP-@YES
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC ^=   0    THEN
...
SSE7111A:
    ...
    OVRF      FILE-JHSEIKL1,TOFILE-JHSEIKL1.TOKDLIB
    OVRF      FILE-JYOKEN1,TOFILE-JYOKEN1.TOKFLIB
    CALL      PGM-SSE7111A.TOKELIBO,PARA-(?SYORIKN1)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC ^=   0    THEN
              ?KEKA4 :=  '売上請求ＤＴ作成'
...
SSE7112B:
    ...
    OVRF      FILE-JYOKEN1,TOFILE-JYOKEN1.TOKFLIB
    CALL      PGM-SSE7112B.TOKELIBO,PARA-(?SYORIKN2)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC ^=   0    THEN
              ?KEKA4 :=  '仕入実績請求作成'
...
SSE7113B:
    ...
    OVRF      FILE-JHSEIKL1,TOFILE-JHSEIKL1.TOKDLIB
    OVRF      FILE-JHJSEKL1,TOFILE-JHJSEKL1.TOKDLIB
    CALL      PGM-SSE7113B.TOKELIBO,PARA-(?SYORIKN3)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC ^=   0    THEN
              ?KEKA4 :=  '請求データ統合'
