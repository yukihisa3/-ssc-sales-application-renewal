...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY73161.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY73161'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP010:
    ...
    ?OPR1  :=  '【エイトファーム受領書発行処理理】'
    ?OPR2  :=  '受領データ取込済みですか？'
    ?OPR3  :=  '　　　　　　　　　　　　　　　　　　　　　　'
    ?OPR4  :=  'この処理で、受領書の発行が行われます。　　　'
    ?OPR5  :=  '完了後、受領書を確認して下さい。　　　　　　'
...
STEP030:
    ...
    SNDMSG '######################################',TO-XCTL
    SNDMSG '## エイトファーム　受領書発行　開始 ##',TO-XCTL
    SNDMSG '######################################',TO-XCTL
/.##エイトファーム受領データセット##./
    SETPF FILE-DYJRYOF2.TOKKLIB,TOFILE-DYJRYOF.TOKKLIB,
    ADD-@NO,ACTCHK-@NO

    CALL SSY7317L.TOKELIBO,PARA-('00012251')
    IF   ?PGCHK1  = '1'    THEN
         SNDMSG '######################################',TO-XCTL
         SNDMSG '## エイトファーム　受領書発行エラー ##',TO-XCTL
         SNDMSG '######################################',TO-XCTL
    END
