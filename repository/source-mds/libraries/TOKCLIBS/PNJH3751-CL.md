...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH3751.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH3751' /.PROGRAM-ID./
...
/.##ﾌﾟﾛｸﾞﾗﾑｶｲｼﾒｯｾｰｼﾞ##./
    ...
    DEFLIBL TOKELIB/TOKDLIB/TOKFLIB/TOKELIBO/TOKJLIB/TOKKLIB
...
NCV3750C:
    ...
    CALL      PGM-NCV3750C.TOKSOLIB,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
/.  データ変換                                                  ./
NJH3753B: /.←S2245053 追加./

    ?STEP :=   'NJH3753B'
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES

    OVRF      FILE-NFHACSF,TOFILE-?FILLIB
    OVRF      FILE-NFHACL1,TOFILE-NFHACL1.TOKDTLIB
    CALL      PGM-NJH3753B.TOKSOLIB,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
              CALL PGM-SNJ0730B.TOKELIBO,
...
NJH3754B: /.←S2245053 NJH3750BからNJH3754Bに変更./
    ...
    CALL      PGM-NJH3754B.TOKSOLIB,PARA-(?PARA,?KANRINO)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
              CALL PGM-SNJ0730B.TOKELIBO,
  ...
  /.取込結果リスト S2245053 追加./
    ...
    ?SYORINM1 := '【ナフコ発注データ変換・伝票データ作成】'
    ?SYORINM2 := 'ナフコ発注データ取込　　終了しました。　　　　'
    ?SYORINM3 := '件数１：発注件数（オンライン）　'
...
SSY3751T:
    ...
    ?MSGX :=  '#ｶﾝﾘNO.  = ' && ?KANRINO
    SNDMSG    ?MSGX,TO-XCTL
    ?BACHINO := ?HIDUKE && ?JIKAN && ?TOKCD
    ?MSGX :=  '#ﾊﾞｯﾁNO. = ' && ?BACHINO
    SNDMSG    ?MSGX,TO-XCTL
    ?MSGX :=  '#部門    = ' && ?BUMON
    SNDMSG    ?MSGX,TO-XCTL

    CALL SSY3751T.TOKELIBO,PARA-(?KBN,?KANRINO,?HIDUKE,?JIKAN,?TOKCD,
                                 ?BUMON,?KEKKA)
    ?PGMEC := @PGMEC
    IF   ?PGMEC ^= 0 THEN
         ?KEKA := 'K519'
