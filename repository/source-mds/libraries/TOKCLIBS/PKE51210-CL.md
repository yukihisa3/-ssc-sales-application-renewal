...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PKE51210.CL`
...
/. ***********************************************************  ./
    ...
    VAR   ?MSG   ,STRING*99(6)
    VAR   ?MSGX  ,STRING*99
    VAR   ?PGMID ,STRING*8,VALUE-'PKE51210'
    VAR   ?STEP  ,STRING*8
    ...
    VAR   ?FILNMF,STRING*6,VALUE-'SNDFII'   /.ﾌｧｲﾙ名ﾃﾞﾌｫﾙﾄ./
    ...
    VAR   ?SNDFIIF ,NAME!MOD                /.ﾌｧｲﾙ.ﾗｲﾌﾞﾗﾘ./
    VAR   ?SNDFIIFN,STRING*17               /.ﾌｧｲﾙ名表示用./
...
FILNMCHG:
    ...
    ?FILID    :=    %NAME(?FILNM)         /.ﾌｧｲﾙ名名前型変換   ./
    ?LIBID    :=    %NAME(?LIBNM)         /.ﾗｲﾌﾞﾗﾘ名名前型変換 ./
    ?SNDFIIF  :=    %NCAT(?FILID,?LIBID)  /.ﾌｧｲﾙ名.ﾗｲﾌﾞﾗﾘ名    ./
    ?SNDFIIFN :=    %STRING(?SNDFIIF)
    ?MSGX     :=    '## ｴﾗｰF名   = ' && ?SNDFIIFN && ' ##'
    SNDMSG MSG-?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
...
SKE5120B:
    ...
    OVRF      FILE-SNDFINF,TOFILE-?SNDFIIF
    CALL      PGM-SKE5121B.TOKELIBO,PARA-(?P1,?P2,?P7)
    IF        @PGMEC    ^=   0   THEN
              ?KEKA4 :=  '【出荷検品ＤＴ更新】'
              ?SKEKA := '2'
              GOTO ABEND1
