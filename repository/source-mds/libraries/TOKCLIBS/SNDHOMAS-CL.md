...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/SNDHOMAS.CL`
...
SNDDATA:
    ...
    FIMPORT FILE-SNDHOMAS.TOKKLIB,PARA-HORMAC,UNIT-4
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【ＰＣへ送信中】'
              GOTO ABEND END
...
DTBAK1:
    ...
    CNVFILE FILE-SNDHOMS1.TOKKLIB,TOFILE-SNDHOMS2.TOKKLIB,
            ADD-@NO,BF-1
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【前回分をバックアップ】'
              GOTO ABEND END
...
DTBAK2:
    ...
    CNVFILE FILE-SNDHOMAS.TOKKLIB,TOFILE-SNDHOMS1.TOKKLIB,
            ADD-@NO,BF-1
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【今回分をバックアップ】'
              GOTO ABEND END
