...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY39990.CL`
...
/.##ﾊﾟﾗﾒﾀ情報##./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY39990'
    VAR       ?STEP     ,STRING*8
...
SHOM0900:
    ...
    ?OPR1  :=  '　　＜ナフコ商品マスタ　商品分類ＣＤ一括置換＞'
    ?OPR2  :=  '　　　　商品分類コードの一括置換を行います'
...
ASSIGN:
    ...
    ASSIGN FILE-NFSHOWK.TOKDLIB!@XCL
    ?PGMEC :=    @PGMEC
    ?PGMEM :=    @PGMEM
    ?PGMES :=    @PGMES
    IF     ?PGMEC ^=   1020 THEN
    ...
    ASSIGN FILE-NFSHOMS.TOKDLIB!@XCL
    ?PGMEC :=    @PGMEC
    ?PGMEM :=    @PGMEM
    ?PGMES :=    @PGMES
    IF     ?PGMEC ^=   1020 THEN
...
CNVDF000:
    ...
    ?MSGX :=  '***   取込元 = HGNAS  ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
    ?MSGX :=  '***   ﾌｧｲﾙID = NFSHOWK.CSV ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES

    CNVDF FILE-NFSHOWK.TOKDLIB,
          PATH-'//HGNAS/NFSHOWK.CSV',
          MODE-@IMP,
          ADD-@NO,
          ACTCHK-@NO
...
SSY3999B:
    ...
    CALL  SSY3999B.TOKELIBO,PARA-(?BUMON,?TANCD)
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := 'ナフコ商品マスタ商品分類ＣＤ一括置換'
              GOTO ABEND END
