...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/CNI10210.CL`
...
/.-----------------------------------------------------------./
    ...
    VAR       ?JMSG   ,STRING*50
    VAR       ?PGKEKAJ,STRING*1,VALUE-' '         /.ﾚﾌﾟﾘｶ結果./
    VAR       ?PGKEKAM,STRING*1,VALUE-' '         /.ﾚﾌﾟﾘｶ結果./
    VAR       ?PGKEKAZ,STRING*1,VALUE-' '         /.ﾚﾌﾟﾘｶ結果./
...
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
STEP040:
    DEFLIBL TOKELIB/TOKFLIB/TOKWLIB/TOKKLIB
              /.リモートワークステーションの終了./
              ...
              DACTWS WS-WKSTN600,SIGNOFF-@YES  /.## 以下、九州　 ##./
              DACTWS WS-WKSTN601,SIGNOFF-@YES
              DACTWS WS-WKSTN602,SIGNOFF-@YES
              DACTWS WS-WKSTN603,SIGNOFF-@YES
              DACTWS WS-WKSTN604,SIGNOFF-@YES
              DACTWS WS-WKSTN605,SIGNOFF-@YES
              DACTWS WS-WKSTN606,SIGNOFF-@YES
              DACTWS WS-WKSTN607,SIGNOFF-@YES
              DACTWS WS-WKSTN608,SIGNOFF-@YES
              DACTWS WS-WKSTN609,SIGNOFF-@YES
              DACTWS WS-WKSTN610,SIGNOFF-@YES
              DACTWS WS-WKSTN611,SIGNOFF-@YES
              DACTWS WS-WKSTNHTU,SIGNOFF-@YES
              DACTWS WS-WKSTNHS1,SIGNOFF-@YES
              DACTWS WS-WKSTNHS2,SIGNOFF-@YES
              DACTWS WS-WKSTNHS3,SIGNOFF-@YES
...
STEP080:
 ...
 /. SAVLIB LIB-TOKFLIB/ONLBLIB,TODEV-DAT,ADD-@NO,REWIND-@NO,
           MODE-@USED   ./
    SAVLIB LIB-TOKFLIB/TOKKLIB/ONLBLIB/OSKFLIB,TODEV-LTO,
           REWIND-@BOTH,MODE-@USED,COMPRESS-@YES
    IF   @PGMEC    ^=   0    THEN
         ?KEKA4 := '【ライブラリバックアップ】'
         GOTO ABEND
    END
...
STEP090:
    ...
    IF   @PGMEC    ^=   0    THEN
         ?KEKA4 := '【資源獲得】'
    DEFLIBL LIBL-TOKFLIB/TOKELIB/TOKKLIB
...
/.## 2014/03/18 計上済みで在庫未更新のデータも更新に変更 ##./
/.  CALL      PGM-TNI0020B.TOKELIB./
    CALL      PGM-TNI0020C.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【在庫引落】'
              GOTO ABEND END
...
STEP110:
    ...
    OVRF      FILE-TOKUJ,TOFILE-TOKUJ.TOKKLIB    /. 20111110 ADD ./
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
...
CPY:
    ...
    IF        @PGMEC    ^=   0    THEN
              ...
              GOTO ABEND END
    /. 20111110 ADD ./
    CNVFILE FILE-TOKUJ.TOKKLIB,TOFILE-TOKUJWK.TOKWLIB,BF-4
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【ＤＴコピー】'
              GOTO ABEND END
...
STEP130:
    ...
    OVRF      FILE-TOKUJ,TOFILE-TOKUJZ.TOKKLIB     /.20111110 ADD ./
...
STEP171:
    ...
    CNVFILE FILE-TOKUJWK.TOKWLIB,TOFILE-TOKUJ.TOKWLIB,ADD-@NO,
            BF-4
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【計上ＤＴ退避３】'
              GOTO ABEND END
...
STEP172:
    ...
    CNVFILE FILE-TOKUJZ.TOKKLIB,TOFILE-TOKUJ.TOKWLIB,BF-4
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【計上ＤＴ退避４】'
              GOTO ABEND END
...
STEP180:
 ...
 /. OVRF      FILE-TOKU,TOFILE-TOKU.TOKWLIB    ./
    OVRF      FILE-TOKUJ,TOFILE-TOKUJ.TOKWLIB
...
STEP190:
    ...
    OVRF      FILE-JISSSYU1,TOFILE-JISSSYU1.TOKKLIB  /.20111110 ADD./
...
STEP210:
 ...
 /. SAVFILE FILE-BACK0128.ONLBLIB/BACK0256.ONLBLIB/
            ...
            MODE-@USED       ./
 /. SAVFILE FILE-BACK0128.ONLBLIB/BACK0256.ONLBLIB/
            BACK2048.ONLBLIB/BACK0264.ONLBLIB,TODEV-LTO,
            REWIND-@BOTH,MODE-@USED,COMPRESS-@YES   ./
 /.##2014/01/07 ＢＭＳデータバックアップ追加 ##./
 /. SAVFILE FILE-BACK0128.ONLBLIB/BACK0256.ONLBLIB/
            BACK2048.ONLBLIB/BACK0264.ONLBLIB/
            BACK1200.ONLBLIB,TODEV-LTO,MODE-@USED,
            COMPRESS-@YES  ./
 /.##2016/06/22 ＥＤＩＣデータバックアップ追加 ##./
    SAVFILE FILE-BACK0128.ONLBLIB/BACK0256.ONLBLIB/
            BACK2048.ONLBLIB/BACK0264.ONLBLIB/
            BACK1200.ONLBLIB/BACK0384.ONLBLIB,TODEV-LTO,MODE-@USED,
            COMPRESS-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【オンラインＤＴバックアップ】'
              GOTO ABEND
    ELSE
              ...
              /.## 2014/01/07 ＢＭＳデータ初期化追加 ##./
              CLRFILE BACK1200.ONLBLIB
              /.## 2016/06/22 ＥＤＩＣデータ初期化追加 ##./
              CLRFILE BACK0384.ONLBLIB
    END
...
STEP215:
 ...
 /. SAVLIB LIB-WSLOGLIB,TODEV-MO,MODE-@USED    ./
    SAVLIB LIB-WSLOGLIB,TODEV-LTO,REWIND-@BOTH,MODE-@USED,
           COMPRESS-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【端末操作ログ採取】'
              GOTO ABEND
...
STEP217:
    ...
    CALL      PGM-SJS0210B.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【各実績データ累積処理】'
              GOTO ABEND END

/.#################################################################./
/.##ＡＳＳＩＧＮ解除　ＡＳＳＩＧＮのままではレプリカ出来ない為   ##./
/.#################################################################./
/.##各実績データレプリカ処理##./
STEP2171:
...
STEP218:
    ...
    CALL REPLJ010.TOKCLIBO,PARA-(?PGKEKAJ)
    IF        ?PGKEKAJ   =   '2'  THEN
              ?KEKA4 := '【各実績データレプリカ処理】'
              GOTO ABEND
    ELSE
              ?MSGX :=  '## 各実績データレプリカ処理（正常） ##'
              SNDMSG    ?MSGX,TO-XCTL
    END
...
STEP219:
    ...
    CALL REPLM010.TOKCLIBO,PARA-(?PGKEKAM)
    IF        ?PGKEKAM   =   '2'  THEN
              ?KEKA4 := '【マスタレプリカ処理（差分）】'
              GOTO ABEND
    ELSE
              ?MSGX :=  '## マスタレプリカ処理（差分）正常 ##'
              SNDMSG    ?MSGX,TO-XCTL
    END
...
STEP220:
    ...
    CALL REPLZ010.TOKCLIBO,PARA-(?PGKEKAZ)
    IF        ?PGKEKAZ   =   '2'  THEN
              ?KEKA4 := '【在庫マスタレプリカ処理（一括）】'
              GOTO ABEND
    ELSE
              ?MSGX :=  '## 在庫マスタレプリカ処理（一括）正常##'
              SNDMSG    ?MSGX,TO-XCTL
    END
