...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/REPLT020.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'REPLT020'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP012:
    ...
    PATH-'/SAKATA/DATA/PRKANRI/TEGPRSVR.CSV',MODE-@IMP,ADD-@NO,
    ACTCHK-@NO
    IF   @PGMEC    ^=   0    THEN
         ?KEKA4 := '【サーバー側　更新結果取得】'
         GOTO ABEND
...
STEP040:
    ...
    PATH-'/SAKATA/DATA/PRKANRI/TEGPRFIL.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO
    IF   @PGMEC    ^=   0    THEN
...
STEP050:
    ...
    SYNCREPL GRP-'TEGAKI-DENPYO-REPLICA2',MODE-@ALL,OUTTYPE-@CRT,
             WAIT-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【実績連携データ送信】'
              GOTO ABEND END
