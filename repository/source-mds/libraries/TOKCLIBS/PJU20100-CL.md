...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PJU20100.CL`
...
/.##ワーク定義##./
    ...
    VAR  ?PGMID    ,STRING*8,VALUE-'PJU20100'   /.PROGRAM-ID./
...
PASSIGN:
    ...
    ASSIGN FILE-BACK0128.ONLBLIB!@XCL
...
DATABACK:
    ...
    CALL SCVMSG.TOKCLIBO,PARA-('ｼﾞｭｼﾝF BAK 128 START')
    ?STEP := 'DATABACK'
    ?MSGX := '***   '  && ?STEP   &&   '        ***'
    SNDMSG   ?MSGX,TO-XCTL
...
RTN:
    ...
    CALL SCVMSG.TOKCLIBO,PARA-('ｼﾞｭｼﾝF BAK 128 END  ')
    ?SNDMSGR := ' ﾌｧｲﾙ名 = ' && ?JHFNM
...
ABEND:
    ...
    RELEASE LIB-ONLBLIB!@XCL
    CALL SCVMSG.TOKCLIBO,PARA-('ｼﾞｭｼﾝF BAK 128 ABEND')
    ?PGMECX := %STRING(?PGMEC)
    ?MSG(1) := '### ' && ?PGMID && ' ABEND ' && ' ###'
    ?MSG(2) := '### PGMEC = ' &&
               %SBSTR(?PGMECX,8,4) && '   ###'
