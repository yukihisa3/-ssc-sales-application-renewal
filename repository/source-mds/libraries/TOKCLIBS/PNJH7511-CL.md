...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH7511.CL`
...
/.##ﾊﾟﾗﾒﾀ定義##./
    ...
    VAR       ?TOKCD1   ,STRING*8,VALUE-'00264901' /.受信取引先./
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH7511'  /.PROGRAM-ID./
...
PFEXPORT:
    ...
    FEXPORT FILE-ONLNSNHZ.ONLBLIB,MODE-@ADD,PARA-ONLSNS,
            UNIT-4
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NCV7511B:
    ...
    CALL      PGM-NCV7511B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
NJH7505B:
    ...
    CALL      PGM-NJH7511B.TOKELIBO,PARA-(?PARA)
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
...
/.##振分け件数ﾘｽﾄ出力##./
    CALL SCVMSG.TOKELIB,PARA-('ｹﾝｽｳ LST 26490 START')
KENLST:
...
/.##振分け件数ﾘｽﾄ出力##./
    CALL SCVMSG.TOKELIB,PARA-('ｹﾝｽｳ LST 264901START')
KENLST1:
    ...
    SBMJOB JOB-KENLST1,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKCLIBO,
           VSIZE-5,RSIZE-16,LOG-@YES!1024,PARA-(?HIDUKE,?JIKAN,
           ?TOKCD1,?LINE,?YUSEN)
