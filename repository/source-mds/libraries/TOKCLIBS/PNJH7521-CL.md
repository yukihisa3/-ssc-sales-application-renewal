...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PNJH7521.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH7521'  /.PROGRAM-ID./
...
PFEXPORT:
    ...
    FEXPORT FILE-ONLNSNJZ.ONLBLIB,MODE-@ADD,PARA-ONLSNS,
            UNIT-5
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NCV7521B:
    ...
    CALL      PGM-NCV7521B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
NJH7521B:
    ...
    CALL      PGM-NJH7521B.TOKELIBO,PARA-(?PARA)
    ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
