...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY88500.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY88500'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY8850I:
    ...
    CALL      PGM-SSY8850I.TOKELIBO,PARA-(?P7,?P5,?P1,?P2,?P3,?P4)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
...
SSY8851B:
    ...
    CALL      PGM-SSY8851B.TOKELIBO,PARA-(?P1,?P2,?P3,?P4)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'データ抽出'
              GOTO ABEND END
...
SSY8852L:
    ...
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    CALL      PGM-SSY8852L.TOKELIBO,PARA-(?P1,?P2,?P3,?P4)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '個口数記入一覧表'
              GOTO ABEND END
