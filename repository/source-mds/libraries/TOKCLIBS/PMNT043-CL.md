...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PMNT043.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PMNT043 '
    VAR       ?STEP     ,STRING*8
...
SMNT043:
    ...
    OVRF      FILE-JYOKEN1,TOFILE-JYOKEN1.TOKFLIB
    OVRF      FILE-HSHOTBR1,TOFILE-HSHOTBR1.TOKKLIB
    CALL      PGM-SMNT043.TOKELIBO,PARA-(?SOKCD,?DSOKCD,
              ?TANCD,?BUMON,?NYURYOKU,?STIME)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
