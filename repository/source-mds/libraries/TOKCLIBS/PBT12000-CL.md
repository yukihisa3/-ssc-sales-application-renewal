...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBT12000.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?CLID    ,STRING*08,VALUE-'PBT12000'        /.CLID         ./
   ...
   /.##パラメタ（出荷）##./
    ...
    VAR ?PA151   ,STRING*08,VALUE-'00000000'   /.管理番号-手書./
   ...
   /.##倉庫別ファイルＩＤ編集用##./
    ...
    VAR ?LIBNM    ,STRING*8,VALUE-'TOKDLIB '
    VAR ?FILID    ,NAME
    VAR ?LIBID    ,NAME
...
SBT0320B:
    ...
    /. ナフコ(手)は専用処理へ./
    IF        ?PA15    =   '00137607'   THEN   GOTO   SBT1110B  END
...
/.##出荷連携データ抽出（ナフコ手書き）VER2##./
SBT1110B:

    /. 抽出区分(手)＝Y　のみ実行./
    IF        ?PA05   ^=   'Y'          THEN   GOTO   SBT0050B  END
    /. ナフコ(手)は専用処理へ./
    IF        ?PA15   ^=   '00137607'   THEN   GOTO   SBT0050B  END
...
/.##出荷連携データ抽出（カインズオンライン）累積用##./
  ...
  /.IF    ?PA03   ^=   ' '        THEN   GOTO   SBT0350B  END./
    ...
    CALL      PGM-SBT0340B.TOKELIBO /.再送モードで抽出./
...
CLRFILE1:
    ...
    /.TEST
    SETPF FILE-?LNKSYKF,TOFILE-LNKSYKTK.INO,ADD-@NO
       TEST ./
