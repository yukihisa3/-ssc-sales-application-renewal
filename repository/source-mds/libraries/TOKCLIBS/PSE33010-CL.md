...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSE33010.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSE33010'
    VAR       ?STEP     ,STRING*8
...
STEP00:
    ...
    ?OPR1  :=  '処理名：ジョイフル山新　請求送信ＤＴ作成処理'
    ?OPR2  :=  '　ジョイフル山新の請求送信データを作成しま'
    ?OPR3  :=  '　す。確認して下さい。'
    ?OPR4  :=  '　データは下記のフォルダーに出力されます。'
    ?OPR5  :=  '　（Ｄ：￥ＹＡＭＡＳＩＮ内です。）'
...
SSE3301B:
    ...
    OVRF      FILE-SEIDYAMA,TOFILE-SEIDYAMA.TOKWLIB
    CALL      PGM-SSE3301B.TOKELIBO,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
/.##############################################################./
    ...
    FIMPORT FILE-SEIDYAMA.TOKWLIB,PARA-YAMASIN,UNIT-1
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
