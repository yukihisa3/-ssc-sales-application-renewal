...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/REPLZ010.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'REPLZ010'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP020:
    ...
    OVRF FILE-JISPCCL1,TOFILE-ZAIPCCL1.TOKWLIB
...
STEP030:
    ...
    CNVDF FILE-ZAIPCCF.TOKWLIB,
    PATH-'/SAKATA/DATA/PRKANRI/ZAIPRFIL.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO
    IF   @PGMEC    ^=   0    THEN
...
STEP035:
    ...
    CALL SPD9996B.TOKELIBO,PARA-(?SCNT)
    IF   @PGMEC    ^=   0    THEN
         ?KEKA4 := '【在庫マスタ件数カウント】'
         GOTO ABEND
    END
...
STEP040:
    ...
    SYNCREPL GRP-'ZAIKO-MASTER-GROUP2',MODE-@ALL,WAIT-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '【在庫マスタデータ送信】'
              GOTO ABEND END
