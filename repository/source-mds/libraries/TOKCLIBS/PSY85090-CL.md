...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY85090.CL`
...
/. *   JOB-ID      :    PSY85090                             *  ./
/. *   JOB-NAME    :    くろがねや出荷リスト出力             *  ./
/. ***********************************************************  ./
    PGM  (P1-?KBN)
/.###ﾊﾟﾗﾒﾀ定義####./
    PARA ?KBN     ,STRING*1,IN,VALUE-' ' /.出力場所判定./
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY85090'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY8507I:
    ...
    CALL      PGM-SSY8507I.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,
                                          ?P5,?P6,?P7,?P8)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
    ...
    IF  ?KBN = '1'  THEN
        ?MSGX :=  '# 出力場所 = ' && '本社' && ' #'
        SNDMSG    ?MSGX,TO-XCTL
        OVRPRTF   FILE-XU04LP,TOFILE-KUROGPRT.XUCL
    END
    IF  ?KBN = '2'  THEN
        ?MSGX :=  '# 出力場所 = ' && '富岡' && ' #'
        SNDMSG    ?MSGX,TO-XCTL
        OVRPRTF   FILE-XU04LP,TOFILE-KUROGPRK.XUCL
    END
    IF  ?KBN = '3'  THEN
        ?MSGX :=  '# 出力場所 = ' && 'その他' && ' #'
        OVRPRTF   FILE-XU04LP,TOFILE-KUROGPRT.XUCL
    ...
    IF  ?KBN = '4'  THEN
        ?MSGX :=  '# 出力場所 = ' && '片岡' && ' #'
        OVRPRTF   FILE-XU04LP,TOFILE-KUROGPKT.XUCL
    END
...
SSY8509L:
    ...
    OVRF      FILE-KGSYUKL2,TOFILE-KGSYUKL2.TOKKLIB
    CALL      PGM-SSY8509L.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6,?P7,
                                          ?P8)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '出荷リスト出力'
              GOTO ABEND END
