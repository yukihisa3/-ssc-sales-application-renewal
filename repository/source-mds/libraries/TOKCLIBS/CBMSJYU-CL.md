...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/CBMSJYU.CL`
...
/.##ワーク定義##./
    ...
    VAR  ?PGMID    ,STRING*8,VALUE-'CBMSJYU'   /.PROGRAM-ID./
...
/.##ワーク定義２##./
    ...
    VAR  ?LIBNM31 ,STRING*08,VALUE-'TOKCOLIB'  /.ﾗｲﾌﾞﾗﾘ名３./
...
/.##2012/11/20 ADD ST##./
    VAR  ?KENSU1  ,STRING*8,VALUE-'00000000'   /.受信件数（全体）./
    VAR  ?KENSU2  ,STRING*8,VALUE-'00000000'   /.受信件数（発注）./
    VAR  ?KENSU3  ,STRING*8,VALUE-'00000000'   /.受信件数（受領）./
    VAR  ?KENSU4  ,STRING*8,VALUE-'00000000'   /.受信件数（受訂）./
    VAR  ?KENSU5  ,STRING*8,VALUE-'00000000'   /.受信件数（返品）./
    VAR  ?KENSU6  ,STRING*8,VALUE-'00000000'   /.受信件数（支払）./
/.##2012/11/20 ADD ED##./
    /.エラーメール発行用ワーク./
...
/.##プログラム開始メッセージ##./
    ...
    SNDMSG MSG-'＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊',TO-XCTL
    SNDMSG MSG-'＊　受信待受処理・ＢＭＳ系開始　＊',TO-XCTL
    SNDMSG MSG-'＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊',TO-XCTL
...
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIB/TOKFLIB/TOKJLIB/BMSFLIB/ONLBLIB/TOKDTLIB
...
/.##受信Ｆバックアップ##./
/.  SBMJOB JOB-BACK1200,JOBD-NEWONL1.XUCL,JOBK-@B,
    PGM-PJU20500.TOKCLIBO,LOG-@YES!1024,
    PTY-5,PGMEL-@I/@L/@S/@T,
...
/.##メッセージ種別振分##./
MSGFURI1:
    CALL SCVMSG.TOKCLIBO,PARA-('ﾒｯｾｰｼﾞｼｭ ﾌﾘﾜｹｼｮﾘ  ST')
    ?STEP := 'MSGFURI1'
    ?MSGX := '***   '  && ?STEP   &&   '        ***'
    SNDMSG  ?MSGX,TO-XCTL
   ...
   /.日本語制御バイト排除処理./
    CASE  ?TORICD OF
        # '00921084' #         /.カインズ./
        OVRF FILE-BMSRCVF,TOFILE-?JFILNM
        CALL PGM-SBMC000B.TOKELIBO
        # '00013020' #         /.バロー資材./
        OVRF FILE-BMSRCVF,TOFILE-?JFILNM
        CALL PGM-SBMC000B.TOKELIBO
        # '00013052' #         /.バロー植物./
...
/.## SBM0011B  28,29 DELETE VER ##./
/.--CALL PGM-SBM0011B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,./
/.--CALL PGM-SBM0010B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
    IF  ?TORICD = '00013020'  THEN
    CALL PGM-SSY4450B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
    ELSE
      CALL PGM-SBM0012B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
    END----./
    CASE  ?TORICD OF
        # '00013020' #         /.バロー./
/.------CALL PGM-SSY4450B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
------./CALL PGM-SSY4452B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00013052' #         /.バロー植物 ./
/.------CALL PGM-SSY4450B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
------./CALL PGM-SSY4452B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
        ...
        CALL PGM-SSY4452B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00271934' #         /.コメリ./
        CALL PGM-SBMK010B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
/.------CALL PGM-SBMK011B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
------./# '00921084' #         /.カインズ./
        CALL PGM-SBMC010B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00321717' #         /.セキチュー資材./
        CALL PGM-SSY4454B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00926061' #         /.セキチュー植物./
        CALL PGM-SSY4454B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00116501' #         /.ムサシ./
        CALL PGM-SSY4456B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00001041' #         /.山新　./
        CALL PGM-SSY4455B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00005116' #         /.ジュンテンドー　./
        CALL PGM-SSY4457B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00072028' #         /.島忠./
        CALL PGM-SSY4458B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00038709' #         /.ビバホーム./
        CALL PGM-SSY4459B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00003043' #         /.ジョイフルエーケー./
        CALL PGM-SSY5390B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00000616' #         /.ハンズマン　　　　./
        CALL PGM-SSY4520B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        # '00031100' #         /.ＨＩヒロセ　　　　./
        CALL PGM-SSY4460B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
        ELSE                   /.イオン関係./
        CALL PGM-SBM0012B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
...
/.##流通ＢＭＳメッセージ種件数リスト##./
MSGFURI2:
    CALL SCVMSG.TOKCLIBO,PARA-('BMSﾒｯｾｰｼﾞｼｭｹﾝLST  ST')
    ?STEP := 'MSGFURI2'
    ?MSGX := '***   '  && ?STEP   &&   '        ***'
    SNDMSG  ?MSGX,TO-XCTL

/.  OVRF FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    CALL PGM-SBM0020L.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
./  SBMJOB JOB-FURILSTB,JOBD-CVCS.XUCL,JOBK-@B,
           PGM-SBM0020L.TOKELIBO,LOG-@YES!1024,PTY-5,PGMEL-@I/@L/
           @S/@T,LIBL-TOKFLIB/TOKELIB/TOKJLIB/TOKELIBO/BMSFLIB,
           PARA-(?JDATE,?JTIME,?TORICD,
             ?KENSU1,?KENSU2,?KENSU3,?KENSU4,?KENSU5,?KENSU6)
/.  ?PGMEC := @PGMEC
    ?PGMEM := @PGMEM
    IF  ?PGMEC ^= 0 THEN
    ...
    END
./
    CALL SCVMSG.TOKCLIBO,PARA-('BMSﾒｯｾｰｼﾞｼｭｹﾝLST  ED')
    ?MSGX := '### 変換開始'
    SNDMSG   ?MSGX,TO-XCTL
    ?MSGX := '### 取引先 = ' && ?TORICD
    SNDMSG   ?MSGX,TO-XCTL
...
/.  SBMJOB JOB-KENLST,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKELIB, ./
    SBMJOB JOB-KENLSTBM,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKCLIBO,
...
/.     4.結果コード                ./
    ...
    CALL SCVMSG.TOKCLIBO,PARA-('ｲｼﾞｮｳ ｼｭｳﾘｮｳ  CBMSJYU')
