...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSSY0005.CL`
...
/.  PARA ?PAR1  ,STRING*8,IN,VALUE-'00000000'
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSSY0005'
    VAR       ?STEP     ,STRING*8
...
PSSY0005:
    ...
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
    SNDMSG    ?PA1,TO-XCTL.@ORGPROF,JLOG-@YES
    SNDMSG    ?PA2,TO-XCTL.@ORGPROF,JLOG-@YES
    SNDMSG    ?PA3,TO-XCTL.@ORGPROF,JLOG-@YES
...
SSY0006B:
    ...
    CALL      PGM-SSY0006B.TOKELIB,PARA-(?P1,?P2,?P3,?P4,?P5,?P6)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
                                  /.##相手商品ｺｰﾄﾞ順##./
    IF        ?P6        =  '2'   THEN
...
/.  OVRPRTF   FILE-PRTF,TOFILE-XU04LP.XUCL,MEDLIB-TOKELIB  ./
    CALL      PGM-SSY0021L.TOKELIB,PARA-(?P5)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
