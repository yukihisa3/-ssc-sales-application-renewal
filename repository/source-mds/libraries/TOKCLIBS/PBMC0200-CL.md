...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBMC0200.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBMC0200'
    VAR ?STEP     ,STRING*8
...
SBMC020B:
  ...
  /.OVRF     FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    ...
    CALL     PGM-SBMC020B.TOKELIBO,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出（カインズ）'
              GOTO ABEND
    END
...
SBMC030L:
  ...
  /.OVRF      FILE-BMSJY2W1,TOFILE-BMSJY2W1.BMSFLIB ./
    CALL      PGM-SBMC030L.TOKELIBO,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行（カインズ）'
              GOTO ABEND
    ELSE
              GOTO RTN
    END

/.##流通BMS受領書ＣＳＶデータ出力VER3##./
SBMC040V:
  ...
  /.OVRF      FILE-BMSJYRC,TOFILE-BMSJY2C.BMSFLIB
    ...
    CALL      PGM-SBMC040V.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力（カインズ）'
              GOTO ABEND
    END
...
CNVDF000:
    ...
    CNVDF FILE-BMSJY3C.BMSFLIB,
/.  PATH-'/SAKATA/DATA/PRKANRI/BMSJY3C.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO    ./
    PATH-'/NAVPC/BMSJY3C.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領ＣＳＶデータ転送（カインズ'
              GOTO ABEND
    ELSE
              GOTO RTN
    END
