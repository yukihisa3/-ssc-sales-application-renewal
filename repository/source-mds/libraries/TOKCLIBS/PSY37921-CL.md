...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY37921.CL`
...
/.##ﾊﾟﾗﾒﾀ情報##./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY37921'
    VAR       ?STEP     ,STRING*8
...
CNVDF000:
    ...
    ?MSGX :=  '***   出力先 = HGNAS  ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
    ?MSGX :=  '***   ﾌｧｲﾙID = NFSHOMS.CSV ***'
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES

  /.社内テスト用
    CNVDF FILE-NFSHOMS.TOKDLIB,
          PATH-'//NAVPC/NFSHOMS.CSV',MODE-@EXP,ADD-@NO,
          ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
./  CNVDF FILE-NFSHOMS.TOKDLIB,
          PATH-'//HGNAS/NFSHOMS.CSV',MODE-@EXP,ADD-@NO,
          ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES

    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := 'ナフコ商品マスタリストＣＳＶ出力'
              GOTO ABEND END
