...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH7799.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PSJH7799'  /.PROGRAM-ID./
    ...
    VAR       ?MAISU    ,STRING*8,VALUE-'        ' /.枚数./
    VAR       ?KENSU    ,STRING*8,VALUE-'        ' /.件数./
/.##ﾃﾞｰﾀ変換PG用ﾊﾟﾗﾒﾀ##./
    VAR ?PARA,STRING*30,VALUE-'                              '
/.##結果FLG用##./
...
SCV0073B:
    ...
    CALL      PGM-SCV0079B.TOKOBJ,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
/.##ﾃﾞｰﾀ変換PGﾍのﾊﾟﾗﾒﾀ作成##./
    ?PARA := ?HIDUKE && ?JIKAN && ?LINE && ?YUSEN && ?MAISU && ?KENSU
...
/.##受信ﾌｧｲﾙノ編集##./
   ...
   /.##変換開始時間取得##./
    ...
    CALL SCVMSG.TOKELIB,PARA-('ｼﾞｭｼﾝDT ﾍﾝｶﾝ 1 START')
/.##データ変換##./
SJH5702B:
    ...
    CALL      PGM-SJH5702B.TOKOBJ,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '92'
    ...
    CALL SCVMSG.TOKELIB,PARA-('ｼﾞｭｼﾝDT ﾍﾝｶﾝ 2 START')
/.##データ変換##./
SJH5703B:
    ...
    OVRF      FILE-KMJOHOL1,TOFILE-KMJOHOL1.TOKFLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-SJH5703B.TOKOBJ,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '92'
              CALL PGM-SCV0090B.TOKELIB,
