...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY3940Z.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY3940Z'
    VAR       ?STEP     ,STRING*8
    /.端末別ファイルＩＤ編集用./
              /.端末番号./
    ...
    VAR       ?NHACLSTW ,STRING*17,VALUE-'                 '
              ...
              /.発注データＣＳＶ（店舗）./
    VAR       ?CSVT     ,STRING*4,VALUE-'NSVT'
    VAR       ?CSVTF    ,STRING*1,VALUE-'F'
    VAR       ?CSVTXXXF ,NAME!MOD
              /.発注データＣＳＶ（商品）./
    VAR       ?CSVS     ,STRING*4,VALUE-'NSVS'
...
/.##IF  ?P4  =  '01'  THEN
    ...
    IF  ?P4  =  '84'  THEN
        ...
        SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
        OVRPRTF FILE-PRTF,TOFILE-PRTF.XUCL,DEV-PRINTHKL,
        OUTQ-XXPKOUQL,MEDLIB-TOKELIBO
    END
    IF  ?P4  =  'TV'  THEN
        ?MSGX :=  '出力先 = ＦＬ西尾'
        SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
        OVRPRTF FILE-PRTF,TOFILE-PRTF.XUCL,DEV-PRINTHKL,
        OUTQ-XXPNISQL,MEDLIB-TOKELIBO
    END
    OVRF      FILE-HCSMXXX1,TOFILE-?HACSXXX1
...
SSY3823V:   /.発注ＣＳＶ出力./
    ...
    OVRF      FILE-HCTPXXX1,TOFILE-?HACTXXX1
    OVRF      FILE-HNCSVTEN,TOFILE-?CSVTXXXF
    OVRF      FILE-HCSMXXX1,TOFILE-?HACSXXX1
    OVRF      FILE-HNCSVMEI,TOFILE-?CSVSXXXF
    CALL      PGM-SSY3940V.TOKELIBO
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := '発注ＣＳＶ　出力'
