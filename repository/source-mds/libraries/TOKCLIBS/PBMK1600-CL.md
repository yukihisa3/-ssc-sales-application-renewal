...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBMK1600.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PBMK1600'
    VAR       ?STEP     ,STRING*8
...
SBMK165B:
    ...
    OVRF     FILE-BMSHE2W,TOFILE-BMSHE2W.BMSFLIB
    CALL     PGM-SBMK165B.TOKELIBO,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ返品データ抽出（コメリ）'
              GOTO ABEND
    END
...
SBM0170L:
    ...
    OVRF      FILE-BMSHE2W1,TOFILE-BMSHE2W1.BMSFLIB
    CALL      PGM-SBMK170L.TOKELIBO,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ返品リスト出力'
              GOTO ABEND
...
SBM0180V:
    ...
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSHEPW1,TOFILE-BMSHE2W1.BMSFLIB
    OVRF      FILE-BMSHEPC,TOFILE-BMSHE2C.BMSFLIB
    CALL      PGM-SBM0180V.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ返品ＣＳＶデータ出力（コメリ）'
              GOTO ABEND
    END
...
CNVDF000:
    ...
    CNVDF FILE-BMSHE2C.BMSFLIB,
/.  PATH-'/SAKATA/DATA/PRKANRI/BMSHE2C.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO    ./
    PATH-'/HGNAS/BMSHE2C.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4    := '返品ＣＳＶデータ転送（コメリ）'
              GOTO ABEND
    ELSE
              GOTO RTN
    END
