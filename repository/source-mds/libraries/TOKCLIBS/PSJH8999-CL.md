...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH8999.CL`
...
STEP070:
    ...
    OVRF FILE-LICFILE2,TOFILE-LICFILE2.ONLBLIB
    OVRF FILE-LICFILE3,TOFILE-LICFILE3.ONLBLIB
    CALL SJH8702L.TOKELIBO,PARA-(?CHK1,?CHK2)
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信結果リスト】'
                    GOTO ABEND
    END
...
./
    IF   ?CHK9  =  '1'  THEN
         ...
         SNDMSG '######################################',TO-XCTL
         CALL PSJH7904.TOKCLIBO,PARA-(?HIDUKE,?JIKAN,?PGCHK9)
         IF   ?PGCHK3  = '1'    THEN
              SNDMSG '######################################',TO-XCTL
    ...
    IF   ?CHK10 =  '1'  THEN
         ...
         SNDMSG '######################################',TO-XCTL
         CALL PSJH7905.TOKCLIBO,PARA-(?HIDUKE,?JIKAN,?PGCHK9)
         IF   ?PGCHK3  = '1'    THEN
              SNDMSG '######################################',TO-XCTL
