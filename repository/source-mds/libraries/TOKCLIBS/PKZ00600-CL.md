...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PKZ00600.CL`
...
/. *   SYSTEM-NAME :    業務改善（出荷業務）　　　　　　　   *  ./
/. *   JOB-ID      :    PKZ00600                             *  ./
/. *   JOB-NAME    :    手書出荷指示データ削除　　　　　　　 *  ./
/. ***********************************************************  ./
    PGM
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PKZ00600'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SKZ0070B:
    ...
    OVRF      FILE-SYUJISL1,TOFILE-SYUJISL1.TOKKLIB
    CALL      PGM-SKZ0070B.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'データ削除'
              GOTO ABEND END
