...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSI87050.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI87050'
    VAR       ?STEP     ,STRING*8
...
SSI87051:
    ...
    CALL      PGM-SSI8705B.TOKELIBO,PARA-(?SYORIKN1,?SYORIKN2)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
SSI87052:
    ...
    CALL      PGM-SSI8705B.TOKELIBO,PARA-(?SYORIKN3,?SYORIKN4)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
RTN:
