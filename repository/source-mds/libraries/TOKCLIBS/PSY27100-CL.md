...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY27100.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?CLID    ,STRING*08,VALUE-'PSY27100'        /.CLID         ./
...
/.##実行PG名称ｾｯﾄ##./
    ?CLNM := 'エンチョー受領データ取込み　　　　　'
/.##ﾗｲﾌﾞﾗﾘﾘｽﾄ登録##./
    DEFLIBL TOKELIB/TOKELIBO/TOKFLIB/ONLBLIB/TOKKLIB
/.##ﾌﾟﾛｸﾞﾗﾑ開始ﾒｯｾｰｼﾞ##./
    ?MSGX :=  '***   '  && ?CLID  &&   ' START  ***'
    SNDMSG    ?MSGX,TO-XCTL
/.## ﾜｰｸｽﾃｰｼｮﾝ名取得##./
...
CLRFILE1:
    ...
    CLRFILE   FILE-ECJYURF.TOKKLIB
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'エンチョー受領情報データクリア'
              GOTO ABEND
    END
...
SSY2710B:
    ...
    OVRF      FILE-ONLENCJY,TOFILE-ONLENCJR.ONLBLIB
    OVRF      FILE-ECJYURL1,TOFILE-ECJYURL1.TOKKLIB
    CALL      PGM-SSY2710B.TOKELIBO
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'エンチョー受領データ取込み'
