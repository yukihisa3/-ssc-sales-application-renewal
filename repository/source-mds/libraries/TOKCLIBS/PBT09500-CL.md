...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBT09500.CL`
...
SBT0036B:
    ...
    OVRF    FILE-JTCDENL1,TOFILE-JTCDENL1.TOKKLIB
    OVRF    FILE-JTCHKYL1,TOFILE-JTCHKYL1.TOKWLIB
    CALL    PGM-SBT0036B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD)
    IF   @PGMEC ^= 0 THEN
          GOTO      ABEND
    END
...
SBT0100B:
    ...
    CALL    PGM-SBT0940B.TOKELIBO,PARA-(?HIDUKE,?JIKAN,?TOKCD)
    IF   @PGMEC ^= 0 THEN
          GOTO      ABEND
    END
