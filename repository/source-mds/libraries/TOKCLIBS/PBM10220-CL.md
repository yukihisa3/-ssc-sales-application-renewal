...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PBM10220.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PBM10220'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
ASSIGN:
    ...
    ASSIGN    FILE-SETGKFK.TOKKLIB!@XCL
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              SNDMSG '他で連携処理が行われています',TOWS-@ORGWS
              ?KEKA4 :=  'コメリ請求合計ファイルロック'
              GOTO ABEND
    END
    ...
    ASSIGN    FILE-SEKKMRF.ONLBLIB!@XCL
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              SNDMSG '他で連携処理が行われています',TOWS-@ORGWS
...
PSORT1:
    ...
    IF        ?TORICD  =  '00271934'  THEN
       SORTF  INFILE-SETGKFA.TOKFLIB,
              OUTFILE-SETGKFK.TOKKLIB,
              KEY-'F01!A',
              SELECT-'F01!EQ!#00271934',
              RCDL-@DSP
       IF     @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '【コメリ分　請求データ抽出】'
              GOTO ABEND
...
STEP050:
  ...
  /.OVRF FILE-IONFURL2,TOFILE-IONFURL2.BMSFLIB ./
    OVRF FILE-SETGKFP,TOFILE-SETGKFK.TOKKLIB
    ...
    CALL SBMK102B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,
...
STEP060:
    ...
    OVRF FILE-BMSSEKL1,TOFILE-BMSSEKL1.BMSFLIB
    OVRF FILE-SEKKMRF,TOFILE-SEKKMRF.ONLBLIB
    CALL  SBMK190B.TOKELIBO
    IF      @PGMEC    ^=   0    THEN
