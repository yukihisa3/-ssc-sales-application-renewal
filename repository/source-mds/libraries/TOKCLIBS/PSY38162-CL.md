...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY38162.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY38162'
    VAR       ?STEP     ,STRING*8
    /.端末別ファイルＩＤ編集用./
              /.端末番号./
...
STEP000:
    ...
    ?OPR1  :=  '＜本発関係資料作成処理（更新のみ）＞'
    ?OPR2  :=  '　本発関係資料作成の更新処理のみを実行します'
    ?OPR3  :=  '　本発関係帳票の発行は行われませんので、注意'
    ?OPR4  :=  '　して下さい。'
    ?OPR5  :=  '　確認して下さい。'
