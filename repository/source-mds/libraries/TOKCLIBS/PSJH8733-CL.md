...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH8733.CL`
...
STEP025:
    ...
    CLRFILE FILE-JEDICOJ1.TOKWLIB
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込ＤＴ（植物）初期化】'
                    GOTO ABEND
    END
...
STEP030:
    ...
    FEXPORT FILE-JEDICOJ1.TOKWLIB,MODE-@ADD,PARA-DCMJAPAN,
            UNIT-5
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（資材）】'
                    GOTO ABEND
...
STEP035:
    ...
    FEXPORT FILE-JEDICOJ2.TOKWLIB,MODE-@ADD,PARA-DCMJAPAN,
            UNIT-6
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（植物）】'
                    GOTO ABEND
...
STEP036:
    ...
    CNVFILE FILE-JEDICOJ1.TOKWLIB,TOFILE-JEDICOSJ.ONLBLIB,
            ADD-@NO,BF-1
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【統合（資材）】'
                    GOTO ABEND
...
STEP037:
    ...
    CNVFILE FILE-JEDICOJ2.TOKWLIB,TOFILE-JEDICOSJ.ONLBLIB,
            ADD-@YES,BF-1
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【統合（植物）】'
                    GOTO ABEND
