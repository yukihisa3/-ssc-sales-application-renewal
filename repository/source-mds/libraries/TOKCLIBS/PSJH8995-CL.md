...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSJH8995.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSJH8995'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY8910I:
    ...
    OVRF      FILE-RCSYUKL2,TOFILE-RCSYUKL2.TOKKLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY8910R.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6,?P5)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
...
SJH8995B:
    ...
    OVRF      FILE-RCJOHOL2,TOFILE-RCJOHOL2.TOKKLIB
    CALL      PGM-SJH8995B.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'フラグ解除（情報）'
              GOTO ABEND END
...
SJH8994B:
    ...
    OVRF      FILE-RCSYUKL5,TOFILE-RCSYUKL5.TOKKLIB
    CALL      PGM-SJH8994B.TOKELIBO,PARA-(?P1,?P2,?P3,?P4,?P6)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'フラグ解除（出荷）'
              GOTO ABEND END
