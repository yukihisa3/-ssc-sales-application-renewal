...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/PSY38300.CL`
...
/. ***********************************************************  ./
    ...
    PARA ?SYURUI,STRING*1,IN,VALUE-' '  /. 確定種類 ./
                                        /.  1:オンライン分 ./
                                        /.  2:本発・手書分 ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY38300'
    VAR       ?STEP     ,STRING*8
...
/.-----------------------------------------------------------./
    ...
    DEFLIBL   TOKELIB/TOKELIBO/TOKDLIB/TOKFLIB/TOKKLIB/TOKSOLIB
             /TOKDTLIB
...
SSY3830I:
    ...
    OVRDSPF   FILE-DSPF,TOFILE-DSPF.XUCL,MEDLIB-TOKELIBO
    CALL      PGM-SSY3830I.TOKELIBO,PARA-(?SYURUI,?BUMON,?TANCD,
                                          ?KANRINO,?SKBCD)
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC     =   4010 THEN
...
SSY3832B:
    ...
    CALL      PGM-SSY3832B.TOKELIBO,PARA-(?BUMON,?TANCD,?KANRINO)
...
SSY3831B:
    ...
    CALL      PGM-SSY3831B.TOKELIBO,PARA-(?SYURUI,?BUMON,?TANCD,
                                          ?KANRINO,?SKBCD)
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := '発注確定処理（オンライン／本発）'
              GOTO ABEND END

RTN:

    OVRDSPF FILE-DSPF,TOFILE-DSPF.TOKELIB,MEDLIB-TOKELIB
    ?KEKA1 :=  '発注確定処理が正常終了しました'
    ?KEKA2 :=  ''
    ?KEKA3 :=  ''
    ?KEKA4 :=  ''
