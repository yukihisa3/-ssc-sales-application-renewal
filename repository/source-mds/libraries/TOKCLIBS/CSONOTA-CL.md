...
**種別**: JCL  
**ライブラリ**: TOKCLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIBS/CSONOTA.CL`
...
/.##ワーク定義##./
    ...
    VAR  ?PGMID    ,STRING*8,VALUE-'CSONOTA '   /.PROGRAM-ID./
...
/.##プログラム開始メッセージ##./
    ...
    SNDMSG MSG-'＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊',TO-XCTL
    SNDMSG MSG-'＊　受信待受処理・その他　開始　＊',TO-XCTL
    SNDMSG MSG-'＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊',TO-XCTL
...
ERRCHK2:
    ...
    IF  ?KEKA ^= '0000'  THEN
        ...
        END
    END
/.  2010/10/15  バックアップステップコメントアウト  ABE
**##受信ファイルバックアップ##**
DATABACK:
...
/.     5.回線制御番号              ./
    IF  ?DTKBN  =  '01'  THEN
        SBMJOB JOB-KENLSTS,JOBD-CVCS.XUCL,JOBK-@B,PGM-KENLST.TOKCLIBO,
...
RTN:
/. 受信ファイルクリア ./
/.  CALL SCVMSG.TOKCLIBO,PARA-('ｿﾉﾀｹｲ ｼﾞｭｼﾝﾌｧｲﾙ ｸﾘｱ ')
    CLRFILE  ?JFILNM
    IF ?PGMEC ^= 0 THEN
       ?KEKA := 'K545'
       GOTO  ABEND
...
/.##回線管理マスタ　使用フラグ、使用取引先クリア ./
    ...
    SNDMSG MSG-'＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃',
           TO-XCTL
    SNDMSG MSG-'＃＜受信データが０件または、存在しま　＃',
           TO-XCTL
    SNDMSG MSG-'＃　せん。データが存在する場合は、　　＃',
           TO-XCTL
    SNDMSG MSG-'＃　再度受信を行なって下さい。　　　  "',
           TO-XCTL
    SNDMSG MSG-'＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃',
           TO-XCTL
    RETURN PGMEC-@PGMEC
...
/.     4.結果コード                ./
    ...
    CALL SCVMSG.TOKCLIBO,PARA-('ｲｼﾞｮｳ ｼｭｳﾘｮｳ  CSONOTA')
