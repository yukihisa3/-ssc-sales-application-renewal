...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NKE0260L.COB`
...
*FLG/ｶｳﾝﾄ
 ...
 01  RUINYKL3-ERR-CNT        PIC  9(07)     VALUE  ZERO.
...
*BRK項目　エリア
     03  BRK-RUI-F97       PIC  9(06)    VALUE ZERO.
     03  BRK-RUI-F97-FLG   PIC  X(03)    VALUE SPACE.
     03  BRK-RUI-F04       PIC  9(08)    VALUE ZERO.
     03  BRK-RUI-F04-FLG   PIC  X(03)    VALUE SPACE.
*メッセージエリア
 01  MSG-AREA.
     03  MSG-START.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  ST-PG          PIC   X(08)  VALUE "NKE0260L".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NKE0260L".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HD0.
     03  FILLER.
         ...
         05  FILLER          PIC  X(08)     VALUE  "NKE0260L".
...
*
 01  HD01.
     03  FILLER         CHARACTER TYPE YA.
         05  FILLER          PIC  X(01)   VALUE  SPACE.
         05  FILLER          PIC  N(05)   VALUE  NC"仕入先情報".
...
*
 01  HD02.
     03  FILLER         CHARACTER TYPE YA.
         ...
         05  FILLER          PIC  X(03)  VALUE SPACE.
         05  FILLER          PIC  N(05)  VALUE NC"入荷担当者".
         05  FILLER          PIC  X(10)  VALUE SPACE.
         05  FILLER          PIC  N(04)  VALUE NC"完納区分".
...
*
 01  HD03.
     03  FILLER         CHARACTER TYPE YA.
         ...
         05  FILLER          PIC  N(02)   VALUE  NC"残数".
         05  FILLER          PIC  X(03)   VALUE  SPACE.
...
*
 01  MS01.
     03  FILLER         CHARACTER TYPE YA.
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS01-SHICD      PIC  9(08).
         05  FILLER          PIC  X(02)     VALUE  SPACE.
         05  MS01-SHINM      PIC  N(15).
*
 01  MS02.
     03  FILLER              CHARACTER TYPE YB.
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-DENNO      PIC  9(07).
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         ...
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-SYOCD      PIC  X(20).
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-TANANO     PIC  X(06).
         05  FILLER          PIC  X(02)     VALUE  SPACE.
         05  MS02-TANCD      PIC  X(02).
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-TANNM      PIC  N(10).
         05  FILLER          PIC  X(02)     VALUE  SPACE.
         05  MS02-KANKBN     PIC  X(01).
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-KANNM      PIC  N(04).
*
 01  MS03.
     03  FILLER              CHARACTER TYPE YB.
         05  FILLER          PIC  X(23)     VALUE  SPACE.
...
*入荷確定累積ファイル読込
     ...
     END-IF.
*
     MOVE    RUI-F97            TO    BRK-RUI-F97.
     MOVE    RUI-F04            TO    BRK-RUI-F04.
     MOVE    "STR"              TO    BRK-RUI-F04-FLG.
*
*ヘッダ印刷
     PERFORM HEAD-WT-SEC.
*
...
*
     ...
     MOVE     PARA-IN-TDATE       TO   RUI-F96.
     MOVE     PARA-IN-TTIME       TO   RUI-F97.
...
*             DISPLAY "PARA-IN-SOKCD = " PARA-IN-SOKCD UPON CONS.
*             DISPLAY "RUI-F96       = " RUI-F96 UPON CONS.
*             DISPLAY "PARA-IN-TDATE = " PARA-IN-TDATE UPON CONS.
*T↑
     IF     ( RUI-F01 = PARA-IN-SOKCD ) AND
            ( RUI-F96 = PARA-IN-TDATE )
*T↓
*             DISPLAY "RUINYKL3-READ-01-01" UPON CONS
*T↑
              IF    PARA-IN-TTIME = ZERO
*T↓
*             DISPLAY "RUINYKL3-READ-01-02" UPON CONS
*T↑
                    CONTINUE
              ELSE
                    IF  RUI-F97 = PARA-IN-TTIME
*T↓
*             DISPLAY "RUINYKL3-READ-01-03" UPON CONS
*T↑
                    CONTINUE
                    ELSE
*T↓
*             DISPLAY "RUINYKL3-READ-01-04" UPON CONS
*T↑
                        MOVE   "END"  TO   END-FLG
                        GO            TO   RUINYKL3-READ-EXIT
                    END-IF
     ...
     ELSE
*T↓
*             DISPLAY "RUINYKL3-READ-01-05" UPON CONS
*T↑
              MOVE     "END"      TO   END-FLG
              GO                  TO   RUINYKL3-READ-EXIT
     END-IF.
...
*
 MAIN-030.
     IF       RUI-F97  NOT = BRK-RUI-F97
              MOVE     "BRK"     TO   BRK-RUI-F97-FLG
     END-IF.
*
 MAIN-040.
     IF       RUI-F04  NOT = BRK-RUI-F04
              MOVE     "BRK"     TO   BRK-RUI-F04-FLG
     END-IF.
*
 MAIN-EXIT.
     EXIT.
...
*    取込時刻セット
     MOVE     RUI-F97(1:2)        TO   HD000-TTIME(1:2).
     MOVE     ":"                 TO   HD000-TTIME(3:1).
     MOVE     RUI-F97(3:2)        TO   HD000-TTIME(4:2).
     MOVE     ":"                 TO   HD000-TTIME(6:1).
     MOVE     RUI-F97(5:2)        TO   HD000-TTIME(7:2).
*    担当者名取得
...
*
*   ブレイク改頁判定
     IF       BRK-RUI-F97-FLG     =   "BRK"
              PERFORM  HEAD-WT-SEC
              MOVE     RUI-F97    TO   BRK-RUI-F97
              MOVE     RUI-F04    TO   BRK-RUI-F04
              MOVE     "   "      TO   BRK-RUI-F97-FLG
              MOVE     "STR"      TO   BRK-RUI-F04-FLG
     END-IF.
*   改頁判定
     IF       LINE-CNT >  50
...
*
*明細１行目(仕入先行)
     IF       BRK-RUI-F04-FLG     =   "STR"
*             仕入先行
              MOVE     RUI-F04             TO   SHI-F01
                                                MS01-SHICD
              PERFORM  ZSHIMS1-READ-SEC
              IF       ZSHIMS1-INV-FLG = SPACE
                       MOVE  SHI-F02       TO   MS01-SHINM
              ELSE
                       MOVE  ALL NC"？"    TO   MS01-SHINM
              END-IF
*             仕入先行印刷
              WRITE    PRT-REC     FROM  MS01   AFTER  1
*             行カウント
              ADD      1                   TO   LINE-CNT
              MOVE     "   "               TO   BRK-RUI-F04-FLG
     END-IF.
*
*
     IF       BRK-RUI-F04-FLG     =   "BRK"
*T↓
*       IF  ( RUI-F02 = 1000098 ) AND
*           ( RUI-F03 = 7  )
*             DISPLAY "BRK-RUI-F04-FLG = " BRK-RUI-F04-FLG
*                  UPON CONS
*       END-IF
*T↑
*             線行印刷
...
*明細２行目(内容）
*   伝票番号
     MOVE     RUI-F02             TO   MS02-DENNO.
*   行番号
     MOVE     RUI-F03             TO   MS02-GYONO.
*   入荷日
     MOVE     RUI-F13(1:4)        TO   MS02-NDATE(1:4).
     MOVE     "/"                 TO   MS02-NDATE(5:1).
     MOVE     RUI-F13(5:2)        TO   MS02-NDATE(6:2).
     MOVE     "/"                 TO   MS02-NDATE(8:1).
     MOVE     RUI-F13(7:2)        TO   MS02-NDATE(9:2).
*   ＪＡＮＣＤ
     MOVE     RUI-F05             TO   MS02-JANCD.
*   自社商品ＣＤ
     MOVE     RUI-F06             TO   MS02-SYOCD(1:8)
                                       MEI-F011.
     MOVE     " "                 TO   MS02-SYOCD(9:1)
     MOVE     RUI-F071            TO   MS02-SYOCD(10:5)
                                       MEI-F0121.
     MOVE     "-"                 TO   MS02-SYOCD(15:1).
     MOVE     RUI-F072            TO   MS02-SYOCD(16:2)
                                       MEI-F0122.
     MOVE     "-"                 TO   MS02-SYOCD(18:1).
     MOVE     RUI-F073            TO   MS02-SYOCD(19:1)
                                       MEI-F0123.
*   棚番
     MOVE     RUI-F09             TO   MS02-TANANO.
*   入荷担当者
     MOVE     PARA-IN-BUMON       TO   TAN-F01.
     MOVE     RUI-F17             TO   MS02-TANCD
                                       TAN-F02.
     PERFORM  TANMS1-READ-SEC.
     IF       TANMS1-INV-FLG = SPACE
              MOVE  TAN-F03       TO   MS02-TANNM
     ELSE
              MOVE  ALL NC"？"    TO   MS02-TANNM
     END-IF.
*   完納区分
     MOVE     RUI-F14             TO   MS02-KANKBN.
     IF       RUI-F14   =  "1"
              MOVE  NC"完了"      TO   MS02-KANNM
     ELSE
              MOVE  NC"未完了"    TO   MS02-KANNM
     END-IF.
*
*---------------------------------------------------
*明細３行目(内容）
*   商品名
     ...
     IF       MEIMS1-INV-FLG = SPACE
              MOVE  MEI-F02       TO   MS03-SYONAME
     ELSE
              MOVE  ALL NC"？"    TO   MS03-SYONAME
     END-IF
*   発注数
     MOVE     RUI-F10             TO   MS03-HACSU.
*   残数
     MOVE     RUI-F11             TO   MS03-ZANSU.
*   入荷数
     MOVE     RUI-F12             TO   MS03-NYUSU.
*   不良品数
     MOVE     RUI-F15             TO   MS03-FURYOSU.
*   不良品区分・区分名
     MOVE     98                  TO   JYO-F01.
     MOVE     RUI-F16             TO   MS03-FURYOKBN
                                       JYO-F02.
     MOVE     SPACE               TO   MS03-FURYONM.
     IF       RUI-F16  NOT = SPACE
              PERFORM  JYOKEN1-READ-SEC
              IF       JYOKEN1-INV-FLG = SPACE
                       MOVE  JYO-F03       TO   MS03-FURYONM
              ELSE
                       MOVE  ALL NC"？"    TO   MS03-FURYONM
              END-IF
     END-IF.
*   エラー区分１
     IF  RUI-F23  = " "
              MOVE SPACE          TO   MS03-ERR1
     ELSE
              MOVE NC"×"         TO   MS03-ERR1
     END-IF.
*    エラー区分２
     IF  RUI-F24  = " "
              MOVE SPACE          TO   MS03-ERR2
     ELSE
              MOVE NC"×"         TO   MS03-ERR2
     END-IF.
*    エラー区分３
     IF  RUI-F25  = " "
              MOVE SPACE          TO   MS03-ERR3
     ELSE
              MOVE NC"×"         TO   MS03-ERR3
     END-IF.
*    エラー区分４
     IF  RUI-F26  = " "
              MOVE SPACE          TO   MS03-ERR4
     ELSE
              MOVE NC"×"         TO   MS03-ERR4
     END-IF.
*    エラー区分５
     IF  RUI-F27  = " "
              MOVE SPACE          TO   MS03-ERR5
     ELSE
              MOVE NC"×"         TO   MS03-ERR5
     END-IF.
*    エラー区分６
     IF  RUI-F28  = " "
              MOVE SPACE          TO   MS03-ERR6
     ELSE
              MOVE NC"×"         TO   MS03-ERR6
     END-IF.
*    エラー区分７
     IF  RUI-F29  = " "
              MOVE SPACE          TO   MS03-ERR7
     ELSE
              MOVE NC"×"         TO   MS03-ERR7
     END-IF.
*    エラー区分８
     IF  RUI-F30  = " "
              MOVE SPACE          TO   MS03-ERR8
     ELSE
              MOVE NC"×"         TO   MS03-ERR8
     END-IF.
*    エラー区分９
     IF  RUI-F31  = " "
              MOVE SPACE          TO   MS03-ERR9
     ELSE
              MOVE NC"×"         TO   MS03-ERR9
     END-IF.
*    エラー区分１０
     IF  RUI-F32  = " "
              MOVE SPACE          TO   MS03-ERR10
     ELSE
              MOVE NC"×"         TO   MS03-ERR10
     END-IF.
*
*    明細印刷
...
*  ＝入荷確定累積ファイル読込件数
     ...
     DISPLAY "         ERR-CNT   = " RUINYKL3-ERR-CNT UPON CONS.
     DISPLAY "         PAGE-CNT  = " PAGE-CNT UPON CONS.
*
     CLOSE     RUINYKL3  ZSOKMS1  ZSHIMS1  JYOKEN1
               MEIMS1    TANMS1   PRTF.
*
     STOP      RUN.
*
