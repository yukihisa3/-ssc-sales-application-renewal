...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NVD0520I.COB`
...
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    業務名　　　　　　　：　Ｄ３６５連携　　　　　　　　　　　*
*    モジュール名　　　　：　倉庫間在庫移動実績リスト出力指示　*
*    作成日／作成者　　　：　2020/04/21   ASS.II               *
*    処理内容　　　　　　：　倉庫間在庫移動実績リスト発行条件　*
*    　　　　　　　　　　　　を画面より指定する。　　　　　　　*
****************************************************************
...
*画面表示用ワーク
 01  DSP-WORK.
     03  WORK-PGID                PIC  X(08)  VALUE  "NVD0520I".
     03  WORK-FORMID              PIC  X(08)  VALUE  "FVD0520I".
*フラグ領域
 01  FLG-AREA.
...
*ファイル ＣＬＯＳＥ
     ...
     CLOSE             ZSOKMS1.
******************* TEST
     DISPLAY "日付区分    ="  LINK-OUT-DATEKBN   UPON CONS.
     DISPLAY "出庫倉庫    ="  LINK-OUT-SOKO      UPON CONS.
     DISPLAY "開始日付    ="  LINK-OUT-FDATE     UPON CONS.
     DISPLAY "終了終了    ="  LINK-OUT-TDATE     UPON CONS.
...
*初期画面の処理
     ...
     MOVE "FVD05201"    TO  DSP-FMT.
     MOVE  SPACE        TO  DSP-FVD05201.
     PERFORM  OPT-CLR-SEC.
*ＢＯＤＹ部入力へ
     MOVE  2            TO  MAIN-FLG.
 DSP-INIT-EXIT.
...
*倉庫ＣＤ
     ...
     EVALUATE  DSP-DKBN
          WHEN SPACE
                          MOVE  NC"指示日"  TO   DSP-SOKSYU
          WHEN "1"
                          MOVE  NC"出庫日"  TO   DSP-SOKSYU
          WHEN "2"
                          MOVE  NC"入庫日"  TO   DSP-SOKSYU
          WHEN OTHER
                          MOVE  SPACE       TO   DSP-SOKSYU
     END-EVALUATE.
...
*
     IF  ERR-FLG  =  ZERO
         MOVE  DSP-DKBN   TO   LINK-OUT-DATEKBN
         MOVE  DSP-SOKCD  TO   LINK-OUT-SOKO
         MOVE  DSP-FDATE  TO   LINK-OUT-FDATE
         IF    DSP-TDATE  =    ZERO
               MOVE "99999999" TO   LINK-OUT-TDATE
         ELSE
               MOVE DSP-TDATE  TO   LINK-OUT-TDATE
         END-IF
         MOVE  99         TO   MAIN-FLG
     END-IF.
 CHK-KAKU-EXIT.
