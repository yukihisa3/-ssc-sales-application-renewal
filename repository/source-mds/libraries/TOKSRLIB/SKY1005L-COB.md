...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SKY1005L.COB`
...
*    作成日／作成者　　　：　2021/03/16 NAV TAKAHASHI          *
*    更新日／更新者　　　：　                                  *
*    更新内容　　　　　　：                                    *
*    再利用ＰＧ　　　　　：　SKY1005L.TOKSLIB                  *
*                                                              *
****************************************************************
 IDENTIFICATION         DIVISION.
 PROGRAM-ID.            SKY1005L.
 AUTHOR.                NAV.
 DATE-WRITTEN.          21/03/16.
****************************************************************
*                                                              *
...
***  ｺﾞｳｹｲ ｴﾘｱ
 01  GOUKEI.
     ...
     03  G-SURYO                  PIC  9(09)     VALUE   ZERO.
***  ｹｲｻﾝ ｴﾘｱ
 01  KEISAN.
...
*        05  SINSEINO             PIC  9(08)     VALUE   ZERO.
     ...
     03  WK-TOKKJ                 PIC  N(10)     VALUE   SPACE.
*日付
 01  DATE-AREA.
 ...
 01  MSG-AREA.
     03  MSG-FIELD.
         ...
         05  MSG09.
             ...
             07  FILLER           PIC  N(15)     VALUE
                 NC"対象の取引先ではありません。　".
         05  MSG10                PIC  N(30)     VALUE
         NC"この伝票発行はＤＣＭ専用です。！！".
     03  MSG-FIELD-R     REDEFINES    MSG-FIELD.
         05  MSG-TBL     OCCURS     10     PIC  N(30).
*
...
*
 01  HD1   CHARACTER          TYPE IS        YB.
     ...
     03  HD1-04                   PIC  X(02).
     03  FILLER                   PIC  X(02)     VALUE   SPACE.
     03  HD1-05                   PIC  ZZZ999999.
     03  FILLER                   PIC  X(02)     VALUE   SPACE.
     03  HD1-06                   PIC  999999.
...
*****取引先入力チェック*****
     ...
     ELSE
*****取引先マスタ検索*****
          IF  TOKCD = 880 OR 882 OR 883 OR 1427 OR 14272 OR 14273
          OR 13938 OR 139381 OR 17137 OR 171371 OR 100403
          OR 100404 OR 100427 OR 100428 OR 100441 OR 100442
          ...
          READ     HTOKMS    INVALID
                   ...
                   GO       TO   MAIN010
                   NOT  INVALID
                   MOVE  TOK-F03 TO   TOKNM  WK-TOKKJ
...
*    数量
     ...
     COMPUTE  G-SURYO  =  G-SURYO  +  DEN-F15.
*    原価単価
...
*--- 項目セット
*    消費税メッセージ
*****MOVE    "ｹｲｹﾞﾝｾﾞｲﾘﾂ"        TO    HDA-01.
     MOVE     SPACE              TO    HDA-01.
*    社名
     MOVE     WK-TOKKJ           TO    HD0-01.
*    店舗名称
     ...
     IF  HTENMS-INV-FLG = SPACE
         MOVE   TEN-F03          TO    HD1-01
     ELSE
         DISPLAY NC"＃＃店舗未登録エラー！＃＃" UPON CONS
...
*#2021/03/05 NAV ST 取引先ＣＤ変換
     EVALUATE DEN-F01
         WHEN  880     WHEN  882    WHEN  883
               ...
               MOVE  "03"        TO    HD1-011
               MOVE  "-"         TO    HD1-012
         WHEN  13938   WHEN  139381
               MOVE  56          TO    HD1-06
               MOVE  "01"        TO    HD1-011
               MOVE  "-"         TO    HD1-012
         WHEN  100403  WHEN  100404
...
*
     WRITE    PRT-REC          FROM   HDA    AFTER    3.
...
*    テスト印字項目セット
     MOVE   ALL "9"              TO   HD1-02  HD1-05  HD1-06
                                      ...
                                      TL1-01  TL1-02  TL1-00.
     MOVE   ALL "*"              TO   HD1-03  HD1-04
                                      DT1-01  DT2-01  DT2-02
                                      TL2-01  TL3-01  HD1-011
                                      HD1-012 HDA-01.
     MOVE   ALL NC"＊"           TO   HD0-01  HD1-01.
     MOVE   NC"_　サカタのタネ" TO   HD1-061.
*    ヘッド印字
     WRITE    PRT-REC          FROM   HDA    AFTER    3.
     WRITE    PRT-REC          FROM   HD0    AFTER    3
...
*    END-IF.
*    ファイルのＣＬＯＳＥ
     CLOSE    SHTDENF   DSPFILE   HTOKMS  HTENMS.
*
 END-EXIT.
     EXIT.
******************************************************************
 END PROGRAM  SKY1005L.
******************************************************************
