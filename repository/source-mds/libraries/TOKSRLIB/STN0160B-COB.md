...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/STN0160B.COB`
...
*
 ...
 FILE-CONTROL.
*----<< _卸確定データ >>--*
     SELECT   KKTANAF   ASSIGN    TO        DA-01-VI-KKTANAL5
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       KKT-F01
                        FILE  STATUS   IS   KKT-STATUS.
*----<< _卸計上データ >>----*
     SELECT   KJTANAF  ASSIGN    TO        DA-01-S-KJTANAF
...
*    _卸計上データ
******************************************************************
 FD  KJTANAF           LABEL     RECORD   IS   STANDARD.
     COPY     KJTANAF  OF        XFDLIB
...
*    ｶｳﾝﾄ
 ...
 01  WK-INV-FLG.
     03  KJTANAF-INV-FLG    PIC  X(03)     VALUE  SPACE.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "STN0160B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "STN0160B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*    MOVE      ZERO            TO   KKT-F01.
*    START     KKTANAF   KEY   >=   KKT-F04  KKT-F01
*        INVALID   KEY
*             MOVE    "END"  TO   END-FLG
*             GO             TO   INIT-EXIT
*    END-START.
...
*
     ...
     INITIALIZE                          OUT-REC.
     MOVE    KKT-F01                 TO  OUT-F01.
     MOVE    KKT-F02X                TO  OUT-F02X.
     MOVE    KKT-F04                 TO  OUT-F04.
     MOVE    KKT-F05                 TO  OUT-F05.
     MOVE    KKT-F06X                TO  OUT-F06X.
     MOVE    KKT-F09                 TO  OUT-F09.
     MOVE    KKT-F10                 TO  OUT-F10.
     MOVE    KKT-F11                 TO  OUT-F11.
     MOVE    KKT-F12                 TO  OUT-F12.
     MOVE    KKT-F13                 TO  OUT-F13.
     MOVE    KKT-F14                 TO  OUT-F14.
     MOVE    KKT-F15                 TO  OUT-F15.
     MOVE    KKT-F16                 TO  OUT-F16.
     MOVE    KKT-F17                 TO  OUT-F17.
     WRITE   OUT-REC.
     ADD     1                       TO  WRT-CNT.
*
 MAIN999.
...
*
     ...
     DISPLAY   NC"計上データ　　" "OUT = " WRT-CNT   UPON CONS.
