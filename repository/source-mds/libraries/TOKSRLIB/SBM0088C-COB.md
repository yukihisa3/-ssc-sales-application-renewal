...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SBM0088C.COB`
...
*     流用元　　　　     ： SBM0082B
*     　　　　           　
*     更新日／更新者     ： 2022/03/22 NAV TAKAHASHI
*     修正概要           ： 日付未セット項目→空白セット
*     　　　　           　
*     更新日／更新者     ： 2022/10/04 NAV TAKAHASHI
*     修正概要           ： 納品日範囲対応　　　　　　　
*     　　　　           ：
*
******************************************************************
...
*
*出荷メッセージ送信ファイル（ビバホーム）
     SELECT   SYKSNDVH  ASSIGN    TO   DA-01-S-SYKSNDVH
                        ACCESS    MODE SEQUENTIAL
                        FILE STATUS    IS   SVH-ST.
*
*流通ＢＭＳ発注メッセージ
...
*ワーク項目
 ...
 01  WRT-CNT3                     PIC  9(07)     VALUE  ZERO.
 01  WKEY.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG                PIC  X(08)     VALUE "SBM0088C".
...
*PROCEDURE DIVISION         USING   PARA-KENSUU.
 ...
 SND-ERR                    SECTION.
     ...
     STOP          RUN.
 SVH-ERR                    SECTION.
     USE      AFTER         EXCEPTION PROCEDURE  SYKSNDVH.
     DISPLAY       SVH-ERR  UPON      CONS.
     DISPLAY       SEC-NAME UPON      CONS.
     DISPLAY       SVH-ST   UPON      CONS.
     MOVE          "4000"   TO        PROGRAM-STATUS.
     STOP          RUN.
 HAC-ERR                    SECTION.
...
**** IF       RD-CNT1    =         1
     IF       WRT-CNT1   =  0
*             メッセージヘッダレコード
              MOVE       SYK-F10          TO   SVH-REC
*移動         MOVE       SYK-F10          TO   WK-REC
*移動         MOVE       "1"              TO   WK-REC(1198:1)
*移動         MOVE       WK-REC           TO   SND-REC
*移動         WRITE      SND-REC
*移動         ADD        1                TO   WRT-CNT1
*
*             出荷リストレコード
...
***************MOVE      WK-REC2(1:57)    TO   WK-REC(1:57)
               MOVE      WK-REC2(1:143)   TO   WK-REC(1:143)
*                        最終納品先名称
               MOVE      X"28"            TO   WK-REC(144:1)
               MOVE      WK-REC2(144:40)  TO   WK-REC(145:40)
               MOVE      X"29"            TO   WK-REC(185:1)
***************MOVE      WK-REC2(184:205) TO   WK-REC(186:291)
               MOVE      WK-REC2(184:291) TO   WK-REC(186:291)
*                        取引先名称
               MOVE      X"28"            TO   WK-REC(477:1)
               MOVE      WK-REC2(475:40)  TO   WK-REC(478:40)
               MOVE      X"29"            TO   WK-REC(518:1)
               MOVE      WK-REC2(515:678) TO   WK-REC(519:68)
*                        カテゴリ名称１（印字用）
               MOVE      X"28"            TO   WK-REC(587:1)
               MOVE      WK-REC2(583:10)  TO   WK-REC(588:10)
               MOVE      X"29"            TO   WK-REC(598:1)
*                        カテゴリ名称２（印字用）
               MOVE      X"28"            TO   WK-REC(599:1)
               MOVE      WK-REC2(593:10)  TO   WK-REC(600:10)
               MOVE      X"29"            TO   WK-REC(610:1)
*
               MOVE      WK-REC2(603:553) TO   WK-REC(611:553)
               MOVE      "1"              TO   WK-REC(1198:1)
*#2022/03/22 NAV ST 不要日付＝空白セット
***************MOVE      SPACE            TO   WK-REC(876:16)
               MOVE      SPACE            TO   WK-REC(876:24)
*#2022/03/22 NAV ED 不要日付＝空白セット
*
               MOVE      WK-REC    TO        SND-REC
               WRITE     SND-REC
               ADD       1         TO        WRT-CNT1
...
*メッセージヘッダレコード出力
     IF        WRT-CNT2   NOT =  0
               MOVE       SVH-REC           TO   WK-REC
               MOVE       WRT-CNT3          TO   WK-REC(451:7)
               MOVE       "1"               TO   WK-REC(1198:1)
               MOVE       WK-REC            TO   SVH-REC
               WRITE      SVH-REC
               ADD        1                 TO   WRT-CNT1
     END-IF.
*
...
*ファイルのクローズ
     CLOSE     BMSSYKF  SYKSNDF  SYKSNDVH  BMSHACF.
*
     STOP      RUN.
*
 END-EXIT.
...
*
     ...
     MOVE     PARA-IN-NDATES              TO   SYK-F349.
*
...
*    DISPLAY "NDATES  =" PARA-IN-NDATES  UPON  CONS.
*    DISPLAY "NDATEE  =" PARA-IN-NDATEE  UPON  CONS.
...
*T
     IF   ( SYK-F011  =  PARA-IN-BDATE ) AND
          ...
          ( SYK-F349  >=  PARA-IN-NDATES ) AND
          ( SYK-F349  <=  PARA-IN-NDATEE )
          CONTINUE
     ELSE
          MOVE   "END"   TO   END-FLG
          GO             TO   BMSSYKF-READ-EXIT
