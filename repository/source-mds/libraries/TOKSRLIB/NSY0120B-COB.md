...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY0120B.COB`
...
*ＤＣＭＪＡＰＡＮ発注情報保存データ
     SELECT   DNJOHOF   ASSIGN    TO        DA-01-VI-DNJOHOL3
                        ORGANIZATION        INDEXED
...
*ＤＣＭＪＡＰＡＮ出荷情報修正
     SELECT   DNSYUKF   ASSIGN    TO        DA-01-VI-DNSYUKL3
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "NSY0120B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NSY0120B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
 FILEERR-SEC3           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   DNSYUKF.
     MOVE      "DNSYUKL3 "   TO   AB-FILE.
     MOVE      DJS-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     DISPLAY "KEY DJJ-K01 = " DJJ-K01 UPON CONS.
     DISPLAY "KEY DJJ-K02 = " DJJ-K02 UPON CONS.
     DISPLAY "KEY DJJ-K03 = " DJJ-K03 UPON CONS.
     DISPLAY "KEY DJJ-K04 = " DJJ-K04 UPON CONS.
     DISPLAY "KEY DJJ-K08 = " DJJ-K08 UPON CONS.
     DISPLAY "KEY DJJ-K05 = " DJJ-K05 UPON CONS.
     DISPLAY "KEY DJJ-K06 = " DJJ-K06 UPON CONS.
     DISPLAY "KEY DJJ-K07 = " DJJ-K07 UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 END     DECLARATIVES.
...
**************数量確認(発注数<>出荷数の時ﾌｧｲﾙに出力)
              ...
              IF    DNSYUKF-INV-FLG = "INV"
                    PERFORM   DNSYUKF-WRITE-SEC
                    MOVE   "1"       TO   DJJ-K09
...
*　　新ＤＣＭ出荷情報ファイル作成処理　　
****************************************************************
 DNSYUKF-WRITE-SEC     SECTION.
*
     MOVE     "DNSYUKF-WRITE-SEC"  TO  S-NAME.
*
...
*****MOVE      DJS-F061            TO  WK-A055.
*****MOVE      WK-A05              TO  DJS-A05.
*納品日セット
     MOVE      DJS-F042            TO  DJS-F0612.
*    出荷予定日（納品予定日‐１を算出セット）
     MOVE    "6"                   TO        LINK-IN-KBN.
     MOVE     1                    TO        LINK-IN-YMD6.
     MOVE     DJS-F042             TO        LINK-IN-YMD8.
     CALL    "SKYDTCKB"  USING     LINK-IN-KBN
                                   LINK-IN-YMD6
                                   LINK-IN-YMD8
                                   LINK-OUT-RET
                                   LINK-OUT-YMD8.
     IF       LINK-OUT-RET   =    ZERO
              MOVE    LINK-OUT-YMD8    TO    DJS-F0611
     ELSE
              MOVE    DJS-F042         TO    DJS-F0611
     END-IF.
...
*
     DISPLAY "DCM KIHON DT    READ CNT = " READ-CNT  UPON CONS.
     DISPLAY "ｳﾘｱｹﾞDT ﾅｼ           CNT = " SKIP2-CNT UPON CONS.
     DISPLAY "DCM SYUKA DT    WT   CNT = " KOS1-CNT  UPON CONS.
*
     CLOSE     SHTDENF  DNJOHOF  DNSYUKF.
*
     STOP      RUN.
*
 END-EXIT.
...
*
     ...
     MOVE     DJJ-K08        TO   DJS-K08.
     READ     DNSYUKF    INVALID
              MOVE    "INV"       TO   DNSYUKF-INV-FLG
              NOT  INVALID
              MOVE    SPACE       TO   DNSYUKF-INV-FLG
     END-READ.
*
 DNSYUKF-READ-EXIT.
     EXIT.
