...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/D365CNV6.COB`
...
*    顧客名　　　　：　（株）サカタのタネ殿　　　　　　　　　　*
*    サブシステム　：　Ｄ３６５連携　　　　　　　　　　        *
*    モジュール名　：　商品変換テーブル雑コードコンバート　　  *
*    作成日／作成者：　2021/06/04 INOUE                        *
*    処理概要　　　：　基幹システムマスタの雑コードを変換する  *
*                      　　　　　　　　　　　　　　　　　      *
*    更新履歴                                                  *
*    更新日／更新者：　                                        *
*                                                              *
...
*
 ...
 FILE-CONTROL.
*----<< 雑コード移行データ　 >>----*
     SELECT   IMPZATD1  ASSIGN              DA-01-VI-IMPZATD1
...
*    雑コード移行データ
******************************************************************
 FD  IMPZATD1  LABEL RECORD   IS   STANDARD.
     COPY      IMPZATD1       OF   XFDLIB
     JOINING   IN                  PREFIX.
******************************************************************
*    商品変換テーブル
******************************************************************
...
*    ｶｳﾝﾄ
 ...
 01  WK-CNT.
     ...
     03  WRT2-CNT            PIC  9(08)     VALUE  ZERO.
     03  KMS-CNT             PIC  9(08)     VALUE  ZERO.
 01  WK-INV-FLG.
     03  WKSHOTBL-INV-FLG    PIC  X(03)     VALUE  SPACE.
     03  IMPZATD1-INV-FLG    PIC  X(03)     VALUE  SPACE.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "D365CNV6".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "D365CNV6".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*　　　　　　　メイン処理　　　　　　　　　　　　　　　　　　　*
****************************************************************
 IMPZATD1-READ-SEC    SECTION.
*
     MOVE    "IMPZATD1-READ-SEC"    TO  S-NAME.
*
     READ     IMPZATD1
              AT  END
                  MOVE     "END"    TO  END-FLG
                  GO                TO  IMPZATD1-READ-EXIT
              NOT AT END
                  ADD       1       TO  READ-CNT
     END-READ.
*
...
*
*  サカタＣＤ変更
     MOVE     IN-F05         TO  TBL-F031.
*  品単ＣＤ変更
     MOVE     IN-F06         TO  TBL-F032.
*
     REWRITE  TBL-REC.
     ADD      1              TO  WRT-CNT.
*
 MAIN020.
*    ＳＵＢ商品変換ＴＢＬ検索
     MOVE     IN-F01         TO  SUB-F01.
     MOVE     IN-F02         TO  SUB-F02.
     READ     SUBTBLL1
         INVALID
***           DISPLAY NC"変換ＴＢＬなし！　商品＝"
***                                        IN-F01 UPON CONS
...
*
*  サカタＣＤ変更
     MOVE     IN-F05         TO  SUB-F031.
*  品単ＣＤ変更
     MOVE     IN-F06         TO  SUB-F032.
*
     REWRITE  SUB-REC.
     ADD      1              TO  WRT2-CNT.
*
...
*
     ...
     DISPLAY  NC"抽出対象外１　" "SKIP= " SKIP-CNT  UPON CONS.
     DISPLAY  NC"抽出対象外２　" "SKIP= " SKIP2-CNT  UPON CONS.
     DISPLAY  NC"変換ＴＢＬ更新" "OUT = " WRT-CNT   UPON CONS.
     DISPLAY  NC"ＳＵＢ変換更新" "OUT = " WRT2-CNT   UPON CONS.
*
     CLOSE     IMPZATD1  SHOTBL1  SUBTBLL1.
*
