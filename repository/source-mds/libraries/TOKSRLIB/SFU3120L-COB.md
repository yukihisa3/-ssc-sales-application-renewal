...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SFU3120L.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 振替ワーク >>--*
     SELECT   SFRLSTW2  ASSIGN         DA-01-VI-SFRLSTW2
...
*----<< 倉庫マスタ >>--*
     SELECT   ZSOKMS1   ASSIGN         DA-01-VI-ZSOKMS1
                        ...
                        STATUS         ZSOKMS1-ST.
*----<< 仕入先マスタ >>--*
     SELECT   ZSHIMS1   ASSIGN         DA-01-VI-ZSHIMS1
                        ORGANIZATION   INDEXED
                        ACCESS    MODE RANDOM
                        RECORD    KEY  SHI-F01
                        STATUS         ZSHIMS1-ST.
...
*
*----<< ﾌｱｲﾙ ｽﾃｰﾀｽ >>--*
 01  SFRLSTW2-ST         PIC  X(02).
 01  ZSOKMS1-ST          PIC  X(02).
 01  ZSHIMS1-ST          PIC  X(02).
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*
 01  HEAD01.
     03  FILLER         CHARACTER TYPE BAIKAKU-1-5.
         ...
         05  FILLER     PIC  X(08)     VALUE  "SFU3120L".
...
*
 01  HEAD06.
     03  FILLER         CHARACTER TYPE PITCH-2.
         ...
         05  FILLER     PIC  X(01)     VALUE  "(".
         05  FILLER     PIC  N(02)     VALUE  NC"予定".
         05  FILLER     PIC  X(01)     VALUE  ")".
         05  FILLER     PIC  X(03)     VALUE  SPACE.
         05  FILLER     PIC  N(04)     VALUE  NC"商品名称".
...
*--------------------------------------------------------------*
 DECLARATIVES.
*----<< 振替ワーク >>--*
 SFRLSTW2-ERR            SECTION.
     USE AFTER     EXCEPTION PROCEDURE      SFRLSTW2.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SFU3120L SFRLSTW2 ERROR " SFRLSTW2-ST " "
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SFU3120L ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 仕入先マスタ >>--*
 ZSHIMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SFU3120L ZSHIMS1 ERROR " ZSHIMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*振替ワークスタート
     ...
     INITIALIZE                        LST-REC.
     START  SFRLSTW2  KEY  IS  >=  LST-F04  LST-F01  LST-F02
                                   LST-F03  LST-F14  LST-F05
                                   LST-F06  LST-F07
                                   LST-F08
            INVALID
            DISPLAY NC"＃＃出力対象データなし！＃＃" UPON CONS
            CLOSE   SFRLSTW2  ZSOKMS1  ZSHIMS1  PRTF
            STOP  RUN
     END-START.
*
...
*  入荷予定日
     MOVE     LST-F14(1:4)   TO   ME-014(1:4).
     MOVE     "/"            TO   ME-014(5:1).
     MOVE     LST-F14(5:2)   TO   ME-014(6:2).
     MOVE     "/"            TO   ME-014(8:1).
     MOVE     LST-F14(7:2)   TO   ME-014(9:2).
*  商品情報
     MOVE     LST-F05        TO   ME-015.
*　仕入先
     MOVE     LST-F12        TO   ME-016  SHI-F01.
     PERFORM  900-SHI-READ.
     MOVE     SHI-F02        TO   ME-017.
*　棚番
     MOVE     LST-F11        TO   ME-018.
*　発注数合計
     MOVE     LST-F15        TO   ME-019.
*　入荷数合計
     MOVE     LST-F19        TO   ME-01A.
*　発注残
...
*　商品名称
     MOVE     LST-F09        TO   ME-031.
     MOVE     LST-F10        TO   ME-032.
*　ＭＳＧ
     MOVE     LST-F23        TO   ME-033.
*
...
*--------------------------------------------------------------*
 ...
 900-SOK-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL ALL    仕入先マスタ　READ                           *
*--------------------------------------------------------------*
 900-SHI-READ           SECTION.
     MOVE     "900-SHI-READ"      TO   S-NAME.
     READ     ZSHIMS1    INVALID
              MOVE      SPACE          TO   SHI-F03
     END-READ.
 900-SHI-READ-EXIT.
     EXIT.
*-----------------<< PROGRAM END >>----------------------------*
