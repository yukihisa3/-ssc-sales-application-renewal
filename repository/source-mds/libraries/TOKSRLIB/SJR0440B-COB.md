...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0440B.COB`
...
*
 ...
 FILE-CONTROL.
*返品累積ファイル
     SELECT   COMRHEF   ASSIGN    TO        DA-01-VI-COMRHELA
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       RHE-F78  RHE-F79
                                            RHE-F01  RHE-F02
                                            RHE-F07  RHE-F04
                                            RHE-F05
...
*メッセージ定義エリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0440B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0440B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
 ...
 01  LINK-OUT-UPTIME         PIC  9(06).
 01  LINK-DEN-ADD-CNT        PIC  9(07).
*
******************************************************************
*             M A I N             M O D U L E                    *
******************************************************************
...
*ファイルＯＰＥＮ
     OPEN     INPUT     HSHOTBL  SHOTBL2  HMEIMS  HTOKMS
                        HTENMS   HJYOKEN.
     OPEN     I-O       COMRHEF  SHTDENF  ZAMZAIF.
*パラメタをメッセージ表示
     DISPLAY "## LINK-BMNCD   = " LINK-BMNCD   UPON CONS.
...
*システム時刻取得
     ...
     MOVE        WK-SYS-TIME1        TO    LINK-OUT-UPTIME.
*システム日付取得
...
*スタートの為、レコード初期化
     ...
     INITIALIZE                       RHE-REC.
*スタートキーセット
     MOVE     LINK-JDATE         TO   RHE-F78.
     MOVE     LINK-JTIME         TO   RHE-F79.
     MOVE     LINK-TOKCD         TO   RHE-F01.
*
     START  COMRHEF  KEY  IS  >=  RHE-F78  RHE-F79  RHE-F01
                                  RHE-F02  RHE-F07  RHE-F04
                                  RHE-F05
           INVALID
           MOVE  "END"           TO      END-FLG
...
*返品累積ファイル読込
     ...
     ELSE  *>ブレイクキーセット
*#2017/12/04 NAV ST
           MOVE  RHE-F01         TO      WK-RHE-F01
*#2017/12/04 NAV ED
           MOVE  RHE-F02         TO      WK-RHE-F02
           MOVE  RHE-F07         TO      WK-RHE-F07
           MOVE  RHE-F04         TO      WK-RHE-F04
...
*キーブレイク確認
*#2017/12/04 NAV ST
*??? IF  RHE-F02  NOT =  WK-RHE-F02      ?????
     IF  RHE-F01  NOT =  WK-RHE-F01
     OR  RHE-F02  NOT =  WK-RHE-F02
*#2017/12/04 NAV ED
     OR  RHE-F07  NOT =  WK-RHE-F07
     OR  RHE-F04  NOT =  WK-RHE-F04
         IF  WK-BIKO-1  =  SPACE
         AND WK-BIKO-2  =  SPACE
             CONTINUE
         ELSE  *>備考行の出力
...
*項目セット
     MOVE    RHE-F01            TO  DEN-F01.
     MOVE    RHE-F04            TO  DEN-F02.
     MOVE    RHE-F05            TO  DEN-F03.
     MOVE    ZERO               TO  DEN-F04.
     MOVE    RHE-F23            TO  DEN-F051.
     EVALUATE DEN-F051
         WHEN "41"  MOVE NC"売上返品" TO DEN-F052
         WHEN "42"  MOVE NC"売上値引" TO DEN-F052
     END-EVALUATE.
     MOVE    LINK-TANCD         TO  DEN-F06.
     MOVE    RHE-F02            TO  DEN-F07.
     MOVE    RHE-F08            TO  DEN-F08  DEN-F09.
     MOVE    RHE-F07            TO  DEN-F112.
     MOVE    RHE-F07            TO  DEN-F111.
*****MOVE    RHE-F07            TO  DEN-F113.
*#2019/01/27 NAV ST
     MOVE    ZERO               TO  DEN-F113.
*    分類（部門）
     MOVE    RHE-F34(1:4)       TO  DEN-F12.
*#2019/01/27 NAV ED
*2014.07.08↓
     MOVE    RHE-F22            TO  DEN-F133.
*2014.07.08↑
*2014.04.15↓
     MOVE    "9"                TO  DEN-F134.
*2014.04.15↑
     MOVE    RHE-F10            TO  DEN-F1411.
     MOVE    RHE-F11            TO  DEN-F1412(1:5).
     MOVE    RHE-F12            TO  DEN-F1412(6:2).
     MOVE    RHE-F13            TO  DEN-F1412(8:1).
     MOVE    RHE-F14            TO  DEN-F1421.
     MOVE    RHE-F15            TO  DEN-F1422.
     MOVE    RHE-F16            TO  WK-SURYO-HEN.
     MOVE    WK-SURYO-HEN       TO  DEN-F15   DEN-F50.
     MOVE    "1"                TO  DEN-F16.
     MOVE    RHE-F17            TO  DEN-F172  DEN-F512.
     MOVE    RHE-F19            TO  DEN-F173  DEN-F513.
     COMPUTE DEN-F181 = DEN-F15 * DEN-F172.
     COMPUTE DEN-F182 = DEN-F15 * DEN-F173.
     MOVE    DEN-F181           TO  DEN-F521.
     MOVE    DEN-F182           TO  DEN-F522.
     MOVE    RHE-F21            TO  DEN-F22.
     MOVE    RHE-F04            TO  DEN-F23.
...
*伝票備考１、２をワークに退避する
     IF  RHE-F24  NOT =  SPACE
         MOVE  RHE-F24          TO  WK-BIKO-1
     ELSE
         MOVE  SPACE            TO  WK-BIKO-1
     END-IF.
     IF  RHE-F25  NOT =  SPACE
         MOVE  RHE-F25          TO  WK-BIKO-2
     ELSE
         MOVE  SPACE            TO  WK-BIKO-2
     END-IF.
...
*返品累積データ　担当者／部門／日付　計上確定セット
     MOVE    LINK-BMNCD         TO  RHE-F84.
     MOVE    LINK-TANCD         TO  RHE-F85.
     MOVE    SYS-DATEW          TO  RHE-F86.
     MOVE    SYS-DATEW          TO  RHE-F96.
     MOVE    WK-SYS-TIME1       TO  RHE-F97.
     MOVE    LINK-BMNCD         TO  RHE-F98.
     MOVE    LINK-TANCD         TO  RHE-F99.
*
 MAIN-010.
     REWRITE RHE-REC.    *>返品累積データ更新
*
 MAIN-020.
     PERFORM  COMRHEF-READ-SEC.
*
...
*T↓
*    IF  RHE-F04      =  8482708
*        DISPLAY "DENNO= " RHE-F04
*        DISPLAY "LINK-TANNO= " LINK-TANCD
*        DISPLAY "TANNO= " RHE-F82
*    END-IF.
*T↑
*処理経過出力
 ...
 READ001.
*バッチ番号チェック
     IF  LINK-JDATE  =  RHE-F78
     AND LINK-JTIME  =  RHE-F79
         CONTINUE
     ELSE
         MOVE    "END"          TO   END-FLG
         GO                     TO   COMRHEF-READ-EXIT
 ...
 READ002.
*取引先チェック
     IF  LINK-TOKCD NOT =  ZERO
         IF  LINK-TOKCD =  RHE-F01
             CONTINUE
         ELSE
           MOVE  "END"          TO        END-FLG
           GO                   TO        COMRHEF-READ-EXIT
...
*計上区分が１以外は読み飛ばし
     ...
     END-IF.
 READ004.
*担当者チェック（ＰＡＲＡ担当者以外は読み飛ばし）
     IF  RHE-F82  NOT =  LINK-TANCD
         GO                     TO        COMRHEF-READ-SEC
     END-IF.
 READ005.
*入力日チェック（ＰＡＲＡ入力日以外は読み飛ばし）
     IF  RHE-F83  NOT =  LINK-NDATE
         GO                     TO        COMRHEF-READ-SEC
     END-IF.
 READ006.
*計上日チェック（計上済のデータは対象外）
...
*
     MOVE      RHE-F01     TO     TB1-F01.
     MOVE      RHE-F09     TO     TB1-F02.
     READ      HSHOTBL
               INVALID
...
*
     MOVE      RHE-F01     TO     TB2-F01.
     MOVE      RHE-F08     TO     TB2-F04.
     MOVE      RHE-F10     TO     TB2-F031.
     MOVE      RHE-F11     TO     TB2-F0321.
     MOVE      RHE-F12     TO     TB2-F0322.
     MOVE      RHE-F13     TO     TB2-F0323.
     READ      SHOTBL2
               INVALID
...
*
     ...
     MOVE      RHE-F11     TO     MEI-F012(1:5).
     MOVE      RHE-F12     TO     MEI-F012(6:2).
     MOVE      RHE-F13     TO     MEI-F012(8:1).
     READ      HMEIMS
               INVALID
...
*
     MOVE   DEN-ADD-CNT       TO      LINK-DEN-ADD-CNT.
*
     CLOSE  HSHOTBL  SHOTBL2  HMEIMS  HTOKMS
            HTENMS   HJYOKEN
            COMRHEF  SHTDENF  ZAMZAIF.
*
 END-EXIT.
     EXIT.
****************************************************************
...
*当月分／次月分の判定
*     次月の場合
     IF  WK-SIMEBI  <  RHE-F07(1:6)
        COMPUTE  ZAI-F04  =  ZAI-F04  +  WK-RHE-F16 *>現在庫数
        COMPUTE  ZAI-F12  =  ZAI-F12  -  WK-RHE-F16 *>次月出庫数
        COMPUTE  ZAI-F13  =  ZAI-F13  -  WK-RHE-F16 *>次月売上数
        COMPUTE  ZAI-F14  =  ZAI-F14  +  WK-RHE-F16 *>次月返品数
     ELSE
        COMPUTE  ZAI-F04  =  ZAI-F04  +  WK-RHE-F16 *>現在庫数
        COMPUTE  ZAI-F06  =  ZAI-F06  +  WK-RHE-F16 *>当月入出数
        COMPUTE  ZAI-F08  =  ZAI-F08  -  WK-RHE-F16 *>当月出庫数
        COMPUTE  ZAI-F09  =  ZAI-F09  -  WK-RHE-F16 *>当月売上数
        COMPUTE  ZAI-F10  =  ZAI-F10  +  WK-RHE-F16 *>当月返品数
     END-IF.
*商品名称マスタ読込み
     PERFORM   HMEIMS-READ-SEC.
*商品名称マスタ存在チェック
...
*    END-IF.
     IF  HMEIMS-INV-FLG  =  SPACE
         MOVE  MEI-F031      TO   ZAI-F30
     END-IF.
     MOVE      RHE-F01       TO   ZAI-F29.
     MOVE      SYS-DATEW     TO   ZAI-F98.
     MOVE      SYS-DATEW     TO   ZAI-F99.
     WRITE     ZAI-REC.
     MOVE      "1"           TO   DEN-F27C.
*2014.04.15↑
*#2021/08/31 NAV ST 在庫更新時は納品日を出荷日にセットする
     MOVE    RHE-F07         TO   DEN-F113.
*#2021/08/31 NAV ED
*
 ZAIKO-UPDATE1-EXIT.
...
*****伝票区分でＤ３６５伝票番号の先頭１桁を変更
     ...
     ELSE
          IF   RHE-F23   =  "42"
               MOVE "N"    TO     WK-D365-PARA-IN1
          ELSE
               MOVE "W"    TO     WK-D365-PARA-IN1
          END-IF
     END-IF.
     MOVE      RHE-F01     TO     WK-D365-PARA-IN2.
