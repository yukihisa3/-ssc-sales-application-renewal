...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SFU3080I.COB`
...
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    業務名　　　　　　　：　社内振替　　　　　　　　　        *
*    モジュール名　　　　：　社内振替情報チェックリスト出力指示*
*    　　　　　　　　　　：　（出力指示）　　　　　　　　　　　*
*    作成日／作成者　　　：　2017/01/05 INOUE                  *
*    処理概要　　　　　　：　リスト発行指示　　　　　　　　　　*
*    流用　　　　　　　　：　SSY3813I                          *
*                                                              *
*    更新日／更新者　　　：　                                  *
...
*画面表示用ワーク
 01  DSP-WORK.
     03  WORK-PGID                PIC  X(08)  VALUE  "SFU3080I".
     03  WORK-FORMID              PIC  X(08)  VALUE  "FFU30801".
*フラグ領域
 01  FLG-AREA.
...
******************************************************************
 ...
 01  LINK-OUT-DTKBN      PIC  X(01).
 01  LINK-OUT-INKBN      PIC  X(01).
 01  LINK-OUT-KANKBN     PIC  X(01).
...
*入力日（開始）チェック
***  未入力チェック
     IF       DSP-NYRDTS  NOT NUMERIC
         OR   DSP-NYRDTS  =  ZERO
              MOVE   ZERO    TO   DSP-NYRDTS
     ELSE
***  論理チェック
              MOVE     "2"            TO   LINK-IN-KBN
              MOVE     ZERO           TO   LINK-IN-YMD6
              MOVE     DSP-NYRDTS     TO   LINK-IN-YMD8
              MOVE     ZERO           TO   LINK-OUT-RET
              MOVE     ZERO           TO   LINK-OUT-YMD
              CALL     "SKYDTCKB"     USING   LINK-IN-KBN
                                              LINK-IN-YMD6
              ...
              IF   LINK-OUT-RET   = 9
                   MOVE   3       TO   ERR-FLG
                   MOVE  "R"      TO   EDIT-OPTION  OF  DSP-NYRDTS
                   MOVE  "C"      TO   EDIT-CURSOR  OF  DSP-NYRDTS
                   GO             TO   PARA-CHK-EXIT
              END-IF
*
              MOVE  "M"      TO   EDIT-OPTION  OF  DSP-NYRDTS
              MOVE  SPACE    TO   EDIT-CURSOR  OF  DSP-NYRDTS
     END-IF.
*
*入力日（終了）チェック
***  未入力チェック
     IF       DSP-NYRDTE  NOT NUMERIC
         OR   DSP-NYRDTE  =  ZERO
         OR   DSP-NYRDTE  =  99999999
              MOVE 99999999  TO   DSP-NYRDTE
     ELSE
***  論理チェック
              MOVE     "2"            TO   LINK-IN-KBN
              MOVE     ZERO           TO   LINK-IN-YMD6
              MOVE     DSP-NYRDTE     TO   LINK-IN-YMD8
              MOVE     ZERO           TO   LINK-OUT-RET
              MOVE     ZERO           TO   LINK-OUT-YMD
              CALL     "SKYDTCKB"     USING   LINK-IN-KBN
                                              LINK-IN-YMD6
              ...
              IF   LINK-OUT-RET   = 9
                   MOVE   3       TO   ERR-FLG
                   MOVE  "R"      TO   EDIT-OPTION  OF  DSP-NYRDTE
                   MOVE  "C"      TO   EDIT-CURSOR  OF  DSP-NYRDTE
                   GO             TO   PARA-CHK-EXIT
              END-IF
*
              MOVE  "M"      TO   EDIT-OPTION  OF  DSP-NYRDTE
              MOVE  SPACE    TO   EDIT-CURSOR  OF  DSP-NYRDTE
     END-IF.
*
*入力日　大小チェック
     IF       DSP-NYRDTS >  DSP-NYRDTE
              MOVE      4    TO   ERR-FLG
              MOVE     "R"   TO   EDIT-OPTION  OF  DSP-NYRDTS
              MOVE     "R"   TO   EDIT-OPTION  OF  DSP-NYRDTE
              MOVE     "C"   TO   EDIT-CURSOR  OF  DSP-NYRDTS
...
*
*入力区分チェック
     IF    (  DSP-NYRKBN NOT =    " "  )  AND
           (  DSP-NYRKBN NOT =    "1"  )  AND
           (  DSP-NYRKBN NOT =    "2"  )
              MOVE      6    TO   ERR-FLG
              MOVE     "R"   TO   EDIT-OPTION  OF  DSP-NYRKBN
              MOVE     "C"   TO   EDIT-CURSOR  OF  DSP-NYRKBN
              GO             TO   PARA-CHK-EXIT
     ELSE
              MOVE     "M"   TO   EDIT-OPTION  OF  DSP-NYRKBN
              MOVE     SPACE TO   EDIT-CURSOR  OF  DSP-NYRKBN
     END-IF.
*
*完納区分チェック
     IF    (  DSP-KANKBN NOT =    " "  )  AND
           (  DSP-KANKBN NOT =    "1"  )  AND
           (  DSP-KANKBN NOT =    "2"  )
              MOVE      6    TO   ERR-FLG
              MOVE     "R"   TO   EDIT-OPTION  OF  DSP-KANKBN
              MOVE     "C"   TO   EDIT-CURSOR  OF  DSP-KANKBN
              GO             TO   PARA-CHK-EXIT
     ELSE
              MOVE     "M"   TO   EDIT-OPTION  OF  DSP-KANKBN
              MOVE     SPACE TO   EDIT-CURSOR  OF  DSP-KANKBN
     END-IF.
*
*発行区分チェック
...
*実行
         WHEN   "E000"
                ...
                MOVE    DSP-NYRDTS    TO   LINK-OUT-DATEF
                MOVE    DSP-NYRDTE    TO   LINK-OUT-DATET
                MOVE    DSP-DTKBN     TO   LINK-OUT-DTKBN
                MOVE    DSP-NYRKBN    TO   LINK-OUT-INKBN
                MOVE    DSP-KANKBN    TO   LINK-OUT-KANKBN
                MOVE    DSP-OUTKBN    TO   LINK-OUT-OUTKBN
                MOVE    DSP-PRTKBN    TO   LINK-OUT-PRTKBN
*↓TEST
*        DISPLAY "LINK-OUT-SOKO   = " LINK-OUT-SOKO   UPON CONS
*        DISPLAY "LINK-OUT-DATEF  = " LINK-OUT-DATEF  UPON CONS
*        DISPLAY "LINK-OUT-DATET  = " LINK-OUT-DATET  UPON CONS
*        DISPLAY "LINK-OUT-DTKBN  = " LINK-OUT-DTKBN  UPON CONS
*        DISPLAY "LINK-OUT-INKBN  = " LINK-OUT-INKBN  UPON CONS
*        DISPLAY "LINK-OUT-KANKBN = " LINK-OUT-KANKBN UPON CONS
