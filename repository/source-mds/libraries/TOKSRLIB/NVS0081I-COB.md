...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NVS0081I.COB`
...
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    業務名　　　　　　　：　Ｄ３６５連携　　　　　　　　　　　*
*    モジュール名　　　　：　Ｄ３６５手動送受信指示（回線Ｍ）　*
*    作成日／作成者　　　：　2020/06/15   NAV TAKAHASHI        *
...
*画面表示用ワーク
 01  DSP-WORK.
     ...
     03  WORK-FORMID              PIC  X(08)  VALUE  "FVS00811".
*フラグ領域
 01  FLG-AREA.
     ...
     03  KAI-INV-FLG              PIC  X(03)  VALUE  SPACE.
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG7.
         05  FILLER              PIC   N(20)
             VALUE NC"！！回線使用中です。確認必要！！".
     03  ERR-MSG8.
         05  FILLER              PIC   N(20)
     VALUE NC"回線が使用中です。他端末状況を確認！！".
     03  ERR-MSG9.
         05  FILLER              PIC   N(20)
     VALUE NC"送受信処理　起動ＮＧ！！　回線確認！！".
*
...
*
 01  FILE-ERR.
     ...
     03  JYO-ERR           PIC N(20) VALUE
                        NC"条件ファイルエラー".
     03  KAI-ERR           PIC N(20) VALUE
                        NC"回線管理マスタエラー".
 01  LINK-OUT-JIKOKEKA     PIC X(01).
 01  LINK-OUT-ATSD         PIC X(01)  VALUE  "1".
 01  LINK-OUT-DATE         PIC 9(08).
 01  LINK-OUT-TIME         PIC 9(04).
 01  LINK-OUT-GROP         PIC 9(02).
...
*初期画面の処理
     ...
     MOVE "FVS00811"    TO  DSP-FMT.
     MOVE  SPACE        TO  DSP-FVS00811.
     PERFORM  OPT-CLR-SEC.
*
     IF   KAI-FLG  =  "1"
          MOVE   ERR-MSG7  TO  DSP-KAIMSG
     ELSE
          MOVE   SPACE     TO  DSP-KAIMSG
     END-IF.
*ＢＯＤＹ部入力へ
     MOVE  2            TO  MAIN-FLG.
 DSP-INIT-EXIT.
...
*        ELSE
              ...
              ELSE
                  ...
                  MOVE  SPACE         TO  LINK-OUT-JIKOKEKA
                  CALL "D365SECK" USING LINK-OUT-DATE
                                        LINK-OUT-TIME
                                        LINK-OUT-GROP
                                        LINK-OUT-RUNNO
                                        LINK-IN-TANCD
...
*****DISPLAY "KAI-F02 = " KAI-F02 UPON CONS.
     READ     JSMKAIF
         INVALID     MOVE  "INV"  TO   KAI-INV-FLG
         NOT INVALID MOVE  SPACE  TO   KAI-INV-FLG
     END-READ.
*
     IF  KAI-INV-FLG = "INV"
         DISPLAY NC"＃＃回線管理マスタ異常！！＃＃" UPON CONS
         DISPLAY NC"＃＃レコード未存在です。　＃＃" UPON CONS
         MOVE    4000             TO   PROGRAM-STATUS
         STOP  RUN
     END-IF.
*
     MOVE     KAI-F07             TO   KAI-FLG.
*
     CLOSE  JSMKAIF.
*
 JSMKAIF-READ-EXIT.
