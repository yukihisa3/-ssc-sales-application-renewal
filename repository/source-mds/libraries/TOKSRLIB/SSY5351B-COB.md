...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY5351B.COB`
...
*     流用元　　　　     ： SBM0082B
*     　　　　           　
*     更新日／更新者     ： 2022/01/27 NAV TAKAHASHI
*     修正概要           ： 荷姿情報出力追加
*     　　　　           ：
*
******************************************************************
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG                PIC  X(08)     VALUE "SSY5351B".
     ...
     03  MSG-END.
         ...
         05  FILLER               PIC  X(11)     VALUE
                                        " END   *** ".
*出荷荷姿情報
 01  WK-SYUKKA-NISUGATA-REC.
     03  WK-SYK-KBN               PIC  X(01).
     03  WK-SYK-ITF               PIC  X(16).
     03  WK-SYK-NISUGATA          PIC  X(02).
     03  WK-SYK-SURYO             PIC  9(06).
     03  WK-SYK-FILLER            PIC  X(1175).
*    日付変換ワーク（パラメタ用）
 01  LINK-AREA.
...
**************MOVE       WK-REC2(1:307)   TO   WK-MEC(1:307)
...
***************2022/01/27 NAV ST 出力数＞３以上の時、荷姿行出力
*###           IF   WRT-CNT1  >  3
********************出荷荷姿情報セット
*###                MOVE      "E"       TO  WK-SYK-KBN
*###                MOVE      ALL "0"   TO  WK-SYK-ITF
********************MOVE      "01"      TO  WK-SYK-NISUGATA
*###                MOVE      "00"      TO  WK-SYK-NISUGATA
********************MOVE      SYK-F458  TO  WK-SYK-SURYO
*###                MOVE      1         TO  WK-SYK-SURYO
*###                MOVE      SPACE     TO  WK-SYK-FILLER
*###                MOVE WK-SYUKKA-NISUGATA-REC TO SND-REC
*###                MOVE      "1"       TO  SND-REC(1198:1)
*
*###                WRITE     SND-REC
*###                ADD       1         TO   WRT-CNT1
*###           END-IF
...
**             MOVE      WK-REC2(515:678) TO   WK-REC(523:678)
               MOVE      "1"       TO        WK-REC(1198:1)
**             IF  ( SYK-F350  =  ZERO  )
                   MOVE  SPACE         TO   WK-REC(868:8)
**             END-IF
**             IF  ( SYK-F351  =  ZERO  )
                   MOVE  SPACE         TO   WK-REC(876:8)
**             END-IF
**             IF  ( SYK-F352  =  ZERO  )
                   MOVE  SPACE         TO   WK-REC(884:8)
**             END-IF
**             IF  ( SYK-F353  =  ZERO  )
**                 MOVE  WK-KAISIBI    TO   WK-REC(892:8)
**             END-IF
               MOVE      WK-REC    TO        SND-REC
...
*    MOVE      "1"       TO        WK-REC(1198:1).
**   IF  ( SYK-F409  =  ZERO  )
         MOVE  SPACE         TO   WK-REC(48:8).
**   END-IF.
**   IF  ( SYK-F410  =  ZERO  )
         MOVE  SPACE         TO   WK-REC(56:8).
**   END-IF.
     MOVE      WK-REC    TO        SND-REC.
...
*    DISPLAY "NDATE   =" PARA-IN-NDATES  UPON  CONS.
