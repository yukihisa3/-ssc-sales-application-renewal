...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY0510L.COB`
...
*    メッセージ　エリア
 01  MSG-AREA.
     03  MSG-ABEND1.
         ...
         05  ERR-PG-ID            PIC  X(08)  VALUE  "NSY0510L".
...
*　　センター納品データ読込み　　　　　　  構造■            *
*============================================================*
 DNCENTF-RD-SEC       SECTION.
     MOVE     "DNCENTF-RD-SEC"   TO   S-NAME.
*    センター納品データ読込み
     READ     DNCENTF  NEXT  AT  END
              IF READ-CNT > ZERO
                 MOVE      WK-GK-SURYO     TO     PRT-SOUGOK
                 PERFORM   SYUKEI-WT-SEC
              END-IF
              MOVE     "END"      TO       FLG-END
              GO                  TO       DNCENTF-RD-EXIT
     END-READ.
 READ010.
*    バッチ番号のチェック
     ...
     END-IF.
     IF       PARA-SOKO   =  SPACE
              GO                 TO   DNCENTF-RD-010
     END-IF.
*    抽出条件のチェック
     IF       PARA-SOKO      =    DKC-F04
              GO                  TO   DNCENTF-RD-010
     ELSE
              GO                  TO   DNCENTF-RD-SEC
     END-IF.
 DNCENTF-RD-010.
