...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0330B.COB`
...
*    作成者／更新者　　　：　NAV T.TAKAHASHI                   *
*    処理概要　　　　　　：　分類（部門）追加　　　　　　　　　*
*    更新日／更新者　　　：　2020/05/25 INOUE                  *
*    更新概要　　　　　　：　Ｄ３６５連携　　　　　　　　　　　*
*    更新日／更新者　　　：　2021/08/31 TAKAHASHI              *
*    更新概要　　　　　　：　在庫更新有りの時、出荷日セット　　*
*    更新日／更新者　　　：　2022/08/12 TAKAHASHI              *
*    更新概要　　　　　　：　範囲指定チェックを変更　　　　　　*
****************************************************************
...
*メッセージ定義エリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0330B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0330B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
 ...
 01  LINK-TOKFROM            PIC  9(08).
 01  LINK-TOKTO              PIC  9(08).
...
*パラメタをメッセージをして表示
     ...
     DISPLAY "## LINK-TOKFROM = " LINK-TOKFROM UPON CONS.
     DISPLAY "## LINK-TOKTO   = " LINK-TOKTO   UPON CONS.
...
*スタートキーセット
     MOVE     "1"                TO   RHE-F80.
     MOVE     LINK-TANFROM       TO   RHE-F82.
     MOVE     LINK-NFROM         TO   RHE-F83.
     MOVE     LINK-TOKFROM       TO   RHE-F01.
*
...
*計上区分が１より大きい場合は終了へ
 ...
 READ020.
*担当者範囲チェック
     IF  LINK-TANFROM  <=  RHE-F82
     AND LINK-TANTO    >=  RHE-F82
         CONTINUE
     ELSE
...
*        GO                     TO        COMRHEF-READ-EXIT
*****END-IF.
*#2022/08/12 NAV ST 入力日範囲チェック方法変更
     IF  LINK-NFROM    <=  RHE-F83
     AND LINK-NTO      >=  RHE-F83
         CONTINUE
     ELSE
         GO                     TO        COMRHEF-READ-SEC
     END-IF.
*#2022/08/12 NAV ED 入力日範囲チェック方法変更
 READ040.
*取引先範囲チェック
     IF  LINK-TOKFROM  <=  RHE-F01
     AND LINK-TOKTO    >=  RHE-F01
         CONTINUE
     ELSE
         GO                     TO        COMRHEF-READ-SEC
     END-IF.
