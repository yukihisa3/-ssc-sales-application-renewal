...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY9217L.COB`
...
*
 ...
 01  PG-ID             PIC  X(08)      VALUE  "SSY9217L".
 01  WK-MSG1           PIC  N(14)
...
*      05  HD8-01                PIC  N(06).
*      05  FILLER                PIC  X(01)  VALUE  SPACE.
*      05  HD8-02                PIC  N(14)  VALUE  SPACE.
...
*--------------------------------------------------------------*
 ...
 DKCERR                 SECTION.
     ...
     DISPLAY  "### SSY9217L DKC ERROR " DKC-ST " "
 ...
 LBLERR                 SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY9217L LBL ERROR " LBL-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
...
*得意先マスタ
 TOKERR                SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY9217L TOK ERROR " TOK-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
...
*店舗マスタ
 TENERR                SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY9217L TEN ERROR " TEN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
 ...
 PRTERR                 SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY9217L PRTFILE ERROR " PRT-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
 ...
 HSBERR                 SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY9216L DCMHSBL2 ERROR " HSB-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
...
*    定番／特売区分
     EVALUATE LBL-F11
         WHEN "1"
              MOVE      NC"　　定期・定番" TO   HD1-01(IY)
         WHEN "2"
              MOVE      NC"　　　　特　売" TO   HD1-01(IY)
         WHEN "3"
              MOVE      NC"　　投入・本発" TO   HD1-01(IY)
         WHEN "4"
              MOVE      NC"　　　新店改装" TO   HD1-01(IY)
         WHEN "6"
              MOVE      NC"ダイレクト客注" TO   HD1-01(IY)
         WHEN "7"
              MOVE      NC"　　　　改　廃" TO   HD1-01(IY)
         WHEN "8"
              MOVE      NC"　　　商管補充" TO   HD1-01(IY)
         WHEN "9"
              MOVE      NC"　　　ＢＹ改廃" TO   HD1-01(IY)
         WHEN "A"
              MOVE      NC"　本部在庫補充" TO   HD1-01(IY)
         WHEN "B"
              MOVE      NC"　　備品用度品" TO   HD1-01(IY)
         WHEN "C"
              MOVE      NC"プロモーション" TO   HD1-01(IY)
         WHEN OTHER
              MOVE      NC"　　定期・定番" TO   HD1-01(IY)
     END-EVALUATE.
*#2020/03/06 NAV ST 発注種別変換マスタより取得
     MOVE     LBL-F03                      TO   HSB-F01.
     MOVE     LBL-F11                      TO   HSB-F03.
     PERFORM  DCMHSBL2-READ-SEC.
     IF  DCMHSBL2-INV-FLG  =  SPACE
              MOVE   HSB-F07(2:7)          TO   HD1-01(IY)
*********DISPLAY "HSB-F07 = " HSB-F07    UPON CONS
*********DISPLAY "HD1-01  = " HD1-01(IY) UPON CONS
     ELSE
              MOVE   NC"　　定　期　　"    TO   HD1-01(IY)
     END-IF.
*#2020/03/06 NAV ED 発注種別変換マスタより取得
*    部門
     ...
     END-PERFORM.
     MOVE     WK-HENKAN-2    TO        HD3-01(IY)(1:1).
     MOVE     WK-HENKAN-3    TO        HD3-01(IY)(2:1).
     MOVE     WK-HENKAN-4    TO        HD3-01(IY)(3:1).
*    個口数（分子）
     MOVE     SPACE          TO        WK-HENKAN-N.
...
*
     EVALUATE LBL-F11
         ...
         WHEN "6"   MOVE NC"ダイレクト"     TO   HD8-01(IY)
         WHEN "7"   MOVE NC"改廃"           TO   HD8-01(IY)
         WHEN "8"   MOVE NC"商管補充"       TO   HD8-01(IY)
         WHEN "9"   MOVE NC"ＢＹ改廃"       TO   HD8-01(IY)
         WHEN "A"   MOVE NC"本部在補充"     TO   HD8-01(IY)
         WHEN "B"   MOVE NC"備品用度品"     TO   HD8-01(IY)
         WHEN "C"   MOVE NC"プロモーシ"     TO   HD8-01(IY)
...
*
     ...
     END-IF.
*
     CLOSE    DSCENTF HTOKMS TENMS1 PRTFILE LBLXXXF.
*#2020/03/06 NAV ST
     CLOSE    DCMHSBL2.
*#2020/03/06 NAV ED
*
