...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NVD045R2.COB`
...
*
 ...
 FILE-CONTROL.
****<<  新入出庫ファイル  >>******************************
     SELECT   DNSFILF   ASSIGN    TO      DA-01-VI-DNSFILL9
                        FILE STATUS          IS  DNS-STATUS
                        ORGANIZATION         IS  INDEXED
                        ACCESS  MODE         IS  SEQUENTIAL
                        RECORD  KEY          IS  DNS-F23
                                                 DNS-F24.
***************************************************************
 DATA                   DIVISION.
***************************************************************
...
**** メッセージ情報           ****
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "NVD045R2".
...
****************************************************************
 LINKAGE                     SECTION.
****************************************************************
 01  PARA-RENDATE            PIC  9(08).
 01  PARA-RENTIME            PIC  9(06).
****************************************************************
 PROCEDURE                   DIVISION
                                USING PARA-RENDATE  PARA-RENTIME.
****************************************************************
 DECLARATIVES.
...
*在庫移動ファイルスタート
     ...
     INITIALIZE                        DNS-REC.
     MOVE     PARA-RENDATE        TO   DNS-F23.
     MOVE     PARA-RENTIME        TO   DNS-F24.
     START  DNSFILF KEY IS  >=  DNS-F23  DNS-F24
            INVALID
            MOVE   "END"          TO   END-FLG
...
*指定日付と異なる場合
     IF     DNS-F23  NOT =  PARA-RENDATE
             MOVE      "END"     TO   END-FLG
             GO                  TO   DNSFILF-READ-EXT
     END-IF.

*指定時刻と異なる場合
     IF     DNS-F24  NOT =  PARA-RENTIME
             MOVE      "END"     TO   END-FLG
             GO                  TO   DNSFILF-READ-EXT
     END-IF.
