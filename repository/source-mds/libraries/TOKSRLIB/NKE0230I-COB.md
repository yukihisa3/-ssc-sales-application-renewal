...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NKE0230I.COB`
...
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         01-GS-DSPF
                        ...
                        STATUS         DSP-ST.
*----<< 入荷検品データ >>--*
     SELECT   RCVNYKXX  ASSIGN         DA-01-S-RCVNYKXX
...
*----<< WORK       >>--*
 ...
 01  KENSU1             PIC  9(07)     VALUE ZERO.
 01  KENSU2             PIC  9(07)     VALUE ZERO.
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### NKE0230I DSPFILE ERROR " DSP-ST " "
     ...
     STOP     RUN.
*----<< 入荷検品データ >>--*
 RCVNYKXX-ERR             SECTION.
     USE AFTER     EXCEPTION PROCEDURE      RCVNYKXX.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE0230I RCVNYKXX ERROR " RCV-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE0230I ZSOKMS1 ERROR " SOK-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 担当者マスタ >>--*
 TANMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE0230I TANMS1 ERROR " TAN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FKE02301.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "NKE0230I"      TO   PGID.
     MOVE    "FKE02301"      TO   FORM.
*
...
*    DISPLAY "TANTOU = " PARA-IN-TANTOU UPON CONS.
     ...
     END-READ.
*
     PERFORM  RCVNYKXX-CNT-SEC.
     MOVE     KENSU1         TO   GAISU.
     MOVE     KENSU2         TO   NAISU.
     COMPUTE  KENSU     =    KENSU1  +  KENSU2.
     MOVE     KENSU          TO   NYUSU.
     IF       KENSU     =    ZERO
              MOVE      3    TO   ERR-FLG
     END-IF.
...
*
 ...
 210-DSP-INIT-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      入荷検品データ件数カウント                  *
*--------------------------------------------------------------*
 RCVNYKXX-CNT-SEC       SECTION.
     MOVE     "RCVNYKXX-CNT-SEC"  TO   S-NAME.
*
 RCVNYKXX-CNT-00.
     OPEN     INPUT     RCVNYKXX.
     MOVE     ZERO                TO   KENSU1 KENSU2.
 RCVNYKXX-CNT-01.
     READ      RCVNYKXX
        AT END
               GO                 TO   RCVNYKXX-CNT-02
        NOT AT END
            IF RCV-F10  =  "1"
               ADD      1         TO   KENSU1
*              DISPLAY "KENSU1="   KENSU1 UPON CONS
               GO                 TO   RCVNYKXX-CNT-01
            END-IF
            IF RCV-F10  =  "2"
               ADD      1         TO   KENSU2
*              DISPLAY "KENSU2="   KENSU2 UPON CONS
               GO                 TO   RCVNYKXX-CNT-01
            END-IF
            IF ( RCV-F10  NOT =  "1" ) AND
               ( RCV-F10  NOT =  "2" )
               DISPLAY NC"発注区分＝１、２以外のデータあり！"
                                              UPON  CONS
