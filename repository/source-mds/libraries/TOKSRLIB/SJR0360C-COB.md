...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0360C.COB`
...
*返品累積データ（入力日）
     SELECT   COMRHELB  ASSIGN    TO        DA-01-VI-COMRHELB
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       HE5-F80 HE5-F82
                                            HE5-F83 HE5-F97
...
*返品抽出データ
     SELECT   AUTWHEF   ASSIGN    TO        DA-01-VI-AUTWHEL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0360C".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0360C".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         ...
         WHEN   "3"      *>計上済
             EVALUATE PARA-DKBN
                 ...
                 WHEN   "2"
                  ...
                  MOVE   PARA-TOKCD  TO HE5-F97
                  MOVE   PARA-DFROM  TO HE5-F83
                  MOVE   PARA-TANST  TO HE5-F82
                  MOVE   PARA-UPTIME TO HE5-F97
                 WHEN   "3"
                  MOVE   "1"        TO HE8-F80
                  MOVE   PARA-TOKCD TO HE8-F01
                  MOVE   PARA-DFROM TO HE8-F86
     ...
     EVALUATE PARA-DKBN
         ...
         WHEN   "1"
            START    COMRHEL4  KEY  >= HE4-F80 HE4-F01 HE4-F07
             ...
             END-START
         WHEN   "2"
            START    COMRHELB  KEY  >= HE5-F80 HE5-F82 HE5-F83
                                       HE5-F97
                INVALID   KEY
                   MOVE      9    TO   END-FG
                   GO   TO   INIT-EXIT
...
*    DISPLAY "WK-HIDUKE = " WK-HIDUKE UPON CONS.
 ...
 MAIN-UPDATE.
*    更新時刻
*    DISPLAY "PARA-UPDATE= " PARA-UPDATE UPON CONS
     IF     F97  =  PARA-UPTIME
            CONTINUE
     ELSE
            GO      TO        MAIN-010
