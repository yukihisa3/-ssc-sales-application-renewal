...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJZ0020I.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 受注残ファイル（Ｌ４） >>-*
     SELECT   JYUZANF   ASSIGN    TO        DA-01-VI-JYUZANL4
...
*画面制御用領域
 01  DSP-CONTROL.
     ...
     03  DSP-PRO                  PIC  X(02).
     03  DSP-FNC                  PIC  X(04).
 01  SET-PGID                     PIC  X(08)  VALUE "SJZ0020I".
 01  SET-FORMID                   PIC  X(08)  VALUE "FJZ00201".
...
*    受注残ファイル読込（ＫＥＹ＝4）
****************************************************************
 JYUZANL4-SET-SEC       SECTION.
*
     MOVE     "JYUZANL4-SET-SEC"   TO   S-NAME.
*
...
*受注残ファイルスタート
     PERFORM JYUZANL4-START-SEC.
*受注残ファイル読込
     IF  JYUZANF-END-FLG  NOT =  "END"
         PERFORM JYUZANL4-READ-SEC
     END-IF.
*ファイルよりワークへセット
...
*
     PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 7
*↓これ必要なのか？？
*?        IF TBL-TANA1(P-CNT IX)  NOT = SPACE
*↑これ必要なのか？？
...
*明細データ存在チェック
*↓これ必要なのか？？
*?   IF  TBL-TANA1(P-CNT WK-MEINO) NOT = SPACE
*?       MOVE  "M"      TO  EDIT-OPTION  OF  DSP-ENDCHK
*?       MOVE   SPACE   TO  EDIT-CURSOR  OF  DSP-ENDCHK
*?   ELSE
*?       IF  ERR-FLG  =  ZERO
...
*    受注残ファイルスタート
****************************************************************
 JYUZANL4-START-SEC    SECTION.
*
     MOVE    "JYUZANL4-START-SEC" TO   S-NAME.
*
...
*
     START  JYUZANF KEY IS >=  JYU-F09  JYU-F13  JYU-F14
                               JYU-F15  JYU-F16  JYU-F17
                               JYU-F11
            INVALID
...
*    受注残ファイル読込
****************************************************************
 JYUZANL4-READ-SEC     SECTION.
*
     MOVE     "JYUZANL4-READ-SEC" TO   S-NAME.
*
 JYUZANL4-010.
     READ   JYUZANF
            NEXT  AT  END    MOVE  "END"   TO   JYUZANF-END-FLG
                             GO            TO   JYUZANL4-READ-EXIT
     END-READ.
 JYUZANL4-020.
*倉庫ＣＤチェック
