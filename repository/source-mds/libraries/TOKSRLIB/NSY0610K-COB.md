...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY0610K.COB`
...
*    作成日／更新日　　　：　2021/02/18                        *
*    作成者／更新者　　　：　ＮＡＶ　　　　　　　　　　　　　　*
*    処理概要　　　　　　：　センター納品データを読み、納品ラ　*
*                        ：　ベルを出力する。　　　　　　　　　*
*                        ：　　　　　　　　　　　　　　        *
**履歴**********************************************************
*    2021/02/18  高橋　　新規作成（ＳＳＹ９２１６Ｌコピー）　　*
*    2023/11/08　高橋　　個口数４桁対応　　　　　　　　　　　　*
*    0000/00/00　　　　　　　　　　　　　　　　　　　　　　　　*
****************************************************************
****************************************************************
...
*----<<プリント>>----*
     SELECT   PRTFILE   ASSIGN    TO        LP-04-PRTF
                        FILE      STATUS    PRT-ST.
*----<<ラベルデータ　　　　　　>>----*
     SELECT   LBLXXXF   ASSIGN    TO        DA-01-VI-DSLBLL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
...
*----<<プリントファイル　　　　　　　>>----*
 ...
 01  PRT-REC.
     03  FILLER            PIC X(200).
*----<<ラベルデータ　　　　　　>>----*
 FD  LBLXXXF             LABEL RECORD   IS   STANDARD.
     COPY     LBLXXXF   OF        XFDLIB
              JOINING   LBL       PREFIX.
*#2019/03/11 NAV ST
******************************************************************
*    発注種別変換マスタ
...
*
 ...
 01  PG-ID             PIC  X(08)      VALUE  "NSY0610K".
 01  WK-MSG1           PIC  N(14)
...
*    漢字変換
 ...
 01  WK-HENKAN3-KANJI.
     03  WK-HENKAN3-N.
         05  WK-HENKAN3-1          PIC  N(01)  VALUE  SPACE.
         05  WK-HENKAN3-2          PIC  N(01)  VALUE  SPACE.
         05  WK-HENKAN3-3          PIC  N(01)  VALUE  SPACE.
 01  WK-HENKAN3                    PIC  9(03).
 01  WK-HENKAN4-KANJI.
     03  WK-HENKAN4-N.
         05  WK-HENKAN4-1          PIC  N(01)  VALUE  SPACE.
         05  WK-HENKAN4-2          PIC  N(01)  VALUE  SPACE.
         05  WK-HENKAN4-3          PIC  N(01)  VALUE  SPACE.
         05  WK-HENKAN4-4          PIC  N(01)  VALUE  SPACE.
 01  WK-HENKAN4                    PIC  9(04).
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*******05  FILLER                PIC  X(07)  VALUE  SPACE.
       05  FILLER                PIC  X(07)  VALUE  SPACE.
*
 01  HD8                         CHARACTER  TYPE  IS  CHR-15.
     03  HD8-A                   OCCURS     4.
       05  FILLER                PIC  X(01)  VALUE  SPACE.
       05  HD8-01                PIC  N(08).
       05  FILLER                PIC  X(01)  VALUE  SPACE.
       05  HD8-02                PIC  N(12)  VALUE  SPACE.
...
*--------------------------------------------------------------*
 DECLARATIVES.
 DKCERR                 SECTION.
     USE AFTER     EXCEPTION PROCEDURE      DNCENTF.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NSY0610K DKC ERROR " DKC-ST " "
     ...
     STOP     RUN.
 LBLERR                 SECTION.
     USE AFTER     EXCEPTION PROCEDURE      LBLXXXF.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NSY0610K LBL ERROR " LBL-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
...
*得意先マスタ
 TOKERR                SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NSY0610K TOK ERROR " TOK-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
...
*店舗マスタ
 TENERR                SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NSY0610K TEN ERROR " TEN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
 ...
 PRTERR                 SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NSY0610K PRTFILE ERROR " PRT-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
 ...
 HSBERR                 SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NSY0610K DCMHSBL2 ERROR " HSB-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     MOVE     4000           TO        PROGRAM-STATUS.
...
*--------------------------------------------------------------*
 100-INIT-RTN           SECTION.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "*** NSY0610K START *** "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS
                                       UPON CONS.
     OPEN     INPUT     DNCENTF   HTOKMS    TENMS1.
     OPEN     OUTPUT    PRTFILE.
     OPEN     OUTPUT    LBLXXXF.
*#2019/03/18 NAV ST
     OPEN     INPUT     DCMHSBL2.
...
*センター納品データスタート
     ...
     START    DNCENTF   KEY  >=   DKC-F01   DKC-F02
                                  DKC-F03   DKC-F04
                                  DKC-F05   DKC-F06
                                  DKC-F19   DKC-F07
         INVALID   KEY
              MOVE    "END"  TO   DKC-FLG  END-FLG
              GO   TO   100-INIT-RTN-EXIT
     END-START.
*    ラベルデータ作成
     DISPLAY "DKC-FLG = " DKC-FLG UPON CONS.
     PERFORM UNTIL      DKC-FLG   =   "END"
*            センター納品データ読込み
             PERFORM DNCENTF-READ-SEC
             IF      DKC-FLG    =    SPACE
*            ラベルデータ出力
                     PERFORM    LBLXXXF-WT-SEC
             END-IF
     END-PERFORM.
     CLOSE   LBLXXXF.
     OPEN    INPUT   LBLXXXF.
*    ラベルデータ読込み
     PERFORM  LBLXXXF-READ-SEC.
*
 100-INIT-RTN-EXIT.
     EXIT.
*--------------------------------------------------------------*
...
*    部門
     MOVE     SPACE          TO        WK-HENKAN3-N.
     MOVE     1              TO        ZERO-FLG.
     MOVE     LBL-F07        TO        WK-HENKAN3.
     PERFORM  VARYING   IZ   FROM  1 BY 1  UNTIL  IZ > 3
              PERFORM   KANJI-HENKAN3-SEC
     END-PERFORM.
     MOVE     WK-HENKAN3-N    TO        HD3-01(IY).
*    個口数（分子）
     MOVE     SPACE          TO        WK-HENKAN4-N.
     MOVE     ZERO           TO        ZERO-FLG.
     MOVE     LBL-F09        TO        WK-HENKAN4.
     PERFORM  VARYING   IZ   FROM  1 BY 1  UNTIL  IZ > 4
              PERFORM   KANJI-HENKAN4-SEC
     END-PERFORM.
     MOVE     WK-HENKAN4-N    TO        HD4-01(IY).
*    個口数（分母）
     MOVE     SPACE          TO        WK-HENKAN4-N.
     MOVE     ZERO           TO        ZERO-FLG.
     MOVE     LBL-F10        TO        WK-HENKAN4.
     PERFORM  VARYING   IZ   FROM  1 BY 1  UNTIL  IZ > 4
              PERFORM   KANJI-HENKAN4-SEC
     END-PERFORM.
     MOVE     WK-HENKAN4-N    TO        HD4-02(IY).
*
*#2019/03/18 NAV ST 発注種別変換マスタより取得
     MOVE     LBL-F03                      TO   HSB-F01.
     MOVE     LBL-F11                      TO   HSB-F03.
     PERFORM  DCMHSBL2-READ-SEC.
     IF  DCMHSBL2-INV-FLG  =  SPACE
              MOVE   HSB-F07               TO   HD8-01(IY)
     ELSE
              MOVE   NC"　　定　期　　"    TO   HD8-01(IY)
     END-IF.
*#2019/03/18 NAV ED 発注種別変換マスタより取得
*店舗名を表示
...
*
         PERFORM   VARYING  IX  FROM   1  BY  1  UNTIL  IX >= 4
              MOVE SPACE    TO    HD0-01(IX)
              MOVE SPACE    TO    HD0-02(IX)
...
*
     ...
     END-IF.
*
     CLOSE    DNCENTF HTOKMS TENMS1 PRTFILE LBLXXXF.
*#2019/03/18 NAV ST
     CLOSE    DCMHSBL2.
*#2019/03/18 NAV ED
*
...
*                  センター納品データ読込み                    *
*--------------------------------------------------------------*
 DNCENTF-READ-SEC            SECTION.
     READ   DNCENTF     NEXT
         AT END
            MOVE  "END"  TO  DKC-FLG
            GO           TO  DNCENTF-READ-EXIT
         NOT   AT  END
            ADD    1     TO  DKC-CNT
     END-READ.
*    バッチ番号のチェック
     ...
     END-IF.
     IF       PARA-SOKO   =  SPACE
              GO                 TO   DNCENTF-READ-0010
     END-IF.
*    抽出条件のチェック
     IF       PARA-SOKO      =   DKC-F04
              GO                 TO   DNCENTF-READ-0010
     ELSE
              GO                 TO   DNCENTF-READ-SEC
     END-IF.
 DNCENTF-READ-0010.
*    部門のチェック
***  DISPLAY "PARA-BUMON = " PARA-BUMON UPON CONS.
...
*                  ラベルデータ出力　　　　　                  *
*--------------------------------------------------------------*
 LBLXXXF-WT-SEC              SECTION.
*    小物
 LBLXXXF-WT-010.
     IF   DKC-F08  =  ZERO
          GO                 TO   LBLXXXF-WT-020
     END-IF.
...
*                  ラベルデータ読込み　　　                    *
*--------------------------------------------------------------*
 LBLXXXF-READ-SEC            SECTION.
     READ   LBLXXXF      NEXT
         AT END
            MOVE  "END"  TO  END-FLG
            GO           TO  LBLXXXF-READ-EXIT
         NOT   AT  END
            ADD    1     TO  LBL-CNT
     END-READ.
*
...
*　　漢字変換処理（３桁）　                構造■2.1.1       *
*============================================================*
 KANJI-HENKAN3-SEC      SECTION.
*    数字を漢字に変換（ワーク変換テーブルにより）
     MOVE    WK-HENKAN3(IZ:1)           TO     IXA.
     IF      IXA       =     ZERO
             MOVE      10              TO     IXA
     END-IF.
     EVALUATE     IZ
         WHEN     1
             IF   IXA        =    10
                  IF    ZERO-FLG  =    1
                        MOVE SU(IXA)   TO   WK-HENKAN3-1
                  ELSE
                        MOVE SPACE     TO   WK-HENKAN3-1
                  END-IF
             ELSE
                  MOVE       SU(IXA)   TO     WK-HENKAN3-1
                  MOVE       1         TO     ZERO-FLG
         ...
         WHEN     2
             IF   IXA        =    10
                  IF    ZERO-FLG  =    1
                        MOVE SU(IXA)   TO   WK-HENKAN3-2
                  ELSE
                        MOVE SPACE     TO   WK-HENKAN3-2
                  END-IF
             ELSE
                  MOVE  SU(IXA)        TO   WK-HENKAN3-2
             END-IF
         WHEN     3
             MOVE      SU(IXA)         TO     WK-HENKAN3-3
     END-EVALUATE.
*
 KANJI-HENKAN3-EXIT.
     EXIT.
...
*　　漢字変換処理（４桁）　                構造■2.1.1       *
*============================================================*
 KANJI-HENKAN4-SEC      SECTION.
*    数字を漢字に変換（ワーク変換テーブルにより）
     MOVE    WK-HENKAN4(IZ:1)           TO     IXA.
     IF      IXA       =     ZERO
             MOVE      10              TO     IXA
     END-IF.
     EVALUATE     IZ
         WHEN     1
             IF   IXA        =    10
                  IF    ZERO-FLG  =    1
                        MOVE SU(IXA)   TO   WK-HENKAN4-1
                  ELSE
                        MOVE SPACE     TO   WK-HENKAN4-1
                  END-IF
             ELSE
                  MOVE       SU(IXA)   TO     WK-HENKAN4-1
                  MOVE       1         TO     ZERO-FLG
         ...
         WHEN     2
             IF   IXA        =    10
                  IF    ZERO-FLG  =    1
                        MOVE SU(IXA)   TO   WK-HENKAN4-2
                  ELSE
                        MOVE SPACE     TO   WK-HENKAN4-2
                  END-IF
             ELSE
                  MOVE  SU(IXA)        TO   WK-HENKAN4-2
                  MOVE       1         TO     ZERO-FLG
         ...
         WHEN     3
             IF   IXA        =    10
                  IF    ZERO-FLG  =    1
                        MOVE SU(IXA)   TO   WK-HENKAN4-3
                  ELSE
                        MOVE SPACE     TO   WK-HENKAN4-3
                  END-IF
             ELSE
                  MOVE  SU(IXA)        TO   WK-HENKAN4-3
             END-IF
         WHEN     4
             MOVE      SU(IXA)         TO     WK-HENKAN4-4
     END-EVALUATE.
*
 KANJI-HENKAN3-EXIT.
     EXIT.
