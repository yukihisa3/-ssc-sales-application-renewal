...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NJH9220B.COB`
...
******************************************************************
 PROGRAM-ID.            NJH9220B.
*                 流用：NJH9120M.TOKSLIBS
******************************************************************
 AUTHOR.                NAV.
 DATE-WRITTEN.          2020/06/09.
*
...
****  文字変換
****    店舗ＣＤ
 01  WK-WHEP-F212                 PIC  X(13).
 01  FILLER                       REDEFINES      WK-WHEP-F212.
     03  HK-WHEP-F212             PIC  9(13).
****    検収日
...
****    行番号
 ...
 01  BRK-WHEP-F208                PIC  9(10)  VALUE  ZERO.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG                PIC  X(08)     VALUE "NJH9220B".
...
****************店舗コード　転送
                MOVE  WHEP-F212        TO  WK-WHEP-F212
                MOVE  HK-WHEP-F212     TO  HEP-F02
****************検収日　　　転送
                MOVE  WHEP-F227        TO  WK-WHEP-F230
                MOVE  HK-WHEP-F230     TO  HEP-F03
****************伝票番号　　転送
                DISPLAY "WHEP-F208 = " WHEP-F208 UPON CONS
                MOVE  WHEP-F208        TO  WK-WHEP-F208
                MOVE  HK-WHEP-F208     TO  HEP-F04
****************行番号　　　転送
                IF  BRK-WHEP-F208 NOT = HK-WHEP-F208
                    MOVE  ZERO         TO  WK-GYO
                    MOVE  HK-WHEP-F208 TO  BRK-WHEP-F208
                END-IF
                ADD   1                TO  WK-GYO
                MOVE  WK-GYO           TO  HEP-F05
****************商品名取得（カナ）
                IF  HEP-F310 = SPACE
                    MOVE  PARA-IN-JUSIN-TOR TO TBL-F01
                    MOVE  HEP-F306(2:13)    TO TBL-F02
                    PERFORM SHOTBL1-READ-SEC
                    IF  SHOTBL1-INV-FLG = "INV"
                        MOVE ALL "*"        TO HEP-F310
                    ELSE
                        ...
                        IF  MEIMS1-INV-FLG = "INV"
                            MOVE ALL "*"    TO HEP-F310
                        ELSE
                            MOVE MEI-F031   TO HEP-F310
                            MOVE MEI-F032   TO HEP-F311
                        END-IF
                    END-IF
                END-IF
                WRITE HEP-REC
...
*ファイルのクローズ
     CLOSE     CVCSG001  EDHENPF  SHOTBL1  MEIMS1.
*
     STOP      RUN.
*
 END-EXIT.
...
*
     ...
     IF  RD-CNT(6:3) = "000" OR "500"
         DISPLAY "## NJH9220B RD-CNT = " RD-CNT  UPON CONS
     END-IF.
*
 CVCSG001-READ-EXIT.
     EXIT.
...
*スタートをかける
     ...
     INITIALIZE                                HE2-REC.
     MOVE     PARA-IN-JUSIN-TOR           TO   HE2-F013.
