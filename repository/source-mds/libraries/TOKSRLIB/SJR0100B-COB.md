...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0100B.COB`
...
*
 ...
 FILE-CONTROL.
*受領データファイル
     SELECT   LIJFILE   ASSIGN    TO        DA-01-S-LIJFILE
...
*    受領データ　
******************************************************************
 FD  LIJFILE           BLOCK CONTAINS  4   RECORDS
...
*受領データ（ヘッダ）
     COPY     RCJYRHDF  OF   XFDLIB   JOINING   JHD    PREFIX.
*受領データ（明細）
     COPY     RCJYRMEF  OF   XFDLIB   JOINING   JME    PREFIX.
*
*    ｶｳﾝﾄ
...
*
*伝票番号ブレイク判定用
 01  WK-DENNO                PIC  9(07)     VALUE  ZERO.
*取引先ブレイク判定用
 01  WK-TORI                 PIC  9(06)     VALUE  ZERO.
*エラーチェック用
...
*数値編集
 01  WK-JME-F05.
     03  WK-JME-F05-1        PIC  9(08)     VALUE  ZERO.
     03  WK-JME-F05-2        PIC  X(03)     VALUE  SPACE.
 01  WK-JME-F06.
     03  WK-JME-F06-1        PIC  X(01)     VALUE  SPACE.
     03  WK-JME-F06-2        PIC  9(09)     VALUE  ZERO.
*
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0100B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0100B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*    対象外伝票区分
*　  伝票区分変換マスタチェック
     MOVE     JHD-F13        TO        HEN-F01.
     MOVE     JHD-F08        TO        HEN-F02.
...
*---          ADD       1            TO    WK-CNT-HEN
*             伝票枚数カウントアップ
              IF   JHD-F02           NOT = WK-DENNO
                   ADD  1            TO    WK-DEN-HEN
                   MOVE JHD-F02      TO    WK-DENNO
              END-IF
       END-IF.
*    受領
       IF   ( JHD-F08 NOT =    "30" )
...
*T↓
*          IF       JHD-F13 = 1996
*                   DISPLAY "TORICD =" JHD-F13 UPON CONS
...
*T↓
*              IF       JHD-F13 = 1996
*                   DISPLAY "INV-FLG=" DENHENF-INV-FLG UPON CONS
*              END-IF
*T↑
               IF     HEN-F05  =   "2"
*T↓
...
*
*   取引先ブレイクで状況ファイル出力
           IF       JHD-F13   NOT =  WK-TORI
*                  受領受信状況ファイル出力
                    PERFORM             COMJYOF-WRT-SEC
*                  取引先コード保管
                    MOVE   JHD-F13      TO   WK-TORI
           END-IF
*
*---       IF   ( JHD-F08 =    "55" OR "56" OR "57" )
*---              GO       TO     MAIN-02
*---       END-IF
     END-IF.
*
...
*    計上日(IN:8桁)
     ...
     END-IF.
*取引先マスタ索引
     MOVE     JHD-F13           TO        TOK-F01.
     PERFORM  HTOKMS-READ-SEC.
*店舗マスタ索引
     MOVE     JHD-F13           TO        TEN-F52.
     MOVE     JHD-F12           TO        TEN-F011.
     PERFORM  HTENMS-READ-SEC.
*商品変換ＴＢＬ索引
     MOVE     JHD-F13           TO        TBL-F01.
     MOVE     JME-F03           TO        TBL-F02.
     PERFORM  HSHOTBL-READ-SEC.
*原単価
     MOVE     JME-F05        TO        WK-JME-F05.
     IF     ( WK-JME-F05-1 NOT NUMERIC ) OR
            ( WK-JME-F05-1 =   ZERO    )
...
*バッチ取引先
     MOVE     JHD-F13        TO        RUI-F03.
*伝票番号
     MOVE     JHD-F02        TO        RUI-F04.
*元伝票番号
*    F05 : INIT
*相手伝票区分
     MOVE     JHD-F08      TO        RUI-F06.
*店舗コード
     MOVE     JHD-F12      TO        RUI-F07.
*計上日
*分類コード
     MOVE     JHD-F10      TO        RUI-F09.
*訂正区分（Ｈ）
*    F10 : INIT
*直接納品先コード
*    F11 : INIT
...
*    F12 : INIT
*行番号
     MOVE     JME-F02        TO        RUI-F31.
*赤黒区分
     MOVE     ZERO           TO        RUI-F32.
*相手商品コード
     MOVE     JME-F03        TO        RUI-F33.
*数量
     MOVE     JME-F04        TO        RUI-F34.
*    COMPUTE  RUI-F34   =    JME-F04   /   10.
*原単価
     IF       WK-ERR-GENTANKA  NOT =  "1"
              MOVE      WK-JME-F05-1   TO        RUI-F35
*             COMPUTE   RUI-F35    =   WK-JME-F05-1   /  100
     END-IF.
*原価金額
     MOVE     JME-F06        TO        WK-JME-F06.
     MOVE     WK-JME-F06-2   TO        RUI-F36.
*    COMPUTE  RUI-F36   =    ONL-F316  /   100.
*商品名カナ１
     MOVE     JME-F10        TO      RUI-F37.
*商品名カナ２
     MOVE     JME-F12        TO      RUI-F38.
*返品理由
*    F39 : INIT
*訂正区分（Ｍ）
*    F40 : INIT
*売単価
     MOVE     JME-F09        TO        RUI-F41.
*    COMPUTE  RUI-F41   =    JME-F09   /   100.
*自社伝票区分
...
*
     CLOSE     LIJFILE  HTENMS   HSHOTBL  DENHENF
