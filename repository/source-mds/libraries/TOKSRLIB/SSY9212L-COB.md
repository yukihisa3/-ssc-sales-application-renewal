...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY9212L.COB`
...
*                            リストを発行する。（ＫＭ／ＤＫ用）*
**履歴**********************************************************
*    2018/02/19  高橋　　ケーヨー追加　　　　　　　　　　　　　*
*    2018/05/01　高橋　　部門範囲追加　　　　　　　　　　　　　*
*    2019/02/21　高橋　　発注種別区分対応　　　　　　　　　　　*
...
*取引先ＣＤ変換
 01  WK-KMK-F304        PIC  X(06).
 01  FILLER             REDEFINES      WK-KMK-F304.
     03  WK-KMK-F304-H  PIC  9(06).
*
...
*----<< BREAK KEY >>--*
 01  BREAK-KEY.
     ...
     03  WK-KMK-F03-1   PIC  9(09).
     03  WK-KMK-M03     PIC  9(03).
*
 01  CHK-KMK-F03        PIC  X(07).
 01  FILLER             REDEFINES      CHK-KMK-F03.
     03  CHK-KMK-F03-1  PIC  9(07).
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  TL00.
...
*#2018/05/01 NAV ST
 01  LINK-SBUMON            PIC  9(03).
 01  LINK-EBUMON            PIC  9(03).
*#2018/05/01 NAV ED
****************************************************************
...
*----<< カーマ出荷確定データ >>--*
 DJSYUKF-ERR            SECTION.
     ...
     DISPLAY  "### SSY9212L DJSYUKF ERROR " DJSYUKF-ST " "
...
*    改ページ
     ...
     OR       KMK-F14   NOT =  WK-KMK-F14
              ...
              MOVE      KMK-F21   TO  WK-KMK-F21
              MOVE      KMK-F264  TO  WK-KMK-F264
              IF  KMK-F304 = 880 OR 1427
                  MOVE      KMK-K06        TO  WK-KMK-F03
              ELSE
                  MOVE      KMK-K06        TO  WK-KMK-F03
              END-IF
              MOVE      KMK-M091  TO  WK-KMK-M091
...
*****MOVE     KMK-F03(1:7)        TO  WK-KMK-F03-1.
     IF  KMK-F304 = 880 OR 1427
         MOVE      KMK-K06        TO  WK-KMK-F03-1
     ELSE
         MOVE      KMK-K06        TO  WK-KMK-F03-1
     END-IF.
*****DISPLAY "LST = " WK-KMK-F03  " : " WK-KMK-F03-1.
...
*#2019/02/21 NAV ST
         WHEN 4
              MOVE NC"　緊急ダイレクト　　" TO HD011-2
*#2019/02/21 NAV ED
         WHEN 5
              MOVE NC"　特注客注ダイレクト" TO HD011-2
         WHEN 51
              MOVE NC"　本部在庫補充　　　" TO HD011-2
         WHEN 52
              MOVE NC"　商管在庫補充　　　" TO HD011-2
*#2019/02/21 NAV ST
              IF   WK-KMK-F304-H = 880 OR 882 OR 883
...
*会社名出力
     ...
     IF   WK-KMK-F304-H = 13938 OR 17137
          MOVE NC"ＤＣＭカーマ株式会社"     TO HD01-01
     END-IF.
     IF   WK-KMK-F304-H = 100403 OR 100441 OR 100427
          MOVE NC"ＤＣＭダイキ株式会社"     TO HD01-01
     END-IF.
     IF   WK-KMK-F304-H = 880 OR 882 OR 883 OR 1427 OR 14272 OR 14
          MOVE NC"ホーマック株式会社　"     TO HD01-01
     END-IF.
*#2018/02/19 NAV ST
     IF   WK-KMK-F304-H = 1731 OR 1732 OR 7601 OR 7602
     OR   173  OR  760
...
*****MOVE      KMK-F03     TO   WK-DENPYO.
     IF  KMK-F304  =  880  OR  1427
         MOVE  KMK-F03(1:7) TO  WK-DENPYO(3:7)
...
*****MOVE   KMK-M07(21:15)        TO   MS02-SYONM2.
     MOVE   KMK-M084(1:20)        TO   MS02-SYONM2.
*    特売ＣＤ
...
*#2018/05/01 NAV ST
*部門範囲チェック
     IF       LINK-SBUMON  <=  KMK-F14
     AND      LINK-EBUMON  >=  KMK-F14
              CONTINUE
     ELSE
              GO        TO        900-KMK-READ
     END-IF.
