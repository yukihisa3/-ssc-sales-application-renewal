...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SBM0440L.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 支払ワークファイル >>--*
     SELECT   BMSSIHW   ASSIGN    TO     DA-01-VI-BMSSH6W2
                        ...
                        STATUS           SIH-ST.
*----<< 請求合計Ｆ >>--*
     SELECT   HSEIGKF   ASSIGN    TO     DA-01-VI-SETGKFV3
                        ORGANIZATION     INDEXED
                        ACCESS    MODE   SEQUENTIAL
...
*    COPY     BMSSIHW.
     COPY     BMSSH6W   OF        XFDLIB
              JOINING   SIH       PREFIX.
*----<< 請求合計Ｆ >>--*
 FD  HSEIGKF            LABEL RECORD   IS   STANDARD.
*    COPY     SETGKFA.
     COPY     SETGKFV   OF        XFDLIB
              JOINING   SEI       PREFIX.
*----<< 店舗マスタ >>XT--*
...
*ワーク対象期間終了
 ...
 01  WK-TORCD.
     03  WK-TORCD0      PIC  9(03).
     03  WK-TORCDR      PIC  9(06).
*
*----<< ﾌｱｲﾙ ｽﾃｰﾀｽ >>--*
 01  SIH-ST             PIC  X(02).
 01  SEI-ST             PIC  X(02).
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HEAD01.
     03  FILLER         CHARACTER TYPE BAIKAKU-1-5.
         ...
         05  FILLER     PIC  X(08)  VALUE "SBM0440L".
...
*----<< 支払合計ファイル >>--*
 SIH-ERR                SECTION.
     ...
     DISPLAY  "### SBM0440L BMSSIHW ERROR " SIH-ST " "
...
*----<< 請求合計Ｆ >>--*
 HSEIGKF-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SBM0440L HSEIGKF ERROR " SEI-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
**** CLOSE    BMSSIHW   HSEIGKF    HTENMS    PRTF.
...
*----<< 店舗マスタ >>--*
 HTENMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SBM0440L HTENMS ERROR " TEN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
*----<< 取引先マスタ >>--*
 TOK-ERR-EXC                 SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SBM0440L HTOKMS ERROR " TOK-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
...
*    請求合計ワーク読込み
     ...
     PERFORM  900-SEI-READ  UNTIL INV-FLG2  = 0      OR
                                  END-FLG2  = "END".
*T
*    DISPLAY "END-FLG2    : "  END-FLG2  UPON  CONS.
*T
     IF    END-FLG2     =   SPACE
*         取引先コード退避
...
*
     ...
     EVALUATE  SIH-F428
         WHEN "1001"
               MOVE    NC"仕入明細　"   TO   DENKBNM
         WHEN "1002"
               MOVE    NC"返品明細　"   TO   DENKBNM
         WHEN "1004"
               MOVE    NC"値引明細　"   TO   DENKBNM
         WHEN "2000"
               MOVE    NC"相殺明細　"   TO   DENKBNM
         WHEN "3001"
               MOVE    NC"相殺前支払"   TO   DENKBNM
         WHEN "3002"
               MOVE    NC"相殺合計　"   TO   DENKBNM
         WHEN "3003"
               MOVE    NC"支払額　　"   TO   DENKBNM
         WHEN  OTHER
               MOVE    ALL NC"＊" TO   DENKBNM
     END-EVALUATE.
*
     MOVE      SIH-F402(1:6)      TO        DENNO.
     MOVE      SIH-F419           TO        WK-KINGAK.
     IF        SIH-F420       =    "-"
               COMPUTE  WK-KINGAK   =   WK-KINGAK * -1
     END-IF.
...
*
     ...
     ELSE
              ...
              MOVE     ZERO            TO   WK-TORCD0
              MOVE     SIH-F402(1:6)   TO   WK-TORCDR
