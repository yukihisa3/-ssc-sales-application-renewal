...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/STE2002B.COB`
...
*   PG-NAME･･･手書き伝票（新手書伝票）ＪＡＮ対応（１７３１用）
*   PG-ID  ･･･S T E 2 0 0 2 B
*   更新履歴 ：
*     2011/10/05 飯田/NAV 基幹サーバ統合
*     2022/08/18 高橋/NAV ケーヨー様仕様変更
****************************************************************
...
*プリンター
 FD  PRTF
     ...
     COPY        FTE20021    OF        XMDLIB.
*表示ファイル
 FD  DSPF
     LABEL       RECORD    IS        OMITTED.
     COPY        FTE20022   OF        XMDLIB.
******************************************************************
...
***  ﾜｰｸ  ｴﾘｱ
 01  WRK-TORICD                   PIC  9(08)     VALUE   ZERO.
 01  WRK-AREA.
...
*
     EVALUATE  PARA-KBN
         WHEN  "1"   MOVE  1731   TO  WRK-TORICD
         DISPLAY NC"＃＃ケーヨー資材　東日本" UPON STATION
         WHEN  "2"   MOVE  1732   TO  WRK-TORICD
         DISPLAY NC"＃＃ケーヨー資材　西日本" UPON STATION
         WHEN  "3"   MOVE  7601   TO  WRK-TORICD
         DISPLAY NC"＃＃ケーヨー植物　東日本" UPON STATION
         WHEN  "4"   MOVE  7602   TO  WRK-TORICD
         DISPLAY NC"＃＃ケーヨー植物　西日本" UPON STATION
         WHEN  OTHER
         DISPLAY NC"＃＃取引先指定エラー！！＃＃" UPON STATION
               STOP  RUN
     END-EVALUATE.
*
     MOVE     ZERO               TO   WK-DEN.
...
*    売上伝票ファイルスタート
     ...
     IF       END-FLG  =  9
              DISPLAY NC"出力対象無し１" UPON STATION
              STOP  RUN
     END-IF.
 INIT001.
*伝票総枚数読込み
     READ     HDENJNL  AT  END
         IF   WK-MAI  =  ZERO
              DISPLAY NC"出力対象無し２" UPON STATION
              STOP  RUN
         ELSE
              GO                 TO   INIT002
         END-IF
 ...
 INIT0011.
*ケーヨー以外読み飛ばし
     IF       DEN-F01     >    WRK-TORICD
         IF   WK-MAI  =  ZERO
              DISPLAY NC"出力対象無し３" UPON STATION
              STOP  RUN
         ELSE
              GO                 TO   INIT002
         END-IF
...
*****DISPLAY "DEN-F132 = " DEN-F132 UPON STATION.
     EVALUATE    PARA-KBN
         WHEN    "1"   MOVE  0173    TO  WK-DEN01
         WHEN    "2"   MOVE  0173    TO  WK-DEN01
         WHEN    "3"   MOVE  0760    TO  WK-DEN01
         WHEN    "4"   MOVE  0760    TO  WK-DEN01
         WHEN    OTHER MOVE  DEN-F01 TO  WK-DEN01
     END-EVALUATE.
*#2022/08/18 NAV ST ＤＣＭ仕入先共通ＣＤ印字
     EVALUATE    PARA-KBN
         WHEN    "1"   MOVE "000056" TO  DCMCD
         WHEN    "2"   MOVE "000056" TO  DCMCD
         WHEN    "3"   MOVE "000250" TO  DCMCD
         WHEN    "4"   MOVE "000250" TO  DCMCD
         WHEN    OTHER MOVE "000056" TO  DCMCD
     END-EVALUATE.
*#2022/08/18 NAV ED
