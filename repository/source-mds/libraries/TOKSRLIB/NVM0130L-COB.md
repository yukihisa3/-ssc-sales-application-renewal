...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NVM0130L.COB`
...
*    モジュール名　　　　：（ＥＸＣＥＬ取込）　　　　　　　　　*
*    　　　　　　　　　　　　商品変換テーブル取込チェックリスト*
*    作成日／作成者　　　：　2020/03/31 INOUE                  *
*    処理概要　　　　　　：　取込チェック結果をリスト出力する。*
*                            （全件・エラー分をＰＡＲＡ制御）　*
*    流用元　　　　　　　：　STB0030L.TOKSRLIB                 *
*    更新日／更新者　　　：　2020/12/21 NAV TAKAHASHI          *
*    更新内容　　　　　　：　直／セ区分見→ＯＲＤ区分に変更　　*
****************************************************************
 IDENTIFICATION         DIVISION.
*
 PROGRAM-ID.            NVM0130L.
 AUTHOR.                NAV.
 DATE-WRITTEN.          2020/03/31.
*
 ...
 FILE-CONTROL.
*商品変換テーブル取込ファイル3
     SELECT   STBMNTW3  ASSIGN    TO        DA-01-VI-STBMNTW3
...
*商品変換テーブル取込ファイル2
     SELECT   STBMNTW2  ASSIGN    TO        DA-01-VI-STBMNTW2
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
...
*---------------------------------------------------------------
 ...
 FILE                   SECTION.
*商品変換テーブル取込ファイル1
 FD  STBMNTW3           LABEL RECORD   IS   STANDARD.
     COPY     STBMNTW3  OF        XFDLIB
              JOINING   SW3       PREFIX.
*商品変換テーブル取込ファイル2
 FD  STBMNTW2           LABEL RECORD   IS   STANDARD.
     COPY     STBMNTW2  OF        XFDLIB
              JOINING   SW2       PREFIX.
*担当者マスタ
...
*FLG/ｶｳﾝﾄ
 ...
 01  STBMNTW3-READ-CNT       PIC  9(07)     VALUE  ZERO.
 01  STBMNTW2-READ-CNT       PIC  9(07)     VALUE  ZERO.
...
*メッセージエリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "NVM0130L".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NVM0130L".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HD00.
     03  FILLER         CHARACTER  TYPE YB-22.
         ...
         05  FILLER          PIC  X(08)     VALUE  "NVM0130L".
...
*        05  FILLER     PIC  N(07) VALUE NC"予備ＪＡＮＣＤ".
         05  FILLER     PIC  N(02) VALUE NC"予備".
         05  FILLER     PIC  X(05) VALUE "JANCD".
         05  FILLER     PIC  X(06) VALUE SPACE.
         05  FILLER     PIC  N(02) VALUE NC"予備".
         05  FILLER     PIC  X(07) VALUE "ｲﾝｽﾄｱCD".
         05  FILLER     PIC  X(06) VALUE SPACE.
         05  FILLER     PIC  N(02) VALUE NC"入数".
         05  FILLER     PIC  X(04) VALUE SPACE.
         05  FILLER     PIC  N(04) VALUE NC"振分倉庫".
         05  FILLER     PIC  X(02) VALUE SPACE.
*#2020/12/21 NAV ST
*********05  FILLER     PIC  N(09) VALUE NC"直送／セット組区分".
         05  FILLER     PIC  N(09) VALUE NC"ＯＲＤセット区分　".
*#2020/12/21 NAV ED
*
 01  MS01.
     03  FILLER              CHARACTER TYPE YB.
...
*    03  FILLER              CHARACTER TYPE YB.
         05  FILLER          PIC  X(40)     VALUE  SPACE.
         05  MS02-YOBIJAN    PIC  X(13).
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-YOBIINS    PIC  X(13).
         05  FILLER          PIC  X(01)     VALUE  SPACE.
         05  MS02-IRISU      PIC  X(05).
         05  MS02-IRISUR     REDEFINES  MS02-IRISU.
             07 MS02-IRISU5  PIC  Z,ZZ9.
         05  FILLER          PIC  X(05)     VALUE  SPACE.
         05  MS02-FURSOKO    PIC  X(02).
         05  FILLER          PIC  X(05)     VALUE  SPACE.
         05  MS02-CYOKUSET   PIC  X(01).
*
*検印欄１
 01  KEN01.
...
*ファイルＯＰＥＮ
     IF       PARA-IN-OUTSYU  =  "1"
              OPEN     INPUT  STBMNTW3  TANMS1
              OPEN     OUTPUT PRTF
     ELSE
              OPEN     INPUT  STBMNTW2  TANMS1
              OPEN     OUTPUT PRTF
     END-IF.
*
...
*商品変換テーブル取込ファイルスタート
     IF  PARA-IN-OUTSYU  = "1"
         PERFORM  STBMNTW3-START-SEC
     ELSE
         PERFORM  STBMNTW2-START-SEC
     END-IF.
     IF  END-FLG = "END"
...
*商品変換テーブル取込ファイル読込
     IF  PARA-IN-OUTSYU  = "1"
         PERFORM STBMNTW3-READ-SEC
     ELSE
         PERFORM STBMNTW2-READ-SEC
     END-IF.
     IF  END-FLG = "END"
...
****************************************************************
 STBMNTW3-START-SEC          SECTION.
*
     MOVE    "STBMNTW3-START-SEC" TO   S-NAME.
*
...
*
     START  STBMNTW3  KEY  IS  >=      SW3-F11  SW3-F13
                                       SW3-F14
            INVALID
            MOVE    "END"         TO   END-FLG
     END-START.
...
*    商品変換テーブル取込ファイル3   読込
****************************************************************
 STBMNTW3-READ-SEC           SECTION.
*
     MOVE    "STBMNTW3-READ-SEC"   TO   S-NAME.
*
     READ     STBMNTW3  AT  END
              MOVE     "END"      TO   END-FLG
*             DISPLAY "AAA" UPON CONS
              GO                  TO   STBMNTW3-READ-EXIT
     END-READ.
*件数カウント
     ADD      1                   TO   STBMNTW3-READ-CNT.
*
*モードブレイク判定
...
****************************************************************
 STBMNTW2-START-SEC          SECTION.
*
     MOVE    "STBMNTW2-START-SEC" TO   S-NAME.
*
...
*
     START  STBMNTW2  KEY  IS  >=      SW2-F060 SW2-F11
                                       SW2-F13  SW2-F14
            INVALID
            MOVE    "END"         TO   END-FLG
     END-START.
...
*    商品変換テーブル取込ファイル2   読込
****************************************************************
 STBMNTW2-READ-SEC           SECTION.
*
     MOVE    "STBMNTW2-READ-SEC"   TO   S-NAME.
*
     READ     STBMNTW2  AT  END
              MOVE     "END"      TO   END-FLG
*             DISPLAY "AAA" UPON CONS
              GO                  TO   STBMNTW2-READ-EXIT
     END-READ.
*エラー区分チェック
...
*   予備ＪＡＮＣＤ
     IF  PARA-IN-OUTSYU  = "1"
         IF    SW3-F11   = "1"
               MOVE  SW3-F262         TO   MS02-YOBIJAN
         ELSE
               IF    SW3-F261  = "Y"
                     MOVE  SW3-F262   TO   MS02-YOBIJAN
               ELSE
                     MOVE  SPACE      TO   MS02-YOBIJAN
               END-IF
         END-IF
     ELSE
         IF    SW2-F11   = "1"
               MOVE  SW2-F262         TO   MS02-YOBIJAN
         ELSE
               IF    SW2-F261  = "Y"
                     MOVE  SW2-F262   TO   MS02-YOBIJAN
               ELSE
...
*   予備インストアＣＤ
     IF  PARA-IN-OUTSYU  = "1"
         IF    SW3-F11   = "1"
               MOVE  SW3-F272         TO   MS02-YOBIINS
         ELSE
               IF    SW3-F271  = "Y"
                     MOVE  SW3-F272   TO   MS02-YOBIINS
               ELSE
                     MOVE  SPACE      TO   MS02-YOBIINS
               END-IF
         END-IF
     ELSE
         IF    SW2-F11   = "1"
               MOVE  SW2-F272         TO   MS02-YOBIINS
         ELSE
               IF    SW2-F271  = "Y"
                     MOVE  SW2-F272   TO   MS02-YOBIINS
               ELSE
...
*   入数
     IF  PARA-IN-OUTSYU  = "1"
         IF    SW3-F11   = "1"
               MOVE  SW3-F282         TO   MS02-IRISU5
         ELSE
               IF    SW3-F281  = "Y"
                     MOVE  SW3-F282   TO   MS02-IRISU5
               ELSE
                     MOVE  SPACE      TO   MS02-IRISU
               END-IF
         END-IF
     ELSE
         IF    SW2-F11   = "1"
               MOVE  SW2-F282         TO   MS02-IRISU5
         ELSE
               IF    SW2-F281  = "Y"
                     MOVE  SW2-F282   TO   MS02-IRISU5
               ELSE
...
*   振分倉庫
     IF  PARA-IN-OUTSYU  = "1"
         IF    SW3-F11   = "1"
               MOVE  SW3-F292         TO   MS02-FURSOKO
         ELSE
               IF    SW3-F291  = "Y"
                     MOVE  SW3-F292   TO   MS02-FURSOKO
               ELSE
                     MOVE  SPACE      TO   MS02-FURSOKO
               END-IF
         END-IF
     ELSE
         IF    SW2-F11   = "1"
               MOVE  SW2-F292         TO   MS02-FURSOKO
         ELSE
               IF    SW2-F291  = "Y"
                     MOVE  SW2-F292   TO   MS02-FURSOKO
               ELSE
...
*   直送／セット組区分
     IF  PARA-IN-OUTSYU  = "1"
         IF    SW3-F11   = "1"
               MOVE  SW3-F302         TO   MS02-CYOKUSET
         ELSE
               IF    SW3-F301  = "Y"
                     MOVE  SW3-F302   TO   MS02-CYOKUSET
               ELSE
                     MOVE  SPACE      TO   MS02-CYOKUSET
               END-IF
         END-IF
     ELSE
         IF    SW2-F11   = "1"
               MOVE  SW2-F302         TO   MS02-CYOKUSET
         ELSE
               IF    SW2-F301  = "Y"
                     MOVE  SW2-F302   TO   MS02-CYOKUSET
               ELSE
...
*商品変換テーブル
     IF  PARA-IN-OUTSYU  = "1"
         DISPLAY "STBMNTW3 PRINT-CNT = " STBMNTW3-READ-CNT
                                                       UPON CONS
     ELSE
         DISPLAY "STBMNTW2 PRINT-CNT = " STBMNTW2-READ-CNT
                                                       UPON CONS
     END-IF.
*
     IF       PARA-IN-OUTSYU  =  "1"
              CLOSE  STBMNTW3  TANMS1  PRTF
     ELSE
              CLOSE  STBMNTW2  TANMS1  PRTF
     END-IF.
*
     STOP      RUN.
*
