...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY9211I.COB`
...
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         01-GS-DSPF
                        ...
                        STATUS         DSP-ST.
*----<< 伝票データ >>--*
     SELECT   DJSYUKF   ASSIGN         DA-01-VI-DJSYUKL3
                        ORGANIZATION   INDEXED
                        ACCESS    MODE SEQUENTIAL
                        RECORD    KEY  KMS-K01   KMS-K02
                                       KMS-K03   KMS-K04
                                       KMS-K08   KMS-K05
                                       KMS-K06   KMS-K07
                        STATUS         DJSYUKF-ST.
...
*
 ...
 01  PARA-SDENCD            PIC   9(09).
 01  PARA-EDENCD            PIC   9(09).
 01  PARA-SBUMON            PIC   9(03).
 01  PARA-EBUMON            PIC   9(03).
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSY9211I DSPFILE ERROR " DSP-CNTL " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
     CLOSE    DJSYUKF   HTOKMS    ZSOKMS1    DSPFILE.
     STOP     RUN.
*----<< 伝票データ >>--*
 DJSYUKF-ERR            SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY9211I DJSYUKF ERROR " DJSYUKF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY9211I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY9211I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSY92111.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SSY9211I"      TO   PGID.
     MOVE    "FSY92111"      TO   FORM.
...
*    受信時間
     ...
     ELSE
         IF        ERR-FLG   =    ZERO
                   MOVE      11   TO   ERR-FLG
         END-IF
         MOVE     "C"        TO   EDIT-CURSOR OF JTIME
         MOVE     "R"        TO   EDIT-OPTION OF JTIME
         GO   TO   220-GRP01-CHECK-EXIT
     END-IF.
     IF  JTIME(3:2)  >=   0
...
*             MOVE  SPACE    TO   EDIT-CURSOR  OF  SNOUDT
     END-IF.
*ADD NAV OONO 2010/02/23 BIN
*    伝票番号チェック
     IF       SDENCD  NOT NUMERIC
              MOVE      ZERO      TO   SDENCD
     END-IF.
     IF       EDENCD  NOT NUMERIC
              MOVE      999999999 TO   EDENCD
     END-IF.
*
     IF       SDENCD  >  EDENCD
              IF   ERR-FLG   =    ZERO
                   MOVE      7    TO   ERR-FLG
              END-IF
              MOVE     "C"   TO   EDIT-CURSOR OF SDENCD
              MOVE     "R"   TO   EDIT-OPTION OF SDENCD
              MOVE     "R"   TO   EDIT-OPTION OF EDENCD
     END-IF.
*#2018/05/01 NAV ST
*    部門チェック
...
*#2018/05/01 NAV ED
*帳票発行区分
     IF       OUTKBN  = SPACE  OR  "1"
              MOVE  "M"      TO   EDIT-OPTION  OF  OUTKBN
              MOVE  SPACE    TO   EDIT-CURSOR  OF  OUTKBN
     ELSE
              IF   ERR-FLG   =    ZERO
                   MOVE      10   TO   ERR-FLG
              END-IF
...
*    売上伝票データ存在チェック
     IF  ERR-FLG        =    ZERO
         ...
         INITIALIZE                    KMS-REC
         MOVE      JDATE          TO   KMS-K01
         MOVE      JTIME          TO   KMS-K02
         MOVE      TORICD         TO   KMS-K03
         MOVE      WK-SOKCD       TO   KMS-K04
         MOVE      SNOUDT         TO   KMS-K08
         MOVE      STENCD         TO   KMS-K05
         START     DJSYUKF   KEY  >=   KMS-K01   KMS-K02
                                       KMS-K03   KMS-K04
                                       KMS-K08   KMS-K05
                                       KMS-K06   KMS-K07
              INVALID   KEY
              ...
              NOT INVALID
                   READ      DJSYUKF
                     AT END
                        ...
                        GO   TO   220-GRP01-CHECK-EXIT
                     NOT AT END
                        IF ( JDATE     =    KMS-K01 ) AND
                           ( JTIME     =    KMS-K02 ) AND
                           ( TORICD    =    KMS-K03 )
***************************( STENCD    <=   KMS-K05 ) AND
*店舗ＣＤ範囲チェック無し  ( ETENCD    >=   KMS-K05 ) AND
*納品日範囲チェック無し    ( SNOUDT    <=   KMS-K08 ) AND
***************************( ENOUDT    >=   KMS-K08)
                             IF ( WK-SOKCD  NOT =    SPACE )
                                  IF   SOKO     =     KMS-K04
                                       CONTINUE
...
*ADD NAV OONO 2010/02/23 BIN
     MOVE     SDENCD              TO   PARA-SDENCD.
     MOVE     EDENCD              TO   PARA-EDENCD.
*ADD NAV OONO 2010/02/23 END
*#2018/05/01 NAV ST
     MOVE     SBUMON              TO   PARA-SBUMON.
     MOVE     EBUMON              TO   PARA-EBUMON.
*"2018/05/01 NAV ED
     MOVE     OUTKBN              TO   PARA-OUTKBN.
*
     MOVE     99                  TO   GR-NO.
*
 240-PARA-SEC-EXIT.
...
*ADD NAV OONO 2010/02/23 BIN
                                  EDIT-CURSOR OF SDENCD
                                  EDIT-CURSOR OF EDENCD
*ADD NAV OONO 2010/02/23 END
*#2018/05/01 NAV ST
...
*#2018/05/01 NAV ED
     ...
     MOVE     "M"            TO   EDIT-OPTION OF JDATE
                                  ...
                                  EDIT-OPTION OF ENOUDT
*ADD NAV OONO 2010/02/23 BIN
                                  EDIT-OPTION OF SDENCD
                                  EDIT-OPTION OF EDENCD
*ADD NAV OONO 2010/02/23 END
*#2018/05/01 NAV ST
