...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SFU3998I.COB`
...
*
 ...
 FILE-CONTROL.
*部門マスタ
     SELECT  BUMONL1   ASSIGN    TO        DA-01-VI-BUMONL1
...
*フラグ領域
 01  FLG-AREA.
     ...
     03  BUMONL1-INV-FLG          PIC  X(03)  VALUE  SPACE.
*ワーク領域
 01  WRK-AREA.
***  プログラムスイッチ（画面遷移制御）
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG3.
         05  FILLER              PIC   N(25)
             VALUE NC"部門コードを入力して下さい。".
     03  ERR-MSG4.
         05  FILLER              PIC   N(25)
             VALUE NC"次レコードは存在しません。".
     03  ERR-MSG5.
         05  FILLER              PIC   N(25)
             VALUE NC"部門マスタに登録済みです。".
     03  ERR-MSG6.
         05  FILLER              PIC   N(25)
             VALUE NC"部門マスタに未登録です。".
     03  ERR-MSG7.
         05  FILLER              PIC   N(25)
...
*
 BUM-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE BUMONL1.
     MOVE        BUM-ST    TO        E-ST.
...
*プログラムＩＤ
     MOVE    "SFU3998I"           TO   DSP-PGID.
*ＦＯＲＭＩＤ
     MOVE    "FFU39981"           TO   DSP-FORMID.
*リバース，カーソルパーク解除
*部門コード
     MOVE    "M"     TO      EDIT-OPTION   OF  DSP-BUMCD.
     MOVE    SPACE   TO      EDIT-CURSOR   OF  DSP-BUMCD.
*部門名
     MOVE    "M"     TO      EDIT-OPTION   OF  DSP-BUMNM.
     MOVE    SPACE   TO      EDIT-CURSOR   OF  DSP-BUMNM.
*部門名略
     MOVE    "M"     TO      EDIT-OPTION   OF  DSP-BUMRY.
     MOVE    SPACE   TO      EDIT-CURSOR   OF  DSP-BUMRY.
*処理区分入力へ
     MOVE    "2"                  TO   PSW.
*
 DSP-INIT-EXIT.
...
*部門コード
     IF       DSP-BUMCD  =  SPACE
              MOVE   3       TO   ERR-FLG
              MOVE  "R"      TO   EDIT-OPTION   OF  DSP-BUMCD
              MOVE  "C"      TO   EDIT-CURSOR   OF  DSP-BUMCD
     END-IF.
*  エラーの場合は、ＥＸＩＴへ
     IF       ERR-FLG  NOT =  ZERO
     ...
     END-IF.
*  部門マスタ読込み
     MOVE     DSP-BUMCD     TO   BUM-F01.
     PERFORM  BUMONL1-READ-SEC.
*    IF       BUMONL1-INV-FLG NOT =  SPACE
*             MOVE   3       TO   ERR-FLG
*             MOVE  "R"      TO   EDIT-OPTION   OF  DSP-BUMCD
*             MOVE  "C"      TO   EDIT-CURSOR   OF  DSP-BUMCD
*    END-IF.
*処理区分により判定
     EVALUATE DSP-SYORI
         WHEN  1
               ...
               END-IF
         WHEN  2
               IF   BUMONL1-INV-FLG = "INV"
                    MOVE  6     TO ERR-FLG
                    MOVE "R"    TO EDIT-OPTION OF DSP-BUMCD
                    MOVE "C"    TO EDIT-CURSOR OF DSP-BUMCD
                    MOVE  SPACE TO DSP-BUMNM  DSP-BUMRY
               ELSE
                    MOVE "4"    TO PSW
                    MOVE "M"    TO EDIT-OPTION OF DSP-BUMCD
                    MOVE  SPACE TO EDIT-CURSOR OF DSP-BUMCD
...
*全レコード解放（排他制御用）
     ...
     OPEN     I-O   BUMONL1.
* 属性クリア
     MOVE     SPACE     TO   BUMONL1-INV-FLG.
     MOVE     " "       TO   EDIT-CURSOR OF DSP-BUMCD.
     MOVE     "M"       TO   EDIT-OPTION OF DSP-BUMCD.
*
     MOVE     DSP-BUMCD     TO  BUM-F01.
     START    BUMONL1   KEY  IS  <  BUM-F01
                 WITH   REVERSED  ORDER
         INVALID  KEY
              MOVE  "INV"    TO  BUMONL1-INV-FLG
         NOT  INVALID  KEY
              PERFORM  BUMONL1-NEXTREAD-SEC
     END-START.
     IF  BUMONL1-INV-FLG  =   "INV"
         MOVE      7    TO   ERR-FLG
         MOVE     "C"   TO   EDIT-CURSOR  OF  DSP-BUMCD
...
***  部門コード
     MOVE     DSP-BUMCD           TO   BUM-F01.
***  部門名（正式）
     MOVE     DSP-BUMNM           TO   BUM-F02.
***  部門名（略称）
     MOVE     DSP-BUMRY           TO   BUM-F03.
***  登録者部門
     MOVE     PARA-BUMONCD        TO   BUM-F92  BUM-F96.
***  登録担当者ＣＤ
     MOVE     PARA-TANCD          TO   BUM-F93  BUM-F97.
***  登録日付　　　　
     MOVE     SYS-DATE            TO   BUM-F94  BUM-F98.
***  登録時刻　　　　
     MOVE     WK-TIME(1:6)        TO   BUM-F95  BUM-F99.
*
*部門マスタ登録
     WRITE    BUM-REC.
*
...
*            部門名（略称）
              MOVE     DSP-BUMRY           TO   BUM-F03
*            更新者部門
              MOVE     PARA-BUMONCD        TO   BUM-F96
*            更新担当者ＣＤ
              MOVE     PARA-TANCD          TO   BUM-F97
*            更新日付
              MOVE     SYS-DATE            TO   BUM-F98
*            更新時刻
              MOVE     WK-TIME(1:6)        TO   BUM-F99
*
              REWRITE  BUM-REC
*
     ELSE
              DISPLAY "## BUMONL1 REWRITE ERR ##"  UPON CONS
              DISPLAY "## BUMSON =" BUM-F01 " ##"  UPON CONS
              STOP  RUN
     END-IF.
*
 FILE-UPD-EXIT.
...
*
     ELSE
              DISPLAY "## BUMONL1 DELETE  ERR ##" UPON CONS
              DISPLAY "## BUMSON =" BUM-F01 " ##" UPON CONS
              STOP  RUN
     END-IF.
*
...
*
     MOVE    BUM-F01              TO   DSP-BUMCD.
     MOVE    BUM-F02              TO   DSP-BUMNM.
     MOVE    BUM-F03              TO   DSP-BUMRY.
*
 MST-DSP-EXIT.
     EXIT.
****************************************************************
*             部門マスタ読込み                           *
****************************************************************
 BUMONL1-READ-SEC         SECTION.
     MOVE   "BUMONL1-READ-SEC"    TO   S-NAME.
*
     READ    BUMONL1
         INVALID
             MOVE    "INV"        TO   BUMONL1-INV-FLG
...
*             部門マスタ順読込み                         *
****************************************************************
 BUMONL1-NEXTREAD-SEC     SECTION.
     MOVE   "BUMONL1-NEXTREAD-SEC"     TO   S-NAME.
*
     READ    BUMONL1    NEXT
         AT END
             MOVE    "INV"        TO   BUMONL1-INV-FLG
...
****************************************************************
 END-SEC               SECTION.
*ファイル ＣＬＯＳＥ
     CLOSE             BUMONL1  DSPFILE.
**
 END-EXIT.
     EXIT.
*****************<<  SFU3998I   END PROGRAM  >>******************
