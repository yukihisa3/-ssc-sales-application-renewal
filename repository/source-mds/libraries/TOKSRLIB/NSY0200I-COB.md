...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY0200I.COB`
...
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         01-GS-DSPF
                        ...
                        STATUS         DSP-ST.
*----<< 出荷確定データ >>--*
     SELECT   DNSYUKF   ASSIGN         DA-01-VI-DNSYUKL3
                        ORGANIZATION   INDEXED
                        ACCESS    MODE SEQUENTIAL
                        RECORD    KEY  SYU-K01   SYU-K02
                                       SYU-K03   SYU-K04
                                       SYU-K08   SYU-K05
                                       SYU-K06   SYU-K07
                        STATUS         DNSYUKF-ST.
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### NSY0200I DSPFILE ERROR " DSP-CNTL " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
*    CLOSE    DNSYUKF   HTOKMS    ZSOKMS1    DSPFILE.
     MOVE     4000                     TO   PROGRAM-STATUS.
     STOP     RUN.
*----<< 出荷確定データ >>--*
 DNSYUKF-ERR            SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### NSY0200I DNSYUKF ERROR " DNSYUKF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOSYU-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### NSY0200I HTOKMS ERROR " HTOSYU-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### NSY0200I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSY02001.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "NSY0200I"      TO   PGID.
     MOVE    "FSY02001"      TO   FORM.
...
*    出荷確定データ存在チェック
     IF  ERR-FLG        =    ZERO
         ...
         INITIALIZE                    SYU-REC
         MOVE      JDATE          TO   SYU-K01
         MOVE      JTIME          TO   SYU-K02
         MOVE      TORICD         TO   SYU-K03
         MOVE      WK-SOKCD       TO   SYU-K04
         MOVE      SNOUDT         TO   SYU-K08
         MOVE      STENCD         TO   SYU-K05
              ...
              NOT INVALID
                   READ      DNSYUKF
                     AT END
                        ...
                        GO   TO   220-GRP01-CHECK-EXIT
                     NOT AT END
                        IF ( JDATE     =    SYU-K01 ) AND
                           ( JTIME     =    SYU-K02 ) AND
                           ( TORICD    =    SYU-K03 )
***************************( STENCD    <=   SYU-K05 ) AND
*店舗ＣＤ範囲チェック無し  ( ETENCD    >=   SYU-K05 ) AND
*納品日範囲チェック無し    ( SNOUDT    <=   SYU-K08 ) AND
***************************( ENOUDT    >=   SYU-K08)
                             IF ( WK-SOKCD  NOT =    SPACE )
                                  IF   SOKO     =     SYU-K04
                                       CONTINUE
