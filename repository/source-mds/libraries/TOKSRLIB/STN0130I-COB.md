...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/STN0130I.COB`
...
***************************************************************
 FILE-CONTROL.
*_卸確定ＤＴ１
     SELECT      KKTANAL5  ASSIGN    TO        DA-01-VI-KKTANAL5
                           ORGANIZATION        INDEXED
                           ACCESS    MODE      DYNAMIC
                           RECORD    KEY       TANA-F01
                           FILE      STATUS    TANA-ST   TANA-ST1.
*_卸確定ＤＴ２
     SELECT      KKTANAL3  ASSIGN    TO        DA-01-VI-KKTANAL3
                           ORGANIZATION        INDEXED
                           ACCESS    MODE      RANDOM
                           RECORD    KEY       TAN-F04
                                               TAN-F05
...
*_卸確定ＤＴ１
 FD  KKTANAL5
     ...
     COPY        KKTANAL5  OF        XFDLIB
     JOINING     TANA      AS        PREFIX.
*_卸確定ＤＴ２
 FD  KKTANAL3
     LABEL       RECORD    IS        STANDARD.
     COPY        KKTANAL3  OF        XFDLIB
...
* エラーメッセージエリア
 01  MSG-AREA.
     ...
     03  MSG20             PIC N(15) VALUE
                     NC"明細が入力されていません".
     03  MSG21             PIC N(15) VALUE
                     NC"条件Ｆに倉庫が未登録です".
 01  MSG-AREAR             REDEFINES MSG-AREA.
     03  MSGIX             PIC N(15) OCCURS    21.
* ＰＦキーガイド
 01  PFKEY-MSG.
...
**** _卸日
     03  WK-TDATE          PIC 9(08)   VALUE  ZERO.
**** 部門
     03  WK-BUMON          PIC 9(04)   VALUE  ZERO.
**** ＳＴＡＴＵＳ　エリア　
 01  ST-AREA.
     03  IN-DATA           PIC X(01).
     03  TANA-ST           PIC X(02).
...
******************************************************************
 DECLARATIVES.
 TANA-ERR               SECTION.
     USE           AFTER     EXCEPTION PROCEDURE         KKTANAL5.
     DISPLAY       TANA-ST   UPON      STA.
     DISPLAY       FILE-ERR1 UPON      STA.
     MOVE          4000      TO        PROGRAM-STATUS.
     STOP          RUN.
 TAN-ERR                SECTION.
     USE           AFTER     EXCEPTION PROCEDURE         KKTANAL3.
...
******************************************************************
 DSP-INIT-SEC           SECTION.
     MOVE     SPACE               TO   FTN01301.
     MOVE     HEN-DATE            TO   SDATE.
     MOVE     HEN-TIME            TO   STIME.
     MOVE     WK-TDATE            TO   TDATE.
*
     MOVE     SPACE               TO   DSP-AREA.
     MOVE    "FTN01301"           TO   DSP-FMT.
...
*明細セット
     ...
     PERFORM  UNTIL   ( INV-SW         =    1 )
         ...
         MOVE  TANA-F88           TO   BI011 (IX01)
         MOVE  TANA-F13           TO   BI012 (IX01)
         MOVE  TANA-F17           TO   BI013 (IX01)
         MOVE  TANA-F14           TO   BI014 (IX01)
         MOVE  TANA-F05           TO   MEI-F011      SUB-F011
         MOVE  TANA-F06           TO   MEI-F012(1:5) SUB-F0121
         MOVE  TANA-F07           TO   MEI-F012(6:2) SUB-F0122
         MOVE  TANA-F08           TO   MEI-F012(8:1) SUB-F0123
         READ      HMEIMS
             INVALID
         ...
         READ      SUBMEIF
             INVALID
                MOVE  SPACE       TO   BI011 (IX01)
             NOT  INVALID
                MOVE  SUB-D01     TO   BI011 (IX01)
         END-READ
**   登録時はデータ有りの時プロテクト
         IF    HI001   =   1
...
*入力項目 CHECK
     IF       HI001               =    1
              ...
              IF   WK-DAISOKO     NOT =    "01"
                   ...
                   END-IF
              END-IF
              MOVE     20             TO   JYOKEN-F01
              MOVE     HI003          TO   JYOKEN-F02
              READ     HJYOKEN
                 INVALID
                       IF    ERR-NO  =  ZERO
                             MOVE   21  TO  ERR-NO
...
* 商品名称マスタ存在チェック
                  ...
                  END-READ
* SUB商品名称マスタ存在チェック
                  MOVE  BI004   (IX01)     TO   SUB-F011
                  MOVE  BI005   (IX01)     TO   SUB-F0121
                  MOVE  BI006   (IX01)     TO   SUB-F0122
                  MOVE  BI007   (IX01)     TO   SUB-F0123
                  IF    INV-SW   =   ZERO
                        MOVE  ZERO         TO   INV-SW
                  END-IF
                  READ      SUBMEIF
...
**** IF       BI011 (IX01)                  NUMERIC
     IF       BI011 (IX01)       NOT =      SPACE
*#2022/12/06 NAV ED
              MOVE  BI011 (IX01)       TO   TANA-F88
     ELSE
         MOVE  SPACE                   TO   TANA-F88
     END-IF.
     ...
     MOVE     BI013 (IX01)             TO   TANA-F17.
     MOVE     WK-TDATE                 TO   TANA-F83.
     MOVE     SYS-DATE                 TO   TANA-F96.
     MOVE     WK-TIME(1:6)             TO   TANA-F97.
     MOVE     LINK-IN-BUMCD            TO   TANA-F98.
     MOVE     LINK-IN-TANCD            TO   TANA-F99.
     IF       INV-SW                   =    1
