...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY0820L.COB`
...
*
 ...
 FILE-CONTROL.
*出荷明細ワーク
     SELECT   DCMSMIF       ASSIGN        TO  01-S-DCMSMIF
...
*ステータス
 ...
 01  WORK-AREA.
     ...
     03  WK-SYUBETU3          PIC  X(02)  VALUE  SPACE.
     03  WK-CENTER            PIC  X(04)  VALUE  SPACE.
*ﾊﾞｯﾁNO.
 01  WK-BATCH.
...
*メッセージ情報
 01  MSG-AREA.
     03  SEC-NAME.
         ...
         05  S-NAME          PIC  X(30).
     03  MSG-ABEND1.
         05  FILLER          PIC  X(12)  VALUE  "### NSY0820L".
     ...
     03  MSG-INPUT.
         05  FILLER          PIC  X(13)  VALUE  "*** DCMSMIF ".
...
*ﾁｮｳﾋｮｳﾃｲｷﾞｴﾘｱ
 01  HEAD01.
     ...
     03  FILLER     PIC  X(08)     VALUE  "NSY0820L".
 ...
 01  HEAD05.
     ...
     03  HD05-015   PIC  N(10)     CHARACTER TYPE PIT-2.
     03  FILLER     PIC  X(02)     VALUE  SPACE.
 ...
 01  HEAD066                       CHARACTER TYPE PIT-2.
     03  FILLER     PIC  X(02)     VALUE  SPACE.
     03  FILLER     PIC  N(12)
         VALUE  NC"☆☆　店直センター経由→".
     03  FILLER     PIC  N(08)     VALUE  NC"　納品センター：".
     03  HD066-01   PIC  X(04).
     03  FILLER     PIC  X(01)     VALUE  SPACE.
     03  HD066-02   PIC  N(15).
...
*出荷明細ワーク
 OUT-ERR-SEC        SECTION.
     USE      AFTER  EXCEPTION   PROCEDURE       DCMSMIF.
     MOVE    "DCMSMIF"    TO    ERR-FL-ID.
...
*=============================================================
 CONTROL-SEC         SECTION.
     DISPLAY  "**  NSY0820L   START  **"   UPON  CONS.
*
     ...
     PERFORM  END-SEC.
*
     DISPLAY  "**  NSY0820L   END    **"   UPON  CONS.
     STOP  RUN.
 CONTROL-EXIT.
     EXIT.
*=============================================================
...
*        店舗ＣＤブレイク判定
     ...
     END-IF.
*発注種別区分
     IF  SYU-F19  NOT  =  WK-SYUBETU1
         MOVE     SYU-F19      TO  WK-SYUBETU1
         MOVE     1            TO  PAGE-FLG
     END-IF.
*形状区分
     IF  SYU-F20  NOT  =  WK-SYUBETU2
         MOVE     SYU-F20      TO  WK-SYUBETU2
         MOVE     1            TO  PAGE-FLG
     END-IF.
*発注単位区分
     IF  SYU-F21  NOT  =  WK-SYUBETU3
         MOVE     SYU-F21      TO  WK-SYUBETU3
         MOVE     1            TO  PAGE-FLG
     END-IF.
*_番（上１桁）ブレイク判定
     ...
     END-IF.
*センターＣＤ（店直センター経由の場合）
     IF  SYU-F22  NOT  =  WK-CENTER
         MOVE     SYU-F22      TO  WK-CENTER
         MOVE     1            TO  PAGE-FLG
     END-IF.
*明細出力
     PERFORM  BODY-WT-SEC.
...
*#2019/02/21 NAV ED
     ...
     END-EVALUATE.
*発注種別区分名
     MOVE     SYU-F24                        TO HD065-02.
*形状区分
     MOVE  SYU-F20              TO   HD065-03.
     EVALUATE  SYU-F20
...
*
     IF  SYU-F23  NOT =  SPACE
         MOVE    SYU-F22          TO       HD066-01
         MOVE    SYU-F23          TO       HD066-02
     ELSE
         MOVE    SPACE            TO       HD066-01  HD066-02
     END-IF.
*印刷
     MOVE    11                  TO  LINE-CNT.
     ...
     WRITE   PRT-REC   FROM   SEN02    AFTER   0.
*
     IF  SYU-F23  NOT =  SPACE
         WRITE   PRT-REC   FROM   HEAD066  AFTER   1
         WRITE   PRT-REC   FROM   SEN02    AFTER   0
         ADD     1                TO       LINE-CNT
     END-IF.
...
*商品名
     ...
     END-READ.
*ＪＡＮＣＤセット
     MOVE  "("                 TO  MEI03-01(1:2).
     MOVE  SYU-F07             TO  MEI03-01(2:13).
     MOVE  ")"                 TO  MEI03-01(15:1).
*
