...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY7957N.COB`
...
*
 ...
 FILE-CONTROL.
*    欠品ファイル
     SELECT   DKKEPNF   ASSIGN        TO    DA-01-VI-DKKEPNL1
...
*    メッセージ　エリア
 01  MSG-AREA.
     03  MSG-ABEND1.
         ...
         05  ERR-PG-ID            PIC  X(08)  VALUE  "SSY7957N".
...
*　　欠品データ
 FILEERR-SEC2           SECTION.
     USE AFTER EXCEPTION PROCEDURE   DKKEPNF.
     MOVE      "DKKEPNF"        TO   ERR-FL-ID.
...
*    欠品データスタート
     ...
     START    DKKEPNF   KEY  >=   KPN-F46   KPN-F47
                                  KPN-F01   KPN-F48
                                  KPN-F07   KPN-F12
                                  KPN-F25
         INVALID   KEY
     ...
     END-START.
*    欠品データ初期ＲＥＡＤ
     PERFORM  DKKEPNF-RD-SEC.
*    センターＣＤ退避
     MOVE     KPN-F07        TO   WK-CENTCD.
*    倉庫退避
     MOVE     KPN-F48        TO   WK-SOKO.
*    部門退避
     MOVE     KPN-F12        TO   WK-BUMON.
*    商品退避
     MOVE     SPACE          TO   WK-SYOHIN.
     MOVE     KPN-F1421      TO   WK-HINM1.
     MOVE     KPN-F1422      TO   WK-HINM2.
*    ＪＡＮＣＤ退避
     MOVE     KPN-F25        TO   WK-JANCD.
*    取引先退避
...
*　　欠品データ読込み　　　　　　  構造_            *
*============================================================*
 DKKEPNF-RD-SEC       SECTION.
     MOVE     "DKKEPNF-RD-SEC"   TO   S-NAME.
*    欠品データ読込み
     READ     DKKEPNF  NEXT  AT  END
*****         IF READ-CNT > ZERO
*****            PERFORM   SYUKEI-WT-SEC
*****         END-IF
              MOVE     "END"      TO       FLG-END
              GO                  TO       DKKEPNF-RD-EXIT
     END-READ.
 READ010.
*    バッチ番号のチェック
...
*    DISPLAY "KPN-F25   = " KPN-F25    UPON CONS.
     IF       WK-BUMON       NOT =     KPN-F12   OR
              WK-SYOHIN      NOT =     KPN-F25
*    項目セット
*        部門
         MOVE      WK-BUMON       TO   PRT-BUMON(TATE)
*        品名
