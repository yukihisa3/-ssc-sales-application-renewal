...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY9541I.COB`
...
****************************************************************
 PROGRAM-ID.            SSY9541I.
*                  流用:SSY9530I.TOKSRLIB
 AUTHOR.                NAV.
 DATE-WRITTEN.          2023/01/18.
 DATE-COMPILED.
 SECURITY.              NONE.
****************************************************************
...
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         01-GS-DSPF
                        ...
                        STATUS         DSP-ST.
*----<< 出荷通知抽出ＤＴ >>--*
     SELECT   YODSNDL3  ASSIGN         DA-01-VI-YODSNDL3
...
*--------------------------------------------------------------*
 01  FLAGS.
     ...
     03  NOTNO-FLG      PIC  9(01)    VALUE  ZERO.
     03  SOKSUMI-FLG    PIC  9(01)    VALUE  ZERO.
     03  HONSUMI-FLG    PIC  9(01)    VALUE  ZERO.
*
*----<< ﾌｱｲﾙ ｽﾃｰﾀｽ >>--*
 01  YODSNDL3-ST       PIC  X(02).
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSY9541I DSPFILE ERROR " DSP-CNTL " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
*    CLOSE    YODSNDL3   HTOKMS    ZSOKMS1    DSPFILE.
     STOP     RUN.
*----<< 出荷通知抽出ＤＴ >>--*
 YODSNDL3-ERR            SECTION.
     USE AFTER     EXCEPTION PROCEDURE      YODSNDL3.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY9541I YODSNDL3 ERROR " YODSNDL3-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY9541I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY9541I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSY95411.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SSY9541I"      TO   PGID.
     MOVE    "FSY95411"      TO   FORM.
...
*
*****DISPLAY "000" UPON CONS.
     MOVE      ZERO           TO   TAISYO-CNT.
     MOVE      SPACE          TO   SND-REC
     INITIALIZE                    SND-REC
     MOVE      JDATE          TO   SND-F11
     MOVE      JTIME          TO   SND-F12
     MOVE      TORICD         TO   SND-F13
     MOVE      WK-SOKCD       TO   SND-F16
     MOVE      NOUDTS         TO   SND-F02
     START     YODSNDL3   KEY  >=  SND-F11  SND-F12
                                   SND-F13  SND-F16
                                   SND-F02
          INVALID   KEY
               MOVE      5    TO   ERR-FLG
...
***************GO   TO   220-GRP01-CHECK-EXIT
     ...
     END-START.
*****DISPLAY "121" UPON CONS.
 221-SONZAI-CHECK-01.
     READ      YODSNDL3
         AT END
                   GO   TO   221-SONZAI-CHECK-EXIT
     END-READ.
******DISPLAY "111" UPON CONS.
*バッチＮＯチェック
     IF ( JDATE     =    SND-F11 ) AND
        ( JTIME     =    SND-F12 ) AND
        ( TORICD    =    SND-F13 )
         CONTINUE
     ELSE
         GO   TO   221-SONZAI-CHECK-EXIT
     END-IF.
******DISPLAY "222" UPON CONS.
*倉庫ＣＤチェック
     IF  SND-F16  NOT =  SOKO
         GO   TO   221-SONZAI-CHECK-EXIT
     END-IF.
******DISPLAY "333" UPON CONS.
*納品日チェック
     IF  SND-F02  >=  NOUDTS
     AND SND-F02  <=  NOUDTE
         CONTINUE
     ELSE
         GO   TO   221-SONZAI-CHECK-EXIT
     END-IF.
******DISPLAY "444" UPON CONS.
*出荷通知番号採番チェック
     IF  SND-F17   =  0
         MOVE      1         TO    NOTNO-FLG
         GO                  TO    221-SONZAI-CHECK-EXIT
     END-IF.
******DISPLAY "555" UPON CONS.
*倉庫未確定チェック
     IF  SND-F18   =  0
         MOVE      1         TO    SOKSUMI-FLG
         GO                  TO    221-SONZAI-CHECK-EXIT
     END-IF.
******DISPLAY "666" UPON CONS.
*本社未確定チェック
     IF  SND-F20   NOT =  0
         MOVE      1         TO    HONSUMI-FLG
         GO                  TO    221-SONZAI-CHECK-EXIT
     END-IF.
*
     ADD           1         TO    TAISYO-CNT.
