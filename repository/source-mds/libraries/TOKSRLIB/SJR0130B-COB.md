...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0130B.COB`
...
*
 ...
 FILE-CONTROL.
*受領データファイル
     SELECT   ARKJYUH  ASSIGN    TO        DA-01-S-ARKJYUH
...
*    受領データ　
******************************************************************
 FD  ARKJYUH           BLOCK CONTAINS 1    RECORDS
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0130B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0130B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
     ...
     MOVE     SPACE     TO        WK-BODY-REC.
     INITIALIZE                   WK-BODY-REC.
*
******************
*システム日付編集*
******************
...
*---          ADD       1            TO    WK-CNT-HEN
*             伝票枚数カウントアップ
              IF   WK-HEAD04         NOT = WK-DENNO
                   ADD  1            TO    WK-DEN-HEN
                   MOVE WK-HEAD04    TO    WK-DENNO
              END-IF
       END-IF.
*    受領
       IF   ( WK-HEAD05 NOT =    "20" ) AND
            ( WK-HEAD05 NOT =    "51" )
*---          レコード件数カウントアップ
*---          ADD       1            TO    WK-CNT-JYR
*             伝票枚数カウントアップ
...
*
     ...
     INITIALIZE                       RUI-REC.
*    計上日(IN:8桁)
     IF  WK-HEAD24   NUMERIC
         MOVE        "2"            TO    LINK-IN-KBN
         MOVE        WK-HEAD24      TO    LINK-IN-YMD8
         CALL        "SKYDTCKB"  USING    LINK-IN-KBN
                                          LINK-IN-YMD6
                                          LINK-IN-YMD8
                                          LINK-OUT-RET
...
*店舗マスタ索引
     ...
     MOVE     WK-HEAD10      TO        TEN-F011.
     PERFORM  HTENMS-READ-SEC.
*商品変換ＴＢＬ索引
     MOVE     PARA-TORI      TO        TBL-F01.
     MOVE     WK-BODY05(4:7) TO        TBL-F02.
     PERFORM  HSHOTBL-READ-SEC.
*原単価
     IF     ( WK-BODY09 NOT NUMERIC ) OR
            ( WK-BODY09 =   ZERO    )
...
*伝票番号
     MOVE     WK-HEAD04      TO        RUI-F04.
*元伝票番号
*    F05 : INIT
*相手伝票区分
     MOVE     WK-HEAD05      TO        RUI-F06.
*店舗コード
     MOVE     WK-HEAD10      TO        RUI-F07.
*計上日
*分類コード
     MOVE     WK-HEAD06      TO        RUI-F09.
*訂正区分（Ｈ）
*    F10 : INIT
*直接納品先コード
*    F11 : INIT
*コメント
     MOVE     WK-HEAD22      TO        RUI-F12.
*行番号
     MOVE     WK-BODY04      TO        RUI-F31.
*赤黒区分（０：黒固定）
     MOVE     "0"            TO        RUI-F32.
*相手商品コード
     MOVE     WK-BODY05(4:7) TO        RUI-F33.
*数量
     MOVE     WK-BODY13      TO        RUI-F34.
*    COMPUTE  RUI-F34   =    XXXXXXXX  /   10.
*原単価
     IF       WK-ERR-GENTANKA  NOT =  "1"
              MOVE WK-BODY09 TO        RUI-F35
*             COMPUTE   RUI-F35    =   XXXXXXXX  /  100
     END-IF.
*原価金額
     MOVE     WK-BODY14      TO        RUI-F36.
*    COMPUTE  RUI-F36   =    XXXXXXXX  /   100.
*商品名カナ１
     MOVE     WK-BODY11        TO      RUI-F37.
*商品名カナ２
     MOVE     WK-BODY12        TO      RUI-F38.
*返品理由
     MOVE     WK-BODY15        TO      RUI-F39.
*訂正区分（Ｍ）
*    F40 : INIT
*売単価
*    F41 : INIT
*自社伝票区分（伝票区分変換マスタで取得の値を設定）
