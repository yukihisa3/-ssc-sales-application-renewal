...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NKE1010I.COB`
...
*    業務名　　　　　　　：　出荷
*    サブシステム　　　　：　出荷検品サブシステム
*    モジュール名　　　　：　出荷データ抽出指示（カインズTC1）
*    作成日／更新日　　　：　2019/06/24
*    作成者／更新者　　　：　NAV
*    処理概要　　　　　　：　画面より、バッチＮｏ、出力倉庫を
*                            入力し、チェック後パラメタに渡
*                            す。
*    更新履歴            ：  2024/03/21 NAV TAKAHASHI
*                            カインズ以外を指定の場合はエラー
*　                          追加
****************************************************************
****************************************************************
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### NKE1010I DSPFILE ERROR " DSP-CNTL " "
...
*----<< 伝票データ >>--*
 SHTDENF-ERR            SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE1010I SHTDENF ERROR " SHTDENF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE1010I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE1010I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FKE10101.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "NKE1010I"      TO   PGID.
     MOVE    "FKE10101"      TO   FORM.
...
*    取引先チェック
     IF  TORICD    NOT =     ZERO
         ...
         IF  TORICD  NOT =  921084
             ...
             MOVE     "R"   TO   EDIT-OPTION OF TORICD
         END-IF
*********2024/03/21 NAV ED カインズは指定出来ない様にする
     END-IF.
*    倉庫コードチェック
     IF     ( SOKO      NOT =     SPACE )
     AND    ( SOKO      NOT =     "00"  )
...
*
     ...
     END-IF.
     MOVE      NOHBI     TO   PARA-NOHBI.
     IF        NOHBI  NOT  NUMERIC
               MOVE  ZERO   TO  PARA-NOHBI
     ELSE
               MOVE  NOHBI  TO  PARA-NOHBI
     END-IF.
*
*    IF        PARA-TORICD = 173
*              MOVE      "1"      TO   PARA-OUTKBN
*    ELSE
*              MOVE      "2"      TO   PARA-OUTKBN
*    END-IF.
*
     MOVE      KUBUN     TO   PARA-KUBUN.
*
     MOVE     99                  TO   GR-NO.
*
 240-PARA-SEC-EXIT.
