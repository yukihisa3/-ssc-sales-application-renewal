...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY0405I.COB`
...
*                                                              *
**履歴**********************************************************
*    2021/02/19  井上　　新規作成（ＮＳＹ０４００Ｉコピー）　　*
*    0000/00/00　　　　　　　　　　　　　　　　　　　　　　　　*
*    0000/00/00　　　　　　　　　　　　　　　　　　　　　　　　*
****************************************************************
...
*
 01  SAV-KEY.
     03  SAV-BKEY.
       05  SAV-BKEY-EDNOCD     PIC  9(05)   VALUE  ZERO.
       05  SAV-BKEY-KBN        PIC  X(02)   VALUE  ZERO.
       05  SAV-BKEY-BUMON      PIC  9(03)   VALUE  ZERO.
     03  SAV-NKEY.
       05  SAV-NKEY-EDNOCD     PIC  9(05)   VALUE  ZERO.
...
****  メッセージ情報  ***
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "NSY0405I".
...
*        合計
         COMPUTE   NIGOK(IXA)  =  DKC-F08  +  DKC-F09  +
                                  DKC-F10  +  DKC-F11
*        最終納品先
         IF        DKC-F18 NOT =  CENTCD
                   MOVE DKC-F18   TO   EDNOCD(IXA)
                   MOVE DKC-F21   TO   EDNONM(IXA)
         END-IF
*
         IF        IXA  =    1
              MOVE DKC-F18        TO   SAV-BKEY-EDNOCD
              MOVE DKC-F07        TO   SAV-BKEY-BUMON
              MOVE DKC-F19        TO   SAV-BKEY-KBN
         END-IF
         IF        IXA  =    14
              MOVE DKC-F18        TO   SAV-NKEY-EDNOCD
...
*
     ...
     START   DDCENTF     KEY  >=  DKC-F01 DKC-F02 DKC-F03
                                  DKC-F04 DKC-F05 DKC-F06
                                  DKC-F18 DKC-F19 DKC-F07
       INVALID      KEY
             MOVE       1         TO   DKC-FLG
       NOT INVALID  KEY
             MOVE       ZERO      TO   DKC-FLG
...
*==========================================================*
 BODY-SUB            SECTION.
     ...
     PERFORM  VARYING  IXB   FROM  1  BY  1
              UNTIL    IXB   >     14
         IF   BUMON(IXB)    IS NOT  NUMERIC
              MOVE   "X"   TO   EDIT-STATUS  OF  KOMONO(IXB)
              MOVE   "X"   TO   EDIT-STATUS  OF  IKEI  (IXB)
              MOVE   "X"   TO   EDIT-STATUS  OF  KESU  (IXB)
              MOVE   "X"   TO   EDIT-STATUS  OF  HOKA  (IXB)
         END-IF
...
*    DISPLAY "SAV-NKEY-EDNOCD="  SAV-NKEY-EDNOCD UPON CONS.
*    DISPLAY "SAV-NKEY-KBN=   "  SAV-NKEY-KBN    UPON CONS.
*    DISPLAY "SAV-NKEY-BUMON= "  SAV-NKEY-BUMON  UPON CONS.
*T
     MOVE    BDATE           TO   DKC-F01.
     MOVE    BTIME           TO   DKC-F02.
     MOVE    TOKCD           TO   DKC-F03.
     ...
     MOVE    NOUHIN          TO   DKC-F05.
     MOVE    CENTCD          TO   DKC-F06.
     MOVE    SAV-NKEY-EDNOCD TO   DKC-F18.
...
*
     ...
     MOVE    NOUHIN          TO   DKC-F05.
     MOVE    CENTCD          TO   DKC-F06.
     MOVE    SAV-BKEY-EDNOCD TO   DKC-F18.
...
*        未入力かどうかNYUMERICで判断
         IF   BUMON(IXA)     IS  NUMERIC    AND
              ...
              IF   INVALID-FLG    =    1
                   DISPLAY NC"納品情報データ異常！" UPON CONS
                   MOVE    4000        TO   PROGRAM-STATUS
                   STOP RUN
*
*                  MOVE      SPACE     TO   DKC-REC
...
************************************************************
 END-SEC                SECTION.
*
     CLOSE    DSPF DDCENTF  ZSOKMS HTOKMS HTENMS.
*
 END-END.
     EXIT.
************************************************************
...
*    倉庫ＣＤチェック
     ...
     MOVE     "NSY0405I"      TO   PGID.
     MOVE     "FSY04051"      TO   FMID.
*
...
*T
     ...
     MOVE    NOUHIN          TO   DKC-F05.
     MOVE    CENTCD          TO   DKC-F06.
     IF    ( EDNOCD(IXA)     NOT  NUMERIC ) OR
           ( EDNOCD(IXA)     =    ZERO    )
             MOVE   CENTCD        TO   DKC-F18
     ELSE
             MOVE   EDNOCD(IXA)   TO   DKC-F18
     END-IF.
     MOVE    HACCD (IXA)     TO   DKC-F19.
     MOVE    BUMON (IXA)     TO   DKC-F07.
     READ    DDCENTF
