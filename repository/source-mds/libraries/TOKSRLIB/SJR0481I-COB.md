...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0481I.COB`
...
*                            計上欄でＹを指定した明細は、計上  *
*                            対象として更新する。　　　　　　  *
*    変更日　　　　　　　：　2022/08/24 NAV TAKAHASHI          *
*    変更内容　　　　　　：　検収日範囲チェック、変更検収日変更*
*    変更日　　　　　　　：　2024/07/18 NAV INOUE              *
*    　　　　　　　　　　　　ＳＵＢ商品名称マスタを参照し、    *
*    　　　　　　　　　　　　表示対象を判定。　　　　　　　    *
*    　　　　　　　　　　：                                    *
****************************************************************
...
*フラグ領域
 01  FLG-AREA.
     ...
     03  COMRHEF-UPD-CNT    PIC  9(07)  VALUE  ZERO.
*↓20240718
     03  SKIP-FLG           PIC  X(04)  VALUE  SPACE.
*↑20240718
*
*ワーク領域
 01  WRK-AREA.
...
*日付ワーク
 01  SYS-DATEA          PIC  9(06).
 01  FILLER             REDEFINES      SYS-DATEA.
     03  SYS-YY         PIC  9(02).
     03  SYS-MM         PIC  9(02).
     03  SYS-DD         PIC  9(02).
...
-          "下さい。".
     ...
     03  ERR-MSG18.
         05  FILLER         PIC  N(30)  VALUE
         NC"更新する場合はＹを入力しＥＮＴＥＲです。".
     03  ERR-MSG19.
         05  FILLER         PIC  N(30)  VALUE
         NC"検収日開始日は必ず指定して下さい。".
     03  ERR-MSG20.
         05  FILLER         PIC  N(30)  VALUE
         NC"検収日は同月内日付範囲で指定して下さい。".
     03  ERR-MSG21.
*A---B-------2---------3---------4---------5---------6---------7E*
         05  FILLER         PIC  N(30)  VALUE
         NC"指定された検収日範囲の場合は、必ず変更検収日を指定し
-          "て下さい".
     03  ERR-MSG22.
         05  FILLER         PIC  N(30)  VALUE
         NC"　".
 ...
 01  ERR-MSG-AREA-R  REDEFINES ERR-MSG-AREA.
     03  ERR-MSG-R          PIC  N(30)  OCCURS 25.
*
 01  FILE-ERR.
...
****************************************************************
 PROCESS-START         SECTION.
     ...
     PERFORM  END-SEC.
     DISPLAY NC"計上対象　更新件数＝" COMRHEF-UPD-CNT UPON CONS.
...
*    当月度、検収日範囲表示
     MOVE   GETUDO          TO       DSP-INIGET.
     MOVE   START-YYMM      TO       DSP-INIKST.
     MOVE   END-YYMM        TO       DSP-INIKED.
*  画面の表示
     MOVE  "SCREEN"         TO  DSP-GRP.
     MOVE  "FJR04811"       TO  DSP-FMT.
     WRITE  DSP-FJR04811.
...
*  項目クリア
     MOVE  SPACE            TO  DSP-FJR04811.
*  ＰＧＩＤ
     MOVE  "SJR0481I"       TO  DSP-PGID.
*  ＦＯＲＭＩＤ
     MOVE  "FJR04811"       TO  DSP-FORMID.
*  システム日付転送
     MOVE  SYS-DATE         TO  DSP-SDATE.
*  システム時間転送
     MOVE  WK-TIME(1:6)     TO  DSP-STIME.
...
*属性初期化
     ...
     MOVE  "M"              TO  EDIT-OPTION OF DSP-DENST.
     MOVE  SPACE            TO  EDIT-CURSOR OF DSP-DENST.
     MOVE  "M"              TO  EDIT-OPTION OF DSP-DENED.
     MOVE  SPACE            TO  EDIT-CURSOR OF DSP-DENED.
     MOVE  "M"              TO  EDIT-OPTION OF DSP-CHGKDT.
     MOVE  SPACE            TO  EDIT-CURSOR OF DSP-CHGKDT.
...
*
     PERFORM VARYING IX FROM 1 BY 1  UNTIL  IX  >  7
        ...
        MOVE  "M"      TO       EDIT-OPTION OF DSP-MKEIJ (IX)
        MOVE  SPACE    TO       EDIT-CURSOR OF DSP-MKEIJ (IX)
...
*検収日開始
     ...
     ELSE
          ...
          MOVE      DSP-KENST  TO       LINK-IN-YMD8
          CALL     "SKYDTCKB"  USING    LINK-IN-KBN
                                        LINK-IN-YMD6
                                        LINK-IN-YMD8
                                        LINK-OUT-RET
          ...
          IF   LINK-OUT-RET   NOT =     ZERO
               ...
               GO TO  HEAD-CHK-EXIT
          ELSE
               MOVE     DSP-KENST  TO   KAISI-YMD
               MOVE     DSP-KENST  TO   SYURYO-YMD
               DIVIDE   KAISI-YY  BY  4
               ...
               END-IF
               MOVE WK-MATUBI(KAISI-MM)  TO  SYURYO-DD
               DISPLAY "KENSYU-HANI = " KAISI-YMD "-" SYURYO-YMD
                        UPON CONS
          END-IF
     END-IF.
*検収日終了
...
**********MOVE  99999999    TO  DSP-KENED
          MOVE  SYURYO-YMD  TO  DSP-KENED
     ELSE
          MOVE     "2"         TO       LINK-IN-KBN
          MOVE      DSP-KENED  TO       LINK-IN-YMD8
          CALL     "SKYDTCKB"  USING    LINK-IN-KBN
                                        LINK-IN-YMD6
                                        LINK-IN-YMD8
                                        LINK-OUT-RET
          ...
          IF   LINK-OUT-RET   NOT =     ZERO
               IF        ERR-FLG   =    ZERO
                         MOVE     17    TO   ERR-FLG
               END-IF
               MOVE     "C"        TO   EDIT-CURSOR OF DSP-KENED
               MOVE     "R"        TO   EDIT-OPTION OF DSP-KENED
               GO TO  HEAD-CHK-EXIT
          ELSE
               IF   KAISI-YMD  <=  DSP-KENED
               AND  SYURYO-YMD >=  DSP-KENED
                    CONTINUE
               ELSE
                    IF  ERR-FLG = ZERO
                        MOVE  20       TO  ERR-FLG
...
*店舗ＣＤ大小チェック
     ...
     END-IF.
*伝票番号　開始
     IF   DSP-DENST  NOT    NUMERIC
     OR   DSP-DENST      =  ZERO
          MOVE  ZERO        TO  DSP-DENST
     END-IF.
*伝票番号　終了
     IF   DSP-DENED  NOT    NUMERIC
     OR   DSP-DENED      =  ZERO
          MOVE  999999999   TO  DSP-DENED
     END-IF.
*伝票番号　大小チェック
     IF   DSP-DENST  >  DSP-DENED
         IF  ERR-FLG = ZERO
             MOVE  4        TO  ERR-FLG
         END-IF
         MOVE  "C"          TO  EDIT-CURSOR OF DSP-DENST
         MOVE  "R"          TO  EDIT-OPTION OF DSP-DENST
         MOVE  "R"          TO  EDIT-OPTION OF DSP-DENED
         GO TO  HEAD-CHK-EXIT
     END-IF.
*変更計上日
     IF   DSP-CHGKDT NOT    NUMERIC
     OR   DSP-CHGKDT     =  ZERO
          MOVE  ZERO        TO  DSP-CHGKDT
     END-IF.
     IF   DSP-CHGKDT NOT =  ZERO
          MOVE     "2"         TO       LINK-IN-KBN
          MOVE      DSP-CHGKDT TO       LINK-IN-YMD8
          CALL     "SKYDTCKB"  USING    LINK-IN-KBN
                                        LINK-IN-YMD6
                                        LINK-IN-YMD8
                                        LINK-OUT-RET
          ...
          IF   LINK-OUT-RET   NOT =     ZERO
               IF        ERR-FLG   =    ZERO
                         MOVE     17    TO   ERR-FLG
               END-IF
               MOVE     "C"        TO   EDIT-CURSOR OF DSP-CHGKDT
               MOVE     "R"        TO   EDIT-OPTION OF DSP-CHGKDT
               GO TO  HEAD-CHK-EXIT
          END-IF
     END-IF.
*    検収日が許可開始日以下の場合、変更検収日は必須とする。
     IF   START-YYMM   >  SYURYO-YMD(1:6)
          IF   DSP-CHGKDT NOT    NUMERIC
          OR   DSP-CHGKDT     =  ZERO
               IF        ERR-FLG   =    ZERO
                         MOVE     21    TO   ERR-FLG
...
*    ワーク作成ＯＰＥＮ
     ...
     MOVE          ZERO     TO  TAISYO-CNT TAISYO-MEISAI.
     PERFORM  MEISAI-WT-SEC  UNTIL  COM-END = "END".
*****IF     TAISYO-CNT  =  ZERO
*****2023/07/28 NAV ST
     IF     TAISYO-MEISAI =  ZERO
            IF  ERR-FLG = ZERO
...
*店舗ＣＤチェック
 ...
 CHK040.
*伝票番号チェック
     IF  DSP-DENST  <=  RHE-F04
     AND DSP-DENED  >=  RHE-F04
         CONTINUE
     ELSE
         GO                      TO   COMRHEF-READ-SEC
     END-IF.
...
*表示対象チェック
     IF  RHE-F05     =  1
         MOVE        RHE-F10     TO   MEI-F011
         MOVE        RHE-F11     TO   MEI-F0121
         MOVE        RHE-F12     TO   MEI-F0122
         MOVE        RHE-F13     TO   MEI-F0123
         PERFORM     SUBMEI-CHK-SEC
         IF          SKIP-FLG  =  "SKIP"
                     GO          TO   COMRHEF-READ-SEC
         END-IF
     ELSE
...
**
     ...
     END-IF.
*
*スクロール前に　画面上の計上区分をワークに更新
     CLOSE        COMRWKF.
     OPEN  I-O    COMRWKF.
     PERFORM MEISAI-UPD-SEC
             VARYING  IX  FROM  1  BY  1  UNTIL  IX  >   7.
     CLOSE        COMRWKF.
     OPEN  INPUT  COMRWKF.
*スクロール
...
*  画面上の計上区分をワークに更新                            *
****************************************************************
 MEISAI-UPD-SEC        SECTION.
*
     MOVE  "MEISAI-UPD-SEC"   TO  S-NAME.
*
 MEISAI-UPD-01.
     MOVE      DSP-TOKCD           TO   RWK-F01.
     MOVE      DSP-MTENCD(IX)      TO   RWK-F03.
     MOVE      DSP-MKENDT(IX)(1:4) TO   RWK-F02(1:4).
     MOVE      DSP-MKENDT(IX)(6:2) TO   RWK-F02(5:2).
     MOVE      DSP-MKENDT(IX)(9:2) TO   RWK-F02(7:2).
     MOVE      DSP-MDENNO(IX)      TO   RWK-F07.
     START     COMRWKF   KEY IS    >=   RWK-F01
                                        RWK-F02
                                        RWK-F03
                                        RWK-F07
        INVALID
               DISPLAY NC"返品一覧ワーク異常！" UPON CONS
...
*
 MEISAI-UPD-02.
     READ  COMRWKF  NEXT
           AT  END  GO  TO  MEISAI-UPD-SEC-EXIT
     END-READ.
*
 MEISAI-UPD-03.
     IF  (  RWK-F01      =  DSP-TOKCD           ) AND
         (  RWK-F03      =  DSP-MTENCD(IX)      ) AND
         (  RWK-F02(1:4) =  DSP-MKENDT(IX)(1:4) ) AND
         (  RWK-F02(5:2) =  DSP-MKENDT(IX)(6:2) ) AND
         (  RWK-F02(7:2) =  DSP-MKENDT(IX)(9:2) ) AND
         (  RWK-F07      =  DSP-MDENNO(IX)      )
            MOVE         DSP-MKEIJ(IX)  TO  RWK-F20
            REWRITE      RWK-REC
            GO           TO  MEISAI-UPD-02
     ELSE
            GO           TO  MEISAI-UPD-SEC-EXIT
     END-IF.
*
 MEISAI-UPD-SEC-EXIT.
     EXIT.
...
*    ＳＵＢ商品名称マスタ検索（表示対象判定）                  *
****************************************************************
 SUBMEI-CHK-SEC         SECTION.
*
     MOVE  "SUBMEI-CHK-SEC" TO  S-NAME.
*
     READ  SUBMEIL1
       INVALID
         MOVE  "SKIP"               TO  SKIP-FLG
         GO                         TO  SUBMEI-CHK-EXIT
     END-READ.
*
     EVALUATE   MEI-F90
         WHEN   "1"    MOVE "    "  TO  SKIP-FLG
         WHEN   "2"    MOVE "    "  TO  SKIP-FLG
         WHEN   "7"    MOVE "    "  TO  SKIP-FLG
         WHEN   OTHER  MOVE "SKIP"  TO  SKIP-FLG
     END-EVALUATE.
*
 SUBMEI-CHK-EXIT.
     EXIT.
...
*
     IF  DSP-MKENDT(IX-GYO) = SPACE *> データ無し行
         GO TO  BODY-CHKC-EXIT
     END-IF.
*
     MOVE  "M"         TO  EDIT-OPTION OF DSP-MKEIJ (IX-GYO).
     MOVE  SPACE       TO  EDIT-CURSOR OF DSP-MKEIJ (IX-GYO).

     IF  DSP-MKEIJ (IX-GYO) = SPACE *> 選択無し行
         GO TO  BODY-CHKC-EXIT
     END-IF.
*
     IF  DSP-MKEIJ (IX-GYO) = "Y"
         CONTINUE
     ELSE
         IF  ERR-FLG = ZERO
             MOVE  9        TO  ERR-FLG
         END-IF
         MOVE  "C"          TO  EDIT-CURSOR OF DSP-MKEIJ (IX-GYO)
         MOVE  "R"          TO  EDIT-OPTION OF DSP-MKEIJ (IX-GYO)
         GO TO  BODY-CHKC-EXIT
     END-IF.
*
     ADD  1   TO  KJ-SEL.
...
*
         IF    CT-SEL   NOT =   ZERO
               PERFORM  PARA-OT-SEC
               CALL "SJR0490I" USING  LINK-JIKKBN
                                      LINK-BUMON
                                      LINK-TANCD
                                      LINK-TOKCD
                                      LINK-TENCD
...
* 実検収日
     MOVE  DSP-CHGKDT       TO  LINK-CHGKDT.
* 店舗ＣＤ
...
*  返品累積ファイル更新                                        *
****************************************************************
 COMRHEF-UPD-SEC          SECTION.
     MOVE  "COMRHEF-UPD-SEC"  TO  S-NAME.
*
 COMRHEF-UPD-01.
     CLOSE     COMRHEF  COMRWKF.
     OPEN      INPUT    COMRWKF.
     OPEN      I-O      COMRHEF.
     MOVE      0            TO  COMRHEF-UPD-CNT.
*
 COMRHEF-UPD-02.
     READ      COMRWKF NEXT
         AT END
               GO           TO  COMRHEF-UPD-EXIT
     END-READ.
*
 COMRHEF-UPD-03.
     IF        RWK-F20      =   " "
               GO           TO  COMRHEF-UPD-02
     END-IF.
*
 COMRHEF-UPD-04.
     MOVE SPACE             TO  RHE-REC.
     INITIALIZE                 RHE-REC.
     MOVE      RWK-F01      TO  RHE-F01.
     MOVE      RWK-F02      TO  RHE-F03.
     MOVE      RWK-F03      TO  RHE-F02.
     MOVE      RWK-F07      TO  RHE-F04.
     MOVE      1            TO  RHE-F05.
*
     START  COMRHEF  KEY IS >=  RHE-F01  RHE-F03  RHE-F02
                                RHE-F04  RHE-F05
            INVALID
               DISPLAY NC"返品累積ファイル異常１" UPON CONS
               MOVE  4010   TO  PROGRAM-STATUS
               STOP  RUN
     END-START.
*
...
*    返品累積データ読込1件目
     READ   COMRHEF  NEXT
        AT END
            DISPLAY NC"返品累積ファイル異常２" UPON CONS
            MOVE  4010   TO  PROGRAM-STATUS
            STOP  RUN
     END-READ.
*
 COMRHEF-UPD-06.
     IF   ( RHE-F01  =  RWK-F01 ) AND
          ( RHE-F02  =  RWK-F03 ) AND
          ( RHE-F03  =  RWK-F02 ) AND
          ( RHE-F04  =  RWK-F07 )
            CONTINUE
     ELSE
            DISPLAY NC"返品累積ファイル異常３" UPON CONS
            DISPLAY NC"取引先＝" RWK-F01 UPON CONS
            DISPLAY NC"　　　　" RHE-F01 UPON CONS
            DISPLAY NC"店舗　＝" RWK-F03 UPON CONS
            DISPLAY NC"　　　　" RHE-F02 UPON CONS
            DISPLAY NC"検収日＝" RWK-F02 UPON CONS
            DISPLAY NC"　　　　" RHE-F03 UPON CONS
            DISPLAY NC"伝票　＝" RWK-F07 UPON CONS
            DISPLAY NC"　　　　" RHE-F04 UPON CONS
            MOVE  4010   TO  PROGRAM-STATUS
            STOP  RUN
     END-IF.
 COMRHEF-UPD-07.
     IF        DSP-CHGKDT   NOT = ZERO
               MOVE       RHE-F07        TO    RHE-F35
               MOVE       DSP-CHGKDT     TO    RHE-F07
     END-IF.
     MOVE      "9"          TO  RHE-F22.
*****コーナンの場合は０に固定　他は９固定 NAV ST
     IF   RHE-F01  =  2363
          MOVE  "0"         TO  RHE-F22
     ELSE
          MOVE  "9"         TO  RHE-F22
     END-IF.
*****コーナンの場合は０に固定　他は９固定 NAV ED
     MOVE      RHE-F23      TO  RHE-F37.
     MOVE      "42"         TO  RHE-F23.
     MOVE      "1"          TO  RHE-F80.
     MOVE      PARA-BUMON   TO  RHE-F81 RHE-F98.
     MOVE      PARA-TANCD   TO  RHE-F82 RHE-F99.
     MOVE      SYS-DATE     TO  RHE-F83 RHE-F96.
     MOVE      WK-TIME(1:6) TO  RHE-F97.
     REWRITE   RHE-REC.
     ADD       1            TO  COMRHEF-UPD-CNT.
*
 COMRHEF-UPD-08.
*    返品累積データ読込2件目以降
     READ   COMRHEF  NEXT
...
****************************************************************
 END-SEC               SECTION.
*ファイル ＣＬＯＳＥ
     CLOSE  DSPFILE  COMRHEF  HTOKMS  HTENMS
*↓20240718
*           DENHENF  JYOKEN1  COMRWKF.
            DENHENF  JYOKEN1  COMRWKF  SUBMEIL1.
*↑20240718
*
 END-EXIT.
     EXIT.
*****************<<  SJR0481I   END PROGRAM  >>******************
