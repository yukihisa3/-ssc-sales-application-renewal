...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SUK0010I.COB`
...
*    サブシステム　　　　：　たねまる連携ＤＴ確認              *
*    モジュール名　　　　：　たねまる連携ＤＴ抽出指示　　　    *
*    作成日／作成者　　　：　2024/02/17 NAV TAKAHASHI          *
*    処理概要　　　　　　：　　　　　　　　　　　　　　　　    *
*                            　　　　　　　　　　　　　　　    *
*    更新日／更新者　　　：　                                  *
*　                                                            *
...
*
 LINKAGE                SECTION.
 01  PARA-OUT-BDATES        PIC   9(08).
 01  PARA-OUT-BDATEE        PIC   9(08).
*
****************************************************************
 PROCEDURE              DIVISION
       USING   PARA-OUT-BDATES     PARA-OUT-BDATEE.
****************************************************************
*--------------------------------------------------------------*
*    LEVEL 0        エラー処理　　　　　　　　　　　　　　　　 *
*--------------------------------------------------------------*
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SUK0010I DSPFILE ERROR " DSP-CNTL " "
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FUK00101.
*
     PERFORM  CLR-GRP001-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SUK0010I"      TO   PGID.
     MOVE    "FUK00101"      TO   FORM.
*
     MOVE     1              TO   GR-NO.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FUK00101"     TO   DSP-FMT.
...
*（日付開始）チェック
***  （日付開始）未入力チェック
     IF       BDATES  NOT NUMERIC
         OR   BDATES  =  ZERO
              MOVE   2       TO   ERR-FLG
              MOVE  "R"      TO   EDIT-OPTION  OF  BDATES
              MOVE  "C"      TO   EDIT-CURSOR  OF  BDATES
              GO             TO   220-GRP01-CHECK-EXIT
     ELSE
***           （日付）論理チェック
              MOVE     "2"            TO   LINK-IN-KBN
              MOVE     ZERO           TO   LINK-IN-YMD6
              MOVE     BDATES         TO   LINK-IN-YMD8
              ...
              END-IF
*
              MOVE  "M"      TO   EDIT-OPTION  OF  BDATES
              MOVE  SPACE    TO   EDIT-CURSOR  OF  BDATES
     END-IF.
 220-GRP01-CHECK-002.
*（日付終了）チェック
***  （日付終了）未入力チェック
     IF       BDATEE  NOT NUMERIC
         OR   BDATEE  =  ZERO
              MOVE   BDATES  TO   BDATEE
     END-IF.
*** （日付）論理チェック
     MOVE     "2"            TO   LINK-IN-KBN.
     MOVE     ZERO           TO   LINK-IN-YMD6.
     MOVE     BDATEE         TO   LINK-IN-YMD8.
     MOVE     ZERO           TO   LINK-OUT-RET.
     MOVE     ZERO           TO   LINK-OUT-YMD8.
     CALL     "SKYDTCKB"     USING   LINK-IN-KBN
                                     LINK-IN-YMD6
     ...
     IF   LINK-OUT-RET   = 9
          MOVE   7       TO   ERR-FLG
          MOVE  "R"      TO   EDIT-OPTION  OF  BDATEE
          MOVE  "C"      TO   EDIT-CURSOR  OF  BDATEE
          GO             TO   220-GRP01-CHECK-EXIT
     ELSE
          MOVE  "M"      TO   EDIT-OPTION  OF  BDATEE
          MOVE  SPACE    TO   EDIT-CURSOR  OF  BDATEE
     END-IF
*
     IF   BDATES  >  BDATEE
          MOVE   6       TO   ERR-FLG
...
*
     MOVE     BDATES              TO   PARA-OUT-BDATES.
     MOVE     BDATEE              TO   PARA-OUT-BDATEE.
*
     MOVE     99                  TO   GR-NO.
*
 240-PARA-SEC-EXIT.
...
*--------------------------------------------------------------*
 CLR-GRP001-RTN           SECTION.
     MOVE     " "            TO   EDIT-CURSOR OF BDATES.
     MOVE     "M"            TO   EDIT-OPTION OF BDATES.
     MOVE     " "            TO   EDIT-CURSOR OF BDATEE.
     MOVE     "M"            TO   EDIT-OPTION OF BDATEE.
 CLR-GRP001-EXIT.
     EXIT.
*--------------------------------------------------------------*
