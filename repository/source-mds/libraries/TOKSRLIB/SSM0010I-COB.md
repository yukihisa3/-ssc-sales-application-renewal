...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSM0010I.COB`
...
*    サブシステム　　　　：　出荷業務　　　　　　　　　　      *
*    業務名　　　　　　　：　出荷業務　出荷明細表　　　　　    *
*    モジュール名　　　　：　出荷明細表発行指示（ケース数算出）*
*    作成日／更新日　　　：　2018/08/21                        *
*    作成者／更新者　　　：　NAV                               *
*    処理概要　　　　　　：　画面より、バッチ_、出力倉庫、納品*
*                            日範囲、店舗範囲、出力区分を入力し*
*                            後続の帳票発行処理に連携する。　  *
****************************************************************
****************************************************************
...
*
 ...
 01  SONZAI-CNT         PIC  9(07)  VALUE  ZERO.
*----<< ﾃﾞｨｽﾌﾟﾚｲ ｺﾝﾄﾛｰﾙ ｴﾘｱ >>-*
 01  GR-NO              PIC  9(02).
 01  WK-GRP             PIC  X(08).
 01  DSP-CNTL.
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSM0010I DSPFILE ERROR " DSP-CNTL " "
...
*----<< 伝票データ >>--*
 SHTDENF-ERR            SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSM0010I SHTDENF ERROR " SHTDENF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSM0010I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSM0010I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSM00101.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SSM0010I"      TO   PGID.
     MOVE    "FSM00101"      TO   FORM.
...
*
     ...
     END-START.
*
 220-SONZAI-CHECK-010.
*
     READ      SHTDENF
               AT END
               GO             TO   220-SONZAI-CHECK-EXIT
     END-READ.
 220-SONZAI-CHECK-020.
*バッチ番号チェック
     IF  JDATE  =  DEN-F46
     OR  JTIME  =  DEN-F47
     OR  TORICD =  DEN-F01
     ...
     ELSE
         GO                   TO   220-SONZAI-CHECK-EXIT
     END-IF.
 220-SONZAI-CHECK-030.
*倉庫ＣＤチェック
     IF  SOKO  NOT =  SPACE
     AND SOKO  NOT =  "00"
         IF   SOKO  NOT =  DEN-F48
              GO              TO   220-SONZAI-CHECK-010
         END-IF
     END-IF.
 220-SONZAI-CHECK-040.
*納品日チェック
     IF  NOUYMD NOT =  ZERO
         IF   NOUYMD  NOT =  DEN-F112
              GO              TO   220-SONZAI-CHECK-010
         END-IF
     END-IF.
 220-SONZAI-CHECK-050.
*店舗ＣＤ範囲
     IF  STENCD  <=  DEN-F07
     AND ETENCD  >=  DEN-F07
         CONTINUE
