...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SBM0173L.COB`
...
*    サブシステム　　　　：　流通ＢＭＳ
*    業務名　　　　　　　：　返品　　　　
*    モジュール名　　　　：　流通ＢＭＳ返品リスト発行（山新）
*    作成日／作成者　　　：　2018/09/10 INOUE
*    処理概要　　　　　　：　発行指示より受け取ったパラメータを
*                        ：　条件に、流通ＢＭＳ返品リスト発行を
*                        ：　する。
*    更新履歴　　　　　　：
*    更新日／更新者　　　：　
...
*
 01  NEW-KEY.
     ...
     02  NEW-DENNO           PIC  9(09).
*
 01  OLD-KEY.
     02  OLD-TENCD           PIC  9(08).
     02  OLD-DENNO           PIC  9(09).
*
 01  HEN-GNOX.
     02  HEN-GNO             PIC  9(02).
     02  FILLER              PIC  X(02).
...
****  見出し行１　　　　　　 ****
 01  MIDASI-1.
     ...
     02  FILLER              PIC  X(08)  VALUE  "SBM0173L".
...
*****                        CHARACTER  TYPE  IS  NIHONGO.
     ...
     02  SYRKBN              PIC  9(02).
     02  FILLER              PIC  X(01)  VALUE  SPACE.
*    02  SYRKBNNM            PIC  N(02)
     02  SYRKBNNM            PIC  N(05)
                             CHARACTER  TYPE  IS  NIHONGO.
     02  FILLER              PIC  X(01)  VALUE  SPACE.
     02  KEIYMD              PIC  9(08).
     02  FILLER              PIC  X(02)  VALUE  SPACE.
...
*        NC"＜伝票合計＞"      CHARACTER  TYPE  IS  YB.
         NC"＜伝票合計＞"      CHARACTER  TYPE  IS  NIHONGO.
...
*        NC"＜店舗合計＞"      CHARACTER  TYPE  IS  YB.
         NC"＜店舗合計＞"      CHARACTER  TYPE  IS  NIHONGO.
* 2023/09/22 UPD START
*    02  FILLER              PIC  X(08)  VALUE  SPACE.
*    02  HENBKSU2            PIC  -,---,--9.
*    02  FILLER              PIC  X(11)  VALUE  SPACE.
...
*        NC"　＜総合計＞"      CHARACTER  TYPE  IS  YB.
         NC"　＜総合計＞"      CHARACTER  TYPE  IS  NIHONGO.
* 2023/09/22 UPD START
*    02  FILLER              PIC  X(08)  VALUE  SPACE.
*    02  HENBKSU3            PIC  -,---,--9.
*    02  FILLER              PIC  X(11)  VALUE  SPACE.
...
******************************************************************
 ...
 PRT-ERR-SEC               SECTION.
     ...
     STOP        RUN.
 HEP-ERR-SEC               SECTION.
     USE         AFTER     EXCEPTION PROCEDURE BMSHE4W.
...
*
     CLOSE       PRTF    BMSHE4W.
*    DISPLAY  "SBM0173L READ =" READ-CNT UPON STA.
*    DISPLAY  "SBM0173L IN   =" IN-CNT   UPON STA.
 END-EXIT.
     EXIT.
**********************************************************
*                    明細データ書き出し　　　　　　　　　*
...
*
*         EVALUATE HEP-F20
*            WHEN  "01"  MOVE NC"定"    TO  DENKUNM
*            WHEN  "03"  MOVE NC"特"    TO  DENKUNM
*            WHEN  OTHER MOVE NC"＊"    TO  DENKUNM
*         END-EVALUATE
          MOVE  HEP-F10           TO        SYRKBN
          EVALUATE HEP-F10
             WHEN    1   MOVE NC"欠品"        TO  SYRKBNNM
             WHEN    2   MOVE NC"品違い"      TO  SYRKBNNM
             WHEN    3   MOVE NC"発注外"      TO  SYRKBNNM
             WHEN    4   MOVE NC"不良品"      TO  SYRKBNNM
             WHEN    5   MOVE NC"破損・汚損"  TO  SYRKBNNM
             WHEN    6   MOVE NC"定番品入替"  TO  SYRKBNNM
             WHEN    7   MOVE NC"季節品入替"  TO  SYRKBNNM
             WHEN   99   MOVE NC"その他"      TO  SYRKBNNM
             WHEN  OTHER MOVE NC"＊＊＊＊＊"  TO  SYRKBNNM
          END-EVALUATE
          MOVE  HEP-F07           TO        KEIYMD
          MOVE  HEP-F03           TO        HEN-DENNOX
          MOVE  HEN-DENNO         TO        DENNO
          MOVE  HEP-F19           TO        BUMON
     END-IF.
*
     ...
     MOVE      HEP-F17            TO        BAITANKA.
     MOVE      HEP-F16            TO        BAIKINGK.
* 2023/09/22 ADD START
     MOVE      HEP-F21            TO        JYOTOYM.
* 2023/09/22 ADD END
*
**************
*帳票書き出し*
