...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NSY1300B.COB`
...
*　　　2014/09/29 高橋/NAV ホーマック用へ改修　　　
*　　　2021/08/19 高橋/NAV 発注日セットをワーク経由に変更
*　
****************************************************************
...
*#2021/08/19 NAV ST
 01  WK-DEN-F111             PIC  9(08)     VALUE  ZERO.
*#2021/08/19 NAV ED
*
 01  WK-AREA.
*システム日付の編集
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "NSY1300B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NSY1300B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*### 1999/11/22 NAV T.T START ###*
*### 2001/05/29 NAV T.T START ###*
*$$$$MOVE     DEN-A27        TO   SYU-F18.      *>ＪＡＮＣＤ
     MOVE     DEN-F25        TO   SYU-F18.      *>ＪＡＮＣＤ
*## 2014/09/29 NAV ST ホーマック用区分追加
*#ＤＣＭ仕入先統合に伴う変更 ST
*$$$$MOVE     DEN-A161(1:1)  TO   SYU-F99(1:1). *>発注種別区分
*$$$$MOVE     DEN-A161(2:1)  TO   SYU-F99(2:1).
*$$$$MOVE     DEN-A163       TO   SYU-F99(3:1). *>形状区分
*$$$$MOVE     DEN-A162       TO   SYU-F99(4:1). *>発注単位区分
     MOVE     DEN-M03        TO   SYU-F99(1:2). *>発注種別区分
     MOVE     DEN-M16        TO   SYU-F99(3:1). *>形状区分
     MOVE     DEN-M15        TO   SYU-F99(4:1). *>発注単位区分
*#2021/08/19 NAV ST
*****MOVE     DEN-F111       TO   SYU-F99(5:8). *>発注日
     MOVE     DEN-F111       TO   WK-DEN-F111.
     MOVE     WK-DEN-F111    TO   SYU-F99(5:8). *>発注日
*#2021/08/19 NAV ED
*#ＤＣＭ仕入先統合に伴う変更 ED
*## 2014/09/29 NAV ED ホーマック用区分追加
*### 2001/05/29 NAV T.T START ###*
     WRITE    SYU-REC.
     ADD      1              TO   WRT-CNT.
*
...
*
     CLOSE     SHTDENLA  SHWSYUKF  SHOTBL1.
*
     STOP      RUN.
*
 END-EXIT.
