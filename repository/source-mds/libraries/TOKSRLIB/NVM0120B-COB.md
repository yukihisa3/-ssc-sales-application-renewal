...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NVM0120B.COB`
...
*    　　　　　　　　　　　　商品変換テーブル取込チェック　　　*
*    　　　　　　　　　　　　※Ｄ３６５連携対応　　　　　　　　*
*    作成日／作成者　　　：　2020/03/27 INOUE                  *
*    処理内容　　　　　　：　商品変換テーブルメンテデータ　　　*
*    　　　　　　　　　　　　について、整合性チェック・件数　　*
*                            カウントを行う。　　　　　　　　　*
*    変更日／作成者　　　：　2020/12/21 NAV ST                 *
*　　変更内容　　　　　　：　直／セ区分→ＯＲＤ区分へ変更　　　*
****************************************************************
...
****************************************************************
 FILE-CONTROL.
*商品変換テーブルメンテナンスＣＳＶ
     SELECT      STBMNTCS    ASSIGN    TO       DA-01-S-STBMNTCS
...
*メッセージ出力領域
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG           PIC  X(08)  VALUE "NVM0120B".
         05  FILLER          PIC  X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER          PIC  X(05)  VALUE " *** ".
         05  END-PG          PIC  X(08)  VALUE "NVM0120B".
         05  FILLER          PIC  X(11)  VALUE
                                         " END   *** ".
     03  MSG-OUT1.
...
****************************************************************
 DECLARATIVES.
 CSV-ERR                     SECTION.
     USE         AFTER       EXCEPTION PROCEDURE STBMNTCS.
...
*
     OPEN        INPUT       STBMNTCS SHOTBL1 TOKMS2
                             MEIMS1   ZSOKMS1 SUBTBLL1 SUBMEIL1.
     OPEN        I-O         STBMNTW1.
*#2020/12/21 NAV ST
     OPEN        INPUT       JYOKEN1.
*#2020/12/21 NAV ED
...
*------------------------------------------*
     ...
     MOVE     CSV-F03        TO   SUB-F01.
     MOVE     CSV-F04        TO   SUB-F02.
...
*【サブ商品名称マスタ存在チェック】
*------------------------------------------*
     MOVE     CSV-F06        TO   SBM-F011.
     MOVE     CSV-F08        TO   SBM-F0121.
     MOVE     CSV-F10        TO   SBM-F0122.
     MOVE     CSV-F12        TO   SBM-F0123.
...
*        予備ＪＡＮＣＤ
          MOVE    CSV-F28       TO        STB-F262
*        予備インストアＣＤ
          MOVE    CSV-F30       TO        STB-F272
*        入数
          MOVE    CSV-F32       TO        STB-F282
*        振分倉庫
          MOVE    CSV-F34       TO        STB-F292
*        直送／セット組区分
          MOVE    CSV-F36       TO        STB-F302
     END-IF.
*
* 【更新モード】
     IF   CSV-F02       =   "Y"
...
*        検品Ｇ
          ...
          MOVE    CSV-F26        TO       STB-F252
*        予備ＪＡＮＣＤ
          MOVE    CSV-F27        TO       STB-F261
          MOVE    CSV-F28        TO       STB-F262
*        予備インストアＣＤ
          MOVE    CSV-F29        TO       STB-F271
          MOVE    CSV-F30        TO       STB-F272
*        入数
          MOVE    CSV-F31        TO       STB-F281
          MOVE    CSV-F32        TO       STB-F282
*        振分倉庫
          MOVE    CSV-F33        TO       STB-F291
          MOVE    CSV-F34        TO       STB-F292
*        直送／セット組区分
          MOVE    CSV-F35        TO       STB-F301
          MOVE    CSV-F36        TO       STB-F302
     END-IF.
*
* 取引先名
...
* D365JANCD/D365商品CD
     IF   SBM-INV-FLG       =   "   "
          MOVE    SBM-D01       TO        STB-F55
          MOVE    SBM-D02       TO        STB-F56
     ELSE
          MOVE    ALL " "       TO        STB-F55
                                          STB-F56
     END-IF.
