...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/NKE0320I.COB`
...
*    業務名　　　　　　　：　返品　　　　　                    *
*    サブシステム　　　　：　検品システム                      *
*    モジュール名　　　　：　返品検品結果確認リスト出力指示    *
*    作成日／作成者　　　：　2019/01/24 INOUE                  *
*    処理概要　　　　　　：　画面より、リスト発行条件を入力。　*
*                            チェック後にパラメタに渡す。　　　*
*                            ・倉庫ＣＤ　　　　　　　　　　    *
*                            ・取込日付（ＦＲＯＭ／ＴＯ）　    *
*                            ・出力区分　　　　　　　　　　    *
...
*
 ...
 01  PARA-OUT-DATEF         PIC   9(08).
 01  PARA-OUT-DATET         PIC   9(08).
 01  PARA-OUT-SYUKBN        PIC   X(01).
*
****************************************************************
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### NKE0320I DSPFILE ERROR " DSP-ST " "
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### NKE0320I ZSOKMS1 ERROR " SOK-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*    ACCEPT   SYS-DATE       FROM DATE.
*    ACCEPT   SYS-TIME       FROM TIME.
*    DISPLAY  "### NKE0320I TANMS1 ERROR " TAN-ST " "
*             SYS-YY "." SYS-MM "." SYS-DD " "
*             SYS-HH ":" SYS-MN ":" SYS-SS " ###"
*                                      UPON CONS.
*    DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FKE03201.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "NKE0320I"      TO   PGID.
     MOVE    "FKE03201"      TO   FORM.
*
...
*
 220-GRP02-CHECK-00.
     IF  DATEF     NOT NUMERIC
         MOVE      ZERO      TO        DATEF
     END-IF.
     IF  DATET     NOT NUMERIC
         MOVE      99999999  TO        DATET
     END-IF.
     IF  DATET     =   ZERO
         MOVE      99999999  TO        DATET
     END-IF.
     ...
     IF  DATEF     NOT =     ZERO
         ...
         IF   LINK-OUT-RET   NOT =     ZERO
              IF        ERR-FLG   =    ZERO
                        MOVE      4    TO   ERR-FLG
              END-IF
              MOVE     "C"        TO   EDIT-CURSOR OF DATEF
              MOVE     "R"        TO   EDIT-OPTION OF DATEF
***           GO   TO   220-GRP02-CHECK-EXIT
         END-IF
     END-IF.
 220-GRP02-CHECK-02.
*    取込日付（ＴＯ）チェック
     IF  DATET     NOT =     99999999
         MOVE     "2"        TO        LINK-IN-KBN
         MOVE      DATET     TO        LINK-IN-YMD8
         CALL     "SKYDTCKB" USING     LINK-IN-KBN
                                       LINK-IN-YMD6
                                       LINK-IN-YMD8
                                       LINK-OUT-RET
         ...
         IF   LINK-OUT-RET   NOT =     ZERO
              IF        ERR-FLG   =    ZERO
                        MOVE      4    TO   ERR-FLG
                        MOVE     "C"   TO   EDIT-CURSOR OF DATET
              END-IF
              MOVE     "R"        TO   EDIT-OPTION OF DATET
              GO   TO   220-GRP02-CHECK-EXIT
         END-IF
     END-IF.
 220-GRP02-CHECK-03.
...
*                                 LINK-OUT-YMD8.
*    IF      LINK-OUT-RET   =    ZERO
*            MOVE LINK-OUT-YMD8  TO   PARA-OUT-DATET
*    ELSE
*            MOVE ZERO           TO   PARA-OUT-DATET
*    END-IF.
*    MOVE     SYS-TIMEW TO   PARA-OUT-TTIME.
*    MOVE     DTKBN     TO   PARA-OUT-DATKBN.
*    MOVE     SYUKBN    TO   PARA-OUT-HAKKBN.
*    MOVE     SYRKBN    TO   PARA-OUT-OUTKBN.
*
     MOVE     SOKCD     TO   PARA-OUT-SOKCD.
     MOVE     DATEF     TO   PARA-OUT-DATEF.
     MOVE     DATET     TO   PARA-OUT-DATET.
     MOVE     SYUKBN    TO   PARA-OUT-SYUKBN.
*
     MOVE     99                  TO   GR-NO.
*
 240-PARA-SEC-EXIT.
...
*--------------------------------------------------------------*
 CLR-HEAD-RTN           SECTION.
     ...
     MOVE     " "            TO   EDIT-CURSOR OF SOKCD
                                  EDIT-CURSOR OF SYUKBN
                                  EDIT-CURSOR OF DATEF
                                  EDIT-CURSOR OF DATET.
     MOVE     "M"            TO   EDIT-OPTION OF SOKCD
                                  EDIT-OPTION OF SYUKBN
                                  EDIT-OPTION OF DATEF
                                  EDIT-OPTION OF DATET.
 CLR-HEAD-EXIT.
     EXIT.
*--------------------------------------------------------------*
