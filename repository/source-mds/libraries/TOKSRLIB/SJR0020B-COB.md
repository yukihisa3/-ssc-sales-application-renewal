...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0020B.COB`
...
*    モジュール名　　　　：　受領データ共通変換                *
*                            　EDICマキバ(受領)
*    作成日　　　　　　　：　2017/07/26 INOUE                  *
*    処理概要　　　　　　：　各社毎の受領返品データを、　　　  *
*                            共通データに変換する。　　　　　　*
*    更新日　　　　　　　：　2020/06/19 NAV TAKAHASHI          *
*    更新内容　　　　　　：　ＪＡＮＣＤセット　編集しセットに　*
*                            変更　（２：１３）セットに変更　　*
****************************************************************
...
*
 ...
 FILE-CONTROL.
*受領データファイル
     SELECT   EDJYURL1  ASSIGN    TO        DA-01-VI-EDJYURL1
...
*    受領データ　
******************************************************************
 FD  EDJYURL1           LABEL RECORD   IS   STANDARD.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0020B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0020B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*店舗マスタ索引
     ...
     MOVE     ONL-F02        TO        TEN-F011.
     PERFORM  HTENMS-READ-SEC.
*商品変換ＴＢＬ索引
     MOVE     PARA-TORI      TO        TBL-F01.
     MOVE     ONL-F307(2:13) TO        TBL-F02.
     PERFORM  HSHOTBL-READ-SEC.
*原単価
     IF     ( ONL-F315 NOT NUMERIC ) OR
            ( ONL-F315 =   ZERO    )
...
*伝票番号
     MOVE     ONL-F04        TO        RUI-F04.
*元伝票番号
*    F05 : INIT
*相手伝票区分
     MOVE     "40"           TO        RUI-F06.
*店舗コード
     MOVE     ONL-F02        TO        RUI-F07.
*計上日
*分類コード
     MOVE     ONL-F226       TO        RUI-F09.
*訂正区分（Ｈ）
*    F10 : INIT
*直接納品先コード
     MOVE     ONL-F210       TO        RUI-F11.
*コメント
*    F12 : INIT
*行番号
     MOVE     ONL-F05        TO        RUI-F31.
*赤黒区分
*　課題未解決にてNOコーディング
     MOVE     ZERO           TO        RUI-F32.
*
*相手商品コード
*# 2020/06/19 NAV ST　ＪＡＮＣＤセット桁数を変更
*****MOVE     ONL-F307       TO        RUI-F33.
     MOVE     ONL-F307(2:13) TO        RUI-F33.
*# 2020/06/19 NAV ED
*数量
*    MOVE     ONL-F325       TO        RUI-F34.
     COMPUTE  RUI-F34   =    ONL-F325  /   10.
*原単価
     IF       WK-ERR-GENTANKA  NOT =  "1"
*             MOVE  ONL-F315 TO        RUI-F35
              COMPUTE   RUI-F35    =   ONL-F315  /  100
     END-IF.
*原価金額
*    MOVE     ONL-F316       TO        RUI-F36.
     COMPUTE  RUI-F36   =    ONL-F316  /   100.
*商品名カナ１
     MOVE     ONL-F311       TO        RUI-F37.
*商品名カナ２
     MOVE     ONL-F312       TO        RUI-F38.
*返品理由
*    F39 : INIT
*訂正区分（Ｍ）
*    F40 : INIT
*売単価
*    MOVE     ONL-F317       TO        RUI-F41.
