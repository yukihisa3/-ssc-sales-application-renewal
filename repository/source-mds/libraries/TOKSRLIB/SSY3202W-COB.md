...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY3202W.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*カインズ伝票データ（共通）
     SELECT   KHSHIRED  ASSIGN    TO        KHSHIRED
...
***  ﾌﾗｸﾞ ｴﾘｱ
 01  FLG-AREA.
     ...
     03  JYOKEN1-INV-FLG          PIC  X(03)     VALUE  ZERO.
...
***  ｶｳﾝﾄ
 01  CNT-AREA.
     ...
     03  MEI-CNT                  PIC  9(02)     VALUE  ZERO.
***  ｺﾞｳｹｲ
 01  GOUKEI.
     ...
     03  HASHTOTAL                PIC  9(02)     VALUE  ZERO.
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
...
***  ﾍﾝｼｭｳ ｴﾘｱ
 ...
 01  WK-HENKAN-1                  PIC  9(08)     VALUE   ZERO.
 01  WK-YYYYMMDD.
     05  WK-YY1                   PIC  9(02).
     05  WK-YY2                   PIC  9(02).
     05  WK-MM                    PIC  9(02).
     05  WK-DD                    PIC  9(02).
*    03  WK-KIRIAGE               PIC  S9(9).
*    03  WK-KIRIAGR   REDEFINES   WK-KIRIAGE
*                                 PIC  X(09).
*    03  WK-AMARI                 PIC  S9(9).
*    03  WK-KHENSYU               PIC  ZZZ9.
***  画面用日付取得
 01  WK-DATE8.
     03  WK-DATE8-YY1             PIC  9(02).
     03  WK-DATE8-YY2             PIC  9(06).
*2013/03/12 NAV ST 納入センター名追加
***  納入センター名ワーク
 01  WK-CENTER-NAME               PIC  X(10)  VALUE  SPACE.
****************************************************************
*    プリントエリア                                            *
****************************************************************
*--------------------------------------------------------------*
...
*
 01  HD3.
     ...
     03  HD3-08                   PIC  999999.
     03  HD3-09                   PIC  9.
     03  FILLER                   PIC  X(01)     VALUE SPACE.
     03  HD3-10                   PIC  999999.
...
*
 01  DT2.
     ...
     03  DT2-05                   PIC  X(02).
     03  FILLER                   PIC  X(01)     VALUE SPACE.
     03  DT2-061                  PIC  ZZZZ9.
     03  DT2-062                  PIC  Z.
     03  FILLER                   PIC  X(02)     VALUE SPACE.
     03  DT2-071                  PIC  ZZZZZ9.
     03  DT2-072                  PIC  Z.
     03  FILLER                   PIC  X(04).
...
*
 01  DT22.
     03  FILLER                   PIC  X(05)     VALUE SPACE.
     03  FIILER                   PIC  X(25)     VALUE
         "*ﾖﾊｸﾆ ﾊ ｷﾆｭｳ ｼﾅｲﾃﾞ ｸﾀﾞｻｲ*".
*--------------------------------------------------------------*
*    テール行                                                  *
*--------------------------------------------------------------*
 01  TL1.
     03  FILLER                   PIC  X(05)     VALUE SPACE.
     03  TL1-00                   PIC  X(10)     VALUE SPACE.
     03  FILLER                   PIC  X(43)     VALUE SPACE.
     03  TL1-01                   PIC  ZZZZ9.
     03  TL1-011                  PIC  Z.
     03  TL1-02                   PIC  ZZ.
     03  TL1-03                   PIC  ZZZZZ9.
     03  TL1-031                  PIC  Z.
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      KHSHIRED.
...
*
     IF       RD-SW     =    0
              ...
              PERFORM   HEAD-WT-SEC
*2013/03/12 NAV ST 納入センター名退避
              MOVE      DEN-A42   TO        WK-CENTER-NAME
**************センター名取得
              MOVE      "23"      TO        JYO-F01
              MOVE      DEN-F278  TO        JYO-F02
              PERFORM  JYOKEN1-READ-SEC
              IF  JYOKEN1-INV-FLG  =  SPACE
                  MOVE JYO-F14     TO       WK-CENTER-NAME
              ELSE
                  MOVE DEN-F278    TO       WK-CENTER-NAME
              END-IF
*2013/03/12 NAV ED 納入センター名退避
              MOVE      1         TO   RD-SW
     END-IF.
*
 FL-READ-EXIT.
...
*伝票　ブレイクチェック
     IF       DEN-F02   NOT =               DENNO1
              IF        L-CNT < 25
                        WRITE  PRT-REC  FROM  DT22  AFTER 2
                        ADD    2        TO    L-CNT
              END-IF
              PERFORM   TAIL-WT-SEC
              PERFORM   HEAD-WT-SEC
...
*2013/03/12 NAV ED 納入センター名退避
     ...
     END-IF.
*明細行カウント
     COMPUTE  MEI-CNT = MEI-CNT + 1.
*
*項目初期化
     MOVE      SPACE              TO        DT1  DT2.
*明細一行目
*    発注区分が”１”の場合は、外注_を出力する。
     IF       DEN-A19  =  "1"
              IF   DEN-A25 NOT =  ZERO
                   MOVE DEN-A25   TO        DT1-01
              END-IF
     END-IF.
*    お取引先品番セット
     IF       DEN-A26   NOT =   ZERO
              MOVE  "NO."         TO        DT1-02
              MOVE   DEN-A26      TO        DT1-03
     END-IF.
*    追加区分
     IF       DEN-A27   =   "0"
              MOVE  "ﾂｲｶﾌｶ"       TO        DT1-04
     END-IF.
***  訂正伝票時処理
     IF        DEN-F15   NOT =     DEN-F50
***            訂正後原価金額
               COMPUTE  DT1-05 =   DEN-F15 * DEN-F172
***            訂正後売価金額
               COMPUTE  DT1-06 =   DEN-F15 * DEN-F173
     END-IF.
*明細２行目
*    商品名称
     MOVE     DEN-A08             TO        DT2-01.
*
*
*
*    商品コード（ＪＡＮコード）
     IF       DEN-A07   =   SPACE
              MOVE      DEN-A06        TO        DT2-02
     ELSE
              MOVE      DEN-A07        TO        DT2-02
     END-IF.
*    色／入数
*****IF       DEN-A33   =    "01"
              IF     DEN-A22(1:1)  NOT =  ZERO
                     MOVE    DEN-A22(1:1)   TO   DT2-03(2:1)
                     MOVE    DEN-A22(2:1)   TO   DT2-03(3:1)
                     MOVE    DEN-A22(3:1)   TO   DT2-03(4:1)
              END-IF.
              IF     DEN-A22(2:1)  NOT =  ZERO
                     MOVE    DEN-A22(2:1)   TO   DT2-03(3:1)
                     MOVE    DEN-A22(3:1)   TO   DT2-03(4:1)
              END-IF.
              IF     DEN-A22(3:1)  NOT =  ZERO
                     MOVE    DEN-A22(3:1)   TO   DT2-03(4:1)
              END-IF.
              IF     DEN-A22(4:1)  NOT =  ZERO
                     IF      DT2-03(4:1) = SPACE
                             MOVE ZERO      TO   DT2-03(4:1)
                     END-IF
                     MOVE  "."              TO   DT2-03(5:1)
                     MOVE    DEN-A22(4:1)   TO   DT2-03(6:1)
              END-IF.
*****ELSE
...
*    サイズ／ケース
*****IF       DEN-A33   =    "01"
              IF     DEN-A23(1:1)  NOT =  ZERO
                     MOVE    DEN-A23(1:1)   TO   DT2-04(1:1)
                     MOVE    DEN-A23(2:1)   TO   DT2-04(2:1)
                     MOVE    DEN-A23(3:1)   TO   DT2-04(3:1)
                     MOVE    DEN-A23(4:1)   TO   DT2-04(4:1)
              END-IF.
              IF     DEN-A23(2:1)  NOT =  ZERO
                     MOVE    DEN-A23(2:1)   TO   DT2-04(2:1)
                     MOVE    DEN-A23(3:1)   TO   DT2-04(3:1)
                     MOVE    DEN-A23(4:1)   TO   DT2-04(4:1)
              END-IF.
              IF     DEN-A23(3:1)  NOT =  ZERO
                     MOVE    DEN-A23(3:1)   TO   DT2-04(3:1)
                     MOVE    DEN-A23(4:1)   TO   DT2-04(4:1)
              END-IF.
              IF     DEN-A23(4:1)  NOT =  ZERO
                     MOVE  DEN-A23(4:1)     TO   DT2-04(4:1)
              END-IF.
*****ELSE
*****         MOVE   DEN-A31      TO        DT2-04
*****END-IF.
*    発注単位
     MOVE     DEN-A24             TO        DT2-05.


***  数量
     MOVE     DEN-F50             TO        WK-SURYO.
***  数量（整数部）
     MOVE     WK-SURYO1           TO        DT2-061.
***  数量（端数部）
     IF       WK-SURYO2 NOT =     ZERO
*****         MOVE     ZERO       TO        DT2-062
*****ELSE
              MOVE     WK-SURYO2  TO        DT2-062
     END-IF.
*
...
***  原単価（整数部）
     MOVE     WK-GENKA1           TO        DT2-08.
***  原単価（端数部）
     IF       WK-GENKA2   NOT =   ZERO
*****         MOVE     ZERO       TO        DT2-09
*****ELSE
              MOVE WK-GENKA2      TO        DT2-09.
*    原価金額
     MOVE     DEN-F521            TO        DT2-10.
*    売価単価
     MOVE     DEN-F513            TO        DT2-11.
*    売価金額
     MOVE     DEN-F522            TO        DT2-12.
*
*ハッシュトータル
     IF       DEN-F15   NOT =     DEN-F50
              COMPUTE   HASHTOTAL   = HASHTOTAL + MEI-CNT
     END-IF.
*原価・売価合計計算
...
*    発注区分印字
     EVALUATE DEN-A19
         WHEN   1
              MOVE "*ｶﾞｲﾁｭｳ*"     TO        HD1-02
         WHEN   2
              MOVE "*ﾅｲﾁｭｳ*"      TO        HD1-02
         WHEN   OTHER
              MOVE  SPACE         TO        HD1-02
     END-EVALUATE.
*ヘッダ２行目
*    社名
     MOVE     DEN-A04             TO        HD2-01.
*    取引先名称
     MOVE     "ｶ)ｻｶﾀﾉﾀﾈﾄｸﾊﾝﾌﾞ"    TO        HD2-02.
*ヘッダ３行目
*    店舗名称
*****MOVE     DEN-F30             TO        HD3-01.
*## 1999/12/08 NAV T.T START ##*
     MOVE     DEN-A34             TO        HD3-01.
*## 1999/12/08 NAV T.T END   ##*
*    社コード
     MOVE     DEN-A17             TO        HD3-02.
*    店舗コード
     MOVE     DEN-F07             TO        HD3-03.
*    分類コード
     MOVE     DEN-A02(1:2)        TO        HD3-04.
*    ハイフン
     MOVE     "-"                 TO        HD3-05.
*    商品区分
     MOVE     DEN-A32             TO        HD3-06.
*    伝票区分
     MOVE     DEN-A01             TO        HD3-07.
*    伝票番号
     MOVE     DEN-F02(3:6)        TO        HD3-08.
*    Ｃ／Ｄ
     MOVE     DEN-F02(9:1)        TO        HD3-09.
*    取引先コード
     MOVE     DEN-F01             TO        HD3-10.
*    発注日
     MOVE     DEN-F111            TO        WK-HENKAN-1.
     MOVE     WK-HENKAN-1         TO        WK-YYYYMMDD.
     MOVE     WK-YY2              TO        HD3-111.
     MOVE     WK-MM               TO        HD3-112.
     MOVE     WK-DD               TO        HD3-113.
*    納品日
     MOVE     DEN-F112            TO        WK-HENKAN-1.
     MOVE     WK-HENKAN-1         TO        WK-YYYYMMDD.
     MOVE     WK-YY2              TO        HD3-121.
     MOVE     WK-MM               TO        HD3-122.
     MOVE     WK-DD               TO        HD3-123.
*
...
***  数量合計（整数部）
     MOVE     WK-SURYO1           TO        TL1-01.
***  数量合計（端数部）
     IF       WK-SURYO2   =       ZERO
              MOVE     ZERO       TO        TL1-011
     ELSE
              MOVE     WK-SURYO2  TO        TL1-011
     END-IF.
***  ハッシュトータル
     MOVE     HASHTOTAL           TO        TL1-02.
*    訂正後数量合計
*****IF       TS-FLG =  1
...
***           訂正後数量合計（整数部）
              MOVE     WK-SURYO1            TO        TL1-03
***           訂正後数量合計（端数部）
              IF       WK-SURYO2  =         ZERO
                       MOVE       ZERO      TO        TL1-031
              ELSE
                       MOVE     WK-SURYO2   TO        TL1-031
              END-IF
     END-IF.
*    原価金額合計
     MOVE   GENKINKEI            TO         TL1-04.
*    売価金額合計
     MOVE   BAIKINKEI            TO         TL1-05.
*2013/03/12 NAV ST 納入センター名追加
***  納入センター名
     MOVE   WK-CENTER-NAME        TO        TL1-00.
*テール２行目
...
***           二重線行
              WRITE     PRT-REC    FROM       DT21    AFTER  0
***           テール２行目
              WRITE     PRT-REC    FROM       TL2     AFTER  3
     ELSE
***           改頁
              MOVE      SPACE      TO         PRT-REC
...
*項目クリア
     MOVE     SPACE               TO   HD1 HD2 HD3
                                       ...
                                       TL1.
*    テスト印字項目セット
     MOVE   ALL "9"               TO   HD3-02   HD3-03   HD3-04
                                       HD3-06   HD3-08   HD3-09
                                       HD3-10   HD3-111  HD3-112
                                       HD3-113  HD3-121  HD3-122
                                       HD3-123  DT1-03   DT2-061
                                       DT2-08   DT2-10   DT2-11
                                       DT2-12   DT2-062
                                       TL1-01   TL1-03
                                       TL1-031  TL1-04   TL1-05
                                       TL1-011.
     MOVE   ALL "*"               TO   HD1-02   HD2-01   HD2-02
                                       HD3-01   HD3-05   HD3-07
                                       DT1-01   DT1-02   DT1-04
                                       DT2-01   DT2-02   DT2-03
                                       DT2-04   DT2-05   DT2-09.
*2013/03/12 NAV ST 納入センター名追加
     MOVE   ALL "*"               TO   TL1-00.
*2013/03/12 NAV ED 納入センター名追加
*ヘッダ部
     WRITE    PRT-REC            FROM  HD1     AFTER  3.
     WRITE    PRT-REC            FROM  HD2     AFTER  2.
     WRITE    PRT-REC            FROM  HD3     AFTER  1.
...
*
     CLOSE    KHSHIRED   PRTFILE    DSPFILE.
     CLOSE    JYOKEN1.
*
 END-EXIT.
     EXIT.
