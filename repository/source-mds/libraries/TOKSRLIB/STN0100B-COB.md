...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/STN0100B.COB`
...
*
 ...
 FILE-CONTROL.
*----<< _卸集計データ >>--*
     SELECT   GKTANAF   ASSIGN    TO        DA-01-VI-GKTANAL2
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       GKT-F02
                        FILE  STATUS   IS   GKT-STATUS.
*----<< _卸結果リストデータ >>----*
     SELECT   GKTANAW  ASSIGN    TO        DA-01-VS-GKTANAW
...
*    _卸集計データ
******************************************************************
 FD  GKTANAF            LABEL     RECORD   IS   STANDARD.
     COPY     GKTANAF   OF        XFDLIB
              JOINING   GKT       PREFIX.
******************************************************************
*    結果リストデータ
******************************************************************
 FD  GKTANAW           LABEL     RECORD   IS   STANDARD.
...
*    ｶｳﾝﾄ
 ...
 01  WK-INV-FLG.
     03  GKTANAW-INV-FLG    PIC  X(03)     VALUE  SPACE.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "STN0100B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "STN0100B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
******************************************************************
 ...
 FILEERR-SEC1           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   GKTANAF.
     MOVE      "GKTANAL2"    TO   AB-FILE.
     MOVE      GKT-STATUS    TO   AB-STS.
...
*T
     ...
     INITIALIZE                     GKT-REC.
     MOVE      PARA-IN-SOKCDS  TO   GKT-F02.
     START     GKTANAF   KEY   >=   GKT-F02
         INVALID   KEY
              MOVE    "END"  TO   END-FLG
              GO             TO   INIT-EXIT
     END-START.
...
*
     MOVE    "GKTANAF-READ-SEC"    TO  S-NAME.
*
     READ     GKTANAF
              AT  END
                  MOVE     "END"    TO  END-FLG
                  GO                TO  GKTANAF-READ-EXIT
              NOT AT END
                  ADD       1       TO  READ-CNT
     END-READ.
*
...
*対象データチェック
 MAIN011.
     IF ( PARA-IN-SOKCDS    <=   GKT-F02 ) AND
        ( PARA-IN-SOKCDE    >=   GKT-F02 )
        CONTINUE
     ELSE
        MOVE  "END"      TO   END-FLG
        GO               TO   MAIN-EXIT
...
*
     DISPLAY   NC"_卸集計データ" "IN  = " READ-CNT  UPON CONS.
