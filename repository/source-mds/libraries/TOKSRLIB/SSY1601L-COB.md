...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SSY1601L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*伝票データ（共通）　　
     SELECT   BMSDENF  ASSIGN    TO         BMSDENF
...
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
     ...
     03  WK-DEN-A22               PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-A23               PIC  9(04)     VALUE  ZERO.
     03  WK-DEN-F3411             PIC  X(22)     VALUE  ZERO.
     03  WK-DEN-F3412             PIC  X(22)     VALUE  ZERO.
 01  SEC-NAME.
...
***  伝票番号
 01  WK-DENPYO                    PIC  X(06).
 01  WK-DENPYO-R  REDEFINES  WK-DENPYO.
     03  WK-DENPYO-H              PIC  9(06).
***  行番号
 01  WK-GYO                       PIC  X(01).
 01  WK-GYO-R     REDEFINES  WK-GYO.
     03  WK-GYO-H                 PIC  9(01).
***  ラベル自由使用欄（直送先名）
 01  WK-JIYU                      PIC  N(60).
 01  WK-JIYU-R    REDEFINES  WK-JIYU.
     03  WK-JIYU-H1               PIC  X(20).
     03  WK-JIYU-H2               PIC  X(100).
***  画面用日付取得
 01  WK-DATE8.
...
*    ＨＩヒロセ以外は読み飛ばし（取引先）
*    PENDING:取引先コード未決定
     IF       DEN-F013 NOT =  31100
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
     MOVE     DEN-F302(1:6)      TO   WK-DENPYO.
     IF       WK-DENPYO-H  NOT =  WK-DEN-NO
              ADD    1            TO      WK-MAI
              MOVE   WK-DENPYO-H  TO      WK-DEN-NO
     END-IF.
*
 DEN-CNT-EXIT.
     EXIT.
...
*
 ...
 READ-010.
     IF       ( SYU-FLG     = 1      )  AND
              ( WK-DENPYO-H NOT = DEN-NO )
              MOVE      "END"     TO        END-FLG
              GO        TO        FL-READ-EXIT.
     IF       DEN-F02       =    MD03
              MOVE      WK-DENPYO-H   TO        DEN-NO
              MOVE      1         TO        SYU-FLG.
 READ-020.
*
     IF       RD-SW     =    0
              MOVE      WK-DENPYO-H   TO   DENNO1
              PERFORM   HEAD-WT-SEC
              MOVE      1         TO   RD-SW
     END-IF.
*
...
*明細１行目
***  商品名（上段）
     MOVE     DEN-F417(1:20)      TO   DT1-01.
***  ＪＡＮコード
     MOVE     DEN-F412(2:13)      TO   DT1-02.
***  訂正伝票時処理
     IF        DEN-F604  NOT = DEN-F453
***            訂正後原価金額
               COMPUTE  DT1-03 =  DEN-F604 * DEN-F446
***            訂正後売価金額
               COMPUTE  DT1-04 =  DEN-F604 * DEN-F448
     END-IF.
*明細２行目
***  商品名（下段）
     MOVE     DEN-F419(1:20)      TO   DT2-01.
***  商品コード
     MOVE     DEN-F413(1:8)       TO   DT2-02.
***  入数
     MOVE     ZERO                TO   DT2-03.
***  ケース
     MOVE     ZERO                TO   DT2-04.
***  数量
     MOVE     DEN-F453            TO        WK-SURYO.
***  数量（整数部）
     MOVE     WK-SURYO1           TO        DT2-051.
***  数量（端数部）
     IF       WK-SURYO2   =      ZERO
              MOVE     ZERO       TO        DT2-052
     ELSE
              MOVE     WK-SURYO2  TO        DT2-052
     END-IF.
*
*    COMPUTE  SURYOKEI  =   SURYOKEI  +  DEN-F50.
*    訂正後数量
     IF       DEN-F604  NOT = DEN-F453
              MOVE      DEN-F604  TO       WK-SURYO
***           訂正後数量（整数部）
...
***           訂正後数量（端数部）
              ...
              END-IF
*
*             COMPUTE  TSSURYOKEI  =  TSSURYOKEI  +  DEN-F604
     END-IF.
***  原単価
     MOVE     DEN-F446            TO        WK-GENKA.
***  原単価（整数部）
     MOVE     WK-GENKA1           TO        DT2-071.
***  原単価（端数部）
     IF       WK-GENKA2   =       ZERO
              MOVE     ZERO       TO        DT2-072
     ELSE
              MOVE WK-GENKA2      TO        DT2-072
     END-IF.
***  原価金額
     MOVE     DEN-F445            TO   DT2-08.
***  売価単価
     MOVE     DEN-F448            TO   DT2-09.
***  売価金額
     MOVE     DEN-F447            TO   DT2-10.
*
*原価・売価合計計算
     COMPUTE  GENKINKEI = GENKINKEI + DEN-F445.
     COMPUTE  BAIKINKEI = BAIKINKEI + DEN-F447.
*訂正原価・売価合計計算
***  訂正有時
     IF       DEN-F604  NOT =   DEN-F453
              COMPUTE   TSGENKEI  = DEN-F604 * DEN-F446 + TSGENKEI
              COMPUTE   TSBAIKEI  = DEN-F604 * DEN-F448 + TSBAIKEI
     ELSE
***  訂正無時
              COMPUTE   TSGENKEI  = TSGENKEI + DEN-F445
              COMPUTE   TSBAIKEI  = TSBAIKEI + DEN-F447
     END-IF.
*
*明細１行目出力　　　
...
*ヘッダ１行目
***  支払条件
*----MOVE     DEN-A24              TO    HD1-01.
***  分割回数
*----MOVE     DEN-A25              TO    HD1-02.
***  ルート
*----MOVE     "*** TA"             TO    HD1-03.
*----MOVE     DEN-A21B             TO    HD1-04.
*----MOVE     " ***"               TO    HD1-05.
***  Ｅ欄
     MOVE     " ** H.I.Hｵﾝﾗｲﾝ ** " TO    HD1-06.
*
*ヘッダ２行目
***  社名
     MOVE   NC"Ｈ．Ｉ．Ｈ"        TO    HD2-01.
***  直送先名称
     MOVE     DEN-F340            TO    WK-DEN-F340.
     MOVE     WK-DEN-F340-X       TO    HD2-02.
*
*ヘッダ２.1行目
***  取引先名（上段）
     MOVE     DEN-F326            TO    HD21-01.
*
*ヘッダ３行目
***  店名
     MOVE     DEN-F311(1:20)      TO    HD3-01.
***  店コード
*----MOVE     DEN-F308(2:2)       TO    HD3-02.
     MOVE     DEN-F308(4:2)       TO    HD3-02.
***  部門コード
     MOVE     DEN-F342(2:2)       TO    HD3-03.
***  伝票区分
     MOVE     DEN-F357            TO    HD3-04.
***  伝票_
     MOVE     WK-DENPYO-H         TO    HD3-05.
***  取引先コード
     MOVE     DEN-F323(2:5)       TO    HD3-06.
***  取引先名（下段）
     MOVE     SPACE               TO    HD3-07.
***  発注日
     MOVE     DEN-F344(3:6)       TO    HD3-08.
***  納品日
     MOVE     DEN-F346(3:6)       TO    HD3-09.
*
*Ｆ欄退避
***  Ｆ欄（島番号）
*    MOVE     DEN-A22             TO   WK-DEN-A22.
***  Ｆ欄（台番）
*    MOVE     DEN-A23             TO   WK-DEN-A23.
*
*Ｌ欄退避
***  Ｌ欄上段（摘要１）
     MOVE     DEN-F341(1:22)      TO   WK-DEN-F3411.
***  Ｌ欄下段（摘要２）
     MOVE     DEN-F341(23:22)     TO   WK-DEN-F3412.
*
...
*ヘッダ部出力
     ...
     WRITE    PRT-REC          FROM    HD21       AFTER    1.
...
*テールクリア
     MOVE     SPACE               TO        TL1     TL3.
     MOVE     ZERO                TO        TL2-01  TL2-02.
*テール１行目
***  原価金額合計
...
*テール２行目
***  Ｆ欄（島番号）
*    MOVE     WK-DEN-A22          TO        TL2-01.
***  Ｆ欄（台番）
*    MOVE     WK-DEN-A23          TO        TL2-02.
***  Ｌ欄上段（摘要１）
     MOVE     WK-DEN-F3411        TO        TL2-03.
*テール２.1目
***  Ｌ欄下段（摘要２）
     MOVE     WK-DEN-F3412        TO        TL21-01.
*テール３行目
...
*?                      END-IF
                   IF  TSGENKEI(I:1)  NOT = ZERO
                       MOVE   1                 TO   ZERO-FLG
                       MOVE   TSGENKEI(I:1)     TO   TL3-01(J:1)
                   ELSE
                       IF      ZERO-FLG   =    1
                               MOVE     ZERO   TO    TL3-01(J:1)
                       END-IF
                   END-IF
                END-PERFORM
              END-IF
***  訂正後売価金額合計
              MOVE      ZERO      TO        ZERO-FLG
              MOVE      SPACE     TO        TL3-02
              IF        TSBAIKEI  =  ZERO
              MOVE      ZERO      TO        TL3-02(17:1)
              ELSE
                PERFORM  VARYING  I FROM  1  BY  1  UNTIL I > 9
                   COMPUTE  J  =  I  *  2
*?                      IF  I  >=  3
...
*?                      END-IF
                   IF  TSBAIKEI(I:1)     NOT = ZERO
                       MOVE   1                 TO   ZERO-FLG
                       MOVE   TSBAIKEI(I:1)     TO   TL3-02(J:1)
                   ELSE
                       IF      ZERO-FLG   =    1
                               MOVE     ZERO   TO    TL3-02(J:1)
                       END-IF
                   END-IF
                END-PERFORM
              END-IF
...
***  テール２.1行目
     WRITE    PRT-REC        FROM      TL21 AFTER   2.
***  テール３行目
...
*項目クリア
     MOVE     SPACE               TO   HD1 HD2 HD3
                                       ...
                                       TL1 TL2.
*テスト印字項目セット
     MOVE   ALL "9"               TO   HD1-04  HD3-02  HD3-03
                                       HD3-04  HD3-05  HD3-06
                                       DT1-02  DT2-03  DT2-04
                                       DT2-051 DT2-071 DT2-08
                                       DT2-09  DT2-10  TL1-01
                                       TL1-02  TL2-01  TL2-02
                                       DT2-052 DT2-072.
     MOVE   ALL "*"               TO   HD1-01  HD1-02
                                       HD2-02
                                       HD3-01  HD3-07  HD3-08
                                       HD3-09  DT1-01  DT2-01
                                       DT2-02  HD1-03  HD1-05
                                       HD1-06  HD21-01 TL2-03
                                       TL21-01.
     MOVE   ALL NC"＊"            TO   HD2-01.
*
*ヘッダ部
     WRITE    PRT-REC          FROM   HD1        AFTER    3.
     WRITE    PRT-REC          FROM   HD2        AFTER    3.
