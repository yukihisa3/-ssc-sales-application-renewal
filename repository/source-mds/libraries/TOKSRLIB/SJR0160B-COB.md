...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSRLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKSRLIB/SJR0160B.COB`
...
*
 ...
 FILE-CONTROL.
*受領データファイル
     SELECT   SNDYJYRF  ASSIGN    TO        DA-01-VI-SNDYJYR1
                        ORGANIZATION   IS   INDEXED
                        ACCESS    MODE IS   SEQUENTIAL
                        RECORD    KEY       ONL-F01  ONL-F07
                                            ONL-F09  ONL-F05
                                            ONL-F08  ONL-F10
...
*    受領データ　
******************************************************************
 FD  SNDYJYRF           LABEL RECORD   IS   STANDARD.
     COPY     SNDYJYRF  OF    XFDLIB
              JOINING   ONL   PREFIX.
******************************************************************
*    店舗マスタ
******************************************************************
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJR0160B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJR0160B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*　　　　　　　メイン処理　　　　　　　　　　　　　　　　　　　*
****************************************************************
 SNDYJYRF-READ-SEC     SECTION.
*
     MOVE    "SNDYJYRF-READ-SEC" TO   S-NAME.
*
     READ     SNDYJYRF
              AT END
              MOVE    "END"      TO    END-FLG
              GO                 TO    SNDYJYRF-READ-EXIT
              NOT AT END
              ADD     1          TO    RD-CNT
     END-READ.
*
...
*バッチ取引先
     MOVE     ONL-F01        TO        COM-F03.
*伝票番号
     MOVE     ONL-F09        TO        COM-F04.
*元伝票番号
*    F05 : INIT
*相手伝票区分
     MOVE     ONL-F07        TO        COM-F06.
*店舗コード
     MOVE     ONL-F05        TO        COM-F07.
*計上日
     MOVE     ONL-F08        TO        COM-F08.
*分類コード
     MOVE     ONL-F11        TO        COM-F09.
*訂正区分（Ｈ）
*    F10 : INIT
*直接納品先コード
*    F11 : INIT
*コメント
     MOVE     ONL-F19        TO        COM-F12.
*行番号
     MOVE     ONL-F10        TO        COM-F31.
*赤黒区分
     MOVE     ZERO           TO        COM-F32.
*相手商品コード
     MOVE     ONL-F12        TO        COM-F33.
*数量
     MOVE     ONL-F17        TO        COM-F34.
*原単価
     MOVE     ONL-F20        TO        COM-F35.
*原価金額
     MOVE     ONL-F21        TO        COM-F36.
*商品名カナ１
     MOVE     ONL-F13        TO        COM-F37.
*商品名カナ２
     MOVE     ONL-F14        TO        COM-F38(1:10).
     MOVE     ONL-F15        TO        COM-F38(12:7).
*返品理由
     MOVE     ONL-F18        TO        COM-F39.
*訂正区分（Ｍ）
*    F40 : INIT
*売単価
*    F41 : INIT
