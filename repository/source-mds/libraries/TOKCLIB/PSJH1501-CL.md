...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PSJH1501.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PSJH1501'  /.PROGRAM-ID./
...
SJH1501B:
    ...
    OVRF      FILE-KENSYUD,TOFILE-SKKENSYU.TOKFLIB
    OVRF      FILE-SIHARAD,TOFILE-SKSIHARA.TOKFLIB
    OVRF      FILE-SHOHIND,TOFILE-SKSHOHIN.TOKFLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-SJH1501B.TOKELIB,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '92'
...
SEKCNT:
    ...
    OVRF      FILE-SKSIHARA,TOFILE-SKSIHARA.TOKFLIB
    OVRF      FILE-SKSHOHIN,TOFILE-SKSHOHIN.TOKFLIB
    CALL      PGM-SEKCNT.TOKELIB
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
