...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/HENPC1.CL`
...
/. ***********************************************************  ./
    PGM  (P1-?CHK)
/.##ﾊﾟﾗﾒﾀ定義##./
    PARA ?CHK    ,STRING*1,OUT,VALUE-' '    /.実行結果./
/.##パラメタ定義##./
...
/.##ワーク定義##./
    ...
    VAR  ?PGMID    ,STRING*8,VALUE-'HENPC1 '  /.PROGRAM-ID./
...
/.##受信ﾌｧｲﾙノ編集##./
    ...
    IF   ?JRL = '0264'  THEN
         ...
         CALL SJU2040B.TOKELIB,PARA-(?HIDUKE,?JIKAN,?TOKCD)
         IF  @PGMEC ^= 0 THEN
             ?KEKA  :=  '70'
             GOTO  ABEND
         END
...
/.##ﾌﾟﾛｸﾞﾗﾑ正常終了##./
RTN:
    CALL SCVMSG.TOKELIB,PARA-('ｾｲｼﾞｮｳ ｼｭｳﾘｮｳ HENPC')
   /.##結果更新##./
/.  DEFLIBL TOKELIB/TOKFLIB                                     ./
/.  ?KEKA := '04'                                               ./
...
ABEND:
    CALL SCVMSG.TOKELIB,PARA-('ｲｼﾞｮｳ ｼｭｳﾘｮｳ  HENPC')
