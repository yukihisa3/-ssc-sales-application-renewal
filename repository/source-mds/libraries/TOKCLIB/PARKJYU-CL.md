...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PARKJYU.CL`
...
/. *     サカタのタネ　特販システム                           * ./
/. *     オンライン手動受信システム　                         * ./
/. *     取引先：アークランドサカモト（受領データ）           * ./
/. *     作成者：NAV T.TAKAHASHI                              * ./
/. *     作成日：02/01/08                     ID:PARKJYU      * ./
/. ************************************************************ ./
    PGM
    VAR       ?PGMEC    ,INTEGER
    ...
    VAR       ?MSG      ,STRING*13
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PARKJYU '
    VAR       ?STEP     ,STRING*8
/.ライブラリリストの登録                                        ./
...
KENJYUSN:
           ...
           END
/.## GP6000へﾃﾞｰﾀ転送 ##./
    SNDFILE COM-KGPATH01.KGNODE01,SFILE-CVCSG001.CVCSLBI,
            DFILE-'ARKJYU',RL-256,BF-1
/.## GP6000-JOB起動   ##./
/.  SNDRSJJ COM-KGPATH01.KGNODE01,DFILE-'HOMACJYU' ./
/.  時間待ち合わせ                                              ./
KWAIT:
    CALL   TWAIT3
...
KENTYUSY:
    ...
    OVRF      FILE-CVCSCARD,TOFILE-ARKJYU.CVCSLBI
