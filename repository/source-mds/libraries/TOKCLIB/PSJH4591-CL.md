...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PSJH4591.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PSJH4591'  /.PROGRAM-ID./
...
SCV0075B:
    ...
    CALL      PGM-SCV1002B.OSKELIB,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
DATABACK:
    ...
    IF   ?JRL = '0128'  THEN
         ...
         CALL SJU2010B.TOKOBJ,PARA-(?HIDUKE,?JIKAN,?TOKCD)
         IF  @PGMEC ^= 0 THEN
             ?KEKA  :=  '70'
             GOTO  ABEND
         END
    ...
    IF   ?JRL = '0256'  THEN
         ...
         CALL SJU2020B.TOKOBJ,PARA-(?HIDUKE,?JIKAN,?TOKCD)
         IF  @PGMEC ^= 0 THEN
             ?KEKA  :=  '70'
             GOTO  ABEND
         END
    ...
    IF   ?JRL = '2048'  THEN
         ...
         CALL SJU2030B.TOKOBJ,PARA-(?HIDUKE,?JIKAN,?TOKCD)
         IF  @PGMEC ^= 0 THEN
             ?KEKA  :=  '70'
             GOTO  ABEND
         END
...
SJH4591B:
    ...
    CALL      PGM-SJH4591B.OSKELIB,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '92'
