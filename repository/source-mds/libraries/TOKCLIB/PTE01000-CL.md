...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PTE01000.CL`
...
STE0110B:
    ...
    CALL      PGM-STE0110B.TOKELIB,PARA-(?SOKCD,?TORICD,?SDEN,?EDEN)
    IF        @PGMEC    ^=   0    THEN
              GOTO   ABEND
    END
/.##出力順毎ＰＧ振分##./
...
STE0120L:
    ...
    CALL      PGM-STE0120L.TOKELIB,PARA-(?KBN)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
STE0130L:
    ...
    OVRF      FILE-SHWHACF,TOFILE-?TEGHACF
    CALL      PGM-STE0130L.TOKELIB,PARA-(?KBN)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
