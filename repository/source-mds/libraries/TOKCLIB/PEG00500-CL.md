...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PEG00500.CL`
...
SEG0060B:
    ...
    OVRF      FILE-JHMTJSWK,TOFILE-TOJITUF.TOKFLIB
    CALL      PGM-SEG0060B.TOKELIB
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
SEG0070B:
    ...
    OVRF      FILE-JHMKENWK,TOFILE-JYUSINF.TOKFLIB
    CALL      PGM-SEG0070B.TOKELIB
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
SJH0013L:
    ...
    CALL      PGM-SJH0013L.TOKELIB,PARA-(?P1,?P2,?P3)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
