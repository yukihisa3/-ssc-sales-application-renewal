...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PSI04100.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI04100'
    VAR       ?STEP     ,STRING*8
...
/.##ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ##./
    ?SYORINM1 :=  '箱根登山興業殿　　請求データ消込処理'

    ?OPR1  :=  '＃＃　箱根登山興業殿　　請求データ消込処理　＃＃'
...
SSI0410B:
    ...
    OVRF      FILE-SITGKFG,TOFILE-SITGKFG.TOKFLIB
    OVRF      FILE-SETGKFK,TOFILE-SETGKFK1.TOKFLIB
    CALL      PGM-SSI0410B.TOKELIB,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
RTN:
    ...
    ?SYORINM2 := 'ビーバートザンの請求消込が正常終了しました。　　'
    ?SYORINM3 := '件数を確認して下さい。　　　　　　　　　　　　　'
...
ABEND:
    ...
    OVRPRTF FILE-PRTF,TOFILE-PRTF.TOKELIB,MEDLIB-TOKELIBO
                /.１２３４５６７８９０１２３４５６７８９０１２３４'./
    ?SYORINM2 := 'ビーバートザンの請求消込が異常終了しました。　　'
