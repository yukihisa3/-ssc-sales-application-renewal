...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PSI02500.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI02500'
    VAR       ?STEP     ,STRING*8
...
SSI0250B:
    ...
    OVRF      FILE-SSIHARAD,TOFILE-SITGKFE.TOKFLIB
    OVRF      FILE-SEIGKF21,TOFILE-SETGKFG1.TOKFLIB
    CALL      PGM-SSI0250B.TOKELIB,PARA-(?SYORIKN1)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
