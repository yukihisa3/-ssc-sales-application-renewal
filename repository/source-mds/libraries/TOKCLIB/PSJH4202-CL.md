...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PSJH4202.CL`
...
SJH4202B:
    ...
    CALL      PGM-SJH4202B.OSKELIB,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '92'
              GOTO ABEND
...
SJT0010B:
    ...
    CALL      PGM-SJT0010B.OSKELIB,PARA-(?HIDUKE,?JIKAN,?LINE,?YUSEN,
                                         ?CHK)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '93'
...
SJT0180B:
    ...
    CALL      PGM-SJT0180B.OSKELIB,PARA-(?HIDUKE,?JIKAN,?TOKCD)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := '95'
              GOTO ABEND
...
PCHK:
    ...
    IF  ?CHK  =  '1'  THEN
        ...
        CALL    PGM-SJT9996L.OSKELIB,PARA-(?HIDUKE,?JIKAN,?TOKCD)
        IF   @PGMEC ^= 0 THEN
         GOTO      ABEND
        END
    ELSE
