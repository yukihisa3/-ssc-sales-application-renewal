...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PKY02010.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PKY02010'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
/.##当日エラーデータ（売上二重計上）バックアップ##./
DATACPY:

    ?STEP :=   %LAST(LABEL)
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'

    SETPF FILE-SHTDENWK.TOKFLIB,TOFILE-SHTERRF.TOKFLIB,
          ACTCHK-@NO
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
SKY0201B:
    ...
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
                                           /.##各種ﾌｧｲﾙ置換え##./
    OVRF      FILE-SHTDENLD,TOFILE-SHTDENLD.TOKFLIB
