...
**種別**: JCL  
**ライブラリ**: TOKCLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLIB/PSV00990.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSV00990'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
CSV0099B:
    ...
    OVRF      FILE-CSVDATA,TOFILE-CSVDATA1.TOKWLIB
    OVRF      FILE-HACYUPRT,TOFILE-HACYUPRT.TOKFLIB
    CALL      PGM-CSV0099B.TOKELIB
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
PFIMPORT:
    ...
    FIMPORT FILE-CSVDATA1.TOKWLIB,PARA-CSVDATA2,UNIT-1
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
