...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PKE10100.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?FILNMH1  ,STRING*6,VALUE-'SNDCZD'      /.ﾌｧｲﾙ名ﾃﾞﾌｫﾙﾄ./
    VAR ?FILNMK1  ,STRING*6,VALUE-'SNDCZK'      /.ﾌｧｲﾙ名ﾃﾞﾌｫﾙﾄ./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PKE10100'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
NKE1010I:
    ...
    CALL      PGM-NKE1010I.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4,?P5,?P6,
                                          ?P8)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NKE1020B:
    ...
    OVRF      FILE-JYOKEN1,TOFILE-JYOKEN1.TOKFLIB
    OVRF      FILE-SNDCZDXX,TOFILE-?SNDDENF
    OVRF      FILE-SNDCZKXX,TOFILE-?SNDDEKF
    CALL      PGM-NKE1020B.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4,?P6,?P8,
                                          ?KENSU)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
PFIMPORT:
    ...
    FIMPORT FILE-?SNDDENF,PARA-KENPIN,UNIT-15
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
          IF  ?PGMEC    ^=   0    THEN
              GOTO ABEND END
    FIMPORT FILE-?SNDDEKF,PARA-KENPIN,UNIT-16
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
          IF  ?PGMEC    ^=   0    THEN
              GOTO ABEND END
