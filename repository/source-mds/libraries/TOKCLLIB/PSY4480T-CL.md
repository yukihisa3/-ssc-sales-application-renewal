...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY4480T.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY44804'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
/.-----------------------------------------------------------./
    ...
    DEFLIBL BMSFLIB/TOKELIB/TOKFLIB/TOKKLIB/TOKSOLIB

/.##ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ##./
    ?PGNM :=  '流通ＢＭＳ出荷ＤＴ作成（ムサシ）'
    ?SYORINM1 :=  '流通ＢＭＳ出荷ＤＴ作成（ムサシ）'
...
STEP051:
    ...
    CALL SBM9061T.TOKSOLIB,PARA-(?SITEI,?SYURUI,
                                 ?JDATE,?JTIME,?TORICD,?SOKO,
                                 ?NOUDT)
    IF      @PGMEC    ^=   0    THEN
       IF      @PGMEC    =  4010    THEN
...
STEP060:
    ...
    OVRF FILE-BMSSYKL3,TOFILE-BMSSYKL3.BMSFLIB
    CALL SSY4488B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,?SOKO,
