...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY45340.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY45340'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY4532B:
    ...
    OVRF      FILE-BMSSHIW,TOFILE-BMSSIHW.BMSFLIB
    CALL      PGM-SSY4532B.TOKSOLIB,PARA-(?TORICD,?SIMEBI)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細ワーク作成'
              GOTO ABEND
    END
...
SSY4534V:
    ...
    OVRF      FILE-BMSSIHC,TOFILE-BMSSIHC.BMSFLIB
    CALL      PGM-SSY4534V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細データＣＳＶ出力'
              GOTO ABEND
    END
...
PCNVDF1:
    ...
    CNVDF FILE-BMSSIHC.BMSFLIB,
          PATH-'//HGNAS/HANZMAN_BMSSIHAF.CSV',MODE-@EXP,
          ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細データＣＳＶ転送'
              GOTO ABEND
    END
