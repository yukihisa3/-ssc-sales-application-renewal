...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNJH375C.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PNJH375C'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP010:
    ...
    ?OPR2  :=  '資材データの受信処理は完了していますか？'
    ?OPR3  :=  '完了している場合、変換処理を実行して下さい。'
    ?OPR4  :=  '＜ナフコ資材取引先用（２８４２０２）＞'
...
STEP030:
  ...
  /. ↓S2245053 ./
    SNDMSG  'ASSIGN NSHACSF.ONLBLIB',TO-XCTL
    ASSIGN  FILE-NSHACSF.ONLBLIB!@XCL
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【排他制御ファイルロック】'
                    GOTO ABEND
    END

    SNDMSG  'DLTFILE NSHACBK.ONLBLIB',TO-XCTL
    DLTFILE NSHACBK.ONLBLIB

    SNDMSG  'CPYFILE NFHACSF-NFHACBK',TO-XCTL
    CPYFILE  FILE-NSHACSF.ONLBLIB,TOFILE-NSHACBK.ONLBLIB,CRTFILE-@YES
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【発注データ退避】'
                    GOTO ABEND
    END
...
STEP070:
    ...
    SNDMSG '######################################',TO-XCTL
    SNDMSG '## ナフコ（資材）　　　　　変換開始 ##',TO-XCTL
    SNDMSG '######################################',TO-XCTL
    CALL PNJH375D.TOKCOLIB,PARA-(?HIDUKE,?JIKAN,?PGCHK1)
    IF   ?PGCHK1  = '1'    THEN
         SNDMSG '######################################',TO-XCTL
         SNDMSG '## ナフコ（資材）　　　　変換エラー ##',TO-XCTL
         SNDMSG '######################################',TO-XCTL
         ?KEKA4 := '【ナフコ受信データ変換処理】'
         GOTO ABEND
    END
