...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01002.CL`
...
/. *   JOB-ID      :    PBM01002                             *  ./
/. *   JOB-NAME    :    流通BMS受領書発行 VER.2(ｾｷﾁｭｰ)       *  ./
/. *               :      店舗（１：５）                     *  ./
/. *               :      伝番（１：９）                     *  ./
/. *               :      行番（１：２）                     *  ./
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01002'
    VAR ?STEP     ,STRING*8
...
SBM0105B:
    ...
    OVRF     FILE-BMSJYRW,TOFILE-BMSJYRW.BMSFLIB
    CALL     PGM-SBM0106B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0112L:
    ...
    OVRF      FILE-BMSJRSW1,TOFILE-BMSJRSW1.BMSFLIB
    CALL      PGM-SBM0112L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SBM0121V:
    ...
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJRSC,TOFILE-BMSJRSC.BMSFLIB
    OVRF      FILE-BMSJRSW1,TOFILE-BMSJRSW1.BMSFLIB
    CALL      PGM-SBM0121V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力'
              GOTO ABEND
    END
...
/.  CNVDF FILE-BMSJYRC.BMSFLIB,
    PATH-'/HGNAS/BMSJYRC.CSV',MODE-@EXP,ADD-@NO,
    ACTCHK-@NO
./  CNVDF FILE-BMSJRSW.BMSFLIB,PATH-'/HGNAS/BMSJRSW.CSV',
