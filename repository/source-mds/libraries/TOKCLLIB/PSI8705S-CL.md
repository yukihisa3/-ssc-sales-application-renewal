...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSI8705S.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI8705S'
    VAR       ?STEP     ,STRING*8
...
/.##ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ##./
    ?SYORINM1 :=  'ＤＣＭ請求データ消込処理'

    ?OPR1  :=  '＃＃　ＤＣＭＪＡＰＡＮ殿　請求データ消込処理　＃＃'
...
SSI87051:
    ...
    CALL      PGM-SSI8705S.TOKSOLIB,PARA-(?SYORIKN1,?SYORIKN2)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
...
SSI87052:
    ...
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    CALL      PGM-SSI8705S.TOKSOLIB,PARA-(?SYORIKN3,?SYORIKN4)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
RTN:
    ...
    ?SYORINM2 := 'ＤＣＭ請求消込処理が正常終了しました。　　　　　'
    ?SYORINM3 := '資材→１：削除２：エラー　植物→３：削除４：エラ'
