...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSI87060.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSI87060'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP025:
    ...
    CLRFILE FILE-DCMSIHD1.TOKKLIB
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ（資材）削除】'
                    GOTO ABEND
    END

    CLRFILE FILE-DCMSIHD2.TOKKLIB
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ（植物）削除】'
                    GOTO ABEND
    END
...
STEP030:
    ...
    FEXPORT FILE-DCMSIHD1.TOKKLIB,MODE-@ADD,PARA-DCMJAPAN,
            UNIT-4
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（資材）】'
                    GOTO ABEND
...
STEP035:
    ...
    FEXPORT FILE-DCMSIHD2.TOKKLIB,MODE-@ADD,PARA-DCMJAPAN,
            UNIT-12
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データ取込処理（植物）】'
                    GOTO ABEND
...
STEP040:
    ...
    CALL SSI8706B.TOKSOLIB,PARA-(?SYORIKN1,?SYORIKN2,?SYORIKN3)
    IF      @PGMEC    ^=   0    THEN
            ?KEKA4 := '【ＪＥＤＩＣＯＳデータ変換】'
                    GOTO ABEND
    END
