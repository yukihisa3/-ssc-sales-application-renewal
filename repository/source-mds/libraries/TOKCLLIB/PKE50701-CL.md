...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PKE50701.CL`
...
/.↓2016/04/26./
    ...
    VAR ?CHUKBN   ,STRING*1,VALUE-'4'            /.抽出区分./
    VAR ?KENSU01  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU02  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU03  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU04  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU05  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU06  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU07  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU08  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU09  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU10  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU11  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU12  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU13  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU14  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU15  ,STRING*8,VALUE-'00000000'     /.抽出件数./
    VAR ?KENSU16  ,STRING*8,VALUE-'00000000'     /.抽出件数./
/.↑2016/04/26./
/.##ﾌﾟﾛｸﾞﾗﾑ開始ﾒｯｾｰｼﾞ##./
    ?MSGX :=  '***   '  && ?PGMID  &&   ' START  ***'
    SNDMSG    ?MSGX,TO-XCTL
...
STEP020:
    ...
    CALL      PGM-SKE5080B.TOKELIBO,PARA-(?P6,?KENSU06)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4  :=  '【店舗マスタ抽出】'
              GOTO ABEND
    END
...
STEP030:
    ...
    CALL      PGM-SKE5141B.TOKELIBO,PARA-(?KENSU09)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4  :=  '【商品送信件数計算】'
              GOTO ABEND
    END
...
STEP040:
    ...
    CALL      PGM-NKE5110B.TOKSOLIB,PARA-(?P6,?KENSU11)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4  :=  '【ロケーションマスタ抽出】'
              GOTO ABEND
    END
...
STEP050:
    ...
    CALL      PGM-SKE5100B.TOKELIBO,PARA-(?P6,?KENSU10)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4  :=  '【検品Ｇマスタ抽出】'
              GOTO ABEND
    END
...
STEP060:
    ...
    CALL      PGM-SKE5090B.TOKELIBO,PARA-(?P6,?KENSU08)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4  :=  '【倉庫別検品取引先マスタ抽出】'
              GOTO ABEND
    END
...
STEP070:
    ...
    CALL      PGM-SKN0030L.TOKELIBO,PARA-(?BUMON,
                                          ?TANTO,
                                          ?CHUKBN,
                                          ?P6,
                                          ?KENSU01,
