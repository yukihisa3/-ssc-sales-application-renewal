...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PVD04500.CL`
...
/.###ﾜｰｸｴﾘｱ定義###./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PVD04500'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
ASSIGN:
    ...
    ASSIGN FILE-DNSFILF.D365DLIB!@XCL
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '排他制御ファイルロック'
...
NVD0450B:
    ...
    CALL PGM-NVD0450B.TOKSOLIB,PARA-(?OUTCNT)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '移動依頼実績データ作成'
...
RTN:
    ...
    RELEASE FILE-DNSFILF.D365DLIB!@XCL
...
ABEND1:
    ...
    ?KEKA1 :=  '移動依頼実績データ作成　排他エラー！！'
    ?KEKA2 :=  '　他端末で移動依頼実績関係の処理を'
    ?KEKA3 :=  '　実行です。確認後、再度実行して下さい'
