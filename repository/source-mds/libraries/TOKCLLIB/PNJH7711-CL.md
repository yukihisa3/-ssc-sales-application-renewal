...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNJH7711.CL`
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH7711'  /.PROGRAM-ID./
...
/.##ﾌﾟﾛｸﾞﾗﾑｶｲｼﾒｯｾｰｼﾞ##./
    ...
    SNDMSG MSG-'＊　データ変換開始　　＊',TO-XCTL.@ORGPROF,JLOG-@YES
    SNDMSG MSG-'＊＊＊＊＊＊＊＊＊＊＊＊',TO-XCTL.@ORGPROF,JLOG-@YES

    DEFLIBL TOKELIB/TOKFLIB/TOKELIBO/TOKJLIB/TOKKLIB/TOKSOLIB
...
NCV7710B:
    ...
    CALL      PGM-NCV7710B.TOKSOLIB,PARA-(?HIDUKE,?JIKAN,?TOKCD,?LINE,
...
NJH7710B:
    ...
    OVRF      FILE-RCJOHOL1,TOFILE-RCJOHOL1.TOKKLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-NJH7710B.TOKSOLIB,PARA-(?PARA)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
...
ABEND:
   ...
   /.##ｴﾗｰ結果更新##./
    ...
    CALL PGM-NCV0090B.TOKELIB,PARA-(?HIDUKE,?JIKAN,?TOKCD,?KEKA)
   /.##ｴﾗｰﾘｽﾄ発行##./
