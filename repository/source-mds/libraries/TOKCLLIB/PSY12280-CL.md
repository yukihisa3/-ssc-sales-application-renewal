...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY12280.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY12280'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY1229B:
    ...
    OVRF      FILE-KNCASEDT,TOFILE-KNCASEDT.TOKDTLIB
    CALL      PGM-SSY1229B.TOKSOLIB,
              PARA-(?BUMON,?TANCD,?JDATE,?JTIME,?JTORI,?OSOKCD,
                    ?CENTERF,?CENTERT,?ROUTEF,?ROUTET,
                    ?BUMONF,?BUMONT,?HDATE,?NDATE,?TANAF,?TANAT)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ケースＰＤラベル発行データ作成'
              GOTO ABEND
    END
...
FIMPORT1:
    ...
    FIMPORT   FILE-KNCASEDT.TOKDTLIB,
              TYPE-@FILE,
              PARA-KONANLBL,
              UNIT-2,
              OPR-@NO
