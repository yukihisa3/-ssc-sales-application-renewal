...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY4480A.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY4480A'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
/.##ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ##./
    ?PGNM :=  '流通ＢＭＳ出荷ＤＴ作成（セキチュー）'
    ?SYORINM1 :=  '流通ＢＭＳ出荷ＤＴ作成（セキチュー）'
...
STEP050:
    ...
    CALL  SBM0060I.TOKELIBO,PARA-(?BUMON,?TANCD,?SOKCD,?DSOKCD,
                                  ?JDATE,?JTIME,?TORICD,?SOKO,
                                  ?NOUDT,?BMNCD,?TNCD)
    IF      @PGMEC    ^=   0    THEN
       IF      @PGMEC    =  4010    THEN
...
STEP051:
    ...
    CALL SBM9061B.TOKSOLIB,PARA-(?SITEI,?SYURUI,
                                 ?JDATE,?JTIME,?TORICD,?SOKO,
                                 ?NOUDT)
    IF      @PGMEC    ^=   0    THEN
       IF      @PGMEC    =  4010    THEN
...
STEP060:
    ...
    OVRF FILE-BMSSYKL3,TOFILE-BMSSYKL3.BMSFLIB
    CALL SSY4482B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,?SOKO,
                                 ?NOUDT,?BMNCD,?TNCD,?SYORIKN1)
    IF      @PGMEC    ^=   0    THEN
       IF      @PGMEC    =  4010    THEN
               GOTO RTN
       ELSE
...
RTN:
    ...
    IF  ?SYORIKN1  ^=  '0000000'  THEN
        ...
        ?SYORINM3 := '件数１：出荷件数（オンライン）　'
        CALL SSN0050L.TOKELIBO,
          PARA-(?SYORINM1,?SYORINM2,?SYORINM3,?SYORIKN1,?SYORIKN3,
                ?SYORIKN3,?SYORIKN4,?SYORIKN5)
    ELSE
        OVRDSPF FILE-DSPF,TOFILE-DSPF.TOKELIB,MEDLIB-TOKELIB
        ?KEKA1 :=  '出荷データが０件でした。'
        ?KEKA2 :=  '指定したバッチＮＯ、倉庫、納品日を'
        ?KEKA3 :=  '確認して下さい。'
        CALL SMG0030I.TOKELIB
