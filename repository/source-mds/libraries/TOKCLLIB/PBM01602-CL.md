...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01602.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PBM01602'
    VAR       ?STEP     ,STRING*8
...
SBM0166B:
    ...
    OVRF     FILE-BMSHPSW,TOFILE-BMSHPSW.BMSFLIB
    CALL     PGM-SBM0166B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ返品データ抽出'
              GOTO ABEND
...
SBM0172L:
    ...
    OVRF      FILE-BMSHPSW1,TOFILE-BMSHPSW1.BMSFLIB
    CALL      PGM-SBM0172L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ返品リスト出力'
              GOTO ABEND
...
SBM0181V:
    ...
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSHPSW1,TOFILE-BMSHPSW1.BMSFLIB
    OVRF      FILE-BMSHPSC,TOFILE-BMSHPSC.BMSFLIB
    CALL      PGM-SBM0181V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
...
/.  CNVDF FILE-BMSHEPC.BMSFLIB,
    ...
    ACTCHK-@NO
./  CNVDF FILE-BMSHPSC.BMSFLIB,PATH-'/HGNAS/BMSHPSC.CSV',
