...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY92010.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY92010'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY9201I:
    ...
    CALL      PGM-SSY9201I.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
...
SSY9202B:
    ...
    CALL      PGM-SSY9202B.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'データ抽出'
              GOTO ABEND END
...
SSY9203L:
    ...
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    CALL      PGM-SSY9203L.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '個口数記入一覧表'
              GOTO ABEND END
