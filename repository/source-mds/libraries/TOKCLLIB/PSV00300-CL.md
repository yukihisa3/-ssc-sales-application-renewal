...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSV00300.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSV00300'
    VAR       ?STEP     ,STRING*8
...
FILECHK:
    ...
    ASSIGN FILE-CSVROYSH.TOKDTLIB!@XCL
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF     ?PGMEC  ^=    0    THEN
           ?KEKA4 := 'ファイル排他獲得１'
...
SSV0030V:
    ...
    OVRF      FILE-CSVROYSH,TOFILE-CSVROYSH.TOKDTLIB
    CALL      PGM-SSV0030V.TOKSOLIB,PARA-(?SIMEBI)
...
FIMPORT1:   /.ＣＳＶ転送./
    ...
    FIMPORT   FILE-CSVROYSH.TOKDTLIB,TYPE-@FILE,
              PARA-SEIKSIHA,UNIT-3,OPR-@NO
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ロイヤル支払明細ＣＳＶ転送'
