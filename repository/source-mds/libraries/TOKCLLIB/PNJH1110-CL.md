...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNJH1110.CL`
...
/. *   SYSTEM-NAME :    発注受信業務　　　　　　　　　　　　 *  ./
/. *   JOB-ID      :    PNJH1110                             *  ./
/. *   JOB-NAME    :    発注データ変換処理（ケーヨー統合）   *  ./
/. ***********************************************************  ./
    PGM (P1-?PGKBN,P2-?HIDUKE,P3-?JIKAN,P4-?PGCHK)

/.##ﾊﾟﾗﾒﾀ定義##./
    PARA      ?PGKBN    ,STRING*1,IN,VALUE-' '        /.ＰＧ区分./
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH1110'  /.PROGRAM-ID./
...
/.##JOB名称##./
    VAR       ?JOBNML   ,STRING*7,VALUE-'KEIYOLT'  /.JOB名LST./
    VAR       ?JOBNMK   ,STRING*7,VALUE-'KEIYOER'  /.JOB名KEKA./
...
/.## 変換取引先情報取得 ##./
    ...
    ELSE
          ?TOKNM := '取引先変換エラー'
    END
    ?MSGX :=  '##(' && ?PGMID && ')発注ﾃﾞｰﾀ変換開始'
    SNDMSG MSG-?MSGX,TO-XCTL.@ORGPROF,SLOG-@YES,JLOG-@YES
    ?MSGX :=  '##(' && ?PGMID && ')変換取引先:' && ?TOKNM
    SNDMSG MSG-?MSGX,TO-XCTL.@ORGPROF,SLOG-@YES,JLOG-@YES
...
NCV3105B:
    ...
    CALL      PGM-NCV3105B.TOKSOLIB,PARA-(?PGKBN,?HIDUKE,?JIKAN,
...
PARAHENS:
    ...
    ?PARA :=   ?PGKBN && ?HIDUKE && ?JIKAN && ?LINE && ?YUSEN
...
NJH1110B:
    ...
    CALL      PGM-NJH1110B.TOKSOLIB,PARA-(?PARA)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
