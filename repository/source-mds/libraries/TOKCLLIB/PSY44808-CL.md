...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY44808.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY44808'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
/.##ﾌﾟﾛｸﾞﾗﾑ名称ｾｯﾄ##./
    ?PGNM :=  'ＢＭＳ送信ＤＴ作成（ジュンテンドー）'
    ?SYORINM1 :=  'ＢＭＳ送信ＤＴ作成（ジュンテンドー）'
...
STEP070:
    ...
    CALL  SBM0082B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,?SOKO,?NOUDT,
                                  ?SYORIKN1)
    IF      @PGMEC    ^=   0    THEN
