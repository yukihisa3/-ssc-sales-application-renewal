...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01005.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01005'
    VAR ?STEP     ,STRING*8
...
SBM0109B:
    ...
    CALL     PGM-SBM0109B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0115L:
    ...
    CALL      PGM-SBM0115L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
CNVDF000:
  ...
  /.実機./
    CNVDF FILE-BMSJY5C.BMSFLIB,PATH-'/HGNAS/BMSJYJT.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
  /.社内./
 /. CNVDF FILE-BMSJY5C.BMSFLIB,PATH-'/NAVPC/BMSJYJT.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
 ./
    IF        @PGMEC    ^=   0    THEN
