...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PVD05000.CL`
...
/.###ﾜｰｸｴﾘｱ定義###./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PVD05000'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
/.###出力パラメタ用###./
    VAR ?SGKENSU  ,STRING*10,VALUE-'          ' /.入荷実績件数./
...
NVD0500B:
    ...
    CALL PGM-NVD0500B.TOKSOLIB,PARA-(?SGKENSU)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'Ｄ３５６連携データ作成（作業）'
              GOTO ABEND
    END
/.##出力ＭＳＧ確認##./
    ?MSGX :=  '# 作業実績件数 = '  && ?SGKENSU  &&   ' #'
