...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/MAEJY1CL.CL`
...
/.###ﾜｰｸｴﾘｱ定義###./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'MAEJY1CL'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
PCNVFILE:
    ...
    CNVFILE FILE-SNDUJYTF.D365DLIB,TOFILE-SNDNJYTF.D365DLIB,
            ADD-@NO,BF-1
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '日次更新受注データ送信Ｆセット'
              GOTO ABEND
    END
