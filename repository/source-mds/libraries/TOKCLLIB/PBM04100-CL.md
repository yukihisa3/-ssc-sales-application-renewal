...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM04100.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PBM04100'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
/.**VAR*******?TORICD ,STRING*8,VALUE-'00038709'*****取引先*****./
    ...
    DEFLIBL BMSFLIB/TOKELIB/TOKFLIB/TOKKLIB/ONLBLIB
...
/.  ASSIGN    FILE-SETGKFV.TOKKLIB!@XCL
./  ASSIGN    FILE-SEIGK50.TOKFLIB!@XCL
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              SNDMSG '他で連携処理が行われています',TOWS-@ORGWS
              ?KEKA4 :=  'ビバホーム請求合計ファイルロック'
              GOTO ABEND
    END
    ...
    ASSIGN    FILE-SEKVIVAH.ONLBLIB!@XCL
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              SNDMSG '他で連携処理が行われています',TOWS-@ORGWS
...
/.  OVRF FILE-SETGKFP,TOFILE-SETGKFV.TOKKLIB
./  OVRF FILE-SETGKFP,TOFILE-SEIGK50.TOKFLIB
    ...
    CALL SBM0410B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
...
STEP060:
    ...
    OVRF FILE-BMSSEKL1,TOFILE-BMSSEKL1.BMSFLIB
    OVRF FILE-SEKVHMF,TOFILE-SEKVIVAH.ONLBLIB
    CALL  SBM0420B.TOKSOLIB,PARA-(?SYORIKN1)
    IF      @PGMEC    ^=   0    THEN
