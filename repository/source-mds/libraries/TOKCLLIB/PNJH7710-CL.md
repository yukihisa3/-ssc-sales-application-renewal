...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNJH7710.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSJH7710'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP040:
    ...
    FEXPORT FILE-HACSNDY.ONLBLIB,MODE-@REP,PARA-SUNDAY,
            UNIT-1
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF      ?PGMEC    ^=   0    THEN
...
STEP060:
    ...
    CALL PNJH7711.TOKCOLIB,PARA-(?HIDUKE,?JIKAN,?PGCHK1)
     IF  ?PGCHK1   = '1'        THEN
         ?KEKA4 := '【バッチ_採番処理】'
                    GOTO ABEND
    END
...
STEP090:
    ...
    FIMPORT FILE-CLR3141.TOKKLIB,PARA-SUNDAY,UNIT-2
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF      ?PGMEC    ^=   0    THEN
            ?KEKA4 := '【受信データクリア処理】'
