...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01004.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01004'
    VAR ?STEP     ,STRING*8
...
SBM0108B:
    ...
    OVRF     FILE-BMSJY4W,TOFILE-BMSJY4W.BMSFLIB
    CALL     PGM-SBM0108B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0114L:
    ...
    OVRF      FILE-BMSJY4W1,TOFILE-BMSJY4W1.BMSFLIB
    CALL      PGM-SBM0114L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SBM0123V:
    ...
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJY4W1,TOFILE-BMSJY4W1.BMSFLIB
    OVRF      FILE-BMSJY4C,TOFILE-BMSJY4C.BMSFLIB
    CALL      PGM-SBM0123V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力'
              GOTO ABEND
    END
...
CNVDF000:
    ...
    CNVDF FILE-BMSJY4C.BMSFLIB,PATH-'/HGNAS/BMSJY4C.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,
          EXTCNV-@YES
  /.CNVDF FILE-BMSJY4C.BMSFLIB,PATH-'/NAVPC/BMSJY4C.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,
          EXTCNV-@YES   ./
    IF        @PGMEC    ^=   0    THEN
