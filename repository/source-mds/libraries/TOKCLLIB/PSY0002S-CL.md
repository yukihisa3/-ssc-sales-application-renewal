...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY0002S.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSY0002S'
    VAR       ?STEP     ,STRING*8
...
/.2020/01/14 SORT KET 追加　_番の後ろにサカタ商品ＣＤ　　./
    ?STEP :=  'SORT1   '
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL
...
SSY0025C:
    ...
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    CALL      PGM-SSY0025C.TOKELIBO,PARA-('1')
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
