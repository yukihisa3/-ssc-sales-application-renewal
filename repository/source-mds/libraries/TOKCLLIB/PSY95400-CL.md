...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY95400.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY95400'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY9542B:
    ...
    OVRF      FILE-YODSNDL4,TOFILE-YODSNDL4.TOKDTLIB
    OVRF      FILE-YODJOHL2,TOFILE-YODJOHL2.TOKDTLIB
    OVRF      FILE-YODSNDSF,TOFILE-YODSNDSF.TOKDTLIB
    OVRF      FILE-SHTDENLJ,TOFILE-SHTDENLJ.TOKKLIB
/.  CALL      PGM-SSY9542B.TOKSOLIB,PARA-(?JDATE,?JTIME,?JTORI,?SOKCD,
./  CALL      PGM-SSY9543B.TOKSOLIB,PARA-(?JDATE,?JTIME,?JTORI,?SOKCD,
                                          ?NDATES,?NDATEE,?SYORIKN1)

    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '出荷通知データ作成（本社確定処理）'
              GOTO ABEND
    END
