...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY12300.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY12300'    /.ﾌﾟﾛｸﾞﾗﾑID./
    ...
    VAR ?NKEITAI  ,STRING*1,VALUE-' '           /.納品形態　　　./
    ...
    VAR ?TANAF    ,STRING*6,VALUE-'000000'      /._番　　(FROM)./
    VAR ?TANAT    ,STRING*6,VALUE-'000000'      /._番　　(TO  )./
...
SSY1230I:
    ...
    OVRF      FILE-KNCASEL3,TOFILE-KNCASEL3.TOKDTLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY1230I.TOKSOLIB
             ,PARA-(?BUMON,?TANCD,?SOKCD,?DSOKCD,
                    ?NKEITAI,
                    ?JDATE,?JTIME,?JTORI,?OSOKCD,?CENTERF,?CENTERT,
                    ?ROUTEF,?ROUTET,?BUMONF,?BUMONT,?HDATE,?NDATE,
                    ?TANAF,?TANAT)
    IF        @PGMEC    ^=   0    THEN
         IF   @PGMEC     =   4010 THEN
...
SSY1231L:
    ...
    OVRPRTF   FILE-PRTF,TOFILE-XU04LP.XUCL,MEDLIB-TOKMDLIB
    OVRF      FILE-KNBULKL2,TOFILE-KNBULKL2.TOKDTLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY1231L.TOKSOLIB,
              PARA-(?JDATE,?JTIME,?JTORI,?OSOKCD,?CENTERF,?CENTERT,
                    ?ROUTEF,?ROUTET,?BUMONF,?BUMONT,?HDATE,?NDATE,
                    ?TANAF,?TANAT)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '一括ＰＤラベル発行一覧表出力'
              GOTO ABEND
    END
...
SSY1232L:
    ...
    OVRPRTF   FILE-PRTF,TOFILE-XU04LP.XUCL,MEDLIB-TOKMDLIB
    OVRF      FILE-KNCASEL2,TOFILE-KNCASEL2.TOKDTLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    OVRF      FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    OVRF      FILE-SHTDENLA,TOFILE-SHTDENLA.TOKFLIB
    CALL      PGM-SSY1232L.TOKSOLIB,
              PARA-(?JDATE,?JTIME,?JTORI,?OSOKCD,?CENTERF,?CENTERT,
                    ?ROUTEF,?ROUTET,?BUMONF,?BUMONT,?HDATE,?NDATE,
                    ?TANAF,?TANAT)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ケースＰＤラベル発行一覧表出力'
              GOTO ABEND
    END
