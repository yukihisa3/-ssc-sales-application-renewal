...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSI78070.CL`
...
/. ***********************************************************  ./
/. *   サカタのタネ                                          *  ./
/. *   SYSTEM-NAME :    請求（インボイス対応）               *  ./
/. *   JOB-ID      :    PSI78070                             *  ./
/. *   JOB-NAME    :    ロイヤルＨＣ支払明細ＣＳＶデータ出力 *  ./
/. *   UPDATE      :    2023/11/17                           *  ./
/. ***********************************************************  ./
    PGM
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSI78070'
    VAR       ?STEP     ,STRING*8
...
FILECHK:
    ...
    ASSIGN FILE-CSVROYNH.TOKDTLIB!@XCL
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF     ?PGMEC  ^=    0    THEN
           ?KEKA4 := 'ファイル排他獲得１'
...
SSV0030V:
    ...
    OVRF      FILE-CSVROYNH,TOFILE-CSVROYNH.TOKDTLIB
    CALL      PGM-SSI7807V.TOKSOLIB,PARA-(?SIMEBI)
...
FIMPORT1:   /.ＣＳＶ転送./
    ...
    FIMPORT   FILE-CSVROYNH.TOKDTLIB,TYPE-@FILE,
              PARA-CSVROYNH,UNIT-1,OPR-@NO
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ロイヤル支払明細ＣＳＶ転送'
