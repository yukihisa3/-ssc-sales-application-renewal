...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSV00100.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSV00100'
    VAR       ?STEP     ,STRING*8
...
FILECHK:
    ...
    ASSIGN FILE-CSVGKFA.TOKDTLIB!@XCL
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF     ?PGMEC  ^=    0    THEN
           ?KEKA4 := 'ファイル排他獲得１'
...
SSV0010V:
    ...
    OVRF      FILE-CSVGKFA,TOFILE-CSVGKFA.TOKDTLIB
    CALL      PGM-SSV0010V.TOKSOLIB
...
FIMPORT1:   /.ＣＳＶ転送./
    ...
    FIMPORT   FILE-CSVGKFA.TOKDTLIB,TYPE-@FILE,
              PARA-SEIKSIHA,UNIT-1,OPR-@NO
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '請求明細ＣＳＶ転送'
