...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY08060.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY08060'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SSY0806I:
    ...
    OVRF      FILE-KPPFILL2,TOFILE-KPPFILL2.TOKDTLIB
    OVRDSPF   FILE-DSPF,TOFILE-DSPF.XUCL,MEDLIB-TOKMDLIB
    CALL      PGM-SSY0806I.TOKSOLIB,PARA-(?P1,?P2,?NOUDT1,?NOUDT2,
                                        ?KBN)
    IF        @PGMEC    ^=   0    THEN
              ?PGMEC := @PGMEC
              IF    ?PGMEC  =  4010  THEN
...
SSY0807L:
    ...
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY0807L.TOKSOLIB,PARA-(?NOUDT1,?NOUDT2)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
    GOTO       RTN
...
SSY0808L:
    ...
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    OVRF      FILE-ZSOKMS1,TOFILE-ZSOKMS1.TOKFLIB
    CALL      PGM-SSY0808L.TOKSOLIB,PARA-(?NOUDT1,?NOUDT2)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
RTN:
