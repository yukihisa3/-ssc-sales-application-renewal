...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PST00200.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PST00200'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SST0010B:
    ...
    CALL      PGM-SST0010B.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4,?P6)
    ?PGMEC     :=    @PGMEC
    ?PGMEM     :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ストックＮＯ管理データ抽出'
