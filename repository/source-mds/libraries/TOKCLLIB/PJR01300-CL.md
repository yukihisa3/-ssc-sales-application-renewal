...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PJR01300.CL`
...
/. *   SYS-NAME    :    受領返品　　　　　　                   *  ./
/. *   JOB-ID      :    PJR01300                               *  ./
/. *   JOB-NAME    :    共通変換（ムサシ：アークランドサカモト)* ./
/. *   CREATE      :    2017/10/10 S11270540                   *  ./
/. *   UPDATE      :                                           *  ./
/. *************************************************************  ./
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PJR01300'  /.PROGRAM-ID./
...
SJR0130B:
    ...
    CALL      PGM-SJR0130B.TOKSOLIB,PARA-(?HIDUKE,?JIKAN,?TOKCD)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
