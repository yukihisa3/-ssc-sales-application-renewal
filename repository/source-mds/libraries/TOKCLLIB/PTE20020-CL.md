...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PTE20020.CL`
...
/. ***********************************************************  ./
    PGM (P1-?KBN)
/.###ﾊﾟﾗﾒﾀ####./
    PARA ?KBN     ,STRING*1,IN,VALUE-' '             /.実行区分./
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PTE20020'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
PTE20020:
    ...
    OVRF      FILE-JYOKEN1,TOFILE-JYOKEN1.TOKFLIB
    OVRPRTF   FILE-PRTF,TOFILE-PRTF.XUCL,MEDLIB-TOKMDLIB
    OVRDSPF   FILE-DSPF,TOFILE-DSPF.XUCL,MEDLIB-TOKMDLIB
    CALL      PGM-STE2002B.TOKSOLIB,PARA-(?KBN)
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND END
