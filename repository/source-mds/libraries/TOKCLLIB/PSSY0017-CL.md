...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSSY0017.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
  ...
  /.VAR ?JHTTEGFN ,STRING*17                 ./ /.ﾌｧｲﾙ名表示用./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSSY0017'
    VAR       ?STEP     ,STRING*8
...
SSY0016B:
  ...
  /.SETPF FILE-?JHTDENF,TOFILE-?JHTTEGF,ADD-@NO,ACTCHK-@NO ./
    SETPF FILE-JHTTEGWF.BMSFLIB,TOFILE-BMSTE3F.BMSFLIB,
          ADD-@NO,ACTCHK-@NO
    /. データＳＯＲＴ 植物./
    SORTF  INFILE-JHTTEGSF.BMSFLIB,
           OUTFILE-JHTTEGWF.BMSFLIB,
           KEY-'F01!A',
           SELECT-'F01!EQ!#00926061',
           RCDL-@DSP
    IF        @PGMEC    ^=   0    THEN
              GOTO ABEND
    END
    ...
    /.##抽出対象の各出荷場所ﾌｧｲﾙへﾃﾞｰﾀをｺﾋﾟｰする##./
    SETPF FILE-JHTTEGWF.BMSFLIB,TOFILE-BMSTE4F.BMSFLIB,
          ADD-@NO,ACTCHK-@NO
...
ABEND:
    ...
    DEFLIBL TOKELIBO/TOKELIB/TOKFLIB/TOKKLIB/TOKDTLIB/TOKSOLIB
    OVRDSPF FILE-DSPF,TOFILE-DSPF.TOKELIB,MEDLIB-TOKELIB
    ?KEKA1 :=  '（セキチュー）手書伝票出荷ＤＴ作成処理'
    ?KEKA2 :=  'が異常終了しました。ログを採取しＮＡＶ'
    ?KEKA3 :=  'へ連絡して下さい。'
