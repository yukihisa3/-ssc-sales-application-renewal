...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01003.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01003'
    VAR ?STEP     ,STRING*8
...
SBM0107B:
    ...
    OVRF     FILE-BMSJRAW,TOFILE-BMSJRAW.BMSFLIB
    CALL     PGM-SBM0107B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0113L:
    ...
    OVRF      FILE-BMSJRAW1,TOFILE-BMSJRAW1.BMSFLIB
    CALL      PGM-SBM0113L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SBM0122V:
    ...
    ?MSGX :=  '***   '  && ?STEP   &&   '        ***'
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJRAC,TOFILE-BMSJRAC.BMSFLIB
    OVRF      FILE-BMSJRAW1,TOFILE-BMSJRAW1.BMSFLIB
    CALL      PGM-SBM0122V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力'
              GOTO ABEND
    END
...
CNVDF000:
    ...
    CNVDF FILE-BMSJRAC.BMSFLIB,PATH-'//HGNAS/BMSJRAC.CSV',
