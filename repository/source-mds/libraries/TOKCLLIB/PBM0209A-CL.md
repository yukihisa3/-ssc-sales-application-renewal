...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM0209A.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PBM0209A'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SBM0209B:
    ...
    CALL      PGM-SBM0209B.TOKSOLIB,PARA-(?TORICD,?SIMEBI)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払データ抽出（ビバホーム）'
              GOTO ABEND
    END
...
SBM0214C:
  ...
  /.
    ...
    END
  ./
    OVRF      FILE-BMSSH6W1,TOFILE-BMSSH6W1.BMSFLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    CALL      PGM-SBM0214C.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細書発行（ビバホーム）'
              GOTO ABEND
    END
