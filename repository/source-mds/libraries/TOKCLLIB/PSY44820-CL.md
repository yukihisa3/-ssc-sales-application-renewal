...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY44820.CL`
...
/. *               :    　セキチュー（手書分）　　　　　　   *  ./
/. *   UPDATE      :                                         *  ./
/. *   UPDATE      :    22/10/13　SBM0032I 0件制御追加       *  ./
/. ***********************************************************  ./
    PGM   (P1-?TRICD)
    PARA ?TRICD   ,STRING*8,IN,VALUE-'00000000'  /.取引先コード ./
/.###ﾜｰｸｴﾘｱ定義####./
    VAR ?FILNM    ,STRING*8,VALUE-'        '    /.ﾌｧｲﾙ名合併用./
    VAR ?FILNMH   ,STRING*8,VALUE-'BMSTE3F'     /.ﾌｧｲﾙ名ﾃﾞﾌｫﾙﾄ./
    VAR ?FILNMT   ,STRING*8,VALUE-'BMSTE4F'     /.ﾌｧｲﾙ名ﾃﾞﾌｫﾙﾄ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PSY44820'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,NAME!MOD
...
STEP060:
 ...
 /. OVRF FILE-JHTTEGF,TOFILE-BMSTEGF.BMSFLIB  ./
    ...
    CALL SSY4483B.TOKSOLIB,PARA-(?SYORIKN1,?JDATE,?JTIME,?TORICD)
    IF      @PGMEC    ^=   0    THEN
/.##   IF      @PGMEC    =  4010    THEN
               GOTO RTN
##./   IF      @PGMEC    =  4010    THEN
...
/.  CALL  SBM0081B.TOKELIBO,PARA-(?JDATE,?JTIME,?TORICD,?SOKO,?NOUDT,
./  CALL  SBM0083B.TOKSOLIB,PARA-(?JDATE,?JTIME,?TORICD,
                                  ?SYORIKN2)
    IF      @PGMEC    ^=   0    THEN
    ...
    CASE ?TRICD OF
      #'00321717'# /. 資材 ./
        CNVFILE   FILE-SYKSNDF.BMSFLIB,
                  TOFILE-SYKSNDS1.ONLBLIB,
                  ADD-@YES,BF-1
        IF        @PGMEC    ^=   0    THEN
              ?KEKA4    :=   '出荷送信データ複写処理（手書）'
              GOTO ABEND
        END
      #'00926061'# /. 植物 ./
        CNVFILE   FILE-SYKSNDF.BMSFLIB,
                  TOFILE-SYKSNDS2.ONLBLIB,
                  ADD-@YES,BF-1
        IF        @PGMEC    ^=   0    THEN
              ?KEKA4    :=   '出荷送信データ複写処理（手書）'
              GOTO ABEND
