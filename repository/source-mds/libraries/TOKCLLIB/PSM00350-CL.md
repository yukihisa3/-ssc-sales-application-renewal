...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSM00350.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSM00100'
    VAR       ?STEP     ,STRING*8
    ...
    VAR       ?SYUMLIB0 ,NAME!MOD                  /.ﾊﾟﾗﾒﾀﾗｲﾌﾞﾗﾘ名./
    VAR       ?SYUMLBN0 ,STRING*17                 /.ﾊﾟﾗﾒﾀﾗｲﾌﾞﾗﾘ名./
    VAR       ?SYUMLIB1 ,NAME!MOD                  /.ﾊﾟﾗﾒﾀﾗｲﾌﾞﾗﾘ名./
    VAR       ?SYUMLBN1 ,STRING*17                 /.ﾊﾟﾗﾒﾀﾗｲﾌﾞﾗﾘ名./
    VAR       ?LIB      ,STRING*8,VALUE-'TOKDTLIB' /.ﾗｲﾌﾞﾗﾘ名./
    VAR       ?KEYNO1    ,STRING*1,VALUE-'1'        /.KEY NO../
    VAR       ?KEYNO2    ,STRING*1,VALUE-'2'        /.KEY NO../
...
/.パラメタファイル名を生成./
    ?SYUMEIFN := ?SYUMEIF && ?P15                 /.ﾌｧｲﾙ名を生成./
    ?SYUMFILE := %NAME(?SYUMEIFN)               /.名前型に変換./
    ?LIBN     := %NAME(?LIB)                    /.名前型に変換./
    ?SYUMLIB0 := %NCAT(?SYUMFILE,?LIBN)         /.名前型の結合./
    ?SYUMLBN0:= %STRING(?SYUMLIB0)             /.文字列型に変換./
    ?MSGX     := '## 出荷明細PF = ' && ?SYUMLBN0 && ' ##'
    SNDMSG ?MSGX,TO-XCTL.@ORGPROF,SLOG-@YES,JLOG-@YES /.画面に出力./
    ?SYUMEIFN := ?SYUMEIF && ?P15 && ?KEYNO1      /.ﾌｧｲﾙ名を生成./
    ?SYUMFILE := %NAME(?SYUMEIFN)               /.名前型に変換./
    ?LIBN     := %NAME(?LIB)                    /.名前型に変換./
    ?SYUMLIB  := %NCAT(?SYUMFILE,?LIBN)         /.名前型の結合./
    ?SYUMLIBN := %STRING(?SYUMLIB)              /.文字列型に変換./
    ?MSGX     := '## 出荷明細L1 = ' && ?SYUMLIBN && ' ##'
    SNDMSG ?MSGX,TO-XCTL.@ORGPROF,SLOG-@YES,JLOG-@YES /.画面に出力./
    ?SYUMEIFN := ?SYUMEIF && ?P15 && ?KEYNO2      /.ﾌｧｲﾙ名を生成./
    ?SYUMFILE := %NAME(?SYUMEIFN)               /.名前型に変換./
    ?LIBN     := %NAME(?LIB)                    /.名前型に変換./
    ?SYUMLIB1 := %NCAT(?SYUMFILE,?LIBN)         /.名前型の結合./
    ?SYUMLBN1:= %STRING(?SYUMLIB1)             /.文字列型に変換./
    ?MSGX     := '## 出荷明細L2 = ' && ?SYUMLBN1 && ' ##'
    SNDMSG ?MSGX,TO-XCTL.@ORGPROF,SLOG-@YES,JLOG-@YES /.画面に出力./
...
SSM0035L:
    ...
    OVRF FILE-SYKMEIL2,TOFILE-?SYUMLIB1
    CALL      PGM-SSM0035L.TOKSOLIB,PARA-(?P13)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '出荷明細表作表'
