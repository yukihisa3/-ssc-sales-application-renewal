...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PHS00100.CL`

## ソースコード

```jcl
/. ***********************************************************  ./
/. *     サカタのタネ　ＮＡＶＳシステム（営業第２部）        *  ./
/. *   SYSTEM-NAME :    ＤＣＭＪＡＰＡＮ発注種別区分変更     *  ./
/. *   JOB-ID      :    PHS00100                             *  ./
/. *   JOB-NAME    :    発注種別変換マスタ保守　　　　　　　 *  ./
/. *               :                                         *  ./
/. ***********************************************************  ./
/.###ﾜｰｸｴﾘｱ定義####./
    PGM
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PHS00100'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SHS0010I:
    ...
    CALL PGM-SHS0010I.TOKSOLIB,PARA-(?BUMON,?TANCD)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '発注種別変換マスタ保守'
