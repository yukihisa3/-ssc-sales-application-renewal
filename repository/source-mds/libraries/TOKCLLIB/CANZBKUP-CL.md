...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/CANZBKUP.CL`
...
/. ***********************************************************  ./
     ...
     VAR  ?MSG   ,STRING*99(6)
     VAR  ?MSGX  ,STRING*99
     VAR  ?PGMID ,STRING*8,VALUE-'CANZBKUP'
     VAR  ?STEP  ,STRING*8
     ...
     VAR  ?LIBNM  ,STRING*8,VALUE-'CANZBKUP'
     VAR  ?FILID  ,NAME
     VAR  ?LIBID  ,NAME
     VAR  ?FURIBK ,NAME!MOD
     VAR  ?FURIBKN,STRING*17

     SNDMSG MSG-'##ｶｲﾝｽﾞＤＴﾊﾞｯｸｱｯﾌﾟ開始##',TO-XCTL
...
STEP030: /.##バックアップ開始##./
    ...
    CPYFILE FILE-CNZLNKTK.LINKSLIB,TOFILE-?FURIBK,CRTFILE-@YES
    IF        @PGMEC    ^=   0   THEN
              IF  @PGMEC =   542 THEN
                  GOTO STEP040
              ELSE
...
STEP050: /.##ﾊﾞｯｸｱｯﾌﾟ開始(重複あり３)##./
    ...
    IF        @PGMEC    ^=   0   THEN
              ...
              SNDMSG MSG-?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
              ?MSGX := '## ﾌｧｲﾙ名=TOKFRR.CANZBKUP#'
              SNDMSG MSG-?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
              CNVFILE FILE-CNZLNKTK.LINKSLIB,
                      TOFILE-CNZLNKTK.CANZBKUP,ADD-@NO,BF-3
              GOTO ABEND
    END

RTN:

    CLRFILE FILE-CNZLNKTK.LINKSLIB
    ?MSGX :=  '***   '  && ?PGMID  &&   ' END    ***'
    SNDMSG    ?MSGX,TO-XCTL
