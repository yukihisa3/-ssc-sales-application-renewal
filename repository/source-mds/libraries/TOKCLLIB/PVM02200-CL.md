...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PVM02200.CL`
...
/. ***********************************************************  ./
    ...
    VAR  ?MSG     ,STRING*99(6)
    VAR  ?MSGX    ,STRING*99
    VAR  ?PGMID   ,STRING*8,VALUE-'PVM02200'
    VAR  ?STEP    ,STRING*8
/.--------------------------------------------------------------./
...
ASSIGN:
    ...
    ASSIGN FILE-SMSMNTCS.HGWORKLB!@XCL
    ?PGMEC :=    @PGMEC
    ?PGMEM :=    @PGMEM
    ?PGMES :=    @PGMES
    IF     ?PGMEC ^=   1020 THEN
    ...
    ASSIGN FILE-SMSMNTWK.HGWORKLB!@XCL
    ?PGMEC :=    @PGMEC
    ?PGMEM :=    @PGMEM
    ?PGMES :=    @PGMES
    IF     ?PGMEC ^=   0    THEN
           ...
           GOTO   ABEND1
    END
    CLRFILE FILE-SMSMNTWK.HGWORKLB
...
PFEXPORT:
    ...
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
                ...
                FEXPORT FILE-SMSMNTCS.HGWORKLB,
                        MODE-@REP,
                        PARA-SMSMNTCS,
                        UNIT-1
                ?PGMEC  := @PGMEC
...
SORT1:
    ...
    SORT      INFILE-SMSMNTCS.HGWORKLB,INRL-290,INBF-14,
              OUTFILE-SMSMNTCS.HGWORKLB,OUTBF-14,
              KEY-5!8!CA,KEY1-14!8!CA,
              RCDL-@DSP
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
...
NVM0220B:
    ...
    SNDMSG    ?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES,SLOG-@YES
                ...
                OVRF   FILE-SMSMNTCS,TOFILE-SMSMNTCS.HGWORKLB
                OVRF   FILE-SMSMNTW1,TOFILE-SMSMNTW1.HGWORKLB
                CALL   PGM-NVM0220B.TOKSOLIB,
                       PARA-(?BUMON,?TANCD,
                             ?CNT1,?CNT2)
                ?PGMEC := @PGMEC
                ?PGMES := @PGMES
...
/.  IF  ?BUMON  =  '2920'  THEN
    ...
    END
./
    OVRF      FILE-SMSMNTW3,TOFILE-SMSMNTW3.HGWORKLB
    OVRF      FILE-SMSMNTW2,TOFILE-SMSMNTW2.HGWORKLB
    CALL      PGM-NVM0230L.TOKSOLIB,
...
RTN:
    ...
    RELEASE   FILE-SMSMNTCS.HGWORKLB!@XCL/SMSMNTWK.HGWORKLB!@XCL
