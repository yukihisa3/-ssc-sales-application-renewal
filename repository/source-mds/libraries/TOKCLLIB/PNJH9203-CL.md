...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNJH9203.CL`
...
/. *   SYSTEM-NAME :    変換伝票データ作成（ＥＤＩＣ用）     *  ./
/. *   JOB-ID      :    PNJH9203                             *  ./
/. *   JOB-NAME    :    エンチョー用　　　　                 *  ./
/. *   CREATE      :    2020.06.09 S22450420                 *  ./
/. ***********************************************************  ./
...
/.##ﾜｰｸﾃｲｷﾞ##./
    ...
    VAR       ?PGMID    ,STRING*8,VALUE-'PNJH9203'  /.PROGRAM-ID./
...
SED0000B:
    ...
    OVRF      FILE-ONLEDBEB,TOFILE-?FILLIB
    OVRF      FILE-ONLEDBEW,TOFILE-ONLEDBEW.ONLBLIB
    CALL      PGM-SED0000B.TOKELIBO
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
...
NJH1903B:
    ...
    OVRF      FILE-CVCSG001,TOFILE-ONLEDBEW.ONLBLIB
    ...
    OVRF      FILE-EDJOHOL1,TOFILE-EDJOHOL1.TOKDLIB
    OVRVLDF   FILE-VLD500,TOFILE-LD500.XUCL
    CALL      PGM-NJH9203B.TOKSOLIB,PARA-(?PARA)
    IF        @PGMEC    ^=   0    THEN
              /.##ABENDｺｰﾄﾞｾｯﾄ##./
              ?KEKA := 'K525'
              CALL PGM-SNJ0730B.TOKELIBO,
