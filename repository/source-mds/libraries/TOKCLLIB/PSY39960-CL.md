...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY39960.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PSY39960'    /.ﾌﾟﾛｸﾞﾗﾑID./
    ...
    VAR ?STKANR   ,STRING*8,VALUE-'        '    /. 開始管理番号./
    VAR ?EDKANR   ,STRING*8,VALUE-'        '    /. 終了管理番号./
...
SSY3996I:
    ...
    CALL PGM-SSY3996I.TOKSOLIB,PARA-(?STKANR,?EDKANR)
...
SSY3994B:
    ...
    CALL      PGM-SSY3994B.TOKSOLIB,PARA-(?STKANR,?EDKANR)

    IF        @PGMEC    ^=   0   THEN
              ?KEKA4    :=   'ナフコ受注出荷累積データ抽出'
              GOTO ABEND
    END
