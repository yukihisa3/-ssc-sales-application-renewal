...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSV00400.CL`
...
/. ***********************************************************  ./
    ...
    VAR       ?MSG      ,STRING*99(6)
    VAR       ?MSGX     ,STRING*99
    VAR       ?PGMID    ,STRING*8,VALUE-'PSV00400'
    VAR       ?STEP     ,STRING*8
...
FILECHK:
    ...
    ASSIGN FILE-CSVGKFE.TOKDTLIB!@XCL
    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF     ?PGMEC  ^=    0    THEN
           ?KEKA4 := 'ファイル排他獲得１'
...
SSV0040V:
    ...
    OVRF      FILE-CSVGKFE,TOFILE-CSVGKFE.TOKDTLIB
    CALL      PGM-SSV0040V.TOKSOLIB

    ?PGMEC := @PGMEC
    ?PGMES := @PGMES
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 := '山新支払明細ＣＳＶデータ出力'
              GOTO   ABEND
    END
...
FIMPORT1:   /.ＣＳＶ転送./
    ...
    FIMPORT   FILE-CSVGKFE.TOKDTLIB,TYPE-@FILE,
              PARA-SEIKSIHA,UNIT-4,OPR-@NO
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  '山新支払明細ＣＳＶ転送'
