...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PVD05500.CL`
...
/.###ﾜｰｸｴﾘｱ定義###./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PVD05500'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
NVD0550B:
    ...
    CALL PGM-NVD0550B.TOKSOLIB
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'ＮＡＶＳ間移動先発注ＤＴ作成'
              GOTO ABEND
    END
