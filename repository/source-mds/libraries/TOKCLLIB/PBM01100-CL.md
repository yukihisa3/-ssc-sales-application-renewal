...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01100.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01100'
    VAR ?STEP     ,STRING*8
...
SBM0110B:
    ...
    OVRF     FILE-BMSJY6W,TOFILE-BMSJY6W.BMSFLIB
    CALL     PGM-SBM0110B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0116L:
    ...
    OVRF      FILE-BMSJY6W1,TOFILE-BMSJY6W1.BMSFLIB
    CALL      PGM-SBM0116L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SBM0125V:
    ...
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJY6W1,TOFILE-BMSJY6W1.BMSFLIB
    OVRF      FILE-BMSJY6C,TOFILE-BMSJY6C.BMSFLIB
    CALL      PGM-SBM0125V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力'
              GOTO ABEND
    END
...
CNVDF000:
 ...
 /.実機用./
    CNVDF FILE-BMSJY6C.BMSFLIB,
          PATH-'/HGNAS/BMSJY6C.CSV',
          MODE-@EXP,
          ADD-@NO,
          ACTCHK-@NO,
          HEADLINE-@YES,
