...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM01170.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PBM01170'
    VAR ?STEP     ,STRING*8
...
SBM0117B:
    ...
    OVRF     FILE-BMSJRSW,TOFILE-BMSJY7W.BMSFLIB
    CALL     PGM-SBM0117B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SBM0118L:
    ...
    OVRF      FILE-BMSJRSW1,TOFILE-BMSJY7W1.BMSFLIB
    CALL      PGM-SBM0118L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SBM0126V:
    ...
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJRSW1,TOFILE-BMSJY7W1.BMSFLIB
    OVRF      FILE-BMSJRSC,TOFILE-BMSJY7C.BMSFLIB
    CALL      PGM-SBM0126V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力'
              GOTO ABEND
    END
...
CNVDF000:
  ...
  /.実機./
    CNVDF FILE-BMSJY7C.BMSFLIB,PATH-'/HGNAS/BMSJYVH.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
  /.社内./
  /.CNVDF FILE-BMSJY7C.BMSFLIB,PATH-'/NAVPC/BMSJYVH.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES ./
