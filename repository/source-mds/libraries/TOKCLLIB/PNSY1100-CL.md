...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNSY1100.CL`
...
/. ***********************************************************  ./
/. *     サカタのタネ　ＮＡＶＳ基幹システム　　　　　        *  ./
/. *   SYSTEM-NAME :    センター納品対応　　　　　　　　　   *  ./
/. *   JOB-ID      :    PNSY1100                             *  ./
/. *   JOB-NAME    :    新ＤＣＭ基本情報ＣＳＶ出力           *  ./
/. ***********************************************************  ./
    PGM
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PNSY1100'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
NSY1100I:
    ...
    CALL      PGM-NSY1100I.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
         IF   ?PGMEC     =   4010 THEN
...
NSY1110B:
    ...
    OVRF      FILE-DNJOHOCV,TOFILE-DNJOHOCV.TOKDTLIB
    CALL      PGM-NSY1110B.TOKSOLIB,PARA-(?P1,?P2,?P3,?P4)
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
    IF        ?PGMEC    ^=   0    THEN
              ?KEKA4 :=  'データ抽出'
...
FIMPORT1:
    ...
    FIMPORT   FILE-CVDNJOHO.TOKDTLIB,
              PARA-NEWDCM,
              UNIT-5
    ?PGMEC    :=    @PGMEC
    ?PGMEM    :=    @PGMEM
