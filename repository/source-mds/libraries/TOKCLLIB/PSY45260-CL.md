...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PSY45260.CL`
...
/. ***********************************************************  ./
    ...
    VAR ?MSG      ,STRING*99(6)
    VAR ?MSGX     ,STRING*99
    VAR ?PGMID    ,STRING*8,VALUE-'PSY45260'
    VAR ?STEP     ,STRING*8
...
SSY4526B:
    ...
    OVRF     FILE-BMSJRSW,TOFILE-BMSJY5W.BMSFLIB
    CALL     PGM-SSY4526B.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                         ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領データ抽出'
              GOTO ABEND
...
SSY4527L:
    ...
    OVRF      FILE-BMSJRSW1,TOFILE-BMSJY5W1.BMSFLIB
    CALL      PGM-SSY4527L.TOKSOLIB,PARA-(?TORICD,?DTPTN,
                                          ?DATEF,?DATET)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書発行'
              GOTO ABEND
...
SSY4528V:
    ...
    SNDMSG    ?MSGX,TO-XCTL
    OVRF      FILE-BMSJRSW1,TOFILE-BMSJY5W1.BMSFLIB
    OVRF      FILE-BMSJRSC,TOFILE-BMSJY5C.BMSFLIB
    CALL      PGM-SSY4528V.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領書ＣＳＶ出力'
              GOTO ABEND
    END
...
CNVDF000:
  ...
  /.実機./
    CNVDF FILE-BMSJY5C.BMSFLIB,PATH-'/HGNAS/616_HANSMAN.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES
  /.社内./
  /.CNVDF FILE-BMSJY5C.BMSFLIB,PATH-'/NAVPC/616_HANSMAN.CSV',
          MODE-@EXP,ADD-@NO,ACTCHK-@NO,HEADLINE-@YES,EXTCNV-@YES ./

    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 := '流通ＢＭＳ受領ＣＳＶデータ転送'
              GOTO ABEND
    ELSE
              GOTO RTN
    END
