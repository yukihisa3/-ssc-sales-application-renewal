...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PKE00601.CL`
...
/.##ﾜｰｸ定義##./
    ...
    VAR   ?KUBUN ,STRING*1,VALUE-'1'      /.区分情報./
    ...
    VAR   ?MSG   ,STRING*99(6)
    VAR   ?MSGX  ,STRING*99
    VAR   ?PGMID ,STRING*8,VALUE-'PKE00601'
    VAR   ?STEP  ,STRING*8
    VAR   ?FILNM ,STRING*8,VALUE-'        ' /.ﾌｧｲﾙ名合併用./
    VAR   ?FILNMS,STRING*6,VALUE-'RCVSYK'   /.ﾌｧｲﾙ名ﾃﾞﾌｫﾙﾄ./
...
PFEXPORT:
    ...
    FEXPORT FILE-?RCVSYUF,MODE-@REP,PARA-KENPIN,
            UNIT-12
    IF        @PGMEC    ^=   0   THEN
              GOTO ABEND
    END
...
SKE5040B:
    ...
    CALL      PGM-SKE5045B.TOKELIBO,PARA-(?JOHO,?LABH,?LABM,?RERR,
                                         ?KUBUN,?KUBUN)
    IF        @PGMEC    ^=   0   THEN
              GOTO ABEND
    END
...
SKE0830I:
    ...
    CALL      PGM-NKE5050I.TOKSOLIB,PARA-(?SOKO,?JOHO,?LABH,?LABM,
                                         ?RERR,?KUBUN)
    IF        @PGMEC    ^=   0    THEN
              GOTO   ABEND
    END
