...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM0207A.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PBM0207A'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SBM0207B:
    ...
    OVRF      FILE-BMSSHAR,TOFILE-BMSSIHW.BMSFLIB
    CALL      PGM-SBM0207B.TOKSOLIB,PARA-(?TORICD,?SIMEBI)
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細ワーク作成'
              GOTO ABEND
    END
...
SBM0212C:
    ...
    ELSE
        ...
        END
    END
    OVRF      FILE-BMSSHAR1,TOFILE-BMSSIHW1.BMSFLIB
    OVRF      FILE-TOKMS2,TOFILE-TOKMS2.TOKFLIB
    OVRF      FILE-TENMS1,TOFILE-TENMS1.TOKFLIB
    CALL      PGM-SBM0212C.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4 :=  '支払明細書発行'
              GOTO ABEND
    END
