...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PBM0040C.CL`
...
/.###ﾜｰｸｴﾘｱ定義####./
    ...
    VAR ?PGMID    ,STRING*8,VALUE-'PBM0040C'    /.ﾌﾟﾛｸﾞﾗﾑID./
...
SBM0050B:
   ...
   /. 2013/01/30 取引先別に制御 ./
    CASE ?TORICD OF
      #'00013020'# /. バロー ./
         CALL PGM-SBM0052B.TOKELIBO,PARA-(?JDATE
                                          ,?JTIME
                                          ,?TORICD
                                          ,?SOKO
                                          ,?TENCD1,?TENCD2
      ...
      #'00751225'# /. アレンザＨＤ（ダイユーエイト）./
         CALL PGM-SBM0062C.TOKSOLIB,PARA-(?JDATE
                                          ,?JTIME
                                          ,?TORICD
                                          ,?SOKO
                                          ,?TENCD1,?TENCD2
      ...
      #'00761995'# /. アレンザＨＤ（タイム　　　　）./
         ...
         END
      #'00271934'# /. コメリ ./
         CALL PGM-SBM0053B.TOKELIBO,PARA-(?JDATE
                                          ,?JTIME
                                          ,?TORICD
                                          ,?SOKO
                                          ,?TENCD1,?TENCD2
      ...
      #'00005116'# /. ジュンテンドー　 ./
         ...
         END
      #'00921084'# /. カインズ　 ./
         OVRF FILE-CZSYKTN3,TOFILE-CZSYKTN3.BMSFLIB
         OVRF FILE-CZDENTNF,TOFILE-CZDENTNF.BMSFLIB
         CALL PGM-NKE1110B.TOKSOLIB,PARA-(?JDATE
                                          ,?JTIME
                                          ,?TORICD
                                          ,?SOKO
                                          ,?TENCD1,?TENCD2
         ...
         END
         OVRF FILE-CZDENTNF,TOFILE-CZDENTNF.BMSFLIB
         OVRF FILE-CZKEPTCF,TOFILE-CZKEPTNF.BMSFLIB
         CALL NKE1100D.TOKSOLIB
         IF   @PGMEC    ^=   0    THEN
              ?KEKA4    :=   '流通ＢＭＳ伝票欠品データ抽出'
              GOTO ABEND
         END
      #'00038709'# /. ビバホーム　 ./
...
OUTPUT12:
    ...
    SNDMSG    ?MSGX,TO-XCTL
 /. ?DENPYON  :=    %STRING(?DENPYO) ./
    ?MSGX     :=    '## 伝票ﾌｧｲﾙ名 = CZDENTNF.BMSFLIB ##'
    SNDMSG MSG-?MSGX,TO-XCTL.@ORGPROF,JLOG-@YES
    /. データＳＯＲＴ ./
    SORTF  INFILE-CZDENTNF.BMSFLIB,
           OUTFILE-CZDENTNF.BMSFLIB,
       /.  RCD-50,  ./
           RCD-10,
           KEY-'F349!A',             /.最終納品先納品日./
           KEY1-'F324!A',            /.取引先ＣＤ./
           KEY2-'F309!A',            /.最終納品先ＣＤ./
           KEY3-'F345!A',            /.商品分類（大）部門ＣＤ./
           KEY4-'F355!A',            /.発注区分./
           KEY5-'F332!A',            /.便_./
           KEY6-'F604!A',            /.親梱包_（納品管理番号）./
           KEY7-'F605!A',            /.梱包_./
           KEY8-'F302!A',            /.取引番号./
           KEY9-'F402!A',            /.行番号./
           RCDL-@DSP
    ...
    /. 納品／欠品明細合計計算 ./
    OVRF      FILE-CZDENTNF,TOFILE-CZDENTNF.BMSFLIB
    OVRF      FILE-CZNOMGKF,TOFILE-CZNOMGKF.BMSFLIB
    CALL      PGM-NKE1115B.TOKSOLIB
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4    :=   '納品／欠品明細合計計算'
              GOTO ABEND
    END
    ...
    /. 納品明細データ作成 ./
    OVRF      FILE-CZDENTNF,TOFILE-CZDENTNF.BMSFLIB
    OVRF      FILE-CZKEPTN2,TOFILE-CZKEPTN2.BMSFLIB
    OVRF      FILE-CZNOMGK1,TOFILE-CZNOMGK1.BMSFLIB
    OVRF      FILE-SUBTBLL1,TOFILE-SUBTBLL1.TOKDTLIB
    OVRF      FILE-CZNOMEI1,TOFILE-CZNOMEI1.BMSFLIB
    CALL      PGM-?PGC
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4    :=   '納品明細データ作成処理'
              GOTO ABEND
    ...
    /. 納品明細データ複写 ./
    CNVFILE   FILE-CZNOMEIF.BMSFLIB,
              TOFILE-CZNOMESF.BMSFLIB,
              ADD-@NO,BF-1
    IF        @PGMEC    ^=   0    THEN
              ?KEKA4    :=   '納品明細データ複写処理'
              GOTO ABEND
