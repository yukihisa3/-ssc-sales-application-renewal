...
**種別**: JCL  
**ライブラリ**: TOKCLLIB  
**ソースファイル**: `source/navs/cobol/programs/TOKCLLIB/PNA16000.CL`
...
/. ************************************************************  ./
    ...
    VAR       ?MSG    ,STRING*99(6)
    VAR       ?MSGX   ,STRING*99
    VAR       ?PGMID  ,STRING*8,VALUE-'PNA16000'
    VAR       ?STEP   ,STRING*8
    VAR       ?WKSTN  ,STRING*8
...
STEP010:
    ...
    CALL SNA1600I.TOKSOLIB,PARA-(?KBN,?JDATE,?JTIME,?JTORCD,?JSOKO,
...
STEP030:
    ...
    CALL SNA1610B.TOKSOLIB,PARA-(?JDATE,?JTIME,?JTORCD,?JSOKO,?JAITE)
    IF   @PGMEC    ^=   0    THEN
         ?KEKA4 :=  '【オンライン分　再取得】'
         GOTO ABEND
    END
...
STEP040:
    ...
    OVRF FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    OVRF FILE-MEIMS1,TOFILE-MEIMS1.TOKFLIB
    CALL SNA1620B.TOKSOLIB,PARA-(?TTORCD,?TSOKO,?TAITE,?TDENST,?TDENED)
    IF   @PGMEC    ^=   0    THEN
         ?KEKA4 :=  '【手書分　　　　再取得】'
         GOTO ABEND
    END
