...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSYK002B.COB`
...
*オージョイフル
     COPY   OJSHIRED OF XFDLIB  JOINING   OJS  AS   PREFIX.
*ダイキ
     COPY   DKSHIRED OF XFDLIB  JOINING   DKS  AS   PREFIX.
*    ｶｳﾝﾄ
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSYK002B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSYK002B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*    _番 2007/08/29 判定場所を変更
     IF     ( PARA-STANA     <=   SYU-F05     ) AND
            ( SYU-F05        <=   PARA-ETANA  )
              CONTINUE
     ELSE
              GO        TO        MAIN-010
...
*### 1999/11/22 NAV T.T START ###*
*### 2008/07/18 ST  取引先個別エリア ##
     EVALUATE DEN-F01
         WHEN 1681 WHEN 1682
              MOVE  DEN-REC  TO   OJS-REC
              MOVE  OJS-A18  TO   SYU-F20
              MOVE  SPACE    TO   SYU-F21  SYU-F22
         WHEN 100403  WHEN  1004031  WHEN  100427  WHEN  100441
              MOVE  DEN-REC  TO   DKS-REC
              MOVE  DKS-F42  TO   SYU-F22
              MOVE  SPACE    TO   SYU-F20  SYU-F21
         WHEN OTHER
              MOVE  SPACE    TO   SYU-F20
              MOVE  SPACE    TO   SYU-F21  SYU-F22
     END-EVALUATE.
*### 2008/07/18 ED  取引先個別エリア ##
     WRITE    SYU-REC.
     ADD      1              TO   WRT-CNT.
*
 MAIN-010.
...
*
     CLOSE     SHTDENLA  SYUMEIKF  SHOTBL1.
*
     STOP      RUN.
*
 END-EXIT.
