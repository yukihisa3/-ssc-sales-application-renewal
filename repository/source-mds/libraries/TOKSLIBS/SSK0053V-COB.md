...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSK0053V.COB`
...
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    サブシステム　　　　：　ケーヨー伝票レス　　　　　　　　　*
*    モジュール名　　　　：　返品計上確認リストＣＳＶデータ作成*
*    作成日／作成者　　　：　14/03/19  YOSHIDA/M               *
*    処理概要　　　　　　：　ケーヨー様からの返品伝票について、*
*    　　　　　　　　　　　　指定された範囲の返品計上確認リスト*
*    　　　　　　　　　　　　ＣＳＶデータを出力する。　　　　  *
*    更新日／更新者　　　：　17/11/29  TAKAHASHI               *
*    処理概要　　　　　　：　ケーヨー伝区を計上伝区に変更　　　*
****************************************************************
****************************************************************
...
*
*****<<返品CSVデータ >>*************************************
     SELECT   KEICSVHP           ASSIGN    TO   KEICSVHP
                                 STATUS         CSV-STATUS.
*                                                              *
****************************************************************
 DATA                   DIVISION.
...
*    FILE = 返品データＣＳＶ　　　　　　　　                   *
*--------------------------------------------------------------*
 FD  KEICSVHP           BLOCK CONTAINS 1    RECORDS.
 01  CSV-REC.
     03  FILLER         PIC       X(500).
*
*----------------------------------------------------------------*
...
***** メッセージエリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSK0053V".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSK0053V".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND1.
...
*             プロセス                      0.0                *
****************************************************************
 SSK0053V-START         SECTION.
*
     MOVE   "SSK0053V-START"      TO   S-NAME.
     PERFORM            INIT-SEC.
     IF   END-FLG  NOT =  "END"
...
*
     ...
     ADD      3                   TO   OUTPUT-CNT.
*　ブレイクキー設定
     MOVE     JHW-F02             TO   BRK-TENCD.
     MOVE     JHW-F04             TO   BRK-DENNO.
*
 INIT-EXIT.
     EXIT.
*
...
*
*  店舗・伝票番号ブレイク時
     IF      ( JHW-F02   NOT =   BRK-TENCD )  OR
             ( JHW-F04   NOT =   BRK-DENNO )
*  合計
              PERFORM   DENKEI-SEC
              MOVE   ZERO         TO    WK-DENKEI
              INITIALIZE                WK-DENKEI
              IF  ( JHW-F02   NOT =   BRK-TENCD )
...
*  明細行編集１
*  取引先ＣＤ
     MOVE     JHW-F01           TO    MD01-TORICD.
*  取引先正式名
     MOVE     JHW-F01           TO    TOK-F01.
     IF  RD-TOK-FLG = SPACE
     ...
     END-IF.
*  店舗ＣＤ
     MOVE     JHW-F02           TO    MD01-TENCD
*  店舗正式名
     MOVE     JHW-F02           TO    TEN-F011
     MOVE     173               TO    TEN-F52
     IF  RD-TEN-FLG = SPACE
     ...
     END-IF.
*  伝票番号
     MOVE     JHW-F04           TO    MD01-DENNO.
*  行番号
     MOVE     JHW-F05           TO    MD01-GYONO.
*  伝区
*#2017/11/29 NAV ST ケーヨー伝区→計上伝伝票区分へ変更
*****MOVE     JHW-F06           TO    MD01-DKBN.
     MOVE     JHW-F23           TO    MD01-DKBN.
*#2017/11/29 NAV ED
     MOVE     ":"               TO    MD01-DKBNC.
*  伝区名称
     MOVE     1                    TO        JYO-F01.
*#2017/11/29 NAV ST ケーヨー伝区→計上伝伝票区分へ変更
*****MOVE     JHW-F06              TO        JYO-F02.
     MOVE     JHW-F23              TO        JYO-F02.
*#2017/11/29 NAV ED
     ...
     END-IF.
*  検収日
     MOVE     JHW-F03              TO        MD01-KENSHBI.
*  実検収日
     MOVE     JHW-F07              TO        MD01-JIKENSHBI.
*  入力日
     MOVE     JHW-F83              TO        MD01-NYRYOKUBI.
*  計上日
     MOVE     JHW-F86              TO        MD01-KEIJYOBI.
*  出荷場所
     MOVE     JHW-F08              TO        MD01-SHUKABHO.
*  入力者ＣＤ
     MOVE     JHW-F82              TO        MD01-NYRYOKUCD.
*  担当者名
     MOVE     JHW-F81              TO        TAN-F01
     MOVE     JHW-F82              TO        TAN-F02
     ...
     END-IF.
*  相手先商品ＣＤ
     MOVE     JHW-F09              TO        MD01-ASHOCD.
*  商品ＣＤ
     MOVE     JHW-F10              TO        MD01-SHOCD.
*  品単１
     MOVE     JHW-F11              TO        MD01-HINTAN1.
*  品単２
     MOVE     JHW-F12              TO        MD01-HINTAN2.
*  品単３
     MOVE     JHW-F13              TO        MD01-HINTAN3.
*  商品名
     MOVE     JHW-F14              TO        MD01-SHONM1.
     MOVE     JHW-F15              TO        MD01-SHONM2.
*  数量　
     MOVE     JHW-F16              TO        MD01-SURYO.
*  原価単価
     MOVE     JHW-F17              TO        MD01-GENKA.
*  原価金額
     MOVE     JHW-F18              TO        MD01-KINGAKU.
*  明細備考
     MOVE     JHW-F21              TO        MD01-MEISAIBK.
*  伝票備考
     MOVE     JHW-F24              TO        GK01-DENMEI1.
     MOVE     JHW-F25              TO        GK01-DENMEI2.
*--------------
*  伝票計加算
*--------------
*  数量　
     COMPUTE  WK-SURYO-DEN    = WK-SURYO-DEN    +  JHW-F16.
*  原価金額
     COMPUTE  WK-KINGAKU-DEN  = WK-KINGAKU-DEN  +  JHW-F18.
*レコードセット
...
*計上区分
     ...
     END-EVALUATE.
*担当者
     MOVE     PARA-TANFROM      TO   HD02-TFROM.
     MOVE     PARA-TANTO        TO   HD02-TTO.
*日付区分・日付
     EVALUATE   PARA-DKBN
          WHEN   "2"
              MOVE     PARA-DFROM         TO   HD02-KFROM
              MOVE     PARA-DTO           TO   HD02-KTO
          WHEN   "6"
              MOVE     PARA-DFROM         TO   HD02-NFROM
              MOVE     PARA-DTO           TO   HD02-NTO
          WHEN   "7"
              MOVE     PARA-DFROM         TO   HD02-KJFROM
              MOVE     PARA-DTO           TO   HD02-KJTO
          WHEN   OTHER
...
*
     CLOSE    KEIJHWL1 TOKMS2 TENMS1 JYOKEN1 TANMS1 KEICSVHP.
