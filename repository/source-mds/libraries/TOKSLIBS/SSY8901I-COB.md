...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY8901I.COB`
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSY8901I DSPFILE ERROR " DSP-CNTL " "
...
*----<< 伝票データ >>--*
 RCSYUKF-ERR            SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY8901I RCSYUKF ERROR " RCSYUKF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTORCK-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY8901I HTOKMS ERROR " HTORCK-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY8901I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSY8901I.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SSY8901I"      TO   PGID.
     MOVE    "FSY8901I"      TO   FORM.
...
*    売上伝票データ存在チェック
     IF  ERR-FLG        =    ZERO
         ...
         INITIALIZE                    RCK-REC
         MOVE      JDATE          TO   RCK-K01
         MOVE      JTIME          TO   RCK-K02
         MOVE      TORICD         TO   RCK-K03
         MOVE      WK-SOKCD       TO   RCK-K04
              ...
              NOT INVALID
                   READ      RCSYUKF
                     AT END
                        ...
                        GO   TO   220-GRP01-CHECK-EXIT
                     NOT AT END
                        IF ( JDATE     =    RCK-K01 ) AND
                           ( JTIME     =    RCK-K02 ) AND
                           ( TORICD    =    RCK-K03 ) AND
                           ( STENCD    <=   RCK-K05 ) AND
                           ( ETENCD    >=   RCK-K05 )
*## 2008/03/11 ST *********( ETENCD    >=   RCK-K05)  AND
***************************( SNOUDT    <=   RCK-K08 ) AND
***************************( ENOUDT    >=   RCK-K08)
                             IF ( WK-SOKCD  NOT =    SPACE )
                                  IF   SOKO     =     RCK-K04
