# NJH2903B

**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/NJH2903B.COB`

## ソースコード

```cobol
****************************************************************
*                                                              *
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    サブシステム　　　　：　出荷管理　　　　　　　　　　　　　*
*    業務名　　　　　　　：　ベンダーオンライン　　　　　　　　*
*    モジュール名　　　　：　変換伝票データ作成　　　　　　　　*
*    作成日／更新日　　　：　10/09/08                          *
*    作成者／更新者　　　：　ＮＡＶ阿部　　　　　　　　　　　　*
*    処理概要　　　　　　：　（ＣＶＣＳ）オンラインデータから　*
*                            変換伝票データファイルを作成する　*
*                            ジャスコ用                        *
*　　2008/03/10北海道／関西へ振分け　　　　　　　　　　　　　*
*　                                                            *
****************************************************************
 IDENTIFICATION         DIVISION.
*
 PROGRAM-ID.            NJH2903B.
 AUTHOR.                NAV ABE.
 DATE-WRITTEN.          10/09/08.
*
 ENVIRONMENT            DIVISION.
 CONFIGURATION          SECTION.
 SOURCE-COMPUTER.       FUJITSU.
 OBJECT-COMPUTER.       FUJITSU.
 SPECIAL-NAMES.
     CONSOLE  IS        CONS.
 INPUT-OUTPUT           SECTION.
 FILE-CONTROL.
*受信データファイル
     SELECT   CVCSG001  ASSIGN    TO        DA-01-S-CVCSG001
                        ACCESS    MODE IS   SEQUENTIAL
                        FILE      STATUS    DEN-STATUS
                        ORGANIZATION   IS   SEQUENTIAL.
*変換伝票データ
     SELECT   JHSHENL1  ASSIGN    TO        DA-01-VI-JHSHENL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       HEN-F46
                                            HEN-F47   HEN-F01
                                            HEN-F02   HEN-F03
                        FILE  STATUS   IS   HEN-STATUS.
*取引先マスタ
     SELECT   TOKMS2    ASSIGN    TO        DA-01-VI-TOKMS2
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       TOK-F01
                        FILE  STATUS   IS   TOK-STATUS.
*商品変換テーブル
     SELECT   SHOTBL1   ASSIGN    TO        DA-01-VI-SHOTBL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       TBL-F01   TBL-F02
                        FILE STATUS    IS   TBL-STATUS.
*商品名称マスタ
     SELECT   MEIMS1    ASSIGN    TO        DA-01-VI-MEIMS1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       MEI-F011
                                            MEI-F012
                        FILE STATUS    IS   MEI-STATUS.
*ルート条件マスタ
     SELECT   JHMRUTL1  ASSIGN    TO        DA-01-VI-JHMRUTL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       RUT-F01   RUT-F02
                                            RUT-F03
                        FILE STATUS    IS   RUT-STATUS.
*出荷場所件数マスタ
     SELECT   JSMKENL1  ASSIGN    TO        DA-01-VI-JSMKENL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       KEN-F01   KEN-F02
                                            KEN-F03   KEN-F04
                        FILE  STATUS   IS   KEN-STATUS.
*当日スケジュールマスタ
     SELECT   JSMDAYL1  ASSIGN    TO        DA-01-VI-JSMDAYL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       TJS-F01  TJS-F02
                                            TJS-F03
                        FILE  STATUS   IS   TJS-STATUS.
*店舗マスタ
     SELECT   TENMS1    ASSIGN    TO        DA-01-VI-TENMS1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       TEN-F52  TEN-F011
                        FILE  STATUS   IS   TEN-STATUS.
*ＶＬＤ５００
     SELECT   VLD500    ASSIGN    TO        VLD500
                        FILE  STATUS   IS   VLD-STATUS.
*********
 DATA                   DIVISION.
 FILE                   SECTION.
******************************************************************
*    受信データ　ＲＬ＝　２５６　ＢＦ＝　１
******************************************************************
 FD  CVCSG001
                        BLOCK CONTAINS      1    RECORDS
                        LABEL RECORD   IS   STANDARD.
*
 01  DEN-REC.
     03  DEN-01                  OCCURS 2.
         05  DEN-01A             PIC  X(03).
         05  DEN-01B             PIC  X(25).
         05  DEN-01C             PIC  X(01).
         05  DEN-01D             PIC  X(99).
******************************************************************
*    取引先マスタ
******************************************************************
 FD  TOKMS2             LABEL RECORD   IS   STANDARD.
     COPY     HTOKMS    OF        XFDLIB
              JOINING   TOK       PREFIX.
******************************************************************
*    商品変換テーブル
******************************************************************
 FD  SHOTBL1            LABEL RECORD   IS   STANDARD.
     COPY     HSHOTBL   OF        XFDLIB
              JOINING   TBL       PREFIX.
******************************************************************
*    商品名称マスタ
******************************************************************
 FD  MEIMS1             LABEL RECORD   IS   STANDARD.
     COPY     HMEIMS    OF        XFDLIB
              JOINING   MEI       PREFIX.
******************************************************************
*    ルート条件マスタ
******************************************************************
 FD  JHMRUTL1           LABEL RECORD   IS   STANDARD.
     COPY     JHMRUTF   OF        XFDLIB
              JOINING   RUT       PREFIX.
******************************************************************
*    出荷場所別件数ファイル
******************************************************************
 FD  JSMKENL1           LABEL RECORD   IS   STANDARD.
     COPY     JHMKENF   OF        XFDLIB
              JOINING   KEN       PREFIX.
******************************************************************
*    当日スケジュールマスタ
******************************************************************
 FD  JSMDAYL1           LABEL RECORD   IS   STANDARD.
     COPY     JSMDAYF   OF        XFDLIB
              JOINING   TJS       PREFIX.
******************************************************************
*    変換伝票データ　ＲＬ＝１０２０
******************************************************************
 FD  JHSHENL1
                        LABEL RECORD   IS   STANDARD.
     COPY     JUSHIRED  OF        XFDLIB
              JOINING   HEN  AS   PREFIX.
******************************************************************
*    店舗マスタ
******************************************************************
 FD  TENMS1             LABEL RECORD   IS   STANDARD.
     COPY     HTENMS    OF        XFDLIB
              JOINING   TEN       PREFIX.
*
******************************************************************
*    ＶＬＤ５００
******************************************************************
 FD  VLD500.
 01  VLD-REC.
     03  VLD-F01           PIC  X(02).
     03  VLD-F02           PIC  9(03).
     03  VLD-F03           PIC  X(02).
     03  VLD-F04           PIC  X(08).
     03  VLD-F05           PIC  9(06).
     03  VLD-F06           PIC  9(01).
     03  VLD-F07           PIC  X(02).
     03  VLD-F08           PIC  9(02).
     03  VLD-F09           PIC  9(02).
     03  VLD-F10           PIC  9(04).
     03  VLD-F11           PIC  9(08).
     03  VLD-F12           PIC  9(04).
     03  VLD-F13           PIC  9(08).
     03  FILLER            PIC  X(48).
*
*****************************************************************
*
 WORKING-STORAGE        SECTION.
*    ｶｳﾝﾄ
 01  END-FG                  PIC  9(01)     VALUE  ZERO.
 01  IDX                     PIC  9(02)     VALUE  ZERO.
 01  RD-CNT                  PIC  9(08)     VALUE  ZERO.
 01  WRT-CNT                 PIC  9(08)     VALUE  ZERO.
 01  CNT-KENSU               PIC  9(08)     VALUE  ZERO.
 01  CNT-KENSU-D             PIC  9(08)     VALUE  ZERO.
 01  CNT-MAISU               PIC  9(08)     VALUE  ZERO.
 01  CNT-KENSU1              PIC  9(08)     VALUE  ZERO.
 01  CNT-KENSU-D1            PIC  9(08)     VALUE  ZERO.
 01  CNT-MAISU1              PIC  9(08)     VALUE  ZERO.
 01  CNT-GYO                 PIC  9(02)     VALUE  ZERO.
 01  INV-RUT                 PIC  9(01)     VALUE  ZERO.
 01  FLG-TOK                 PIC  9(01)     VALUE  ZERO.
 01  WK-TOKCD                PIC  9(08)     VALUE  ZERO.
 01  WK-RUTO                 PIC  X(02)     VALUE  SPACE.
 01  WK-TENPO-CHK1           PIC  X(01)     VALUE  SPACE.
 01  WK-TENPO-CHK2           PIC  X(01)     VALUE  SPACE.
 01  WK-TOKCD1               PIC  9(08)     VALUE  ZERO.
 01  WK-TOKCD2               PIC  9(08)     VALUE  ZERO.
 01  HTENMS-INV-FLG          PIC  X(03)     VALUE  SPACE.
 01  WK-DENPYO-NO            PIC  9(09)     VALUE  ZERO.
*
*データ退避ワーク
 01  WK-JSA-REC.
     03  WK-JSA.
         05  JSA-F01              PIC  X(03).
         05  JSA-F02.
             07  JSA-F021         PIC  X(06).
             07  JSA-F022         PIC  X(02).
         05  JSA-F03              PIC  9(05).
         05  JSA-F04              PIC  X(02).
         05  JSA-F05              PIC  X(10).
         05  JSA-F06              PIC  X(20).
         05  JSA-F07              PIC  X(01).
 01  WK-JSBH-REC.
     03  WK-JSBH.
         05  JSBH-F01             PIC  X(03).
         05  JSBH-F02.
             07  JSBH-F021        PIC  X(06).
             07  JSBH-F022        PIC  X(02).
         05  JSBH-F03             PIC  9(05).
         05  JSBH-F04             PIC  X(02).
         05  JSBH-F05             PIC  9(07).
         05  JSBH-F06             PIC  9(02).
         05  JSBH-F07             PIC  X(01).
         05  JSBH-F08             PIC  X(01).
         05  JSBH-F09             PIC  9(05).
         05  JSBH-F10             PIC  X(06).
         05  JSBH-F11             PIC  X(12).
         05  JSBH-F12             PIC  X(04).
         05  JSBH-F13             PIC  X(01).
         05  JSBH-F14             PIC  X(02).
         05  JSBH-F15.
             07  JSBH-F151        PIC  X(01).
             07  JSBH-F152        PIC  X(02).
         05  JSBH-F16             PIC  X(01).
         05  JSBH-F17             PIC  X(06).
         05  JSBH-F18             PIC  9(06).
         05  JSBH-F19             PIC  X(04).
         05  JSBH-F20             PIC  9(06).
         05  JSBH-F21             PIC  X(12).
         05  JSBH-F22.
             07  JSBH-F221        PIC  X(13).
             07  JSBH-F222        PIC  X(01).
             07  JSBH-F223        PIC  X(01).
         05  JSBH-F23.
             07  JSBH-F231        PIC  X(08).
             07  JSBH-F232        PIC  X(07).
             07  JSBH-F233        PIC  X(01).
 01  WK-JSB-REC.
     03  WK-JSB.
         05  JSB-F01              PIC  X(03).
         05  JSB-F02.
             07  JSB-F021         PIC  X(06).
             07  JSB-F022         PIC  X(02).
         05  JSB-F03              PIC  9(05).
         05  JSB-F04              PIC  X(02).
         05  JSB-F05              PIC  X(07).
         05  JSB-F06              PIC  9(02).
         05  JSB-F07              PIC  X(01).
         05  JSB-F08              PIC  X(01).
         05  JSB-F09              PIC  X(20).
         05  JSB-F10              PIC  X(13).
         05  JSB-F11              PIC  X(20).
         05  JSB-F12              PIC  X(06).
         05  JSB-F13              PIC  9(05)V9.
         05  JSB-F14              PIC  9(06)V99.
         05  JSB-F15              PIC  9(06).
         05  JSB-F16              PIC  X(02).
         05  JSB-F17              PIC  X(01).
         05  JSB-F18              PIC  X(03).
         05  JSB-F19              PIC  X(12).
         05  JSB-F20              PIC  X(02).
*
 01  WK-AREA.
*システム日付の編集
     03  SYS-DATE          PIC 9(06).
     03  SYS-DATEW         PIC 9(08).
 01  WK-ST.
     03  DEN-STATUS        PIC  X(02).
     03  HEN-STATUS        PIC  X(02).
     03  TOK-STATUS        PIC  X(02).
     03  TBL-STATUS        PIC  X(02).
     03  MEI-STATUS        PIC  X(02).
     03  RUT-STATUS        PIC  X(02).
     03  KEN-STATUS        PIC  X(02).
     03  TJS-STATUS        PIC  X(02).
     03  VLD-STATUS        PIC  X(02).
     03  TEN-STATUS        PIC  X(02).
*
 01  MSG-AREA.
     03  MSG-START.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  ST-PG          PIC   X(08)  VALUE "SJH2901B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH2901B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH2901B".
         05  FILLER         PIC   X(11)  VALUE
                                         " ABEND *** ".
     03  ABEND-FILE.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  AB-FILE        PIC   X(08).
         05  FILLER         PIC   X(06)  VALUE " ST = ".
         05  AB-STS         PIC   X(02).
         05  FILLER         PIC   X(05)  VALUE " *** ".
     03  SEC-NAME.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  FILLER         PIC   X(07)  VALUE " SEC = ".
         05  S-NAME         PIC   X(30).
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  FILLER         PIC   X(09)  VALUE " INPUT = ".
         05  IN-CNT         PIC   9(06).
         05  FILLER         PIC   X(05)  VALUE " *** ".
     03  MSG-OUT.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  FILLER         PIC   X(09)  VALUE " OUTPUT= ".
         05  OUT-CNT        PIC   9(06).
         05  FILLER         PIC   X(05)  VALUE " *** ".
*
*取引先コード変換
 01  WK-TORICD             PIC  X(05).
 01  WK-TORICD-R           REDEFINES   WK-TORICD.
     03  WK-TORICD-H       PIC  9(05).
*伝票番号変換
 01  WK-DENNO              PIC  X(07).
 01  WK-DENNO-R            REDEFINES   WK-DENNO.
     03  WK-DENNO-H        PIC  9(07).
*店舗コード変換
 01  WK-TENPO              PIC X(04).
 01  WK-TENPO-R            REDEFINES   WK-TENPO.
     03  WK-TENPO-H        PIC 9(04).
*
 01  LINK-AREA.
     03  LINK-IN-KBN        PIC   X(01).
     03  LINK-IN-YMD6       PIC   9(06).
     03  LINK-IN-YMD8       PIC   9(08).
     03  LINK-OUT-RET       PIC   X(01).
     03  LINK-OUT-YMD8      PIC   9(08).
*
 LINKAGE                SECTION.
 01  PARA-AREA.
     03  PARA-JDATE         PIC   9(08).
     03  PARA-JTIME         PIC   9(04).
     03  PARA-KSYU          PIC   X(01).
     03  PARA-YUSEN         PIC   X(01).
*
******************************************************************
*             M A I N             M O D U L E                    *
******************************************************************
 PROCEDURE              DIVISION USING PARA-AREA.
 DECLARATIVES.
 FILEERR-SEC1           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   CVCSG001.
     MOVE      "CVCSG001"   TO   AB-FILE.
     MOVE      DEN-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC2           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   JHSHENL1.
     MOVE      "JHSHENL1"   TO   AB-FILE.
     MOVE      HEN-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC3           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   TOKMS2.
     MOVE      "TOKMS2"     TO   AB-FILE.
     MOVE      TOK-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC4           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   SHOTBL1.
     MOVE      "SHOTBL1"    TO   AB-FILE.
     MOVE      TBL-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC5           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   MEIMS1.
     MOVE      "MEIMS1"     TO   AB-FILE.
     MOVE      MEI-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC6           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   JSMKENL1.
     MOVE      "JSMKENL1"   TO   AB-FILE.
     MOVE      KEN-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC7           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   JSMDAYL1.
     MOVE      "JSMDAYL1"   TO   AB-FILE.
     MOVE      TJS-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC8           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   JHMRUTL1.
     MOVE      "JHMRUTL1"   TO   AB-FILE.
     MOVE      RUT-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC9           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   TENMS1.
     MOVE      "TENMS1"     TO   AB-FILE.
     MOVE      TEN-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 FILEERR-SEC10          SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   VLD500.
     MOVE      "VLD500  "   TO   AB-FILE.
     MOVE      VLD-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
     STOP      RUN.
*
 END     DECLARATIVES.
*****************************************************************
*                                                                *
******************************************************************
 GENERAL-PROCESS       SECTION.
*
     MOVE     "PROCESS-START"     TO   S-NAME.
     PERFORM  INIT-SEC.
     PERFORM  MAIN-SEC
              UNTIL     END-FG    =    9.
     PERFORM  END-SEC.
*
****************************************************************
*　　　　　　　初期処理　　　　　　　　　　　　　　　　　　　　*
****************************************************************
 INIT-SEC               SECTION.
     MOVE     "INIT-SEC"          TO   S-NAME.
     OPEN     INPUT     CVCSG001
                        SHOTBL1   MEIMS1
                        JHMRUTL1  TOKMS2  TENMS1.
     OPEN     EXTEND    JHSHENL1.
     OPEN     I-O       JSMKENL1  JSMDAYL1.
     OPEN     OUTPUT    VLD500.
     DISPLAY  MSG-START UPON CONS.
*
     MOVE     ZERO      TO        END-FG    RD-CNT    WRT-CNT.
     MOVE     ZERO      TO        IN-CNT    OUT-CNT.
     MOVE     SPACE     TO        WK-JSA-REC.
     INITIALIZE                   WK-JSA-REC.
     MOVE     SPACE     TO        WK-JSBH-REC.
     INITIALIZE                   WK-JSBH-REC.
     MOVE     SPACE     TO        WK-JSB-REC.
     INITIALIZE                   WK-JSB-REC.
*
******************
*システム日付編集*
******************
     ACCEPT      SYS-DATE  FROM      DATE.
     MOVE       "3"        TO        LINK-IN-KBN.
     MOVE        SYS-DATE  TO        LINK-IN-YMD6.
     CALL       "SKYDTCKB"   USING   LINK-IN-KBN
                                     LINK-IN-YMD6
                                     LINK-IN-YMD8
                                     LINK-OUT-RET
                                     LINK-OUT-YMD8.
     IF          LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8  TO   SYS-DATEW
     ELSE
         MOVE    ZERO           TO   SYS-DATEW
     END-IF.
*
     READ     CVCSG001
              AT END    MOVE      9         TO  END-FG
              NOT AT END
                        ADD       1         TO  RD-CNT
     END-READ.
*
 INIT-EXIT.
     EXIT.
****************************************************************
*　　　　　　　メイン処理　　　　　　　　　　　　　　　　　　　*
****************************************************************
 MAIN-SEC     SECTION.
*
     MOVE    "MAIN-SEC"          TO   S-NAME.
     PERFORM  VARYING   IDX      FROM      1  BY  1
              UNTIL     IDX      >         2
*
*データ情報処理
        IF    DEN-01A(IDX)  =    "JSA"
              MOVE   SPACE        TO   WK-JSA-REC
              INITIALIZE               WK-JSA-REC
              MOVE   DEN-01(IDX)  TO   WK-JSA-REC
        END-IF
*
        IF    DEN-01A(IDX)  =    "JSB"
              IF   DEN-01C(IDX)   =    "H"
*                  伝票ヘッダ処理
*                  出荷場所件数マスタ出力
                   IF   CNT-MAISU     >    ZERO
                   OR   CNT-MAISU1    >    ZERO
                        PERFORM  JSMKENL1-WRT-SEC
                   END-IF
                   MOVE      SPACE          TO   WK-JSBH-REC
                   INITIALIZE                    WK-JSBH-REC
                   MOVE      DEN-01(IDX)    TO   WK-JSBH-REC
*******************ADD       1              TO   CNT-MAISU
*******************IF  WK-TOKCD = 87372
***********************ADD   1   TO   CNT-KENSU
***********************ADD   1   TO   CNT-KENSU-D
*******************END-IF
*******************IF  WK-TOKCD = 87373
***********************ADD   1   TO   CNT-KENSU1
***********************ADD   1   TO   CNT-KENSU-D1
*******************END-IF
                   MOVE      ZERO           TO   CNT-KENSU-D
                   MOVE      ZERO           TO   CNT-KENSU-D1
                   MOVE      ZERO           TO   CNT-GYO
              ELSE
                   IF   DEN-01C(IDX)   =   "0"
                        CONTINUE
                   ELSE
*                       明細レコード
                        MOVE     SPACE      TO   WK-JSB-REC
                        INITIALIZE               WK-JSB-REC
                        MOVE    DEN-01(IDX) TO   WK-JSB-REC
************************ADD      1          TO   CNT-KENSU
************************ADD      1          TO   CNT-KENSU-D
                        PERFORM  EDIT-SEC
                       IF  WK-TOKCD = 87372
                           ADD   1   TO   CNT-KENSU
                           ADD   1   TO   CNT-KENSU-D
                       END-IF
                       IF  WK-TOKCD = 87373
                           ADD   1   TO   CNT-KENSU1
                           ADD   1   TO   CNT-KENSU-D1
                       END-IF
                   END-IF
              END-IF
        END-IF
*
     END-PERFORM.
*
     READ     CVCSG001
              AT END    MOVE      9         TO  END-FG
              NOT AT END
                        ADD       1         TO  RD-CNT
     END-READ.
*
 MAIN-EXIT.
     EXIT.
****************************************************************
*　　　　　　　ファイル出力　　　　　　　　　　　　　　　　　　*
****************************************************************
 EDIT-SEC              SECTION.
*
     MOVE    "EDIT-SEC"     TO    S-NAME.
     MOVE     SPACE         TO    HEN-REC.
     INITIALIZE                   HEN-REC.
*各量販店領域転送
*取引先コード
     MOVE     JSA-F021(1:5) TO    WK-TORICD.
     MOVE     WK-TORICD-H   TO    HEN-A01.
*ＳＡ
     MOVE     JSA-F022      TO    HEN-A02.
*伝票番号
     MOVE     JSBH-F05      TO    WK-DENNO.
     MOVE     WK-DENNO-H    TO    HEN-A03.
*行番号
     MOVE     JSB-F06       TO    HEN-A04.
*システム区分
     MOVE     JSA-F07       TO    HEN-A05.
*大分類
     MOVE     JSBH-F12      TO    HEN-A06.
*伝票区分
     MOVE     JSBH-F14      TO    HEN-A07.
*配送区分（便区分）
     MOVE     JSBH-F151     TO    HEN-A081.
*配送区分（便名）
     MOVE     JSBH-F152     TO    HEN-A082.
*発注区分
     MOVE     JSBH-F16      TO    HEN-A09.
*取引先名称
     MOVE     JSA-F06       TO    HEN-A10.
*社名
     MOVE     JSBH-F21      TO    HEN-A11.
*発注日
     MOVE     JSBH-F18      TO    HEN-A12.
*納品日
     MOVE     JSBH-F20      TO    HEN-A13.
*ルート
*取引先品番／ＪＡＮコード
     MOVE     JSB-F10       TO    HEN-A15.
*商品名称
     MOVE     JSB-F09       TO    HEN-A16.
*規格
     MOVE     JSB-F11       TO    HEN-A17.
*商品発注コード
     MOVE     JSB-F12       TO    HEN-A18.
*数量
     MOVE     JSB-F13       TO    HEN-A19.
*原価単価
     MOVE     JSB-F14       TO    HEN-A20.
*売価単価
     MOVE     JSB-F15       TO    HEN-A21.
*単位
     MOVE     JSB-F17       TO    HEN-A22.
*入数
     MOVE     JSB-F18       TO    HEN-A23.
*クラス
     MOVE     JSB-F20       TO    HEN-A24.
*原価金額
     COMPUTE  HEN-A25  ROUNDED  =  JSB-F13  *  JSB-F14.
*売価金額
     COMPUTE  HEN-A26  ROUNDED  =  JSB-F13  *  JSB-F15.
*店コード
     MOVE     JSBH-F10      TO    HEN-A27.
*店舗名称
     MOVE     JSBH-F11      TO    HEN-A28.
*伝票枚数
     MOVE     ZERO          TO    HEN-A29.
*伝票番号ＳＥＱ－ＮＯ
     MOVE     JSBH-F09      TO    HEN-A30.
*納品経路
     MOVE     JSBH-F22      TO    HEN-A31.
*階_
     MOVE     JSBH-F13      TO    HEN-A32.
*チラシ
     MOVE     JSBH-F19      TO    HEN-A33.
*Ｆ欄上段
     MOVE     JSBH-F23      TO    HEN-A34.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
     ADD      1             TO   WRT-CNT.
*
 EDIT-EXIT.
     EXIT.
****************************************************************
*　　　　　　　変換伝票転送                                    *
****************************************************************
 TENSO-SEC             SECTION.
*
     MOVE    "TENSO-SEC"    TO    S-NAME.
*取引先コード
*****MOVE     HEN-A01       TO    HEN-F01.
     MOVE     87372         TO    HEN-F01.
*店コード
     MOVE     HEN-A27(1:4)  TO    WK-TENPO.
     MOVE     WK-TENPO-H    TO    HEN-F07.
*店舗マスタ検索（センター区分）
     MOVE        SPACE      TO    TEN-REC.
     INITIALIZE                   TEN-REC.
*****イオン振分用取引先
     MOVE        87371      TO    TEN-F52.
     MOVE        HEN-F07    TO    TEN-F011.
     READ    TENMS1
       INVALID     MOVE  "INV"   TO        HTENMS-INV-FLG
       NOT INVALID MOVE  SPACE   TO        HTENMS-INV-FLG
     END-READ.
*計上取引先判定
     IF  HTENMS-INV-FLG = SPACE
         EVALUATE TEN-F77
             WHEN 3
                 MOVE     1      TO        WK-TENPO-CHK1
                 MOVE     87372  TO        WK-TOKCD1
                 MOVE     87372  TO        HEN-F01  WK-TOKCD
             WHEN 6
                 MOVE     1      TO        WK-TENPO-CHK2
                 MOVE     87373  TO        WK-TOKCD2
                 MOVE     87373  TO        HEN-F01  WK-TOKCD
             WHEN OTHER
                 MOVE     1      TO        WK-TENPO-CHK1
                 MOVE     87372  TO        WK-TOKCD1
                 MOVE     87372  TO        HEN-F01  WK-TOKCD
         END-EVALUATE
     ELSE
         MOVE     1      TO        WK-TENPO-CHK1
         MOVE     87372  TO        WK-TOKCD1
         MOVE     87372  TO        HEN-F01  WK-TOKCD
     END-IF.
*伝票ナンバー
     MOVE     HEN-A03       TO    HEN-F02.
     MOVE     HEN-F02       TO    HEN-F23.
*行_
     MOVE     HEN-A04       TO    HEN-F03.
*取区
     MOVE     40            TO    HEN-F051.
     MOVE     NC"売上伝票"  TO    HEN-F052.
*担当者コード
     MOVE     99            TO    HEN-F06.
*  商品変換テーブル検索
     MOVE     SPACE         TO    TBL-REC.
     INITIALIZE                   TBL-REC.
     MOVE     HEN-F01       TO    TBL-F01.
     MOVE     HEN-A15       TO    TBL-F02.
     READ     SHOTBL1
       INVALID
         MOVE      SPACE    TO    TBL-REC
         INITIALIZE               TBL-REC
     END-READ.
*出荷場所／伝発場所
     MOVE     TBL-F04       TO    HEN-F08
                                  HEN-F09.
*振分ルート取得
     IF  HEN-F02  NOT =  WK-DENPYO-NO
         MOVE   HEN-F02    TO        WK-DENPYO-NO
         IF   WK-TOKCD = 87372
              ADD    1          TO        CNT-MAISU
         ELSE
              ADD    1          TO        CNT-MAISU1
         END-IF
*********PERFORM JSMKENL1-WRT-SEC
     END-IF.
*発注日  (年を条件ファイルの置換値とプラスして西暦４桁にする）
     MOVE    "3"            TO    LINK-IN-KBN.
     MOVE     HEN-A12       TO    LINK-IN-YMD6.
     CALL    "SKYDTCKB"  USING    LINK-IN-KBN
                                  LINK-IN-YMD6
                                  LINK-IN-YMD8
                                  LINK-OUT-RET
                                  LINK-OUT-YMD8.
     IF       LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8    TO   HEN-F111
     ELSE
         MOVE    ZERO             TO   HEN-F111
     END-IF.
*発注日  (年を条件ファイルの置換値とプラスして西暦４桁にする）
     MOVE    "3"            TO    LINK-IN-KBN.
     MOVE     HEN-A13       TO    LINK-IN-YMD6.
     CALL    "SKYDTCKB"  USING    LINK-IN-KBN
                                  LINK-IN-YMD6
                                  LINK-IN-YMD8
                                  LINK-OUT-RET
                                  LINK-OUT-YMD8.
     IF       LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8    TO   HEN-F112
     ELSE
         MOVE    ZERO             TO   HEN-F112
     END-IF.
*分類（部門）
     MOVE     HEN-A06       TO    HEN-F12.
*商品区分　　
     MOVE     SPACE         TO    HEN-F131.
*伝票区分　　
     MOVE     HEN-A07       TO    HEN-F132.
*伝発区分
     MOVE     9             TO    HEN-F134.
*自社商品コード
     MOVE     TBL-F031      TO    HEN-F1411.
*自社商品単品コード
     MOVE     TBL-F032      TO    HEN-F1412.
*商品名　　　
*    商品名称マスタ検索
     MOVE     SPACE         TO    MEI-REC.
     INITIALIZE                   MEI-REC.
     MOVE     TBL-F031      TO    MEI-F011.
     MOVE     TBL-F032      TO    MEI-F012.
     READ     MEIMS1
       INVALID
*        商品名（商品名）
         MOVE     HEN-A16(1:15)  TO   HEN-F1421
         MOVE     HEN-A16(16:5)  TO   HEN-F1422(1:5)
*        商品名（規格）
         MOVE     HEN-A17(1:10)  TO   HEN-F1422(6:10)
       NOT INVALID
*        商品名（商品名）
         MOVE     HEN-A16(1:15)  TO   HEN-F1421
         MOVE     HEN-A16(16:5)  TO   HEN-F1422(1:5)
*        商品名（規格）
         MOVE     HEN-A17(1:10)  TO   HEN-F1422(6:10)
     END-READ.
*数量
     MOVE     HEN-A19       TO    HEN-F15.
*単
     MOVE    "1"            TO    HEN-F16.
*原価単価
     MOVE     HEN-A20       TO    HEN-F172.
*売価単価
     MOVE     HEN-A21       TO    HEN-F173.
*原価金額
     MOVE     HEN-A25       TO    HEN-F181.
*売価金額
     MOVE     HEN-A26       TO    HEN-F182.
*店舗名（備考）
*****MOVE     HEN-A28       TO    HEN-F22.
*自社得意先コード
*****IF  FLG-TOK   =    ZERO
*    得意先マスタ検索
         MOVE      SPACE    TO    TOK-REC.
         INITIALIZE               TOK-REC.
         MOVE      HEN-F01  TO    TOK-F01.
         READ      TOKMS2
            INVALID
              MOVE   SPACE  TO    TOK-REC
              INITIALIZE          TOK-REC
         END-READ.
         MOVE      TOK-F52  TO    HEN-F24.
*********MOVE      1        TO    FLG-TOK
*****ELSE
*****    MOVE      TOK-F52  TO    HEN-F24
*****END-IF.
*相手商品コード（ＪＡＮコードの転送に変更）97/10/22
     MOVE     HEN-A15       TO    HEN-F25
*伝票発行区分
     MOVE     9             TO    HEN-F272.
*オンライン区分
     MOVE     1             TO    HEN-F274.
*エントリー区分
     MOVE     1             TO    HEN-F275.
*付番区分
     MOVE     9             TO    HEN-F276.
*量販店区分
     MOVE    "A"            TO    HEN-F278.
*ＷＳ_
     MOVE     1             TO    HEN-F28.
*変換値
*****MOVE     WK-NEN        TO    HEN-F29.
*店舗名（カナ）
     MOVE     HEN-A28       TO    HEN-F30.
*システム日付
     MOVE     SYS-DATEW     TO    HEN-F99.
*受信日付
     MOVE     PARA-JDATE    TO    HEN-F46.
*受信時刻
     MOVE     PARA-JTIME    TO    HEN-F47.
*振分倉庫コード
*    ルート条件マスタ検索
     MOVE     ZERO          TO    INV-RUT.
     MOVE     SPACE         TO    RUT-REC.
     INITIALIZE                   RUT-REC.
     MOVE     HEN-F01       TO    RUT-F01.
     MOVE     SPACE         TO    RUT-F02.
     MOVE     SPACE         TO    RUT-F03  HEN-F42.
     READ     JHMRUTL1
         INVALID
           MOVE   1         TO    INV-RUT
           MOVE   TOK-F81   TO    HEN-F48  WK-RUTO
         NOT INVALID
           MOVE   RUT-F05   TO    HEN-F48  WK-RUTO
     END-READ.
*_番
     MOVE     TBL-F08       TO    HEN-F49.
*訂正前数量
     MOVE     HEN-A19       TO    HEN-F50.
*修正原価単価
     MOVE     HEN-A20       TO    HEN-F512.
*修正売価単価
     MOVE     HEN-A21       TO    HEN-F513.
*修正原価金額
     MOVE     HEN-A25       TO    HEN-F521.
*修正売価金額
     MOVE     HEN-A26       TO    HEN-F522.
*
 TENSO-EXIT.
     EXIT.
****************************************************************
*　　　　　　　出荷場所件数マスタ出力                          *
****************************************************************
 JSMKENL1-WRT-SEC        SECTION.
*
     MOVE   "JSMKENL1-WRT-SEC"  TO   S-NAME.
     MOVE    SPACE         TO        KEN-REC.
     INITIALIZE                      KEN-REC.
     MOVE    PARA-JDATE    TO        KEN-F01.
     MOVE    PARA-JTIME    TO        KEN-F02.
     MOVE    WK-TOKCD      TO        KEN-F03.
     IF      INV-RUT       =         ZERO
             MOVE  RUT-F05     TO    KEN-F04
     ELSE
             MOVE  TOK-F81     TO    KEN-F04
     END-IF.
*****MOVE    WK-RUTO       TO        KEN-F04.
     READ    JSMKENL1
       INVALID
         CONTINUE
       NOT INVALID
         GO  TO   JSMKENL1-010
     END-READ.
*
     MOVE    SPACE         TO        KEN-REC.
     INITIALIZE                      KEN-REC.
     MOVE    PARA-JDATE    TO        KEN-F01.
     MOVE    PARA-JTIME    TO        KEN-F02.
     MOVE    WK-TOKCD      TO        KEN-F03.
     IF      INV-RUT       =         ZERO
             MOVE  RUT-F05     TO    KEN-F04
     ELSE
             MOVE  TOK-F81     TO    KEN-F04
     END-IF.
*****MOVE    WK-RUTO       TO        KEN-F04.
     MOVE    PARA-KSYU     TO        KEN-F05.
     MOVE    PARA-YUSEN    TO        KEN-F06.
     IF      WK-TOKCD  =  87372
             MOVE    CNT-KENSU-D   TO        KEN-F10
     END-IF.
     IF      WK-TOKCD  =  87373
             MOVE    CNT-KENSU-D1  TO        KEN-F10
     END-IF.
*****MOVE    CNT-KENSU-D   TO        KEN-F10.
     MOVE    1             TO        KEN-F11.
     WRITE   KEN-REC.
     GO      TO   JSMKENL1-WRT-EXIT.
*
 JSMKENL1-010.
*
     MOVE    PARA-KSYU     TO        KEN-F05.
     MOVE    PARA-YUSEN    TO        KEN-F06.
*****ADD     CNT-KENSU-D   TO        KEN-F10.
     IF      WK-TOKCD  =  87372
             ADD     CNT-KENSU-D   TO        KEN-F10
     END-IF.
     IF      WK-TOKCD  =  87373
             ADD     CNT-KENSU-D1  TO        KEN-F10
     END-IF.
     ADD     1             TO        KEN-F11.
     REWRITE KEN-REC.
*
 JSMKENL1-WRT-EXIT.
     EXIT.
****************************************************************
*　　　　　　　終了処理　　　　　　　　　　　　　　　　　　　　*
****************************************************************
 END-SEC       SECTION.
*
     MOVE     "END-SEC"  TO      S-NAME.
*
*****IF        WK-TENPO-CHK1 = "1"
***************出荷場所件数マスタ出力
***************MOVE      WK-TOKCD1    TO   WK-TOKCD
               PERFORM   JSMKENL1-WRT-SEC.
*****END-IF.
*
*****IF        WK-TENPO-CHK2 = "1"
***************出荷場所件数マスタ出力
***************MOVE      WK-TOKCD2    TO   WK-TOKCD
***************PERFORM   JSMKENL1-WRT-SEC
*****END-IF.
*
     IF        CNT-MAISU     >    ZERO
*              出荷場所件数マスタ出力
***************PERFORM  JSMKENL1-WRT-SEC
*              当日スケジュールマスタ出力
               PERFORM   JSMDAYL1-WRT-SEC
     END-IF.
     IF        CNT-MAISU1    >    ZERO
*              出荷場所件数マスタ出力
***************PERFORM   JSMKENL1-WRT-SEC
*              当日スケジュールマスタ出力
               PERFORM   JHMTJSL2-WRT-SEC
     END-IF.
*
     MOVE      RD-CNT    TO      IN-CNT.
     MOVE      WRT-CNT   TO      OUT-CNT.
     DISPLAY   MSG-IN    UPON CONS.
     DISPLAY   MSG-OUT   UPON CONS.
     DISPLAY   MSG-END   UPON CONS.
*
     CLOSE     CVCSG001  JHSHENL1
               SHOTBL1   MEIMS1
               JHMRUTL1  TOKMS2
               JSMKENL1  JSMDAYL1.
*
*    ＶＬＤＦ出力処理
     IF        CNT-MAISU     >    ZERO
     OR        CNT-MAISU1    >    ZERO
               PERFORM   VLD500-OUTPUT-SEC
     END-IF.
     CLOSE     VLD500.
*
     STOP      RUN.
*
 END-EXIT.
     EXIT.
****************************************************************
*　　　　　　　当日スケジュールマスタ出力　　　　　　　　　　　*
****************************************************************
 JSMDAYL1-WRT-SEC        SECTION.
*
     MOVE   "JSMDAYL1-WRT-SEC"  TO   S-NAME.
     MOVE    SPACE         TO        TJS-REC.
     INITIALIZE                      TJS-REC.
     MOVE    PARA-JDATE    TO        TJS-F01.
     MOVE    PARA-JTIME    TO        TJS-F02.
     MOVE    WK-TOKCD1     TO        TJS-F03.
     READ    JSMDAYL1
       INVALID
         CONTINUE
       NOT INVALID
         GO  TO   JSMDAYL1-010
     END-READ.
*
     MOVE    SPACE         TO        TJS-REC.
     INITIALIZE                      TJS-REC.
     MOVE    PARA-JDATE    TO        TJS-F01.
     MOVE    PARA-JTIME    TO        TJS-F02.
     MOVE    WK-TOKCD1     TO        TJS-F03.
     MOVE    CNT-KENSU     TO        TJS-F09.
     MOVE    CNT-MAISU     TO        TJS-F10.
     WRITE   TJS-REC.
     GO      TO   JSMDAYL1-WRT-EXIT.
*
 JSMDAYL1-010.
*
     MOVE    CNT-KENSU     TO        TJS-F09.
     MOVE    CNT-MAISU     TO        TJS-F10.
     REWRITE TJS-REC.
*
 JSMDAYL1-WRT-EXIT.
     EXIT.
****************************************************************
*　　　　　　　当日スケジュールマスタ出力　　　　　　　　　　　*
****************************************************************
 JHMTJSL2-WRT-SEC        SECTION.
*
     MOVE   "JHMTJSL2-WRT-SEC"  TO   S-NAME.
     MOVE    SPACE         TO        TJS-REC.
     INITIALIZE                      TJS-REC.
     MOVE    PARA-JDATE    TO        TJS-F01.
     MOVE    PARA-JTIME    TO        TJS-F02.
     MOVE    WK-TOKCD2     TO        TJS-F03.
     READ    JSMDAYL1
       INVALID
         CONTINUE
       NOT INVALID
         GO  TO   JHMTJSL2-010
     END-READ.
*
     MOVE    SPACE         TO        TJS-REC.
     INITIALIZE                      TJS-REC.
     MOVE    PARA-JDATE    TO        TJS-F01.
     MOVE    PARA-JTIME    TO        TJS-F02.
     MOVE    WK-TOKCD2     TO        TJS-F03.
***  MOVE    CNT-KENSU1    TO        TJS-F08.
***  MOVE    CNT-MAISU1    TO        TJS-F09.
     MOVE    CNT-KENSU1    TO        TJS-F09.
     MOVE    CNT-MAISU1    TO        TJS-F10.
     WRITE   TJS-REC.
     GO      TO   JHMTJSL2-WRT-EXIT.
*
 JHMTJSL2-010.
*
***  MOVE    CNT-KENSU1    TO        TJS-F08.
***  MOVE    CNT-MAISU1    TO        TJS-F09.
     MOVE    CNT-KENSU1    TO        TJS-F09.
     MOVE    CNT-MAISU1    TO        TJS-F10.
     REWRITE TJS-REC.
*
 JHMTJSL2-WRT-EXIT.
     EXIT.
****************************************************************
*　　　　　　　ＶＬＤ５００出力処理                            *
****************************************************************
 VLD500-OUTPUT-SEC       SECTION.
*
     IF   WK-TENPO-CHK1  =  "1"
           MOVE    "VLD500-OUTPUT-SEC"  TO    S-NAME
           MOVE      SPACE              TO    VLD-REC
           INITIALIZE                         VLD-REC
           MOVE      500                TO    VLD-F02
           MOVE      "NW"               TO    VLD-F03
           MOVE      52                 TO    VLD-F10
           MOVE      PARA-JDATE         TO    VLD-F11
           MOVE      PARA-JTIME         TO    VLD-F12
           MOVE      WK-TOKCD1          TO    VLD-F13
           DISPLAY "PARA-JDATE = " PARA-JDATE UPON CONS
           DISPLAY "PARA-JTIME = " PARA-JTIME UPON CONS
           DISPLAY "WK-TOKCD1  = " WK-TOKCD1  UPON CONS
           WRITE     VLD-REC
     END-IF.
*
     IF   WK-TENPO-CHK2  =  "1"
           MOVE    "VLD500-OUTPUT-SEC"  TO    S-NAME
           MOVE      SPACE              TO    VLD-REC
           INITIALIZE                         VLD-REC
           MOVE      500                TO    VLD-F02
           MOVE      "NW"               TO    VLD-F03
           MOVE      52                 TO    VLD-F10
           MOVE      PARA-JDATE         TO    VLD-F11
           MOVE      PARA-JTIME         TO    VLD-F12
           MOVE      WK-TOKCD2          TO    VLD-F13
           DISPLAY "PARA-JDATE = " PARA-JDATE UPON CONS
           DISPLAY "PARA-JTIME = " PARA-JTIME UPON CONS
           DISPLAY "WK-TOKCD2  = " WK-TOKCD2  UPON CONS
           WRITE     VLD-REC
     END-IF.
*
 VLD500-OUTPUT-EXIT.
     EXIT.
*-------------< PROGRAM END >------------------------------------*

```
