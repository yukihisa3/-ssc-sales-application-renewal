...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SED0020I.COB`
...
*----<< 取引先マスタ >>--*
     SELECT   HTOKMS    ASSIGN         DA-01-VI-TOKMS2
                        ...
                        STATUS         HTOKMS-ST.
*----<< EDIC返品MSG >>--*
     SELECT   EDHENPF   ASSIGN         DA-01-VI-EDHENPL2
                        ORGANIZATION   INDEXED
                        ACCESS    MODE RANDOM
                        RECORD    KEY  HEN-F013
                        STATUS         EDHENPF-ST.
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SED0020I DSPFILE ERROR " DSP-CNTL " "
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SED0020I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< ＥＤＩＣ返品ＭＳＧ >>--*
 EDHENPF-ERR            SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SED0020I EDHENPF ERROR " EDHENPF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FED00201.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FED00201"     TO   DSP-FMT.
...
*ＥＤＩＣ返品ＭＳＧ存在チェック
***  ＥＤＩＣ返品ＭＳＧ　ＲＥＡＤ
     MOVE     TORICD TO   HEN-F013.
     PERFORM  900-HEN-READ.
     IF       HEN-INVALID-FLG = "1"
              MOVE     HEN-F011  TO  JUSHIN
     ELSE
              MOVE NC"返品ＭＳＧが存在しません" TO MSGSPC
              MOVE  "R"    TO   EDIT-OPTION  OF  TORICD
              MOVE  "C"    TO   EDIT-CURSOR  OF  TORICD
              MOVE   SPACE TO   TORINM
              GO           TO   PARA-CHK-EXIT
...
*
     EVALUATE DSP-FNC
         ...
         WHEN ENT
              ...
              MOVE      HEN-F011  TO   PARA-DATE
              MOVE      HEN-F012  TO   PARA-TIME
         WHEN OTHER
              MOVE NC"ＰＦキーが違います"   TO   MSGSPC
     END-EVALUATE.
*
...
*--------------------------------------------------------------*
 ...
 900-TOK-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL ALL    EDIC受領MSG    READ                          *
*--------------------------------------------------------------*
 900-HEN-READ           SECTION.
     MOVE     "900-HEN-READ"      TO   S-NAME.
     READ     EDHENPF
       INVALID
              MOVE      SPACE          TO   HEN-INVALID-FLG
       NOT INVALID
              MOVE      "1"            TO   HEN-INVALID-FLG
     END-READ.
 900-HEN-READ-EXIT.
     EXIT.
*-----------------<< PROGRAM END >>----------------------------*
