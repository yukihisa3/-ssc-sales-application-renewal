...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SHC0011B.COB`
...
*出荷明細ワークファイル
     SELECT   SHWHACWK   ASSIGN    TO        DA-01-S-SHWHACWK
...
*    発注集計表データファイル
******************************************************************
 FD  SHWHACWK            BLOCK     CONTAINS  5    RECORDS.
     COPY     SHWHACWK  OF        XFDLIB
              JOINING   HAC       PREFIX.
******************************************************************
*    取引先マスタ
******************************************************************
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SHC0011B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SHC0011B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
****************************************************************
 INIT-SEC               SECTION.
     ...
     OPEN     INPUT     SHTDENLA  TOKMS2  SHOTBL1  HJYOKEN.
     OPEN     OUTPUT    SHWHACWK.
...
*                     消費税税率が取得出来ない場合は異常終了
********INVALID
**************MOVE  ZERO     TO   WK-ZEIRITU
        INVALID
              DISPLAY NC"消費税率取得エラー！！" UPON CONS
              MOVE 4000      TO   PROGRAM-STATUS
              STOP  RUN
********NOT INVALID
**************MOVE  JYO-F04  TO   WK-ZEIRITU
        NOT INVALID
*                     項目変更
...
**************EVALUATE  TOK-F88
*                 WHEN  "0"
*                 COMPUTE HAC-F21 = HAC-F16 * WK-ZEIRITU
*                 WHEN  "4"
*                 COMPUTE HAC-F21 ROUNDED = HAC-F16 * WK-ZEIRITU
*                 WHEN  "9"
*                 COMPUTE HAC-F21 = HAC-F16 * WK-ZEIRITU  + 0.99
*                 WHEN  OTHER
*                 CONTINUE
**************END-EVALUATE
      IF  WK-ZEIKAITEI NOT = ZERO    *>改訂日が０以上
       IF WK-ZEIKAITEI <= DEN-F112   *>納日が改訂日以上、新税率
          EVALUATE      TOK-F88
              WHEN      "0"
              COMPUTE   HAC-F21 = HAC-F16 * WK-ZEIRITU3
              WHEN      "4"
              COMPUTE   HAC-F21 ROUNDED = HAC-F16 * WK-ZEIRITU3
              WHEN      "9"
              COMPUTE   HAC-F21 = HAC-F16 * WK-ZEIRITU3 + 0.99
              WHEN      OTHER
              MOVE      HAC-F16     TO        HAC-F21
          END-EVALUATE
       ELSE                          *>納日が改訂日以下、旧税率
          EVALUATE      TOK-F88
              WHEN      "0"
              COMPUTE   HAC-F21 = HAC-F16 * WK-ZEIRITU1
              WHEN      "4"
              COMPUTE   HAC-F21 ROUNDED = HAC-F16 * WK-ZEIRITU1
              WHEN      "9"
              COMPUTE   HAC-F21 = HAC-F16 * WK-ZEIRITU1 + 0.99
              WHEN      OTHER
              MOVE      HAC-F16     TO        HAC-F21
          END-EVALUATE
       END-IF
...
*
     ...
     IF        CHK-FLG  = 1
               DISPLAY NC"＃＃欠品あり＃＃" UPON CONS
     END-IF.
*
     CLOSE     SHTDENLA  TOKMS2  SHWHACWK  SHOTBL1  HJYOKEN.
*
     STOP      RUN.
*
 END-EXIT.
