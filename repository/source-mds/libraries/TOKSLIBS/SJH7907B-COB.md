...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SJH7907B.COB`
...
*    ｶｳﾝﾄ
 ...
 01  WK-DEPB10-WK            PIC  9(06)     VALUE  ZERO.
*ルート変換
 01  HEN-RUT.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJH7907B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH7907B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*          出荷場所件数マスタ出力
           IF    CNT-MAISU1   >    ZERO
           AND   WK-DEPB10-WK =    100403
                 PERFORM  JHMKENL1-WRT-SEC
           END-IF
           IF    CNT-MAISU2   >    ZERO
           AND   WK-DEPB10-WK =    100441
                 PERFORM  JHMKENL1-WRT1-SEC
           END-IF
           MOVE      SPACE       TO    WK-DEPB-REC
           INITIALIZE                  WK-DEPB-REC
           ...
           IF        WK-DEPB10  =  100403
                     ADD    1    TO    CNT-MAISU1
                     MOVE WK-DEPB10 TO WK-DEPB10-WK
           END-IF
           IF        WK-DEPB10  =  100441
                     ADD    1    TO    CNT-MAISU2
                     MOVE WK-DEPB10 TO WK-DEPB10-WK
...
*2009/07/30 データ内の取引先ＣＤを使用する。
     ...
     IF      WK-TOKCD = 100403
             MOVE      1         TO        WK-TOKCD1-CHK
                                           PARA-TOKCK1
             MOVE      100404    TO        WK-TOKCD1  PARA-TOKCD1
                                           HEN-F01
     END-IF.
     IF      WK-TOKCD = 100441
             MOVE      1         TO        WK-TOKCD2-CHK
                                           PARA-TOKCK2
             MOVE      100442    TO        WK-TOKCD2  PARA-TOKCD2
                                           HEN-F01
     END-IF.
...
*　　　　　　　出荷場所件数マスタ出力                          *
****************************************************************
 JHMKENL1-WRT1-SEC        SECTION.
*
     MOVE   "JHMKENL1-WRT1-SEC"  TO   S-NAME.
     MOVE    SPACE         TO        KEN-REC.
     INITIALIZE                      KEN-REC.
     MOVE    PARA-JDATE    TO        KEN-F01.
     MOVE    PARA-JTIME    TO        KEN-F02.
...
*
     ...
     IF        CNT-MAISU1    >    ZERO
           IF  WK-DEPB10-WK  =    100403
*              出荷場所件数マスタ出力
               PERFORM   JHMKENL1-WRT-SEC
           END-IF
*          当日スケジュールマスタ出力
           PERFORM   JHMTJSL1-WRT1-SEC
     END-IF.
     IF        CNT-MAISU2    >    ZERO
           IF  WK-DEPB10-WK  =    100441
*              出荷場所件数マスタ出力
               PERFORM   JHMKENL1-WRT1-SEC
           END-IF
*              当日スケジュールマスタ出力
               PERFORM   JHMTJSL1-WRT2-SEC
     END-IF.
*
...
*　　　　　　　当日スケジュールマスタ出力　　　　　　　　　　　*
****************************************************************
 JHMTJSL1-WRT1-SEC       SECTION.
*
     MOVE   "JHMTJSL1-WRT1-SEC" TO   S-NAME.
...
*　　　　　　　当日スケジュールマスタ出力　　　　　　　　　　　*
****************************************************************
 JHMTJSL1-WRT2-SEC       SECTION.
*
     MOVE   "JHMTJSL1-WRT2-SEC" TO   S-NAME.
     MOVE    SPACE         TO        TJS-REC.
     INITIALIZE                      TJS-REC.
     MOVE    PARA-JDATE    TO        TJS-F01.
     MOVE    PARA-JTIME    TO        TJS-F02.
...
*
     ...
     MOVE    PARA-JTIME    TO        TJS-F02.
     MOVE    WK-TOKCD2     TO        TJS-F03.
     MOVE    CNT-KENSU2    TO        TJS-F08.
     MOVE    CNT-MAISU2    TO        TJS-F09.
     WRITE   TJS-REC.
     GO      TO    JHMTJSL1-WRT2-EXIT.
*
 JHMTJSL1-210.
*
     MOVE    CNT-KENSU2    TO        TJS-F08.
