...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBMK170L.COB`
...
*    サブシステム　　　　：　ＨＧ基幹ブシステム（共通）
*    業務名　　　　　　　：　　　　　　
*    モジュール名　　　　：　流通ＢＭＳ返品リスト発行（コメリ）
*    作成日／作成者　　　：　13/03/07  /NAV
*    処理概要　　　　　　：　流通ＢＭＳ返品リスト発行指示より受
*                        ：　取ったパラメータを条件に、流通ＢＭ
*                        ：　Ｓ返品リストを発行する。
*    更新履歴　　　　　　：
*
...
***********************************************************
 ...
 FILE-CONTROL.
*流通ＢＭＳ返品ワーク
     SELECT     BMSHE2W    ASSIGN    TO  DA-01-VI-BMSHE2W1
...
*
 01  NEW-KEY.
     ...
     02  NEW-DENNO           PIC  9(10).
*
 01  OLD-KEY.
     02  OLD-TENCD           PIC  9(08).
     02  OLD-DENNO           PIC  9(10).
*
 01  HEN-GNOX.
     02  HEN-GNO             PIC  9(02).
     02  FILLER              PIC  X(02).
...
****  見出し行１　　　　　　 ****
 01  MIDASI-1.
     ...
     02  FILLER              PIC  X(08)  VALUE  "SBMK170L".
...
****  見出し行５***
 01  MIDASI-5.
* 2023/10/23 ARK UPD START
*    02  FILLER              PIC  X(09)  VALUE  SPACE.
     02  FILLER              PIC  X(01)  VALUE  SPACE.
* 2023/10/23 ARK UPD END
     02  FILLER              PIC  N(02)
         CHARACTER  TYPE  IS  NIHONGO    VALUE
         NC"伝区".
* 2023/10/23 ARK UPD START
...
* 2023/10/23 ARK UPD END
     02  FILLER              PIC  N(06)
         CHARACTER  TYPE  IS  NIHONGO    VALUE
         NC"返品値引理由".
* 2023/10/23 ARK ADD START
     02  FILLER              PIC  X(09)  VALUE  SPACE.
     02  FILLER              PIC  N(04)
         CHARACTER  TYPE  IS  NIHONGO    VALUE
         NC"元取引日".
* 2023/10/23 ARK ADD END
*
****  線１　　　　　 ****
 01  HASEN-1.
...
*    02  FILLER              PIC  X(99) VALUE  ALL "-".
     02  FILLER              PIC  X(29) VALUE  SPACE.
     02  FILLER              PIC  X(107) VALUE  ALL "-".
* 2023/10/23 ARK UPD END
****  線３　　　　　 ****
 01  HASEN-3.
     02  FILLER              PIC  X(136) VALUE  ALL "-".
...
****  登録番号　　　　　　 ****
 01  TOUROKU-NO.
     02  FILLER              PIC  X(51)  VALUE  SPACE.
     02  FILLER              PIC  N(06)  VALUE
         NC"＜登録番号＞"    CHARACTER  TYPE  IS  YB.
     02  FILLER              PIC  X(05)  VALUE  SPACE.
     02  TOUROKNO            PIC  X(15).
* 2023/10/23 ARK ADD END
*    原票合計
 01  DKEI-AREA.
...
******************************************************************
 ...
 PRT-ERR-SEC               SECTION.
     ...
     STOP        RUN.
 HEP-ERR-SEC               SECTION.
     USE         AFTER     EXCEPTION PROCEDURE BMSHE2W.
...
*
     CLOSE       PRTF    BMSHE2W.
*    DISPLAY  "SBMK170L READ =" READ-CNT UPON STA.
*    DISPLAY  "SBMK170L IN   =" IN-CNT   UPON STA.
 END-EXIT.
     EXIT.
**********************************************************
*                    明細データ書き出し　　　　　　　　　*
...
*日付種類
     IF  DEN-FLG = 0
          MOVE  HEP-F04(1:4)      TO        TENCD
          MOVE  HEP-F06           TO        TENNM
*
          MOVE  HEP-F08           TO        SYRKBN
          EVALUATE  HEP-F08
              WHEN  "01"
                  MOVE  NC"仕入"      TO    SYRKBNM
              WHEN  "02"
...
*    返品値引理由
     EVALUATE  HEP-F10
           WHEN  "002"
               MOVE  " 02"                    TO    HENCD
               MOVE  NC"破損・汚損　　　"     TO    HENNM
           WHEN  "003"
               MOVE  " 03"                    TO    HENCD
               MOVE  NC"納品（規格）違い"     TO    HENNM
           WHEN  "004"
               MOVE  " 04"                    TO    HENCD
               MOVE  NC"取引先了解　　　"     TO    HENNM
           WHEN  "005"
               MOVE  " 05"                    TO    HENCD
               MOVE  NC"発注間違い　　　"     TO    HENNM
           WHEN  "006"
               MOVE  " 06"                    TO    HENCD
               MOVE  NC"バイヤー指示　　"     TO    HENNM
           WHEN  "031"
               MOVE  " 31"                    TO    HENCD
               MOVE  NC"数量訂正　　　　"     TO    HENNM
           WHEN  "032"
               MOVE  " 32"                    TO    HENCD
               MOVE  NC"単価訂正　　　　"     TO    HENNM
           WHEN  "033"
               MOVE  " 33"                    TO    HENCD
               MOVE  NC"取引先コード誤り"     TO    HENNM
           WHEN  "034"
               MOVE  " 34"                    TO    HENCD
               MOVE  NC"その他　　　　　"     TO    HENNM
           WHEN  "099"
               MOVE  " 99"                    TO    HENCD
               MOVE  NC"その他　　　　　"     TO    HENNM
           WHEN  OTHER
...
* 2023/10/23 ARK ADD START
*    登録番号書き出し
     IF      SV-TOUROKU   NOT =   SPACE
             MOVE    SV-TOUROKU   TO         TOUROKNO
             WRITE   P-REC  FROM  TOUROKU-NO    AFTER  1
     END-IF.
* 2023/10/23 ARK ADD END
*
 SOUWRT-EXIT.
...
*
     READ     BMSHE2W  NEXT  AT  END
              MOVE     1         TO   END-SW
              MOVE     99999999  TO   NEW-TENCD
              MOVE   9999999999  TO   NEW-DENNO
              GO                 TO   DEN-READ-EXIT
     END-READ.
*
     IF       HEP-F013           =    PAR-TORCD
        IF    PAR-SYURUI         =   "1"
          IF (HEP-F011           >=   PAR-YMDFROM   AND
              HEP-F011           <=   PAR-YMDTO)
              MOVE     HEP-F04(1:4)   TO   NEW-TENCD
              MOVE     HEP-F03(1:10)  TO   NEW-DENNO
              ADD      1              TO   IN-CNT
              MOVE     1              TO   HIT-FLG
          END-IF
        ELSE
...
*    登録番号セーブ
     IF      (HIT-FLG              =   1    AND
              HEP-F19(1:1)         =   "T")
              MOVE      HEP-F19(1:14)  TO  SV-TOUROKU
     END-IF.
* 2023/10/23 ARK ADD END
*
 DEN-READ-EXIT.
