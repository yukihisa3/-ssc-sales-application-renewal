...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3830I.COB`
...
*画面表示用ワーク
 01  DSP-WORK.
     03  WORK-PGID                PIC  X(08)  VALUE  "SSY3830I".
     03  WORK-FORMID              PIC  X(08)  VALUE  "FSY38301".
*フラグ領域
 01  FLG-AREA.
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG4.
         05  FILLER              PIC   N(20)
             VALUE NC"既に発注確定処理済み管理番号です".
     03  ERR-MSG5.
         05  FILLER              PIC   N(20)
             VALUE NC"本発用の管理番号ではありません".
     03  ERR-MSG6.
         05  FILLER              PIC   N(20)
...
******************************************************************
 ...
 01  LINK-IN-TANTOU      PIC  X(02).
 01  LINK-OUT-KANRINO    PIC  9(08).
 01  LINK-OUT-SKBCD      PIC  X(02).
*
**************************************************************
...
*ファイルのＯＰＥＮ
     OPEN     I-O       DSPFILE.
     OPEN     INPUT     NFJOHOF   NHJOHOF   SAKUBAF   HTANMS.
*ワークの初期化
     INITIALIZE   FLG-AREA.
     INITIALIZE   LINK-OUT-KANRINO
                  LINK-OUT-SKBCD.
*初期画面の表示
...
*****DISPLAY "DSP-SKBCD = " DSP-SKBCD UPON CONS.
...
*実行
         WHEN   "E000"
                MOVE    DSP-KANRI     TO   LINK-OUT-KANRINO
                MOVE    DSP-SKBCD     TO   LINK-OUT-SKBCD
*        DISPLAY "LINK-KANRINO    = " LINK-OUT-KANRINO UPON CONS
*        DISPLAY "LINK-SKBCD      = " LINK-OUT-SKBCD   UPON CONS
                MOVE    "END"         TO   END-FLG
*終了
         WHEN   "F005"
                MOVE    "END"    TO   END-FLG
...
*確定種類名
     IF      LINK-IN-SYURUI       =    "1"
             MOVE NC"オンライン発注分　　" TO   DSP-SYURUI
     ELSE
             MOVE NC"本発／手書発注分　　" TO   DSP-SYURUI
     END-IF.
*担当者コード・名　転送
     MOVE    LINK-IN-BUMON     TO   TAN-F01  DSP-TANCD(1:4).
     MOVE    "-"               TO            DSP-TANCD(5:1).
     MOVE    LINK-IN-TANTOU    TO   TAN-F02  DSP-TANCD(6:2).
     READ    HTANMS
             INVALID
                  MOVE   NC"？？？？？"    TO     DSP-TANMEI
             NOT INVALID
                  MOVE   TAN-F03           TO     DSP-TANMEI
     END-READ.
*項目属性クリア　
...
****************************************************************
 END-SEC               SECTION.
*ファイル ＣＬＯＳＥ
     CLOSE             NFJOHOF NHJOHOF  SAKUBAF  HTANMS  DSPFILE.
**
 END-EXIT.
     EXIT.
*****************<<  SSY3830I   END PROGRAM  >>******************
