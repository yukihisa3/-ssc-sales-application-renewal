...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBM9030B.COB`
...
*    作成日／更新日　　　：　2012/11/21                        *
*    作成者／更新者　　　：　ＮＡＶ武井　                      *
*    処理概要　　　　　　：　受領訂正メッセージファイル削除する*
*                                                              *
****************************************************************
 IDENTIFICATION        DIVISION.
 PROGRAM-ID.           SBM9030B.
 AUTHOR.               NAV.
 DATE-WRITTEN.         12/11/21.
****************************************************************
...
*
 01  JYT-BR-KEY2.
     03  JYT-BR-KEY21         PIC  X(08).
     03  JYT-BR-KEY22         PIC  X(04).
     03  JYT-BR-KEY23         PIC  X(08).
 01  WK-BR-KEY2.
...
*I       05  RENKEI-NO-FROM PIC X(09) VALUE SPACE.
     ...
     03  MSG-KENSU6A.
         ...
         05  MSG-KN6A2 PIC N(27) VALUE  ALL  NC"＊".
         05  MSG-KN6A3 PIC N(01) VALUE  NC"」".
     03  MSG-KENSU6.
         05  MSG-KN61  PIC N(20) VALUE
         NC"「＊受領訂正メッセージ　　削除件数　＝　".
...
****************************************************************
 INIT-SEC              SECTION.
*
     DISPLAY  "***  START SBM9030B  ***"   UPON  CONS.
...
* 定型
 INSTART-100.
     MOVE       ZERO              TO   JYT-F011.
     GO  TO    INSTART-300.
* 非定型
 INSTART-200.
     IF  PARA-SHURUI  =  "1"
*
         MOVE     PARA-JYUSINHI       TO   JYT-F011
         MOVE     PARA-JYUSINTIME     TO   JYT-F012
         MOVE     PARA-TOKCD          TO   JYT-F013
         MOVE     PARA-JYUSINHI       TO   WK-BR-KEY21
         MOVE     PARA-JYUSINTIME     TO   WK-BR-KEY22
         MOVE     PARA-TOKCD          TO   WK-BR-KEY23
     ELSE
         MOVE     PARA-SOUJYUSIN      TO   JYT-F011
     END-IF.
*
*
 INSTART-300.
     START  BMSJYTF  KEY  IS  >=
                                   JYT-F011  JYT-F012
                                   JYT-F013
*
            INVALID
...
**
     GO  TO   MAIN-900.
 MAIN-210.
     IF  JYT-BR-KEY2  =  WK-BR-KEY2
         PERFORM SAKUJYO-SEC
         GO  TO  MAIN-900
     END-IF.
     IF  JYT-BR-KEY2  NOT =  WK-BR-KEY2
         MOVE  "1"    TO   END-FLG
     GO  TO   MAIN-EXIT
     END-IF.
 MAIN-220.
     IF  JYT-F011     =  PARA-SOUJYUSIN
         ...
         GO  TO    MAIN-900
     END-IF.
     IF  JYT-F011   NOT =  PARA-SOUJYUSIN
         MOVE  "1"     TO   END-FLG
         GO  TO    MAIN-EXIT
     END-IF.
*
...
*ファイルのCLOSE処理
     CLOSE     BMSJYTF.
     DISPLAY  "***  END   SBM9030B  ***"   UPON  CONS.
*
 END-EXIT.
     EXIT.
*
...
*
     IF   PARA-TEIKEI  =  "2"
         MOVE     JYT-F011    TO   JYT-BR-KEY21
         MOVE     JYT-F012    TO   JYT-BR-KEY22
         MOVE     JYT-F013    TO   JYT-BR-KEY23
     END-IF.
*
     ADD      1           TO   READ-CNT.
*
