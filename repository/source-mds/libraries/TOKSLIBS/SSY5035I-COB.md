...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY5035I.COB`
...
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         01-GS-DSPF
                        ...
                        STATUS         DSP-ST.
*----<< 出荷情報ファイル >>--*
     SELECT   KHSYUKF   ASSIGN         DA-01-VI-KHSYUKL5
...
*
 01  MSG-AREA.
     ...
     03  MSG04               PIC  N(20)  VALUE
              NC"取引先コードが違います。".
     03  MSG05               PIC  N(21)  VALUE
              NC"トステムビバ出荷確定データに存在しません。".
...
*
 ...
 01  PARA-TORICD1           PIC   9(08).
 01  PARA-TORICD2           PIC   9(08).
 01  PARA-TORICD3           PIC   9(08).
 01  PARA-TORICD4           PIC   9(08).
 01  PARA-TORICD5           PIC   9(08).
...
*
****************************************************************
 PROCEDURE              DIVISION USING PARA-DSOKO
                                       PARA-TORICD1
                                       PARA-TORICD2
                                       PARA-TORICD3
                                       PARA-TORICD4
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSY5035I DSPFILE ERROR " DSP-CNTL " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
     CLOSE    KHSYUKF   HTOKMS    ZSOKMS1    DSPFILE.
     STOP     RUN.
*----<< 出荷情報ファイル >>--*
 DJSYUKF-ERR            SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY5035I DJSYUKF ERROR " KHSYUKF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY5035I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*----<< 倉庫マスタ >>--*
 ZSOKMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY5035I ZSOKMS1 ERROR " ZSOKMS1-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FSY50351.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SSY5035I"      TO   PGID.
     MOVE    "FSY50351"      TO   FORM.
*取引先コードパラメタ表示
     MOVE     PARA-TORICD1   TO   TTORI.
     MOVE     PARA-TORICD1   TO   TOK-F01
     READ      HTOKMS
       INVALID
              MOVE      ALL NC"＊" TO   TTORIN
       NOT INVALID
              MOVE      TOK-F02    TO   TTORIN
     END-READ.
...
*    取引先チェック
     ...
     IF  TORICD  =  ZERO
         ...
         MOVE     "R"   TO   EDIT-OPTION OF TORICD
     ELSE
         IF   TORICD  =  PARA-TORICD1 OR PARA-TORICD2 OR
                         PARA-TORICD3 OR PARA-TORICD4 OR
                         PARA-TORICD5
              CONTINUE
         ELSE
             IF   ERR-FLG   =    ZERO
...
*    倉庫コードチェック
     ...
     END-IF.
*    店舗コードチェック
     IF       STENCD  NOT NUMERIC      OR
              STENCD  =   ZERO
              MOVE      ZERO      TO   STENCD
     END-IF.
     IF       ETENCD  NOT NUMERIC      OR
              ETENCD  =   ZERO
...
*    納品日範囲チェック
*    （開始納品日チェック）
     IF       SNOUDT   NOT NUMERIC    OR
              SNOUDT   =   ZERO
              MOVE     ZERO  TO   SNOUDT
     END-IF.
*    （終了納品日チェック）
     IF       ENOUDT   NOT NUMERIC    OR
              ENOUDT   =   ZERO
...
*             MOVE  SPACE    TO   EDIT-CURSOR  OF  SNOUDT
     END-IF.
*    伝票番号チェック
     IF       SDENNO  NOT NUMERIC      OR
              SDENNO  =   ZERO
              MOVE      ZERO      TO   SDENNO
     END-IF.
     IF       EDENNO  NOT NUMERIC      OR
              EDENNO  =   ZERO
              MOVE      999999999 TO   EDENNO
     END-IF.
*
     IF       SDENNO  >  EDENNO
              IF   ERR-FLG   =    ZERO
                   MOVE      7    TO   ERR-FLG
              END-IF
              MOVE     "C"   TO   EDIT-CURSOR OF SDENNO
              MOVE     "R"   TO   EDIT-OPTION OF SDENNO
              MOVE     "R"   TO   EDIT-OPTION OF EDENNO
     END-IF.
*
*    出荷情報データ存在チェック
     IF  ERR-FLG        =    ZERO
         ...
         INITIALIZE                    KHS-REC
         MOVE      JDATE          TO   KHS-F01
         MOVE      JTIME          TO   KHS-F02
         MOVE      TORICD         TO   KHS-F03
         MOVE      WK-SOKCD       TO   KHS-F04
         MOVE      STENCD         TO   KHS-F05
         MOVE      SNOUDT         TO   KHS-F08
         MOVE      SDENNO         TO   KHS-F06
...
**                                     DISPLAY "CCC" UPON CONS
                                       ...
                                       MOVE "R"  TO
                                            ...
                                            EDIT-OPTION OF SDENNO
                                            EDIT-OPTION OF EDENNO
                                  END-IF
                             ELSE
                                  CONTINUE
                             END-IF
...
*                      DISPLAY "SNOUDT  = "  SNOUDT  UPON CONS
*                      DISPLAY "KMS-K01 = "  KMS-K01 UPON CONS
**                     DISPLAY "KMS-K02 = "  KMS-K02 UPON CONS
**                     DISPLAY "KMS-K03 = "  KMS-K03 UPON CONS
*                      DISPLAY "KMS-K05 = "  KMS-K05 UPON CONS
*                      DISPLAY "KMS-K08 = "  KMS-K08 UPON CONS
...
*
     ...
     MOVE     SDENNO              TO   PARA-SDENNO.
     MOVE     EDENNO              TO   PARA-EDENNO.
*
     MOVE     99                  TO   GR-NO.
*
 240-PARA-SEC-EXIT.
...
*--------------------------------------------------------------*
 CLR-HEAD-RTN           SECTION.
     MOVE     " "            TO   EDIT-CURSOR OF JDATE
                                  ...
                                  EDIT-CURSOR OF SDENNO
                                  EDIT-CURSOR OF EDENNO.
