...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY1510L.COB`
...
*    サブシステム　　　　：　出荷管理　　　　　　　　　　　　　*
*    業務名　　　　　　　：　セキチューオンライン　　　        *
*    モジュール名　　　　：　セキチューオンライン仕入伝票発行　*
*    作成日／更新日　　　：　99/08/11                          *
*    作成者／更新者　　　：　ＮＡＶ萩原                        *
*    処理概要　　　　　　：　　　　　　　　　　　　　　　　　　*
*    伝票フォーマット　　：　ＴＡ_型                          *
*    2004/07/05 一部ﾚｲｱｳﾄ変更の実施                          *
*    2010/04/28 植物部門用作成                               *
****************************************************************
 IDENTIFICATION         DIVISION.
 PROGRAM-ID.            SSY1510L.
 AUTHOR.                HAGIWARA.
 DATE-WRITTEN.          10/04/28.
****************************************************************
...
****************************************************************
 ...
 FILE-CONTROL.
*カインズ伝票データ（共通）
     SELECT   SKSHIRED  ASSIGN    TO        SKSHIRED
...
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
     ...
     03  WK-KYAKUTYU              PIC  X(07)     VALUE  SPACE.
 01  SEC-NAME.
...
*
 01  TL4.
     03  FILLER                   PIC  X(58)     VALUE SPACE.
     03  TL4-011                  PIC  ZZZZ9.
     03  TL4-012                  PIC  Z.
     03  FILLER                   PIC  X(21)     VALUE SPACE.
     03  TL4-02                   PIC  ZZZZZZZZ9.
     03  FILLER                   PIC  X(06)     VALUE SPACE.
     03  TL4-03                   PIC  ZZZZZZZZ9.
*
****************************************************************
 PROCEDURE              DIVISION.
****************************************************************
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      SKSHIRED.
...
*明細１行目
***  商品名１
     MOVE     DEN-A401            TO        DT1-01.
***  商品コード
     MOVE     DEN-A29             TO        DT1-02.
***  訂正伝票時処理
...
***            ハッシュトータル算出
               COMPUTE  WK-H-TOTAL = WK-H-TOTAL +  DEN-F03
     END-IF.
*明細２行目
***  商品名２
     MOVE     DEN-A402            TO        DT2-01.
***  サイズ
     MOVE     DEN-A32             TO        DT2-02.
***  色
     MOVE     DEN-A33(1:1)        TO        DT2-03.
***  取引先商品コード
     IF       DEN-A403    =       SPACE
              MOVE SPACE          TO        DT2-04  DT2-05  DT2-06
         ELSE
              MOVE SPACE          TO        DT2-04
              MOVE DEN-A403       TO        DT2-05
              MOVE SPACE          TO        DT2-06.
***  数量
...
*
*備考（Ｆ欄）退避
     MOVE     DEN-A24(1:6)        TO        WK-BIKOU3.
*特売開始日
     MOVE     DEN-A44             TO        WK-TOKUBAI.
*伝票区分
     MOVE     DEN-A06             TO        WK-DENKU.
*客注番号
     MOVE     DEN-A49             TO        WK-KYAKUTYU.
*原価・売価合計計算
...
*ヘッダ１行目
***  備考２
***  MOVE     DEN-A23             TO        HD1-01.
***  ＳＡ
***  MOVE     "("                 TO        HD1-02.
***  MOVE     DEN-A092            TO        HD1-03.
***  MOVE     ")"                 TO        HD1-04.
*ヘッダ２行目
***  社名
     MOVE     DEN-A20(1:11)       TO        HD2-01.
***  取引先名１
     MOVE     DEN-A101(1:20)      TO        HD2-02.
*ヘッダ３行目
***  発注店名１
     MOVE     DEN-A11(1:15)       TO        HD3-01.
***  店コード
     MOVE     DEN-A042            TO        HD3-02.
***  分類コード
     MOVE     DEN-A05             TO        HD3-03.
***  伝票区分
     MOVE     DEN-A06             TO        HD3-04.
***  伝票番号
     MOVE     DEN-A03             TO        HD3-05.
***  取引先コード
     MOVE     DEN-A091            TO        HD3-06.
***  取引先名２
     MOVE     DEN-A102(1:20)      TO        HD3-07.
***  発注日（年）
     MOVE     DEN-A071            TO        HD3-081.
***  発注日（月）
     MOVE     DEN-A072            TO        HD3-082.
***  発注日（日）
     MOVE     DEN-A073            TO        HD3-083.
***  納品日（年）
     MOVE     DEN-A081            TO        HD3-091.
***  納品日（月）
     MOVE     DEN-A082            TO        HD3-092.
***  納品日（日）
     MOVE     DEN-A083            TO        HD3-093.
*
...
* テール１行目
***  特売開始日
     IF  WK-TOKUBAI  NOT =  ZERO
         MOVE     WK-TOKUBAI      TO        TL1-01
     END-IF.
***  ハッシュトータル
*****MOVE     WK-H-TOTAL          TO        TL1-023.
***  数量合計
...
***  伝票区分印字
     EVALUATE  WK-DENKU
         WHEN  "01"  MOVE "ﾃﾝﾊｯﾁｭｳ   "  TO  TL1-011
         WHEN  "02"  MOVE "ﾊﾞｲﾔｰﾊｯﾁｭｳ"  TO  TL1-011
         WHEN  "04"  MOVE "ｷｬｸﾁｭｳ    "  TO  TL1-011
         WHEN  OTHER MOVE "          "  TO  TL1-011
     END-EVALUATE.
***  客注番号
     MOVE     WK-KYAKUTYU         TO        TL3-01.
*    訂正後数量合計
*****IF       TS-FLG =  1
...
***           訂正後数量合計（整数部）
              MOVE     WK-SURYO1            TO        TL4-011
***           訂正後数量合計（端数部）
              IF       WK-SURYO2  =         ZERO
                       MOVE       ZERO      TO        TL4-012
              ELSE
                       MOVE     WK-SURYO2   TO        TL4-012
              END-IF
              MOVE     TSGENKEI             TO        TL4-02
              MOVE     TSBAIKEI             TO        TL4-03
     END-IF.
*    原価金額合計
...
*
     CLOSE    SKSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
