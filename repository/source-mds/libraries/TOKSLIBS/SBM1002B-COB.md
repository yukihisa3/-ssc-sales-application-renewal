...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBM1002B.COB`
...
*
 ...
 FILE-CONTROL.
*流通ＢＭＳ発注メッセージ
     SELECT   BMSJYTF   ASSIGN    TO        DA-01-VI-BMSJYTL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       JYT-F011  JYT-F012
                                            JYT-F013
                                            WITH DUPLICATES
...
*ワークに発注メッセージを展開
     COPY   BMSJYTF  OF XFDLIB  JOINING   HWK  AS   PREFIX.
*ワーク項目
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SBM1002B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SBM1002B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
******************************************************************
 ...
 DECLARATIVES.
 HAC-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE BMSJYTF.
...
*流通ＢＭＳ発注メッセージスタート
     ...
     INITIALIZE                   JYT-REC.
     MOVE     PARA-HIDUKE    TO   JYT-F011.
     MOVE     PARA-JIKAN     TO   JYT-F012.
     MOVE     PARA-TORICD    TO   JYT-F013.
     START    BMSJYTF  KEY  >=    JYT-F011  JYT-F012  JYT-F013
         INVALID   KEY
...
*********会社ＣＤで取引先ＣＤを判定する。
         ...
         MOVE     JYT-F206        TO   TER-F04
*********MOVE     JYT-F309(1:4)   TO   WK-TENPO
*********MOVE     WK-TENPO-H      TO   TER-F05
*##2015/05/28 NAV ST 店舗ＣＤ５桁対応
******** MOVE     JYT-F309(1:4)   TO   WK-TENPO
******** MOVE     WK-TENPO-H      TO   TER-F05
         IF  JYT-F309(5:1) = SPACE
             MOVE JYT-F309(1:4)   TO   WK-TENPO
             MOVE WK-TENPO-H      TO   TER-F05
         ELSE
             MOVE JYT-F309(1:5)   TO   WK-TENPO5
             MOVE WK-TENPO5-H     TO   TER-F05
         END-IF
*##2015/05/28 NAV ED 店舗ＣＤ５桁対応
         EVALUATE JYT-F206(1:4)
             WHEN "5200"
             MOVE 87372           TO   HWK-F013 JYT-F013 TER-F06
             WHEN "0105"
             MOVE 87373           TO   HWK-F013 JYT-F013 TER-F06
             WHEN "5300"
             MOVE 87374           TO   HWK-F013 JYT-F013 TER-F06
             WHEN "3618"
             MOVE 33189           TO   HWK-F013 JYT-F013 TER-F06
             WHEN "2400"
             MOVE 87373           TO   HWK-F013 JYT-F013 TER-F06
*##2015/11/17 NAV ST イオンリテールストア追加
             WHEN "7900"
             MOVE 87376           TO   HWK-F013 JYT-F013 TER-F06
*##2015/11/17 NAV ED
             WHEN OTHER
             MOVE 99999           TO   HWK-F013 JYT-F013 TER-F06
         END-EVALUATE
         MOVE   "3"               TO   TER-F07
         WRITE  TER-REC
         ADD    1                 TO   ERRT-CNT
     ELSE
              MOVE FUR-F03        TO   HWK-F013  JYT-F013
     END-IF.
*イオン取引先受信ワーク索引
...
*ファイルクローズ
     CLOSE     BMSJYTF  IONFURF  IONTWKF  IONTERWK.
*
     STOP      RUN.
*
 END-EXIT.
...
*　　　　　　流通ＢＭＳ発注メッセージ読込
****************************************************************
 BMSJYTF-READ-SEC          SECTION.
*
     READ     BMSJYTF
              AT  END       MOVE  "END"   TO  END-FLG
                            GO TO BMSJYTF-READ-EXIT
...
*件数表示
     ...
     END-IF.
*指定バッチ_チェック
     IF     ( PARA-HIDUKE  =  JYT-F011 ) AND
            ( PARA-JIKAN   =  JYT-F012 ) AND
            ( PARA-TORICD  =  JYT-F013 )
              CONTINUE
     ELSE
              MOVE      "END"     TO   END-FLG
              GO                  TO   BMSJYTF-READ-EXIT
