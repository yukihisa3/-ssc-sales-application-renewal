...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/NJH4202B.COB`
...
*受信データファイル
     SELECT   JUNTENWK  ASSIGN    TO        DA-01-S-JUNTENWK
                        ACCESS    MODE IS   SEQUENTIAL
                        FILE      STATUS    JUN-STATUS
                        ORGANIZATION   IS   SEQUENTIAL.
*受信データファイル
     SELECT   JUNTENJR  ASSIGN    TO        DA-01-S-JUNTENJR
                        ACCESS    MODE IS   SEQUENTIAL
...
*    ｶｳﾝﾄ
 ...
 01  JUN-CNT                 PIC  9(08)     VALUE  ZERO.
 01  JJR-CNT                 PIC  9(08)     VALUE  ZERO.
 ...
 01  WK-GENKA                PIC  9(09)     VALUE  ZERO.
*文字数値変換
 01  WK-HENKAN               PIC  X(02).
 01  WK-HENKAN-R             REDEFINES  WK-HENKAN.
     03  WK-HEN-1            PIC  9(02).
*
*伝票ヘッダーレコード退避ワーク
 01  WK-DEPB-REC.
     03  WK-DEPB01          PIC  X(03).
     03  WK-DEPB02          PIC  9(09).
     03  WK-DEPB03          PIC  9(06).
     03  WK-DEPB04          PIC  9(03).
     03  WK-DEPB05          PIC  9(02).
     03  WK-DEPB06          PIC  9(02).
     03  WK-DEPB07          PIC  9(02).
     03  WK-DEPB08          PIC  9(08).
     03  WK-DEPB09          PIC  9(08).
     03  WK-DEPB10          PIC  9(04).
     03  WK-DEPB11          PIC  X(07).
     03  WK-DEPB12.
       05  WK-DEPB121       PIC  X(11).
       05  WK-DEPB122       PIC  X(08).
       05  WK-DEPB123       PIC  X(04).
       05  WK-DEPB124.
         07  WK-DEPB1241    PIC  X(07).
         07  WK-DEPB1242    PIC  X(01).
         07  WK-DEPB1243    PIC  X(06).
     03  WK-DEPB13          PIC  X(01).
     03  WK-DEPB14          PIC  X(36).
*
*伝票ヘッダーオプション退避ワーク
 01  WK-DEPC-REC.
     03  WK-DEPC01          PIC  X(03).
     03  WK-DEPC02          PIC  X(30).
     03  WK-DEPC03          PIC  X(20).
     03  WK-DEPC04.
       05  WK-DEPC041       PIC  X(20).
       05  WK-DEPC042       PIC  X(20).
     03  WK-DEPC05          PIC  9(08).
     03  WK-DEPC06          PIC  9(08).
     03  WK-DEPC07          PIC  X(18).
     03  WK-DEPC08          PIC  X(01).
*
*    明細レコード退避ワーク
 01  WK-DEPD-REC.
     03  WK-DEPD01          PIC  X(03).
     03  WK-DEPD02          PIC  9(02).
     03  WK-DEPD03          PIC  X(13).
     03  WK-DEPD04          PIC  X(07).
     03  WK-DEPD05          PIC  X(03).
     03  WK-DEPD06          PIC  9(05).
     03  WK-DEPD07          PIC  X(01).
     03  WK-DEPD08.
         05  WK-DEPD081     PIC  9(07).
         05  WK-DEPD082     PIC  X(02).
     03  WK-DEPD09          PIC  9(07).
     03  WK-DEPD10          PIC  X(25).
     03  WK-DEPD11          PIC  X(31).
     03  WK-DEPD12          PIC  X(12).
     03  WK-DEPD13          PIC  X(10).
*
 01  WK-AREA.
*システム日付の編集
 ...
 01  WK-ST.
     ...
     03  JUN-STATUS        PIC  X(02).
     03  JJR-STATUS        PIC  X(02).
*
 01  MSG-AREA.
     03  MSG-START.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  ST-PG          PIC   X(08)  VALUE "NJH4202B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NJH4202B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
     ...
     03  MSG-TC.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  FILLER         PIC   X(09)  VALUE " TC CNT= ".
         05  OUT-TC         PIC   9(06).
         05  FILLER         PIC   X(05)  VALUE " *** ".
     03  MSG-JR.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  FILLER         PIC   X(09)  VALUE " JR CNT= ".
         05  OUT-JR         PIC   9(06).
         05  FILLER         PIC   X(05)  VALUE " *** ".
*
 01  LINK-AREA.
...
*
 FILEERR-SEC9           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   JUNTENWK.
     MOVE      "JUNTENWK"   TO   AB-FILE.
     MOVE      JUN-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
...
*
        ...
        END-IF
*
        IF    DEN-01A(IDX)  =  "B02" OR "C02" OR "D02"
              PERFORM  JUNTENWK-WRITE-SEC
        END-IF
*
        IF    DEN-01A(IDX)  =  "B03" OR "C03" OR "D03"
              PERFORM  JUNTENWK-WRITE-SEC
        END-IF
*
     END-PERFORM.
...
*
*    レコード区分
     MOVE     WK-DEPC01     TO        HEN-A15.
*    予備１
     MOVE     WK-DEPC02     TO        HEN-A16.
*    店舗名称（カナ）
     MOVE     WK-DEPC03     TO        HEN-A17.
*    取引先名称（カナ）
     MOVE     WK-DEPC04     TO        HEN-A18.
*    原価金額
     MOVE     WK-DEPC05     TO        HEN-A19.
*    売価金額
     MOVE     WK-DEPC06     TO        HEN-A20.
*    予備２
     MOVE     WK-DEPC07     TO        HEN-A21.
*    オプションラインコード
     MOVE     WK-DEPC08     TO        HEN-A22.
*
*    レコード区分
...
*商品名　　　
     MOVE        HEN-A32(1:15)  TO   HEN-F1421.
     MOVE        HEN-A32(16:10) TO   HEN-F1422.
*2001/11/21 規格名の先頭５バイトを商品名にセットする。
     MOVE        HEN-A34(1:5)   TO   HEN-F1422(11:5).
*数量
     MOVE        HEN-A28   TO        HEN-F15.
*単
     MOVE       "1"        TO        HEN-F16.
*原価単価
*****MOVE        HEN-A30   TO        HEN-F172.
     COMPUTE  WK-GENKA  = HEN-A301 * 100.
     IF          HEN-A302 NOT = SPACE
                 MOVE  HEN-A302    TO   WK-HENKAN
                 COMPUTE WK-GENKA  =  WK-GENKA  +  WK-HEN-1
     END-IF.
     COMPUTE     HEN-F172  =  WK-GENKA  /  100.
*売価単価
     MOVE        HEN-A31   TO        HEN-F173.
*原価金額
     COMPUTE     HEN-F181 ROUNDED = HEN-F15 * HEN-F172.
*売価金額
...
*修正原価単価
     ...
     IF          HEN-A302 NOT = SPACE
                 ...
                 COMPUTE WK-GENKA  =  WK-GENKA  +  WK-HEN-1
     END-IF.
     COMPUTE     HEN-F512  =  WK-GENKA  /  100.
*修正売価単価
     MOVE        HEN-A31   TO        HEN-F513.
*修正原価金額
     COMPUTE     HEN-F521 ROUNDED = HEN-F50 * HEN-F512.
*修正売価金額
...
*
     ...
     MOVE      JUN-CNT   TO      OUT-TC.
     MOVE      JJR-CNT   TO      OUT-JR.
     DISPLAY   MSG-IN    UPON CONS.
     DISPLAY   MSG-OUT   UPON CONS.
     DISPLAY   MSG-END   UPON CONS.
     DISPLAY   MSG-TC    UPON CONS.
*
...
*　　ＴＣデータレコード⇒ＴＣ発注用ワーク
****************************************************************
 JUNTENWK-WRITE-SEC      SECTION.
*
     MOVE   "JUNTENWK-WRITE-SEC"  TO    S-NAME.
     MOVE    SPACE                TO    JUN-REC.
     INITIALIZE                         JUN-REC.
     MOVE    DEN-01(IDX)          TO    JUN-REC.
     WRITE   JUN-REC.
*
     ADD     1                    TO    JUN-CNT.
*
...
*　　ＴＣデータレコード⇒ＴＣ受領用ワーク
****************************************************************
 JUNTENJR-WRITE-SEC      SECTION.
*
     MOVE   "JUNTENJR-WRITE-SEC"  TO    S-NAME.
     MOVE    SPACE                TO    JJR-REC.
     INITIALIZE                         JJR-REC.
     MOVE    DEN-01(IDX)          TO    JJR-REC.
     WRITE   JJR-REC.
*
     ADD     1                    TO    JJR-CNT.
*
