...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SZI0020B.COB`
...
****************************************************************
 FILE-CONTROL.
*在庫ＥＸＣＥＬデータ（作業系）　※ＣＳＶ形式
     SELECT      ZAISGYXX    ASSIGN    TO       DA-01-S-ZAISGYXX
...
*在庫ＥＸＣＥＬデータ（作業系）　
 FD  ZAISGYXX
     ...
     BLOCK       CONTAINS    43        RECORDS.
     COPY        ZAISGYXX    OF        XFDLIB
...
*メッセージ出力領域
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG           PIC  X(08)  VALUE "SZI0020B".
         05  FILLER          PIC  X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER          PIC  X(05)  VALUE " *** ".
         05  END-PG          PIC  X(08)  VALUE "SZI0020B".
         05  FILLER          PIC  X(11)  VALUE
                                         " END   *** ".
     03  MSG-OUT1.
...
****************************************************************
 DECLARATIVES.
 CSV-ERR                     SECTION.
     USE         AFTER       EXCEPTION PROCEDURE ZAISGYXX.
...
*      58(スト_ＣＨＧ)：必須
     IF  CSV-F04 =   SPACE
         MOVE   "Y"          TO        WK-ERR-KBN
         MOVE   "1"          TO        ERR-KBN(03)
*T       DISPLAY NC"倉庫ＣＤエラー　" CSV-F04  UPON CONS
     END-IF.
*
*  『作業日』
*      55(棚移動)　　　：必須
*      58(スト_ＣＨＧ)：必須
     IF  ( CSV-F05 = ZERO      ) OR
         ...
         MOVE   "Y"          TO        WK-ERR-KBN
         MOVE   "1"          TO        ERR-KBN(03)
*T       DISPLAY NC"作業日エラー　"  CSV-F05  UPON CONS
     END-IF.
*
*  『サカタ商品ＣＤ』
*      55(棚移動)　　　：必須
*      58(スト_ＣＨＧ)：必須
     IF  CSV-F06 =   SPACE
         MOVE   "Y"          TO        WK-ERR-KBN
         MOVE   "1"          TO        ERR-KBN(03)
*T       DISPLAY NC"サカタ商品ＣＤエラー　" CSV-F06  UPON CONS
     END-IF.
*
*  『サカタ品単１』
*      55(棚移動)　　　：必須だが””もＯＫ
*      58(スト_ＣＨＧ)：必須だが””もＯＫ
*T       DISPLAY NC"サカタ品単１　" CSV-F07  UPON CONS
*
*  『サカタ品単２』
*      55(棚移動)　　　：必須だが””もＯＫ
*      58(スト_ＣＨＧ)：必須だが””もＯＫ
*T       DISPLAY NC"サカタ品単２　" CSV-F08  UPON CONS
*
*  『サカタ品単３』
*      55(棚移動)　　　：必須だが””もＯＫ
*      58(スト_ＣＨＧ)：必須だが””もＯＫ
*T       DISPLAY NC"サカタ品単３　" CSV-F09  UPON CONS
*
*  『出庫棚番』
*      55(棚移動)　　　：必須だが””もＯＫ
*      58(スト_ＣＨＧ)：任意
...
*      58(スト_ＣＨＧ)：不要
     IF  ( CSV-F15 NOT = " "  ) AND
         ...
         MOVE   "Y"          TO        WK-ERR-KBN
         MOVE   "1"          TO        ERR-KBN(03)
*T       DISPLAY NC"未出庫ＦＬＧエラー　"  CSV-F05  UPON CONS
     END-IF.
*
*  『備考』
*      55(棚移動)　　　：任意
...
*T↓
*    IF       CSV-F01   =    6
*         DISPLAY NC"連番　　　　＝" CSV-F01  UPON CONS
*         DISPLAY NC"代表倉庫ＣＤ＝" LINK-IN-DSOKO UPON CONS
*         DISPLAY NC"→制限なし　　"          UPON CONS
*    END-IF.
*T↑
*
*
...
*------------------------------------------*
     ...
     MOVE     CSV-F07        TO     MEI-F0121.
     MOVE     CSV-F08        TO     MEI-F0122.
     MOVE     CSV-F09        TO     MEI-F0123.
     PERFORM  MEI-READ-SEC.
     IF       MEI-INV-FLG =  "INV"
              MOVE    "Y"    TO   WK-ERR-KBN
              MOVE    "1"    TO   ERR-KBN(04)
*T            DISPLAY NC"商品名称マスタなし　"  CSV-F06 "-"
*T                                              CSV-F07 "-"
*T                                              CSV-F08 "-"
*T                                              CSV-F09
*T                                              UPON CONS
...
*T            DISPLAY NC"作業日論理エラー"  CSV-F05 UPON CONS
              MOVE    "Y"      TO      WK-ERR-KBN
...
*T↓
*    IF CSV-F01 = 3
*    DISPLAY NC"作業日　　＝"  CSV-F05  UPON CONS
...
*
* _作業日≦システム日付はＯＫ
     IF  CSV-F05       <=  SYS-DATE8
         CONTINUE
     ELSE
         MOVE   "Y"        TO   WK-ERR-KBN
         MOVE   "1"        TO   ERR-KBN(06)
*T      DISPLAY NC"作業日＞システム日付エラー" UPON CONS
*T      DISPLAY NC"　　　　　　　　　作業日＝" CSV-F05
*T                                                     UPON CONS
*T      DISPLAY NC"　　　　　　システム日付＝" SYS-DATE8
*T                                                     UPON CONS
         GO                TO   DATA-CHK-07
...
* 倉庫ＣＤ（先）　製品在庫移動のみ
* 作業日
     MOVE     CSV-F05           TO        ZAI-F056.
* サカタ商品ＣＤ
     MOVE     CSV-F06           TO        ZAI-F057.
* 品単１
     MOVE     CSV-F07           TO        ZAI-F058.
* 品単２
     MOVE     CSV-F08           TO        ZAI-F059.
* 品単３
     MOVE     CSV-F09           TO        ZAI-F05A.
* 作業系_出庫棚番
     MOVE     CSV-F10           TO        ZAI-F05B.
* 作業系_入庫棚番
     MOVE     CSV-F11           TO        ZAI-F05C.
* 作業系_ストック_（元）
     IF       CSV-F02           =        "58"
              MOVE   CSV-F12    TO        ZAI-F05D
     END-IF.
* 作業系_ストック_（先）
     IF       CSV-F02           =        "58"
              MOVE   CSV-F13    TO        ZAI-F05E
     END-IF.
* 作業系_数量
     MOVE     CSV-F14           TO        ZAI-F05F.
* 作業系_備考
     MOVE     CSV-F16           TO        ZAI-F05G.
* 作業系_未出庫ＦＬＧ
     MOVE     CSV-F15           TO        ZAI-F05H.
* 入出庫系_出庫棚番
* 入出庫系_入庫棚番
* 入出庫系_ストック_
* 入出庫系_数量
