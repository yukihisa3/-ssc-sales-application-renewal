...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SKZ0120B.COB`
...
*---<<  出荷状況管理ファイル  >>---*
 JIS-ERR              SECTION.
     ...
     DISPLAY  "### SKZ0120B SYUJISL4   ERROR " JIS-ST " "
...
*----<< 条件ファイル >>--*
 JYO-ERR              SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     "4000"         TO   PROGRAM-STATUS.
     DISPLAY  "### SKZ0120B HJYOKEN    ERROR " JYO-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
...
*出荷状況管理ファイルＳＴＡＲＴ
     ...
     INITIALIZE                JIS-REC.
     MOVE  "1"          TO     JIS-F95
     START SYUJISF KEY IS >=   JIS-F95  JIS-F01  JIS-F02  JIS-F03
                               JIS-F04  JIS-F05  JIS-F06
           INVALID
           DISPLAY NC"＃＃対象データ無しＳＴ＃＃" UPON CONS
           MOVE    "END"       TO       END-FLG
           GO                  TO       INIT-RTN-EXIT
     END-START.
*出荷状況管理ファイル初期読込み
     ...
     IF   END-FLG  =  "END"
          DISPLAY NC"＃＃対象データ無し初期ＲＤ＃＃" UPON CONS
     END-IF.
*
 INIT-RTN-EXIT.
     EXIT.
