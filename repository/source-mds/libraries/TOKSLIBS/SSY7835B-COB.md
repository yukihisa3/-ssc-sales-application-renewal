...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY7835B.COB`
...
*
 ...
 FILE-CONTROL.
*受信データファイル
     SELECT   ROYALKEP  ASSIGN    TO        DA-01-S-ROYALKEP
                        ACCESS    MODE IS   SEQUENTIAL
                        FILE      STATUS    JJR-STATUS
                        ORGANIZATION   IS   SEQUENTIAL.
*受領データファイル
     SELECT   KEPSYUL1  ASSIGN    TO        DA-01-VI-KEPSYUL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       JYR-F03   JYR-F09
                                            JYR-F06   JYR-F10
                                            JYR-F11
...
*    明細レコード退避ワーク
 01  WK-DEPD-REC.
     ...
     03  WK-DEPD17          PIC  9(08)V99.
     03  WK-DEPD18          PIC  9(08)V99.
     03  WK-DEPD19          PIC  X(13).
     03  WK-DEPD20          PIC  9(11).
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSY7835B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSY7835B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*各量販店領域転送
*    受信日
     MOVE    SYS-DATEW      TO        JYR-F01.
*    年月度
     MOVE    WK-DEPB09(1:6) TO        JYR-F02.
*    取引先ＣＤ
     MOVE    WK-DEPB15(4:6) TO        JYR-F03.
*    締日
     MOVE    ZERO           TO        JYR-F04.
*    発注日
     MOVE    WK-DEPB07      TO        JYR-F05.
*    納品日
     MOVE    WK-DEPB08      TO        JYR-F06.
*    受領日
     MOVE    WK-DEPB09      TO        JYR-F07.
*    出荷確定日
     MOVE     ZERO          TO        JYR-F08.
*    店ＣＤ
     MOVE     WK-DEPD02     TO        JYR-F09.
*    伝票番号
     MOVE     WK-DEPB05     TO        JYR-F10.
*    行番号
     MOVE     WK-DEPD03     TO        JYR-F11.
*    伝票区分
     MOVE     WK-DEPD04     TO        JYR-F12.
*    商品ＣＤ
     MOVE     WK-DEPD05     TO        JYR-F13.
*    商品名１
     MOVE     WK-DEPD16(1:15) TO      JYR-F14.
*    商品名２
     MOVE     WK-DEPD16(16:15) TO     JYR-F15.
*    発注数
     MOVE     WK-DEPD08     TO        JYR-F16.
*    検収数
     MOVE     WK-DEPD13     TO        JYR-F17.
*    原価単価
     MOVE     WK-DEPD09     TO        JYR-F18.
*    原価金額
     MOVE     WK-DEPD11     TO        JYR-F19.
*    売価単価
     MOVE     WK-DEPD10     TO        JYR-F20.
*    売価金額
     MOVE     WK-DEPD12     TO        JYR-F21.
*    欠品数量
     MOVE     ZERO          TO        JYR-F22.
*    ＲＨＣ自責金額
     MOVE     WK-DEPD17     TO        JYR-F23.
*    修正後納品金額
     MOVE     WK-DEPD18     TO        JYR-F24.
*    符号
     IF       WK-DEPB04  =  "13"
              MOVE   1      TO        JYR-F25
     END-IF.
