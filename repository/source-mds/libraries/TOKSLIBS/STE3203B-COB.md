...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/STE3203B.COB`
...
*   サブシステム名：   手書伝票発行                       *
*   プログラム名　：   カインズ                           *
*   プログラムID  ：   STE3203B（備考欄印字機能有り）     *
*   使用伝票      ：   ＜ターンアラウンドタイプ_型＞     *
*   作成者        ：   ナブ・アシスト                     *
*   作成日        ：   1999.05.19                         *
*   更新日／更新者：   2011.10.07 /YOSHIDA.M              *
*   更新概要      ：   基幹サーバ統合                     *
*   更新日／更新者：   2015.02.03 /T.TAKAHASHI            *
*   更新概要      ：   カインズ流通ＢＭＳ対応             *
*   更新日／更新者：   2015.12.11 /T.TAKAHASHI            *
*   更新概要      ：   センター毎出力対応　　　           *
...
*
 ...
 FILE-CONTROL.
*伝票データ
     SELECT   HKYOTU1        ASSIGN    TO        DA-01-VI-JHTTEGL2
...
*プリンター
 FD  PRTF
     ...
     COPY        FTE32031  OF        XMDLIB.
*表示ファイル
 FD  DSPF
     LABEL       RECORD    IS        OMITTED.
     COPY        FTE32032  OF        XMDLIB.
******************************************************************
...
*****各ファイルオープン
     OPEN        I-O       DSPF
                 ...
                 OUTPUT    PRTF.
*****画面初期化
     MOVE     SPACE              TO   FTE32032.
     MOVE     ZERO               TO   WK-DEN.
     MOVE     ZERO               TO   CNT-DENPYO.
*////PERFORM  DSP-WRT-SEC.
*****帳票初期化
     MOVE        SPACE     TO        FTE32031.
*****伝票枚数カウント
     MOVE     ZERO         TO    WK-MAI.
***2011.10.07 ST
...
*****上記，条件以上にてファイル読込みスタート
     START    HKYOTU1       KEY  IS >=
              KYO-F01  KYO-F278 KYO-F07 KYO-F112 KYO-F02
              KYO-F03
          INVALID  KEY
...
*****2015/02/03 NAV ST
     IF       KYO-F12  NOT =  SPACE
              MOVE KYO-F12(1:2)   TO   HD04(1:2)
              MOVE "-"            TO   HD04(3:1)
              MOVE "1"            TO   HD04(4:1)
     END-IF.
*****2015/02/03 NAV ED
