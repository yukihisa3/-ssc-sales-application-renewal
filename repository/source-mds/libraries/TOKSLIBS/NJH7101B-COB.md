...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/NJH7101B.COB`
...
*基本情報データ
     SELECT   HDJOHOL1  ASSIGN    TO        DA-01-VI-HDJOHOL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       HDJ-F01   HDJ-F04
                                            HDJ-F06
                        FILE  STATUS   IS   HDJ-STATUS.
*取引先マスタ
...
*    ｶｳﾝﾄ
 ...
 01  WK-TOKCD                PIC  9(05)     VALUE  ZERO.
*ルート変換
 01  HEN-RUT.
...
*ファイルヘッドレコード退避ワーク
 01  WK-DEPA-REC.
     03  WK-DEPA01          PIC  X(02).
     03  WK-DEPA02          PIC  X(02).
     03  WK-DEPA03          PIC  9(05).
     03  WK-DEPA04          PIC  9(08).
     03  WK-DEPA05          PIC  9(06).
     03  WK-DEPA06          PIC  9(08).
     03  WK-DEPA07          PIC  9(06).
     03  WK-DEPA08          PIC  9(05).
     03  WK-DEPA09          PIC  X(01).
     03  WK-DEPA10          PIC  X(05).
     03  WK-DEPA11          PIC  X(01).
     03  WK-DEPA12          PIC  X(79).
*ヘッドレコード退避ワーク
 01  WK-DEPB-REC.
     03  WK-DEPB01          PIC  X(02).
     03  WK-DEPB02          PIC  X(02).
     03  WK-DEPB03          PIC  9(05).
     03  WK-DEPB04          PIC  X(07).
     03  WK-DEPB05          PIC  X(05).
     03  WK-DEPB06          PIC  X(01).
     03  WK-DEPB07          PIC  X(03).
     03  WK-DEPB08          PIC  X(04).
     03  WK-DEPB09          PIC  X(20).
     03  WK-DEPB10          PIC  X(10).
     03  WK-DEPB11          PIC  X(02).
     03  WK-DEPB12          PIC  X(04).
     03  WK-DEPB13          PIC  X(01).
     03  WK-DEPB14          PIC  X(04).
     03  WK-DEPB15          PIC  X(04).
     03  WK-DEPB16          PIC  9(08).
     03  WK-DEPB17          PIC  9(08).
     03  WK-DEPB18          PIC  X(03).
     03  WK-DEPB19          PIC  X(17).
     03  WK-DEPB20          PIC  X(18).
*    明細レコード退避ワーク
 01  WK-DEPD-REC.
     03  WK-DEPD01          PIC  X(02).
     03  WK-DEPD02          PIC  X(02).
     03  WK-DEPD03          PIC  9(05).
     03  WK-DEPD04          PIC  X(02).
     03  WK-DEPD05          PIC  X(01).
     03  WK-DEPD06          PIC  X(13).
     03  WK-DEPD07          PIC  X(25).
     03  WK-DEPD08          PIC  X(15).
     03  WK-DEPD09          PIC S9(06)V9(02).
     03  WK-DEPD10          PIC S9(05).
     03  WK-DEPD11          PIC S9(06)V9(02).
     03  WK-DEPD12          PIC S9(08)V9(02).
     03  WK-DEPD13          PIC S9(08).
     03  WK-DEPD14          PIC  X(24).
*
 01  WK-AREA.
*システム日付の編集
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "NJH7101B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NJH7101B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*伝票番号　取得
     ...
     READ     JYOKEN1
         INVALID
              MOVE      4000      TO   PROGRAM-STATUS
              DISPLAY "Ｊ本田用伝票番号取得エラー" UPON CONS
              STOP      RUN
         NOT  INVALID
              MOVE      JYO-F04   TO   CNT-DENNO
     END-READ.
*
     READ     CVCSG001
...
*          出荷場所件数マスタ出力
               ...
               END-IF
*          サカタ伝票_　採番
               IF    CNT-DENNO   =   JYO-F06
                     MOVE     JYO-F05      TO   CNT-DENNO
               ELSE
                     ADD      1            TO   CNT-DENNO
                END-IF
                MOVE      SPACE       TO    WK-DEPB-REC
...
*伝票明細転送
*    データ種別
     MOVE     WK-DEPD01     TO        HEN-A21.
*    レコード区分（Ｍ１）
     MOVE     WK-DEPD02     TO        HEN-A22.
*    レコードシーケンス_
     MOVE     WK-DEPD03     TO        HEN-A23.
*    発注行番号
     MOVE     WK-DEPD04     TO        HEN-A24.
*    商品コード区分
     MOVE     WK-DEPD05     TO        HEN-A25.
*    商品コード
     MOVE     WK-DEPD06     TO        HEN-A26.
*    商品名
     MOVE     WK-DEPD07     TO        HEN-A27.
*    規格
     MOVE     WK-DEPD08     TO        HEN-A28.
*    入数
     MOVE     WK-DEPD09     TO        HEN-A29.
*    発注ケース数
     MOVE     WK-DEPD10     TO        HEN-A30.
*    発注バラ数量
     MOVE     WK-DEPD11     TO        HEN-A31.
*    原価単価
     MOVE     WK-DEPD12     TO        HEN-A32.
*    売価単価
     MOVE     WK-DEPD13     TO        HEN-A33.
*    予備
     MOVE     WK-DEPD14     TO        HEN-A34.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
*取引先コード
     MOVE    HEN-A05             TO        HEN-F01.
     MOVE    HEN-A05             TO        WK-TOKCD.
*伝票ナンバー
*    MOVE    HEN-A04             TO        WK-DENNO-1.
*    MOVE    WK-HEN-DENNO        TO        HEN-F02.
     MOVE    CNT-DENNO           TO        HEN-F02
                                           HEN-F23.
*    MOVE    HEN-A04             TO        WK-DENNO-H.
*    MOVE    WK-HEN-DENNO-H      TO        HEN-F23.
*行_
     MOVE    HEN-A24             TO        HEN-F03.
*伝区
     MOVE        40              TO        HEN-F051.
...
*分類（部門）
     MOVE        HEN-A14         TO        HEN-F12.
*商品区分　　
     MOVE        SPACE           TO        HEN-F131.
*伝票区分　　
*    受信データに伝票区分が含まれていないのでSPACEを固定
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名
         MOVE    HEN-A27(1:15)   TO   HEN-F1421(1:15)
         MOVE    HEN-A27(16:10)  TO   HEN-F1422(1:10)
       NOT INVALID
         MOVE    HEN-A27(1:15)   TO   HEN-F1421(1:15)
         MOVE    HEN-A27(16:10)  TO   HEN-F1422(1:10)
     END-READ.
*数量
     MOVE        HEN-A31         TO        HEN-F15.
*単
     MOVE       "1"              TO        HEN-F16.
*原価単価
*****DISPLAY "TBL-F05 = " TBL-F05  UPON CONS.
*****MOVE        HEN-A32         TO        HEN-F172.
     MOVE        TBL-F05         TO        HEN-F172.
*売価単価
     MOVE        HEN-A33         TO        HEN-F173.
*原価金額
*****COMPUTE  WK-YEN   =   HEN-A31   *   HEN-A32.
     COMPUTE  WK-YEN   =   HEN-A31   *   TBL-F05.
     MOVE        WK-YEN          TO        HEN-F181.
*売価金額
     COMPUTE  WK-YEN   =   HEN-A31   *   HEN-A33.
     MOVE        WK-YEN          TO        HEN-F182.
*店舗名（備考）
...
*店舗名（カナ）
     MOVE        HEN-A10         TO        HEN-F30.
*システム日付
     MOVE        SYS-DATEW       TO        HEN-F99.
*ルート（直送区分）
*    受信データにルートが含まれていないので"00"を固定
*    MOVE        HEN-A23         TO        HEN-F42.
*****MOVE    "00"                TO        HEN-F42.
     MOVE        HEN-A07         TO        HEN-F42.
*受信日付
...
*訂正前数量
     MOVE        HEN-A31         TO        HEN-F50.
*修正原価単価
*****MOVE        HEN-A32         TO        HEN-F512.
     MOVE        TBL-F05         TO        HEN-F512.
*修正売価単価
     MOVE        HEN-A33         TO        HEN-F513.
*修正原価金額
...
*　　　　　　　ファイル出力（基本情報データ）　　　　　　　　　*
****************************************************************
 HDJ-EDIT-SEC          SECTION.
*
     MOVE    "HDJ-EDIT-SEC" TO        S-NAME.
     MOVE     SPACE         TO        HDJ-REC.
     INITIALIZE                       HDJ-REC.
*伝票ヘッダー転送
     MOVE     HEN-A01       TO        HDJ-A01.
     MOVE     HEN-A02       TO        HDJ-A02.
     MOVE     HEN-A03       TO        HDJ-A03.
     MOVE     HEN-A04       TO        HDJ-A04.
     MOVE     HEN-A05       TO        HDJ-A05.
     MOVE     HEN-A06       TO        HDJ-A06.
     MOVE     HEN-A07       TO        HDJ-A07.
     MOVE     HEN-A08       TO        HDJ-A08.
     MOVE     HEN-A09       TO        HDJ-A09.
     MOVE     HEN-A10       TO        HDJ-A10.
     MOVE     HEN-A11       TO        HDJ-A11.
     MOVE     HEN-A12       TO        HDJ-A12.
     MOVE     HEN-A13       TO        HDJ-A13.
     MOVE     HEN-A14       TO        HDJ-A14.
     MOVE     HEN-A15       TO        HDJ-A15.
     MOVE     HEN-A16       TO        HDJ-A16.
     MOVE     HEN-A17       TO        HDJ-A17.
     MOVE     HEN-A18       TO        HDJ-A18.
     MOVE     HEN-A19       TO        HDJ-A19.
     MOVE     HEN-A20       TO        HDJ-A20.
*伝票明細転送
     MOVE     HEN-A21       TO        HDJ-A21.
     MOVE     HEN-A22       TO        HDJ-A22.
     MOVE     HEN-A23       TO        HDJ-A23.
     MOVE     HEN-A24       TO        HDJ-A24.
     MOVE     HEN-A25       TO        HDJ-A25.
     MOVE     HEN-A26       TO        HDJ-A26.
     MOVE     HEN-A27       TO        HDJ-A27.
     MOVE     HEN-A28       TO        HDJ-A28.
     MOVE     HEN-A29       TO        HDJ-A29.
     MOVE     HEN-A30       TO        HDJ-A30.
     MOVE     HEN-A31       TO        HDJ-A31.
     MOVE     HEN-A32       TO        HDJ-A32.
     MOVE     HEN-A33       TO        HDJ-A33.
     MOVE     HEN-A34       TO        HDJ-A34.
*
     PERFORM  HDJ-TENSO-SEC.
*
     WRITE    HDJ-REC.
*
 HDJ-EDIT-EXIT.
...
*　　　　　　　共通情報転送（基本情報データ）                  *
****************************************************************
 HDJ-TENSO-SEC         SECTION.
*
     MOVE    "HDJ-TENSO-SEC"     TO        S-NAME.
*受信日付
     MOVE     PARA-JDATE         TO        HDJ-F011.
*受信時刻
     MOVE     PARA-JTIME         TO        HDJ-F012.
*取引先コード
     MOVE     WK-TOKCD           TO        HDJ-F013.
*倉庫コード
     MOVE     HEN-F48            TO        HDJ-F02.
*店コード
     MOVE     HDJ-A08            TO        HDJ-F03.
*サカタ伝票番号
     MOVE     HEN-F02            TO        HDJ-F04.
*Ｊ本田　発注番号
     MOVE     HDJ-A04            TO        HDJ-F05.
*行_
     MOVE     HDJ-A24            TO        HDJ-F06.
*納品日
     MOVE     HEN-F112           TO        HDJ-F07.
*
 HDJ-TENSO-EXIT.
     EXIT.
****************************************************************
*　　　　　　　条件ファイル更新                                *
****************************************************************
 JYOKEN1-WRT-SEC         SECTION.
*
     MOVE     "JYOKEN1-WRT-SEC"   TO   S-NAME.
*
     MOVE     CNT-DENNO           TO   JYO-F04.
     REWRITE  JYO-REC.
*
 JYOKEN1-WRT-EXIT.
     EXIT.
