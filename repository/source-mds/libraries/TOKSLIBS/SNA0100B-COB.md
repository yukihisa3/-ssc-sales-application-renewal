...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SNA0100B.COB`
...
*    作成者／更新者　：飯田/NAV
*    処理概要　　　　：
*      苗業務システムへ取消し依頼を行った結果を受け、
*      売上伝票ファイル・連携Ｎｏ管理テーブル・小売連携累積
*      ファイルに対し、その結果の更新処理を行う。
*    作成日／更新日　：2012/07/17
*    作成者／更新者　：井上/NAV
*    処理概要　　　　：
...
*
 01  HEAD01.
     03  FILLER  PIC  X(08)  VALUE "SNA0100B".
 ...
 01  MEISAI01  CHARACTER TYPE IS PIT-2-2W2H.
     ...
     03  FILLER  PIC  N(06)  VALUE NC"取り消し結果".
...
*A---B-------2---------3---------4---------5---------6---------7E*
*****  NC"この連携_は、苗業務システム側で取り消しできませんでし
*-        "た。"
*
 01  MEISAI06.
     03  FILLER          PIC  X(19)  VALUE SPACE.
     03  FILLER          PIC  N(32)  VALUE SPACE
                              CHARACTER TYPE IS PIT-1V5-2W.
*
 01  MEISAI06-MSG.
     03  FILLER          PIC  N(32)  VALUE
...
****************************************************************
 INIT-SEC              SECTION.
     ...
     DISPLAY   "**  START SNA0100B  **"  UPON CONS.
...
****************************************************************
 UPDB-SEC            SECTION.
     ...
     IF  KTR-F05 = "NG"  *> 結果ＣＤ
         PERFORM  UPDC-SEC  *> ＮＧ更新
     ELSE
         PERFORM  UPDC2-SEC *> ＯＫ更新
     END-IF.
...
*ファイル ＣＬＯＳＥ
     ...
     CLOSE  HTANMS.
     CLOSE  PRTFILE.
     DISPLAY   "**  END   SNA0100B  **"  UPON CONS.
*
     MOVE  S-NAME-SV        TO  S-NAME.
*
*↓
...
*↑
        DISPLAY  NC"＊　取り消し結果通知が　　＊" UPON CONS
