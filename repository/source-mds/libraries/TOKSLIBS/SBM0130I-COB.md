...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBM0130I.COB`
...
*
 ...
 01  PARA-DTPTN             PIC   X(01).
 01  PARA-DATE1             PIC   9(08).
 01  PARA-DATE2             PIC   9(08).
****************************************************************
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SBM0130I DSPFILE ERROR " DSP-CNTL " "
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SBM0130I HTOKMS ERROR " HTOKMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE          TO   FBM01301.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FBM01301"     TO   DSP-FMT.
...
***  日付種類
     MOVE    "M"      TO  EDIT-OPTION  OF  DTPTN.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DTPTN.
***  取引先
     MOVE    "M"      TO  EDIT-OPTION  OF  TORICD.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  TORICD.
***  開始日
     MOVE    "M"      TO  EDIT-OPTION  OF  DATE1.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DATE2.
***  終了日
     MOVE    "M"      TO  EDIT-OPTION  OF  DATE2.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DATE2.
*
 215-DSP-SYOKI-EXIT.
     EXIT.
...
*    日付種類チェック
     ...
     OR     DTPTN    =   "2"
            ...
            MOVE     SPACE TO   EDIT-CURSOR  OF  DTPTN
     ELSE
            MOVE NC"日付種類には１か２を入力して下さい。"
            TO  MSGSPC
            MOVE     "C"   TO   EDIT-CURSOR OF DTPTN
            MOVE     "R"   TO   EDIT-OPTION OF DTPTN
            GO             TO   PARA-CHK-EXIT
     END-IF.
*日付範囲チェック
*    （開始日チェック）
     ...
     OR       DATE1    =     ZERO
              MOVE     ZERO  TO   DATE1
              MOVE    "M"    TO   EDIT-OPTION  OF  DATE1
              MOVE     SPACE TO   EDIT-CURSOR  OF  DATE1
     END-IF.
*    （終了日チェック）
     IF       DATE2    NOT NUMERIC
     OR       DATE2    =     ZERO
              MOVE     99999999   TO   DATE2
              MOVE    "M"    TO   EDIT-OPTION  OF  DATE2
              MOVE     SPACE TO   EDIT-CURSOR  OF  DATE2
     END-IF.
*    開始日論理チェック
     ...
     END-IF.
*    終了日論理チェック
     IF       DATE2   NOT =  ZERO     AND  99999999
              MOVE     "2"            TO   LINK-IN-KBN
              MOVE     ZERO           TO   LINK-IN-YMD6
...
*納品日入力範囲チェック
     IF       DATE1   >  DATE2
              MOVE  NC"開始が終了を超えています"  TO  MSGSPC
              MOVE  "R"      TO   EDIT-OPTION  OF  DATE1
              MOVE  "R"      TO   EDIT-OPTION  OF  DATE2
              MOVE  "C"      TO   EDIT-CURSOR  OF  DATE1
              GO             TO   PARA-CHK-EXIT
...
*
     EVALUATE DSP-FNC
         ...
         WHEN ENT
              ...
              MOVE      DTPTN     TO   PARA-DTPTN
              MOVE      DATE1     TO   PARA-DATE1
              MOVE      DATE2     TO   PARA-DATE2
         WHEN OTHER
              MOVE NC"ＰＦキーが違います"   TO   MSGSPC
     END-EVALUATE.
*
