...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SJK0050I.COB`
...
*画面定義ファイル
     SELECT  DSPFILE   ASSIGN    TO        GS-DSPF
                       ...
                       FILE      STATUS    DSP-ST.
*伝票データワーク(伝票単位順）
     SELECT  KPNDATL1  ASSIGN    TO        KPNDATL1
                       ORGANIZATION        INDEXED
                       ACCESS    MODE      RANDOM
                       RECORD    KEY       KP1-F46
                                           KP1-F47
                       ...
                       FILE      STATUS    KP1-ST.
*伝票データワーク(商品単位順）
     SELECT  KPNDATL2  ASSIGN    TO        KPNDATL2
                       ORGANIZATION        INDEXED
                       ACCESS    MODE      RANDOM
                       RECORD    KEY       KP2-F46
                                           KP2-F47
                       ...
                       FILE      STATUS    KP2-ST.
*自動欠品結果ファイル
     SELECT  JIDKEKF   ASSIGN    TO        JIDKEKLA
                       ORGANIZATION        INDEXED
                       ACCESS    MODE      RANDOM
                       RECORD    KEY       KEK-F46
                                           KEK-F47
...
****************************************************************
 FD  KPNDATL1
                       LABEL     RECORD    IS   STANDARD.
                       COPY      KPNDATL1  OF   XFDLIB
                       JOINING   KP1       AS   PREFIX.
****************************************************************
*    FILE = 伝票データワーク（商品単位順）                     *
****************************************************************
 FD  KPNDATL2
                       LABEL     RECORD    IS   STANDARD.
                       COPY      KPNDATL2  OF   XFDLIB
                       JOINING   KP2       AS   PREFIX.
****************************************************************
*    FILE = 自動欠品結果ファイル
****************************************************************
 FD  JIDKEKF
...
*フラグ領域
 01  FLG-AREA.
     ...
     03  JIDKEKF-INV-FLG          PIC  X(03)  VALUE  SPACE.
*カウンタ領域
 01  COUNTER.
...
*
 01  FILE-ERR.
     ...
     03  DSP-ERR           PIC N(20) VALUE
                        NC"画面ファイルエラー".
     03  KP1-ERR           PIC N(20) VALUE
                        NC"欠品ファイルエラー１".
     03  KP2-ERR           PIC N(20) VALUE
                        NC"欠品ファイルエラー２".
...
**************************************************************
 ...
 DSP-ERR                   SECTION.
     ...
     STOP        RUN.
 KP1-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE KPNDATL1.
     DISPLAY     KP1-ERR   UPON      CONS.
     DISPLAY     SEC-NAME  UPON      CONS.
     DISPLAY     KP1-ST    UPON      CONS.
     MOVE        "4000"    TO        PROGRAM-STATUS.
     STOP        RUN.
 KP2-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE KPNDATL2.
     DISPLAY     KP2-ERR   UPON      CONS.
     DISPLAY     SEC-NAME  UPON      CONS.
     DISPLAY     KP2-ST    UPON      CONS.
     MOVE        "4000"    TO        PROGRAM-STATUS.
     STOP        RUN.
 KEK-ERR                   SECTION.
...
*ファイルのＯＰＥＮ
     OPEN     I-O       DSPFILE   KPNDATL1 KPNDATL2.
     OPEN     INPUT     SHTDENF   HTOKMS   ZSOKMS  JIDKEKF.
*ワークの初期化
     INITIALIZE         FLG-AREA.
*カウンタの初期化
     INITIALIZE         COUNTER.
...
****************************************************************
 END-SEC               SECTION.
*ファイル ＣＬＯＳＥ
     CLOSE             SHTDENF  HTOKMS  DSPFILE  ZSOKMS
                       KPNDATL1 KPNDATL2  JIDKEKF.
...
*部門コード右詰
*開始
*    IF  DSP-BUMON1    NOT =     SPACE
*        MOVE   DSP-BUMON1   TO   WK-BUMON
*        PERFORM   UNTIL  WK-BUMON1(4) NOT = SPACE
*            PERFORM VARYING IXA FROM  3 BY -1 UNTIL IXA = 0
*                  MOVE   WK-BUMON1(IXA) TO  WK-BUMON1(IXA + 1)
*            END-PERFORM
*            MOVE      ZERO      TO  WK-BUMON1(1)
*        END-PERFORM
*        MOVE      WK-BUMON      TO   DSP-BUMON1
*    ELSE
*        MOVE   "0000"           TO   DSP-BUMON1
*    END-IF.
*終了
*    IF  DSP-BUMON2    NOT =     SPACE
*        MOVE   DSP-BUMON2   TO   WK-BUMON
*        PERFORM   UNTIL  WK-BUMON1(4) NOT = SPACE
*            PERFORM VARYING IXA FROM  3 BY -1 UNTIL IXA = 0
*                  MOVE   WK-BUMON1(IXA) TO  WK-BUMON1(IXA + 1)
*            END-PERFORM
*            MOVE      ZERO      TO  WK-BUMON1(1)
*        END-PERFORM
*        MOVE      WK-BUMON      TO   DSP-BUMON2
*    ELSE
*        MOVE   "9999"           TO   DSP-BUMON2
*    END-IF.
     IF   DSP-BUMON2 = SPACE
         MOVE   "9999"           TO   DSP-BUMON2
     END-IF.
*部門入力範囲チェック
     IF       DSP-BUMON1  >  DSP-BUMON2
              MOVE  11       TO   ERR-FLG
              MOVE  "R"      TO   EDIT-OPTION  OF  DSP-BUMON1
              MOVE  "R"      TO   EDIT-OPTION  OF  DSP-BUMON2
              MOVE  "C"      TO   EDIT-CURSOR  OF  DSP-BUMON1
              GO             TO   PARA-CHK-EXIT
     ELSE
              MOVE  "M"      TO   EDIT-OPTION  OF  DSP-BUMON2
              MOVE  "M"      TO   EDIT-OPTION  OF  DSP-BUMON2
              MOVE  SPACE    TO   EDIT-CURSOR  OF  DSP-BUMON1
     END-IF.
*伝票出力順未入力チェック
     IF       DSP-KBN    NOT =    1   AND   2   AND   3
              MOVE   6       TO   ERR-FLG
...
***2011.10.11 EN
        INVALID
              ...
              MOVE  "C"      TO   EDIT-CURSOR  OF  DSP-BTDATE
        NOT INVALID
              IF     DSP-SOKCD NOT = SPACE  AND  "00"
                     MOVE   SOKCD    TO     DEN-F48
              END-IF
              READ  SHTDENF
                 AT END
                    MOVE   7   TO   ERR-FLG
...
*実行
         WHEN   "E000"
                MOVE    "3"      TO   PSW
                MOVE    DSP-NOUDT1    TO    PARA-NOUDT1
                MOVE    DSP-NOUDT2    TO    PARA-NOUDT2
...
******************GO               TO   DEN-READ-EXIT
     ...
     END-IF.
*納品日範囲指定
     IF       DEN-F112   >=    DSP-NOUDT1
     AND      DEN-F112   <=    DSP-NOUDT2
              CONTINUE
     ELSE
              ADD         1    TO   SKIP-CNT
              GO               TO   DEN-READ-SEC
     END-IF.
*部門範囲指定
     IF       DEN-F12    >=    DSP-BUMON1
     AND      DEN-F12    <=    DSP-BUMON2
              CONTINUE
     ELSE
              ADD         1    TO   SKIP-CNT
              GO               TO   DEN-READ-SEC
...
*出力順選択
     IF       DEN-F50   NOT =   DEN-F15
        IF       DSP-KBN          =    1
                 MOVE     DEN-REC   TO    KP1-REC
                 PERFORM  JIDKEKF-READ-SEC
                 IF  JIDKEKF-INV-FLG = SPACE
                     MOVE KEK-F70   TO    KP1-F70
                 ELSE
                     MOVE SPACE     TO    KP1-F70
                 END-IF
                 READ     KPNDATL1
                       INVALID  KEY
                          WRITE     KP1-REC
                 END-READ
        ELSE
                 MOVE     DEN-REC   TO    KP2-REC
                 PERFORM  JIDKEKF-READ-SEC
                 IF  JIDKEKF-INV-FLG = SPACE
                     MOVE KEK-F70   TO    KP2-F70
                 ELSE
                     MOVE SPACE     TO    KP2-F70
                 END-IF
                 READ     KPNDATL2
                       INVALID  KEY
                          WRITE     KP2-REC
...
***  終了納品日
     ...
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DSP-NOUDT2.
***  開始部門　
     MOVE    "M"      TO  EDIT-OPTION  OF  DSP-BUMON1.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DSP-BUMON1.
***  終了部門　
     MOVE    "M"      TO  EDIT-OPTION  OF  DSP-BUMON2.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DSP-BUMON2.
***  出力順　　　　　
     MOVE    "M"      TO  EDIT-OPTION  OF  DSP-KBN.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DSP-KBN.
*
...
*    自動欠品結果ファイル読込
****************************************************************
 JIDKEKF-READ-SEC      SECTION.
     MOVE     "JIDKEKF-READ-SEC"  TO   S-NAME.
*
     MOVE    DEN-F46              TO   KEK-F46
     MOVE    DEN-F47              TO   KEK-F47
     MOVE    DEN-F01              TO   KEK-F01
     MOVE    DEN-F48              TO   KEK-F48
     MOVE    DEN-F02              TO   KEK-F02
     MOVE    DEN-F04              TO   KEK-F04
     MOVE    DEN-F051             TO   KEK-F051
     MOVE    DEN-F07              TO   KEK-F07
     MOVE    DEN-F112             TO   KEK-F112
     MOVE    DEN-F03              TO   KEK-F03
     READ  JIDKEKF
           INVALID
           MOVE  "INV"            TO   JIDKEKF-INV-FLG
           NOT   INVALID
           MOVE  SPACE            TO   JIDKEKF-INV-FLG
     END-READ.
*
 JIDKEKF-READ-EXIT.
     EXIT.
*****************<<  SJK0050I   END PROGRAM  >>******************
