...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY9014L.COB`
...
****************************************************************
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    サブシステム　　　　：　オージョイフル物品受領ＣＳＶ　　　*
*    業務名　　　　　　　：　物品受領書                        *
*    モジュール名　　　　：　物品受領書                        *
*    作成日／更新日　　　：　07/05/29                          *
*    作成者／更新者　　　：　NAV                               *
*    処理概要　　　　　　：　オージョイフル物品受領ＣＳＶデータ*
*    　　　　　　　　　　　　を出力する。　　　　　　　　　　  *
****************************************************************
****************************************************************
...
*
*****<<  受領ﾃﾞｰﾀCSV   >>**************************************
     SELECT   OJFJYRDT           ASSIGN    TO   DCMJYRDT
                                 STATUS         DCM-STATUS.
*                                                                *
****************************************************************
 DATA                   DIVISION.
...
*--------------------------------------------------------------*
 FD  OJFJYRDT           BLOCK CONTAINS 1    RECORDS.
 01  DCM-REC.
     03  FILLER         PIC       X(500).
*
*----------------------------------------------------------------*
...
*----------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*ヘッダ情報格納領域
     COPY   OJOJYRDT OF XFDLIB  JOINING   DJR  AS   PREFIX.
**** エンドフラグ
...
***** メッセージエリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSY9014L".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSY9014L".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND1.
...
*             プロセス                      0.0                *
****************************************************************
 SSY9014L-START         SECTION.
*
     MOVE   "SSY9014L-START"      TO   S-NAME.
     PERFORM            INIT-SEC.
...
*
     ...
     ELSE
              EVALUATE   LINK-TOKCD
                  WHEN   1681
                  MOVE NC"オージョイフル受領ＤＴ（資材）"
                                          TO  TAITL0-01
                  WHEN   1682
                  MOVE NC"オージョイフル受領ＤＴ（植物）"
                                          TO  TAITL0-01
                  WHEN   OTHER
                  MOVE NC"＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊"
                                          TO  TAITL0-01
...
*    オージョイフル物品受領データスタート
****************************************************************
 OJFJYRF-START-SEC          SECTION.
*
     MOVE    "OJFJYRF-START-SEC" TO   S-NAME.
*
     MOVE     SPACE              TO   JYU-REC.
     INITIALIZE                       JYU-REC.
     MOVE     NENGETUDO          TO   JYU-F17.
     MOVE     LINK-TOKCD         TO   JYU-F00.
     START    OJFJYRF  KEY  IS  >=    JYU-F17  JYU-F00  JYU-F04
