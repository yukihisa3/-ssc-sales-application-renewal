...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBT0305B.COB`
...
*ＪＡＮＣＤ変換ＴＢＬ
     SELECT  JANHENF     ASSIGN    TO       DA-01-VI-JANHENL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      RANDOM
                        RECORD    KEY       JAN-F01 JAN-F02
                        FILE      STATUS    IS   JAN-ST.
*********
 DATA                   DIVISION.
 FILE                   SECTION.
******************************************************************
...
*    ＪＡＮＣＤ変換ＴＢＬ
******************************************************************
 FD  JANHENF             LABEL RECORD   IS   STANDARD.
     COPY     JANHENF    OF       XFDLIB
              JOINING   JAN       PREFIX.
******************************************************************
...
*ワーク項目
 ...
 01  JANHENF-INV-FLG         PIC  X(03)     VALUE  SPACE.
...
*メッセージ出力
 01  FILE-ERR.
     ...
     03  SPM-ERR           PIC  N(20)  VALUE
         NC"出荷パターンマスタエラー".
     03  JAN-ERR           PIC  N(20)  VALUE
         NC"ＪＡＮＣＤ変換ＴＢＬエラー".
*
 01  MSG-AREA.
     03  MSG-START.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  ST-PG          PIC   X(08)  VALUE "SBT0305B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SBT0305B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
******************************************************************
 ...
 SPM-ERR                   SECTION.
     ...
     STOP        RUN.
 JAN-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE JANHENF.
...
************#2017/06/06 NAV ST
            IF  DEN-F01 = 13938 OR 139381 OR 17137 OR 171371 OR
                        100403 OR 100404 OR 100441 OR 100442 OR
                        100427 OR 100428
...
*2013/02/18↑追加
     END-IF.
*ＪＡＮＣＤ変換ＴＢＬ索引*******************
     MOVE     DEN-F01                      TO   JAN-F01.
     MOVE     DEN-F25                      TO   JAN-F02.
     PERFORM  JANHENF-READ-SEC.
     IF  JANHENF-INV-FLG = SPACE
         MOVE JAN-F03                      TO   SYU-F14
*********MOVE "0"                          TO   SYU-F17
*********MOVE SPACE                        TO   SYU-F18
     END-IF.
********************************************
...
*    ＪＡＮＣＤ変換ＴＢＬ検索                                  *
****************************************************************
 JANHENF-READ-SEC         SECTION.
*
     READ  JANHENF
       INVALID
         MOVE  "INV"             TO  JANHENF-INV-FLG
       NOT INVALID
         MOVE  SPACE             TO  JANHENF-INV-FLG
     END-READ.
*
 JANHENF-READ-EXIT.
     EXIT.
