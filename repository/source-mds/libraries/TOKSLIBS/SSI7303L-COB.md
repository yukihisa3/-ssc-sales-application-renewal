...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSI7303L.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 支払明細データ >>--*
     SELECT   DYSIHF   ASSIGN         DA-01-VI-DYSIHL2
                        ORGANIZATION   INDEXED
                        ACCESS    MODE SEQUENTIAL
                        RECORD    KEY  SIH-F10  SIH-F00
                                       SIH-F01  SIH-F02
                                       SIH-F03
                        STATUS         SIH-ST.
*----<< 店舗マスタ >>--*
...
*--------------------------------------------------------------*
 ...
 01  IDX.
     03  I              PIC  9(03).
 01  ID-PROGRAM.
     03  PG-ID          PIC  X(08)     VALUE  "SSI7303L".
*
*----<< ﾌｱｲﾙ ｽﾃｰﾀｽ >>--*
 01  SIH-ST             PIC  X(02).
 01  TEN-ST             PIC  X(02).
...
*
 01  SIHARAI-JYOUHO.
     ...
     03  WK-TORGK-KEI   PIC S9(09)    VALUE  ZERO.
...
*
 ...
 01  MS06.
     03  FILLER         CHARACTER TYPE YA.
         05  FILLER     PIC  X(79)     VALUE  SPACE.
         05  FILLER     PIC  N(05)     VALUE  NC"＃取引計＃".
         05  FILLER     PIC  X(02)     VALUE  SPACE.
         05  MS06-TGKIN PIC  ---,---,--9.
 01  MS07.
     03  FILLER         CHARACTER TYPE YA.
...
*----<< 支払明細データ >>--*
 SHI-ERR                SECTION.
     USE AFTER     EXCEPTION PROCEDURE      DYSIHF.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### " PG-ID "  DYSIHF  ERROR " SIH-ST " "
...
*    START  DYSIHF  KEY  IS  >=  SIH-F11  SIH-F01  SIH-F02
*                                  SIH-F03  SIH-F04
*           INVALID
...
*
     IF   END-FLG NOT = "END"
          MOVE  SIH-F00      TO   WK-TORICD
          MOVE  SIH-F02      TO   WK-TENCD
          MOVE  SIH-F10      TO   WK-SIHKBN
     END-IF.
*
 100-INIT-RTN-EXIT.
     EXIT.
...
*********    相殺明細タイトル行出力制御フラグセット
     ...
     ELSE
*********    店明細行出力時、相殺明細タイトル制御フラグクリア
           IF   SIH-FLG  NOT = SPACE
                MOVE   SPACE  TO   SIH-FLG
           END-IF
           PERFORM  MEISAI-WT-SEC
     END-IF.
...
*
     MOVE     SPACE               TO   MS01.
*店舗情報取得
     IF   WK-TENCD1  NOT =  SIH-F02
*         店舗コード
          MOVE     SIH-F02        TO   MS01-TENCD
*         店舗名
          MOVE     SIH-F02        TO   WK-TENCD1
**********MOVE     SIH-F02        TO   TEN-F52
**********#2018/01/29 NAV ST
**********MOVE     1225           TO   TEN-F52
          MOVE     SIH-F00        TO   TEN-F52
**********#2018/01/29 NAV ED
          MOVE     SIH-F02        TO   TEN-F011
     ...
     END-IF.
*伝票区分
     MOVE     SIH-F04(1:2)        TO  MS01-DENKB.
     EVALUATE SIH-F04(1:2)
*        WHEN "10" MOVE NC"ＴＡ"       TO  MS01-DENKM
*        WHEN "20" MOVE NC"手書"       TO  MS01-DENKM
*        WHEN "30" MOVE NC"返品"       TO  MS01-DENKM
         WHEN "00" MOVE NC"定番"       TO  MS01-DENKM
         WHEN "01" MOVE NC"合同"       TO  MS01-DENKM
         WHEN "02" MOVE NC"企画"       TO  MS01-DENKM
         WHEN "03" MOVE NC"新店"       TO  MS01-DENKM
         WHEN "04" MOVE NC"_替"       TO  MS01-DENKM
         WHEN "05" MOVE NC"特売"       TO  MS01-DENKM
         WHEN "07" MOVE NC"ダイレクト" TO  MS01-DENKM
         WHEN "08" MOVE NC"手書"       TO  MS01-DENKM
         WHEN "50" MOVE NC"仕入"       TO  MS01-DENKM
         WHEN "51" MOVE NC"返品"       TO  MS01-DENKM
         WHEN "52" MOVE NC"値引"       TO  MS01-DENKM
         WHEN "55" MOVE NC"仕入修正"   TO  MS01-DENKM
         WHEN "56" MOVE NC"返品修正"   TO  MS01-DENKM
         WHEN "57" MOVE NC"値引修正"   TO  MS01-DENKM
     END-EVALUATE.
*伝票番号
     MOVE     SIH-F06             TO  MS01-DENNO.
*検収日
     MOVE     SIH-F03(3:2)        TO  MS01-YY.
     MOVE     NC"年"              TO  MS01-NEN.
     MOVE     SIH-F03(5:2)        TO  MS01-MM.
     MOVE     NC"月"              TO  MS01-TUK.
     MOVE     SIH-F03(7:2)        TO  MS01-DD.
     MOVE     NC"日"              TO  MS01-DAY.
*支払金額
     MOVE     SIH-F07             TO  MS01-SKIN.
     ADD      SIH-F07             TO  WK-TENPO-KEI.
     ADD      SIH-F07             TO  WK-SIHGK-KEI.
...
**** IF   WK-TENCD1  NOT =  SIH-F03
     MOVE     SIH-F08        TO   MS04-SOSCD.
*相殺名称
     MOVE     SIH-F09        TO   MS04-SOSME.
*相殺金額
     MOVE     SIH-F07             TO  MS04-SOSKN.
     ADD      SIH-F07             TO  WK-SOSAI-KEI.
...
*    LEVEL  7      取引先計出力                                *
*--------------------------------------------------------------*
 TORIKEI-WT-SEC         SECTION.
*
     IF       LINE-CNT  >    56
              PERFORM   HEAD-WT-SEC
     END-IF.
*
     COMPUTE  WK-TORGK-KEI   = WK-SIHGK-KEI + WK-SOSAI-KEI.
*
     MOVE     WK-TORGK-KEI   TO   MS06-TGKIN.
     WRITE    PRT-REC        FROM MS06      AFTER     1.
     ADD      1              TO   LINE-CNT.
*
     ADD      WK-TORGK-KEI   TO   WK-SOUGK-KEI.
     MOVE     ZERO           TO   WK-SIHGK-KEI.
     MOVE     ZERO           TO   WK-SOSAI-KEI.
     MOVE     ZERO           TO   WK-TORGK-KEI.
*
 SOKEI-WT-EXIT.
     EXIT.
*--------------------------------------------------------------*
...
*
     MOVE     WK-SOUGK-KEI   TO   MS07-SGKIN.
     WRITE    PRT-REC        FROM MS07      AFTER     1.
     ADD      1              TO   LINE-CNT.
*
 SOKEI-WT-EXIT.
     EXIT.
...
*取引先ＣＤ
     MOVE     SIH-F00             TO   HD02-TORCD.
*買掛締日付
     MOVE     SIH-F12(3:2)        TO   HD03-YY.
     MOVE     SIH-F12(5:2)        TO   HD03-MM.
     MOVE     SIH-F12(7:2)        TO   HD03-DD.
*買掛締日付開始
     MOVE     SIH-F11(3:2)        TO   HD04-YYS.
     MOVE     SIH-F11(5:2)        TO   HD04-MMS.
     MOVE     SIH-F11(7:2)        TO   HD04-DDS.
*買掛締日付終了
     MOVE     SIH-F12(3:2)        TO   HD04-YYE.
     MOVE     SIH-F12(5:2)        TO   HD04-MME.
     MOVE     SIH-F12(7:2)        TO   HD04-DDE.
*ＨＥＡＤ出力
