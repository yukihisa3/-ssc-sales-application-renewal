...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSKT150N.COB`
...
*--------------------------------------------------------------*
 01  PGM-ID                  PIC  X(08)     VALUE  "OSKT150N".
*----<< ﾃﾞｨｽﾌﾟﾚｲ ｺﾝﾄﾛｰﾙ ｴﾘｱ >>-*
 01  DSP-CNTL.
     03  DSP-FMT             PIC  X(08).
     03  DSP-GRP             PIC  X(08).
...
************************************* KEY ADD
         ...
         INVALID   KEY
                   MOVE      9       TO   EOF-FLG
         NOT INVALID   KEY
*****DISPLAY "EOF-FLG1   = " EOF-FLG UPON CONS
              PERFORM  DEN-RD-SEC
     END-START.
*****DISPLAY "EOF-FLG2   = " EOF-FLG UPON CONS.
*****DISPLAY "DSP-JISH01 = " DSP-JISH01(1) UPON CONS.
...
*--------------------------------------------------------------*
 DEN-RD-SEC           SECTION.
     ...
     END-READ.
*
*****DISPLAY "DEN-F01X= " DEN-F01 UPON CONS.
*****DISPLAY "DEN-F02X= " DEN-F02 UPON CONS.
 DEN-RD-010.
 ...
 DEN-RD-020.
*自社伝票_が９９９９を越えたら対象外
*2012/03/29 NAV チェック桁数を変更 ST*
*##2014/01/14 NAV ST 桁数を５→７に変更
*****IF       DEN-F02   >      9999
*****IF       DEN-F02   >      99999
     IF       DEN-F02   >      9999999
              MOVE      9         TO   EOF-FLG
...
************** KEY ADD
              MOVE     ZERO  TO   DEN1-F07 DEN1-F112
              MOVE     MEI-TENCD  (I)   TO   DEN1-F07
              START    HDENJNL1 KEY >=
                             DEN1-F01  DEN1-F23  DEN1-F04
************** KEY ADD
**************               DEN1-F051 DEN1-F03
                             DEN1-F051 DEN1-F07  DEN1-F112
                             DEN1-F03
                INVALID  KEY
                     MOVE     9        TO   EOF-FLG
                NOT  INVALID  KEY
