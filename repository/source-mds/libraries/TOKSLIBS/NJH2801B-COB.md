...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/NJH2801B.COB`
...
*データ情報退避ワーク
 01  WK-FH-REC.
     ...
     03  WK-FH05                  PIC       9(06).
     03  WK-FH06.
         05  WK-FH061             PIC       9(04).
         05  WK-FH062             PIC       9(02).
         05  WK-FH063             PIC       9(02).
     03  WK-FH07.
         05  WK-FH071             PIC       9(06).
         05  WK-FH072             PIC       9(02).
     03  WK-FH08.
         05  WK-FH081             PIC       9(06).
         05  WK-FH082             PIC       9(02).
     03  WK-FH09                  PIC       9(03).
     03  WK-FH10                  PIC       9(06).
     03  WK-FH11                  PIC       9(05).
     03  WK-FH12                  PIC       X(01).
...
*    ヘッドレコード退避ワーク
 01  WK-DH-REC.
     ...
     03  WK-DH09                  PIC       9(06).
     03  WK-DH10.
         05  WK-DH101             PIC       9(06).
         05  WK-DH102             PIC       9(02).
     03  WK-DH11                  PIC       X(15).
     03  WK-DH12                  PIC       X(15).
     03  WK-DH13                  PIC       X(20).
     03  WK-DH14                  PIC       X(15).
...
*＜伝票明細レコード＞
 01  WK-DD-REC.
     ...
     03  WK-DD06                  PIC       9(04).
     03  WK-DD07                  PIC       X(02).
     03  WK-DD08                  PIC       9(05)V9(01).
     03  WK-DD09                  PIC       9(07)V9(02).
     03  WK-DD10                  PIC       9(07).
     03  WK-DD11                  PIC       9(10).
     03  WK-DD12                  PIC       9(10).
     03  WK-DD13                  PIC       X(09).
...
*システム日付の編集
 ...
 01  WK-WAREKI-HEN.
     03  WAREKI-YY         PIC 9(02).
     03  WAREKI-MM         PIC 9(02).
     03  WAREKI-DD         PIC 9(02).
 01  WK-ST.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "NJH2801B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NJH2801B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*ルート属性変換
 01  WK-RUTO-HENKAN.
     03  WK-HENKAN                PIC       X(02).
     03  WK-HENKAN-R              REDEFINES WK-HENKAN.
         05  WK-HEN-RUTO          PIC       9(02).
*
 01  LINK-AREA.
...
*各量販店領域転送
*    取引先コード
     MOVE     WK-DH101      TO        HEN-A01.
*    伝票_
     MOVE     WK-DH03       TO        HEN-A02.
*    行_
     MOVE     WK-DD03       TO        HEN-A03.
*    伝票区分
     MOVE     WK-DH07       TO        HEN-A04.
*    分類コード
     MOVE     WK-DH06       TO        HEN-A05.
*    社名
     MOVE     WK-DH11       TO        HEN-A06.
*    店舗名
     MOVE     WK-DH12       TO        HEN-A07.
*    取引先名称
     MOVE     WK-DH13       TO        HEN-A08.
*    発注日
     MOVE     WK-DH08       TO        HEN-A09.
*    納品日
     MOVE     WK-DH09       TO        HEN-A10.
*    直送先名
     MOVE     WK-DH14       TO        HEN-A11.
*    ルート
     MOVE     WK-DH14(4:2)  TO        WK-HENKAN.
     MOVE     WK-HEN-RUTO   TO        HEN-A12.
*    商品コード＜色柄サイズ１＞
     MOVE     WK-DD04       TO        HEN-A13.
*    商品名
     MOVE     WK-DD14       TO        HEN-A14.
*    ＪＡＮコード
     MOVE     WK-DD15       TO        HEN-A15.
*    発注入数
     MOVE     WK-DD05       TO        HEN-A16.
*    ケース数
     MOVE     WK-DD06       TO        HEN-A17.
*    単位
     MOVE     WK-DD07       TO        HEN-A18.
*    数量
     MOVE     WK-DD08       TO        HEN-A19.
*    原価単価
     MOVE     WK-DD09       TO        HEN-A20.
*    売価単価
     MOVE     WK-DD10       TO        HEN-A21.
*    原価金額
     MOVE     WK-DD11       TO        HEN-A22.
*    売価金額
     MOVE     WK-DD12       TO        HEN-A23.
*    社コード
     MOVE     WK-DH04       TO        HEN-A24.
*    店舗コード
     MOVE     WK-DH05       TO        HEN-A25.
*    ＜配送センターコード＞
     MOVE     WK-DH15       TO        HEN-A27.
*    ＜ＥＤＩ区分　　　　＞
     MOVE     WK-DH16       TO        HEN-A28.
*    ＜在庫区分　　　　　＞
     MOVE     WK-DH17       TO        HEN-A29.
*    ＜店舗区分　　　　　＞
     MOVE     WK-DH18       TO        HEN-A30.
*    ＜納入ＲＴ　　　　　＞
     MOVE     WK-DH19       TO        HEN-A31.
*    ＜ＰＴ識別　　　　　＞
     MOVE     WK-DH20       TO        HEN-A32.
*    ＜直送名２　　　　　＞
     MOVE     WK-DH21       TO        HEN-A33.
*    ＜細分類コード　　　＞
     MOVE     WK-DD16       TO        HEN-A34.
*    ＜色柄サイズ２　　　＞
     MOVE     WK-DD13       TO        HEN-A35.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
*****MOVE        HEN-A09   TO        WK-WAREKI-HEN.
*****COMPUTE     WAREKI-YY  =  WAREKI-YY + 88.
*****MOVE  WK-WAREKI-HEN   TO        HEN-A09.
*****COMPUTE     HEN-F111  = WK-NEN * 10000 + HEN-A09.
     ...
     END-IF.
*発注日  (年を条件ファイルの置換値とプラスして西暦４桁にする）
*****MOVE        HEN-A10   TO        WK-WAREKI-HEN.
*****COMPUTE     WAREKI-YY  =  WAREKI-YY + 88.
*****MOVE  WK-WAREKI-HEN   TO        HEN-A10.
*****COMPUTE     HEN-F112  = WK-NEN * 10000 + HEN-A10.
     MOVE       "4"        TO        LINK-IN-KBN.
     ...
     CALL       "SKYDTCKB"   USING   LINK-IN-KBN
                                     ...
                                     LINK-OUT-YMD8.
     IF          LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8  TO   HEN-A10
                                     HEN-F112
     ELSE
         MOVE    ZERO           TO   HEN-F10
                                     HEN-F112
     END-IF.
*分類（部門）
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名（商品名）
         MOVE    HEN-A14(1:15)  TO   HEN-F1421
*        商品名（規格）
         MOVE    HEN-A14(16:10) TO   HEN-F1422
       NOT INVALID
*        商品名（商品名）
         MOVE    HEN-A14(1:15)  TO   HEN-F1421
*        商品名（規格）
...
*    ルート条件マスタ検索
     ...
     MOVE     HEN-A12(7:2) TO        RUT-F03  HEN-F42.
*****DISPLAY "HEN-A01 = " HEN-A01 UPON CONS.
*****DISPLAY "HEN-A12 = " HEN-A12 UPON CONS.
     READ     JHMRUTL1
         INVALID
           MOVE  1         TO        INV-RUT
