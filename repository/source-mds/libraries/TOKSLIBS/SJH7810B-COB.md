...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SJH7810B.COB`
...
*                            変換伝票データファイルを作成する　*
*　　　　　　　　　　　　　　ロイヤルホームセンター　　　　　　*
*    2008/10/21 新ＥＤＩシステムへ移行                         *
****************************************************************
 IDENTIFICATION         DIVISION.
*
 PROGRAM-ID.            SJH7810B.
 AUTHOR.                NAV.
 DATE-WRITTEN.          08/10/21.
*
...
*ファイルヘッドレコード退避ワーク
 01  WK-DEPA-REC.
     ...
     03  WK-DEPA04          PIC  9(07).
     03  WK-DEPA05          PIC  9(11).
     03  WK-DEPA06          PIC  9(06).
     03  WK-DEPA07          PIC  9(06).
     03  WK-DEPA08          PIC  X(100).
     03  WK-DEPA09          PIC  X(86).
     03  WK-DEPA10          PIC  X(10).
     03  WK-DEPA11          PIC  9(11).
     03  WK-DEPA12          PIC  9(01).
...
*ヘッドレコード退避ワーク
 01  WK-DEPB-REC.
     ...
     03  WK-DEPB09          PIC  X(28).
     03  WK-DEPB10          PIC  9(01).
     03  WK-DEPB11          PIC  9(09).
     03  WK-DEPB12          PIC  X(145).
     03  WK-DEPB13          PIC  X(08).
     03  WK-DEPB14          PIC  X(02).
     03  WK-DEPB15          PIC  9(11).
     03  WK-DEPB16          PIC  9(01).
     03  WK-DEPB17          PIC  9(06).
*    明細レコード退避ワーク
 01  WK-DEPD-REC.
     ...
     03  WK-DEPD08          PIC  X(12).
     03  WK-DEPD09          PIC  9(05).
     03  WK-DEPD10          PIC  9(01).
     03  WK-DEPD11          PIC  9(08)V9(02).
     03  WK-DEPD12          PIC  9(01).
     03  WK-DEPD13          PIC  9(09).
     03  WK-DEPD14          PIC  9(01).
     03  WK-DEPD15          PIC  9(08)V9(02).
     03  WK-DEPD16          PIC  9(01).
     03  WK-DEPD17          PIC  9(09).
     03  WK-DEPD18          PIC  9(02).
     03  WK-DEPD19          PIC  9(01).
     03  WK-DEPD20          PIC  9(01).
     03  WK-DEPD21          PIC  9(02).
     03  WK-DEPD22          PIC  9(06).
     03  WK-DEPD23          PIC  X(100).
     03  WK-DEPD24          PIC  X(10).
     03  WK-DEPD25          PIC  X(09).
     03  WK-DEPD26          PIC  X(08).
     03  WK-DEPD27          PIC  X(02).
     03  WK-DEPD28          PIC  9(11).
     03  WK-DEPD29          PIC  9(01).
     03  WK-DEPD30          PIC  9(06).
*
 01  WK-AREA.
*システム日付の編集
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJH7810B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH7810B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
****************************************************************
 ...
 INIT-020.
     OPEN     I-O       JHMKENL1  JHMTJSL1  RHJOHOF.
 INIT-030.
     OPEN     OUTPUT    VLD500.
...
*    売価金額２
     MOVE     WK-DEPD17     TO        HEN-A34.
*    課税区分
     MOVE     WK-DEPD18     TO        HEN-A35.
*    消費税（原価）
     MOVE     WK-DEPD19     TO        HEN-A36.
*    消費税（売価）
     MOVE     WK-DEPD20     TO        HEN-A37.
*    税率
     MOVE     WK-DEPD21     TO        HEN-A38.
*    自社_番（ゴンドラ_）
     MOVE     WK-DEPD22     TO        HEN-A39.
*    商品名
     MOVE     WK-DEPD23     TO        HEN-A40.
*    色柄サイズ（カナセット）
     MOVE     WK-DEPD24     TO        HEN-A41.
*    ＦＩＬ
     MOVE     WK-DEPD25     TO        HEN-A42.
*    ＲＳＥＱ１
     MOVE     WK-DEPD26     TO        HEN-A43.
*    ＲＳＥＱ２
     MOVE     WK-DEPD27     TO        HEN-A44.
*    取引先コード
     MOVE     WK-DEPD28     TO        HEN-A45.
*    ヘッダフラグ
     MOVE     WK-DEPD29     TO        HEN-A46.
*    ＳＥＱ_
     MOVE     WK-DEPD30     TO        HEN-A47.
*発注基本情報ファイル用
*ファイルヘッダ情報
     MOVE     WK-DEPA-REC   TO        JOH-A00.
*伝票ヘッダ情報
     MOVE     WK-DEPB-REC   TO        JOH-B00.
*伝票明細情報
     MOVE     WK-DEPD-REC   TO        JOH-C00.
*
     PERFORM  TENSO-SEC.
*
     WRITE    JOH-REC.
...
*取引先コード
     MOVE    HEN-A06(4:6)        TO        HEN-F01  JOH-F03.
     MOVE    HEN-A06(4:6)        TO        WK-TOKCD.
*伝票ナンバー
     MOVE    HEN-A05             TO        HEN-F02  JOH-F06
                                           HEN-F23.
*行番号
     MOVE    HEN-A20             TO        HEN-F03  JOH-F07.
*伝区
     MOVE        40              TO        HEN-F051.
...
*担当者コード
     MOVE        99              TO        HEN-F06.
*店コード
     MOVE    HEN-A02             TO        HEN-F07  JOH-F05.
*  商品変換テーブル検索（ＪＡＮＣＤで検索）
     MOVE        SPACE           TO        TBL-REC.
     INITIALIZE                            TBL-REC.
     MOVE        HEN-F01         TO        TBL-F01.
     MOVE        HEN-A22         TO        TBL-F02.
     READ    SHOTBL1
       INVALID
         MOVE    SPACE           TO        TBL-REC
         INITIALIZE                        TBL-REC
...
*    END-IF.
     MOVE        HEN-A08         TO        HEN-F112  JOH-F08.
*****MOVE        HEN-F112        TO        HEN-F113.
*分類（部門）
     MOVE        HEN-A03         TO        HEN-F12.
*商品区分　　
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名
         MOVE    HEN-A40(1:15)   TO   HEN-F1421
         MOVE    HEN-A40(16:15)  TO   HEN-F1422
       NOT INVALID
         MOVE    HEN-A40(1:15)   TO   HEN-F1421
         MOVE    HEN-A40(16:15)  TO   HEN-F1422
     END-READ.
...
*売価単価
     MOVE        HEN-A30         TO        HEN-F173.
*原価金額
     COMPUTE     HEN-F181  =  HEN-A26  *  HEN-A28.
*売価金額
     COMPUTE     HEN-F182  =  HEN-A26  *  HEN-A30.
*店舗名（備考）
...
*    得意先マスタ検索
     ...
     END-IF.
*相手商品コード
*****MOVE        HEN-A24         TO        HEN-F25.
     MOVE        HEN-A22         TO        HEN-F25.
*伝票発行区分
     MOVE        9               TO        HEN-F272.
*オンライン区分
     MOVE        1               TO        HEN-F274.
...
*店舗名（カナ）
     MOVE        HEN-A09(1:10)   TO        HEN-F30.
*システム日付
     MOVE        SYS-DATEW       TO        HEN-F99.
*ルート（直送区分）
     MOVE        HEN-A10         TO        HEN-F42.
*受信日付
