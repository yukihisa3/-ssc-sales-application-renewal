...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3766B.COB`
...
*      ＴＲＡＮＴＲＡＮ連携データ（箱数ファイル）
       ...
       WHEN  KY-HK3 > KY-ST3
                       *> "＞"：箱数無し、数量訂正Ｆ有り
        DISPLAY "SSY3766B NFHAKOF RECORD NOT FOUND KEY="
          KY-ST3 "*"  UPON CONS
          MOVE  "4000"      TO  PROGRAM-STATUS
          STOP RUN
**         PERFORM  UNTIL FG-NFSUTEF-END = "END"
...
*  基本情報マスタ
     ...
     IF  FG-NFJOHOF-INV = 1
         DISPLAY "SSY3766B NFJOHOF RECORD NOT FOUND KEY="
                 ST3-F01
             "," ST3-F06
             "," ST3-F07
             "," ST3-F08
...
*発注形態区分
     ...
     MOVE  10               TO  FXD-HACH-KEITAI-KBN.
*直送区分
     IF  FG-SAKUBAF-INV = 1
         MOVE ZERO          TO  FXD-TYOKSO-KBN
     ELSE
         MOVE SKB-F03       TO  FXD-TYOKSO-KBN *> 直送ＣＤ
     END-IF.
*商品CD（８桁）インストア
