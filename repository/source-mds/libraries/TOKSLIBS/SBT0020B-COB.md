...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBT0020B.COB`
...
*　　更新日／更新者　　　：　2017/06/06                        *
*　　更新日／更新者　　　：　NAV TAKAHASHI                     *
*    変更概要　　　　　　：　カーマ／ダイキの時、形状区分の　　*
*    　　　　　　　　　　　　１桁目に発注種別区分をセット　　　*
*　　更新日／更新者　　　：　2019/06/20                        *
*　　更新日／更新者　　　：　NAV TAKAHASHI                     *
*    変更概要　　　　　　：　ジュンテンドー流通ＢＭＳ移行対応　*
*　　更新日／更新者　　　：　2021/02/26                        *
*　　更新日／更新者　　　：　NAV TAKAHASHI                     *
*    変更概要　　　　　　：　ＤＣＭ仕入先統合対応　　　　　　　*
*　　更新日／更新者　　　：　2022/03/14                        *
...
*#2019/06/20 NAV ST ジュンテンドー用（流通ＢＭＳ移行後）
 01  WK-JUNTEN.
     03  WK-JUNTEN-01        PIC  X(06).
     03  WK-JUNTEN-02        PIC  X(06).
     03  WK-JUNTEN-03        PIC  X(02).
     03  WK-JUNTEN-04        PIC  X(02).
     03  WK-JUNTEN-05        PIC  X(03).
*#2019/06/20 NAV ED
*プログラムＳＴＡＴＵＳ.
 01  WK-ST.
     03  DEN-ST        PIC  X(02).
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SBT0020B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SBT0020B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*発注種別区分退避
 01  WK-HATYU-SYUBETU.
     03  WK-SYUBETU         PIC   X(02)  VALUE  SPACE.
     03  WK-SYUBETU-HEN     PIC   X(01)  VALUE  SPACE.
*ダイキ情報格納領域
     COPY   DISHIRED OF XFDLIB  JOINING   DIS  AS   PREFIX.
*カーマ情報格納領域
...
*#2019/06/20 NAV ST ジュンテンドー流通ＢＭＳ移行対応
     IF   DEN-F01  =  5116
          MOVE  DEN-XI0004(1:6)   TO   WK-JUNTEN-01
          MOVE  DEN-XI0004(7:6)   TO   WK-JUNTEN-02
          MOVE  DEN-XI0004(13:2)  TO   WK-JUNTEN-03
          MOVE  DEN-XI0004(15:2)  TO   WK-JUNTEN-04
          MOVE  DEN-XI0004(17:3)  TO   WK-JUNTEN-05
     END-IF.
*#2019/06/20 NAV ED
*レコード初期化
...
*                  バッチ日付でどちらで判定するか変更する。
*2013/03/04以前は旧バージョン／以降は新バージョン
         IF   DEN-F46  < 20130304
              IF       DEN-XI0004(123:1)  = 1
...
*********MOVE DEN-F07                 TO  SYU-F05
         IF   DEN-F01  =  5116
              IF   WK-JUNTEN-04  =  "02"
                   MOVE WK-JUNTEN-01(1:3)  TO  SYU-F05(3:3)
                   MOVE "00"               TO  SYU-F05(1:2)
              ELSE
                   MOVE DEN-F07            TO  SYU-F05
              END-IF
         ELSE
*********#2023/06/15 NAV ST
              IF   DEN-F01  =  761995
                   IF  DEN-F42(3:1)  =  "C"     *> センター
                       MOVE    DEN-F44(1:5)  TO  SYU-F05
                   ELSE                         *> センター以外
                       MOVE    DEN-F07       TO  SYU-F05
                   END-IF
              ELSE
...
*********MOVE DEN-F07                 TO  SYU-F09
         IF   DEN-F01  =  5116
              IF   WK-JUNTEN-04  =  "02"
                   MOVE WK-JUNTEN-01(1:3)  TO  SYU-F09(3:3)
                   MOVE "00"               TO  SYU-F09(1:2)
              ELSE
                   MOVE DEN-F07            TO  SYU-F09
              END-IF
         ELSE
...
*部門コード
*                       ↓ダイキのみ
     IF       DEN-F01 = 100427 OR 100441 OR 100428 OR 100442
*                          ↓0:直送 1:センター
        IF       DEN-F40 = 0
                 MOVE     DEN-XI0004(22:4)       TO   SYU-F11
*2021/02/26 NAV ST ＤＣＭ仕入先統合
                 IF   DEN-F46 >= 20210301
                      MOVE     "0"               TO   SYU-F11(1:1)
                      MOVE     DEN-XI0004(70:3)  TO   SYU-F11(2:3)
                 END-IF
*2021/02/26 NAV ED ＤＣＭ仕入先統合
        ELSE
                 MOVE     DEN-XI0004(22:4)       TO   SYU-F11
*2021/02/26 NAV ST ＤＣＭ仕入先統合
                 IF   DEN-F46 >= 20210301
...
*#2017/03/10 NA ST
*                       ↓カーマのみ
     IF       DEN-F01 = 13938  OR  17137
                     OR 139381 OR  171371
*                          ↓0:直送 1:センター
        IF       DEN-F40 = 0
                 MOVE     "0"                    TO   SYU-F11(1:1)
                 MOVE     DEN-XI0004(277:3)      TO   SYU-F11(2:3)
*2021/02/26 NAV ST ＤＣＭ仕入先統合
                 IF   DEN-F46 >= 20210301
                      MOVE     DEN-XI0004(70:3)  TO   SYU-F11(2:3)
                 END-IF
...
*#2019/06/20 NAV ST
     IF   DEN-F01  =  5116
          IF   WK-JUNTEN-04  =  "02"
               MOVE "00"               TO  SYU-F11(1:2)
               MOVE WK-JUNTEN-03       TO  SYU-F11(3:2)
          END-IF
     END-IF.
*#2019/06/20 NAV ED
*形状コード　
...
*****発注種別ＬＩＮＫＳ連携用区分に変更
     ...
     OR 100403 OR 100404 OR 100441 OR 100442 OR 100427 OR 100428
        EVALUATE  WK-SYUBETU
            WHEN  "00"  MOVE  "A"  TO  WK-SYUBETU-HEN
            WHEN  "01"  MOVE  "B"  TO  WK-SYUBETU-HEN
            WHEN  "02"  MOVE  "C"  TO  WK-SYUBETU-HEN
            WHEN  "03"  MOVE  "D"  TO  WK-SYUBETU-HEN
            WHEN  "05"  MOVE  "E"  TO  WK-SYUBETU-HEN
            WHEN  "51"  MOVE  "F"  TO  WK-SYUBETU-HEN
            WHEN  "52"  MOVE  "G"  TO  WK-SYUBETU-HEN
            WHEN  "71"  MOVE  "H"  TO  WK-SYUBETU-HEN
            WHEN  "81"  MOVE  "I"  TO  WK-SYUBETU-HEN
            WHEN  "82"  MOVE  "J"  TO  WK-SYUBETU-HEN
            WHEN  "83"  MOVE  "K"  TO  WK-SYUBETU-HEN
            WHEN  OTHER MOVE  "*"  TO  WK-SYUBETU-HEN
       END-EVALUATE
*******DISPLAY "WK-SYUBETU     = " WK-SYUBETU     UPON CONS
*******DISPLAY "WK-SYUBETU-HEN = " WK-SYUBETU-HEN UPON CONS
     END-IF.
*#2017/06/06 NAV ED
*                       ↓ダイキのみ
     IF       DEN-F01 = 100427 OR 100441 OR 100428 OR 100442
...
*****************MOVE     "0"                    TO   SYU-F12(1:1)
                 MOVE     WK-SYUBETU-HEN         TO   SYU-F12(1:1)
*****************#2017/06/06 NAV ED
...
*****************MOVE     "0"                    TO   SYU-F12(1:1)
                 MOVE     WK-SYUBETU-HEN         TO   SYU-F12(1:1)
*****************#2017/06/06 NAV ED
                 MOVE     DEN-XI0004(274:1)      TO   SYU-F12(2:1)
*2021/02/26 NAV ST ＤＣＭ仕入先統合
                 IF   DEN-F46 >= 20210301
                      MOVE     DEN-XI0004(69:1)  TO   SYU-F12(2:1)
                 END-IF
...
*#2019/06/20 NAV ST
     IF   DEN-F01  =  5116
          IF   WK-JUNTEN-04  =  "02"
               MOVE WK-JUNTEN-05(2:2)  TO  SYU-F12
          END-IF
     END-IF.
*#2019/06/20 NAV ED
*#2017/03/10 NAV ED
...
*初期化
     INITIALIZE KMS-REC.
     INITIALIZE DIS-REC.
     INITIALIZE WK-HATYU-SYUBETU.
*カーマの場合
     IF  PARA-IN-ONL-JUSIN-TOR  =  13938  OR 17137
                                OR 139381 OR 171371
              MOVE      DEN-REC  TO   KMS-REC
              MOVE      KMS-A35  TO   WK-SYUBETU
*2021/02/26 NAV ST ＤＣＭ仕入先統合
              IF   DEN-F46 >= 20210301
                   MOVE     DEN-XI0004(19:2)  TO   WK-SYUBETU
              END-IF
*2021/02/26 NAV ED ＤＣＭ仕入先統合
     END-IF.
*ダイキの場合
     IF  PARA-IN-ONL-JUSIN-TOR  =  100403 OR 100404 OR 100427
                                OR 100428 OR 100441 OR 100442
              MOVE      DEN-REC  TO   DIS-REC
              MOVE      DIS-A39  TO   WK-SYUBETU
*2021/02/26 NAV ST ＤＣＭ仕入先統合
              IF   DEN-F46 >= 20210301
                   MOVE     DEN-XI0004(19:2)  TO   WK-SYUBETU
              END-IF
