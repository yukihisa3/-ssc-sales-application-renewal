...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SJH2103B.COB`
...
*                            変換伝票データファイルを作成する　*
*                            片倉工業用                        *
*2009/7/17 カタクラの商品ＣＤを、インストア⇒ＪＡＮに変更      *
*2009/10/14 伝票振分方法を、ルートから伝票１行目の場所に変更   *
****************************************************************
...
*ヘッドレコード退避用ワーク
 01  WK-HEAD-REC.
     ...
     03  WK-HD10                  PIC       X(06).
     03  WK-HD11.
         05  WK-HD111             PIC       X(15).
         05  WK-HD112             PIC       X(15).
     03  WK-HD12                  PIC       X(06).
     03  WK-HD13                  PIC       X(06).
     03  WK-HD14                  PIC       X(15).
     03  WK-HD15                  PIC       X(20).
...
*    明細レコード退避ワーク
 01  WK-MEIS-REC.
     ...
     03  WK-MS03                  PIC       9(01).
     03  WK-MS04.
         05  WK-MS041             PIC       X(02).
         05  WK-MS042             PIC       X(06).
     03  WK-MS05.
         05  WK-MS051             PIC       X(12).
         05  WK-MS052             PIC       X(12).
     03  WK-MS06                  PIC       X(20).
     03  WK-MS07                  PIC       X(13).
     03  WK-MS08.
         05  WK-MS081             PIC       X(04).
         05  WK-MS082             PIC       X(02).
     03  WK-MS09                  PIC       9(06)V9.
     03  WK-MS10                  PIC       9(06)V99.
     03  WK-MS11                  PIC       9(09).
     03  WK-MS12                  PIC       9(06).
     03  WK-MS13                  PIC       9(09).
     03  FILLER                   PIC       X(15).
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJH2103B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH2103B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
     ...
     MOVE     SPACE     TO        WK-MEIS-REC.
     INITIALIZE                   WK-MEIS-REC.
*
******************
*システム日付編集*
******************
...
*             出荷場所件数マスタ出力
              ...
              MOVE      SPACE         TO    WK-HEAD-REC
              INITIALIZE                    WK-HEAD-REC
              MOVE      DEN-01(IDX)   TO    WK-HEAD-REC
              ADD       1        TO   CNT-MAISU
...
*明細行
        IF    DEN-01B(IDX)  =     "D"
              MOVE      SPACE         TO    WK-MEIS-REC
              INITIALIZE                    WK-MEIS-REC
              MOVE      DEN-01(IDX)   TO    WK-MEIS-REC
              ADD       1        TO   CNT-KENSU
...
*各量販店領域転送
*    取引先コード
     MOVE     WK-HD10   TO        HEN-A01.
*    伝票_
     MOVE     WK-HD09   TO        HEN-A02.
*    行_
     MOVE     WK-MS03   TO        HEN-A03.
*    部門（Ｈ）
     MOVE     WK-HD07   TO        HEN-A04.
*    伝票区分
     MOVE     WK-HD08   TO        HEN-A05.
*    発注者名
     MOVE     WK-HD03   TO        HEN-A06.
*    取引先名
     MOVE     WK-HD11   TO        HEN-A07.
*    発注日
     MOVE     WK-HD12   TO        HEN-A08.
*    納品日
     MOVE     WK-HD13   TO        HEN-A09.
*    納入場所
     MOVE     WK-HD14   TO        HEN-A10.
*2009/08/03納入場所が下記の場合、ルートにＡをセット
*    "* ﾒ-ｶ- ﾁﾖｸｿｳ ﾋﾝ"
     IF       WK-HD14 = "* ﾒ-ｶ- ﾁﾖｸｿｳ ﾋﾝ"
              MOVE "A"  TO        HEN-A12
     END-IF.
*    催事名
     MOVE     WK-HD15   TO        HEN-A11.
*    ルート
     MOVE     WK-HD06   TO        HEN-A12.
*    部門（Ｂ）
     MOVE     WK-MS041  TO        HEN-A13.
*    商品コード
     MOVE     WK-MS042  TO        HEN-A14.
*    商品名
     MOVE     WK-MS05   TO        HEN-A15.
*    取引先品番
     MOVE     WK-MS06   TO        HEN-A16.
*    ＪＡＮコード
     MOVE     WK-MS07   TO        HEN-A17.
*    入数
     MOVE     WK-MS081  TO        HEN-A18.
*    ケース
     MOVE     WK-MS082  TO        HEN-A19.
*    数量
     MOVE     WK-MS09   TO        HEN-A20.
*    原単価
     MOVE     WK-MS10   TO        HEN-A21.
*    原価金額
     MOVE     WK-MS11   TO        HEN-A22.
*    売単価
     MOVE     WK-MS12   TO        HEN-A23.
*    売価金額
     MOVE     WK-MS13   TO        HEN-A24.
*    店コード
     MOVE     WK-HD05   TO        HEN-A25.
*    店舗名
     MOVE     WK-HD04   TO        HEN-A27.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
*****MOVE        HEN-A14   TO        TBL-F02(3:6).
     MOVE        HEN-A17   TO        TBL-F02.
     READ    SHOTBL1
       INVALID
         MOVE    SPACE     TO        TBL-REC
         INITIALIZE                  TBL-REC
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名
         MOVE    HEN-A15   TO        HEN-F142
       NOT INVALID
         MOVE    HEN-A15   TO        HEN-F142
     END-READ.
*数量
     MOVE        HEN-A20   TO        HEN-F15.
*単
     MOVE       "1"        TO        HEN-F16.
*原価単価
     MOVE        HEN-A21   TO        HEN-F172.
*売価単価
     MOVE        HEN-A23   TO        HEN-F173.
*原価金額
     MOVE        HEN-A22   TO        HEN-F181.
*売価金額
     MOVE        HEN-A24   TO        HEN-F182.
*店舗名（備考）
*****MOVE        HEN-A27   TO        HEN-F22.
*自社得意先コード
     IF  FLG-TOK   =    ZERO
*    得意先マスタ検索
...
*****MOVE        HEN-A14   TO        HEN-F25(3:6).
*****MOVE        SHI32     TO        HEN-F25.
*2009/07/17 ST ｲﾝｽﾄｱからJANCDへ変更する
     MOVE        HEN-A17   TO        HEN-F25.
*2009/07/17 ED
*伝票発行区分
     MOVE        9         TO        HEN-F272.
*オンライン区分
...
*訂正前数量
     MOVE        HEN-A20   TO        HEN-F50.
*修正原価単価
     MOVE        HEN-A21   TO        HEN-F512.
*修正売価単価
     MOVE        HEN-A23   TO        HEN-F513.
*修正原価金額
     MOVE        HEN-A22   TO        HEN-F521.
*修正売価金額
     MOVE        HEN-A24   TO        HEN-F522.
*
 TENSO-EXIT.
     EXIT.
****************************************************************
