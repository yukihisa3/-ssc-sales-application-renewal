...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBM1003B.COB`
...
*
 ...
 FILE-CONTROL.
*流通ＢＭＳ発注メッセージ
     SELECT   BMSHEPF   ASSIGN    TO        DA-01-VI-BMSHEPL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      SEQUENTIAL
                        RECORD    KEY       HEP-F011  HEP-F012
                                            HEP-F013
                                            WITH DUPLICATES
...
*ワークに発注メッセージを展開
     COPY   BMSHEPF  OF XFDLIB  JOINING   HWK  AS   PREFIX.
*ワーク項目
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SBM1003B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SBM1003B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
******************************************************************
 ...
 DECLARATIVES.
 HAC-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE BMSHEPF.
...
*流通ＢＭＳ発注メッセージスタート
     ...
     INITIALIZE                   HEP-REC.
     MOVE     PARA-HIDUKE    TO   HEP-F011.
     MOVE     PARA-JIKAN     TO   HEP-F012.
     MOVE     PARA-TORICD    TO   HEP-F013.
     START    BMSHEPF  KEY  >=    HEP-F011  HEP-F012  HEP-F013
         INVALID   KEY
...
*********会社ＣＤで取引先ＣＤを判定する。
         ...
         MOVE     HEP-F206        TO   TER-F04
*********MOVE     HEP-F309(1:4)   TO   WK-TENPO
*********MOVE     WK-TENPO-H      TO   TER-F05
*##2015/05/28 NAV ST 店舗ＣＤ５桁対応
******** MOVE     HEP-F309(1:4)   TO   WK-TENPO
******** MOVE     WK-TENPO-H      TO   TER-F05
         IF  HEP-F309(5:1) = SPACE
             MOVE HEP-F309(1:4)   TO   WK-TENPO
             MOVE WK-TENPO-H      TO   TER-F05
         ELSE
             MOVE HEP-F309(1:5)   TO   WK-TENPO5
             MOVE WK-TENPO5-H     TO   TER-F05
         END-IF
*##2015/05/28 NAV ED 店舗ＣＤ５桁対応
         EVALUATE HEP-F206(1:4)
             WHEN "5200"
             MOVE 87372           TO   HWK-F013 HEP-F013 TER-F06
             WHEN "0105"
             MOVE 87373           TO   HWK-F013 HEP-F013 TER-F06
             WHEN "5300"
             MOVE 87374           TO   HWK-F013 HEP-F013 TER-F06
             WHEN "3618"
             MOVE 33189           TO   HWK-F013 HEP-F013 TER-F06
             WHEN "2400"
             MOVE 87373           TO   HWK-F013 HEP-F013 TER-F06
*##2015/11/17 NAV ST イオンリテールストア追加
             WHEN "7900"
             MOVE 87376           TO   HWK-F013 HEP-F013 TER-F06
*##2015/11/17 NAV ED
             WHEN OTHER
             MOVE 99999           TO   HWK-F013 HEP-F013 TER-F06
         END-EVALUATE
         MOVE   "4"               TO   TER-F07
         WRITE  TER-REC
         ADD    1                 TO   ERRT-CNT
     ELSE
              MOVE FUR-F03        TO   HWK-F013  HEP-F013
     END-IF.
*イオン取引先受信ワーク索引
...
*ファイルクローズ
     CLOSE     BMSHEPF  IONFURF  IONTWKF  IONTERWK.
*
     STOP      RUN.
*
 END-EXIT.
...
*　　　　　　流通ＢＭＳ発注メッセージ読込
****************************************************************
 BMSHEPF-READ-SEC          SECTION.
*
     READ     BMSHEPF
              AT  END       MOVE  "END"   TO  END-FLG
                            GO TO BMSHEPF-READ-EXIT
...
*件数表示
     ...
     END-IF.
*指定バッチ_チェック
     IF     ( PARA-HIDUKE  =  HEP-F011 ) AND
            ( PARA-JIKAN   =  HEP-F012 ) AND
            ( PARA-TORICD  =  HEP-F013 )
              CONTINUE
     ELSE
              MOVE      "END"     TO   END-FLG
              GO                  TO   BMSHEPF-READ-EXIT
