...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SIT0070I.COB`
...
*             ＩＴ統制対応                                     *
*    更新日／更新者　　　：　09/12/25  /NAV OONO               *
*             ルート順追加                                     *
*    更新日／更新者　　　：　14/07/31  /NAV INOUE              *
*             納品形態追加                                     *
*    更新日／更新者　　　：　16/04/22  /NAV INOUE              *
*             新センターコード追加                             *
*    更新日／更新者　　　：　19/03/19  /NAV INOUE              *
...
*↓2016/04/22
*新センターコードマスタ
**** SELECT      CENTERM1  ASSIGN    TO        DA-01-VI-CENTERM1
*                          ORGANIZATION        INDEXED
*                          ACCESS    MODE      RANDOM
*                          RECORD    KEY       CEN-F01
*                                              CEN-F02
...
*FD  CENTERM1
*    LABEL       RECORD    IS        STANDARD.
*    COPY        CENTERM   OF        XFDLIB
*    JOINING     CEN       AS        PREFIX.
*↑2016/04/22
*マスタ更新履歴ファイル
 FD  MSTLOGF
     BLOCK       CONTAINS   8        RECORDS
...
*↓2016/04/22
     03  CENTERM-INV-FLG   PIC X(03) VALUE     SPACE.
*↑2016/04/22
 01  ST-AREA.
     03  IN-DATA           PIC X(01).
     03  TEN-ST            PIC X(02).
...
*↓2016/04/22
*CEN-ERR                   SECTION.
*    USE         AFTER     EXCEPTION PROCEDURE CENTERM1.
*    DISPLAY     CEN-ST    UPON      CONS.
...
***09/12/25 変更開始
*    デリバリー区分
     IF  (DELIVE   NOT   NUMERIC)
     OR  (RUTO     =     ZERO   )
          IF SYUKKA NOT = SPACE
             MOVE     "R"        TO   EDIT-OPTION  OF  DELIVE
             MOVE     "C"        TO   EDIT-CURSOR  OF  DELIVE
             MOVE      MSG14     TO   MSG1
             MOVE     "MSG1"     TO   DSP-GRP
             PERFORM   DSP-WR-SEC
             MOVE     "GRP001"   TO   DSP-GRP
...
*    新センターコード
**** IF  NEWCCD   =   SPACE
*        MOVE      SPACE     TO   CENMEI
*        MOVE     "M"        TO   EDIT-OPTION  OF  NEWCCD
*        MOVE      SPACE     TO   EDIT-CURSOR  OF  NEWCCD
*        MOVE     "GRP001"   TO   DSP-GRP
*    ELSE
*        MOVE      ZERO      TO      ERR-FLG
*        MOVE      TORICD    TO      CEN-F01
*        MOVE      NEWCCD    TO      CEN-F02
*        PERFORM   CENTERM-READ-SEC
*        IF CENTERM-INV-FLG = "INV"
*           MOVE     "R"        TO   EDIT-OPTION  OF  NEWCCD
*           MOVE     "C"        TO   EDIT-CURSOR  OF  NEWCCD
*           MOVE      MSG22     TO   MSG1
*           MOVE     "MSG1"     TO   DSP-GRP
***         MOVE ALL NC"＊"     TO   CENMEI
*           PERFORM   DSP-WR-SEC
*           MOVE     "GRP001"   TO   DSP-GRP
*           PERFORM   DSP-WR-SEC
*           GO   TO   MAIN-030
...
*↓2014/07/31
*    納品形態(新センター区分)追加
     IF ( NOUKEI   NOT   NUMERIC )   OR
        (( NOUKEI   NOT = 00 ) AND ( NOUKEI   NOT = 01 ))
             MOVE     "R"        TO   EDIT-OPTION  OF  NOUKEI
             MOVE     "C"        TO   EDIT-CURSOR  OF  NOUKEI
             MOVE      MSG21     TO   MSG1
             MOVE     "MSG1"     TO   DSP-GRP
             PERFORM   DSP-WR-SEC
             MOVE     "GRP001"   TO   DSP-GRP
             ...
             GO   TO   MAIN-030
       ELSE
         MOVE     "M"        TO   EDIT-OPTION  OF  NOUKEI
         MOVE      SPACE     TO   EDIT-CURSOR  OF  NOUKEI
         MOVE     "GRP001"   TO   DSP-GRP
         PERFORM   DSP-WR-SEC
     END-IF.
*↑2014/07/31
...
*
     ...
     INITIALIZE     TORICD  TORINM  TENPCD  TNAME   TKANA   TRYAKU
                    ...
                    R002    SYUBT   SYUKKA  RUTO    RUTONO  DELIVE
*↓2014/07/31
                    NOUKEI
*↑2014/07/31
*↓2016/04/22
...
***09/12/25 変更開始  デリバリー区分に集金日代用
     MOVE     TEN-F14        TO        DELIVE.
***09/12/25 変更終了　大野
*
*↓2014/07/31
     MOVE     TEN-F12(1)     TO        NOUKEI.
*↑2014/07/31
*
*↓2016/04/22
*****MOVE     TEN-F90        TO        NEWCCD.
*    IF       TEN-F90  NOT = SPACE
*             MOVE     TEN-F52        TO        CEN-F01
*             MOVE     TEN-F90        TO        CEN-F02
*             PERFORM  CENTERM-READ-SEC
*****END-IF.
*↑2016/04/22
*
...
***09/12/25 変更開始  デリバリー区分に集金日代用
     MOVE     DELIVE         TO        TEN-F14.
***09/12/25 変更終了　大野
*
*↓2014/07/31
     MOVE     NOUKEI         TO        TEN-F12(1).
*↑2014/07/31
*
*↓2016/04/22
*****MOVE     NEWCCD         TO        TEN-F90.
*↑2016/04/22
*
...
*  新センターコードマスタ参照
****************************************************************
*CENTERM-READ-SEC       SECTION.
*
*    READ     CENTERM1       INVALID
*             MOVE "INV"     TO        CENTERM-INV-FLG
*             MOVE ALL NC"＊" TO       CENMEI
*    NOT  INVALID
*             MOVE SPACE     TO        CENTERM-INV-FLG
*             MOVE CEN-F03   TO        CENMEI
*    END-READ.
*
*    MOVE      "SCREEN"      TO        DSP-GRP.
