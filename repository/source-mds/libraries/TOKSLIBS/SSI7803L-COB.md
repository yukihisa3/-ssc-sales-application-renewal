...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSI7803L.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 支払明細ファイル >>--*
     SELECT   ROYSMEPF  ASSIGN         DA-01-VI-ROYSMEL2
...
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
 01  PG-ID              PIC  X(08)     VALUE   "SSI7803L".
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
...
*----<< BREAK KEY >>--*
 01  BREAK-KEY.
         05  SSI-KEY  PIC  9(09)    VALUE  ZERO.
         05  SEI-KEY  PIC  9(09)    VALUE  ZERO.
*
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HEAD01.
     03  FILLER         CHARACTER TYPE BAIKAKU-1-5.
         ...
         05  FILLER     PIC  X(08)     VALUE  "SSI7803L".
...
*
 01  HEAD04.
     03  FILLER         CHARACTER TYPE MODE-2.
         05  FILLER     PIC  N(120)     VALUE  ALL NC"_".
*
 01  MEIS01-A.
     03  FILLER         CHARACTER TYPE PITCH-1-5.
         05  ME-03-A    PIC  N(18).
...
*ＨＥＡＤ印字
     ...
     END-IF.
*
     MOVE     SPACE          TO   MEIS01-A.
*伝票区分名称セット
     EVALUATE SSI-F05
         WHEN  01
              MOVE  NC"定番"               TO   ME-03-A
         WHEN  02
              MOVE  NC"特売"               TO   ME-03-A
         WHEN  03
              MOVE  NC"緊急"               TO   ME-03-A
         WHEN  04
              MOVE  NC"客注品"             TO   ME-03-A
         WHEN  06
              MOVE  NC"修理品"             TO   ME-03-A
         WHEN  08
              MOVE  NC"市場買"             TO   ME-03-A
         WHEN  10
              MOVE  NC"売上仕入"           TO   ME-03-A
         WHEN  13
              MOVE  NC"返品"               TO   ME-03-A
         WHEN  15
              MOVE  NC"原価修正"           TO   ME-03-A
         WHEN  80
              MOVE  NC"配送Ｃフィー"       TO   ME-03-A
         WHEN  81
              MOVE  NC"店直フィー"         TO   ME-03-A
         WHEN  82
              MOVE  NC"店直配送Ｃフィー"   TO   ME-03-A
         WHEN  83
              MOVE  NC"取引高歩引"         TO   ME-03-A
         WHEN  84
              MOVE  NC"情報フィー"         TO   ME-03-A
         WHEN  85
              MOVE  NC"物流改善フィー"     TO   ME-03-A
         WHEN  86
              MOVE  NC"支払決済歩引"       TO   ME-03-A
         WHEN  87
              MOVE  NC"新店オープンフィー" TO   ME-03-A
         WHEN  88
              MOVE  NC"会員セールフィー"   TO   ME-03-A
         WHEN  89
              MOVE  NC"欠品ペナルティ"     TO   ME-03-A
         WHEN  90
              MOVE  NC"リベート請求"       TO   ME-03-A
         WHEN  91
              MOVE
              NC"前月以前の返品・相殺過剰未収分の相殺"
                                           TO   ME-03-A
         WHEN OTHER
              MOVE  ALL NC"＊"             TO   ME-03-A
     END-EVALUATE.
*店舗コード
     MOVE     SSI-F02                  TO   ME-05-A TEN-F011.
*店舗名称
     MOVE     SSI-F03                  TO   ME-06-A.
*伝票番号
     MOVE     SSI-KEY                  TO   ME-08-A.
*支払金額
     MOVE     SSI-F12                  TO   ME-09-A.
     MOVE     SSI-F12                  TO   WK-KINGAKU.
*レコード区分内容，量販伝票_内容，データ内容
     EVALUATE CHK-FLG
         WHEN "1"
              MOVE NC"請求データなし"       TO   ME-10-A
              ADD     1                     TO   CNT-SEI
         WHEN "2"
              IF      SEI-F06  =  WK-KINGAKU
                      MOVE  NC"ＯＫデータ"  TO   ME-10-A
                      ADD   1               TO   CNT-OK
                      MOVE  1               TO   OUT-FLG
              ELSE
                      MOVE  NC"ＮＧデータ"  TO   ME-10-A
                      MOVE  NC"金額不一致"  TO   ME-12-A
                      ADD   1               TO   CNT-UNMATCH
              END-IF
     END-EVALUATE.
*明細行印字
     IF       OUT-FLG  =  ZERO
              WRITE  PRT-REC  FROM  MEIS01-A  AFTER  1
              ADD    1       TO   LINE-CNT
     ELSE
...
*ＨＥＡＤ印字
     ...
     END-IF.
*
     MOVE     SPACE          TO   MEIS01-B.
*伝票区分名称セット
     EVALUATE SEI-F07
         WHEN 40
              MOVE  NC"売上"           TO   ME-03-B
         WHEN 41
              MOVE  NC"返品"           TO   ME-03-B
         WHEN 42
              MOVE  NC"値引"           TO   ME-03-B
         WHEN OTHER
              MOVE  ALL NC"＊"         TO   ME-03-B
     END-EVALUATE.
*店舗コード
     MOVE     SEI-F03                  TO   ME-05-B TEN-F011.
     MOVE     SEI-F01                  TO   TEN-F52.
*店舗名称
     PERFORM  900-TEN-READ.
     IF       TEN-F02  =  SPACE
              MOVE      ALL NC"＊"     TO   ME-06-B
     ELSE
              MOVE      TEN-F02        TO   ME-06-B
     END-IF.
*伝票番号
     MOVE     SEI-KEY                  TO   ME-08-B.
*請求金額
     MOVE     SEI-F06                  TO   ME-09-B.
*レコード区分内容，量販伝票_内容，データ内容
     MOVE     NC"支払データなし"       TO   ME-10-B.
     ADD      1                        TO   CNT-SSI.
*明細行印字
     WRITE    PRT-REC   FROM      MEIS01-B    AFTER     1.
     ADD      1         TO        LINE-CNT.
*
 220-HENSYU-RTN-EXIT.
     EXIT.
