...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/TSY0203I.COB`
...
*    作成者　　　　：　NAV Y.YOSHIDA.                        *
*                                                            *
*    更新日　　　　： 93/03/12  93/03/27  93/04/05           *
*      更新者　　　：　NAV T.A    T.A       H.K              *
*      更新内容　　：　バグ対応　仕様変更  SSKT010から作成   *
*    更新日　　　　： 93/05/14                               *
*      更新者　　　：  T.A                                   *
...
* 2020/12/16 高橋　　　Ｄ３６５連携対応
*　　　　　　（小売連携区分取得方法を変更　オーダー区分セット）
* 2020/12/24 高橋　　　Ｄ３６５連携対応　修正時対応　　
* 2021/01/12 高橋　　　Ｄ３６５連携対応　値引伝票相殺禁止
* 2021/05/14 井上　　　未計上区分追加　　　　　　　　　　　
* 2021/09/22 高橋　　　チェック機能追加　　　　　　　　　　
****************************************************************
...
*----<< 画面ファイル >>-*
 FD  DSPFILE.
     COPY     MSY02011  OF   XMDLIB.
*--------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*--------------------------------------------------------------*
 01  PGM-ID                  PIC  X(08)     VALUE  "TSY0203I".
*----<< ﾃﾞｨｽﾌﾟﾚｲ ｺﾝﾄﾛｰﾙ ｴﾘｱ >>-*
 01  DSP-CNTL.
     03  DSP-FMT             PIC  X(08).
     03  DSP-GRP             PIC  X(08).
...
*----<< ﾃﾞｰﾀ ﾃｰﾌﾞﾙ >>-*
 01  TABLE-AREA.
     03  TBL                 OCCURS    12.
         ...
         05  TBL-MAESUU      PIC  9(09)V99.
         05  TBL-MAEGTAN     PIC  9(09)V99.
         05  TBL-MAEBTAN     PIC  9(09)V99.
         05  TBL-MAEGKIN     PIC  9(11).
         05  TBL-MAEBKIN     PIC  9(11).
...
*画面退避用
     COPY   MSY02011  OF XMDLIB  JOINING   SAV  AS   PREFIX.
*
 01  SAVE-AREA.
...
***  伝票キーエリア
     ...
     03  WK-F02K             PIC  9(09)  VALUE  ZERO.
...
*2014/03/19↑
*2021/01/12↓
     03  MSG44               PIC  N(20)  VALUE
     NC"値引伝票は相殺できません！！".
*2021/01/12↑
*2021/05/14↓
     03  MSG45               PIC  N(20)  VALUE
     NC"未計上区分は空白または１を入力".
*2021/05/14↑
*
...
*2021/05/14↓
*****03  MSG-TBL             PIC  N(20)  OCCURS       44.
     03  MSG-TBL             PIC  N(20)  OCCURS       45.
*2021/05/14↑
*2021/01/12↑
*2014/03/19↑
*2013/12/26↑
...
**************　伝票区分＝５０，５１，５２，５５はエラー
              IF   JYO-F02  =  "50      "
                   ...
                   GO  TO   CHK-1-EXIT
              END-IF
*#2021/01/12 NAV ST 値引伝票は相殺不可とする。
*2021/05/25   IF   JYO-F02  =  "42      "  *>伝区＝４２（値引）
*   以下停止  AND  R00002   >  ZERO        *>相殺の時
*                  MOVE  44 TO  ERR-FLG
*                  MOVE "C" TO  EDIT-CURSOR OF R00008
*                  MOVE "R" TO  EDIT-OPTION OF R00002  *>相殺
*                  MOVE "R" TO  EDIT-OPTION OF R00005  *>取引先
*                  MOVE "R" TO  EDIT-OPTION OF R00007  *>伝票NO
*                  MOVE "R" TO  EDIT-OPTION OF R00008  *>伝区
*                  GO  TO   CHK-1-EXIT
*********     END-IF
*#2021/01/12 NAV ED 値引伝票は相殺不可とする。
              MOVE     JYO-F03        TO   R00009
     END-IF.
*# D365 関係　2020/04/21 ADD ST
*****登録で相殺区分＝０，１，３，５，７の場合は採番
     IF  ( R00001  =  1 )
*## 2020/06/01 NAV ST 相殺区分＝０の時のみＤ３６５伝ＮＯ採番
*****AND ( R00002  =  0  OR  1  OR  3  OR  5  OR  7 )
     AND ( R00002  =  0  )
*## 2020/06/01 NAV ED 相殺区分＝０の時のみＤ３６５伝ＮＯ採番
*# Ｄ３６５伝票番号採番
...
*画面＆ＴＢＬクリア
     ...
     MOVE      SPACE     TO   MSY02011   TABLE-AREA.
     INITIALIZE               TBL-XX.
     MOVE      ZERO      TO   WK-TAIHI.
...
*2013/12/26↑
              ...
              PERFORM JYO-RD-RTN
              IF      INV-FLG  =  1
*                     DISPLAY "HJYOKEN INV KEY=50"  UPON STAT
                      MOVE     41        TO   ERR-FLG
...
*　小売連携区分をセットする。
************* INITIALIZE     MEI-REC
*  DISPLAY  "DEN-F1411= "  DEN-F1411  UPON CONS
*  DISPLAY  "DEN-F1412= "  DEN-F1412  UPON CONS
...
*TT2020/12/16 NAV ST   IF  SUB-F19  =  "1"
...
*#2020/12/24 NAV ED
              END-IF
*            ↑2020/05/07
*#2021/09/22 NAV ST 伝区＝４２の時はＯＲＤセット区分はセット×
              IF   DEN-F051  =  42
                   MOVE         SPACE     TO    DEN-F32
*******************MOVE         SPACE     TO    DEN-F33
              END-IF
*#2021/09/22 NAV ED 伝区＝４２の時はＯＲＤセット区分はセット×
              WRITE     DEN-REC
            END-IF
       END-IF
     END-PERFORM.
...
***2011.10.12 EN
     ...
     PERFORM  UNTIL  END-FLG  =  1
              IF  R00001      =  3
                  ...
                  END-IF
              END-IF
**************#2018/03/30 NAV ST 受注残ファイル作成（４０のみ）
              IF   R00008 =  40
                   MOVE    "3"            TO    PARA-KOSINKBN
                   MOVE    TBL-WTANA(I)   TO    WK-TANABAN
...
*****DISPLAY "SAV-HTENCD = " SAV-HTENCD UPON CONS.
*****DISPLAY "R00010     = " R00010     UPON CONS.
*****DISPLAY "R00030     = " R00030     UPON CONS.
*****IF       SAV-HTENCD   =    ZERO
*****    MOVE R00010         TO   DEN-F07
*****ELSE
*****    MOVE SAV-HTENCD     TO   DEN-F07
...
*# D365 2020/04/20 NAV ED
     EVALUATE  R00001
        WHEN    "1"
           ...
           MOVE   9         TO  DEN-F27B
****## ADD  2008.08.07
***********MOVE   TAN-F02   TO  DEN-F59  DEN-F60
...
*##2014/04/21 NAV ST
     MOVE     DEN-F35        TO   TBL-F35(DEN-F03).
*##2014/04/21 NAV ED
*##2020/07/23 NAV ST
...
*--------------------------------------------------------------*
 DSP-RD-RTN           SECTION.
*
     MOVE    "TSY0203I"      TO        PGID.
     MOVE    "MSY02011"      TO        FORM.
...
****************************
     ...
     END-IF.
*#2021/09/122NAV ST オンラインＭＳＧセット
     IF  WK-ONL-CHK-FLG  =  0
         MOVE  SPACE              TO   WNGMSG
     ELSE
         MOVE  MSGWNG-03          TO   WNGMSG
     END-IF.
****************************
     MOVE     "ALLF"         TO   DSP-GRP.
...
*--------------------------------------------------------------*
 ...
 ZAI-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  ALL   担当者マスタ　　　ＲＥＡＤ　2008.08.07       *
*--------------------------------------------------------------*
 TAN-RD-RTN           SECTION.
     MOVE     0         TO   INV-FLG.
