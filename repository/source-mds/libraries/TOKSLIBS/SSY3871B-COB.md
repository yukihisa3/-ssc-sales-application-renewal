...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3871B.COB`
...
*読込・書込カウント領域
 01  CNT-AREA.
     ...
     03  OT-CNT4            PIC  9(07)  VALUE ZERO.
...
*日付変換サブルーチン用ワーク
 ...
 01  IX                     PIC  9(08).
 01  WK-ST3-F09             PIC  9(07)  VALUE  ZERO.
*------------------------------------------------------------*
 LINKAGE              SECTION.
*------------------------------------------------------------*
* 入力パラメータ
...
*      ＴＲＡＮＴＲＡＮ連携データ（箱数ファイル）
       ...
       WHEN  KY-HK3 > KY-ST3
                       *> "＞"：箱数無し、数量訂正Ｆ有り
        DISPLAY "SSY3871B NFHAKOF RECORD NOT FOUND KEY="
          KY-ST3 "*"  UPON CONS
          MOVE  "4000"      TO  PROGRAM-STATUS
          STOP RUN
**         PERFORM  UNTIL FG-NFSUTEF-END = "END"
...
*********MOVE 1             TO  HKD-TYOKUSO-KBN
         MOVE ZERO          TO  HKD-TYOKUSO-KBN
     ELSE
*********MOVE SKB-F03       TO  STD-TYOKS-MOT-CD *> 直送ＣＤ
         MOVE SKB-F04       TO  HKD-TYOKUSO-KBN
*********EVALUATE SKB-F03
...
*********END-EVALUATE
     END-IF.
*直送元コード”Ｅ１”甘木の場合→０にする。
*****IF  WHK3-F05 = "E1"  OR  "CC"
*        MOVE ZERO              TO  HKD-TYOKUSO-KBN
*****END-IF.
*カンマセット
...
*    MOVE  ST3-F13          TO  KH1-F09. *> 納品日
     ...
     IF  FG-NFJOHOF-INV = 1
         DISPLAY "SSY3871B NFJOHOF RECORD NOT FOUND KEY="
                 ST3-F01
             "," ST3-F05
             "," ST3-F06
             "," ST3-F09
...
*****MOVE  ST3-F09          TO  STD-DENNO.
     MOVE  ST3-F09          TO  WK-ST3-F09.
     MOVE  WK-ST3-F09       TO  STD-DENNO.
*納品数（１０倍）
...
*作場マスタ索引（直送先CD取得）
     ...
     ELSE
*********MOVE SKB-F03       TO  STD-TYOKS-MOT-CD *> 直送ＣＤ
         MOVE SKB-F04       TO  STD-TYOKS-MOT-CD *> 直送ＣＤ
     END-IF.
*甘木の場合は、直送区分を０にする。
*****IF  ST3-F05  = "E1"  OR  "CC"
*        MOVE ZERO          TO  STD-TYOKS-MOT-CD
...
*2015/06/30↓
     ...
     REWRITE KH1-REC.
*2015/06/30↑
*****DISPLAY "KY-HK3-F05 = " KY-HK3-F05 UPON CONS.
*****DISPLAY "KY-ST3-F05 = " KY-ST3-F05 UPON CONS.
*
 EDWT-NFSUTDT-EXIT.
     EXIT.
****************************************************************
...
*2011/12/19　FAX HATYU-DATE <= SYSTEM-DATE SET
     MOVE  SYS-DATE         TO  FXD-HACH-YMD.
*納品予定日
*****MOVE  WHK3-F08         TO  FXD-NOHN-YOTEI-YMD.
     MOVE  ST3-F08          TO  FXD-NOHN-YOTEI-YMD.
*入荷予定日
...
*********MOVE 1             TO  FXD-TYOKSO-KBN *> 直送ＣＤ
         MOVE 1             TO  FXD-TYOKSO-KBN *> 直送ＣＤ
         MOVE SKB-F04       TO  FXD-TYOKUSO-KBN
*********EVALUATE SKB-F03
...
*
     ...
     DISPLAY  "OT-CNT4    = "  OT-CNT4  UPON CONS.
* ファイルのＯＰＥＮ
     CLOSE  NFHAKOF.
     CLOSE  NFSUTEF.
     CLOSE  NFJOHOF.
