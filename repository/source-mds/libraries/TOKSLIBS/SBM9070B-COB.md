...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBM9070B.COB`
...
*
*請求メッセージファイル
     SELECT  BMSSEKF  ASSIGN TO    DA-01-VI-BMSSEKL1
             ORGANIZAITION         INDEXED
             ACCESS      MODE      SEQUENTIAL
             RECORD      KEY       SEK-F011  SEK-F012
                                   SEK-F013
                         FILE      STATUS    SEK-ST.
****************************************************************
 DATA                DIVISION.
...
*
 01  SEK-BR-KEY2.
     03  SEK-BR-KEY21         PIC  X(08).
     03  SEK-BR-KEY22         PIC  X(04).
     03  SEK-BR-KEY23         PIC  X(08).
 01  WK-BR-KEY2.
...
****************************************************************
 INIT-SEC              SECTION.
*
     DISPLAY  "***  START SBM9070B  ***"   UPON  CONS.
...
* 定型
 INSTART-100.
     MOVE       ZERO              TO   SEK-F011.
     GO  TO    INSTART-300.
* 非定型
 INSTART-200.
     IF  PARA-SHURUI  =  "1"
*
         MOVE     PARA-JYUSINHI       TO   SEK-F011
         MOVE     PARA-JYUSINTIME     TO   SEK-F012
         MOVE     PARA-TOKCD          TO   SEK-F013
         MOVE     PARA-JYUSINHI       TO   WK-BR-KEY21
         MOVE     PARA-JYUSINTIME     TO   WK-BR-KEY22
         MOVE     PARA-TOKCD          TO   WK-BR-KEY23
     ELSE
         MOVE     PARA-SOUJYUSIN      TO   SEK-F011
     END-IF.
*
*
 INSTART-300.
     START  BMSSEKF  KEY  IS  >=
                                   SEK-F011  SEK-F012
                                   SEK-F013
*
            INVALID
...
**
     GO  TO   MAIN-900.
 MAIN-210.
     IF  SEK-BR-KEY2  =  WK-BR-KEY2
         PERFORM SAKUJYO-SEC
         GO  TO  MAIN-900
     END-IF.
     IF  SEK-BR-KEY2  NOT =  WK-BR-KEY2
         MOVE  "1"    TO   END-FLG
     GO  TO   MAIN-EXIT
     END-IF.
 MAIN-220.
     IF  SEK-F011     =  PARA-SOUJYUSIN
         ...
         GO  TO    MAIN-900
     END-IF.
     IF  SEK-F011   NOT =  PARA-SOUJYUSIN
         MOVE  "1"     TO   END-FLG
         GO  TO    MAIN-EXIT
     END-IF.
*
...
*ファイルのCLOSE処理
     CLOSE     BMSSEKF.
     DISPLAY  "***  END   SBM9070B  ***"   UPON  CONS.
*
 END-EXIT.
     EXIT.
*
...
*
     IF   PARA-TEIKEI  =  "2"
         MOVE     SEK-F011    TO   SEK-BR-KEY21
         MOVE     SEK-F012    TO   SEK-BR-KEY22
         MOVE     SEK-F013    TO   SEK-BR-KEY23
     END-IF.
*
     ADD      1           TO   READ-CNT.
*
