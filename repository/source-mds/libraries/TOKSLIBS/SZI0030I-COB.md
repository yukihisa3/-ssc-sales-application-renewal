...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SZI0030I.COB`
...
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    業務名　　　　　　　：　在庫ＥＸＣＥＬ連携                *
*    モジュール名　　　　：　在庫ＥＸＣＥＬデータ件数確認      *
*    作成日／作成者　　　：　2016/06/13 INOUE                  *
*    処理概要　　　　　　：　パラメタより受け取ったデータ件数  *
*                            を表示する。　　　　　　　　　　  *
*    更新日／更新者　　　：　2016/07/08 INOUE                  *
*                            作業区分(G1/G5)対応
****************************************************************
****************************************************************
...
*
 ...
 01  PARA-IN-CNT2           PIC   9(07).
 01  PARA-IN-CNT3           PIC   9(07).
 01  PARA-IN-CNT4           PIC   9(07).
 01  PARA-IN-CNT5           PIC   9(07).
 01  PARA-IN-CNT6           PIC   9(07).
 01  PARA-IN-CNT7           PIC   9(07).
 01  PARA-IN-CNT8           PIC   9(07).
****************************************************************
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SZI0030I DSPFILE ERROR " DSP-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
     CLOSE    TANMS1    DSPFILE.
     STOP     RUN.
*----<< 担当者マスタ >>--*
 TANMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SZI0030I TANMS1 ERROR " TAN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*T↓
*    DISPLAY NC"受取＿実行区分＝" PARA-IN-JKUBUN UPON CONS.
*    DISPLAY NC"受取＿部門　　＝" PARA-IN-BUMON  UPON CONS.
*    DISPLAY NC"受取＿担当者　＝" PARA-IN-TANCD  UPON CONS.
*    DISPLAY NC"受取＿件数２　＝" PARA-IN-CNT2   UPON CONS.
*    DISPLAY NC"受取＿件数３　＝" PARA-IN-CNT3   UPON CONS.
*    DISPLAY NC"受取＿件数４　＝" PARA-IN-CNT4   UPON CONS.
*    DISPLAY NC"受取＿件数５　＝" PARA-IN-CNT5   UPON CONS.
*    DISPLAY NC"受取＿件数６　＝" PARA-IN-CNT6   UPON CONS.
*    DISPLAY NC"受取＿件数７　＝" PARA-IN-CNT7   UPON CONS.
*    DISPLAY NC"受取＿件数８　＝" PARA-IN-CNT8   UPON CONS.
*T↑
     ACCEPT   SYS-DATE       FROM DATE.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     ...
     MOVE     SPACE               TO   DSP-FZI00301.
*
*    PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SZI0030I"      TO   DSP-PGID.
     MOVE    "FZI00301"      TO   DSP-FORMID.
*モード表示
     IF       PARA-IN-JKUBUN        =   "1" OR "3" OR "5"
              MOVE NC"チェックのみ　"        TO   DSP-MODENM
     END-IF.
     IF       PARA-IN-JKUBUN        =   "2" OR "4" OR "6"
              MOVE NC"チェック・更新"        TO   DSP-MODENM
     END-IF.
*担当者名称取得
     MOVE      PARA-IN-BUMON       TO   TAN-F01 DSP-TANCD(1:4).
     MOVE      "-"                 TO           DSP-TANCD(5:1).
     MOVE      PARA-IN-TANCD       TO   TAN-F02 DSP-TANCD(6:2).
     READ      TANMS1
               INVALID  KEY
...
*件数表示
* 棚移動
     MOVE     PARA-IN-CNT2                  TO   DSP-KENSU1.
* ストック_ＣＨＧ
     MOVE     PARA-IN-CNT3                  TO   DSP-KENSU2.
* 製品在庫移動
     MOVE     PARA-IN-CNT4                  TO   DSP-KENSU3.
* 廃棄
     MOVE     PARA-IN-CNT5                  TO   DSP-KENSU4.
* セット組
     MOVE     PARA-IN-CNT6                  TO   DSP-KENSU5.
* ばらし
     MOVE     PARA-IN-CNT7                  TO   DSP-KENSU6.
* エラー
     MOVE     PARA-IN-CNT8                  TO   DSP-KENSU7.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FZI00301"     TO   DSP-FMT.
