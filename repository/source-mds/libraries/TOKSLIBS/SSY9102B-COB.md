...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY9102B.COB`
...
*2011/10/07 YOSHIDA.M        基幹サーバ統合                    *
*2013/08/15 NAV TAKAHASHI    発注種別区分変更                  *
*2019/03/18 NAV TAKAHASHI    発注種別変換マスタ対応            *
****************************************************************
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSY9102B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSY9102B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
     ...
     INITIALIZE DIS-REC.
*
     IF       PARA-TORICD  =  13938  OR 17137
                           OR 139381 OR 171371
              MOVE      DEN-REC  TO   KMS-REC
     END-IF.
*
     IF       PARA-TORICD  =  100403 OR 100404 OR 100427 OR 100428
     OR                      100441 OR 100442
...
*
     IF       DEN-F01  =  13938  OR 17137
                       OR 139381 OR 171371
              MOVE      KMS-A40  TO    DKC-F07
              MOVE      KMS-A38  TO    WK-HATYU-SYUBETSU
     END-IF.
*
     IF       DEN-F01  =  100403 OR 100404 OR 100427 OR 100428
     OR                   100441 OR 100442
              MOVE DIS-A06(2:3)  TO    DKC-F07
              MOVE      DIS-A19  TO    WK-HATYU-SYUBETSU
     END-IF.
     EVALUATE WK-HATYU-SYUBETSU
         WHEN  "0"
              MOVE     "1"        TO   DKC-F16
         WHEN  "1"
              MOVE     "7"        TO   DKC-F16
...
*********#2017/04/06 NAV ED
         WHEN OTHER
              MOVE     "1"        TO   DKC-F16
     END-EVALUATE.
*#2019/03/18 NAV ST 発注種別区分はそのままセット
     MOVE     WK-HATYU-SYUBETSU   TO   DKC-F16.
*#2019/03/18 NAV ED 発注種別区分はそのままセット
     READ     DSCENTF
         INVALID
...
*
     IF       DEN-F01  =  100403 OR 100404 OR 100427 OR 100428
     OR                   100441 OR 100442
              MOVE      DIS-A06  TO    DKC-F07
              MOVE      DIS-A19  TO    WK-HATYU-SYUBETSU
     END-IF.
*   システム日付－２０日算出
     MOVE  "6"               TO  LINK-IN-KBN.
     MOVE  2                 TO  LINK-IN-YMD6.
     MOVE  DEN-F111          TO  LINK-IN-YMD8.
     ...
     CALL  "SKYDTCKB"      USING LINK-IN-KBN
                                 ...
                                 LINK-OUT-RET
                                 LINK-OUT-YMD8.
     MOVE  LINK-OUT-YMD8     TO  WK-NOUDT-KIJYUN.
*
     IF      DEN-F01  =  100403 OR 100404 OR 100427 OR 100428
     OR                  100441 OR 100442
         IF  DEN-F07  =  4982
             MOVE DEN-F112             TO  WK-DEN-F112
             IF  WK-NOUDT-KIJYUN < WK-DEN-F112
                 MOVE   "1"            TO  DKC-F17
                 IF  DIS-A19  =  "4"
                     MOVE  SPACE       TO  DKC-F17
                 END-IF
             END-IF
         END-IF
     END-IF.
