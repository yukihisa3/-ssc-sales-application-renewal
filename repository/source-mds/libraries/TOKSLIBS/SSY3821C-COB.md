...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3821C.COB`
...
*    退避エリア
 01  SAV-AREA.
     03  WK-PRW-F01               PIC  9(08)  VALUE  ZERO.
     03  WK-PRW-F05               PIC  X(13)  VALUE  ZERO.
     03  WK-PRW-F12               PIC  X(02)  VALUE  SPACE.
     03  WK-PRW-F06               PIC  9(08)  VALUE  ZERO.
     03  WK-PRW-F07               PIC  9(08)  VALUE  ZERO.
     ...
     03  BK-HACU     OCCURS 14    PIC  9(08)  VALUE  ZERO.
     03  BK-TRAY     OCCURS 14    PIC  9(08)  VALUE  ZERO.
*    集計エリア
 01  SUM-AREA.
...
*    ルート番号退避
 01  WK-KAKUNOU.
     03  WK-PRW-F07NO             PIC  9(02)  VALUE  ZERO.
*    エラーセクション名
 01  SEC-NAME.
...
*    商品名編集エリア
 01  SYOHPRW-AREA.
     03  SYOHPRW-1.
         05  WK-SYOHPRW-11        PIC  X(14).
         05  WK-SYOHPRW-12        PIC  X(01).
     03  SYOHPRW-2.
         05  WK-SYOHPRW-21        PIC  X(01).
         05  WK-SYOHPRW-22        PIC  X(14).
*    部門・担当者編集エリア
 01  BUMTANPRW-AREA.
...
*    メッセージ　エリア
 01  MSG-AREA.
     03  MSG-ABEND1.
         ...
         05  ERR-PG-ID            PIC  X(08)  VALUE  "SSY3821C".
     ...
     03  MSG-ABEND2.
         ...
         05  FILLER               PIC  X(04)  VALUE  " ###".
*日付変換サブルーチン用ワーク
 01  LINK-PRW-KBN           PIC X(01).
 01  LINK-PRW-YMD6          PIC 9(06).
 01  LINK-PRW-YMD8          PIC 9(08).
...
*　　発注書印刷ワーク
 FILEERR-SEC2           SECTION.
     USE AFTER EXCEPTION PROCEDURE   HCPRXXX1.
     MOVE      "HCPRXXX1"         TO   ERR-FL-ID.
     MOVE      PRW-STATUS        TO   ERR-STCD.
     DISPLAY   MSG-ABEND1       UPON CONS.
     DISPLAY   MSG-ABEND2       UPON CONS.
     DISPLAY   SEC-NAME         UPON CONS.
     MOVE     "4000"            TO   PROGRAM-STATUS.
...
*    使用ファイル　ＯＰＥＮ
     OPEN     INPUT      HCPRXXX1 NFSHOMS1 TANMS1  NFTENMS1
                         SAKUBAL1.
     OPEN     OUTPUT     PRTF.
*    システム日付の取得
*システム日付・時刻の取得
     ACCEPT   WRK-DATE2         FROM   DATE.
     MOVE     "3"                 TO   LINK-PRW-KBN.
     MOVE     WRK-DATE2           TO   LINK-PRW-YMD6.
     MOVE     ZERO                TO   LINK-PRW-YMD8.
     MOVE     ZERO                TO   LINK-OUT-RET.
     MOVE     ZERO                TO   LINK-OUT-YMD.
     CALL     "SKYDTCKB"       USING   LINK-PRW-KBN
                                       LINK-PRW-YMD6
                                       LINK-PRW-YMD8
                                       LINK-OUT-RET
                                       LINK-OUT-YMD.
...
*    プリントエリア初期化
     MOVE     SPACE      TO       PRT-FSY38212.
     MOVE     ZERO       TO       READ-CNT.
     MOVE     ZERO       TO       GOKEI-AREA.
     MOVE     SPACE      TO       FLG-TK.
*    発注書印刷ワーク初期ＲＥＡＤ
     PERFORM  HCPRXXX1-READ-SEC.
     MOVE     PRW-F01     TO       WK-PRW-F01.
     MOVE     PRW-F05     TO       WK-PRW-F05.
     IF       PRW-F01(1:1) = 9
              MOVE PRW-F06 TO      WK-PRW-F06
     ELSE
              MOVE PRW-F02 TO      WK-PRW-F06
     END-IF.
     MOVE     PRW-F07     TO       WK-PRW-F07.
     MOVE     PRW-F12     TO       WK-PRW-F12.
     MOVE     0           TO       TATE.
     PERFORM  TEN-SET-SEC.
*
 INIT-EXT.
...
*　　発注書印刷ワーク読込み　　　　　　　  構造_            *
*============================================================*
 HCPRXXX1-READ-SEC       SECTION.
     MOVE     "HCPRXXX1-READ-SEC"   TO   S-NAME.
*    発注書印刷ワーク読込み
     READ     HCPRXXX1   AT  END
              MOVE     "END"      TO       FLG-END
              GO                  TO       HCPRXXX1-READ-EXIT
     END-READ.
 READ010.
*    対象データ件数カウント
     ADD      1                   TO       READ-CNT.
...
*    発注数合計レコード時
     IF      PRW-F13  =  99997
        PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 12
             MOVE      PRW-F202(IX) TO  PRT-GGOKEI(IX)
             ADD       PRW-F202(IX) TO  SUM-GGOKEI
        END-PERFORM
...
*    トレー数合計レコード時
     IF      PRW-F13  =  99998
        PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 12
             COMPUTE   PRT-GTRESU(IX)  =   PRW-F202(IX) + 0.9
...
*    箱数合計レコード時
     IF      PRW-F13  =  99999
        PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 12
             MOVE      PRW-F202(IX)  TO  PRT-KEIHAK(IX)
             ADD       PRW-F202(IX)  TO  SUM-KEIHAK
        END-PERFORM
...
*    使用ファイルＣＬＯＳＥ
     CLOSE                        HCPRXXX1 TANMS1 NFSHOMS1
                                  NFTENMS1 SAKUBAL1
                                  PRTF.
*    終了メッセージ
     DISPLAY "************************" UPON  CONS.
     DISPLAY "*    発注書印刷　　    *" UPON  CONS.
...
*    INITIALIZE               WK-TENPO.
 ...
 TEN010.
     PERFORM VARYING IX FROM   1  BY   1
             UNTIL ( IX > 12 )    OR ( PRW-F201(IX) = 0 )
        MOVE    PRW-F04       TO     TEN-F01
        MOVE    PRW-F201(IX)  TO     TEN-F02
        READ    NFTENMS1
                INVALID
                   MOVE   TEN-F02    TO  PRT-MTENCD(IX)
                   MOVE   ALL NC"？" TO  PRT-MTENNM(IX)
                   MOVE   ZERO       TO  PRT-KENCD(IX)
                NOT INVALID
                   MOVE   TEN-F02    TO  PRT-MTENCD(IX)
                   MOVE   TEN-F05    TO  PRT-MTENNM(IX)
                   MOVE   TEN-F12    TO  PRT-KENCD(IX)
        END-READ
     END-PERFORM.
*
 TEN020.
...
*    作場ＣＤ漢字変換→セット
     ...
     END-PERFORM.
     MOVE     SAKUBAPRW-AREA     TO   PRT-HSAKCD.
*
*    出荷日漢字変換→セット
     ...
     END-PERFORM.
*    変換文字転送
     MOVE   WK-HENKAN-KANJI      TO   PRT-HSYUDT.
*
*    部門ＣＤ漢字変換→セット
...
*TEST
*    サカタコード
     MOVE    PRW-F09      TO        PRT-MSHOCD(TATE).
*    相手コード
     MOVE    PRW-F08      TO        PRT-MNAFCD(TATE).
*    商品名,規格
     MOVE    PRW-F08     TO         SHO-F01.
     READ    NFSHOMS1
        INVALID
             MOVE      ALL NC"＊" TO   PRT-MSHON1(TATE)
             MOVE      ALL NC"＊" TO   PRT-MSHON2(TATE)
        NOT INVALID
             MOVE      SHO-F05    TO   PRT-MSHON1(TATE)
             MOVE      SHO-F06    TO   PRT-MSHON2(TATE)
     END-READ.
*    入数
     MOVE    PRW-F11      TO        PRT-MIRISU(TATE).
*    発注数
     PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 12
             MOVE      PRW-F202(IX) TO  PRT-MSURYO(TATE IX)
     END-PERFORM.
* 商品最終ページ判定
     IF PRW-F23 = "1"
        MOVE    PRW-F21       TO     PRT-MSURYO(TATE 13)
*       MOVE    PRW-F22       TO     PRT-MSURYO(TATE 14)
        COMPUTE PRT-MSURYO(TATE 14) = PRW-F22 + 0.9
     END-IF.
*
 DATA-SET-EXIT.
     EXIT.
...
*    数字を漢字に変換（ワーク変換テーブルにより）
     MOVE    WK-PRW-F06(IZ:1)          TO     WK-DATE.
     IF   (  WK-DATE  =  ZERO  )
     AND  (  IZ       =  3  OR  5  OR  7  )
