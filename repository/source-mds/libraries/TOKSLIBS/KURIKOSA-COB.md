# KURIKOSA

**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/KURIKOSA.COB`

## ソースコード

```cobol
****************************************************************
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    業務名　　　　　　　：　在庫管理システム　　　　　　　　　*
*    モジュール名　　　　：　請求データ繰越（共通）            *
*    作成日／更新日　　　：　12/03/04                          *
*    作成者／更新者　　　：　ＮＡＶ　　　　　　　　　　　　　　*
*    処理概要　　　　　　：　請求合計Ｆと，請求合計Ｆ２でマッチ*
*                            ングを行い．請求合計の合計が０以外*
*                            の時ＯＵＴＦＩＬＥに出力する．    *
*2013/06/28 「締切日」転送仕様変更                             *
*                  　                                          *
****************************************************************
 IDENTIFICATION         DIVISION.
****************************************************************
 PROGRAM-ID.            KURIKOSA.
 AUTHOR.                NAV.
 DATE-WRITTEN.          12/03/04.
 ENVIRONMENT            DIVISION.
 CONFIGURATION          SECTION.
 SPECIAL-NAMES.
     CONSOLE     IS     CONS
     STATION     IS     STAT.
*--------------------------------------------------------------*
 INPUT-OUTPUT           SECTION.
 FILE-CONTROL.
*---<<  ＩＮ１ファイル  >>---*
     SELECT   SETGKFB  ASSIGN    TO        DA-01-SETGKFB
                        ACCESS    MODE      IS   SEQUENTIAL
                        FILE      STATUS    IS   IN-ST.
*----<< 請求合計Ｆ >>--*
     SELECT   SETGKFA   ASSIGN    TO        DA-01-VI-SETGKFA2
                        ORGANIZATION        IS   INDEXED
                        ACCESS    MODE      IS   RANDOM
                        RECORD    KEY       IS   OUT-F01
                                                 OUT-F03
                                                 OUT-F04
                                                 OUT-F05
                        FILE      STATUS    IS   OUT-ST.
*
*--------------------------------------------------------------*
 DATA                   DIVISION.
 FILE                   SECTION.
*---<<  ＩＮ１ファイル  >>---*
 FD  SETGKFB.
     COPY        SETGKFA   OF        XFDLIB
                 JOINING   IN        PREFIX.
*----<< 請求合計Ｆ >>--*
 FD  SETGKFA.
     COPY        SETGKFA   OF        XFDLIB
                 JOINING   OUT       PREFIX.
*--------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*--------------------------------------------------------------*
 01  STATUS-AREA.
     03  IN-ST               PIC  X(02).
     03  OUT-ST              PIC  X(02).
 01  PSW-AREA.
     03  END-FLG             PIC  X(03)  VALUE SPACE.
 01  CNT-AREA.
     03  KEP-FLG             PIC  X(01)  VALUE SPACE.
     03  IN-CNT1             PIC  9(06)  VALUE ZERO.
     03  IN-CNT2             PIC  9(06)  VALUE ZERO.
     03  OUT-CNT             PIC  9(06)  VALUE ZERO.
     03  READ-CNT            PIC  9(06)  VALUE ZERO.
     03  WRITE-CNT           PIC  9(06)  VALUE ZERO.
     03  REWRITE-CNT         PIC  9(06)  VALUE ZERO.
     03  DELETE-CNT          PIC  9(06)  VALUE ZERO.
*
*↓2013/06/28
 LINKAGE                SECTION.
 01  PARA-SIMEBI    PIC   9(08).
*↑2013/06/28
*--------------------------------------------------------------*
*             ＭＡＩＮ　　　　ＭＯＤＵＬＥ                     *
*--------------------------------------------------------------*
*↓2013/06/28
*PROCEDURE              DIVISION.
 PROCEDURE              DIVISION  USING PARA-SIMEBI.
*↑2013/06/28
**
 DECLARATIVES.
 FILEERR-SEC1           SECTION.
     USE AFTER     EXCEPTION
                   PROCEDURE      SETGKFB.
     MOVE      4000          TO   PROGRAM-STATUS.
     STOP     RUN.
**
 FILEERR-SEC2           SECTION.
     USE AFTER     EXCEPTION
                   PROCEDURE      SETGKFA.
     MOVE      4000          TO   PROGRAM-STATUS.
     STOP     RUN.
 END     DECLARATIVES.
****************************************************************
 GENERAL-PROCESS             SECTION.
     PERFORM       INIT-SEC.
     PERFORM       MAIN-SEC
                   UNTIL     END-FLG   =    "END".
     PERFORM       END-SEC.
     STOP      RUN.
 GENERAL-END.
     EXIT.
****************************************************************
*      _０　　初期処理                                        *
****************************************************************
 INIT-SEC               SECTION.
*
     OPEN     INPUT     SETGKFB.
     OPEN     I-O       SETGKFA.
     MOVE     SPACE          TO   END-FLG.
*
     READ     SETGKFB
          AT  END
              MOVE     "END" TO   END-FLG
          NOT  AT  END
              ADD       1    TO   READ-CNT
     END-READ.
*
 INIT-END.
     EXIT.
****************************************************************
*      _０　　メイン処理                                      *
****************************************************************
 MAIN-SEC               SECTION.
*
     MOVE     IN-F01        TO   OUT-F01.
     MOVE     IN-F03        TO   OUT-F03.
     MOVE     IN-F04        TO   OUT-F04.
     MOVE     IN-F05        TO   OUT-F05.
     READ     SETGKFA
          INVALID KEY
              PERFORM       OUTPUT-SEC
          NOT INVALID KEY
              PERFORM       REWRITE-SEC
     END-READ.
*
     READ     SETGKFB
          AT  END
              MOVE     "END" TO   END-FLG
          NOT  AT  END
              ADD       1    TO   READ-CNT
     END-READ.
*
 MAIN-END.
     EXIT.
****************************************************************
*　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　*
****************************************************************
 OUTPUT-SEC             SECTION.
*
     MOVE    SPACE     TO     OUT-REC.
     INITIALIZE               OUT-REC.
     MOVE    IN-REC    TO     OUT-REC.
*↓2013/06/28
     MOVE    PARA-SIMEBI(3:6) TO  OUT-F02.
*↑2013/06/28
     WRITE   OUT-REC
     END-WRITE.
     ADD     1         TO     WRITE-CNT.
*
 OUTPUT-END.
     EXIT.
****************************************************************
*　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　*
****************************************************************
 REWRITE-SEC            SECTION.
*
*****ADD     IN-F06    TO     OUT-F06.
     IF      OUT-F06   =      ZERO
             DELETE           SETGKFA
             ADD       1      TO  DELETE-CNT
     ELSE
             REWRITE          OUT-REC
             ADD       1      TO  REWRITE-CNT
     END-IF.
*
 REWRITE-END.
     EXIT.
****************************************************************
*      3.0        終了処理                                     *
****************************************************************
 END-SEC                SECTION.
*
     CLOSE    SETGKFB  SETGKFA.
*カウント表示
     DISPLAY "READ    CNT = "  READ-CNT      UPON CONS.
     DISPLAY "WRITE   CNT = "  WRITE-CNT     UPON CONS.
     DISPLAY "REWRITE CNT = "  REWRITE-CNT   UPON CONS.
     DISPLAY "DELETE  CNT = "  DELETE-CNT    UPON CONS.
*
 END-END.
     EXIT.
*****************<<  PROGRAM  END  >>***********************

```
