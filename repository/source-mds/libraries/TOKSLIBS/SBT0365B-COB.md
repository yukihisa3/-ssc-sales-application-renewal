...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBT0365B.COB`
...
*
 ...
 FILE-CONTROL.
*カインズ出荷連携累積データ
     SELECT   CNZSYRL2  ASSIGN    TO        DA-01-VI-CNZSYRL2
...
*    カインズ出荷連携累積データ
******************************************************************
 FD  CNZSYRL2           BLOCK     CONTAINS   1   RECORDS
                        LABEL     RECORD     IS  STANDARD.
     COPY     CNZSYRL2  OF        XFDLIB
              JOINING   SYU       PREFIX.
******************************************************************
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SBT0365B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SBT0365B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*
 DECLARATIVES.
 SYU-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE CNZSYRL2.
...
*カインズ出荷連携累積データ スタート
     ...
     MOVE     PARA-IN-TEG-DEN-FR    TO   SYU-F302.
     MOVE     PARA-IN-TEG-NOUHIN-FR TO   SYU-F346.
     START    CNZSYRL2  KEY  >=   SYU-F92
                                  SYU-F02   SYU-F013
                                  SYU-F302  SYU-F346
         INVALID   KEY
              MOVE "END"     TO   END-FLG
...
*　　　　　　カインズ出荷連携累積データ読込
****************************************************************
 CNZSYRL2-READ-SEC          SECTION.
*
     READ     CNZSYRL2  NEXT
              AT  END       MOVE  "END"   TO  END-FLG
                            GO TO CNZSYRL2-READ-EXIT
...
*取引先チェック         ＰＡＲＡ取引先ブレイクで終了
 CNZSYRL2-READ-2.
     IF       PARA-IN-TEG-TOR       =  SYU-F013
              CONTINUE
     ELSE
              MOVE      "END"     TO   END-FLG
              GO                  TO   CNZSYRL2-READ-EXIT
     END-IF.
*取引番号（伝票番号）  ＰＡＲＡ伝票番号（ＴＯ）超で終了
 CNZSYRL2-READ-3.
     IF       PARA-IN-TEG-DEN-TO      <    SYU-F302
              MOVE      "END"     TO   END-FLG
              GO                  TO   CNZSYRL2-READ-EXIT
     END-IF.
*納品日範囲チェック    範囲外は読み飛ばし
 CNZSYRL2-READ-4.
     IF       PARA-IN-TEG-NOUHIN-FR   >    SYU-F346
              GO                  TO   CNZSYRL2-READ-SEC
     END-IF.
     IF       PARA-IN-TEG-NOUHIN-TO   <    SYU-F346
              GO                  TO   CNZSYRL2-READ-SEC
     END-IF.
*
 CNZSYRL2-READ-EXIT.
