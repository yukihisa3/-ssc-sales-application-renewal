...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3823V.COB`
...
*発注ＣＳＶ（店舗）　　
     SELECT   HCCSVTEN  ASSIGN    TO        DA-01-HCCSVTEN
...
*発注ＣＳＶ（明細）　　
     SELECT   HCCSVMEI  ASSIGN    TO        DA-01-HCCSVMEI
                        ORGANIZATION        SEQUENTIAL
                        ACCESS    MODE      SEQUENTIAL
...
*発注ＣＳＶ（店舗）
******************************************************************
 FD  HCCSVTEN            LABEL RECORD   IS   STANDARD
     BLOCK    CONTAINS   1         RECORDS.
     COPY     HCCSVTEN   OF        XFDLIB
              JOINING    CTP       PREFIX.
******************************************************************
*発注書商品明細ワーク
******************************************************************
...
*発注ＣＳＶ（明細）
******************************************************************
 FD  HCCSVMEI            LABEL RECORD   IS   STANDARD
     BLOCK    CONTAINS   1         RECORDS.
     COPY     HCCSVMEI   OF        XFDLIB
              JOINING    CSM       PREFIX.
******************************************************************
*ナフコ商品マスタ
******************************************************************
...
*メッセージ出力領域
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG           PIC  X(08)  VALUE "SSY3823V".
         05  FILLER          PIC  X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER          PIC  X(05)  VALUE " *** ".
         05  END-PG          PIC  X(08)  VALUE "SSY3823V".
         05  FILLER          PIC  X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER          PIC  X(05)  VALUE " *** ".
...
****************************************************************
 INIT-SEC               SECTION.
     ...
     OPEN     INPUT     HCTPXXX1  HCSMXXX1  NFSHOMS1.
     OPEN     OUTPUT    HCCSVTEN  HCCSVMEI.
...
*
     PERFORM  VARYING  IX FROM 1 BY 1 UNTIL
        ...
        OR  ( HTP-F01 NOT = WK-HTP-F01 )
        OR  ( HTP-F05 NOT = WK-HTP-F05 )
        OR  ( HTP-F09 NOT = WK-HTP-F09 )
        OR  ( IX          > 500        )
*             発注ＣＳＶ（店舗）SET
              PERFORM  HCCSVTEN-SET-SEC
*             発注書店舗ワーク読込み
              PERFORM  HCTPXXX1-READ-SEC
     END-PERFORM.
*
     IF     ( HTP-F01 NOT = WK-HTP-F01 )
        OR  ( HTP-F05 NOT = WK-HTP-F05 )
        OR  ( HTP-F09 NOT = WK-HTP-F09 )
*             発注ＣＳＶ（店舗）OUT
              WRITE   CTP-REC
...
*発注ＣＳＶ（店舗）SET
****************************************************************
 HCCSVTEN-SET-SEC     SECTION.
*
     MOVE     "HCCSVTEN-SET-SEC"  TO  S-NAME.
*
 HCCSVTEN-SET-01.
...
*発注ＣＳＶ（明細)SET
****************************************************************
 HCCSVMEI-SET-SEC     SECTION.
*
     MOVE     "HCCSVMEI-SET-SEC"  TO  S-NAME.
*
 HCCSVMEI-SET-01.
     ...
     MOVE    ","       TO   CSM-A03.
     MOVE    HSM-F09   TO   CSM-F04.
     MOVE    ","       TO   CSM-A04.
     MOVE   X"28"      TO   CSM-A05.
*    商品名,規格
     MOVE    HSM-F09     TO         SHO-F01.
     READ    NFSHOMS1
        INVALID
             MOVE      ALL NC"＊" TO   CSM-F05
             MOVE      ALL NC"＊" TO   CSM-F06
        NOT INVALID
             MOVE      SHO-F05    TO   CSM-F05
             MOVE      SHO-F06    TO   CSM-F06
     END-READ.
     MOVE   X"29"      TO   CSM-A06.
     MOVE    ","       TO   CSM-A07.
     MOVE   X"28"      TO   CSM-A08.
     MOVE   X"29"      TO   CSM-A09.
     MOVE    ","       TO   CSM-A10.
     PERFORM  VARYING  IX FROM 1 BY 1 UNTIL
              IX          > 500
              IF      HSM-F201(IX)  NOT = ZERO
                      MOVE  HSM-F201(IX) TO   CSM-F071(IX)
              END-IF
              MOVE    ","           TO   CSM-A11(IX)
     END-PERFORM.
*
...
*
     CLOSE     NFSHOMS1.
     CLOSE     HCTPXXX1  HCCSVTEN.
     CLOSE     HCSMXXX1  HCCSVMEI.
*
