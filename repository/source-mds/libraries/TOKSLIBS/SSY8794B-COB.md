...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY8794B.COB`
...
*
 ...
 FILE-CONTROL.
*ＤＣＭＪＡＰＡＮ出荷確定ファイル
     SELECT   DJSYUKF   ASSIGN    TO        DA-01-VI-DJSYUKL1
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSY8794B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSY8794B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*******
 ...
 01  WK-SAKUJO-KIJYUN   PIC S9(02)     VALUE  ZERO.
*
 01  LINK-AREA.
...
******************
     ...
     IF          LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8  TO   SYS-DATEW  WK-ZAIKO-DATE
     ELSE
         MOVE    ZERO           TO   SYS-DATEW
     END-IF.
*----<< 削除月数取得 >>-*
     MOVE    83              TO   JYO-F01.
     MOVE    "DATE2"         TO   JYO-F02.
     READ    HJYOKEN
             INVALID
...
*----<< 基準日付算出 >>-*
     MOVE     99                  TO   WK-ZAI-2.
     COMPUTE  WK-SAKUJO-KIJYUN  =  WK-ZAI-12  -  WK-SAKUJO.
     IF  WK-SAKUJO-KIJYUN  <=  ZERO
         ADD    -1      TO        WK-ZAI-11
         MOVE   12      TO        WK-ZAI-12
         COMPUTE WK-ZAI-12 = WK-ZAI-12 + WK-SAKUJO-KIJYUN
     ELSE
         MOVE WK-SAKUJO-KIJYUN TO WK-ZAI-12
     END-IF.
*----<< 削除基準日出力 >>-*
...
*    件数表示
     IF      READ-CNT(6:3) = "000" OR "500"
             DISPLAY "RD-CNT = " READ-CNT " : " DEL-CNT UPON CONS
     END-IF.
*    受信日のチェック
     IF      WK-ZAIKO-DATE  <   DJS-K01
             MOVE     "END"    TO  END-FLG
             GO                TO  DJSYUKF-READ-EXIT
     END-IF.
*    送信ＦＬＧチェック
...
*
     DISPLAY "DCM ｼﾞｮｳﾎｳﾃﾞｰﾀ READ CNT = " READ-CNT  UPON CONS.
     DISPLAY "DCM ｼﾞｮｳﾎｳﾃﾞｰﾀ DELE CNT = " DEL-CNT   UPON CONS.
*
     CLOSE     DJSYUKF  HJYOKEN.
*
     STOP      RUN.
*
 END-EXIT.
