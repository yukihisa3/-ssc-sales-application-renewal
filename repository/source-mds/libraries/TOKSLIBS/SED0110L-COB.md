...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SED0110L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*ＥＤＩＣ伝票データ（共通）　　
     SELECT   EDSHIRED  ASSIGN    TO        EDSHIRED
...
***  ﾌﾗｸﾞ ｴﾘｱ
 01  FLG-AREA.
     ...
     03  HTENMS-INV-FLG           PIC  X(03)     VALUE  ZERO.
***  ｶｳﾝﾄ
 01  CNT-AREA.
...
***  ﾜｰｸ ｴﾘｱ
 ...
 01  EDJOHOF-INV-FLG              PIC  X(03)     VALUE  SPACE.
 01  WK-HIDUKE                    PIC  9(08)     VALUE  ZERO.
 01  WK-DEN-F111.
     03  WK-DEN-F111-1            PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-F111-2            PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-F111-3            PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-F111-4            PIC  9(02)     VALUE  ZERO.
 01  WK-DEN-F112.
     03  WK-DEN-F112-1            PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-F112-2            PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-F112-3            PIC  9(02)     VALUE  ZERO.
     03  WK-DEN-F112-4            PIC  9(02)     VALUE  ZERO.
 01  SEC-NAME.
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      EDSHIRED.
...
*--- << ＥＤＩＣ基本情報ファイル >> ---*
 000-JOH-ERR            SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      EDJOHOF.
...
***
                   ...
                   PERFORM   DENP-WT-SEC
                             UNTIL     END-FLG   =   "END"
                   CLOSE     EDSHIRED  PRTFILE  EDJOHOF  HTENMS
                   OPEN      INPUT     EDSHIRED
                   OPEN      OUTPUT    PRTFILE
                   OPEN      INPUT     EDJOHOF
                   OPEN      INPUT     HTENMS
...
*明細２行目
***  商品名（下段）
     MOVE     JOH-F414            TO        DT2-01.
***  商品コード
     MOVE     DEN-F25             TO        DT2-02.
***  入数
*****MOVE     JOH-F422            TO        DT2-021.
     COMPUTE  DT2-021  =  JOH-F422 / 10.
***  ケース
*****MOVE     JOH-F423            TO        DT2-022.
     COMPUTE  DT2-022  =  JOH-F423 / 10.
***  数量
...
*ヘッダ２行目　
***  社名
     MOVE     JOH-F207            TO    HD2-01.
*
*ヘッダ　３　行目　
***  店名
     MOVE     JOH-F216            TO    HD3-01.
***  店舗名カナ取得
     ...
     IF       HTENMS-INV-FLG = "INV"
              MOVE  JOH-F216  TO    HD3-01
     ELSE
              MOVE  TEN-F04   TO    HD3-01
     END-IF.
***  店コード
     MOVE     DEN-F07             TO    HD3-02.
***  部門
     MOVE     DEN-F12(1:3)        TO    HD3-03.
***  伝区
     MOVE     DEN-F132(2:1)       TO    HD3-04(2:1).
***  伝票_
     MOVE     DEN-F02             TO    HD3-05.
***  取引先ＣＤ
     MOVE     DEN-F01             TO    HD3-06.
***  取引先名（下段）
     MOVE     JOH-F226            TO    HD3-07.
***  発注日
     MOVE     DEN-F111            TO    WK-HIDUKE.
     MOVE     WK-HIDUKE           TO    WK-DEN-F111.
     MOVE     WK-DEN-F111-2       TO    HD3-081.
     MOVE     WK-DEN-F111-3       TO    HD3-082.
     MOVE     WK-DEN-F111-4       TO    HD3-083.
***  納品日
     MOVE     DEN-F112            TO    WK-HIDUKE.
     MOVE     WK-HIDUKE           TO    WK-DEN-F112.
     MOVE     WK-DEN-F112-2       TO    HD3-091.
     MOVE     WK-DEN-F112-3       TO    HD3-092.
     MOVE     WK-DEN-F112-4       TO    HD3-093.
*
...
*
     CLOSE    EDSHIRED   PRTFILE    DSPFILE   EDJOHOF  HTENMS.
*
 END-EXIT.
     EXIT.
****************************************************************
...
*
     MOVE     DEN-F46        TO   JOH-F011.
     MOVE     DEN-F47        TO   JOH-F012
     MOVE     DEN-F01        TO   JOH-F013.
     MOVE     DEN-F48        TO   JOH-F02.
     MOVE     DEN-F07        TO   JOH-F03.
     MOVE     DEN-F112       TO   JOH-F04.
     MOVE     DEN-F02        TO   JOH-F05.
     MOVE     DEN-F03        TO   JOH-F06.
     READ     EDJOHOF    INVALID
              DISPLAY NC"ＥＤＩＣ基本情報存在無！" UPON CONS
