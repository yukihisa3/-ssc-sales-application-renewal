...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSE007LP.COB`
...
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     ...
     DISPLAY  "### SSE007LP DSPFILE ERROR " DSP-CNTL " "
              WK-Y "." WK-M "." WK-D " "
              WK-HH ":" WK-MN ":" WK-SS " ###"
                                       UPON CONS.
***  CLOSE    HSEIGKF   HTOKMS    HTENMS    PRTF      DSPFILE.
     STOP     RUN.
*----<< 請求合計ファイル >>--*
 SEI-ERR                SECTION.
...
*----<< 取引先マスタ >>--*
 HTOKMS-ERR             SECTION.
     ...
     ACCEPT   WK-DATE       FROM DATE.
     ACCEPT   WK-TIME       FROM TIME.
     DISPLAY  "### SSE007LP HTOKMS ERROR " TOK-ST " "
              WK-Y "." WK-M "." WK-D " "
              WK-HH ":" WK-MN ":" WK-SS " ###"
                                       UPON CONS.
***  CLOSE    HSEIGKF   HTOKMS    HTENMS    PRTF      DSPFILE.
...
*----<< 店舗マスタ >>--*
 HTENMS-ERR             SECTION.
     ...
     ACCEPT   WK-DATE       FROM DATE.
     ACCEPT   WK-TIME       FROM TIME.
     DISPLAY  "### SSE007LP HTENMS ERROR " TEN-ST " "
              WK-Y "." WK-M "." WK-D " "
              WK-HH ":" WK-MN ":" WK-SS " ###"
                                       UPON CONS.
***  CLOSE    HSEIGKF   HTOKMS    HTENMS    PRTF      DSPFILE.
...
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     MOVE     SPACE          TO   FSE00701.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FSE00701"     TO   DSP-FMT.
...
*
     ...
     MOVE     "."            TO   ME-082P.
     MOVE     WK-D           TO   ME-083.
     MOVE     SEI-F05        TO   ME-09.
     MOVE     SEI-F06        TO   ME-10.
*ｻﾝﾃﾞｰ対応 2008/12/16
     IF       SEI-F01  =  30402  OR  304021
              MOVE     SEI-F12        TO   ME-105
**************2013/05/21 NAV ST 分類ＣＤを追加出力する
     ELSE
              IF  SEI-F01 = 87371
              OR  SEI-F01 = 87372
              OR  SEI-F01 = 87373
...
**************2016/03/23 NAV ST
              OR  SEI-F01 = 87376
**************2016/03/23 NAV ED
                  MOVE     SEI-F12        TO   ME-105(7:4)
                  IF  SEI-F12 = SPACE
                      MOVE "ﾌﾞﾝﾙｲNG]]]"   TO   ME-105
                  END-IF
              ELSE
                  MOVE     SEI-F07        TO   ME-105(1:2)
              END-IF
**************2013/05/21 NAV ED 分類ＣＤを追加出力する
     END-IF.
*
     IF       NEW       NOT  =    OLD
     AND      LINE-CNT  NOT  =    9
...
****
     ...
     MOVE     SEI-F02(1:2)   TO   HD-051.
     MOVE     SEI-F02(3:2)   TO   HD-052.
     MOVE     SEI-F02(5:2)   TO   HD-053.
*
...
*--------------------------------------------------------------*
 900-SEI-START-READ     SECTION.
     START    HSEIGKF   KEY  >=   SEI-F01   SEI-F03
                                  SEI-F04   SEI-F05
              INVALID   KEY
