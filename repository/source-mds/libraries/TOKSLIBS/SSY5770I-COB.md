# SSY5770I

**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY5770I.COB`

## ソースコード

```cobol
****************************************************************
*    顧客名　　　　　　　：　（株）サカタのタネ殿　　　　　　　*
*    サブシステム　　　　：　グッデイＥＤＩ
*    業務名　　　　　　　：　受注・売上　　　　　　　　
*    モジュール名　　　　：　受領書ＣＳＶデータ出力指示
*    作成日／更新日　　　：　2014/07/03                        *
*    作成者／更新者　　　：　NAV                               *
*    処理概要　　　　　　：　画面より、検品日範囲、店舗範囲、  *
*                            伝票区分を入力し、パラメタに渡    *
*                            す。                              *
****************************************************************
****************************************************************
 IDENTIFICATION         DIVISION.
****************************************************************
 PROGRAM-ID.            SSY5770I.
 AUTHOR.                NAV.
 DATE-WRITTEN.          14/07/03.
 DATE-COMPILED.
 SECURITY.              NONE.
****************************************************************
 ENVIRONMENT            DIVISION.
****************************************************************
 CONFIGURATION          SECTION.
 SOURCE-COMPUTER.       FUJITSU.
 OBJECT-COMPUTER.       FUJITSU.
 SPECIAL-NAMES.
         STATION   IS   STAT
         CONSOLE   IS   CONS.
*
 INPUT-OUTPUT           SECTION.
 FILE-CONTROL.
*----<< 表示ファイル >>--*
     SELECT   DSPFILE   ASSIGN         01-GS-DSPF
                        FORMAT         DSP-FMT
                        GROUP          DSP-GRP
                        PROCESSING     DSP-PRO
                        UNIT CONTROL   DSP-CON
                        FUNCTION       DSP-FNC
                        STATUS         DSP-ST.
*----<< 受領累積ファイル >>--*
     SELECT   GDJYURL2   ASSIGN         DA-01-VI-GDJYURL2
                        ORGANIZATION   INDEXED
                        ACCESS    MODE SEQUENTIAL
                        RECORD    KEY  GDJ-F04   GDJ-F05
                                       GDJ-F06   GDJ-F07
                                       GDJ-F08
                        STATUS         GDJYURL2-ST.
*----<< 取引先マスタ >>--*
*    SELECT   HTOKMS    ASSIGN         DA-01-VI-TOKMS2
*                       ORGANIZATION   INDEXED
*                       ACCESS    MODE RANDOM
*                       RECORD    KEY  TOK-F01
*                       STATUS         HTOKRH-ST.
*----<< 倉庫マスタ >>--*
*    SELECT   ZSOKMS1   ASSIGN         DA-01-VI-ZSOKMS1
*                       ORGANIZATION   INDEXED
*                       ACCESS    MODE RANDOM
*                       RECORD    KEY  SOK-F01
*                       STATUS         ZSOKMS1-ST.
*
****************************************************************
 DATA                   DIVISION.
****************************************************************
 FILE                   SECTION.
*----<< 表示ファイル >>--*
 FD  DSPFILE            LABEL     RECORD   IS   STANDARD.
     COPY     FSY57701  OF        XMDLIB.
*----<< 受領累積ファイル >>--*
 FD  GDJYURL2           LABEL     RECORD   IS   STANDARD.
     COPY     GDJYURL2  OF        XFDLIB
              JOINING   GDJ       PREFIX.
*----<< 取引先マスタ >>--*
*FD  HTOKMS             LABEL RECORD   IS   STANDARD.
*    COPY     HTOKMS    OF        XFDLIB
*             JOINING   TOK       PREFIX.
*----<< 倉庫マスタ >>--*
*FD  ZSOKMS1            LABEL RECORD   IS   STANDARD.
*    COPY     ZSOKMS1   OF        XFDLIB
*             JOINING   SOK       PREFIX.
*--------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*--------------------------------------------------------------*
 01  FLAGS.
     03  ERR-FLG        PIC  9(02)    VALUE  ZERO.
*
*----<< ﾌｱｲﾙ ｽﾃｰﾀｽ >>--*
 01  GDJYURL2-ST       PIC  X(02).
*01  HTOKRH-ST         PIC  X(02).
*01  ZSOKMS1-ST        PIC  X(02).
*
*----<< ﾋﾂﾞｹ ﾜｰｸ >>--*
 01  SYS-DATE           PIC  9(06).
 01  FILLER             REDEFINES      SYS-DATE.
     03  SYS-YY         PIC  9(02).
     03  SYS-MM         PIC  9(02).
     03  SYS-DD         PIC  9(02).
 01  SYS-DATEW          PIC  9(08).
 01  FILLER             REDEFINES      SYS-DATEW.
     03  SYS-YYW        PIC  9(04).
     03  SYS-MMW        PIC  9(02).
     03  SYS-DDW        PIC  9(02).
 01  WK-SYSYMD.
     03  WK-SYSYY       PIC  9(04).
     03  FILLER         PIC  X(01)     VALUE "/".
     03  WK-SYSMM       PIC  Z9.
     03  FILLER         PIC  X(01)     VALUE "/".
     03  WK-SYSDD       PIC  Z9.
 01  SYS-TIME           PIC  9(08).
 01  FILLER             REDEFINES      SYS-TIME.
     03  SYS-HH         PIC  9(02).
     03  SYS-MN         PIC  9(02).
     03  SYS-SS         PIC  9(02).
     03  SYS-MS         PIC  9(02).
 01  SYS-TIME2          PIC  9(08).
 01  FILLER             REDEFINES      SYS-TIME2.
     03  SYS-TIMEW      PIC  9(06).
     03  FILLER         PIC  9(02).
*
 01  WK-SOKCD           PIC  X(02)  VALUE  SPACE.
*----<< ﾃﾞｨｽﾌﾟﾚｲ ｺﾝﾄﾛｰﾙ ｴﾘｱ >>-*
 01  GR-NO              PIC  9(02).
 01  WK-GRP             PIC  X(08).
 01  DSP-CNTL.
     03  DSP-ST         PIC  X(02).
     03  DSP-ST2        PIC  X(04).
     03  DSP-FMT        PIC  X(08).
     03  DSP-GRP        PIC  X(08).
     03  DSP-PRO        PIC  X(02).
     03  DSP-FNC        PIC  X(04).
     03  DSP-CON        PIC  X(06).
*
*----<< ﾌｱﾝｸｼﾖﾝ ｷｰ ﾃ-ﾌﾞﾙ >>-*
 01  FNC-TABLE.
     03  ENT            PIC  X(04)     VALUE     "E000".
     03  PF04           PIC  X(04)     VALUE     "F004".
     03  PF05           PIC  X(04)     VALUE     "F005".
     03  PF06           PIC  X(04)     VALUE     "F006".
*
*----<< ﾌｱﾝｸｼﾖﾝ ｷｰ ｶﾞｲﾄﾞ >>-*
 01  GUIDE01       PIC  N(40)  VALUE   NC"_終　了".
 01  GUIDE02       PIC  N(40)  VALUE
         NC"_取　消　_終　了　_項目戻り".
*
 01  MSG-AREA.
     03  MSG01               PIC  N(20)  VALUE
              NC"ＰＦキーが違います".
     03  MSG02               PIC  N(20)  VALUE
              NC"　　　　　　　　　　　　　".
     03  MSG03               PIC  N(20)  VALUE
              NC"　　　　　　　　　　　　　".
     03  MSG04               PIC  N(20)  VALUE
              NC"　　　　　　　　　　　　".
     03  MSG05               PIC  N(20)  VALUE
              NC"グッデイ受領累積ファイルに存在しません。".
     03  MSG06               PIC  N(20)  VALUE
              NC"伝票区分が違います。".
     03  MSG07               PIC  N(20)  VALUE
              NC"開始が終了を超えています。".
     03  MSG08               PIC  N(20)  VALUE
              NC"開始検品日を正しく入力してください。".
     03  MSG09               PIC  N(20)  VALUE
              NC"終了検品日を正しく入力してください。".
*
 01  FILLER                  REDEFINES   MSG-AREA.
     03  MSG-TBL             PIC  N(20)  OCCURS       9.
*
 01  SEC-AREA.
     03  SEC-NAME.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  FILLER         PIC   X(07)  VALUE " SEC = ".
         05  S-NAME         PIC   X(30).
*
 01  LINK-AREA.
     03  LINK-IN-KBN        PIC   X(01).
     03  LINK-IN-YMD6       PIC   9(06).
     03  LINK-IN-YMD8       PIC   9(08).
     03  LINK-OUT-RET       PIC   X(01).
     03  LINK-OUT-YMD8      PIC   9(08).
*
 LINKAGE                SECTION.
 01  PARA-KENDTS            PIC   9(08).
 01  PARA-KENDTE            PIC   9(08).
 01  PARA-TENCDS            PIC   9(03).
 01  PARA-TENCDE            PIC   9(03).
 01  PARA-DENKU             PIC   X(02).
*
****************************************************************
 PROCEDURE              DIVISION USING PARA-KENDTS
                                       PARA-KENDTE
                                       PARA-TENCDS
                                       PARA-TENCDE
                                       PARA-DENKU.
****************************************************************
*--------------------------------------------------------------*
*    LEVEL 0        エラー処理　　　　　　　　　　　　　　　　 *
*--------------------------------------------------------------*
 DECLARATIVES.
*----<< 表示ファイル >>--*
 DSPFILE-ERR            SECTION.
     USE AFTER     EXCEPTION PROCEDURE      DSPFILE.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY5770I DSPFILE ERROR " DSP-CNTL " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
*    CLOSE    GDJYURL2   HTOKMS    ZSOKMS1    DSPFILE.
     STOP     RUN.
*----<< 受領累積ファイル >>--*
 GDJYURL2-ERR            SECTION.
     USE AFTER     EXCEPTION PROCEDURE      GDJYURL2.
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSY5770I GDJYURL2 ERROR " GDJYURL2-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
*    CLOSE    GDJYURL2   HTOKMS    ZSOKMS1    DSPFILE.
     STOP     RUN.
*----<< 取引先マスタ >>--*
*HTOKRH-ERR             SECTION.
*    USE AFTER     EXCEPTION PROCEDURE      HTOKMS.
*    ACCEPT   SYS-DATE       FROM DATE.
*    ACCEPT   SYS-TIME       FROM TIME.
*    MOVE     4000           TO   PROGRAM-STATUS.
*    DISPLAY  "### SSY5770I HTOKMS ERROR " HTOKRH-ST " "
*             SYS-YY "." SYS-MM "." SYS-DD " "
*             SYS-HH ":" SYS-MN ":" SYS-SS " ###"
*                                      UPON CONS.
*    DISPLAY  SEC-NAME                 UPON CONS.
*    CLOSE    GDJYURL2   HTOKMS    ZSOKMS1    DSPFILE.
*    STOP     RUN.
*----<< 倉庫マスタ >>--*
*ZSOKMS1-ERR             SECTION.
*    USE AFTER     EXCEPTION PROCEDURE      ZSOKMS1.
*    ACCEPT   SYS-DATE       FROM DATE.
*    ACCEPT   SYS-TIME       FROM TIME.
*    MOVE     4000           TO   PROGRAM-STATUS.
*    DISPLAY  "### SSY5770I ZSOKMS1 ERROR " ZSOKMS1-ST " "
*             SYS-YY "." SYS-MM "." SYS-DD " "
*             SYS-HH ":" SYS-MN ":" SYS-SS " ###"
*                                      UPON CONS.
*    DISPLAY  SEC-NAME                 UPON CONS.
*    CLOSE    GDJYURL2   HTOKMS    ZSOKMS1    DSPFILE.
*    STOP     RUN.
 END DECLARATIVES.
*--------------------------------------------------------------*
*    LEVEL   1     ﾌﾟﾛｸﾞﾗﾑ ｺﾝﾄﾛｰﾙ                              *
*--------------------------------------------------------------*
 000-PROG-CNTL          SECTION.
     MOVE    "000-PROG-CNTL"      TO   S-NAME.
     PERFORM  100-INIT-RTN.
     PERFORM  200-MAIN-RTN   UNTIL     GR-NO    =    99.
     PERFORM  300-END-RTN.
     STOP RUN.
 000-PROG-CNTL-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  2      ｼｮｷ ｼｮﾘ                                     *
*--------------------------------------------------------------*
 100-INIT-RTN           SECTION.
     MOVE    "100-INIT-RTN"       TO   S-NAME.
     ACCEPT   SYS-DATE       FROM DATE.
     MOVE    "3"        TO        LINK-IN-KBN.
     MOVE     SYS-DATE  TO        LINK-IN-YMD6.
     CALL    "SKYDTCKB" USING     LINK-IN-KBN
                                  LINK-IN-YMD6
                                  LINK-IN-YMD8
                                  LINK-OUT-RET
                                  LINK-OUT-YMD8.
     IF       LINK-OUT-RET   =    ZERO
         MOVE LINK-OUT-YMD8  TO   SYS-DATEW
     ELSE
         MOVE ZERO           TO   SYS-DATEW
     END-IF.
*
     MOVE     SYS-YYW        TO   WK-SYSYY.
     MOVE     SYS-MMW        TO   WK-SYSMM.
     MOVE     SYS-DDW        TO   WK-SYSDD.
*
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "*** SSY5770I START *** "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS
                                       UPON CONS.
     OPEN     I-O       DSPFILE.
     OPEN     INPUT     GDJYURL2.
*    OPEN     INPUT     HTOKMS.
*    OPEN     INPUT     ZSOKMS1.
*----<< ﾜｰｸ ｼｮｷｾｯﾄ >>-*
     MOVE     0              TO   GR-NO.
 100-INIT-RTN-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  2      ﾒｲﾝ ｼｮﾘ                                     *
*--------------------------------------------------------------*
 200-MAIN-RTN           SECTION.
     MOVE    "200-MAIN-RTN"       TO   S-NAME.
*----<< 画面初期表示   >>-*
     PERFORM  210-DSP-INIT   UNTIL     GR-NO    NOT  =    0.
*----<< 条件指定　　   >>-*
     PERFORM  220-INP-GRP01  UNTIL     GR-NO    NOT  =    1.
*----<< 確認入力　　   >>-*
     PERFORM  230-INP-KKNN   UNTIL     GR-NO    NOT  =    9.
*----<< パラメタセット >>-*
     PERFORM  240-PARA-SEC   UNTIL     GR-NO    NOT  =    10.
 200-MAIN-RTN-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  2      ｴﾝﾄﾞ ｼｮﾘ                                    *
*--------------------------------------------------------------*
 300-END-RTN            SECTION.
     MOVE    "300-END-RTN"        TO   S-NAME.
     CLOSE    DSPFILE.
     CLOSE    GDJYURL2.
*    CLOSE    HTOKMS.
*    CLOSE    ZSOKMS1.
*
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "*** SSY5770I END *** "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS
                                       UPON CONS.
 300-END-RTN-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      画面初期表示                                *
*--------------------------------------------------------------*
 210-DSP-INIT           SECTION.
     MOVE    "210-DSP-INIT"       TO   S-NAME.
     MOVE     SPACE          TO   FSY57701.
*
     PERFORM  CLR-HEAD-RTN.
     PERFORM  CLR-TAIL-RTN.
*
     MOVE    "SSY5770I"      TO   PGID.
     MOVE    "FSY57701"      TO   FORM.
*倉庫コードパラメタより倉庫コード入力チェック
*    IF       PARA-DDENKU  =  "01"
*             MOVE    " "    TO   EDIT-STATUS OF DENKU
*    ELSE
*             MOVE      PARA-DENKU TO   SOK-F01 DENKU
*             READ      ZSOKMS1
*                 INVALID  KEY
*                       MOVE      ALL NC"＊" TO   DENKUNM
*             NOT INVALID  KEY
*                       MOVE      SOK-F02    TO   DENKUNM
*             END-READ
*             MOVE    "X"    TO   EDIT-STATUS OF DENKU
*    END-IF.
*
     MOVE     SPACE          TO   DSP-CNTL.
     MOVE     "FSY57701"     TO   DSP-FMT.
     MOVE     "SCREEN"       TO   DSP-GRP.
     PERFORM  900-DSP-WRITE.
*
     MOVE     1              TO   GR-NO.
 210-DSP-INIT-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      条件指定                                    *
*--------------------------------------------------------------*
 220-INP-GRP01          SECTION.
     MOVE     "220-INP-GRP01"     TO   S-NAME.
     MOVE     "GRP001"       TO   WK-GRP.
     PERFORM  900-DSP-READ.
*
     EVALUATE DSP-FNC
         WHEN PF05
              MOVE      4010      TO   PROGRAM-STATUS
              MOVE      99        TO   GR-NO
         WHEN ENT
              PERFORM   CLR-HEAD-RTN
              PERFORM   220-GRP01-CHECK-SEC
              IF        ERR-FLG   =    ZERO
                        MOVE      9    TO   GR-NO
              END-IF
         WHEN OTHER
              MOVE      1         TO   ERR-FLG
     END-EVALUATE.
*
 220-INP-GRP01-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      条件チェック　                              *
*--------------------------------------------------------------*
 220-GRP01-CHECK-SEC    SECTION.
     MOVE     "220-CRP01-CHECK-SEC"    TO   S-NAME.
     MOVE     ZERO      TO        ERR-FLG.
*
*    IF     ( JDATE     =    ZERO ) AND
*           ( JTIME     =    ZERO ) AND
*           ( TORICD    =    ZERO )
*             MOVE      2         TO   ERR-FLG
*             MOVE     "C"        TO   EDIT-CURSOR OF JDATE
*             MOVE     "R"        TO   EDIT-OPTION OF JDATE
*             MOVE     "R"        TO   EDIT-OPTION OF JTIME
*             MOVE     "R"        TO   EDIT-OPTION OF TORICD
*             GO   TO   220-GRP01-CHECK-EXIT
*    END-IF.
*    受信日付チェック
*    IF  JDATE     NOT =     ZERO
*        MOVE     "2"        TO        LINK-IN-KBN
*        MOVE      JDATE     TO        LINK-IN-YMD8
*        CALL     "SKYDTCKB" USING     LINK-IN-KBN
*                                      LINK-IN-YMD6
*                                      LINK-IN-YMD8
*                                      LINK-OUT-RET
*                                      LINK-OUT-YMD8
*        IF   LINK-OUT-RET   NOT =     ZERO
*             IF        ERR-FLG   =    ZERO
*                       MOVE      3    TO   ERR-FLG
*             END-IF
*             MOVE     "C"        TO   EDIT-CURSOR OF JDATE
*             MOVE     "R"        TO   EDIT-OPTION OF JDATE
*             GO   TO   220-GRP01-CHECK-EXIT
*        END-IF
*    END-IF.
*    受信時間
*    IF  JTIME     NOT NUMERIC
*        MOVE      ZERO      TO   JTIME
*    END-IF.
*    取引先チェック
*    IF  TORICD    NOT =     ZERO
*        MOVE      SPACE     TO   TOK-REC
*        INITIALIZE               TOK-REC
*        MOVE      TORICD    TO   TOK-F01
*        READ      HTOKMS
*            INVALID
*                  MOVE      SPACE     TO   TORINM
*                  IF   ERR-FLG   =    ZERO
*                       MOVE      4    TO   ERR-FLG
*                  END-IF
*                  MOVE     "C"   TO   EDIT-CURSOR OF TORICD
*                  MOVE     "R"   TO   EDIT-OPTION OF TORICD
*            NOT INVALID
*                  MOVE      TOK-F02   TO   TORINM
*        END-READ
*    END-IF.
*検品日範囲チェック
*    （開始検品日チェック）
*    IF       KENDTS   NOT NUMERIC
*    OR       KENDTS   =   ZERO
*             MOVE     ZERO  TO   KENDTS
*    END-IF.
*    （終了検品日チェック）
*    IF       KENDTE   NOT NUMERIC
*    OR       KENDTE   =   ZERO
*             MOVE  99999999 TO   KENDTE
*    END-IF.
*----開始検品日論理チェック
     MOVE     "2"            TO   LINK-IN-KBN.
     MOVE     ZERO           TO   LINK-IN-YMD6.
     MOVE     KENDTS         TO   LINK-IN-YMD8.
     MOVE     ZERO           TO   LINK-OUT-RET.
     MOVE     ZERO           TO   LINK-OUT-YMD8.
     CALL     "SKYDTCKB"  USING   LINK-IN-KBN
                                  LINK-IN-YMD6
                                  LINK-IN-YMD8
                                  LINK-OUT-RET
                                  LINK-OUT-YMD8.
     IF ( LINK-OUT-RET   = 9   ) OR
        ( KENDTS   NOT NUMERIC ) OR
        ( KENDTS   =   ZERO    )
          IF   ERR-FLG   =    ZERO
               MOVE      8    TO   ERR-FLG
          END-IF
          MOVE  "R"      TO   EDIT-OPTION  OF  KENDTS
          MOVE  "C"      TO   EDIT-CURSOR  OF  KENDTS
     END-IF.
*----終了検品日論理チェック
     MOVE     "2"            TO   LINK-IN-KBN.
     MOVE     ZERO           TO   LINK-IN-YMD6.
     MOVE     KENDTE         TO   LINK-IN-YMD8.
     MOVE     ZERO           TO   LINK-OUT-RET.
     MOVE     ZERO           TO   LINK-OUT-YMD8.
     CALL     "SKYDTCKB"  USING   LINK-IN-KBN
                                  LINK-IN-YMD6
                                  LINK-IN-YMD8
                                  LINK-OUT-RET
                                  LINK-OUT-YMD8.
     IF ( LINK-OUT-RET   = 9   ) OR
        ( KENDTE   NOT NUMERIC ) OR
        ( KENDTE   =   ZERO    )
          IF   ERR-FLG   =    ZERO
               MOVE      9    TO   ERR-FLG
          END-IF
          MOVE  "R"      TO   EDIT-OPTION  OF  KENDTE
          MOVE  "C"      TO   EDIT-CURSOR  OF  KENDTE
     END-IF.
*----大小論理チェック
     IF       KENDTS      >  KENDTE
              IF   ERR-FLG   =    ZERO
                   MOVE   7       TO   ERR-FLG
                   MOVE  "R"      TO   EDIT-OPTION  OF  KENDTS
                   MOVE  "R"      TO   EDIT-OPTION  OF  KENDTE
                   MOVE  "C"      TO   EDIT-CURSOR  OF  KENDTS
              END-IF
     END-IF.
*店舗コード範囲チェック
     IF       TENCDS  NOT NUMERIC
              MOVE      ZERO      TO   TENCDS
     END-IF.
     IF       TENCDE  NOT NUMERIC
              MOVE      999       TO   TENCDE
     END-IF.
*
     IF       TENCDS  >  TENCDE
              IF   ERR-FLG   =    ZERO
                   MOVE      7    TO   ERR-FLG
              END-IF
              MOVE     "C"   TO   EDIT-CURSOR OF TENCDS
              MOVE     "R"   TO   EDIT-OPTION OF TENCDS
              MOVE     "R"   TO   EDIT-OPTION OF TENCDE
     END-IF.
*伝票区分チェック
     IF     ( DENKU     NOT =     SPACE )
       AND  ( DENKU     NOT =     "01"  )
       AND  ( DENKU     NOT =     "02"  )
       AND  ( DENKU     NOT =     "03"  )
       AND  ( DENKU     NOT =     "04"  )
       AND  ( DENKU     NOT =     "05"  )
       AND  ( DENKU     NOT =     "07"  )
       AND  ( DENKU     NOT =     "08"  )
       AND  ( DENKU     NOT =     "09"  )
         IF   ERR-FLG   =    ZERO
              MOVE      6    TO   ERR-FLG
         END-IF
         MOVE     "C"   TO   EDIT-CURSOR OF DENKU
         MOVE     "R"   TO   EDIT-OPTION OF DENKU
     END-IF.
*
*受領累積データ存在チェック
     IF  ERR-FLG        =    ZERO
         MOVE      SPACE          TO   GDJ-REC
         INITIALIZE                    GDJ-REC
         MOVE      KENDTS         TO   GDJ-F04
         MOVE      TENCDS         TO   GDJ-F05
         MOVE      DENKU          TO   GDJ-F06
         MOVE      ZERO           TO   GDJ-F07
         MOVE      ZERO           TO   GDJ-F08
         START     GDJYURL2   KEY  >=  GDJ-F04   GDJ-F05
                                       GDJ-F06   GDJ-F07
                                       GDJ-F08
              INVALID   KEY
                   MOVE      5    TO   ERR-FLG
                   MOVE     "C"   TO   EDIT-CURSOR OF KENDTS
                   MOVE     "R"   TO   EDIT-OPTION OF KENDTS
                   MOVE     "R"   TO   EDIT-OPTION OF KENDTE
                   MOVE     "R"   TO   EDIT-OPTION OF TENCDS
                   MOVE     "R"   TO   EDIT-OPTION OF TENCDE
*                  MOVE     "R"   TO   EDIT-OPTION OF DENKU
                   IF   DENKU     NOT =     SPACE
                        MOVE     "R"   TO   EDIT-OPTION OF DENKU
                   END-IF
                   GO   TO   220-GRP01-CHECK-EXIT
              NOT INVALID
                   READ      GDJYURL2
                     AT END
                        MOVE      5    TO   ERR-FLG
                        MOVE     "C"   TO   EDIT-CURSOR OF KENDTS
                        MOVE     "R"   TO   EDIT-OPTION OF KENDTS
                        MOVE     "R"   TO   EDIT-OPTION OF KENDTE
                        MOVE     "R"   TO   EDIT-OPTION OF TENCDS
                        MOVE     "R"   TO   EDIT-OPTION OF TENCDE
                        IF   DENKU     NOT =     SPACE
                             MOVE  "R" TO   EDIT-OPTION OF DENKU
                        END-IF
                        GO   TO   220-GRP01-CHECK-EXIT
                     NOT AT END
                        PERFORM   220-DATA-CHECK-SEC
*-----------------------IF ( JDATE     =    GDJ-F01 ) AND
*                          ( JTIME     =    GDJ-F02 ) AND
*                          ( TORICD    =    GDJ-F03 )
*                            IF ( WK-SOKCD  NOT =    SPACE )
*                                 IF   DENKU     =     GDJ-F04
*                                      CONTINUE
*                                 ELSE
*                                      MOVE  5   TO   ERR-FLG
*                                      MOVE "C"  TO
*                                           EDIT-CURSOR OF JDATE
*                                      MOVE "R"  TO
*                                           EDIT-OPTION OF JDATE
*                                           EDIT-OPTION OF JTIME
*                                           EDIT-OPTION OF TORICD
*                                           EDIT-OPTION OF DENKU
*                                           EDIT-OPTION OF TENCDS
*                                           EDIT-OPTION OF TENCDE
*                                           EDIT-OPTION OF KENDTS
*                                           EDIT-OPTION OF KENDTE
*                                 END-IF
*                            ELSE
*                                 CONTINUE
*                            END-IF
*                       ELSE
*                            MOVE      5    TO   ERR-FLG
*                            MOVE "C"  TO   EDIT-CURSOR OF JDATE
*                            MOVE "R"  TO   EDIT-OPTION OF JDATE
*                            MOVE "R"  TO   EDIT-OPTION OF JTIME
*                            MOVE "R"  TO   EDIT-OPTION OF TORICD
*                            MOVE "R"  TO   EDIT-OPTION OF TENCDS
*                            MOVE "R"  TO   EDIT-OPTION OF TENCDE
*                            MOVE "R"  TO   EDIT-OPTION OF KENDTS
*                            MOVE "R"  TO   EDIT-OPTION OF KENDTE
*                            IF    WK-SOKCD NOT =    SPACE
*                                  MOVE    "R"   TO
*                                           EDIT-OPTION OF DENKU
*                            END-IF
*                            GO   TO   220-GRP01-CHECK-EXIT
*-----------------------END-IF
                   END-READ
         END-START
     END-IF.
*
 220-GRP01-CHECK-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      対象データ存在チェック  2014.06.17          *
*--------------------------------------------------------------*
 220-DATA-CHECK-SEC    SECTION.
*
     MOVE     "220-DATA-CHECK-SEC "    TO   S-NAME.
*
*検品日範囲内チェック
     IF ( KENDTS <= GDJ-F04 ) AND ( GDJ-F04 <= KENDTE  )
          CONTINUE
     ELSE
          MOVE      5    TO   ERR-FLG
          MOVE     "C"   TO   EDIT-CURSOR OF KENDTS
          MOVE     "R"   TO   EDIT-OPTION OF KENDTS
          MOVE     "R"   TO   EDIT-OPTION OF KENDTE
          MOVE     "R"   TO   EDIT-OPTION OF TENCDS
          MOVE     "R"   TO   EDIT-OPTION OF TENCDE
          IF    DENKU    NOT =    SPACE
                MOVE    "R"   TO  EDIT-OPTION OF DENKU
          END-IF
          GO   TO   220-DATA-CHECK-EXIT
     END-IF.
*店舗範囲内チェック
     IF ( TENCDS <= GDJ-F05 ) AND ( GDJ-F05 <= TENCDE  )
          CONTINUE
     ELSE
          MOVE      5    TO   ERR-FLG
          MOVE     "C"   TO   EDIT-CURSOR OF KENDTS
          MOVE     "R"   TO   EDIT-OPTION OF KENDTS
          MOVE     "R"   TO   EDIT-OPTION OF KENDTE
          MOVE     "R"   TO   EDIT-OPTION OF TENCDS
          MOVE     "R"   TO   EDIT-OPTION OF TENCDE
          IF    DENKU    NOT =    SPACE
                MOVE    "R"   TO  EDIT-OPTION OF DENKU
          END-IF
          GO   TO   220-DATA-CHECK-EXIT
     END-IF.
*伝票区分チェック
     IF ( DENKU = SPACE ) OR ( DENKU = GDJ-F06 )
          GO   TO   220-DATA-CHECK-EXIT
     END-IF.
*不一致時→次レコード再チェック
     READ GDJYURL2
       AT END
*↓TEST
*         DISPLAY "NOUDATE NOTHING " UPON CONS
*↑TEST
          MOVE  5   TO   ERR-FLG
          MOVE "C"  TO
          EDIT-CURSOR OF KENDTS
          MOVE "R"  TO
          EDIT-OPTION OF KENDTS
          EDIT-OPTION OF KENDTE
          EDIT-OPTION OF TENCDS
          EDIT-OPTION OF TENCDE
          EDIT-OPTION OF DENKU
          GO   TO   220-DATA-CHECK-EXIT
     END-READ.
*
     GO   TO   220-DATA-CHECK-SEC.
*
 220-DATA-CHECK-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      確認入力　　　                              *
*--------------------------------------------------------------*
 230-INP-KKNN           SECTION.
     MOVE     "230-INP-KKNN"      TO   S-NAME.
     MOVE     "KKNN"         TO   WK-GRP.
     PERFORM  900-DSP-READ.
*
     EVALUATE DSP-FNC
         WHEN PF05
              MOVE      4010      TO   PROGRAM-STATUS
              MOVE      99        TO   GR-NO
         WHEN PF04
              MOVE      0         TO   GR-NO
         WHEN PF06
              MOVE      1         TO   GR-NO
         WHEN ENT
              PERFORM   CLR-TAIL-RTN
              MOVE      10        TO   GR-NO
         WHEN OTHER
              MOVE      1         TO   ERR-FLG
     END-EVALUATE.
*
 230-INP-KKNN-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      パラメタセット                              *
*--------------------------------------------------------------*
 240-PARA-SEC           SECTION.
     MOVE     "240-PARA-SEC"      TO   S-NAME.
*
     MOVE     KENDTS              TO   PARA-KENDTS.
     MOVE     KENDTE              TO   PARA-KENDTE.
     MOVE     TENCDS              TO   PARA-TENCDS.
     MOVE     TENCDE              TO   PARA-TENCDE.
     MOVE     DENKU               TO   PARA-DENKU.
*    IF       WK-SOKCD  =      SPACE
*             MOVE      SPACE     TO   PARA-DENKU
*    END-IF.
*
     MOVE     99                  TO   GR-NO.
*
 240-PARA-SEC-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL ALL     ﾃﾞｨｽﾌﾟﾚｰ  READ                              *
*--------------------------------------------------------------*
 900-DSP-READ           SECTION.
     MOVE     "900-DSP-READ"      TO   S-NAME.
     MOVE     "SCREEN"       TO   DSP-GRP.
     IF       ERR-FLG   =    0
              MOVE      SPACE               TO   MSG
              MOVE      "D"      TO   EDIT-OPTION OF MSG
     ELSE
              MOVE      MSG-TBL (ERR-FLG)   TO   MSG
     END-IF.
     IF       GR-NO     =    1
              MOVE      GUIDE01   TO   GUIDE
     ELSE
              MOVE      GUIDE02   TO   GUIDE
     END-IF.
     MOVE     WK-SYSYMD           TO   SYSYMD.
     ACCEPT   SYS-TIME2      FROM TIME.
     MOVE     SYS-TIMEW           TO   SYSTIM.
*
     PERFORM  900-DSP-WRITE.
*
     IF       MSG  NOT  =    SPACE
              MOVE "AL"      TO   DSP-PRO
     ELSE
              MOVE "NE"      TO   DSP-PRO
     END-IF.
     MOVE     SPACE          TO   MSG.
*
     MOVE     WK-GRP         TO   DSP-GRP.
     READ     DSPFILE.
     MOVE     SPACE          TO   DSP-PRO.
 900-DSP-READ-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL ALL     ﾃﾞｨｽﾌﾟﾚｰ  WRITE                             *
*--------------------------------------------------------------*
 900-DSP-WRITE          SECTION.
     MOVE     "900-DSP-WRITE"     TO   S-NAME.
     MOVE     SPACE          TO   DSP-PRO.
     WRITE    FSY57701.
 900-DSP-WRITE-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      ＨＥＡＤ　属性クリア　　　　　　　　　　　　*
*--------------------------------------------------------------*
 CLR-HEAD-RTN           SECTION.
     MOVE     " "            TO   EDIT-CURSOR OF KENDTS
                                  EDIT-CURSOR OF KENDTE
                                  EDIT-CURSOR OF TENCDS
                                  EDIT-CURSOR OF TENCDE
                                  EDIT-CURSOR OF DENKU.
     MOVE     "M"            TO   EDIT-OPTION OF KENDTS
                                  EDIT-OPTION OF KENDTE
                                  EDIT-OPTION OF TENCDS
                                  EDIT-OPTION OF TENCDE
                                  EDIT-OPTION OF DENKU.
 CLR-HEAD-EXIT.
     EXIT.
*--------------------------------------------------------------*
*    LEVEL  3      ＴＡＩＬ　属性クリア　　　　　　　　　　　　*
*--------------------------------------------------------------*
 CLR-TAIL-RTN           SECTION.
     MOVE     " "        TO   EDIT-CURSOR OF KKNN.
     MOVE     "M"        TO   EDIT-OPTION OF KKNN.
 CLR-TAIL-EXIT.
     EXIT.
*-----------------<< PROGRAM END >>----------------------------*

```
