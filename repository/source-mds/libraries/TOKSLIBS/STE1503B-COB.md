...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/STE1503B.COB`
...
***  ｺﾞｳｹｲ ｴﾘｱ
 01  GOUKEI.
     ...
     03  G-SURYO                  PIC  9(05)     VALUE   ZERO.
***  ｹｲｻﾝ ｴﾘｱ
 01  KEISAN.
...
*
 01  HEAD-02                      CHARACTER  TYPE  IS  YA.
     ...
     03  HD02-4                   PIC  X(02).
     03  FILLER                   PIC  X(01)     VALUE   SPACE.
     03  HD02-5                   PIC  ZZZZZZ9.
     03  FILLER                   PIC  X(01)     VALUE   SPACE.
     03  HD02-6                   PIC  ZZZZZ9.
...
*    ｺﾞｳｹｲ
 01  TAIL-01.
     ...
     03  TL1-2                    PIC  ZZZZZZZZ9.
     03  FILLER                   PIC  X(06)     VALUE   SPACE.
     03  TL1-3                    PIC  ZZZZZZZZ9.
 01  TAIL-02.
     03  FILLER                   PIC  X(05)     VALUE   SPACE.
     03  TL2-1                    PIC  X(15).
******************************************************************
...
*****END-IF.
*
     CLOSE    HTOKMS   DENJNL1   HTENMS   PRTF.
 END-EXIT.
     EXIT.
**********************************************************
*                 見出しデータ編集書き出し               *
...
*
*納品日
     MOVE        DEN-F112  TO        HD02-8.
*店舗コード
     MOVE        DEN-F07   TO        HD02-2.
*分類コード
     MOVE        DEN-F12   TO        HD02-3.
*伝票区分
     MOVE        DEN-F132  TO        HD02-4.
*伝票番号
     MOVE        DEN-F02   TO        HD02-5.
*取引先コード
     MOVE        DEN-F01   TO        HD02-6.
*社名
     MOVE        DEN-F01   TO        TOK-F01.
     READ  HTOKMS  INVALID
           MOVE   ALL "*"  TO        HD01-1
           NOT INVALID
           MOVE   TOK-F04  TO        HD01-1
     END-READ.
*取引先名
     MOVE   NC"（株）サカタのタネ"   TO   HD02-7.
*店舗名（カナ）
     MOVE        DEN-F01   TO        TEN-F52.
     MOVE        DEN-F07   TO        TEN-F011.
     READ  HTENMS  INVALID
           MOVE   DEN-F30  TO        HD02-1
           NOT INVALID
           MOVE   TEN-F04  TO        HD02-1
     END-READ.
*ヘッダ印字
     WRITE  PRT-REC  FROM  HEAD-01  AFTER  6.
...
*数量合計
     MOVE        G-SURYO   TO        TL1-1.
*原価金額合計
     MOVE        G-GENKA   TO        TL1-2.
*売価金額合計
     MOVE        G-BAIKA   TO        TL1-3.
*合計行印字調整
     COMPUTE     CNT-AFTER   =   25   -   L-CNT.
*合計行出力
     WRITE  PRT-REC  FROM  TAIL-01  AFTER  CNT-AFTER.
*備考１
     MOVE        WK-BIKO1  TO        TL2-1.
     WRITE  PRT-REC  FROM  TAIL-02  AFTER  3
*備考２
     MOVE        WK-BIKO2  TO        TL2-1.
     WRITE  PRT-REC  FROM  TAIL-02  AFTER  1
     MOVE        SPACE     TO        PRT-REC.
...
*テスト印字文字セット
     MOVE   ALL "9"              TO   HD01-1  HD02-1 HD02-2
                                      HD02-5  HD02-6 HD02-8.
     MOVE   ALL "9"              TO   BD2-3   BD2-4   BD2-51
                                      BD2-52  BD2-61  BD2-62
                                      BD2-7   BD2-8   BD2-9
                                      BD2-8   BD2-9.
     MOVE   ALL "9"              TO   TL1-1   TL1-2   TL1-3.
     MOVE   ALL "*"              TO   BD1-1   BD2-1   BD2-2
                                      HD01-1  HD02-1   HD02-3
                                      HD02-4  TL2-1.
     MOVE   NC"（株）サカタのタネ" TO HD02-7.
*ヘッダ印字
     WRITE  PRT-REC  FROM  HEAD-01  AFTER  6.
     ...
     WRITE  PRT-REC  AFTER 3.
*明細印字
     PERFORM VARYING L-CNT FROM 1 BY 1 UNTIL L-CNT > 6
             WRITE  PRT-REC  FROM  BODY-01  AFTER  1
             WRITE  PRT-REC  FROM  BODY-02  AFTER  1
     END-PERFORM.
*テイル印字
...
*****WRITE  PRT-REC  AFTER PAGE.
*
     INITIALIZE      HEAD-01  HEAD-02  BODY-01
                     BODY-02  TAIL-01  TAIL-02.
     MOVE   ZERO           TO       L-CNT.
*
 TEST-PRT-EXIT.
     EXIT.
******************************************************************
*END PROGRAM  STE1503B.
******************************************************************
