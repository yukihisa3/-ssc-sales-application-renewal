...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY5065V.COB`
...
*----------------------------------------------------------------*
 WORKING-STORAGE        SECTION.
*明細情報格納領域
     COPY   TOSMEISF OF XFDLIB  JOINING   MEI  AS   PREFIX.
*テイル情報格納領域
     COPY   TOSTILSF OF XFDLIB  JOINING   TIL  AS   PREFIX.
*ＣＳＶ情報格納領域
     COPY   TOSCSVSF OF XFDLIB  JOINING   CSV  AS   PREFIX.
**** エンドフラグ
...
***** メッセージエリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSY5065V".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSY5065V".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND1.
...
*             プロセス                      0.0                *
****************************************************************
 SSI5001V-START         SECTION.
*
     MOVE   "SSI5001V-START"      TO   S-NAME.
     PERFORM            INIT-SEC.
...
*
     ...
     EVALUATE  MEI-F07
             WHEN  "21"
                   COMPUTE WK-SEIKIN = WK-SEIKIN + MEI-F09
             WHEN  "22"
                   COMPUTE WK-SEIKIN = WK-SEIKIN - MEI-F09
             WHEN  OTHER
                   COMPUTE WK-SEIKIN = WK-SEIKIN + MEI-F09
...
*****ADD      MEI-F15             TO   WK-SIHKIN.
     EVALUATE  MEI-F07
             WHEN  "21"
                   COMPUTE WK-SIHKIN = WK-SIHKIN + MEI-F15
             WHEN  "22"
                   COMPUTE WK-SIHKIN = WK-SIHKIN - MEI-F15
             WHEN  OTHER
                   COMPUTE WK-SIHKIN = WK-SIHKIN + MEI-F15
