...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSE8705B.COB`
...
*  20180309  高橋　　　ケーヨー対応（ＤＣＭ統合、手書伝票制御）*
****************************************************************
...
*$$2013/12/13 NAV ST 消費税増税対応
 01  WK-SYOHIZEI1          PIC S9(10)V9(02)   VALUE  ZERO.
 01  WK-SYOHIZEI2          PIC S9(10)V9(02)   VALUE  ZERO.
*$$2013/12/13 NAV ED 消費税増税対応
...
*----<< 消費税率設定 >>--*
 01  WK-SYOUHIZEI.
     03  WK-ZEIRITU1    PIC  9(01)V9(02) VALUE  ZERO.
     03  WK-ZEIRITU2    PIC  9(01)V9(02) VALUE  ZERO.
     03  WK-ZEIKAITEI   PIC  9(08)  VALUE  ZERO.
     03  WK-ZEIRITU3    PIC  9(01)V9(02) VALUE  ZERO.
     03  WK-ZEIRITU4    PIC  9(01)V9(02) VALUE  ZERO.
 01  WK-GOKEI1          PIC S9(10)  VALUE  ZERO.
 01  WK-GOKEI2          PIC S9(10)  VALUE  ZERO.
*$$2013/12/13 NAV ED 消費税増税対応
*日付変換サブルーチン用ワーク
...
*----<< ワークファイル >>--*
 SETGK87-ERR            SECTION.
     ...
     DISPLAY  "### SSE8705B SETGK87 ERROR " SEI-ST  " "
...
*----<< 店舗マスタ >>--*
 HTENMS-ERR             SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSE8705B HTENMS ERROR " TEN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
*----<< 条件ファイル >>--*
 HJYOKEN-ERR            SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSE8705B HJYOKEN ERROR " JYO-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
*----<< 配信データ >>--*
 SNDSEIKY-ERR           SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSE8705B SNDSEIKY  ERROR " SND-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
*----<< 件数データ >>--*
 HACKENDT-ERR           SECTION.
     ...
     ACCEPT   SYS-TIME       FROM TIME.
     MOVE     4000           TO   PROGRAM-STATUS.
     DISPLAY  "### SSE8705B HACKENDT  ERROR " KEN-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
*----<< 売上データ >>--*  DEL  2011/08/02
*SHTDENF-ERR            SECTION.
***##USE AFTER     EXCEPTION PROCEDURE      SHTDENF.
***##ACCEPT   SYS-DATE       FROM DATE.
***##ACCEPT   SYS-TIME       FROM TIME.
***##MOVE     4000           TO   PROGRAM-STATUS.
***##DISPLAY  "### SSE8705B SHTDENL1  ERROR " DEN-ST " "
***##         SYS-YY "." SYS-MM "." SYS-DD " "
***##         SYS-HH ":" SYS-MN ":" SYS-SS " ###"
***##                                  UPON CONS.
***##STOP     RUN.
...
*----<< ﾜｰｸ ｼｮｷｾｯﾄ >>-*
     ...
     INITIALIZE         WK-AREA.
*$$2013/12/13 NAV ST 請求金額合計／消費税率　初期化
     MOVE     ZERO      TO  WK-GOKEI1  WK-GOKEI2.
     INITIALIZE         WK-SYOUHIZEI.
*$$2013/12/13 NAV ED 請求金額合計／消費税率　初期化
*
     OPEN  INPUT  HJYOKEN.
*条件Ｆ（消費税率）
     MOVE     "99"           TO   JYO-F01.
...
*$$2013/12/13 NAV ST 消費税増税対応
              ...
              DISPLAY "WK-ZEIRITU1  = " WK-ZEIRITU1  UPON CONS
              DISPLAY "WK-ZEIRITU2  = " WK-ZEIRITU2  UPON CONS
              DISPLAY "WK-ZEIKAITEI = " WK-ZEIKAITEI UPON CONS
              DISPLAY "WK-ZEIRITU3  = " WK-ZEIRITU3  UPON CONS
              DISPLAY "WK-ZEIRITU4  = " WK-ZEIRITU4  UPON CONS
*$$2013/12/13 NAV ED 消費税増税対応
     END-IF.
*条件Ｆ（締日）
     MOVE     "59"           TO   JYO-F01.
...
*********WHEN  100404  WHEN 100441  WHEN  100442
         WHEN  100404  WHEN 100441  WHEN  100442  WHEN  139381
*%%2018/02/22 NAV ST
         WHEN  1731    WHEN 1732
*%%2018/02/22 NAV ED
*#2017/03/09 NAV ED
...
*****MOVE  "(ｶﾌﾞ)ｻｶﾀﾉﾀﾈ ﾎｰﾑｶﾞｰﾃﾞﾝﾌﾞ"                   TO HED-F15.
     MOVE  "(ｶﾌﾞ)ｻｶﾀﾉﾀﾈ ｴｲｷﾞｮｳﾀﾞｲ2ﾌﾞ"                  TO HED-F15.
*#2018/05/29 NAV ED
*### EVALUATE  SEI-F01
*#       WHEN  883  WHEN  14273
...
*****MOVE  NC"_サカタのタネ　ホームガーデン部" TO HED-F16.
     MOVE  NC"_サカタのタネ　営業第２部　　　" TO HED-F16.
*#2018/05/29 NAV ED
*### EVALUATE  SEI-F01
*#       WHEN  883  WHEN  14273
...
*#2018/03/09 NAV ST ５０番伝票の場合、５０を００に変更
     IF    SEI-F01  =  1731 OR 172 OR 7601 OR 7602
           IF  MEI-F02(1:2)  =  "50"
               MOVE  "00"      TO     MEI-F02(1:2)
           END-IF
     END-IF.
*#2018/03/09 NAV ED
*### 2010/02/02 ダイキも追加する。
...
***********DISPLAY "WK-DENNOX = " WK-DENNOX UPON CONS
           PERFORM   VARYING  I  FROM 1  BY   1  UNTIL CHK-FLG = 1
                ...
                END-IF
           END-PERFORM
***********DISPLAY "KETA-CNT = " KETA-CNT UPON CONS
     ...
     END-IF.
*$$2013/12/13 NAV ST 請求金額の集計位置を変更
*$$2013/12/13 NAV ED 請求金額の集計位置を変更
*納品事業会社コード
     EVALUATE  SEI-F01
...
*#2017/03/09 NAV ST
         WHEN  139381 WHEN    171371 MOVE  01   TO  MEI-F033
*#2017/03/09 NAV ED
...
*%%2018/02/22 NAV ST
         WHEN  1731   WHEN    1732   MOVE  13   TO  MEI-F033
         WHEN  7601   WHEN    7602   MOVE  13   TO  MEI-F033
*%%2018/02/22 NAV ED
     END-EVALUATE.
*個別取引先コード
     EVALUATE  SEI-F01
...
*#2017/03/09 NAV ST
         WHEN  139381 MOVE  013938 TO    MEI-F034
         WHEN  171371 MOVE  017137 TO    MEI-F034
*#2017/03/09 NAV ED
...
*%%2018/02/22 NAV ST
         WHEN  1731   MOVE  000173 TO    MEI-F034
         WHEN  1732   MOVE  000173 TO    MEI-F034
         WHEN  7601   MOVE  000760 TO    MEI-F034
         WHEN  7602   MOVE  000760 TO    MEI-F034
*%%2018/02/22 NAV ED
     END-EVALUATE.
*    発注日
...
*$$2013/12/13 NAV ST 請求金額の集計方法を変更
*    請求額
     IF    SEI-F06  <  ZERO
           MOVE   "-"      TO     MEI-F031
     ELSE
...
*$$ ↓以下処理を追加
     IF  WK-ZEIKAITEI  NOT =  ZERO
           IF  LINK-OUT-YMD  >=  WK-ZEIKAITEI
*$$************改定後消費税対象
               ADD  SEI-F06      TO     WK-GOKEI2
           ELSE
*$$************改定前消費税対象
               ADD  SEI-F06      TO     WK-GOKEI1
           END-IF
     ELSE
*$$************改定前消費税対象
           ADD  SEI-F06          TO     WK-GOKEI1
     END-IF.
*$$ ↑追加
*$$2013/12/13 NAV ED 請求金額の集計方法を変更
*    請求先事業部コード
     EVALUATE  SEI-F01
...
*    消費税額計算
*$$2013/12/13 NAV ST 消費税増税対応　計算方法を変更
*****COMPUTE  WK-SYOHIZEI  =  WK-GOKEI  *  ( WK-ZEI  -  1).
     DISPLAY "OLD SEIKYU GK = " WK-GOKEI1     UPON CONS.
     DISPLAY "NEW SEIKYU GK = " WK-GOKEI2     UPON CONS.
     IF    WK-GOKEI1  NOT =  ZERO
           COMPUTE WK-SYOHIZEI1 = WK-GOKEI1 * WK-ZEIRITU2
     END-IF.
     IF    WK-GOKEI2  NOT =  ZERO
           COMPUTE WK-SYOHIZEI2 = WK-GOKEI2 * WK-ZEIRITU4
     END-IF.
     DISPLAY "OLD SYOHIZEI  = " WK-SYOHIZEI1  UPON CONS.
     DISPLAY "NEW SYOHIZEI  = " WK-SYOHIZEI2  UPON CONS.
     COMPUTE WK-SYOHIZEI = WK-SYOHIZEI1 + WK-SYOHIZEI2.
     DISPLAY "TOTAL SYOHZEI = " WK-SYOHIZEI   UPON CONS.
*$$2013/12/13 NAV ED 消費税増税対応　計算方法を変更
     IF    WK-SYOHIZEI  <  ZERO
           MOVE   "-"      TO     TAL-F041
     ELSE
           MOVE   "0"      TO     TAL-F041
