...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/NJH5501B.COB`
...
*    ｶｳﾝﾄ
 ...
 01  WK-TOKCD1               PIC  9(06)     VALUE  ZERO.
 01  WK-TOKCD-CHK1           PIC  X(01)     VALUE  SPACE.
 01  WK-TOKCD2               PIC  9(06)     VALUE  ZERO.
...
*ヘッドレコード退避ワーク
 01  WK-DEPB-REC.
     ...
     03  WK-DEPB041         PIC  9(06).
     03  WK-DEPB042         PIC  9(02).
     03  WK-DEPB043         PIC  X(01).
     03  WK-DEPB05          PIC  9(04).
     03  WK-DEPB06          PIC  9(02).
     03  WK-DEPB07          PIC  9(06).
     03  WK-DEPB08          PIC  9(06).
     03  WK-DEPB091         PIC  9(06).
     03  WK-DEPB092         PIC  9(02).
     03  WK-DEPB10          PIC  X(12).
     03  WK-DEPB11          PIC  X(08).
     03  WK-DEPB12          PIC  X(15).
     03  WK-DEPB13          PIC  X(05).
...
*店舗ＣＤ変換（文字⇒数値）
 ...
 01  WK-TENPO-R            REDEFINES  WK-TENPO.
     03  WK-HEN-TENPO      PIC  9(05).
*数量
 01  WK-SURYO              PIC  9(06).
 01  WK-SURYO-R            REDEFINES  WK-SURYO.
     03  WK-HEN-SURYO      PIC  9(05)V9.
*原価金額
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "NJH5501B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NJH5501B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*    企業コード（発注者）
     MOVE     WK-DEPB041    TO        HEN-A041.
*    ステーションアドレス（発注者）
     MOVE     WK-DEPB042    TO        HEN-A042.
*    予備
     MOVE     WK-DEPB043    TO        HEN-A043.
*    分類コード
...
*    企業コード（受注者）
     MOVE     WK-DEPB091    TO        HEN-A091.
*    ステーションアドレス（受注者）
     MOVE     WK-DEPB092    TO        HEN-A092.
*    社名
...
*伝票明細転送
*    レコード区分
     MOVE     WK-DEPD01     TO        HEN-A17.
*    データ種別
     MOVE     WK-DEPD02     TO        HEN-A18.
*    行番号
     MOVE     WK-DEPD03     TO        HEN-A19.
*    商品コード
     MOVE     WK-DEPD04     TO        HEN-A20.
*    入数
     MOVE     WK-DEPD05     TO        HEN-A21.
*    発注単位数
     MOVE     WK-DEPD06     TO        HEN-A22.
*    単位
     MOVE     WK-DEPD07     TO        HEN-A23.
*    数量
     MOVE     WK-DEPD08     TO        WK-SURYO.
     MOVE     WK-HEN-SURYO  TO        HEN-A24.
*    原価単価
     MOVE     WK-DEPD09     TO        WK-GENKA.
     MOVE     WK-HEN-GENKA  TO        HEN-A25.
*    売価単価
     MOVE     WK-DEPD10     TO        HEN-A26.
*    原価金額
     MOVE     WK-DEPD11     TO        HEN-A27.
*    売価金額
     MOVE     WK-DEPD12     TO        HEN-A28.
*    受注者商品コード
     MOVE     WK-DEPD13     TO        HEN-A29.
*    商品名
     MOVE     WK-DEPD14     TO        HEN-A30.
*    添付
     MOVE     WK-DEPD15     TO        HEN-A31.
*    予備
     MOVE     WK-DEPD16     TO        HEN-A32.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
*取引先コード
**** MOVE    HEN-A091      TO        HEN-F01.
     MOVE    2205          TO        WK-TOKCD.
     EVALUATE  HEN-A041
         WHEN  602408
               MOVE  22051 TO        HEN-F01 WK-TOKCD2
               MOVE  "1"   TO        WK-TOKCD-CHK2
         WHEN  OTHER
               MOVE HEN-A091 TO      HEN-F01 WK-TOKCD1
...
*担当者コード
     MOVE        99        TO        HEN-F06.
*店コード
*****DISPLAY "HEN-A11 = " HEN-A11(1:5) UPON CONS.
     MOVE    HEN-A11(1:5)  TO        WK-TENPO.
     MOVE    WK-HEN-TENPO  TO        HEN-F07.
*  商品変換テーブル検索
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名
         MOVE    HEN-A30(1:15)  TO   HEN-F1421(1:15)
         MOVE    HEN-A30(16:10) TO   HEN-F1422(1:10)
       NOT INVALID
         MOVE    HEN-A30(1:15)  TO   HEN-F1421(1:15)
         MOVE    HEN-A30(16:10) TO   HEN-F1422(1:10)
     END-READ.
*数量
     MOVE        HEN-A24   TO        HEN-F15.
*単
     MOVE       "1"        TO        HEN-F16.
*原価単価
     MOVE        HEN-A25   TO        HEN-F172.
*売価単価
     MOVE        HEN-A26   TO        HEN-F173.
*原価金額
     MOVE        HEN-A27   TO        HEN-F181.
*売価金額
     MOVE        HEN-A28   TO        HEN-F182.
*店舗名（備考）
...
*訂正前数量
     MOVE        HEN-A24   TO        HEN-F50.
*修正原価単価
     MOVE        HEN-A25   TO        HEN-F512.
*修正売価単価
     MOVE        HEN-A26   TO        HEN-F513.
*修正原価金額
     MOVE        HEN-A27   TO        HEN-F521.
*修正売価金額
     MOVE        HEN-A28   TO        HEN-F522.
*
 TENSO-EXIT.
     EXIT.
****************************************************************
...
*****MOVE    WK-TOKCD      TO        KEN-F03.
     EVALUATE  HEN-A041
         WHEN  602408
               MOVE  22051 TO        KEN-F03
*
         WHEN  OTHER
               MOVE  2205    TO      KEN-F03
*
     END-EVALUATE.
     IF      INV-RUT       =         ZERO
...
****************************************************************
 VLD500-OUTPUT-SEC       SECTION.
     IF  WK-TOKCD-CHK1  =  "1"
       ...
       DISPLAY "WK-TOKCD   = " WK-TOKCD1  UPON CONS
       WRITE     VLD-REC
     END-IF.
     IF  WK-TOKCD-CHK2  =  "1"
       MOVE   "VLD500-OUTPUT-SEC" TO   S-NAME
       ...
       DISPLAY "PARA-JDATE = " PARA-JDATE UPON CONS
       DISPLAY "PARA-JTIME = " PARA-JTIME UPON CONS
       DISPLAY "WK-TOKCD   = " WK-TOKCD2  UPON CONS
       WRITE     VLD-REC
     END-IF.
 VLD500-OUTPUT-EXIT.
     EXIT.
