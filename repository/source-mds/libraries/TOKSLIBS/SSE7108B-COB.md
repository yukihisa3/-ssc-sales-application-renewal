...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSE7108B.COB`
...
*
 ...
 FILE-CONTROL.
*----<< 仕入実績集計データ >>--*
     SELECT   JDSYUKF   ASSIGN         DA-01-VI-JDSYUKL3
                        ORGANIZATION   INDEXED
                        ACCESS    MODE SEQUENTIAL
                        RECORD    KEY  SYU-F99   SYU-F01
                                       SYU-F04   SYU-F02
                                       SYU-F03
                        STATUS         SYU-ST.
*----<< 条件ファイル >>--*
...
*----<< 伝票データ >>--*
 JDSYUKF-ERR            SECTION.
     ...
     DISPLAY  "### SSE7108B JDSYUKF ERROR " SYU-ST " "
...
*----<< 条件ファイル >>--*
 HJYOKEN-ERR            SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSE7108B HJYOKEN ERROR " JYO-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
*----<< 仕入実績請求Ｆ >>--*
 JDJSEKF-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### OSKT320 JDJSEKF ERROR " SEI-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     STOP     RUN.
     ...
     IF       INV-FLG   =    0
              ...
              IF   SIME-DD   =    31
                   CALL "OSKTLSTD"     USING     SSKTLSTD-DATE
                                                 SSKTLSTD-RET
              END-IF
              DISPLAY "SSKTLSTD-DATE = " SSKTLSTD-DATE UPON CONS
     ELSE
              DISPLAY "JYOKENF-INV ERR" UPON CONS
              MOVE      1         TO   END-FLG
     END-IF.
 100-INIT-RTN-EXIT.
     EXIT.
...
*--------------------------------------------------------------*
 900-JYO-READ           SECTION.
     ...
     READ     HJYOKEN   INVALID
              MOVE      1         TO   INV-FLG
     DISPLAY  "### SSE7108B HJYOKEN INVALID KEY=" JYO-F01 " ###"
                                       UPON CONS
     END-READ.
 900-JYO-READ-EXIT.
     EXIT.
...
*--------------------------------------------------------------*
 900-SYU-START-READ     SECTION.
     START    JDSYUKF   KEY  >=   SYU-F99   SYU-F01   SYU-F04
                                  SYU-F02   SYU-F03
              INVALID   KEY
                        MOVE      1         TO   END-FLG
