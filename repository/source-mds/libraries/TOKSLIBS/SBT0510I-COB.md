...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBT0510I.COB`
...
*---<<  カインズＤＰＴマスタ  >>---*
     SELECT   CNZDPTL1  ASSIGN    TO        DA-01-VI-CNZDPTL1
                        ...
                        RECORD    KEY       IS   DPT-F01
                        FILE      STATUS    IS   DPT-STATUS.
*---<<  カインズカテゴリーマスタ  >>---*
     SELECT   CNZCTEL1  ASSIGN    TO        DA-01-VI-CNZCTEL1
                        ORGANIZATION        IS   INDEXED
                        ACCESS    MODE      IS   DYNAMIC
                        RECORD    KEY       IS   CTE-F01
                        FILE      STATUS    IS   CTE-STATUS.
*---------------------------------------------------------------
 DATA                   DIVISION.
 FILE                   SECTION.
*---<<  画面ファイル  >>---*
 FD  DSPF.
     COPY     FBT05101  OF        XMDLIB
              JOINING   DSP       PREFIX.
*---<<  カインズＤＰＴマスタ  >>---*
 FD  CNZDPTL1.
...
*    02  TOK-STATUS          PIC  X(02).
     02  DPT-STATUS          PIC  X(02).
     02  CTE-STATUS          PIC  X(02).
****  フラグ  ***
 01  PSW-AREA.
...
****  メッセージ情報  ***
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SBT0510I".
...
**
 FILEERR-SEC2                SECTION.
     USE AFTER       EXCEPTION
                     PROCEDURE    CNZDPTL1.
     MOVE     "CNZDPTL1"     TO   ERR-FL-ID.
     MOVE     DPT-STATUS     TO   ERR-STCD.
     DISPLAY  MSG-ABEND1     UPON   CONS.
     DISPLAY  MSG-ABEND2     UPON   CONS.
     MOVE     4000           TO   PROGRAM-STATUS.
     STOP     RUN.
 FILEERR-SEC3                SECTION.
     USE AFTER       EXCEPTION
                     PROCEDURE    CNZCTEL1.
     MOVE     "CNZCTEL1"     TO   ERR-FL-ID.
     MOVE     CTE-STATUS     TO   ERR-STCD.
     DISPLAY  MSG-ABEND1     UPON   CONS.
     DISPLAY  MSG-ABEND2     UPON   CONS.
     MOVE     4000           TO   PROGRAM-STATUS.
     STOP     RUN.
...
*
     MOVE    "FBT05101"      TO   DSP-FORMAT.
     MOVE    SPACE           TO   DSP-FBT05101.
...
*--------------------------------------------------------------*
 SYORI-SUB                   SECTION.
     MOVE   "D"     TO   EDIT-OPTION  OF  DSP-DPTCD.
     MOVE   SPACE   TO   EDIT-CURSOR  OF  DSP-DPTCD.
...
*--------------------------------------------------------------*
 HEAD-SUB                    SECTION.
     MOVE   "D"     TO   EDIT-OPTION  OF  DSP-CTECD.
     MOVE   SPACE   TO   EDIT-CURSOR  OF  DSP-CTECD.
     PERFORM       MSG-SEC.
        ...
        "E000"
           PERFORM       HEADCHK-SUB
                  IF     ERR-MSG-CD   =    ZERO
                         ...
                         END-IF
                         MOVE   "D"     TO
                                EDIT-OPTION  OF  DSP-DPTCD
                         MOVE   SPACE   TO
                                EDIT-CURSOR  OF  DSP-DPTCD
                  END-IF
       WHEN
         OTHER
           MOVE   01     TO   ERR-MSG-CD
...
*    EXIT.
*--------------------------------------------------------------*
*      2.2.2.1   カインズＤＰＴマスタ存在チェック              *
*--------------------------------------------------------------*
 DPTCHK-SUB             SECTION.
     MOVE    ZERO            TO   ERR-MSG-CD.
     MOVE    DSP-DPTCD       TO   DPT-F01.
     READ    CNZDPTL1
         INVALID   KEY
             MOVE  1         TO   INVALID-FLG
         NOT INVALID KEY
...
* ＩＮＶＡＬＩＤ処理
     IF  INVALID-FLG    =    1
         IF   WK-SYORI   NOT =   1
              ...
              MOVE   "R"     TO   EDIT-OPTION  OF  DSP-DPTCD
              MOVE   "C"     TO   EDIT-CURSOR  OF  DSP-DPTCD
         ELSE
              MOVE   "D"     TO   EDIT-OPTION  OF  DSP-DPTCD
              MOVE   " "     TO   EDIT-CURSOR  OF  DSP-DPTCD
         END-IF
     ELSE
* ＮＯＴ ＩＮＶＡＬＩＤ処理
         IF   WK-SYORI       =    1
...
*--------------------------------------------------------------*
 FILE-SUB               SECTION.
     MOVE    DPT-F02          TO   DSP-DPTNM.
     MOVE    DPT-F03          TO   DSP-CTECD.
     MOVE    DPT-F03          TO   CTE-F01.
     READ    CNZCTEL1
         INVALID
             MOVE  SPACE      TO   DSP-CTENM
         NOT INVALID
             MOVE  CTE-F02    TO   DSP-CTENM
     END-READ.
*
     IF      WK-SYORI  =   2
             MOVE  "2"        TO   MAIN-FLG
     ...
     EVALUATE      DSP-FUNC
        ...
        WHEN  "E000"
                  ...
                  IF   ERR-MSG-CD      =   ZERO
                       MOVE   "4"     TO   MAIN-FLG
                       MOVE   "D"     TO
                              EDIT-OPTION  OF  DSP-CTECD
                       MOVE   SPACE   TO
                              EDIT-CURSOR  OF  DSP-CTECD
                  END-IF
        WHEN  OTHER
                  MOVE    01          TO   ERR-MSG-CD
     END-EVALUATE.
...
*    MOVE  SPACE        TO   DSP-MAS002.
     MOVE  SPACE        TO   DSP-DPTNM  DSP-CTECD  DSP-CTENM.
 BODYDEL-END.
     EXIT.
*--------------------------------------------------------------*
*      2.3.2     ＢＯＤＹ入力チェック                          *
...
*
     MOVE    ZERO            TO   ERR-MSG-CD.
     MOVE    DSP-CTECD       TO   CTE-F01.
     READ    CNZCTEL1
         INVALID   KEY
             MOVE   1        TO   INVALID-FLG
             MOVE   08       TO   ERR-MSG-CD
             MOVE   "R"      TO   EDIT-OPTION  OF  DSP-CTECD
             MOVE   "C"      TO   EDIT-CURSOR  OF  DSP-CTECD
             MOVE   SPACE    TO   DSP-CTENM
         NOT INVALID KEY
             MOVE   ZERO     TO   INVALID-FLG
             MOVE   ZERO     TO   ERR-MSG-CD
             MOVE   "D"      TO   EDIT-OPTION  OF  DSP-CTECD
             MOVE   " "      TO   EDIT-CURSOR  OF  DSP-CTECD
             MOVE   CTE-F02  TO   DSP-CTENM
     END-READ.
*
 BODYCHK-END.
...
*
 FILPRT-01.
     IF      WK-SYORI   =    1
             MOVE   SPACE           TO   DPT-REC
             INITIALIZE                  DPT-REC
             MOVE   DSP-DPTCD       TO   DPT-F01
             MOVE   DSP-DPTNM       TO   DPT-F02
             MOVE   DSP-CTECD       TO   DPT-F03
             MOVE   PARA-BUMONCD    TO   DPT-F92
             MOVE   PARA-TANCD(1:2) TO   DPT-F93
             MOVE   SYS-DATE        TO   DPT-F94
             MOVE   WK-TIME(1:6)    TO   DPT-F95
             MOVE   SPACE           TO   DPT-F96
             MOVE   SPACE           TO   DPT-F97
             MOVE   ZERO            TO   DPT-F98
             MOVE   ZERO            TO   DPT-F99
     END-IF.
*
 FILPRT-02.
     IF      WK-SYORI   =    2
             MOVE   DSP-DPTNM       TO   DPT-F02
             MOVE   DSP-CTECD       TO   DPT-F03
             MOVE   PARA-BUMONCD    TO   DPT-F96
             MOVE   PARA-TANCD(1:2) TO   DPT-F97
             MOVE   SYS-DATE        TO   DPT-F98
             MOVE   WK-TIME(1:6)    TO   DPT-F99
     END-IF.
*
 FILPRT-02.
*  処理モードにより追加・更新・削除
