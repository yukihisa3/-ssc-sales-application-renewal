...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3843B.COB`
...
*    業務名　　　　　　　：　ナフコ出荷支援システム　　　　　　*
*    モジュール名　　　　：　本発売上伝票ファイル作成　　　　　*
*    作成日／作成者　　　：　2015/05/21 NAV TAKAHASHI          *
*    処理概要　　　　　　：　手書伝票基本データを順で読み、    *
*                            マスタを参照しながら、売上伝票    *
*                            ファイルを作成する。　　          *
*    更新日／作成者　　　：　2015/07/03 NAV INOUE              *
*    更新概要　　　　　　：　在庫マスタを不使用に　　　　　    *
****************************************************************
...
****************************************************************
 FILE-CONTROL.
*手書伝票基本データ
     SELECT      NHTDENWK    ASSIGN    TO        DA-01-S-NFDENWK
                             FILE      STATUS    DEN2-ST.
*伝票データＦ
*売上伝票ファイル
...
*\\\ 93.06.04 END   \\\
     ...
     03  NHTDENF-INV-FLG     PIC  X(03)  VALUE  SPACE.
*計算領域
 01  WRK-AREA.
...
****************************************************************
 DECLARATIVES.
 DEN2-ERR                    SECTION.
     USE         AFTER       EXCEPTION PROCEDURE NHTDENWK.
...
*
*    OPEN        I-O         NHTDENWK  HTOKMS  ZAMZAIF
     OPEN        I-O         NHTDENWK  HTOKMS
                 INPUT       HSHOTBL   HMEIMS  HTANMS
                 I-O         NHTDENF.
*システム日付・時刻の取得
...
*伝票データ作成
*****売上伝票ファイルを検索する
     PERFORM  NHTDENF-READ-SEC.
*既に登録済の場合は更新しない。
     IF  NHTDENF-INV-FLG = SPACE
         GO                  TO        DEN2-READ
     END-IF.
*
...
*                商品変換テーブル読込み                        *
****************************************************************
 NHTDENF-READ-SEC          SECTION.
*
     MOVE      "NHTDENF-READ-SEC" TO    S-NAME.
*
...
*
     READ      NHTDENF
               INVALID
               MOVE      "INV"    TO    NHTDENF-INV-FLG
               NOT  INVALID
               MOVE      SPACE    TO    NHTDENF-INV-FLG
     END-READ.
*
 NHTDENF-READ-EXIT.
     EXIT.
...
*
*    CLOSE NHTDENWK NHTDENF HTOKMS HSHOTBL HMEIMS ZAMZAIF
     CLOSE NHTDENWK NHTDENF HTOKMS HSHOTBL HMEIMS
           HTANMS.
 END-EXIT.
     EXIT.
****************************************************************
