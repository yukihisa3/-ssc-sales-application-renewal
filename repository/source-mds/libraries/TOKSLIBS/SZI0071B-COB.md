...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SZI0071B.COB`
...
*    03  BR-EXL-F054         PIC  X(02)  VALUE  SPACE.
     03  BR-EXL-F05N         PIC  9(03)  VALUE  ZERO.
 01  WK-AREA.
...
*メッセージ出力領域
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG           PIC  X(08)  VALUE "SZI0071B".
         05  FILLER          PIC  X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER          PIC  X(05)  VALUE " *** ".
         05  END-PG          PIC  X(08)  VALUE "SZI0071B".
         05  FILLER          PIC  X(11)  VALUE
                                         " END   *** ".
     03  MSG-OUT1.
     ...
     03  MSG-OUT10.
         ...
         05  MSG-OUT10-FIL2  PIC  N(12)  VALUE
                             NC"在庫マスタ作成　　　　＝".
     ...
     03  MSG-OUT11.
         ...
         05  MSG-OUT11-FIL2  PIC  N(12)  VALUE
                             NC"作業実績ファイル作成　＝".
...
****************************************************************
 DECLARATIVES.
 EXL-ERR                     SECTION.
     USE         AFTER       EXCEPTION PROCEDURE ZAIWKXX1.
...
*   棚番：作業系２_棚番
     MOVE   EXL-F05Q              TO   SGY-N-F10.
*   数量：作業系２_数量
     MOVE   EXL-F05R              TO   SGY-N-F11.
*   備考：作業系２_備考
     MOVE   EXL-F05S              TO   SGY-N-F12.
*   計上フラグ     F13:INITIAL
*   担当者
...
*   棚番：作業系２_棚番
     MOVE   EXL-F05Q              TO   SGY-S-F10.
*   数量：作業系２_数量
     MOVE   EXL-F05R              TO   SGY-S-F11.
*   備考：作業系２_備考
     MOVE   EXL-F05S              TO   SGY-S-F12.
*   計上フラグ     F13:INITIAL
*   担当者
...
*          DISPLAY NC"商＝" EXL-F057              UPON CONS
*          DISPLAY NC"品＝" EXL-F058 EXL-F059 EXL-F05A
*                  NC"棚＝" EXL-F05Q " INVALID" UPON CONS
*↑TEST
...
*          DISPLAY NC"商＝" EXL-F057              UPON CONS
*          DISPLAY NC"品＝" EXL-F058 EXL-F059 EXL-F05A
*                  NC"棚＝" EXL-F05Q " " "NOT-INVALID" UPON CONS
*↑TEST
           PERFORM    ZAI-CAL-SEC
...
*出庫レコード
*                  現在庫
           COMPUTE  ZAI-F04  =  ZAI-F04  -  EXL-F05R
           IF       EXL-F056 >  ZAI-SIME
*                  次月出庫
                    COMPUTE  ZAI-F12  =  ZAI-F12  +  EXL-F05R
           ELSE
*                  当月入出庫
                    COMPUTE  ZAI-F06  =  ZAI-F06  -  EXL-F05R
*                  当月出庫
                    COMPUTE  ZAI-F08  =  ZAI-F08  +  EXL-F05R
           END-IF
     ELSE
*入庫レコード
*                  現在庫
           ...
           IF       EXL-F056 >  ZAI-SIME
*                  次月入庫
                    COMPUTE  ZAI-F11  =  ZAI-F11  +  EXL-F05R
           ELSE
*                  当月入出庫
                    COMPUTE  ZAI-F06  =  ZAI-F06  +  EXL-F05R
*                  当月入庫
                    COMPUTE  ZAI-F07  =  ZAI-F07  +  EXL-F05R
           END-IF
     END-IF.
*
 ZAI-CAL-EXIT.
...
* 入庫棚番
     IF       EXL-F05P          =         "1"
              MOVE   EXL-F05Q   TO        RUI-F19
* 出庫棚番
     ELSE
              MOVE   EXL-F05Q   TO        RUI-F18
     END-IF.
* 数量
     MOVE     EXL-F05R          TO        RUI-F20.
* 未出庫ＦＬＧ:INITIAL
* 備考
     MOVE     EXL-F05S          TO        RUI-F22.
* 入出庫区分
     MOVE     EXL-F05P          TO        RUI-F23.
* 予備：INITIAL
* 取込日付
...
*
     ...
     MOVE      RUI-WT-CNT   TO    MSG-OUT08.
     MOVE      ZAI-UP-CNT   TO    MSG-OUT09.
     MOVE      ZAI-WT-CNT   TO    MSG-OUT010.
     MOVE      SGY-WT-CNT   TO    MSG-OUT011.
