...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3701L.COB`
...
****************************************************************
 ...
 FILE-CONTROL.
*伝票データ（共通）
     SELECT   NFSHIRED  ASSIGN    TO        NFSHIRED
...
***  ﾜｰｸ ｴﾘｱ
 01  WRK-AREA.
     ...
     03  WK-FRAN1                 PIC  X(07)     VALUE  SPACE.
     03  WK-FRAN2                 PIC  X(15)     VALUE  SPACE.
     03  WK-LRAN1                 PIC  X(20)     VALUE  SPACE.
     03  WK-LRAN2                 PIC  X(16)     VALUE  SPACE.
     03  WK-LRAN3                 PIC  X(07)     VALUE  SPACE.
 01  SEC-NAME.
...
*
 01  TL2.
     ...
     03  TL2-LRAN                 PIC  X(16).
*
 01  TL3.
     03  FILLER                   PIC  X(36)     VALUE SPACE.
     03  TL3-LRAN                 PIC  X(07).
*
*伝票訂正用（合計欄）
 01  TL4.
...
*--- << 仕入データエラー >> ---*
 000-MAST-ERR           SECTION.
     USE AFTER          EXCEPTION      PROCEDURE      NFSHIRED.
...
****************************************************************
 DEN-CNT-SEC            SECTION.
*
     READ     NFSHIRED AT  END
              MOVE     "END"     TO   END-FLG
              GO                 TO   DEN-CNT-EXIT
     END-READ.
*    ナフコ以外は読み飛ばし（取引先）
     IF       DEN-F01  NOT =  137607
              GO                 TO   DEN-CNT-SEC
     END-IF.
*    伝票枚数カウント
...
*明細１行目
***  商品名
     MOVE     DEN-A49             TO        DT1-01.
***  ＪＡＮ商品コード
     MOVE     DEN-A37             TO        DT1-02.
***  色
     MOVE     DEN-A51             TO        DT1-03.
***  サイズ
     MOVE     DEN-A52             TO        DT1-04.
***  原単価（百万の桁）
     IF       DEN-A43        >=   1000000
              MOVE      DEN-A43(1:1)   TO   DT1-05
     END-IF.
***  売単価（百万の桁）
     IF       DEN-A45        >=   1000000
              MOVE      DEN-A45(1:1)   TO   DT1-07
     END-IF.
***  訂正伝票時処理
     IF       DEN-F15   NOT =     DEN-F50
***           訂正後原価金額
              COMPUTE   DT1-06 =  DEN-F15 * DEN-F172
***           訂正後売価金額
              COMPUTE   DT1-08 =  DEN-F15 * DEN-F173
     END-IF.
*明細２行目
***  規格
     MOVE     DEN-A57             TO        DT2-01.
***  発注者商品コード
     MOVE     DEN-A50             TO        DT2-02.
***  柄
     MOVE     DEN-A38             TO        DT2-03.
***  単位
     MOVE     DEN-A40             TO        DT2-04.
***  数量
...
*
*Ｆ欄の上段
     MOVE     DEN-A29             TO        WK-FRAN1.
*Ｆ欄の下段
     MOVE     DEN-A28             TO        WK-FRAN2.
*Ｌ欄の上段
     MOVE     DEN-A31             TO        WK-LRAN1.
*Ｌ欄の中段
     MOVE     DEN-A30             TO        WK-LRAN2.
*Ｌ欄の下段
     MOVE     DEN-A32             TO        WK-LRAN3.
*原価・売価合計計算
...
*ヘッダ１行目
***  Ａ欄
     MOVE     DEN-A23             TO        HD0-01.
***  Ｄ欄の上段
     MOVE     DEN-A24             TO        HD0-02.
***  訂正フラグ＝１の時　訂正区分に"◯"を印刷
     IF       DEN-F53   =    1
              MOVE      NC"◯"    TO        HD0-03
     END-IF.
***  Ｅ欄
     MOVE     DEN-A27             TO        HD0-04.
*ヘッダ２行目
***  Ｄ欄の下段左
     MOVE     DEN-A25             TO        HD1-01.
***  Ｄ欄の下段右
     MOVE     DEN-A26             TO        HD1-02.
***  取引先名
     MOVE     DEN-A16             TO        HD1-03.
*ヘッダ３行目
***  社名
     MOVE     DEN-A14             TO        HD2-01.
*ヘッダ４行目
***  店名
...
***  分類コード
     MOVE     DEN-A08             TO        HD3-03.
***  伝票区分
     MOVE     DEN-A09             TO        HD3-04.
***  伝票番号
     MOVE     DEN-A04             TO        HD3-05.
***  取引先コード
     MOVE     DEN-A12             TO        HD3-06.
***  取引先電話番号
     MOVE     DEN-A17             TO        HD3-07.
***  発注日（年）
     MOVE     DEN-A10(1:2)        TO        HD3-081.
***  発注日（月）
     MOVE     DEN-A10(3:2)        TO        HD3-082.
***  発注日（日）
     MOVE     DEN-A10(5:2)        TO        HD3-083.
***  納品日（年）
     MOVE     DEN-A11(1:2)        TO        HD3-091.
***  納品日（月）
     MOVE     DEN-A11(3:2)        TO        HD3-092.
***  納品日（日）
     MOVE     DEN-A11(5:2)        TO        HD3-093.
*
...
* テール１行目
***  Ｆ欄の上段
     MOVE     WK-FRAN1            TO        TL0-01.
***  Ｆ欄の下段
     MOVE     WK-FRAN2            TO        TL1-01.
***  Ｌ欄の上段
     MOVE     WK-LRAN1            TO        TL1-LRAN.
***  数量合計
...
***  Ｌ欄の中段
     MOVE     WK-LRAN2            TO        TL2-LRAN.
***  Ｌ欄の下段
     MOVE     WK-LRAN3            TO        TL3-LRAN.
*テール２行目
...
*項目クリア
     MOVE     SPACE               TO   HD0 HD1 HD2 HD3
                                       ...
                                       TL0 TL1 TL2 TL3 TL4.
*    テスト印字項目セット
     MOVE   ALL "9"               TO   HD0-01  HD3-02  HD3-03
                                       HD3-04  HD3-05  HD3-06
                                       HD3-081 HD3-082 HD3-083
                                       HD3-091 HD3-092 HD3-093
                                       DT2-071 DT2-091 DT2-11
                                       DT2-10  DT2-12
                                       TL1-021 TL1-04  TL1-05
                                       TL1-031.
     MOVE   ALL "*"               TO   HD0-02  HD0-04
                                       HD1-01  HD1-02  HD1-03
                                       HD2-01
                                       HD3-01  HD3-07
                                       DT1-01  DT1-02  DT1-03
                                       DT1-04  DT2-01  DT2-02
                                       DT2-03  DT2-04  TL0-01
                                       TL1-01  TL1-LRAN
                                       TL2-LRAN  TL3-LRAN.
*ヘッダ部
*****WRITE    PRT-REC            FROM  HD0     AFTER  3.
...
*ボディ部
     ...
     END-PERFORM.
*テール部
     WRITE    PRT-REC          FROM   TL0      AFTER     1.
     WRITE    PRT-REC          FROM   TL1      AFTER     1.
     WRITE    PRT-REC          FROM   TL2      AFTER     1.
     WRITE    PRT-REC          FROM   TL3      AFTER     1.
*
...
*
     CLOSE    NFSHIRED   PRTFILE    DSPFILE.
*
 END-EXIT.
     EXIT.
