...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SNJ0050I.COB`
...
*画面ファイル
*スポット受信スケジュールマスタ
     SELECT  JSMSPTF   ASSIGN    TO        JSMSPTL1
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG15.
         05  FILLER              PIC   N(25)
             VALUE NC"受信ＤＴ区分を入力して下さい　　　　　　".
     03  ERR-MSG15.
         05  FILLER              PIC   N(25)
             VALUE NC"受信ＤＴ区分の入力が間違っています　　　".
 01  ERR-MSG-AREA-R      REDEFINES     ERR-MSG-AREA.
     03  ERR-MSG-R   OCCURS  16  PIC   N(25).
*
 01  FILE-ERR.
...
*             項目転送  （画面 → スポット受信マスタ）
****************************************************************
 MOVE-JSMSPTF        SECTION.
     MOVE     "MOVE-JSMSPTF-SEC"  TO   S-NAME.
...
*作成区分
     IF      DSP-SHORI  =  1
             MOVE       "1"       TO   SJS-F01
     END-IF.
     IF  DSP-SHORI = 1 *> 1:登録処理
*        スケジュール作成日，時間
         MOVE  PAR-TANCD         TO  SJS-F05
         MOVE  SYS-DATE          TO  SJS-F06
         MOVE  WK-TIME(1:4)      TO  SJS-F07
     ELSE              *> 2:更新処理
*        スケジュール更新日，時間
         MOVE  PAR-TANCD         TO  SJS-F08
         MOVE  SYS-DATE          TO  SJS-F09
         MOVE  WK-TIME(1:4)      TO  SJS-F10
     END-IF.
*受信ＤＴ区分
     MOVE    DSP-DTKBN            TO   SJS-F20.
*
 MOVE-JSMSPTF-EXIT.
     EXIT.
****************************************************************
*             スポット受信マスタ　ＲＥＡＤ                     *
****************************************************************
 JSMSPTF-READ             SECTION.
     MOVE     "JSMSPTF-READ"      TO   S-NAME.
*
...
*画面の初期化
     MOVE    SPACE                TO   DSP-FNJ0050I.
*ＰＧＩＤ
     MOVE    "SNJ0050I"           TO   DSP-PGID.
*システム日付転送
     MOVE    SYS-DATE             TO   DSP-SDATE.
*システム時間転送
     MOVE    WK-TIME(1:6)         TO   DSP-STIME.
...
****************************************************************
 END-SEC               SECTION.
*ファイル ＣＬＯＳＥ
     CLOSE             JSMSPTF  HTOKMS  DSPFILE.
**
 END-EXIT.
     EXIT.
*****************<<  SNJ0050I   END PROGRAM  >>******************
