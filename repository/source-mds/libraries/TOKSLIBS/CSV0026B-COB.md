...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/CSV0026B.COB`
...
**** メッセージ情報           ****
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "CSV0026B".
...
****  フラグ                  ****
 ...
 01  CSYOF-INV-FLG         PIC  X(03)  VALUE  SPACE.
...
****  店舗格納ワーク          ****
 01  WK-TBL-TENPO.
*----< 2006/09/04 START >----*
*****03  TBL-TENPO           PIC  9(05)  OCCURS  200.
*****03  TBL-TENPO           PIC  9(05)  OCCURS  300.
     03  TBL-TENPO           PIC  9(05)  OCCURS  416.
*----< 2006/09/04  END  >----*
*
************************************************************
 LINKAGE            SECTION.
...
*
     ...
     MOVE    HAC-F05             TO   CSV-F05.
     READ    CSYOF
             INVALID     MOVE "INV" TO CSYOF-INV-FLG
             NOT INVALID MOVE SPACE TO CSYOF-INV-FLG
     END-READ.
*
 CVSSYOF-READ-EXIT.
     EXIT.
...
*****PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 300
     PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 416
*----< 2006/09/04  END  >----*
                                       OR TBL-FLG = 1
...
************************************************************
 300-END-SEC           SECTION.
     CLOSE             HTENMS   HACYUPRT  CSYOF.
*
     DISPLAY "* HTENMS   (INPUT) = " TEN-CNT  " *"  UPON CONS.
     DISPLAY "* HACYUPRT (INPUT) = " HAC-CNT  " *"  UPON CONS.
     DISPLAY "* CSYOF  (WRITE) = " WRT-CNT  " *"  UPON CONS.
     DISPLAY "* CSYOF(REWRITE) = " RWT-CNT  " *"  UPON CONS.
*
 300-END-SEC-EXT.
     EXIT.
*****************<<  PROGRAM  END  >>***********************
