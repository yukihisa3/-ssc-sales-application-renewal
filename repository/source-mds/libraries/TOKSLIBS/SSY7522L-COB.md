...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY7522L.COB`
...
*明細単位消費税計算
 01  TAX-MEIS.
     03  TAX-MEIS-R          OCCURS  6.
         ...
         05  TAX-TANKA1      PIC  9(10).
         05  TAX-TANKA2      PIC  9(10).
         05  TAX-SURYO       PIC S9(10).
         05  MEIS-KINGAKU1   PIC S9(10).
         05  MEIS-KINGAKU2   PIC S9(10).
     03  TAX-GK-KINGAKU1     PIC S9(10).
     03  TAX-GK-KINGAKU2     PIC S9(10).
*
*日付変換サブルーチン用ワーク
...
*----<< 明細単位税抜き金額算出 >>--*
     ...
     COMPUTE  TAX-KINGAKU(1)  = SIH-F204 / 10.
     COMPUTE  TAX-SURYO  (1)  = SIH-F205 / 10.
     MOVE  SIH-F206               TO        TAX-FUGO(2).
     COMPUTE  TAX-KINGAKU(2)  = SIH-F207 / 10.
     COMPUTE  TAX-SURYO  (2)  = SIH-F208 / 10.
     MOVE  SIH-F209               TO        TAX-FUGO(3).
     COMPUTE  TAX-KINGAKU(3)  = SIH-F210 / 10.
     COMPUTE  TAX-SURYO  (3)  = SIH-F211 / 10.
     MOVE  SIH-F212               TO        TAX-FUGO(4).
     COMPUTE  TAX-KINGAKU(4)  = SIH-F213 / 10.
     COMPUTE  TAX-SURYO  (4)  = SIH-F214 / 10.
     MOVE  SIH-F215               TO        TAX-FUGO(5).
     COMPUTE  TAX-KINGAKU(5)  = SIH-F216 / 10.
     COMPUTE  TAX-SURYO  (5)  = SIH-F217 / 10.
     MOVE  SIH-F218               TO        TAX-FUGO(6).
     COMPUTE  TAX-KINGAKU(6)  = SIH-F219 / 10.
     COMPUTE  TAX-SURYO  (6)  = SIH-F220 / 10.
*    単価算出
     PERFORM  VARYING  IX  FROM  1  BY  1  UNTIL  IX  >  6
      IF  TAX-SURYO(IX) >  ZERO
          COMPUTE TAX-TANKA1(IX)  =
                  TAX-KINGAKU(IX) / TAX-SURYO(IX)
          IF  JWW-F06 <= SIH-F06
              COMPUTE TAX-TANKA2(IX) ROUNDED =
                      TAX-TANKA1(IX) /  JWW-F07
              COMPUTE WK-ZEI = JWW-F08 * 100
          ELSE
              COMPUTE TAX-TANKA2(IX) ROUNDED =
                      TAX-TANKA1(IX) /  JWW-F04
              COMPUTE WK-ZEI = JWW-F05 * 100
          END-IF
*****IF  SIH-F04 = 43
*        DISPLAY "TAX-TANKA1    = "  TAX-TANKA1(IX)    UPON CONS
*        DISPLAY "TAX-TANKA2    = "  TAX-TANKA2(IX)    UPON CONS
*****END-IF
      END-IF
     END-PERFORM.
*
*    消費税抜き計算
     PERFORM  VARYING  IX  FROM  1  BY  1  UNTIL  IX  >  6
      IF  TAX-KINGAKU(IX) >  ZERO
          IF  TAX-FUGO(IX) = "-"
              COMPUTE TAX-SURYO(IX) = TAX-SURYO(IX) * -1
          END-IF
*         消費税込金額算出
          COMPUTE MEIS-KINGAKU1(IX) =
                  TAX-SURYO(IX)  *  TAX-TANKA1(IX)
          COMPUTE MEIS-KINGAKU2(IX) =
                  TAX-SURYO(IX)  *  TAX-TANKA2(IX)
*****IF  SIH-F04 = 2007921
*    OR  SIH-F04 = 43
*        DISPLAY "SIH-F03       = "  SIH-F03           UPON CONS
*        DISPLAY "IX            = "  IX                UPON CONS
*        DISPLAY "TAX-FUGO      = "  TAX-FUGO(IX)      UPON CONS
*        DISPLAY "TAX-SURYO     = "  TAX-SURYO(IX)     UPON CONS
*        DISPLAY "MEIS-KINGAKU1 = "  MEIS-KINGAKU1(IX) UPON CONS
*        DISPLAY "MEIS-KINGAKU2 = "  MEIS-KINGAKU2(IX) UPON CONS
*****END-IF
          ADD     MEIS-KINGAKU1(IX)  TO  TAX-GK-KINGAKU1
          ADD     MEIS-KINGAKU2(IX)  TO  TAX-GK-KINGAKU2
      END-IF
     END-PERFORM.
*
*店舗計／総合計に加算
     ADD       TAX-GK-KINGAKU1    TO        GENKEIK.
     ADD       TAX-GK-KINGAKU1    TO        SOGKEIK.
     ADD       TAX-GK-KINGAKU2    TO        GENKEIN.
     ADD       TAX-GK-KINGAKU2    TO        SOGKEIN.
*
 SYUKEI-EXIT.
     EXIT.
**********************************************************
...
*伝票種類名
     ...
     END-IF.
*原価金額合計（税込）
     MOVE  TAX-GK-KINGAKU1        TO        PRT-GENKAK.
     MOVE  TAX-GK-KINGAKU2        TO        PRT-GENKAN.
*税率
     MOVE  "("                    TO        PRT-KUGIR3.
