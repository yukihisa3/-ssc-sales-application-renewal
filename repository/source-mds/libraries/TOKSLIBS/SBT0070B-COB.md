...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SBT0070B.COB`
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SBT0070B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SBT0070B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-IN.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*振替累積ファイル２スタート（送信区分＝" "（新規））
     ...
     MOVE     2                     TO   FR2-F02I.
     MOVE     "70"                  TO   FR2-F022.
     START    FURRUIL2 KEY >= FR2-F96 FR2-F02J FR2-F02I FR2-F022
         INVALID   KEY
              MOVE "END"     TO   END-FLG
              GO             TO   INIT-EXIT
     END-START.
...
*納品日
*             ↓（出荷日）
     MOVE     FR2-F029            TO   SYU-F02.
*伝票番号　　
     MOVE     FR2-F023            TO   SYU-F03.
*取引先コード
     MOVE     "00000000    "      TO   SYU-F04.
*店舗コード
     MOVE     FR2-F02W            TO   SYU-F05.
*受信日
     MOVE     SYS-DATEW           TO   SYU-F06.
*受信時刻
     MOVE     SYS-HHMMSS          TO   SYU-F07.
*届先コード
     MOVE     FR2-F02W            TO   SYU-F08.
*仕分店コード１
     MOVE     FR2-F02W            TO   SYU-F09.
*仕分店コード２
     MOVE     FR2-F02W            TO   SYU-F10.
*部門コード
...
*出荷指示数
     IF       MEI-F89   =   "1"
              DIVIDE  FR2-F02F BY MEI-F07  GIVING  WK-DEN-F15
                                           REMAINDER   AMARI
              IF    AMARI   NOT = 0
                    ...
                    MOVE    FR2-F02F   TO  WK-SUURYOU
                    DISPLAY NC"数量　　＝" WK-SUURYOU UPON CONS
                    DISPLAY NC"場所　　＝" FR2-F02W UPON CONS
                    DISPLAY NC"出荷日　＝" FR2-F029 UPON CONS
                    DISPLAY NC"伝票　　＝" FR2-F023 UPON CONS
                    DISPLAY NC"行　　　＝" FR2-F024 UPON CONS
***                 MOVE    4000       TO  PROGRAM-STATUS
***                 MOVE    "END"      TO  END-FLG
***                 GO      TO             MAIN-L2-EXIT
                    GO      TO             MAIN-L2-99
...
*納品日
*             ↓（出荷日）
     MOVE     FR3-F029            TO   SYU-F02.
*伝票番号　　
     MOVE     FR3-F023            TO   SYU-F03.
*取引先コード
     MOVE     "00000000    "      TO   SYU-F04.
*店舗コード
     MOVE     FR3-F02W            TO   SYU-F05.
*受信日
     MOVE     SYS-DATEW           TO   SYU-F06.
*受信時刻
     MOVE     SYS-HHMMSS          TO   SYU-F07.
*届先コード
     MOVE     FR3-F02W            TO   SYU-F08.
*仕分店コード１
     MOVE     FR3-F02W            TO   SYU-F09.
*仕分店コード２
     MOVE     FR3-F02W            TO   SYU-F10.
*部門コード
     MOVE     SPACE               TO   SYU-F11.
*形状コード　
     MOVE     SPACE               TO   SYU-F12.
...
*出荷指示数
     IF       MEI-F89   =   "1"
              DIVIDE  FR3-F02F BY MEI-F07  GIVING  WK-DEN-F15
                                           REMAINDER   AMARI
              IF    AMARI    NOT = 0
                    DISPLAY  NC"数量が入数で割り切れません！"
                                                     UPON CONS
                    ...
                    MOVE    MEI-F07    TO  WK-MEI-F07
                    DISPLAY NC"入り数　＝" WK-MEI-F07 UPON CONS
                    MOVE    FR3-F02F   TO  WK-SUURYOU
                    DISPLAY NC"数量　　＝" WK-SUURYOU UPON CONS
                    DISPLAY  NC"場所　　＝" FR3-F02W UPON CONS
                    DISPLAY  NC"出荷日　＝" FR3-F029 UPON CONS
                    DISPLAY  NC"伝票　　＝" FR3-F023 UPON CONS
                    DISPLAY  NC"行　　　＝" FR3-F024 UPON CONS
***                   MOVE     4000       TO  PROGRAM-STATUS
***                   MOVE     "END"      TO  END-FLG
***                   GO       TO             MAIN-L3-EXIT
                    GO       TO             MAIN-L3-99
...
*商品名称マスタ取得
     ...
     MOVE       FR2-F02B      TO         MEI-F0121.
     MOVE       FR2-F02C      TO         MEI-F0122.
     MOVE       FR2-F02D      TO         MEI-F0123.
...
*商品名称マスタ取得
     ...
     MOVE       FR3-F02B      TO         MEI-F0121.
     MOVE       FR3-F02C      TO         MEI-F0122.
     MOVE       FR3-F02D      TO         MEI-F0123.
     PERFORM    HMEIMS-READ-SEC.
     IF         HMEIMS-INV-FLG    NOT =  SPACE
                GO            TO         FURRUIL3-READ-SEC
     END-IF.
...
*I--------------------------MOVE  1       TO  MEI-F07  入数
     ...
     IF       HMEIMS-INV-FLG NOT = SPACE
        IF    PARA-IN-SOUSIN-KB    =    "1"
              ...
              DISPLAY NC"商品ＣＤ＝" FR3-F02A FR3-F02B UPON CONS
***           MOVE  4000    TO  PROGRAM-STATUS
***           MOVE  "END"   TO  END-FLG
              GO            TO  HMEIMS-READ-EXIT
        ELSE
              DISPLAY NC"商品名称マスタ未登録！"     UPON CONS
              DISPLAY NC"商品ＣＤ＝" FR2-F02A FR2-F02B UPON CONS
***           MOVE  4000    TO  PROGRAM-STATUS
***           MOVE  "END"   TO  END-FLG
              GO            TO  HMEIMS-READ-EXIT
        END-IF
