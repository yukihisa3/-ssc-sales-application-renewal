...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY874KL.COB`
...
*
 ...
 FILE-CONTROL.
*----<< ＤＣＭ出荷確定データ >>--*
     SELECT   DJSYUKF   ASSIGN         DA-01-VI-DJSYUKL4
...
*----<< カーマ出荷確定データ >>--*
 DJSYUKF-ERR            SECTION.
     ...
     DISPLAY  "### SSY874KL DJSYUKF ERROR " DJSYUKF-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
     CLOSE    DJSYUKF  TENMS1.
     STOP     RUN.
*----<< 店舗マスタ >>--*
 TENMS1-ERR             SECTION.
     ...
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "### SSY874KL TENMS1 ERROR " HTENMS-ST " "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS " ###"
                                       UPON CONS.
     DISPLAY  SEC-NAME                 UPON CONS.
...
*--------------------------------------------------------------*
 100-INIT-RTN           SECTION.
     ...
     END-IF.
*
     DISPLAY  "*** SSY874KL START *** "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS
                                       UPON CONS.
     OPEN     INPUT     DJSYUKF  TENMS1.
     OPEN     OUTPUT    PRTF.
*
...
*
     ...
     END-IF.
*
     CLOSE    DJSYUKF  PRTF  TENMS1.
*
     ACCEPT   SYS-DATE       FROM DATE.
     ACCEPT   SYS-TIME       FROM TIME.
     DISPLAY  "*** SSY874KL END *** "
              SYS-YY "." SYS-MM "." SYS-DD " "
              SYS-HH ":" SYS-MN ":" SYS-SS
                                       UPON CONS.
 300-END-RTN-EXIT.
...
*    出荷日
     MOVE     KMK-F06(1:4)   TO   WK-H-YYYY.
     MOVE     KMK-F06(5:2)   TO   WK-H-MM.
     MOVE     KMK-F06(7:2)   TO   WK-H-DD.
     MOVE     "/"            TO   WK-H-KU1 WK-H-KU2.
...
*ブレイクチェック
     ...
     END-IF.
*店舗ＣＤ範囲チェック
     IF       LINK-STENCD  <=  KMK-F21
     AND      LINK-ETENCD  >=  KMK-F21
              CONTINUE
     ELSE
              GO        TO        900-KMK-READ
     END-IF.
*店舗マスタ検索（センター区分）
     ...
     INITIALIZE                            TEN-REC.
     MOVE        KMK-K03         TO        TEN-F52.
     MOVE        KMK-F21         TO        TEN-F011.
     READ    TENMS1
       INVALID
         MOVE    SPACE           TO        TEN-REC
         INITIALIZE                        TEN-REC
     END-READ.
*センター区分（店舗コードがセンターの場合、対象外）
     IF  TEN-F75  =  "1"
              GO        TO        900-KMK-READ
     END-IF.
*
