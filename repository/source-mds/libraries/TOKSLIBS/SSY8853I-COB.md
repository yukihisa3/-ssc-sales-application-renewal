...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY8853I.COB`
...
*#2019/03/18 NAV ST
     02  DCMHSBL2-INV-FLG    PIC  X(03)  VALUE SPACE.
*#2019/03/18 NAV ED
****  処理スイッチ  ****
 01  WK-AREA.
****  インデックス  ****
...
****  メッセージ情報  ***
 01  MSG-AREA1-1.
     02  MSG-ABEND1.
       ...
       03  ERR-PG-ID         PIC  X(08)  VALUE  "SSY8853I".
...
*        定番特売区分（名称）
         EVALUATE  DKC-F16
              WHEN    "1"
                   MOVE "1"              TO   HACCD(IXA)
                   MOVE NC"定期　　　　" TO   HACNM(IXA)
              WHEN    "2"
                   MOVE "2"              TO   HACCD(IXA)
                   MOVE NC"特売　　　　" TO   HACNM(IXA)
              WHEN    "3"
                   MOVE "3"              TO   HACCD(IXA)
                   MOVE NC"投入　　　　" TO   HACNM(IXA)
              WHEN    "4"
                   MOVE "4"              TO   HACCD(IXA)
                   MOVE NC"新店改装　　" TO   HACNM(IXA)
              WHEN    "6"
                   MOVE "6"              TO   HACCD(IXA)
                   MOVE NC"緊急ダイレク" TO   HACNM(IXA)
*#2019/02/21 NAV ST
              WHEN    "F"
                   MOVE "F"              TO   HACCD(IXA)
                   MOVE NC"客注ダイレク" TO   HACNM(IXA)
              WHEN    "A"
                   MOVE "A"              TO   HACCD(IXA)
                   MOVE NC"本部在庫補充" TO   HACNM(IXA)
              WHEN    "B"
                   MOVE "B"              TO   HACCD(IXA)
                   MOVE NC"専用_在庫補" TO   HACNM(IXA)
              WHEN    "C"
                   MOVE "C"              TO   HACCD(IXA)
                   MOVE NC"用度品　　　" TO   HACNM(IXA)
              WHEN    "D"
                   MOVE "D"              TO   HACCD(IXA)
...
*
         IF        IXA  =    1
              ...
              MOVE DKC-F16        TO   SAV-BKEY-KBN
         END-IF
         IF        IXA  =    14
              MOVE DKC-F07        TO   SAV-NKEY-BUMON
              MOVE DKC-F16        TO   SAV-NKEY-KBN
         END-IF
*
         PERFORM   DKCNTF-READ-SUB
     END-PERFORM.
...
*定番特売区分（名称）
*2013/08/15 NAV ST 発注種別区分変更に伴う追加　７：改廃
*****IF   HACCD(IXA)  =  "1" OR "2" OR "3" OR "4" OR "5" OR "6"
*####IF   HACCD(IXA)  =  "1" OR "2" OR "3" OR "4"  OR "6"
*####IF   HACCD(IXA)  =  "1" OR "2" OR "3" OR "4"  OR "6"
...
*        部門ＣＤ
         MOVE      DKC-F07        TO   BUMON (IXA)
*********DISPLAY  "DKC-F07 = " DKC-F07  UPON CONS
*        部門名
**09/02/23 STR
         EVALUATE     DKC-F07
         WHEN    001
