...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/ONCHECK.COB`
...
*    作成者／更新者　：NAV
*    処理概要　　　　：
*      PRIMERGY6000の営業日欄の曜日を取得しパラメタに渡す。
...
*　システム日付／時刻／曜日
 ...
 01  YOUBI-AREA.
     ...
     03  WK-YOUBI           PIC S9(01)  VALUE  ZERO.
*
*　日付論理チェック
 01  WK-CHKDATE.
...
*
     DISPLAY   "**  START ONCHECK    **"  UPON CONS.
*
*ファイルのＯＰＥＮ
     OPEN  INPUT  HCONTF.
*ワークの初期化
...
*    ﾚｺｰﾄﾞ非存在時は、前曜日を渡す
     IF       FG-HCONTF-INV = 1
              ...
              COMPUTE WK-YOUBI  =  SYS-YOUBI - 1
              IF      WK-YOUBI <=  0
                      MOVE  7      TO  WK-YOUBI
              END-IF
              MOVE    WK-YOUBI     TO  PARA-YOUBI
     ELSE
              MOVE    CON-F023     TO  PARA-YOUBI
     END-IF.
*
     DISPLAY NC"曜日＝"   PARA-YOUBI   UPON CONS.
*
 MAIN-99.
     MOVE     "END"  TO  END-FLG.
 MAIN-EXIT.
