...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SJH9014B.COB`
...
*
 ...
 FILE-CONTROL.
*受信データファイル
     SELECT   OJYURYO  ASSIGN    TO        DA-01-S-OJYURYO
                        ACCESS    MODE IS   SEQUENTIAL
                        FILE      STATUS    JJR-STATUS
                        ORGANIZATION   IS   SEQUENTIAL.
*受領データファイル
     SELECT   OJFJYRL1  ASSIGN    TO        DA-01-VI-OJFJYRL1
...
*    ｶｳﾝﾄ
 ...
 01  OJFJYRL1-INV-FLG        PIC  X(03)     VALUE  SPACE.
*ヘッドレコード退避ワーク
 01  WK-DEPB-REC.
...
*
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SJH9014B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SJH9014B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*各量販店領域転送
*    取引先ＣＤ
     MOVE     WK-DEPB10(1:4) TO       JYR-F00.
*    受信日
     MOVE     ZERO          TO        JYR-F01.
*    発注日
     MOVE       "3"         TO        LINK-IN-KBN.
     MOVE     WK-DEPB08     TO        LINK-IN-YMD6.
     CALL     "SKYDTCKB"   USING      LINK-IN-KBN
                                      LINK-IN-YMD6
                                      LINK-IN-YMD8
                                      LINK-OUT-RET
                                      LINK-OUT-YMD8.
     IF          LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8  TO   JYR-F02
     ELSE
         MOVE    ZERO           TO   JYR-F02
     END-IF.
*    納品日
     MOVE       "3"         TO        LINK-IN-KBN.
     MOVE     WK-DEPB09     TO        LINK-IN-YMD6.
     CALL     "SKYDTCKB"   USING      LINK-IN-KBN
                                      LINK-IN-YMD6
                                      LINK-IN-YMD8
                                      LINK-OUT-RET
                                      LINK-OUT-YMD8.
     IF          LINK-OUT-RET   =    ZERO
         MOVE    LINK-OUT-YMD8  TO   JYR-F03
     ELSE
         MOVE    JYR-F02        TO   JYR-F03
     END-IF.
*    店舗コード
     MOVE     WK-DEPB05     TO        JYR-F04.
*    店舗名
     MOVE     WK-DEPB13     TO        JYR-F05.
*    伝票番号
     MOVE     WK-DEPB03     TO        JYR-F06.
*    行番号
     MOVE     WK-DEPD03     TO        JYR-F07.
*    伝票区分
     MOVE     WK-DEPB07     TO        JYR-F08.
*    存在チェック
     READ  OJFJYRL1
           INVALID      MOVE  "INV"  TO  OJFJYRL1-INV-FLG
           NOT  INVALID MOVE  SPACE  TO  OJFJYRL1-INV-FLG
     END-READ.
*    判定
     IF    OJFJYRL1-INV-FLG = SPACE
           GO                 TO      EDIT-EXIT
     END-IF.
*    商品ＣＤ
     MOVE     ZERO          TO        JYR-F09.
*    商品名１
     MOVE     WK-DEPD14(1:15) TO      JYR-F101(1:15).
*    商品名２
     MOVE     WK-DEPD14(16:5) TO      JYR-F102(1:5).
     MOVE     WK-DEPD15(1:10) TO      JYR-F102(6:10).
*    発注数
     MOVE     WK-DEPD08     TO        JYR-F11.
*    検収数
     MOVE     WK-DEPD16     TO        JYR-F12.
*    原価単価
     MOVE     WK-DEPD09     TO        JYR-F13.
*    原価金額
     COMPUTE  JYR-F14 = WK-DEPD16 * WK-DEPD09.
*    ＪＡＮＣＤ
     MOVE     WK-DEPD04     TO        JYR-F15.
*    納品書番号（計上区分）
     MOVE     WK-DEPD18     TO        JYR-F16.
*    年月度
     MOVE     JYR-F03(1:6)  TO        JYR-F17.
*
     WRITE    JYR-REC.
     ADD      1             TO   WRT-CNT.
*
