...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY3921L.COB`
...
*
 ...
 FILE                   SECTION.
*    ナフコ商品分類パターンファイル
 FD  NFSHBWK1            LABEL RECORD   IS   STANDARD.
...
*FLG/ｶｳﾝﾄ
 ...
 01  NFSHBWK1-READ-CNT       PIC  9(07)     VALUE  ZERO.
...
*メッセージエリア
 01  MSG-AREA.
     03  MSG-START.
         ...
         05  ST-PG          PIC   X(08)  VALUE "SSY3921L".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "SSY3921L".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*--<< ﾌﾟﾘﾝﾄ AREA >>-*
 01  HD00.
     03  FILLER         CHARACTER  TYPE YB-22.
         ...
         05  FILLER          PIC  X(08)     VALUE  "SSY3921L".
*        05  FILLER          PIC  X(27)     VALUE  SPACE.
         05  FILLER          PIC  X(15)     VALUE  SPACE.
*        05  FILLER          PIC  N(22)     VALUE
*        NC"＜ナフコ商品分類パターン取込チェックリスト＞".
         05  FILLER          PIC  N(25)     VALUE
         NC"＜ナフコ商品分類パターンマスタ取込チェックリスト＞".
...
*ナフコ商品分類パターンマスタ取込ファイルスタート
     PERFORM  NFSHBWK1-START-SEC.
     IF   END-FLG = "END"
          ...
          GO                    TO    INIT-EXIT
     END-IF.
*ナフコ商品分類パターンマスタ取込ファイル読込
     PERFORM NFSHBWK1-READ-SEC.
     IF   END-FLG = "END"
          DISPLAY NC"＃＃　エラーデータ　なし　＃＃"  UPON CONS
          PERFORM  HEAD-WT-SEC
          WRITE    PRT-REC      FROM  LST-DATA-X  AFTER  5
...
* ナフコ商品分類パターンマスタ取込ファイルスタート
****************************************************************
 NFSHBWK1-START-SEC          SECTION.
*
     MOVE    "NFSHBWK1-START-SEC"  TO   S-NAME.
*
     MOVE     SPACE               TO   EXL-REC.
     INITIALIZE                        EXL-REC.
*
     MOVE     SPACE               TO   EXL-F060.
     MOVE     SPACE               TO   EXL-F051.
     MOVE     SPACE               TO   EXL-F053.
*
     START  NFSHBWK1  KEY  IS  >=      EXL-F060 EXL-F051 EXL-F053
            INVALID
            MOVE    "END"         TO   END-FLG
     END-START.
*
...
*    ナフコ商品分類パターンマスタ読込
****************************************************************
 NFSHBWK1-READ-SEC           SECTION.
*
     MOVE    "NFSHBWK1-READ-SEC"   TO   S-NAME.
*
     READ     NFSHBWK1  AT  END
              MOVE     "END"      TO   END-FLG
              GO                  TO   NFSHBWK1-READ-EXIT
     END-READ.
*エラー区分チェック
     IF       EXL-F060  NOT =  "1"
              MOVE     "END"      TO   END-FLG
...
*   改頁判定
     ...
     END-IF.
*   商品分類コード
     MOVE     EXL-F051            TO   MS01-SHOBNRUI.
*   商品分類名
     MOVE     EXL-F052            TO   MS01-SHOBNMEI.
*   パターンコード
...
*件数印字
*ナフコ商品分類パターンマスタ
     DISPLAY "NFSHBWK1 PRINT-CNT = " NFSHBWK1-READ-CNT UPON CONS.
*
     CLOSE     NFSHBWK1  TANMS1  PRTF.
*
     STOP      RUN.
*
 END-EXIT.
