...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SHA0020L.COB`
...
*
 ...
 01  NEW-KEY.
     ...
     03  NEW-F02             PIC  9(07).
*内部統制メッセージ
 01  WK-MSG-CD               PIC  X(01)  VALUE  SPACE.
...
****  明細行４               ****
 01  MEISAI4        CHARACTER     TYPE   IS   YB.
     ...
     02  PRT35               PIC  X(10).
****  明細行５               ****
 01  MEISAI5        CHARACTER     TYPE   IS   YB.
     02  FILLER              PIC  X(01)  VALUE  SPACE.
     02  PRT36               PIC  N(20).
****  テーブル                ****
 01  TBL-AREA.
...
*
     EVALUATE  WK-MSG-CD
         ...
         WRITE    P-REC         AFTER   1
         WHEN  "1"
         MOVE   NC"↑この伝票は承認が必要です。" TO  PRT36
         WRITE    P-REC  FROM  MEISAI5  AFTER  1
         WHEN  "2"
         MOVE   NC"↑担当者マスタ未登録です。"   TO  PRT36
         WRITE    P-REC  FROM  MEISAI5  AFTER  1
     END-EVALUATE.
*****MOVE     SPACE         TO      P-REC.
*****WRITE    P-REC         AFTER   1.
