...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SJS0043L.COB`
 ...
 01  WK-SYUKEI-AREA.
     03  WK-S-AREA.
       05  WK-S-AREA2  OCCURS 20. *> サカタ２０分類
         07  WK-S-AREA3  OCCURS 2. *> 1:該当年、2:前年
           ...
           09  WK-S-NEBSU   PIC S9(13)V99  PACKED-DECIMAL.
           09  WK-S-NEBGAK  PIC S9(13)     PACKED-DECIMAL.
     03  WK-G-AREA. *> 取引先計
     ...
     03  WK-T-AREA.  *> 総計
       05  WK-T-AREA2  OCCURS 2. *> 1:該当年、2:前年
         ...
         07  WK-T-NEBSU     PIC S9(13)V99  PACKED-DECIMAL.
         07  WK-T-NEBGAK    PIC S9(13)     PACKED-DECIMAL.
*
****  見出し行１             ****
 01  MIDASI1  CHARACTER TYPE IS  YA.
     02  FILLER             PIC  X(01)  VALUE SPACE.
     02  FILLER             PIC  X(08)  VALUE "SJS0043L".
...
****  明細行３               ****
 01  MEISAI3.
     ...
     02  PRT3-NEBSU         PIC  ---,---,--9.99.
     02  PRT3-NEBGAK        PIC  ---,---,--9.
...
*メッセージ情報
 01  MSG-AREA.
     03  MSG-ABEND1.
         05  FILLER          PIC  X(12)  VALUE  "### SJS0043L".
...
*=============================================================
 CONTROL-SEC         SECTION.
     DISPLAY  "**  SJS0043L   START  **"   UPON  CONS.
*
     PERFORM  INIT-SEC.
     PERFORM  MAIN-SEC    UNTIL  MAIN-FLG  =  99.
     PERFORM  END-SEC.
*
     DISPLAY  "**  SJS0043L    END   **"   UPON  CONS.
     STOP  RUN.
 CONTROL-EXIT.
     EXIT.
*=============================================================
...
*初期画面の処理
     ...
     MOVE "FJS00431"        TO    DSP-FMT.
     MOVE  SPACE            TO    DSP-FJS00431.
     PERFORM  OPT-CLR-SEC.
*年月
...
*    MOVE  WK-STSYUNO       TO  WK-RVS-END-SYUNO.
     MOVE  WK-EDSYUNO       TO  WK-RVS-END-SYUNO.
...
* 開始検索ファイルを開始ファイルインデックスに変換。
    DISPLAY "WK-STFILE-TOU = " WK-STFILE-TOU UPON CONS.
     EVALUATE  TRUE
       ...
       WHEN  WK-STFILE-TOU = "JISSSB3 "
         ...
         MOVE  3            TO  FG-EDFILE
     END-EVALUATE.
    DISPLAY "FG-STFILE = " FG-STFILE UPON CONS.

* 終了検索ファイルを終了ファイルインデックスに変換。
    DISPLAY "WK-EDFILE-TOU = " WK-EDFILE-TOU UPON CONS.
*    EVALUATE  TRUE
*      WHEN  WK-EDFILE-TOU = "JISSSYUF"
*        MOVE  ZERO         TO  FG-EDFILE
*      WHEN  WK-EDFILE-TOU = "JISSSB1 "
*        MOVE  1            TO  FG-EDFILE
*      WHEN  WK-EDFILE-TOU = "JISSSB2 "
*        MOVE  2            TO  FG-EDFILE
*      WHEN  WK-EDFILE-TOU = "JISSSB3 "
*        MOVE  3            TO  FG-EDFILE
*    END-EVALUATE.
*   DISPLAY "FG-EDFILE = " FG-EDFILE UPON CONS.
...
* 当年
     ...
     MOVE  WK-EDSYUNO       TO  WK-END-SYUNO.
     MOVE  DSP-EDTOR        TO  WK-END-TOR.
     DISPLAY "PRINTC-MAE-SEC IX-FIL= " IX-FIL  UPON CONS.
     EVALUATE  TRUE
 ...
 BODY-PRINT-SEC             SECTION.
     IF  DSP-BNR20 = "1" *> サカタ２０分類実績分のみ
         IF      WK-G-SURYO  (1) = ZERO
             ...
             AND WK-G-NEBSU  (1) = ZERO
             AND WK-G-NEBGAK (1) = ZERO
             AND WK-G-SURYO  (2) = ZERO
             AND WK-G-KINGAK (2) = ZERO
             AND WK-G-HEPSU  (2) = ZERO
             AND WK-G-HEPGAK (2) = ZERO
             AND WK-G-NEBSU  (2) = ZERO
             AND WK-G-NEBGAK (2) = ZERO
     ...
     IF  DSP-BNR20 = "1" *> サカタ２０分類実績分のみ
         ...
         PERFORM  VARYING IX  FROM 1 BY 1 *> 明細
           ...
           IF      WK-S-SURYO  (IX 1) = ZERO
               ...
               AND WK-S-NEBSU  (IX 1) = ZERO
               AND WK-S-NEBGAK (IX 1) = ZERO
               AND WK-S-SURYO  (IX 2) = ZERO
               AND WK-S-KINGAK (IX 2) = ZERO
               AND WK-S-HEPSU  (IX 2) = ZERO
               AND WK-S-HEPGAK (IX 2) = ZERO
               AND WK-S-NEBSU  (IX 2) = ZERO
               AND WK-S-NEBGAK (IX 2) = ZERO
               CONTINUE
            ELSE
...
* 該当年サカタ２０分類明細
     ...
     MOVE  WK-S-NEBSU  (IX 1)    TO  PRT2-NEBSU.
     MOVE  WK-S-NEBGAK (IX 1)    TO  PRT2-NEBGAK.
...
***      MOVE  ALL "_"      TO  PRT3-G
     ...
     MOVE  WK-S-NEBSU  (IX 2)    TO  PRT3-NEBSU.
     MOVE  WK-S-NEBGAK (IX 2)    TO  PRT3-NEBGAK.
...
*=============================================================
 GOKEI-PRINT-SEC            SECTION.
     ...
     PERFORM  GOKEI-PRINTB-SEC.  *> 取引先計
     PERFORM  GOKEI-PRINTB2-SEC. *> 前年同週取引先計
...
*=============================================================
 GOKEI-PRINTB-SEC           SECTION.
     ...
     MOVE  WK-G-NEBSU (1)   TO  PRT2-NEBSU.
...
*=============================================================
 GOKEI-PRINTB2-SEC           SECTION.
     ...
     MOVE  WK-G-NEBSU (2)   TO  PRT2-NEBSU.
