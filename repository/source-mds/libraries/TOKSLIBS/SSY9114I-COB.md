...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY9114I.COB`
...
*画面定義ファイル
     SELECT  DSPFILE   ASSIGN    TO        GS-DSPF
                       ...
                       FILE      STATUS    DSP-ST.
*伝票データワーク(伝票単位順）
     SELECT  KMKEPNL1  ASSIGN    TO        KMKEPNL1
                       ORGANIZATION        INDEXED
                       ACCESS    MODE      RANDOM
                       RECORD    KEY       KPN-F46
                                           KPN-F47
...
*メッセージの取得
 01  ERR-MSG-AREA.
     ...
     03  ERR-MSG9.
         05  FILLER              PIC   N(20)
             VALUE NC"開始センターＣＤを正しく入力して下さい".
     03  ERR-MSG10.
         05  FILLER              PIC   N(20)
             VALUE NC"終了センターＣＤを正しく入力して下さい".
     03  ERR-MSG11.
         05  FILLER              PIC   N(20)
...
*
 ...
 01  PARA-CENT1                  PIC  9(05).
 01  PARA-CENT2                  PIC  9(05).
**************************************************************
 PROCEDURE             DIVISION  USING  PARA-DATE   PARA-TIME
                                        PARA-TORICD PARA-SOKO
                                        PARA-DSOKO
                                        PARA-CENT1  PARA-CENT2.
**************************************************************
 DECLARATIVES.
 DEN-ERR                   SECTION.
 ...
 DSP-ERR                   SECTION.
     ...
     STOP        RUN.
 KPN-ERR                   SECTION.
     USE         AFTER     EXCEPTION PROCEDURE KMKEPNL1.
...
*ファイルのＯＰＥＮ
     OPEN     I-O       DSPFILE   KMKEPNL1.
     OPEN     INPUT     SHTDENF   HTOKMS   ZSOKMS   TENMS1.
*ワークの初期化
     INITIALIZE         FLG-AREA.
*カウンタの初期化
     INITIALIZE         COUNTER.
...
*センターコード範囲チェック
*    （開始センターチェック）
     IF       DSP-CENT1  NOT NUMERIC
     OR       DSP-CENT1  =   ZERO
              MOVE     ZERO  TO   DSP-CENT1
     END-IF.
*    （終了センターコードチェック）
     IF       DSP-CENT2  NOT NUMERIC
     OR       DSP-CENT2  =   ZERO
              MOVE  99999 TO   DSP-CENT2
     END-IF.
*センターコード入力範囲チェック
     IF       DSP-CENT1  >  DSP-CENT2
              MOVE  11       TO   ERR-FLG
              MOVE  "R"      TO   EDIT-OPTION  OF  DSP-CENT1
              MOVE  "R"      TO   EDIT-OPTION  OF  DSP-CENT2
              MOVE  "C"      TO   EDIT-CURSOR  OF  DSP-CENT1
              GO             TO   PARA-CHK-EXIT
     ELSE
              MOVE  "M"      TO   EDIT-OPTION  OF  DSP-CENT2
              MOVE  "M"      TO   EDIT-OPTION  OF  DSP-CENT2
              MOVE  SPACE    TO   EDIT-CURSOR  OF  DSP-CENT1
     END-IF.
*売上伝票存在チェック
...
*実行
         WHEN   "E000"
                MOVE    "3"      TO   PSW
                MOVE    DSP-BTDATE    TO    PARA-DATE
                MOVE    DSP-BTTIME    TO    PARA-TIME
                MOVE    DSP-BTTORI    TO    PARA-TORICD
                MOVE    DSP-SOKCD     TO    PARA-SOKO
                MOVE    DSP-CENT1     TO    PARA-CENT1
                MOVE    DSP-CENT2     TO    PARA-CENT2
*終了
         WHEN   "F005"
                MOVE    "END"    TO   END-FLG
                MOVE    "4010"   TO   PROGRAM-STATUS
...
*倉庫コード　　
     ...
     END-IF.
*センターコード範囲指定
     IF       DEN-F07    >=    DSP-CENT1
     AND      DEN-F07    <=    DSP-CENT2
              CONTINUE
     ELSE
              ADD         1    TO   SKIP-CNT
              GO               TO   DEN-READ-SEC
...
***  開始センターコード
     MOVE    "M"      TO  EDIT-OPTION  OF  DSP-CENT1.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DSP-CENT1.
***  終了センターコード
     MOVE    "M"      TO  EDIT-OPTION  OF  DSP-CENT2.
     MOVE    SPACE    TO  EDIT-CURSOR  OF  DSP-CENT2.
*
 DSP-SYOKI-EXIT.
     EXIT.
*****************<<  SSY9114I   END PROGRAM  >>******************
