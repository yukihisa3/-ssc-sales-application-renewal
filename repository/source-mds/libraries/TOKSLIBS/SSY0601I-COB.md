...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/SSY0601I.COB`
...
*    作成日／更新日　：　92/11/27                              *
*    作成者／更新者　：　NAV                                   *
*    更新日／更新者　：　93/02/23 （メモ_５－１_４ー２）
*                                                              *
*    更新日／更新者　：　08/07/29  NAV/TAKEI                   *
*    修正内容        ：  内部統制対応・担当者
****************************************************************
****************************************************************
...
*----<< 量販データ >>-*
     SELECT   HRYOJNL   ASSIGN    TO        DA-01-VI-RYOJNL1
                        ORGANIZATION        INDEXED
                        ACCESS    MODE      DYNAMIC
...
*----<< 伝票データ >>-*
 FD  HRYOJNL            BLOCK     CONTAINS   4   RECORDS
                        LABEL     RECORD     IS  STANDARD.
     COPY     SHTRYTF   OF   XFDLIB    JOINING   RYO  AS   PREFIX.
*----<< 画面ファイル >>-*
 FD  DSPFILE.
     COPY     FSY06011   OF   XMDLIB.
*--------------------------------------------------------------*
...
*
     ...
     IF       RYO-OPEN-FLG  =  1  CLOSE    HRYOJNL.
...
*----------    伝票データＦ -----------------------------------*
 RYO-ERR                     SECTION.
     USE         AFTER       EXCEPTION PROCEDURE   HRYOJNL.
     ACCEPT      WK-DATE    FROM  DATE.
     ACCEPT      WK-TIME    FROM  TIME.
...
*
     ...
     START    HRYOJNL   KEY  NOT  <  RYO-F011    RYO-F012
                                     RYO-F02
              INVALID   KEY
     ...
     IF  KKNN  =  "B"
         ...
         EVALUATE  R00001
           WHEN  1
              PERFORM  VARYING I FROM 1 BY 1 UNTIL  I > 5
                PERFORM  VARYING  J  FROM  1  BY  1  UNTIL J > 10
                   IF  WK-TENCD(I J) NUMERIC
                   AND WK-TENCD(I J) NOT = ZERO
                       MOVE  WK-TENCD(I J) TO  WK-TENCD-B(CNT)
                   ELSE
                       MOVE  ZERO          TO  WK-TENCD-B(CNT)
                   END-IF
                   IF  WK-SURYO(I J) NUMERIC
                   AND WK-SURYO(I J) NOT = ZERO
                       MOVE  WK-SURYO(I J) TO  WK-TENSU-B(CNT)
                   ELSE
                       MOVE  ZERO          TO  WK-TENSU-B(CNT)
                   END-IF
                   ADD   1             TO  CNT
                END-PERFORM
              END-PERFORM
...
*
*##2008/08/18 初期画面で入力されて担当者ＣＤを更新
*****MOVE     R00002         TO   RYO-F03.
     MOVE     LNK-M-TAN      TO   RYO-F03.
*#2016/06/30 NAV ST 担当者CD予備領域にセット
     MOVE     LNK-M-TAN      TO   RYO-F80(1:2).
*#2016/06/30 NAV ED
     MOVE     R00005         TO   RYO-F04.
     MOVE     R00006         TO   RYO-F05.
*-- 1999/10/05 注文番号更新位置変更
*****MOVE     R00007         TO   RYO-F06.
...
*--
*****COMPUTE  RYO-F071  =  R00008  +  HENKAN-TI * 10000.
*-- 1999/10/05 日付ﾁｪｯｸ変更
***  INITIALIZE                    LINK-YMD-AREA.
     MOVE     "3"            TO   LINK-IN-KBN.
     MOVE     R00009         TO   LINK-IN-YMD6.
...
*--
*****COMPUTE  RYO-F072  =  R00009  +  HENKAN-TI * 10000.
     ...
     PERFORM  VARYING  I  FROM  1  BY  1  UNTIL  I  >  5
         MOVE     BDY (I)    TO   WK-BDY (I)
*********DISPLAY "WK-TOKCD = " WK-TENCD(1 1) UPON CONS
*********DISPLAY "WK-SURYO = " WK-SURYO(1 1) UPON CONS
         PERFORM  VARYING  J  FROM  1  BY  1  UNTIL  J  > 10
              COMPUTE   K  =  (I - 1) * 10 + J
              IF   WK-TENCD (I J)  IS  NUMERIC
...
*--
*****COMPUTE  RYO-F99  =  SYS-DATE  +  HENKAN-TI  *  10000.
     IF       INV-FLG  =  1
              WRITE     RYO-REC
     ELSE
              REWRITE   RYO-REC
...
*#2016/06/30 NAV ST 予備領域の担当者CDをセットする。
     MOVE     RYO-F80(1:2)   TO   R00002.
*#2016/06/30 NAV ED
     MOVE     RYO-F04        TO   R00005.
     MOVE     RYO-F05        TO   R00006.
*-- 1999/10/05 ７桁注文番号から１５桁注文番号へ
*****MOVE     RYO-F06        TO   R00007.
