...
**種別**: COBOL プログラム  
**ライブラリ**: TOKSLIBS  
**ソースファイル**: `source/navs/cobol/programs/TOKSLIBS/NJH2301B.COB`
...
*    ｶｳﾝﾄ
 ...
 01  WK-GYOCNT               PIC  9(02)     VALUE  ZERO.
 01  WK-TOKCD                PIC  9(08)     VALUE  ZERO.
 01  BRK-TORICD              PIC  9(06)     VALUE  ZERO.
 01  CUR-TORICD              PIC  9(06)     VALUE  ZERO.
*
*    データ情報退避ワーク
 01  WK-FL-REC.
     03  WK-FL01                  PIC       X(02).
...
*    ヘッドレコード退避ワーク
 01  WK-DH-REC.
     ...
     03  WK-DH14                  PIC       X(20).
     03  WK-DH15                  PIC       X(01).
     03  WK-DH151                 PIC       X(01).
     03  WK-DH152                 PIC       X(05).
     03  WK-DH16                  PIC       X(08).
     03  WK-DH17                  PIC       9(08).
     03  WK-DH18                  PIC       9(01).
     03  WK-DH19                  PIC       9(06).
...
*    計算項目
 01  CNT-KEISAN.
     03  WK-HEN-A23         PIC   9(09)  VALUE ZERO.
     03  WK-HEN-A25         PIC   9(09)  VALUE ZERO.
 01  MSG-AREA.
     03  MSG-START.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  ST-PG          PIC   X(08)  VALUE "NJH2301B".
         05  FILLER         PIC   X(11)  VALUE
                                         " START *** ".
     03  MSG-END.
         05  FILLER         PIC   X(05)  VALUE " *** ".
         05  END-PG         PIC   X(08)  VALUE "NJH2301B".
         05  FILLER         PIC   X(11)  VALUE
                                         " END   *** ".
     03  MSG-ABEND.
         05  FILLER         PIC   X(05)  VALUE " *** ".
...
*指定日付後，日付算出
 01  LINK-SYSDATE.
     03  L1-SYS-Y          PIC 9(02).
     03  L1-SYS-M          PIC 9(02).
     03  L1-SYS-D          PIC 9(02).
 01  LINK-HIZUKE.
     03  L1-HIZUKE         PIC 9(02).
 01  LINK-HENKAN-DATE.
     03  L1-HEN-Y          PIC 9(02).
     03  L1-HEN-M          PIC 9(02).
     03  L1-HEN-D          PIC 9(02).
 01  LINK-RET.
     03  L1-RET            PIC 9(01).
*
 01  LINK-AREA.
...
*
 FILEERR-SEC9           SECTION.
     USE       AFTER    EXCEPTION
                        PROCEDURE   MKESHIRE.
     MOVE      "MKESHIRE"   TO   AB-FILE.
     MOVE      MKE-STATUS   TO   AB-STS.
     DISPLAY   MSG-ABEND         UPON CONS.
     DISPLAY   SEC-NAME          UPON CONS.
     DISPLAY   ABEND-FILE        UPON CONS.
     MOVE      4000         TO   PROGRAM-STATUS.
...
****************************************************************
 INIT-SEC               SECTION.
     ...
     OPEN     I-O       JHMKENL1  JSMDAYL1.
     OPEN     OUTPUT    MKESHIRE  VLD500.
     DISPLAY  MSG-START UPON CONS.
*
     MOVE     ZERO      TO        END-FG    RD-CNT    WRT-CNT.
     MOVE     ZERO      TO        IN-CNT    OUT-CNT.
     MOVE     SPACE     TO        WK-FL-REC.
     INITIALIZE                   WK-FL-REC.
     MOVE     SPACE     TO        WK-DH-REC.
     INITIALIZE                   WK-DH-REC.
     MOVE     SPACE     TO        WK-DD-REC.
     INITIALIZE                   WK-DD-REC.
*
******************
*システム日付編集*
******************
...
*             出荷場所件数マスタ出力
              ...
              MOVE      SPACE         TO    WK-DH-REC
              INITIALIZE                    WK-DH-REC
              MOVE      DEN-01(IDX)   TO    WK-DH-REC
*             得意先コードブレイク時スケジュールマスタ出力
              IF   CNT-MAISU     =    ZERO
                   IF   WK-DH07  =    "002320"
                        MOVE     2320       TO  BRK-TORICD
                   ELSE
                        MOVE     110020     TO  BRK-TORICD
                   END-IF
              ELSE
                   IF   WK-DH07  =    "002320"
                        MOVE     2320       TO  CUR-TORICD
                   ELSE
                        MOVE     110020     TO  CUR-TORICD
                   END-IF
                   IF   CUR-TORICD    NOT =     BRK-TORICD
                        MOVE     BRK-TORICD TO  HEN-A01
*                       当日スケジュールマスタ出力
                        PERFORM  JSMDAYL1-WRT-SEC
                        MOVE     CUR-TORICD TO  BRK-TORICD
                   END-IF
              END-IF
              ADD       1        TO   CNT-MAISU
...
*各量販店領域転送
*    取引先コード
     MOVE     WK-DH07       TO        HEN-A01.
*    伝票_
     MOVE     WK-DH06       TO        HEN-A02.
*    行_
     MOVE     WK-GYOCNT     TO        HEN-A03.
*    部門（Ｈ）
     MOVE     WK-DH03       TO        HEN-A04.
*    伝票区分
     MOVE     WK-DH15       TO        HEN-A05.
*    発注者名（社名）
     MOVE     WK-FL08       TO        HEN-A06.
*    取引先名称（１～４０桁まで）
     MOVE     WK-DH13       TO        HEN-A071.
     MOVE     WK-DH14       TO        HEN-A072.
*    発注日
     MOVE     WK-DH04       TO        HEN-A08.
*    納品日
     MOVE     WK-DH05       TO        HEN-A09.
*    便
*****MOVE     XXXXXXX       TO        HEN-A10.
*    仕入申請書_
     MOVE     WK-DH09       TO        HEN-A12.
*    発注企業名称
     MOVE     WK-FL09       TO        HEN-A111.
     MOVE     WK-FL10       TO        HEN-A112.
*    ルート
     MOVE     WK-DH08       TO        HEN-A14.
*    商品コード
     MOVE     WK-DD04       TO        HEN-A15.
*    品名
     MOVE     WK-DD02       TO        HEN-A161.
*    規格
     MOVE     WK-DD03       TO        HEN-A162.
*    取引先品番
     MOVE     WK-DD05       TO        HEN-A17.
*    ＪＡＮコード
     MOVE     WK-DD05       TO        HEN-A18.
*    入数
     MOVE     WK-DD06       TO        HEN-A19.
*    ケース数
     MOVE     WK-DD07       TO        HEN-A20.
*    数量
     MOVE     WK-DD08       TO        HEN-A21.
*    原単価
     MOVE     WK-DD09       TO        HEN-A22.
*    原価金額
     COMPUTE  WK-HEN-A23  =  WK-DD08  *  WK-DD09.
     MOVE     WK-HEN-A23    TO        HEN-A23.
*    売単価
     MOVE     WK-DD10       TO        HEN-A24.
*    売価金額
     COMPUTE  WK-HEN-A25  =  WK-DD08  *  WK-DD10.
     MOVE     WK-HEN-A25    TO        HEN-A25.
*    店舗コード
     MOVE     WK-DH02       TO        HEN-A26.
*    伝票枚数
*****MOVE     ZERO          TO        HEN-A27.
*    店舗名
     MOVE     WK-DH12       TO        HEN-A28.
*    Ａ欄
     MOVE     WK-DH10       TO        HEN-A29.
*    Ｂ欄
     MOVE     WK-DH11       TO        HEN-A30.
*
     PERFORM  TENSO-SEC.
*
     WRITE    HEN-REC.
...
*<98/02/20 変更 START >
     IF  HEN-A01  =  "002320"
         MOVE    2320      TO        HEN-F01
     ELSE
         MOVE    110020    TO        HEN-F01
     END-IF.
*<98/02/20 変更 END   >
*****MOVE        2320      TO        HEN-F01.
*伝票ナンバー
     MOVE        HEN-A02   TO        HEN-F02.
     MOVE        HEN-F02   TO        HEN-F23.
*行_
     MOVE        HEN-A03   TO        HEN-F03.
*取区
     MOVE        40        TO        HEN-F051.
...
*担当者コード
     MOVE        99        TO        HEN-F06.
*店コード
     MOVE        HEN-A26   TO        HEN-F07.
*  商品変換テーブル検索
     MOVE        SPACE     TO        TBL-REC.
     INITIALIZE                      TBL-REC.
     MOVE        HEN-F01   TO        TBL-F01.
*****MOVE        2320      TO        TBL-F01.
     MOVE        HEN-A18   TO        TBL-F02.
     READ    SHOTBL1
       INVALID
         MOVE    SPACE     TO        TBL-REC
         INITIALIZE                  TBL-REC
...
*納品日がゼロの時
     IF   HEN-A09  =  ZERO
          MOVE   SYS-DATEW(3:6)  TO   LINK-SYSDATE
          MOVE   02              TO   L1-HIZUKE
          CALL   "SKTADDDT"     USING LINK-SYSDATE
                                      LINK-HIZUKE
                                      LINK-HENKAN-DATE
                                      LINK-RET
          IF     LINK-RET  =  0
                 MOVE "3"         TO       LINK-IN-KBN
                 MOVE  LINK-HENKAN-DATE    TO     LINK-IN-YMD6
                 CALL "SKYDTCKB" USING     LINK-IN-KBN
                                           LINK-IN-YMD6
                                           LINK-IN-YMD8
                                           LINK-OUT-RET
...
*    商品名称マスタ検索
     ...
     READ    MEIMS1
       INVALID
*        商品名
         MOVE    HEN-A161(1:15)  TO  HEN-F1421
         MOVE    HEN-A161(16:5)  TO  HEN-F1422(1:5)
         MOVE    HEN-A162(1:10)  TO  HEN-F1422(6:10)
       NOT INVALID
         MOVE    HEN-A161(1:15)  TO  HEN-F1421
         MOVE    HEN-A161(16:5)  TO  HEN-F1422(1:5)
         MOVE    HEN-A162(1:10)  TO  HEN-F1422(6:10)
     END-READ.
*数量
     MOVE        HEN-A21   TO        HEN-F15.
*単
     MOVE       "1"        TO        HEN-F16.
*原価単価
     MOVE        HEN-A22   TO        HEN-F172.
*売価単価
     MOVE        HEN-A24   TO        HEN-F173.
*原価金額
     MOVE        HEN-A23   TO        HEN-F181.
*売価金額
     MOVE        HEN-A25   TO        HEN-F182.
*自社得意先コード
     IF  HEN-F01   NOT =     WK-TOKCD
*    得意先マスタ検索
         MOVE    SPACE         TO    TOK-REC
         INITIALIZE                  TOK-REC
         MOVE    HEN-A01       TO    TOK-F01
         READ    TOKMS2
             INVALID
               MOVE  SPACE     TO    TOK-REC
               INITIALIZE            TOK-REC
...
*****MOVE     HEN-F01      TO        RUT-F01.
     MOVE     2320         TO        RUT-F01.
     MOVE     SPACE        TO        RUT-F02.
     MOVE     HEN-A14      TO        RUT-F03  HEN-F42.
     READ     JHMRUTL1
         INVALID
           MOVE  1         TO        INV-RUT
...
*訂正前数量
     MOVE        HEN-A21   TO        HEN-F50.
*修正原価単価
     MOVE        HEN-A22   TO        HEN-F512.
*修正売価単価
     MOVE        HEN-A24   TO        HEN-F513.
*修正原価金額
     MOVE        HEN-A23   TO        HEN-F521.
*修正売価金額
     MOVE        HEN-A25   TO        HEN-F522.
*
 TENSO-EXIT.
     EXIT.
****************************************************************
*             検収データ出力処理                               *
****************************************************************
 HENSYU-JYURYO-SEC SECTION.
*
     MOVE    "HENSYU-JYURYO-SEC"      TO    S-NAME.
*検収データファイルの初期化
     MOVE     SPACE         TO        MKE-REC.
     INITIALIZE                       MKE-REC.
*    取引先コード
     MOVE     WK-DH07       TO        MKE01.
*    伝票_
     MOVE     WK-DH06       TO        MKE02.
*    行_
     MOVE     WK-GYOCNT     TO        MKE03.
*    部門（Ｈ）
     MOVE     WK-DH03       TO        MKE04.
*    伝票区分
     MOVE     WK-DH15       TO        MKE05.
*    発注者名（社名）
     MOVE     WK-FL08       TO        MKE06.
*    取引先名称（１～４０桁まで）
     MOVE     WK-DH13       TO        MKE071.
     MOVE     WK-DH14       TO        MKE072.
*    発注日
     MOVE     WK-DH04       TO        MKE08.
*    納品日
     MOVE     WK-DH05       TO        MKE09.
*    便
*****MOVE     XXXXXXX       TO        MKE10.
*    仕入申請書_
     MOVE     WK-DH09       TO        MKE12.
*    発注企業名称
     MOVE     WK-FL09       TO        MKE111.
     MOVE     WK-FL10       TO        MKE112.
*    ルート
     MOVE     WK-DH08       TO        MKE14.
*    商品コード
     MOVE     WK-DD04       TO        MKE15.
*    品名
     MOVE     WK-DD02       TO        MKE161.
*    規格
     MOVE     WK-DD03       TO        MKE162.
*    取引先品番
     MOVE     WK-DD05       TO        MKE17.
*    ＪＡＮコード
     MOVE     WK-DD05       TO        MKE18.
*    入数
     MOVE     WK-DD06       TO        MKE19.
*    ケース数
     MOVE     WK-DD07       TO        MKE20.
*    数量
     MOVE     WK-DD08       TO        MKE21.
*    原単価
     MOVE     WK-DD09       TO        MKE22.
*    原価金額
     COMPUTE  WK-HEN-A23  =  WK-DD08  *  WK-DD09.
     MOVE     WK-HEN-A23    TO        MKE23.
*    売単価
     MOVE     WK-DD10       TO        MKE24.
*    売価金額
     COMPUTE  WK-HEN-A25  =  WK-DD08  *  WK-DD10.
     MOVE     WK-HEN-A25    TO        MKE25.
*    店舗コード
     MOVE     WK-DH02       TO        MKE26.
*    伝票枚数
*****MOVE     XXXXXXX       TO        MKE27.
*    店舗名
     MOVE     WK-DH12       TO        MKE28.
*    Ａ欄
     MOVE     WK-DH10       TO        MKE29.
*    Ｂ欄
     MOVE     WK-DH11       TO        MKE30.
*    ヘッダ伝票区分
     MOVE     WK-DH151      TO        MKE31.
*    明細伝票区分
     MOVE     WK-DD11       TO        MKE32.
*    理由
     MOVE     WK-DD12       TO        MKE33.
*    伝票データファイルＷＲＩＴＥ
     WRITE    MKE-REC.
*
 HENSYU-JYURYO-EXIT.
...
*****MOVE    HEN-A01       TO        KEN-F03.
     MOVE    2320          TO        KEN-F03.
     IF      INV-RUT       =         ZERO
...
*****MOVE    HEN-A01       TO        TJS-F03.
     MOVE    2320          TO        TJS-F03.
     READ    JSMDAYL1
       INVALID
         CONTINUE
       NOT INVALID
...
*****MOVE      HEN-A01            TO    VLD-F13.
     MOVE      2320               TO    VLD-F13.
     DISPLAY "VLD-F11    = " VLD-F11    UPON CONS.
     DISPLAY "VLD-F12    = " VLD-F12    UPON CONS.
     DISPLAY "VLD-F13    = " VLD-F13    UPON CONS.
     WRITE     VLD-REC.
*
 VLD500-OUTPUT-EXIT.
     EXIT.
